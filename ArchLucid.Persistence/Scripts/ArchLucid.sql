...
/* TB-310 / ADR 0045: committed run header evidence-anchor immutability (lifecycle columns remain mutable). */
IF OBJECT_ID(N'dbo.Runs', N'U') IS NOT NULL
BEGIN
    EXEC(N'
...
/* Analytics non-clustered columnstore indexes (Improvement #36). ONLINE omitted for local Standard/Express. */
IF OBJECT_ID(N'dbo.AuditEvents', N'U') IS NOT NULL
   AND NOT EXISTS (
       SELECT 1
       FROM sys.indexes
...
/*
  402: Add Azure Data Collection Rules to the database-backed never-show catalog.
*/
...
/*
  403: Add App Service Plans and Enterprise Policies to the database-backed
  never-show catalog.
*/
