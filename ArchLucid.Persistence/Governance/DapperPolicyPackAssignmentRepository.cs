...
/// <summary>
///     SQL Server implementation of <see cref="IPolicyPackAssignmentRepository" /> using Dapper against
///     <c>dbo.PolicyPackAssignments</c>.
/// </summary>
/// <remarks>
///     <para>
///         <strong>List semantics:</strong> Returns tenant-wide rows, workspace rows matching the caller's workspace id,
///         and project rows
...
///     </para>
///     <para>
///         <strong>Callers:</strong> <see cref="PolicyPackResolver" /> and <see cref="EffectiveGovernanceResolver" /> via
///         DI;
    ...
    : IPolicyPackAssignmentRepository
{
    ...
    /// <inheritdoc />
    /// <remarks>
    ///     Does not filter <c>IsEnabled</c> in SQL—callers filter so disabled rows can be listed by future admin APIs if
    ///     needed.
    ...
    /// <inheritdoc />
    public async Task<PolicyPackAssignment?> GetByTenantAndAssignmentIdAsync(Guid tenantId, Guid assignmentId,
        CancellationToken ct)
    {
        const string sql = """
                           SELECT TOP 1
        ...
        PolicyPackAssignment? row = await connection.QuerySingleOrDefaultAsync<PolicyPackAssignment>(
            new CommandDefinition(sql, new { TenantId = tenantId, AssignmentId = assignmentId },
                cancellationToken: ct));
