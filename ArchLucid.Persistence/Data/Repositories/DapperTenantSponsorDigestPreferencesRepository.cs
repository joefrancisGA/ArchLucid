...
/// <inheritdoc cref="ITenantSponsorDigestPreferencesRepository" />
[ExcludeFromCodeCoverage(Justification = "SQL-dependent repository; exercised via ArchLucid.sql / DbUp.")]
public sealed class DapperTenantSponsorDigestPreferencesRepository(ISqlConnectionFactory connectionFactory)
    : ITenantSponsorDigestPreferencesRepository
{
    private readonly ISqlConnectionFactory _connectionFactory =
        connectionFactory ?? throw new ArgumentNullException(nameof(connectionFactory));

    /// <inheritdoc />
    public async Task<SponsorDigestPreferencesResponse?> GetByTenantAsync(Guid tenantId,
        CancellationToken cancellationToken)
    {
        const string sql = """
                           SELECT
                               ...
                               HourOfDay,
                               UpdatedUtc
                           FROM dbo.TenantSponsorDigestPreferences
                           WHERE TenantId = @TenantId;
                           """;

        await using SqlConnection connection = await _connectionFactory.CreateOpenConnectionAsync(cancellationToken);
        TenantSponsorDigestPreferencesRow? row = await connection.QueryFirstOrDefaultAsync<TenantSponsorDigestPreferencesRow>(
            new CommandDefinition(sql, new { TenantId = tenantId }, cancellationToken: cancellationToken));

        return row is null ? null : SponsorDigestPreferencesMapper.ToResponse(row);
    }

    /// <inheritdoc />
    public async Task<SponsorDigestPreferencesResponse?> UpsertAsync(
        Guid tenantId,
        bool emailEnabled,
        ...
        CancellationToken cancellationToken)
    {
        ...
        string emails = SponsorDigestPreferencesMapper.SerializeEmails(recipientEmails);

        const string mergeSql = """
                                MERGE dbo.TenantSponsorDigestPreferences AS t
                                USING (
                                    SELECT
    ...
    public async Task<IReadOnlyList<Guid>> ListEmailEnabledTenantIdsAsync(CancellationToken cancellationToken)
    {
        const string sql = """
                           SELECT TenantId
                           FROM dbo.TenantSponsorDigestPreferences WITH (NOLOCK)
    ...
    {
        const string sql = """
                           UPDATE dbo.TenantSponsorDigestPreferences
                           SET EmailEnabled = 0,
                               UpdatedUtc = SYSUTCDATETIME()
                           WHERE TenantId = @TenantId
                             AND EmailEnabled = 1;
