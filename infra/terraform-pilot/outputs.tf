output "apply_order_doc" {
  value       = "See docs/library/REFERENCE_SAAS_STACK_ORDER.md and docs/deployment/PILOT_PROFILE.md"
...
output "composition_roots" {
  value       = local.composition_roots
  description = "Metadata composition roots (foundation / platform / app). Validated, never Azure-applied."
}

output "nested_infrastructure_roots" {
  value       = local.nested_infrastructure_roots
  description = "Ordered leaf stacks for the opt-in multi-root path (paths relative to repository root)."
}
