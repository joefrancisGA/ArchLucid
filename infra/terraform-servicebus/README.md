...
- IAM: optional role assignments grant **Data Sender** to the API identity and **Data Sender + Data Receiver** to the worker (outbox drain + subscription consumer). When a Logic App–scoped subscription is enabled, pass the matching **`…_logic_app_managed_identity_principal_id`** from outputs of **`infra/terraform-logicapps/`** after deploying the optional dedicated host: **governance** → **`governance_logic_app_principal_id`**; **Marketplace** → **`marketplace_fulfillment_logic_app_principal_id`**; **trial lifecycle** → **`trial_lifecycle_logic_app_principal_id`**; **incident ChatOps** → **`incident_chatops_logic_app_principal_id`**; **promotion customer notify** → **`promotion_customer_notify_logic_app_principal_id`** (namespace **Data Receiver** scope today).
