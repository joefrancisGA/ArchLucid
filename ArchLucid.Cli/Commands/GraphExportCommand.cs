...
internal static class GraphExportCommand
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        using (HttpClient http = ArchLucidApiClient.CreateSharedApiHttpClient(baseUrl, config))
        {
            ...
            {
                await Console.Error.WriteLineAsync(
                    $"{(int)response.StatusCode} {response.StatusCode}: {TrimPreview(bodyJson, 480)}");
...

    {
        ...
        return mapped ?? WarnEmpty($"Graph body deserialized to null ({response.RequestMessage?.RequestUri}).");
    }
