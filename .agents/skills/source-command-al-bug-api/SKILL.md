---
name: "source-command-al-bug-api"
description: "Launch a Cloud Agent to run the /al-bug hunt-fix-ship workflow on master"
---

# source-command-al-bug-api

Use this skill when the user asks to run the migrated source command `al-bug-api`.

## Command Template

# /al-bug-api — Cloud Agent bug hunt (`/al-bug` via API)

Starts a **Codex Cloud Agent** (Composer 2.5 standard, not Fast) that follows the full **`/al-bug`** workflow in `.cursor/commands/al-bug.md`. The local chat agent does **not** hunt bugs — it only launches the cloud run and returns the agent URL.
