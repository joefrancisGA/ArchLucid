...
public sealed class SimulationEngine(IArchitectureAnalysisService analysisService) : ISimulationEngine
{
    ...
    public async Task<SimulationResult> SimulateAsync(SimulationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        SimulationReadProfile baselineProfile =
            request.Options?.BaselineReadProfile ?? SimulationReadProfile.StrictReadOnly;

        SimulationReadProfile simulatedProfile =
            request.Options?.SimulatedReadProfile ?? baselineProfile;

        bool singlePass = baselineProfile.Equals(simulatedProfile);

        ArchitectureAnalysisReport baselineReport =
            await analysisService.BuildAsync(
                ToAnalysisRequest(request.BaselineArchitectureRunId, baselineProfile),
                cancellationToken);
        ...
        SimulationDiff diff = BuildDiff(
            ...
            request.BaselineArchitectureRunId,
            singlePass,
            baselineReport,
            simulatedReport);
    ...
    private static SimulationArtifactsSnapshot BuildArtifacts(
        ...
        ManifestDiffResult? manifestDiff)
    {
        ...
        string manifestLine = manifestDiff is null
            ...
            : string.Format(
                CultureInfo.InvariantCulture,
                "Manifest diff: +{0}/-{1} services, +{2}/-{3} datastores.",
        ...
        return string.Format(
            CultureInfo.InvariantCulture,
            "Baseline warnings={0}, simulated warnings={1}. {2}",
            baseline.Warnings.Count,
            simulated.Warnings.Count,
            manifestLine);
    }

    private static EvaluationScore BuildScores(
        ArchitectureAnalysisReport baseline,
        ArchitectureAnalysisReport simulated,
        bool singlePass)
    {
    ...
    private static double ComputeSimulationQualityScore(
        ArchitectureAnalysisReport baseline,
        ArchitectureAnalysisReport simulated,
        bool singlePass)
    {
    ...
    private sealed record SimulationDiffDetailDto(
        ...
        int BaselineSummaryLength,
        int SimulatedSummaryLength,
        bool HadManifestDiff,
        int? ManifestAddedServices,
        int? ManifestRemovedServices,
        int? ManifestAddedDatastores,
        int? ManifestRemovedDatastores);
}
