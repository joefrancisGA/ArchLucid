...
using DrParagraph = DocumentFormat.OpenXml.Drawing.Paragraph;
...
using DvNonVisualGraphicFrameDrawingProperties = DocumentFormat.OpenXml.Drawing.NonVisualGraphicFrameDrawingProperties;
...
using DrPicShapeProperties = DocumentFormat.OpenXml.Drawing.Pictures.ShapeProperties;
    ...
    : IArchitectureAnalysisConsultingDocxExportService
{
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            true))
        {
            ...
            await AddCoverPageAsync(mainPart, body, report, options, cancellationToken);
            AddPageBreak(body);
            ...
            {
                await AddArchitectureOverviewAsync(body, mainPart, report, options, cancellationToken);
            }
    ...
    private async Task AddCoverPageAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var logoBytes = await _logoProvider.GetLogoBytesAsync(options, cancellationToken);
    ...
    private async Task AddArchitectureOverviewAsync(
        Body body,
        MainDocumentPart mainPart,
        ArchitectureAnalysisReport report,
        ConsultingDocxTemplateOptions options,
        CancellationToken cancellationToken)
    {
        ...
        {
            var imageBytes = await _diagramImageRenderer.RenderMermaidPngAsync(
                report.Diagram,
                cancellationToken);
        ...
        ConsultingDocxTemplateOptions options)
    {
        ...
        foreach (var group in grouped)
        {
            ...
            AddBullet(body, $"Latest Parse Success: {(group.OrderByDescending(x => x.CreatedUtc).First().ParseSucceeded ? "Yes" : "No")}");
        }
