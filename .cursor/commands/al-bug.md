...
Optional: `python3 scripts/agent/al-bug-seed-from-flake-log.py --preview` for tests that flaked ≥ 3 times in 30 days. Flakes are not hunt-ready (slow ≠ racy).

Optional: for API zones, run `dotnet test ArchLucid.Api.Tests/ArchLucid.Api.Tests.csproj --filter FullyQualifiedName~SchemaAuthz`. Paste catalog failures as `(candidate) [class:authz-scope]` — still not hunt-ready without reachability. This is a contract catalog, not a pen test.
...
6. The shipped test must **fail** if the production hunk alone is reverted (revert-to-fail honesty). Owners may batch-check with `python3 scripts/agent/al-bug-verify-proven-revert.py`. New unguarded `(proven)` rows fail the ABQ-34 ratchet vs `scripts/ci/al-bug-unguarded-proven-baseline.json` (`--fail-on-new-unguarded`); do not mass-retick historical rows.
