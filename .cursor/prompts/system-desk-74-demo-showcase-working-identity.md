# SY-74 — Working demo/showcase identity is an architecture name, not a showcase run id as Home

Do not fork live no-sample recovery. Demo mode may keep sample **content**; Working chrome must still present it as an architecture.

## Goal

Showcase Working entry: architecture identity path (even if backed by sample data), labeled demo/sample per claim discipline. Do not land Working demo on `/architecture/reviews/{SHOWCASE_…}` as Home.

## Why

Demo is what buyers remember. A run URL demo sells a pipeline.
...
1. Working demo entry helper + test.
2. Do not claim sample data is a real sealed record.

## Acceptance criteria

- Working demo Home/start is not a peer showcase run URL.
