# IH-047 — Sketch a change from the findings document

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** exploration. **Depends on:** IH-008, CE-001 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working nested findings chrome includes the existing Sketch a change entry (CE). Defaults Practice. Do not remount a second runner.

## Why

Exploration that requires leaving the document is not inhabited thought.

## Context

- `cheap-exploration-envelope-runner-entry.ts` · SG-116 skip
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Mount CE Sketch CTA on Working nested findings / desk command bar. Confirm cost-cap as CE already does.
2. Do not remount CE-001 runner. Do not draft-diff.
