# SG-077 — In-flight job/envelope on the desk leftover

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** desk. **Depends on:** SG-018, CE-029 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

In-flight child jobs (Record run or Practice envelope) show on the architecture desk. Wait is background (0096). Do not re-run DW.

## Why

In-flight that only lives on review-detail makes wait the job.
