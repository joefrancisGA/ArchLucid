# IH-024 — Nested review-detail back is the architecture leftover

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-015, SG-026 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If the operator opens the inspector, Back returns to the architecture (desk or nested findings), not Home or the reviews inbox as Monday.

## Why

SG-026 shipped the rule. Inhabit leftovers are remaining back-hrefs.

## Context

- `working-back-href` · LY-024 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Inventory remaining Back / breadcrumb substitutes on nested review that do not return to the architecture.
2. No system-wide breadcrumbs (TB-2090).
