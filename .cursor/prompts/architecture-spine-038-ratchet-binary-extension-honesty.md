# AS-038 — CI ratchet: new binary extension needs extract or honesty row

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-ratchet. **Depends on:** AS-002, AS-031.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Guard: wizard-accepted extensions in the intake accept-list must appear in AS-002 inventory as analyze, store-honesty, or explicitly unsupported. New `.foo` without a row fails CI.

## Why

Someone will add webp “because browsers” and drop it from decide again.

## Context

- AS-002 inventory
- First-pilot accept attribute / file input

## What to build

1. Vitest or Python guard comparing accept-list to inventory.
2. Document how to add a row.

## Acceptance criteria

- CI fails on uninventoried accepted extensions.
