# AS-048 — API attach/detach inventory snapshot on an architecture

**Wave:** architecture-spine (**AS**). **Cluster:** inventory-bind. **Depends on:** AS-047.

Do not implement from the wave index. Implement only *What to build*.

## Goal

HTTP bind/unbind with ExecuteAuthority (or existing architecture admin). OpenAPI. 404 on missing architecture. Do not fetch Azure.

## Why

UI cannot use SQL directly.

## Context

- OPENAPI_CONTRACT_DRIFT.md
- Architecture routes under /v1/architecture

## What to build

1. POST/DELETE + GET current binding.
2. Generated TS.
3. API tests.
