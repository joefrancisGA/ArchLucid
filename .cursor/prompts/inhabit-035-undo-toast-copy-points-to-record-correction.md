# IH-035 — Undo toast copy points to Record correction after the window

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** reversibility. **Depends on:** IH-033, LY-051 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

The 300s undo toast on Working finding mutations says the window is short and Record correction remains on the row. The toast is not authority.

## Why

Architects treat Undo like Excel. The copy must not overclaim.

## Context

- `ReversibleMutationSuccessCallout.tsx` · LY-052 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Toast copy: visible undo for five minutes; then amend on the finding. Do not change `MUTATION_UNDO_WINDOW_SECONDS`.
