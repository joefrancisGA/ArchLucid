# IP-008 — Finding inspect support band threads Simulator mode

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** completeness. **Depends on:** IP-001, IH-043.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

Finding-inspect header, body, and inspect-section `FindingSemanticSupportBandChip` receive `structuralExecutionMode` (run or host) so Simulator cannot render green Career **Supported**.

## Why

Inhabited rows and peer table remap via `presentDecisionGradeSemanticSupportBand`. Inspect (`FindingDetailHeader.tsx`, `FindingDetailInspectBody.tsx`, `FindingSemanticSupportBandInspectSection.tsx`) omit mode. A livelihood inspect still looks Career-complete on Simulator.

## Context

- Inspect files above · `finding-detail-page-model.ts` · `FindingSemanticSupportBandChip` · `GovernanceFindingRow` pattern (`useAgentExecutionMode` / run mode)

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Thread `structuralExecutionMode` from the run (preferred) or host `useAgentExecutionMode()` into every inspect chip mount. Reuse `presentDecisionGradeSemanticSupportBand`. Do not flip G-REAL-06. Do not enable emit LLM judge.
2. Do not delete the inspect route. Guided inspect may keep the same honesty (Simulator remap is execution-mode, not Working-only).
3. Flip inspect support-band inventory. Owner `IP-008`. Focused Vitest: inspect chip with Simulator + Supported → Practice / not-record copy, not green Supported.
