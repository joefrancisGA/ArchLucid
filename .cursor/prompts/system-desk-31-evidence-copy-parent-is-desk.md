# SY-31 — Related-links / evidence-copy parents point at the architecture desk on Working

Do not fork TB-2048 Learn more job-match. Parent hrefs only.

## Goal

Working evidence-copy arrays that use `{ label: "Architecture reviews", href: REVIEWS_LIST_PATH }` become architecture portfolio or “Inbox” (explicit). Ask/graph/intelligence copy included.

## Why

Related links are how people climb out of a tool. Climbing to the reviews hub re-homes them on jobs.

## Context

- `ask-review-questions-evidence-copy.ts`
- `governance-findings-evidence-copy.ts`
- `evidence-graph-evidence-copy.ts`
- `architecture-intelligence-evidence-copy.ts`

## What to build

1. Working vs Guided copy resolvers if needed.
2. Vitest snapshots/strings.

## Acceptance criteria

- Working Ask related-links do not use Reviews hub as the parent object.
