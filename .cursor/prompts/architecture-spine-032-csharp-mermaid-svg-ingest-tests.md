# AS-032 — C# ingest tests for mermaid and sanitized SVG

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-ratchet. **Depends on:** AS-007, AS-008.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Scoped tests in ContextIngestion or Decisioning test project: mermaid fixture; SVG with script rejected/stripped; graph node count.

## Why

TS-only tests will not catch server compile path regressions.

## Context

- ArchLucid.ContextIngestion.Tests or equivalent
- agent-compile-check on the test csproj

## What to build

1. Add fixtures + tests.
2. One scoped compile.

## Acceptance criteria

- dotnet test filter on new tests green.
