# IH-011 — Inventory: recents, pins, and finding selection are not inhabited continuity
...
Inventory last-open, recents, pins, and selected finding that are tab-local, run-id shaped, or missing from the account.

## Why

An all-day seat must reopen the system, not a random inspector URL.

## Context

- SG-088 leftover · LY-093 leftover · PT-14 · ADR 0090 lease
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Table: last-open key, recents widget, pins store, finding selection persist (URL vs prefs vs memory).
2. Do not invent presence. Server sync leftover is IH-066.
