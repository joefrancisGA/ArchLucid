# SG-015 — Compat matrix: AO/SY/SN done-tests vs remaining instrument leaks

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** contract. **Depends on:** SG-004–SG-014.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Publish `docs/architecture/SYSTEM_GRAVITY_COMPAT_MATRIX.md`: each AO-50 / SY-80 / SN-040 done test vs whether the **instrument** (not locator) still fails 0098.

## Why

Locator ratchets can be green while the architect still lives in the job.

## Context

- SYSTEM_DESK_ACCEPTANCE · SYSTEM_NOT_JOB_ACCEPTANCE · architecture-object-acceptance-guard · inventories SG-004–014

## What to build

1. Table: done test, still-green?, instrument leftover (yes/no), owning SG prompt.
2. Vitest that the matrix file exists and names ADR 0098.
3. Do not weaken AO-50 / SY-80 / SN-040 tests.
