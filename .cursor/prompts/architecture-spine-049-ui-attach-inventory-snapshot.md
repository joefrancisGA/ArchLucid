# AS-049 — Working desk: attach inventory snapshot control

**Wave:** architecture-spine (**AS**). **Cluster:** inventory-bind. **Depends on:** AS-048.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working architecture desk control to bind a snapshot id the tenant already has (picker or paste). Carbon. Disable until valid (TB-2005). Guided: hide or eval copy, not the instrument picker if snapshots are operator-only.

## Why

All-day: bind the estate to this architecture.

## Context

- ADR 0080 Working never buyer-polished
- AS-048
...
1. Form + last-saved.
2. Vitest.
3. Do not build a new Azure login wizard (IE owns collector auth).

## Acceptance criteria

- Operator can bind without leaving the architecture locator.
