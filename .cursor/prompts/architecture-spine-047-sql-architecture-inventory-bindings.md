...
**Wave:** architecture-spine (**AS**). **Cluster:** inventory-bind. **Depends on:** AS-046.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Table: architecture id, snapshot id, boundBy, boundUtc, rowversion. Tenant catalog only. Numbered migration **and** `ArchLucid.Persistence/Scripts/ArchLucid.sql` (+ unified schema if still mirrored). Rollback script if peers have one.

## Why

Need a durable bind the desk can show after refresh.

## Context

- Single-DDL user rule
- ADR 0037 no RLS

## What to build

1. DDL + repo + tests that script contains the table.
2. FK only to tables that exist; if snapshot table is not shipped, bind to a documented opaque snapshot id + honesty (do not create the IE snapshot table here).

## Acceptance criteria

- Unified SQL file updated.
- No RLS.
