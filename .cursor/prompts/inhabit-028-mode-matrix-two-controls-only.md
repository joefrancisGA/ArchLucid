# IH-028 — Working shows two customer controls, not a mode matrix

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** default-day. **Depends on:** IH-001, MG leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working production chrome exposes workspace density only as shipped and execute gravity as Record/Practice. Simulator/Real is stamp honesty, not a third chooser. Operator-experience is not Mode.

## Why

A livelihood tool cannot require a 2×2×2 mode product.

## Context

- ADR 0094 · LY-094 leftover · `OPERATOR_UI_EXPERIENCE_MODES.md`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working settings/help: two controls. Host Mode appears only as honesty (IH-025), not as a peer chooser.
2. Do not delete Guided. Do not collapse env flags in this prompt.
