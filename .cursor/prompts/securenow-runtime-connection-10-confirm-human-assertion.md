...
Show proposed connections (from upload **and/or** from A’s app-settings inferences that are catalog-templates / unresolved hosts) and let the operator **confirm or dismiss**. Confirmed rows become `ProvenanceKind.HumanAssertion` and paint on Data Flow with label **Declared by operator** (or **Confirmed connection** — sentence case, one locked string). Dismissed rows never paint.

## Why

`{0}` tenant templates and unresolved hosts cannot be closed without a human. Silent inference on uploaded files would mix operator intent with hostname guesses.
...
2. UI: table of from → to (host/catalog), setting name, source (env | upload), status (proposed / confirmed / dismissed). Primary confirm disabled until a row is selected. Sentence case. Visible-boundary buttons.
...
- Do **not** auto-confirm. Do **not** call Azure at confirm time.
