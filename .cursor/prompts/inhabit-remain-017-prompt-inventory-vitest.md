# IR-017 — Vitest: IR-00 index + IR-001–018 files exist

**Wave:** inhabit remain leftovers (**IR**). **Not wave 35.** **Cluster:** close. **Depends on:** prompt-set PR.

Do not implement from the remain index. Implement only *What to build*.

## Goal

Add `archlucid-ui/src/lib/inhabit-remain-prompt-inventory.test.ts` asserting the remain index plus IR-001–018 files exist. Do not change the IH-079 ratchet (exactly 80 `inhabit-NNN` files).

## Why

Without a file ratchet, leftover prompt files will be dropped. IH-079 must keep counting only `inhabit-{NNN}-` wave-34 files (three-digit prefix after `inhabit-`).

## Context

- `inhabit-prompt-inventory.test.ts` (IH-079) · `.cursor/prompts/inhabit-remain-00-index.md`

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. 00-index present; exactly one file per `inhabit-remain-001`–`018`; 18 numbered remain files. Do not count remain files toward IH-079’s 80.
2. This prompt-set PR may already include the test — confirm it is green.
3. Do not implement product behavior here.
