# SY-68 — Recurrence, advisory scan, and ITSM Working returns open the architecture

Do not build new ITSM features. Href/return only.

## Goal

Working deep links from schedules / scans / tickets: architecture desk or nested job when ArchitectureId exists on the record.

## Why

Satellite products that only know run ids keep the job as the enterprise object.

## Context

- Recurrence / advisory / ITSM UI link builders (grep reviewDetailPath / reviews list)

## What to build

1. Convert Working builders + tests per surface you touch (one PR may do all three if small).

## Acceptance criteria

- At least one satellite per type uses nested href when architectureId present.
