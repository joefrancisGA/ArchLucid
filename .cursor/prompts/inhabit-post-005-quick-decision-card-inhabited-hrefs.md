# IP-005 — Quick-decision card CTAs stay on the architecture

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** dual-place. **Depends on:** IP-001, IR-005, IR-004.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

On Working, when `architectureId` is on the card context, quick-decision **Open finding** and **Record disposition** (and dense-table row navigation) use nested findings `focusedFinding` / inhabited inspect, not `getFindingDetailHref` / `getFindingGovernanceDispositionHref` review-detail routes.

## Why

IR-005 threaded `architectureId` onto cards but did not change navigation. `QuickDecisionWorkspacePrimaryFindingCard.tsx` still links to `/architecture/reviews/{runId}/findings/{id}`. Review-detail findings tab remains a parallel editor.

## Context

- `QuickDecisionWorkspacePrimaryFindingCard.tsx` · secondary cards · `finding-evidence-navigation.ts` · `governanceFindingInspectHref` · `RunDetailFindingsDenseTable` · `QuickDecisionWorkspaceCardContext`

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. When Working + `architectureId` known, Open finding / Record disposition / dense-table inspect use `governanceFindingInspectHref` (or equivalent nested `focusedFinding`). Guided/unlinked keep review-detail inspect. Do not delete the inspect route.
2. Do not send Graph/Compare/Impact preview to draft-diff. Nested review tabs stay a full inspector strip (ADR 0098) — this prompt only fixes **finding work CTAs**.
3. Flip quick-decision href inventory. Owner `IP-005`. Focused Vitest: Working card with architectureId href contains `/architectures/{id}/findings` and `focusedFinding`; not `/reviews/{runId}/findings/{id}` as the primary CTA.
