# AS-065 — PilotStrict Working Real may optionally hold on Unsupported (default off)

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-gate. **Depends on:** AS-064, AS-056.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Config flag default **false**: when true, Working Real PilotStrict treats Unsupported decision-grade as a finalize hold. Simulator never uses this hold. Document next to TB-1228. Do not default on.

## Why

Some tenants will want fail-closed semantics. Global default-on would false-reject V1.
...
1. Options + tests default off; on + Unsupported blocks; Simulator ignores flag.
2. Honesty copy when flag off.
