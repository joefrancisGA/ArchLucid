# LN-033 — Ratchet: semantic band is not default commit gate

**Wave:** livelihood-grade-no (livelihood UX wave 26) (**LN**). **Cluster:** ratchet. **Depends on:** LN-008.

Do not implement from the wave index. Implement only *What to build*.

## Goal

ADR 0085 stays: semantic is warn, not sync block. LN must not invert this.

## Why

Blocking on LLM faithfulness is non-reproducible.
...
1. Acceptance line + test if a hold flag exists (default off).
2. Structural 0082 stays persist gate.
3. No judge default-on.
