# IH-037 — Finding CAS 409 Keep mine / theirs on the inhabited desk

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** reversibility. **Depends on:** ADR 0076, LW leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Disposition 409 on Working nested findings uses Keep mine / theirs (or equivalent) inline, not a toast that disappears.

## Why

Concurrent afternoon without presence still collides. Lost-write must be on the document.

## Context

- ADR 0076 · ADR 0088 pattern · LY-049 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working disposition 409 recovery is inline on the findings document. CAS token required. No last-write-wins.
