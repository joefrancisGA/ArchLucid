# SY-58 — Working operate-analysis daily strip stays empty (tools live on the desk)

Do not fork AO-14 empty operate-analysis array. Prevent regressions that put Graph/Ask/Compare back in first viewport.

## Goal

`SIDEBAR_DAILY_HREFS_BY_GROUP_WORKING['operate-analysis']` remains `[]`. Nested tools are reached from the desk command bar (SY-60) and shortcuts.

## Why

Putting Graph back in the first viewport recreates Insights-as-product.

## Context

- `sidebar-nav-daily-links.ts`
- `sidebar-nav-daily-links.test.ts`

## What to build

1. Guard test: Working daily operate-analysis does not include Insights paths.

## Acceptance criteria

- Test fails if Evidence graph is a Working daily href again.
