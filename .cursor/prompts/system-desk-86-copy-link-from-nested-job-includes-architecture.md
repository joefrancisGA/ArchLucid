# SY-86 — Copy link on a nested job copies the nested URL, never a rewritten peer URL

Do not fork SY-20 share helper. Button wiring.

## Goal

Review-detail copy-link on nested route uses `window` path or share helper that preserves `/architectures/{id}/reviews/`.

## Why

A copy button that canonicalizes back to peer URLs undoes nesting.

## Context

- Copy link control on review workspace

## What to build

1. Prefer current pathname when already nested.
2. Vitest on helper.

## Acceptance criteria

- Nested page copy output contains `/architectures/`.
