# SY-44 — Print / provenance / finding inspect inherit nested architecture prefix

Do not fork print honesty (FC). Ensure child segments hang off nested review, not peer.

## Goal

Working print, provenance, finding inspect URLs are under `architectureNestedReviewPath`. Redirect peer children when architectureId known (extend AO-06).

## Why

A nested review whose print button still copies a peer URL splits the object.

## Context

- Review print/provenance/inspect pages
- `working-architecture-review-routes.ts`

## What to build

1. Child URL helpers + redirects + Vitest.

## Acceptance criteria

- Print href with architectureId includes `/architectures/{id}/reviews/`.
