# IH-010 — Inventory: keyboard does not land on the first finding
...
Inventory Working nested findings: whether focus lands on the first finding card so Alt+1/2/3 and Alt+J/K work without a mouse.

## Why

Keyboard exists. The inhabited day still starts with a click.

## Context

- PC-11 · `useFindingCardShortcuts.ts` · LY-091 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Note default focus, skip links, and whether the palette’s first actions are dispose/amend/finalize.
2. No single-letter shortcuts (WCAG 2.1.4).
