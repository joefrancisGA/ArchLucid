# SY-61 — Browser tab title on Working desk/tools is the architecture display name

Do not change Guided titles. `document.title` / Next metadata.

## Goal

Working nested architecture routes: `{displayName} · ArchLucid` (or existing product suffix). Job routes may append “ · Review” as a verb, not as the document name.

## Why

OS task switcher is Monday morning. A GUID run title teaches the job.

## Context

- Architecture identity page metadata
- Nested tool layouts

## What to build

1. Title helper + Vitest.
2. Unlinked peer review may keep review title + honesty.

## Acceptance criteria

- Identity desk title includes displayName, not only “Review”.
