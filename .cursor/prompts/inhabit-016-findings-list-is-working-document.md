# IH-016 — Findings list is the working document

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-015.

Do not implement from the wave index. Implement only *What to build*.

## Goal

On Working nested findings, the list is the document: identity is the architecture, the open child job is a subtitle, rows are the work.

## Why

A queue that feels like a pipeline step is not inhabited.

## Context

- `GovernanceFindingsQueueClient.tsx` · architecture-scoped findings · density sort already shipped
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working nested findings presentation: architecture H1, job subtitle, no pipeline stepper, no peer Start CTA.
2. Do not collapse review tabs. Inspector remains available as a tool.
