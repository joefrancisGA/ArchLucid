# IP-014 — Vitest: IP-00 index + IP-001–015 files exist

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** close. **Depends on:** prompt-set PR.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

Add `archlucid-ui/src/lib/inhabit-post-prompt-inventory.test.ts` asserting the post-IR index plus IP-001–015 files exist. Do not change the IH-079 ratchet (exactly 80 `inhabit-NNN` files) or the IR-017 remain-file count (18 `inhabit-remain-NNN`).

## Why

Without a file ratchet, leftover prompt files will be dropped. IH-079 must keep counting only `inhabit-{NNN}-` wave-34 files.

## Context

- `inhabit-remain-prompt-inventory.test.ts` (IR-017) · `inhabit-prompt-inventory.test.ts` (IH-079)

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. 00-index present; exactly one file per `inhabit-post-001`–`015`; 15 numbered post-IR files. Do not count them toward IH-079’s 80 or IR-017’s 18.
