# AS-083 — CLI: try --real vs explicit --rehearse (no silent default Career)

**Wave:** architecture-spine (**AS**). **Cluster:** rehearsal-cli. **Depends on:** AS-076.

Do not implement from the wave index. Implement only *What to build*.

## Goal

`archlucid try --real` already exists (ADR 0033). Add or align `--rehearse` / Simulator wording so CLI matches doors. Do not change default Mode in shipped appsettings.

## Why

CLI users are Working professionals too.

## Context

- ADR 0033
- ValidateConfigEvaluator Mode checks

## What to build

1. Help text + tests.
2. Do not require AOAI for `--rehearse`.

## Acceptance criteria

- CLI vocabulary matches UI.
