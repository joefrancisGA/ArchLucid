# IH-064 — Finding selection persists on URL and/or account prefs

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** continuity. **Depends on:** IH-011, PT-14 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working nested findings selected finding survives refresh via URL query (preferred) and/or user preferences. Not sessionStorage as sole SoT.

## Why

Losing the finding you were on is not an all-day instrument.

## Context

- PT-14 · workbench column persist leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Deep-link selected finding on Working nested findings. Refresh restores it. Tenant-scoped. No token in the URL.
