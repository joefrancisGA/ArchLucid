# SY-27 — Findings dual-pane default pathname is nested on Working

Do not fork LS-01 dual-pane selection. Pathname default only.

## Goal

`ArchitectureFindingsDualPane.tsx` `usePathname() ?? reviewDetailPath(props.runId)` → nested when `props.architectureId` exists.

## Why

Dual-pane is all-day work. A peer default pathname makes the job the app shell.
...
1. Use `resolveArchitectureReviewHref`.
2. Component or helper test.

## Acceptance criteria

- Default path nested when architectureId present.
