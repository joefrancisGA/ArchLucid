# AS-007 — Mermaid source as a review context document
...
Accept `.mmd` / mermaid fenced source as structured parse into the AS-006 model. Reuse IE-18 mermaid ingest if it already exists; if IE-18 is unshipped, implement the review-input parser here and leave IE collector prompts untouched.

## Why

Architects already keep C4/flowchart Mermaid next to the brief. That is structured topology, not pixels.

## Context

- IE-18 prompt (structured diagram ingest)
- CONTEXT_INGESTION.md
- AS-006 schema

## What to build

1. Parser: flowchart/C4-ish node and edge lines; fail-soft on unknown directives.
2. Wizard: `.mmd` and `text/plain` files whose body looks like mermaid are not dropped.
3. Tests: fixture mermaid → nodes+edges; garbage does not throw.
4. Do not statically import mermaid on UI hot paths (IE-16 constraint).

## Acceptance criteria

- A review whose only attachment is a mermaid file gets a graph, not an empty package.
