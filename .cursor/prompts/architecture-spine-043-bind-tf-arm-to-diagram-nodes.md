# AS-043 — Bind existing Terraform/ARM ingest nodes to diagram nodes

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-bind. **Depends on:** AS-018.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Reuse AS-018 matchers for resources already on the graph from IaC ingest (DX-27/30/31/37/42). No new IaC parser.

## Why

The estate is often already in the repo as .tf; the drawing should snap to it.

## Context

- Existing ingest engines
- AS-018

## What to build

1. Call the same binder after IaC graph seed + diagram compile.
2. Tests: tf resource address in a mermaid label binds.
