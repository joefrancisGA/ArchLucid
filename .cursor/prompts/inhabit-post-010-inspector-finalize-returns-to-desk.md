# IP-010 — Inspector finalize returns to the architecture desk

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** document. **Depends on:** IP-001, IH-022.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

On Working, when `parentArchitectureId` is known, inspector `CommitRunButton` mounts (review-package Do this next strip and any other inspector finalize that omits the prop) pass `parentArchitectureId` so success uses `resolveFinalizeSuccessDeskHref` (desk + `highlightReviewId`), matching inhabited-document finalize.

## Why

ADR 0100: finalize returns to the desk. Findings-document finalize already passes the id. `ReviewPackageDoThisNextStrip.tsx` `CommitRunButton` omits it — success Close stays on the inspector.

## Context

- `ReviewPackageDoThisNextStrip.tsx` · `CommitRunButton.tsx` · `ReviewPackagePrimaryAction.tsx` (already has prop) · `RunDetailPageViewShell` / sticky actions (verify other omissions)

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Thread `parentArchitectureId` from the review page model into every Working inspector finalize mount that currently omits it. Do not make nested review-detail Monday morning. Do not merge kernels.
2. If a mount cannot know architecture, keep current Close behavior (unlinked job). Do not architecture-scope peer queue.
3. Flip inspector-finalize inventory. Owner `IP-010`. Focused Vitest or source assert: Do-this-next strip `CommitRunButton` includes `parentArchitectureId`.
