# SN-035 — OpenAPI for clone/envelope spawn if new fields

**Wave:** system-not-job (livelihood UX wave 25) (**SN**). **Cluster:** contract. **Depends on:** SN-008.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If clone/envelope spawn gained fields (invariant override, door), snapshot OpenAPI + TS types.

## Why

Wire drift is how the next client omits the door stamp.
...
1. Snapshot if changed.
2. If no wire change, document that in the PR.
3. Do not invent draft-compare API.
