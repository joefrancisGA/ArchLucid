# IH-053 — Room card: one MUST question, yes / no / another

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** room. **Depends on:** IH-009, PC-09 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working inhabited architecture (findings or desk) can show one current MUST elicitation: yes / no / another question. Answers append to the asserted trail. Not a wizard takeover.

## Why

R4 projector persona is the liability stance. A tabbed SPA without a room card fails the founding debate.

## Context

- Debate R4 · ADR 0055 · PC-09 · `presenter` mounts
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Non-modal room card on Working architecture when a MUST is unanswered. Answers write asserted trail. Keyboard reachable.
2. No avatars. No chat thread. Do not replace Socratic intake with a blank form.
