# SG-119 — Vitest: SG-00 index + SG-001–SG-120 files exist

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** close. **Depends on:** prompt-set PR.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Add `archlucid-ui/src/lib/system-gravity-prompt-inventory.test.ts` asserting the index plus SG-001–SG-120 files exist (same pattern as SN-037 / CG-099).

## Why

Without a file ratchet, prompt files will be dropped on later waves.

## Context

- system-not-job-prompt-inventory.test.ts · career-gravity-prompt-inventory.test.ts

## What to build

1. Vitest: 00-index present; exactly one file per SG-001–120; 120 numbered files.
