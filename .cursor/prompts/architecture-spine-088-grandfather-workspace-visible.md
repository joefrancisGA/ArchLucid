# AS-088 — Grandfather: existing architectures stay workspace-visible

**Wave:** architecture-spine (**AS**). **Cluster:** share-data. **Depends on:** AS-087.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Backfill RestrictToShares = 0. Tests that list APIs unchanged for grandfathered rows. Do not lock historical packages.

## Why

Surprise hiding is a production incident.
...
1. Migration default / backfill.
2. API test: unrestricted architecture visible to any workspace reader as today.

## Acceptance criteria

- No behavior change until opt-in.
