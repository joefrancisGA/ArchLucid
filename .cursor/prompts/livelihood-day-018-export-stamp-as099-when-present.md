# LY-018 — Export stamp uses overlay scorer version when present

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** judge. **Depends on:** LY-006.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Career export / stamp lines that today hardcode as057-v1 should prefer the overlay scorer version when an as099 overlay exists for the snapshot.

## Why

A packet that says as057-v1 after LLM finalize is a false provenance line.
...
1. UI FINDING_SEMANTIC_SUPPORT_BAND_SCORER_VERSION may stay the heuristic default; overlay version wins when the API returns it.
2. Do not claim the export is legally verified.
