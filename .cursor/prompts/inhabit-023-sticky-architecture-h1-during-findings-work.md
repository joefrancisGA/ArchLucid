# IH-023 — Sticky architecture H1 during findings work

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-016, SG-025 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

While Working nested findings (or inspect panel) is open, H1 and document.title stay the architecture display name.

## Why

0098 chrome can still slip to the review title on findings-only routes.

## Context

- `deriveWorkingInstrumentReviewHeaderPresentation` · LY-023 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Ratchet: Working nested findings header uses architecture display name. Review title is subtitle.
2. Guided may keep review-centric headers.
