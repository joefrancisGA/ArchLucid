# IH-060 — Palette work actions before nav rows

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** keyboard. **Depends on:** IH-017, SG-053 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working palette on inhabited findings: Accept/Remediate/Reject, Amend, Finalize, Sketch, Open architecture — before nav destinations.

## Why

PC-11 shipped work-first. Inhabit leftovers are findings-document actions.

## Context

- `resolve-visible-command-palette-actions.ts` · LY-038 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. When Working nested findings is open, palette ranks dispose/amend/finalize/sketch above Finish setup and peer Start.
2. Guided may keep eval actions.
