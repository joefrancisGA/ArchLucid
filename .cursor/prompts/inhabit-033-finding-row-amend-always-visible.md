# IH-033 — Finding-row amend / record correction always visible

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** reversibility. **Depends on:** IH-006, PC-10 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

On Working inhabited findings, record correction / amend is on the row (or inspect) without depending on the 300s toast still being mounted.

## Why

After five minutes the livelihood path must still exist in the document.

## Context

- PC-10 · SG-021 leftover · `FindingDispositionRecordCorrectionControl`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working nested findings rows expose amend/record-correction after the undo window. Do not lengthen the toast.
2. Sealed parent still immutable; correction is append-only rationale.
