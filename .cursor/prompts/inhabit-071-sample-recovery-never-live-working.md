# IH-071 — Live Working recovery never offers sample / Claims Intake

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** dual-spine. **Depends on:** PT-02 leftover, WD-08 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working error recovery on inhabited findings/desk never offers sample packages or Claims Intake as the next step.

## Why

Live failures that think in demo are not a livelihood tool.

## Context

- `isLiveOperatorShellRecoveryContext` · TB-2155
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Recovery contract on Working nested findings: what’s intact, retry, desk, Report Problem. No sample href on live recovery.
