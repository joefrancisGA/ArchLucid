# IH-048 — Assumption delta vs last seal on the inhabited desk

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** exploration. **Depends on:** PC-06 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working architecture desk shows read-only assumption delta vs last seal on the same architecture without a billable run.

## Why

Cheap thought before Sketch. PC-06 shipped a panel; inhabit needs it on the desk you live in.
...
1. Visible delta entry on Working architecture when a prior seal exists. No new execute. No R12 Career what-if in this prompt.
