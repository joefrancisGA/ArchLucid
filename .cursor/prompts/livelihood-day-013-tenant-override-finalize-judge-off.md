# LY-013 — Tenant may opt out of finalize judge spend

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** judge. **Depends on:** LY-011.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Document and, if a tenant finding-engine control already exists for this flag, bind EnableLlmJudgeOnFinalize false as the off switch. Do not invent a second admin surface if overlaying InsightDensity controls would confuse emit vs finalize.

## Why

Real finalize cost must be disableable without flipping host Mode.
...
1. If no tenant key exists, document host JSON opt-out only and add a leftover note — do not invent a new RBAC page unless copy already claims a control.
2. Simulator still skips even if stored true.
