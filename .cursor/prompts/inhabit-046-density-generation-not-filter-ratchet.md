# IH-046 — Ratchet: insight density is generation, not a support-band filter

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** completeness. **Depends on:** AS-066, LY-088 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Ratchet: inhabit chrome must not fuse density demotion with support-band filtering. typed-engine-protected unchanged.

## Why

The tempting completeness fix is to hide demoted engines and call it done.

## Context

- ADR 0070 · `HOLD_NO_COVERAGE_ENGINES.md`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Vitest/docs ratchet: inhabit findings copy says density is generation. No new coverage engine. No predicate rewrite.
