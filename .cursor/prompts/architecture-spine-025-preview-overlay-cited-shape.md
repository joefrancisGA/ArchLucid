# AS-025 — Diagram preview can highlight a shape id

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-desk. **Depends on:** AS-008/009/010 as available; can ship mermaid-first.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Preview overlay: mermaid/SVG/draw.io/vsdx viewers support `?shape=` or equivalent highlight. Fail-soft if the format cannot highlight yet.

## Why

Jump without highlight is still a scavenger hunt.

## Context

- StoredEvidenceFileCells / preview dialog

## What to build

1. Highlight contract + mermaid implementation first if others are hard.
2. Honesty if vsdx highlight not ready: “Open file; shape id X.”
3. Tests: mermaid highlight class present.

## Acceptance criteria

- At least one format highlights; others are honest.
