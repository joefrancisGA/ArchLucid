# SY-54 — Unfinished-work Working href is the architecture desk, not hub `?filter=`

Do not delete the hub filter. Change Working next-best-action.

## Goal

`reviews-hub-unfinished-work-href.ts` Working: last-open desk or nested in-flight job. Hub filter stays for Inbox users who are already on the hub.

## Why

Needs-attention that dumps you on the hub makes the inbox the task list Home.

## Context

- `reviews-hub-unfinished-work-href.ts`
- Home next-best-action

## What to build

1. Working resolver + Vitest.

## Acceptance criteria

- Working unfinished CTA is not `REVIEWS_LIST_PATH?filter=`.
