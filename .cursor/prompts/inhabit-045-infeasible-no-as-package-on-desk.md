# IH-045 — Reasoned no is a package on the inhabited desk

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** completeness. **Depends on:** PT-16 leftover, LN leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working hard/soft infeasible remains a first-class package on the architecture desk (receipt, why, envelope), not an error appendix inside the inspector only.

## Why

R13: the no is the product. Inhabit fails if no lives only in a nested tab.

## Context

- ADR 0050 · ADR 0093 · PT-16 · LN desk copy
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working desk/findings shows infeasible receipt entry when the child job is infeasible. Export gates for uncited hard stay LN.
2. Do not classify new hard without citation.
