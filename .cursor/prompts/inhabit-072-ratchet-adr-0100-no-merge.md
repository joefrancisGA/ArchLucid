# IH-072 — Ratchet: ADR 0100 exists and kernels stay unmerged

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** ratchet. **Depends on:** IH-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Vitest/C#: ADR 0100 file exists; FAQ answers present; `DraftRequests` / `Runs` unmerged (reuse SN no-merge ratchet).

## Why

Without a ratchet, inhabit will be “implemented” by merging tables.

## Context

- `system-not-job-no-merge-kernels-ratchet.ts`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Add `inhabit-adr-guard.test.ts` (or extend IH-001 guards). Cite SN no-merge ratchet. Do not create a third kernel.
