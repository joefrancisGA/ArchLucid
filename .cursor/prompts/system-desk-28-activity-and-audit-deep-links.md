# SY-28 — Activity / audit Working deep links nest under the architecture

Do not change audit event payloads except optional architectureId already on the DTO. Href builders only.

## Goal

Working activity rows that open a review use `resolveArchitectureReviewHref`. Tenant audit log may still list many architectures (inbox-like); the **open** action goes to the desk/job nest.

## Why

Audit is a livelihood surface. Opening a peer run from Activity undoes the locator.

## Context

- Operator activity / audit link helpers (grep `reviewDetailPath` near Activity)

## What to build

1. Convert Working builders.
2. Vitest on helper.

## Acceptance criteria

- Known architectureId ⇒ nested job URL from Activity.
