# IH-019 — Finalize is a verb on the findings document and returns to the desk

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-016, SG-022 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working finalize from nested findings (palette + chrome) returns to the architecture desk with the sealed child highlighted. It is not Home.

## Why

SG-022 shipped return-to-desk from job row. Inhabit needs the same from the findings document.

## Context

- `resolveFinalizeSuccessDeskHref` · LY-022 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Finalize CTA reachable from Working nested findings. Success href is the architecture desk, not the governance register as Home.
2. Do not unseal. Career honesty gates unchanged.
