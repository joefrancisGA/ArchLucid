# IH-063 — Keyboard docs match inhabit landing

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** keyboard. **Depends on:** IH-059, LY-091 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

`archlucid-ui/docs/KEYBOARD_SHORTCUTS.md` and in-app overlay match: Working lands on findings; Alt+R is desk; Alt+N is nested start.

## Why

Docs that still say Alt+N is /runs/new train the job as the object.

## Context

- `useShortcutNavigation.test.tsx` SN-019
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Update keyboard docs to inhabit facts. Tests already pin SN-019 — do not regress Guided peer URLs.
