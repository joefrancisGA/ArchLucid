# IR-013 — Draft room header and L0 MUST stay on findings

**Wave:** inhabit remain leftovers (**IR**). **Not wave 35.** **Cluster:** room. **Depends on:** IH-053, IR-012.

Do not implement from the remain index. Implement only *What to build*.

## Goal

On Working after spawn, unanswered MUST elicitation from the draft room header and Quick-start L0 panel is the inhabited findings room card (or a deep link to it), not a separate place that leaves the findings document.

## Why

`ArchitectureDraftRoomHeaderButton.tsx` and `QuickStartL0MustQuestionsPanel.tsx` both `requiresLeavingArchitectureFindings: true`. After spawn the Career draft editor is spawn-locked; sending MUST work back to draft chrome exiles the afternoon document.

## Context

- `components/architecture/ArchitectureDraftRoomHeaderButton.tsx` · `components/architecture/QuickStartL0MustQuestionsPanel.tsx` · `InhabitedFindingsRoomCard.tsx` · spawn-lock · IH-053

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. When Working + spawn-locked / inhabited findings, unanswered MUST from header/L0 opens or focuses `InhabitedFindingsRoomCard` on the architecture. Pre-spawn draft intake may keep L0 on the draft. Do not unseal. Do not remount a wizard takeover.
2. No avatars/chat. Do not lengthen 300s. Do not flip G-REAL-06.
3. Flip both inventory rows to `requiresLeavingArchitectureFindings: false`, owner `IR-013`. Focused Vitest on Working header/L0 href after spawn.
