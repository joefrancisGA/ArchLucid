# IH-073 — Ratchet: MUTATION_UNDO_WINDOW_SECONDS stays 300

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** ratchet. **Depends on:** IH-035.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Vitest pins `MUTATION_UNDO_WINDOW_SECONDS === 300`. Inhabit amend is the livelihood path, not a longer toast.

## Why

Every livelihood wave tries to lengthen the toast.

## Context

- `mutation-reversibility-registry.ts` · LY-043 skip
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Guard in inhabit tests. Do not change the constant.
