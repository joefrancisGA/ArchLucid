# AS-060 — Persist support band as overlay, not a sealed-message mutation

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-wire. **Depends on:** AS-059, AS-057.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Store band + scorer version + hashed evidence excerpt pointer on an overlay table or existing quality overlay. Do not mutate sealed Finding.Message (LP-15 restatement is append-only). After seal, band is frozen with the snapshot or copied onto the overlay immutably.

## Why

Re-scoring must not rewrite the claim the architect signed.

## Context

- LP-15 architect restatement append-only
- ADR 0039

## What to build

1. Persistence + tests.
2. Sealed row cannot change Message when band backfill runs.

## Acceptance criteria

- Seal immutability holds.
