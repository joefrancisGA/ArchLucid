# AS-019 — Unlabeled diagram boxes are NotVerifiable, not invented resources (R5)
...
Shapes with no text and no bound id do not become CanonicalObject resources. They may exist as anonymous geometry for preview only, or a single NotVerifiable “unlabeled shape” held-check count.

## Why

False-hard from an empty rectangle is worse than a miss (R5).

## Context

- ADR 0050 / R5
- AS-005 pixel honesty

## What to build

1. Compiler skip unlabeled; optional held-check count on the review.
2. Tests: fixture with 3 unlabeled rects → 0 resource nodes from them.

## Acceptance criteria

- No invented resources from empty boxes.
