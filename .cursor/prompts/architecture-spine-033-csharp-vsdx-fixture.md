# AS-033 — C# .vsdx fixture tests (zip-slip + shape text)

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-ratchet. **Depends on:** AS-010.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Checked-in tiny vsdx (or dynamically built zip in test) covering: shape text → node; `../` zip entry rejected.

## Why

vsdx parse is the highest-risk parser (zip + xml).
...
1. Fixture + tests.
2. No Visio dependency.

## Acceptance criteria

- Path traversal fails closed.
- Happy-path node extracted.
