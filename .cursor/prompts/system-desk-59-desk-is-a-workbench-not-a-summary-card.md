# SY-59 — Architecture desk is a workbench: jobs, current draft, in-flight, and tool entry

Do not fork AO-20 identity header. Add workbench density, not marketing cards.

## Goal

Working desk first viewport: identity, in-flight chip, current draft pane, jobs table, command bar (SY-60). Latest seal is present but not the only content. No giant empty hero.

## Why

A summary card that only links out makes the desk a launch pad — Monday morning still happens elsewhere.

## Context

- `ArchitectureIdentityDesk.tsx`
- `ArchitectureIdentityDesk.test.tsx`

## What to build

1. Layout order + testids for workbench regions.
2. Carbon density (`space-y-4`).

## Acceptance criteria

- Desk testid regions include jobs + tools entry, not only rename/archive.
