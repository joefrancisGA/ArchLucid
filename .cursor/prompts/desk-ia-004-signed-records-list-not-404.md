# DI-004 —  `/signed-records` list is not 404 leftover
...
Rewrite target must serve the list (or redirect to the new Governance list). Palette/crumbs cannot 404.
...
- next.config rewrite `/signed-records` → `/manifests`

## What to build

1. Index route exists.
2. Redirect tests.
3. Do not leave rewrite to a missing page.
