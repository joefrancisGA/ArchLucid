# AO-44 — Working deep-page back is the architecture desk

**Do not fork AD-03.** Do not restore breadcrumbs (TB-2090).

## Goal

Working findings-by-id, evidence-trace, provenance, print: back/close goes to nested review, then to architecture desk — never to Home marketing or buyer-inverted hubs.

Print opener keepalive (WA-17 leftover) must not dump to a peer URL.

## Why

Inverted back links are how you get lost in a 142-route SPA.

## Context

- AD-03
- finding evidence-trace pages
- print page

## What to build

1. Shared `workingBackHref` helper: child → nested job → desk.
2. Vitest matrix of deep pages.
3. Guided back may stay review-centric.
