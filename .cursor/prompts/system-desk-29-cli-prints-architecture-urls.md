# SY-29 — CLI print/open emits architecture locator URLs for Working-shaped output

Do not fork AO-11. Leftover run-centric print.

## Goal

When the CLI knows `ArchitectureId`, printed UI URLs use nested review or identity path. `--guided` or missing id may print peer review URLs.

## Why

Operators paste CLI output into tickets. That becomes the bookmark.

## Context

- ArchLucid.Cli print/format helpers (grep `/architecture/reviews/`)

## What to build

1. Shared URL format helper in Contracts or Cli.
2. Scoped C# tests.
3. `agent-compile-check.ps1` on the touched project.

## Acceptance criteria

- Test: architecture id present ⇒ printed href contains `/architecture/architectures/`.
