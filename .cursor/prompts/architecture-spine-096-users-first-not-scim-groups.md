# AS-096 — V1 shares are users (oid); SCIM groups out of this wave

**Wave:** architecture-spine (**AS**). **Cluster:** share-hold. **Depends on:** AS-087.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Do not implement group-to-share expansion. Document leftover. If UI picker only lists users, say so. SCIM group membership remains for roles (existing), not architecture shares in V1 of this wave.

## Why

Group ACL is a second product. User shares close the consultancy hole.

## Context

- ADR 0032 SCIM
- GroupToRoleMapper — do not reuse incorrectly

## What to build

1. Contract sentence + UI copy.
2. Test: group id rejected or ignored with 400.

## Acceptance criteria

- No fake group shares.
