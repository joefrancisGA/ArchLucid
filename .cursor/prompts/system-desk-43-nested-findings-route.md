# SY-43 — App Router: `/architecture/architectures/{architectureId}/findings`

Do not fork findings grid/disposition (ADR 0076 409, 300s undo). Nested list scoped to the architecture.

## Goal

Working findings for **this** system. Tenant governance findings queue stays an inbox (sidebar), not Alt+G default (SY-11). Do not collapse review-detail finding tabs.

## Why

Findings are the daily work. They must not require opening a run as the app.

## Context

- Governance findings queue client
- `governance-findings-architecture-scope.ts`

## What to build

1. Nested findings page reusing queue with architecture filter required.
2. Helper + Vitest.
3. Keyboard Alt+1–3 still work on this grid.

## Acceptance criteria

- Nested findings 404s on architecture/tenant mismatch (existing identity GET).
