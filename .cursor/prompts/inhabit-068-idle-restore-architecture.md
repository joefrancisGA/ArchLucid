# IH-068 — Idle restore is the architecture

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** continuity. **Depends on:** LY-062 leftover, 4h Working idle.

Do not implement from the wave index. Implement only *What to build*.

## Goal

After Working idle timeout resume, restore the architecture desk/findings, not a pipeline Home.

## Why

Four-hour idle is the meeting. Restore must inhabit.

## Context

- `SESSION_IDLE_WORKING_TIMEOUT_MS` · ADR 0059 BFF activity
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working idle resume href is last-open architecture. 401 resume wrappers unchanged (0089).
