# SY-63 — New window / restore prefers last-open architecture, not last peer review URL

Do not fork CR-05 session honesty. Locator only. BFF stays LK.

## Goal

Working restore path uses last-open architecture id. Do not restore `/architecture/reviews/{id}` as the session home when architectureId is known.

## Why

Second monitors and restored sessions are how all-day users live.

## Context

- Desk continuity restore
- `working-start-route.ts`

## What to build

1. Restore helper + Vitest (extend AO-48).

## Acceptance criteria

- Restore href is identity path when last-open set.
