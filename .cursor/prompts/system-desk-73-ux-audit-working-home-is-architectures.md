# SY-73 — UX audit Working screenshots must not treat Reviews as Home

Do not fork AO-46. Registry leftover after nested tools.

## Goal

`ux-audit-route-registry.ts` Working home slug is architectures/overview, not reviews hub. Update if nested Ask/Graph become capture routes — they are tools, not Home.

## Why

Sales screenshots freeze the wrong object.

## Context

- `archlucid-ui/e2e/ux-audit-route-registry.ts`
- `ux-audit-working-home-guard.test.ts`

## What to build

1. Registry + guard update.

## Acceptance criteria

- Guard still fails if Working home capture is the reviews list.
