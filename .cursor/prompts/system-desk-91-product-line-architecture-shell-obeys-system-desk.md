# SY-91 — Architecture product line (not SecureNow) uses SY locator/work-surface rules

Do not fork PL-01–05 / SN branding. Architecture shell only.

## Goal

`NEXT_PUBLIC_ARCHLUCID_PRODUCT=architecture` Working chrome uses nested architecture URLs and Alt+R remap. Security/SecureNow shell is out of scope unless it reuses the same architecture desk components — then those components must stay nested.

## Why

Product-line split must not resurrect reviews-as-Home on port 3000.

## Context

- `product-line-catalog.ts`
- Architecture nav builders

## What to build

1. Test or comment guard: Architecture product line Start is not reviews hub.
2. No second API host.

## Acceptance criteria

- Architecture product line still hits `resolveWorkingStartHref` architecture paths.
