# SY-46 — Palette work actions open nested Ask/Compare/Graph/Findings

Do not fork AO-41 subset. Actions must use nested paths.

## Goal

This review / This architecture palette group: Ask, Compare, Graph, Findings use nested helpers. No bare Insights paths in Working palette nav href allowlist.

## Why

Palette is how experts skip the sidebar. It must not resurrect peer products.

## Context

- `filter-working-palette-nav-hrefs.ts`
- `command-palette-handler-actions.ts`

## What to build

1. Allow nested paths; keep bare Insights **out** of Working palette.
2. Vitest.

## Acceptance criteria

- Working palette cannot navigate to bare `ASK_REVIEW_QUESTIONS_PATH`.
