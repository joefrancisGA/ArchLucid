...
Bind changes are production-change adjacent: Required durable audit (TB-956 / ADR 0083 pattern). Tenant IDOR: architecture A cannot bind using snapshot ids from a crafted other-tenant id (catalog isolation). Same-tenant unauthorized role 403.

## Why

Estate binding is a livelihood write.

## Context

- ADR 0083 pattern
- ADR 0037
- AUDIT_COVERAGE_MATRIX.md if you add an event type

## What to build

1. Audit event + tests.
2. IDOR tests.
3. Do not swallow audit failure (ADR 0075 spirit).

## Acceptance criteria

- No silent bind.
- Cross-tenant snapshot id fails closed.
