# SY-56 — Working sidebar: Reviews item visible, labeled Inbox, below Architectures

Do not fork AO-14. Do not hide the item. Do not put it behind a desktop More **if it were a review-detail tab** — this is sidebar.

## Goal

Working nav label for `REVIEWS_LIST_PATH` is Inbox (or “All jobs”). Architectures remains the daily noun. Tooltip: cross-architecture inbox.

## Why

A sidebar item still named Packages/Reviews will be treated as Home even if Alt+R changed.

## Context

- Nav i18n / Working label resolver
- `sidebar-nav-daily-links.ts` Working pilot daily hrefs

## What to build

1. Working label branch + Vitest on daily order (Architectures before Inbox).

## Acceptance criteria

- Working visible label for the reviews list path is not “Packages” as the primary noun.
