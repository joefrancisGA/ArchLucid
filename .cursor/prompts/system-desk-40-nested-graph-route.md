# SY-40 — App Router: `/architecture/architectures/{architectureId}/graph`

Do not fork graph viz. Nested shell; evidence graph **surface** name stays (TB-2097).

## Goal

Nested Evidence graph binds to the open architecture’s current package/evidence. Empty: honesty + Start review on this architecture.

## Why

Top-level graph is a canvas product. Nested graph is the system’s evidence.

## Context

- Evidence graph page
- `evidence-graph-route.ts`

## What to build

1. `architectureNestedGraphPath` + thin page.
2. Vitest helper.

## Acceptance criteria

- Graph URL includes architectureId segment.
