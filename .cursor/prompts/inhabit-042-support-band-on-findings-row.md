# IH-042 — Support band on the findings row

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** completeness. **Depends on:** IH-016, AS leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working inhabited findings rows show semantic support band (Supported / Unchecked / Unsupported / NotScored) where the contract already computes it. Not a new scorer.

## Why

Band-only-on-stamp trains false confidence on the document.

## Context

- ADR 0085 · ADR 0099 · LY-073 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Row or inspect chip for support band on Working nested findings. Simulator never shows Career Supported (IH-043).
