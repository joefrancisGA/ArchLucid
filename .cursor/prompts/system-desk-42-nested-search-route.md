# SY-42 — App Router: `/architecture/architectures/{architectureId}/search`

Do not fork AO-32 scope resolver. Give search a nested URL.

## Goal

Working search while a system is open lives at nested search. Tenant-wide search remains available from portfolio (no open system) as an explicit inbox-like tool — not the default while a desk is open.

## Why

Search that is always tenant-wide while Payments API is open is another peer product.

## Context

- `search-review-evidence-route.ts`
- `working-architecture-search-scope.ts`

## What to build

1. Nested path + page mounting existing search with AO-32 scope.
2. Vitest.

## Acceptance criteria

- Open architecture ⇒ search URL nested.
