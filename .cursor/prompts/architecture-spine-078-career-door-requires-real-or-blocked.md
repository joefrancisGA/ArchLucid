# AS-078 — Career door requires Real or a blocked honesty state

**Wave:** architecture-spine (**AS**). **Cluster:** rehearsal-chrome. **Depends on:** AS-077.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Selecting Career when host cannot run Real: blocked state with why (missing AOAI, Mode pinned Simulator) and CTA to Rehearsal or platform settings. Do not silently execute Simulator under Career.

## Why

Silent fallback is the trust-killer TB-1299 already forbade for commit-eligible Real.

## Context

- DEGRADED_MODE.md / Real fail-closed
- ADR 0033

## What to build

1. Gate + copy + tests: Career + Simulator host ≠ execute as Career.
2. Do not flip Mode in appsettings as a side effect.

## Acceptance criteria

- Career cannot launder Simulator.
