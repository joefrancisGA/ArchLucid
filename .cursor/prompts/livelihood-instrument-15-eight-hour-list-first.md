# LI-15 — Eight-hour accessibility: list-first, no Working teaching traps

**WD-12 is unique — implement that residual here.** `FindingEvidenceGraphOutline` already exists as a toggle. VPAT honest limits stay honest — do not claim full screen-reader coverage.

## Goal

Working-mode evidence/graph surfaces default to a **keyboard-reachable list/outline**; the canvas is optional. Working does not auto-open first-visit help, shortcut coaches, or “where to go next” strips. Focus order on review workspace tabs stays a single strip (no desktop More).

## Why

VPAT (`docs/security/VPAT_2_5_WCAG_2_1_AA.md`) does not claim comprehensive keyboard coverage of wizards, tables, and custom controls; complex graphics may pass axe and still fail as a work surface. All-day use is list + keyboard + predictable focus. `FirstVisitHelpAutoOpen` already skips full operator shell / architect chrome, but `GUIDED_TEACHING_CHROME_SURFACE_IDS` / other coaches can still mount if they ignore `useTeachingChromeVisible()`. Graph default is still canvas-first.
...
- `use-teaching-chrome-visible.ts`
...
2. Gate every id in `GUIDED_TEACHING_CHROME_SURFACE_IDS` so Working does not mount them. Fix any mount that ignores `useTeachingChromeVisible()`.
