# AS-052 — Bound snapshot freshness on the architecture desk

**Wave:** architecture-spine (**AS**). **Cluster:** inventory-bind. **Depends on:** AS-049.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Show snapshot captured-at / age next to the bind control. Stale threshold: document a constant; warn, do not auto-collect (no collector fork).

## Why

All-day: yesterday’s snapshot is not today’s estate.

## Context

- IE freshness if AE-05 exists — consume copy, don’t implement audit freshness

## What to build

1. Age display + warn band.
2. Tests.

## Acceptance criteria

- Freshness is visible without opening IE workbenches.
