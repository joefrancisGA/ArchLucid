# IP-011 — Inhabited findings banner and first-paint trail

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** dual-spine. **Depends on:** IP-001, IH-016, LN infeasible lead.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

On Working nested findings, the run-scope banner must not teach “Open review” as the afternoon escape hatch when inhabited chrome is mounted (`suppressPipelineChrome`). Prefer job-as-subtitle copy already used on inhabited chrome. Optionally improve first-paint so infeasible + transparency trail are not blank until `useQuery` if a cheap existing bundle/prop exists — do not invent a new progress API.

## Why

Lowest remaining rank. `GovernanceFindingsQueueScopeSection` still renders “Showing findings for review … Open review” whenever `scopedRunId` is set, ungated on `suppressPipelineChrome`. Reasoned **no** (R13) on nested findings waits on client fetch.

## Context

- `GovernanceFindingsQueueScopeSection.tsx` · `InhabitedFindingsDocumentChrome.tsx` · `RunDetailInfeasibleDecisionLead` · `suppressPipelineChrome`

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. When `suppressPipelineChrome` (or inhabited nested findings), hide or reword the run-scope banner: no primary “Open review” to peer `/architecture/reviews/{runId}`. Clear-scope / compare-lifecycle may stay if they do not become Monday morning. Peer queue without inhabit keeps the banner (IR-015).
2. First-paint: only if an existing server/page bundle can feed infeasible + trail without `GET /v1/runs/{runId}/progress`. If not cheap, document the gap in the inventory as remaining and do not invent a fetch. Do not remount CE Sketch.
3. Flip banner (and first-paint if shipped) inventory. Owner `IP-011`. Focused Vitest: inhabited + scopedRunId does not render Open-review to `/architecture/reviews/{id}` as the banner CTA.
