# AS-092 — Working desk share panel

**Wave:** architecture-spine (**AS**). **Cluster:** share-ui. **Depends on:** AS-089, AS-090.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Panel: restrict toggle, principal picker (existing user list), role select, remove. Carbon. Last-saved. Livelihood document guard (do not add to deferred inventory). Working density. Not a Guided teaching modal.

## Why

Sharing that exists only in API will not be used.

## Context

- useLivelihoodDocumentGuards
- ArchitectureIdentityRenameForm is deferred — do not copy that mistake
- ADR 0080

## What to build

1. Panel + Vitest.
2. Wire useLivelihoodDocumentGuards.
3. TB-2005.

## Acceptance criteria

- Dirty share form warns on navigate.
- Working, not buyer-polished.
