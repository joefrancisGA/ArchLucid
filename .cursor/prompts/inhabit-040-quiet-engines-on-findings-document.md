# IH-040 — Quiet engines named on the findings document

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** completeness. **Depends on:** IH-007, IH-021.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working nested findings shows which typed engines were quiet / not measured (PC-01 floor language), not only on Career export.

## Why

Desk completeness theater is the remaining false-confidence leak.

## Context

- PC-01 · PC-13 · FC-19 leftover · `insight-density-measurement-floor`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Mount a quiet-engine / measurement-floor sentence on Working nested findings (including non-empty lists).
2. Do not change the density predicate. Do not add a 40th engine.
