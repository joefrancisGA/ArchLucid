# AS-064 — Finalize warns on Unchecked decision-grade; does not block by default

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-gate. **Depends on:** AS-059.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working finalize: inline warning when Unchecked > 0. CTA remains enabled (warn, not TB-2005 hard fail). Copy: structural citations present; semantic support not confirmed.

## Why

Blocking on Unchecked would freeze every paraphrase. Warning is the livelihood nudge.

## Context

- RunDetailPreFinalizeGateHonestyStrip
- TB-2005 — warnings may leave CTA enabled
...
- Operator sees the gap before seal.
