# AS-015 — ESI inventoryKind distinguishes diagram-structured vs diagram-pixel vs citation

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-intake. **Depends on:** AS-014.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Extend inventory kinds so the Evidence table can say “structured diagram (analyzed)” vs “stored image (not extracted)” vs “cited — original not stored”. Do not make citation rows downloadable (ESI).

## Why

All-day desk: the architect must see which drawings actually fed decide.

## Context

- RunDetailEvidenceInventorySection
- enrichRunDetailEvidenceInventoryWithCatalog

## What to build

1. Kind + helper copy.
2. Tests: stored PNG without extract shows not-extracted; mermaid parse success shows analyzed.
3. Citation rows stay non-downloadable.

## Acceptance criteria

- Evidence table tells the truth about analysis participation.
