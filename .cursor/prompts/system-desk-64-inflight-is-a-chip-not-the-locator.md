# SY-64 — In-flight execute stays a chip on the architecture; wait page is not Home

Do not fork PC-08 / AO-21. Verify leftovers that still navigate Start to an in-flight **review URL**.

## Goal

`resolveWorkingStartHref` already prefers parent architecture. Kill remaining “open the run to wait” CTAs on Working Home.

## Why

Wait-as-the-job is how the pipeline becomes Monday morning.

## Context

- `ArchitectureIdentityDeskInFlightSection.tsx`
- Home in-flight modules

## What to build

1. CTAs → desk + chip, not peer progress URL.
2. Vitest.

## Acceptance criteria

- Working in-flight primary CTA is architecture desk or nested job under architectures prefix — not wait-hero-only route.
