# SG-014 — Inventory spawn-lock handoff continue-in-the-review copy
...
Produce `docs/architecture/SYSTEM_GRAVITY_HANDOFF_COPY_INVENTORY.md`: Handoff panels that send the architect to live in the review instead of returning to the desk with a job chip. Shrink-only. Do not mutate copy yet (SG-016+).
...
- ArchitectureDraftHandoffPanel · LK-04 · SN-013

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_HANDOFF_COPY_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
