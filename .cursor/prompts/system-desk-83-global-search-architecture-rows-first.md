# SY-83 — Working global search ranks architecture identities before review jobs

Do not fork search backend relevance beyond ranking/presentation if API already returns both.

## Goal

Working `/` search and header search: architecture hits first, labeled as systems; reviews labeled as jobs of {name} with nested hrefs.

## Why

Search that returns runs first trains the job as the document.

## Context

- Global search result mapper
...
- Fixture with both types: first Working group is architectures.
