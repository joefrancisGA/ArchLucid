# SG-003 — Vocabulary: review is a nested job inspector

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** copy. **Depends on:** SG-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working copy treats a review as a **job of this architecture** (inspector), never as a destination product. User-facing Record/Practice (0097) stays. TB-645: architecture, review, finding, sealed review record.

## Why

Handoff and first-session strings still say continue in the review as if it were the desk.

## Context

- `ArchitectureDraftHandoffPanel.tsx` · `buyer-polish-copy.ts` Working split · `operator-nav-labels.ts`

## What to build

1. Inventory leftover strings that call the nested review Home, package, or desk.
2. Replace Working leftovers with job-of-this-architecture language. Guided may keep eval phrasing.
3. Vitest: Working handoff copy does not say the review is Monday morning.
