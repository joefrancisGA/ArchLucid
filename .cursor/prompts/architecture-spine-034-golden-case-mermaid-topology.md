# AS-034 — Golden corpus case: mermaid-only topology

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-golden. **Depends on:** AS-016, AS-007.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Add the next golden case after `LatestGoldenCorpusCaseNumber` (read the constant first — do not guess). Mermaid with two systems and a trust-boundary subgraph. Expect graph + at least one engine can attach RelatedNodeIds. No fake frontier transcripts.

## Why

Without a golden, diagram compile will rot.
...
1. New case directory + README.
2. Do not add a coverage engine to make the case “interesting.”
3. Record per existing golden instructions.

## Acceptance criteria

- Harness includes the case; latest number bumped once.
