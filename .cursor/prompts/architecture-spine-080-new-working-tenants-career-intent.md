# AS-080 — New Working tenants default UI intent to Career (still no host flip)
...
First-run Working: chooser defaults to Career intent (AS-078 blocked if Real unavailable). Existing tenants: do not migrate silently; keep current Mode but label Rehearsal if Simulator. Document grandfather.

## Why

New seats should feel like a career tool. Old local Simulator clones should not break.

## Context

- expert-intake-posture / LP-13
- AS-078

## What to build

1. Default + grandfather tests.
2. Guided unchanged.

## Acceptance criteria

- New Working ≠ unlabeled Simulator.
