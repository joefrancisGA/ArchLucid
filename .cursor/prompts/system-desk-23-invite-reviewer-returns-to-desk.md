# SY-23 — Invite-reviewer back link returns to the architecture desk or nested job

Do not change invite authz. Href only.

## Goal

`InviteReviewerPageView` Working back/share uses nested path when architectureId is in query or loaded DTO.

## Why

Admin invite is a spoke. Sending them to a peer package URL teaches the job as the product.

## Context

- `archlucid-ui/src/app/(operator)/administration/users/_sections/InviteReviewerPageView.tsx`

## What to build

1. Resolve architectureId; `resolveArchitectureReviewHref`.
2. Vitest if a pure helper is extracted; else component test.

## Acceptance criteria

- Working invite back link is nested when architectureId known.
