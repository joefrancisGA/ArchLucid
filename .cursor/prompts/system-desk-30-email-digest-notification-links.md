# SY-30 — Email / digest / in-app notification links prefer the architecture locator

Do not change notification authorization. Href templates only.

## Goal

Working-facing notification templates: primary CTA is architecture desk or nested job. Do not send bare `/architecture/reviews/{guid}` when ArchitectureId is on the event.

## Why

The inbox people actually use is email. If email teaches the run, the SPA never wins Monday.

## Context

- Digest / notification URL builders in Application (grep reviews path)

## What to build

1. Template helper + tests.
2. Tenant isolation unchanged.

## Acceptance criteria

- Fixture with architectureId does not emit peer review path as the only href.
