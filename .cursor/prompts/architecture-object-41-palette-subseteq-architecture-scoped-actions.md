# AO-41 — Working palette ⊆ architecture-scoped actions + inboxes

**Do not fork PC-12 palette ⊆ nav.** Tighten to ADR 0077.

## Goal

Working palette rows that navigate must be: architecture list/desk, nested job, inboxes, settings, help. No orphan Insights routes that ignore the open architecture.

PC-12 evidence naming stays.

## Why

Palette is how power users skip the IA. If it still dumps 134 routes, the wave failed.
...
1. Filter Working palette by roles.
2. Vitest: no unclassified Working dest.
3. Guided palette may remain wider.

## Acceptance criteria

- Work actions still before nav (PC-11).
