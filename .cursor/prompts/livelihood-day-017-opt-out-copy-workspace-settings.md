# LY-017 — Opt-out copy: finalize semantic judge

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** copy. **Depends on:** LY-013.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If workspace finding-engine settings mention semantic support, distinguish emit (off) from finalize (on, disableable). Sentence case.

## Why

A single “LLM judge” toggle will be read as emit-on.
...
1. Copy only. Do not merge with InsightDensity EnableLlmJudge tenant override.
2. TB-645. No ghost buttons.
