# SY-77 — CI/Vitest ratchet: Working-only modules cannot import `reviewDetailPath`

Do not ban `reviewDetailPath` globally (Guided needs it). Allowlist Guided/buyer/marketing.

## Goal

Inventory Working-only files (desk, nested tools, Working home, Working shortcuts). ESLint `no-restricted-imports` or Vitest source grep. New Working files must use `resolveArchitectureReviewHref`.

## Why

Without a ratchet, the next overlay will import `reviewDetailPath` again.
...
1. `working-review-detail-path-import-guard.test.ts` with allowlist.
2. Allowlist must be explicit Guided/buyer paths.

## Acceptance criteria

- Adding `reviewDetailPath` to `ArchitectureIdentityDesk.tsx` fails the guard.
