# IH-006 — Inventory: disposition reversible only via the 300s toast
...
Inventory finding disposition UIs where after the toast expires the only recover path is buried or missing on the row.

## Why

The afternoon’s work is disposition. Five-minute undo is auditor-correct and livelihood-hostile unless amend is first-class.

## Context

- ADR 0071 · PC-10 · `MUTATION_UNDO_WINDOW_SECONDS` · `FindingDispositionRecordCorrectionControl`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Table of finding-row / inspect / queue surfaces: toast undo, record-correction visible, disposition history.
2. Do not lengthen 300s. IH-033–038 own the close.
