# SG-041 — Ratchet: nested review tabs stay a full strip

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** inspector. **Depends on:** SG-016.

Do not implement from the wave index. Implement only *What to build*.

## Goal

When nested review-detail is open, desktop tabs stay a full strip (`moreTabIds` empty). Desk command bar is not a More menu for those tabs.

## Why

Inspector leftovers exile the architect into a job app.

## Context

- resolve-review-detail-visible-tabs.ts · DI-009 · no-collapse-workspace-tabs.mdc

## What to build

1. Implement only the leftover. Do not collapse tabs. Do not delete Guided.
2. Vitest or grep ratchet named in What to build.
3. Record/Practice user copy (0097) on any new strings.
