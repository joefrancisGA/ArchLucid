# LY-016 — Cost honesty: Unchecked-row finalize completions

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** judge. **Depends on:** LY-005.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Document that Real finalize pays one Premium completion per judged Unchecked decision-grade row (cap via existing tenant AI budget / completion pipeline, not a new bucket).

## Why

Unbounded Unchecked sets can surprise the monthly wallet.
...
1. Reuse IAgentTierCompletionRouter + existing accounting. Do not add a separate judge wallet.
2. If a hard per-snapshot cap is missing, add a leftover note rather than a new product flag unless tests demand it.
