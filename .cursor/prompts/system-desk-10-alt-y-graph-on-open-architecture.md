# SY-10 — Alt+Y on Working opens Evidence graph as a desk verb

Do not fork AO-31 / PC-12. Interim: desk until SY-40 nested graph exists.

## Goal

Working Alt+Y → nested graph (SY-40) or architecture desk graph action — not bare `EVIDENCE_GRAPH_PATH`.

## Why

The graph is evidence of **this** system, not a tenant canvas you wander into from the atlas.

## Context

- `archlucid-ui/src/lib/evidence-graph-route.ts`
- `archlucid-ui/src/lib/resolve-working-insights-nav-href.ts`
- `archlucid-ui/src/lib/shortcut-registry.ts`

## What to build

1. Working graph shortcut resolver + Vitest.
2. Keep “Evidence graph” as the surface name (TB-2097); do not retitle the route “Evidence trail.”

## Acceptance criteria

- Working Alt+Y is architecture-scoped.
