# SY-38 — App Router: `/architecture/architectures/{architectureId}/compare`

Do not fork Compare engine (R12 still two sealed packages). Nested shell only.

## Goal

Nested Compare defaults to sibling jobs of this architecture (AO-29). No tenant-wide empty picker as the Working default.

## Why

Compare as `/insights/compare-two-reviews` is a second product. Nested Compare is a verb on the system.

## Context

- Compare two-reviews page
- `resolve-architecture-compare-defaults.ts`

## What to build

1. `architectureNestedComparePath`.
2. Thin nested page mounting existing Compare client with defaults.
3. Vitest defaults still siblings.

## Acceptance criteria

- Nested Compare does not require leaving the architecture prefix.
