# AS-095 — Hidden architecture: consistent 404 (document the choice)

**Wave:** architecture-spine (**AS**). **Cluster:** share-security. **Depends on:** AS-091.

Do not implement from the wave index. Implement only *What to build*.

## Goal

ADR 0087 should pick 404 (hide existence) or 403 (acknowledge). **Recommend 404** for restrict-to-shares to reduce title leakage via timing. Implement consistently across GET/list/export. Tests.

## Why

Mixing 403 and 404 leaks existence.

## Context

- AS-086 ADR — if ADR already said 403, implement that; do not bikeshed in code

## What to build

1. One helper for “not visible as not found.”
2. Tests both IDs.
