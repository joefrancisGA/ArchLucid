# IP-013 — Flip post-IR rows in the new leak inventories

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** ratchet. **Depends on:** each product IP it closes.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

After product IP-002–011 (and skip IP-012), `inhabit-post-ir-leak-inventories.ts` and `INHABIT_POST_IR_LEAK_INVENTORIES.md` match. Tests still inventory surfaces but do not require a leftover to stay leaking once that IP shipped. Closed `inhabit-leak-inventories.ts` IR/IH rows stay closed.

## Why

Without a ratchet, the next diagnosis will re-open shipped inspector leaks.

## Context

- `inhabit-post-ir-leak-inventories.ts` (IP-001) · `.test.ts` · markdown

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. For each shipped post-IR leftover, flip the matching boolean and `ownerPrompt`. Sync markdown. Do not revert remain/wave-34 inventories.
2. Do not rewrite `INHABIT_COMPAT_MATRIX.md`. Do not start wave 35.
3. This prompt-set PR does **not** flip product rows. Only implement IP-013 in the same session as (or after) the product IPs.
