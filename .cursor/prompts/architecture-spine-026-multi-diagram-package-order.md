# AS-026 — Multiple diagrams have stable order and ids on the package

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-desk. **Depends on:** AS-016.

Do not implement from the wave index. Implement only *What to build*.

## Goal

N diagrams on one review: stable sort (filename, then evidenceItemId). Graph merge is deterministic. Citations include evidenceItemId.

## Why

Two “network.drawio” files must not collide shape id `1`.
...
1. Id namespace = evidenceItemId + shape id.
2. Tests: two mermaid files with node A → two graph nodes or namespaced ids, never one merged A without ids.

## Acceptance criteria

- Deterministic merge; no silent collapse across files.
