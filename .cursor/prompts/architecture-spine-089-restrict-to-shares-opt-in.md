# AS-089 — Opt-in RestrictToShares on the architecture
...
API+UI to set RestrictToShares. Setting true while share list empty: fail-closed (would hide from everyone including setter) OR auto-insert Admin share for actor — pick **auto-insert actor as Admin** and document. Confirmation copy.

## Why

Empty restrict is a foot-gun.

## Context

- TB-2005 disable until valid
- AS-092 UI

## What to build

1. Server rule + tests empty list.
2. Audit (AS-093 can be same PR if small; else call AS-093).

## Acceptance criteria

- Cannot restrict to nobody.
