# IH-002 — Quote 0098 instrument vs 0100 inhabit

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** kernel-adr. **Depends on:** IH-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Document the split: 0098 is instrument chrome after spawn (H1, locator, inspector). 0100 is inhabited work: the afternoon’s document is findings on that architecture, not chrome alone.

## Why

SG shipped locator ≠ instrument. The paying day can still screenshot as a nested review job with honest H1. Inhabit is the remaining kernel.

## Context

- ADR 0098 · ADR 0100 · `SYSTEM_GRAVITY_ACCEPTANCE_2026-09-13.md`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Add a short quote block in ADR 0100 and `OPERATOR_UI_EXPERIENCE_MODES.md` (or inhabit help) that 0098 owns chrome identity and 0100 owns inhabited findings work.
2. Do not rewrite ADR 0098 body. Pointer only.
