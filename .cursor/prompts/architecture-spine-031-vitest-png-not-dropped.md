# AS-031 — Vitest ratchet: PNG is not silently dropped from intake

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-ratchet. **Depends on:** AS-004, AS-014.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Merge-blocking Vitest that a PNG File in `buildIntakeContextDocumentsFromEvidenceFiles` does not yield `[]` with no honesty document. Lock AS-002 inventory png row.

## Why

The next wizard cleanup will `return null` again without this.

## Context

- intake-context-documents-from-files.test.ts
- AS-002

## What to build

1. Add/extend tests.
2. Optional grep guard: `toIntakeContextDocument` still has no unguarded `return null` for images without a comment pointing at AS-005.

## Acceptance criteria

- CI fails if PNG disappears again.
