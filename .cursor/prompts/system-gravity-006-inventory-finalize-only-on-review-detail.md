# SG-006 — Inventory finalize CTAs that exist only on review-detail
...
Produce `docs/architecture/SYSTEM_GRAVITY_FINALIZE_EXILE_INVENTORY.md`: Finalize/stamp that cannot be reached from the architecture child-job row without exile. Shrink-only. Do not mutate copy yet (SG-016+).
...
- DecisionReceiptExportButton · finalize hash #finalize-review · first-review-guide-status.ts

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_FINALIZE_EXILE_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
