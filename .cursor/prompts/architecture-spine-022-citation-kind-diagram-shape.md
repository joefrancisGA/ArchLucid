# AS-022 — Evidence citation kind `diagram:` + shape id

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-cite. **Depends on:** AS-016.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Add a resolvable citation kind for diagram shapes/edges that `GenericArchitectureAdvicePatterns.HasConcreteEvidenceCitation` (or the TB-1221 Kind B resolver) treats as concrete package evidence — equivalent to `doc:` / ARM paths. Document the grammar.

## Why

Without this, diagram-derived findings demote as generic advice even when they point at a shape.

## Context

- DECISION_GRADE_FINDING_PROVENANCE_FAIL_CLOSED_CONTRACT.md
- GenericArchitectureAdvicePatterns
- ADR 0082 — do not rewrite the ADR body; add Related pointer if needed

## What to build

1. Resolver accepts `diagram:` refs that exist on the package.
2. Dangling `diagram:` is not concrete evidence.
3. Tests: present shape id → hasConcreteEvidence; missing id → false.
4. Honesty: `diagram:` is not semantic faithfulness (TB-1228).

## Acceptance criteria

- Diagram-grounded findings can stay Decision-grade when the shape exists.
