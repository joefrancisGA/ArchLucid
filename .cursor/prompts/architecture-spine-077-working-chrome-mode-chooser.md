# AS-077 — Working chrome: Career / Rehearsal chooser

**Wave:** architecture-spine (**AS**). **Cluster:** rehearsal-chrome. **Depends on:** AS-076.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Persistent Working control (command bar / desk header): Career vs Rehearsal. Not a buyer pill on Guided. Keyboard. Carbon.

## Why

Hidden execution mode in admin is not an all-day control.

## Context

- resolveProductionDeskChrome
- ADR 0080
- ADR 0081 label-only nav — chooser is not a nav row helper

## What to build

1. Chooser component + tests.
2. State: tenant pref or architecture pref — document; prefer architecture-level if mixed reviews exist, else tenant.
3. Do not use NEXT_PUBLIC_OPERATOR_EXPERIENCE to hide it on Working.

## Acceptance criteria

- Working header shows the door.
