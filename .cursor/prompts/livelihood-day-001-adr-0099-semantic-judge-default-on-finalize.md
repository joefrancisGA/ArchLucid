# LY-001 — ADR 0099: LLM judge default-on for Real finalize

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** kernel-adr. **Depends on:** none — run first.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Author `docs/architecture/adrs/0099-semantic-support-llm-judge-default-on-finalize.md` (Proposed). Real finalize/readiness runs the Premium semantic-support judge by default. Emit `EnableLlmJudge` stays false. Simulator/Fallback skip. Warn-not-block stays.

## Why

AS-074 / LN-025 left the judge off. Working Career Real finalize still stamps heuristic Unchecked on paraphrases the excerpts actually back.
...
1. Write ADR 0099 with Decision, Trade-offs, Constraints, Expected impact (include security), Consequences.
2. README row after 0098. Do not rewrite ADR 0085 body.
3. Guard tests: file exists; emit default remains false; finalize default true.
4. Do not flip host Mode. No G-REAL-06.
