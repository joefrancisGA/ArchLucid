# AS-058 — Async support-ratio job remains Lane B (optional enqueue)

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-score. **Depends on:** AS-056.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If RAG-V1-005 / eval_agent_faithfulness already enqueue, document Working consumption of the result into the band when present. Do not make the job blocking on execute. Do not start a second eval stack.

## Why

Lane B already exists. Working should display it when the row is there.
...
1. Read path + honesty “async, may lag.”
2. Tests: missing async row → Unchecked not Supported.
3. No new worker type unless an existing jobs CLI already has a slot — prefer existing.

## Acceptance criteria

- Execute latency unchanged.
