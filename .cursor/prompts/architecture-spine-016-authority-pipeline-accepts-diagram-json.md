# AS-016 — Authority pipeline compiles structured diagrams into the review graph

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-decide. **Depends on:** AS-006, AS-013.

Do not implement from the wave index. Implement only *What to build*.

## Goal

On execute, structured diagram documents compile into GraphSnapshot (or merge into the existing graph) **before** typed finding engines run. InferenceSource distinguishes StructuredParse from inventory ObservedFact (AS-050).

## Why

Parsers without a decide-path merge leave diagrams as dead JSON.

## Context

- AuthorityPipelineStagesExecutor / graph seed
- ADR 0084
- R5: missing properties → no invented resource

## What to build

1. Merge stage: diagram nodes/edges into graph; stable ids.
2. Do not overlay agent ProposedChanges here (TB-2221 stays the overlay gate).
3. C# tests: mermaid-only run has nodes; pixel-only run does not add topology nodes.

## Acceptance criteria

- Typed engines can relate to diagram nodes via RelatedNodeIds.
