# IH-039 — Inventory remaining toast-only amend leaks after IH-033
...
After IH-033, inventory any remaining Working finding mutate success that is toast-only with no row amend.

## Why

Call-site drift will recreate the five-minute cliff.

## Context

- `livelihood-document-guard-inventory.ts` pattern
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Shrink-only inventory + Vitest that new Working finding disposition success mounts include row amend or inspect history.
