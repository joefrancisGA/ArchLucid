# LY-015 — LLM judge prompt pack mitigations

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** judge. **Depends on:** LY-007.

Do not implement from the wave index. Implement only *What to build*.

## Goal

FindingSemanticSupportBandLlmJudgePrompts: Role, OutputContract, BandRules, FaithfulnessRules, ForbiddenBehaviors, LivelihoodHonesty. JSON only. No new findings.

## Why

A one-line “score the claim” prompt will invent Supported.
...
1. Keep prompts in ArchLucid.AgentRuntime. Tests assert disjoint/exact-quote/unsure→Unchecked language.
2. Do not send tenant ids in the user prompt.
