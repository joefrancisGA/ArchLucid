# AS-091 — IDOR tests: restricted architecture is not readable by unshared workspace members

**Wave:** architecture-spine (**AS**). **Cluster:** share-security. **Depends on:** AS-090.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Same-tenant user without share: GET architecture, GET review, list hub, export, bind, dispose → 404 or 403 per AS-095, never 200 with body. Cross-tenant still catalog-fail as today.

## Why

This is the security proof of ADR 0087.

## Context

- Existing IDOR test patterns
- ADR 0037
