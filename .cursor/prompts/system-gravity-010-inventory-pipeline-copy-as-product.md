# SG-010 — Inventory pipeline copy that is still the product
...
Produce `docs/architecture/SYSTEM_GRAVITY_PIPELINE_COPY_INVENTORY.md`: Working strings that teach intake → execute → wait → finalize as the object of the day. Shrink-only. Do not mutate copy yet (SG-016+).
...
- first-week-route-guidance.ts · CORE_PILOT.md · getting-started help

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_PIPELINE_COPY_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
