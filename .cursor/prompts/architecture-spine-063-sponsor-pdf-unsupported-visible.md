# AS-063 — Sponsor PDF cannot omit Unsupported decision-grade rows

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-desk. **Depends on:** AS-062.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Sponsor/first-value PDF: Unsupported findings remain visible (or a dedicated count + list). Do not bury in appendix-only if decision-grade. Simulator still rehearsal-labeled (LP-06).

## Why

The livelihood failure mode is a clean PDF of structurally cited falsehoods.

## Context

- SponsorFirstValuePdfGate
- LP-06

## What to build

1. PDF builder + tests with a fixture Unsupported row.
2. Do not unsell TB-983 Simulator ROI forbid.

## Acceptance criteria

- Unsupported appears on the sponsor artifact.
