# AS-028 — Sealed snapshot holds derived graph; original bytes stay submitted evidence

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-seal. **Depends on:** AS-016.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Honesty copy and exports: the sealed review record is the snapshot (graph + findings + trail). The vsdx/png is submitted evidence, not the seal. Do not call the file the sealed review record (ESI-07).

## Why

Livelihood: ARB must not think the Visio file is the governed decision.
...
1. Export/help/stamp sentences.
2. Tests: copy lint or unit on honesty helper.
3. Do not put raw vsdx bytes into GoldenManifest.

## Acceptance criteria

- Two objects stay distinct in UI and export.
