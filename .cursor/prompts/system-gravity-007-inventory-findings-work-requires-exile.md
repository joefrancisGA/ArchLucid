# SG-007 — Inventory findings work that requires leaving the desk
...
Produce `docs/architecture/SYSTEM_GRAVITY_FINDINGS_EXILE_INVENTORY.md`: Disposition, inspect, record correction, and checklist bands that only mount on review-detail. Shrink-only. Do not mutate copy yet (SG-016+).
...
- GovernanceFindingsQueueClient · FindingDispositionRecordCorrectionControl · architecture nested findings route

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_FINDINGS_EXILE_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
