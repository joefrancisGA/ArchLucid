# AS-011 — PlantUML / C4 text as structured diagram
...
Parse a bounded subset of PlantUML C4 (`System_Boundary`, `Container`, `Rel`) into AS-006. Fail-soft on activity/sequence diagrams (honesty: not architecture topology).

## Why

Many architecture repos already have C4-PlantUML next to code. That is the work object.

## Context

- AS-006
- C4-PlantUML syntax subset

## What to build

1. Subset parser documented in the contract.
2. Sequence-only files → NotVerifiable topology + stored file, no invented containers.
3. Tests: C4 container fixture; sequence fixture does not create fake systems.

## Acceptance criteria

- C4 text participates; sequence diagrams do not pretend to be topology.
