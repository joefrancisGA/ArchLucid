# SY-69 — Decision register defaults to the open architecture when one is in context

Do not hide the register. Scope default only.

## Goal

Working register: if open architecture id exists, filter/scope to that system with a visible chip. Clear chip → all jobs (inbox mode).

## Why

An unscoped register while a system is open is another tenant-wide product.

## Context

- Decision register page filters

## What to build

1. Default scope + chip + Vitest.

## Acceptance criteria

- Open architecture ⇒ register query includes architectureId.
