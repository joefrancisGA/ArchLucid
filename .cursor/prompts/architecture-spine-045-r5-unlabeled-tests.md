# AS-045 — R5 tests: unlabeled and pixel-only never mint resources

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-ratchet. **Depends on:** AS-019, AS-005.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Consolidate C#+Vitest matrix: unlabeled boxes, pixel-only PNG, empty PDF extract → 0 invented CanonicalObject resources.

## Why

One matrix test is the livelihood guard for diagram ingest.

## Context

- AS-019, AS-005, AS-039

## What to build

1. Table-driven tests.
2. Name them so AS-100 can cite them.
