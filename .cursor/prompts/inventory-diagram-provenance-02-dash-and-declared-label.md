...
Locked stroke for observed **and** declared: `#64748b` (current forest). Declared adds `stroke-dasharray="4 3"` (not IDA-06 peering `6 4`). Do **not** use `--al-accent-interactive` / teal on the line. If IDA-06 is present, never collapse/hide a declared on-path label.
