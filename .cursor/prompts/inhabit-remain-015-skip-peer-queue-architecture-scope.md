# IR-015 — Explicit skip: peer queue stays peer

**Wave:** inhabit remain leftovers (**IR**). **Not wave 35.** **Cluster:** skip. **Depends on:** IH-015, IH-024.

Do not implement from the remain index. Implement only *What to build*.

## Goal

Do not implement. Document that `/governance/findings?runId=` remains a run-scoped peer queue: `architectureIdKnown: false` and `returnsToArchitecture: false` are intentional.

## Why

IR-004/005 close dual-place on Working inspect and quick-decision. Agents must not “complete inhabit” by collapsing peer review into the inhabited document. Peer review is a second product on purpose (ADR 0098 / 0100 Guided/peer URLs).

## Context

- `INHABIT_FINDINGS_EDITOR_SURFACE_ROWS` governance-queue · `INHABIT_NESTED_REVIEW_BACK_HREF_ROWS` peer row · IH-076–078 skip style

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. No product change. Optionally add a one-line comment on the peer inventory rows: intentional peer queue; IR-015 skip.
2. Do not implement G-REAL-06, draft-diff, presence, unseal, or merge kernels in this session.
3. If this prompt is pasted, the session reports skip and exits. Ratchet: a test or comment that peer `returnsToArchitecture` stays false.
