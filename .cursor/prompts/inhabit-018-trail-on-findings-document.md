# IH-018 — Transparency trail on the findings document
...
Asserted / inferred / skipped MUST trail is visible on the inhabited findings document (or a non-modal drawer from it), not only on review Overview.

## Why

R4 trail is the liability precondition. Export-only trail is too late.

## Context

- ADR 0073 · PT-08 · `TransparencyTrailPanel` · FC-09 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Mount trail summary on Working nested findings. Missing trail still blocks Career finalize (0073).
2. Do not toast-only the trail.
