# IH-008 — Inventory: exploration still requires a clone ceremony
...
Inventory Working paths where trying an alternative means leaving the inhabited desk for a full Sketch / clone-from-snapshot ceremony with empty Compare inputs.

## Why

CE shipped Sketch a change. Compare stays committed-manifest. The inhabited afternoon still cannot think cheaply next to the current job.

## Context

- ADR 0092 · CE-001 · SN-038 skip · LY-046 skip
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. List desk/findings chrome missing Sketch entry, last-sketch sibling, or committed Compare from the open architecture.
2. Do not remount CE runner. Do not draft-diff Compare.
