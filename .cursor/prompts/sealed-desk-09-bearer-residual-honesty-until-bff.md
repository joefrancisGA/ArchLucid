...
**Obsolete after wave 12:** LK-05–07 **implement** the BFF. Do not paste this file to “document instead of fix.” If LK-05 has not landed, P0 honesty below remains valid; after P1/P2, replace the residual with closed-for-Working-GA language from those prompts.
...
Trust-center (or the canonical identity residual doc) states that the architect SPA still stores access tokens in `sessionStorage` (XSS can steal the session) until LK-05–07’s BFF. Staging/prod access-token TTL is short per ADR 0059 P0. No customer chrome banner. CLI Bearer remains supported. Do not implement the BFF here.

## Why

Livelihoods depending on the sealed record cannot sit on an unpublished XSS = account-takeover story. Implementing the BFF is LK-05–07 (dedicated PRs). Until that lands, silence is a casual-SPA posture.

## Context

- `docs/architecture/adrs/0059-spa-bff-http-only-session-plan.md` (still Proposed — leave Proposed until LK-05)
