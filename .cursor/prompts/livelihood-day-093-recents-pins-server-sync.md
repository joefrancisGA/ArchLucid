# LY-093 — Recents/pins server sync leftover

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** throughput. **Depends on:** SG-088 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Recents widget is architectures. Pins sync is server, not a new SoT in sessionStorage.

## Why

Client-only recents vanish across devices.
...
1. Leftover. BFF session remains SoT for auth, not recents if a server recents API already exists — reuse it.
