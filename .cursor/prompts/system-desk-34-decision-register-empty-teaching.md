# SY-34 — Decision register empty state does not send Working users to the reviews hub as Home

Do not change register data. CTA only.

## Goal

`decision-register-empty-teaching.ts` Working CTA: architectures portfolio or last-open desk.

## Why

Empty governance surfaces that “start a review” with no parent recreate the two-start-product bug.

## Context

- `archlucid-ui/src/lib/decision-register-empty-teaching.ts`

## What to build

1. Working href + copy + test.

## Acceptance criteria

- Working empty CTA is not `REVIEWS_NEW_PATH` without a parent architecture.
