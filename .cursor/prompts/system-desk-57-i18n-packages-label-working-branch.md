# SY-57 — `OPERATOR_NAV_LINK_LABELS.packages` Working branch

Companion to SY-56 if labels live in i18n constants.

## Goal

Do not globally rename Guided. Working shell uses Inbox.

## Why

Constants used by shortcuts and nav must not fight each other.

## Context

- `archlucid-ui/src/lib/i18n.ts`
- `operator-nav-labels.ts`

## What to build

1. Resolver `navLabelReviewsOrInbox(working)` + tests.

## Acceptance criteria

- Guided still allowed to say Reviews.
