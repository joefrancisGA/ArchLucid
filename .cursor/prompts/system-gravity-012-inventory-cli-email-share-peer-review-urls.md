# SG-012 — Inventory CLI/email/share peer review URLs
...
Produce `docs/architecture/SYSTEM_GRAVITY_OUTBOUND_URL_INVENTORY.md`: Outbound Working URLs that still print `/architecture/reviews/{id}` when parent architecture is known. Shrink-only. Do not mutate copy yet (SG-016+).
...
- working-share-href.ts · CLI open helpers · digest templates

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_OUTBOUND_URL_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
