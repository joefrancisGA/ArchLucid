# SG-008 — Inventory spawn that swaps primary chrome to the job
...
Produce `docs/architecture/SYSTEM_GRAVITY_SPAWN_CHROME_INVENTORY.md`: Start-review / spawn-lock handoff that replaces architecture H1 with review-only chrome. Shrink-only. Do not mutate copy yet (SG-016+).
...
- ArchitectureDraftHandoffPanel · architecture-draft-handoff-gate.ts · first-review-guide-status.ts

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_SPAWN_CHROME_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
