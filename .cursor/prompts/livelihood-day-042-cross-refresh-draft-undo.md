# LY-042 — Cross-refresh draft undo leftover

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** reversibility. **Depends on:** LY-041.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If this prompt is run, implement cross-refresh draft undo only as named here: restore last in-tab draft revision after reload via existing CAS, not a new kernel. If too large, record residual — do not fake it with sessionStorage as source of truth.

## Why

Refresh wiping undo is not editor-grade.
...
1. Prefer server draft history if it already exists. No localStorage source of truth (ADR 0089 leftover is 401 resume, not undo SoT).
2. Do not lengthen 300s.
