# LY-044 — Server undo optional — only if this prompt stays scoped

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** reversibility. **Depends on:** LY-042.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Do not add a general server undo log for sealed records. Optional: draft-only undo stack already implied by 0071. If implementing, CAS every write.

## Why

Unseal-via-undo is forbidden (0039).
...
1. Sealed records stay immutable. Draft only.
