# SY-37 — Working `/insights/ask-review-questions` redirects to nested Ask when architecture known

Depends on SY-36. Do not delete Guided peer Ask.

## Goal

Working + last-open or query architectureId → 308/replace to nested Ask (preserve search). Unscoped Working: send to architecture portfolio with honesty, not a blank Insights island. Guided: no redirect.

## Why

Peer bookmarks must not keep the peer product alive (same as AO-06 for reviews).

## Context

- Ask insights `page.tsx`
- `resolve-working-insights-nav-href.ts`

## What to build

1. Client or server redirect using last-open cache / search params.
2. Vitest on the href function.

## Acceptance criteria

- Working with last-open never stays on bare Insights Ask.
