# IP-006 — Completion notification opens inhabited findings

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** dual-place. **Depends on:** IP-001, IP-002.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

On Working, when the completed run has a known `architectureId`, the review-completion notification / toast deep link opens nested findings, not review-detail.

## Why

`use-review-completion-notification.ts` resolves review-detail. After analysis the architect should land on the afternoon document, not Overview.

## Context

- `hooks/use-review-completion-notification.ts` · `resolveWorkingInhabitedFindingsLandingHref`

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working + known architecture → inhabited findings landing. Guided/unlinked keep review-detail. In-flight activity inspector links stay as pipeline progress (not this prompt).
2. Do not auto-open presenter. Do not architecture-scope the peer queue.
3. Flip completion-toast inventory row. Owner `IP-006`. Focused Vitest on the href helper.
