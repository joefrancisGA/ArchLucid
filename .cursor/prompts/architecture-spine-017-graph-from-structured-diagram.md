# AS-017 — Deterministic graph compile from AS-006 (label-only confidence < 1)

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-decide. **Depends on:** AS-016.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Compiler: label-only type hints are DeterministicInference with confidence < 1 (IE-18). Never ObservedFact unless bound to inventory (AS-018/050).

## Why

A box labeled “SQL” is not proof of an Azure SQL server.

## Context

- IE-18 service-type dictionary note
- GraphEdge.InferenceSource

## What to build

1. Implement/adjust compiler confidence + provenance fields.
2. Tests: label-only node is not ObservedFact; bound inventory id is ObservedFact (may wait for AS-018).

## Acceptance criteria

- Sealed graph provenance is honest.
