# SY-45 — Working Insights nav rows are actions that keep the architecture prefix

Do not fork AO-40 bind-tool gate. After nested routes exist, hrefs must be nested helpers not gated peer links.

## Goal

`resolveWorkingInsightsNavHref` returns nested Ask/Compare/Graph/Search. Gate “no architecture open” → portfolio with honesty, not Insights home.

## Why

Sidebar Insights group still looks like a product line. It should look like verbs.

## Context

- `resolve-working-insights-nav-href.ts`
- `apply-working-bind-tool-nav-gate.ts`
- operate-analysis nav builder

## What to build

1. Point hrefs at nested builders.
2. Update AO-31/40 tests rather than duplicating.

## Acceptance criteria

- Working Insights Ask nav href contains `/architecture/architectures/`.
