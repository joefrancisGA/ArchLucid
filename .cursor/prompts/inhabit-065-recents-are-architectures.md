# IH-065 — Recents widget is architectures, not peer reviews

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** continuity. **Depends on:** SG-088 leftover, LY-098 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working recents list architectures (inhabit). Reviews appear as children, not peer Home objects.

## Why

Recents that are run IDs retrain the job as Monday.

## Context

- last-open architecture · SG-040 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working recents widget rows are architecture locators. Opening one lands desk or nested findings, not peer review Home.
