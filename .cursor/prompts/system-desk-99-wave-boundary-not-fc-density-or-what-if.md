# SY-99 — Written wave boundary: SY does not close false-confidence, density, or what-if cost

Docs only. Do not implement FC/DX/R12.

## Goal

`SYSTEM_DESK_COMPOSER_PROMPTS.md` + SY-100 audit: explicitly **not** in this wave — insight density engines, career-export honesty leftovers (FC), dual buyer skin, batch what-if pricing, live presence. Prevents SY PRs from smuggling those in.

## Why

A 100-prompt set will attract “while we’re here” density/FC work and never finish the object model.
...
1. Boundary section already in the owner index — add the same paragraph to the acceptance stub.
2. No product code.

## Acceptance criteria

- Acceptance stub lists out-of-scope issues #2+.
