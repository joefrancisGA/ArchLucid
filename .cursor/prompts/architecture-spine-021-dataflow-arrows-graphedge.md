# AS-021 — Diagram connectors become GraphEdges with diagram provenance
...
Edges from structured parse become GraphEdges. Citation `diagram:{evidenceItemId}:{shapeOrEdgeId}`. Do not infer protocol from line color.

## Why

Arrows are the architecture. Color is decoration.

## Context

- GraphEdge
- AS-022 citation kind

## What to build

1. Edge compile + provenance.
2. Tests: connector A→B yields edge; style-only properties ignored.
3. Do not invent reverse edges.

## Acceptance criteria

- Path engines can walk diagram-origin edges that also exist on the graph.
