# SY-25 — Draft intake already-submitted / handoff uses nested review URL on Working

Do not fork LK-04 / AO-07 spawn lock. Destination only.

## Goal

`architecture-draft-intake-mode.ts` and `GuidedIntakeAlreadySubmittedCallout` Working links use `resolveArchitectureReviewHref`. Guided keeps peer.

## Why

Handoff is the moment the product chooses the object you will live in.

## Context

- `archlucid-ui/src/lib/architecture/architecture-draft-intake-mode.ts`
- `archlucid-ui/src/app/(operator)/architecture/reviews/new/GuidedIntakeAlreadySubmittedCallout.tsx`
- `archlucid-ui/src/lib/architecture/architecture-draft-spawn-handoff-working-guard.test.ts`

## What to build

1. Thread architectureId.
2. Extend AO-07 guard.

## Acceptance criteria

- Working handoff href is nested when identity exists.
