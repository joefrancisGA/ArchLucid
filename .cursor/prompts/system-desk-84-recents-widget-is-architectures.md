# SY-84 — Working recents widget lists architectures, not peer review URLs

Do not fork last-open persistence. Widget mapping only.

## Goal

Recents on Home/palette: identity paths. A recent job appears as a subtitle on the architecture row, not as a separate top-level recent file.

## Why

Recents are literally Monday morning.

## Context

- Recents / last-open UI
...
- Recents hrefs are `architectureIdentityPath` when id known.
