# IH-034 — Disposition history on inspect, not toast-dependent

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** reversibility. **Depends on:** IH-033.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Inspect/panel shows prior dispositions for the finding (who/when/rationale) so undo expiry is not amnesia.

## Why

Professionals need the trail of their own afternoon, not a disappearing toast.

## Context

- ADR 0076 · audit trail · IS-12 reverse-with-audit
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working finding inspect shows disposition history. Empty history is honest, not hidden.
2. Do not invent finding-comment chat.
