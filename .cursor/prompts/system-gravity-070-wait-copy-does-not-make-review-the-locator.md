# SG-070 — Wait copy does not make the review the locator leftover

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** copy. **Depends on:** SG-018, DW leftover locator only.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working in-progress copy must not send the architect to live in review-detail as the object. Do not re-run DW-001–024. Do not invent progress URL.

## Why

Pipeline-as-product copy trains the job as Monday morning.

## Context

- SYSTEM_GRAVITY_PIPELINE_COPY_INVENTORY.md · CORE_PILOT.md · first-week-route-guidance.ts

## What to build

1. Fix Working leftovers only. Guided may keep evaluator stepper copy.
2. Vitest: Working fixture does not contain the forbidden pipeline-as-Home sentence named in the inventory row.
3. Do not re-run FD-12 / DW-003 bodies except leftover strings.
