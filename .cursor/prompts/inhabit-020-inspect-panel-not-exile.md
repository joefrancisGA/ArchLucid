# IH-020 — Finding inspect is a panel, not exile from the architecture

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-016, SG-027 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working finding inspect returns to architecture-scoped findings. Prefer a non-modal panel/drawer on the document over a peer route that drops the architecture identity.

## Why

Inspect as a new place trains the job as the object.

## Context

- SG-027 leftover · LY-025 leftover · HelpDrawerContent non-modal pattern
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Inspect back-target is architecture-nested findings. If a panel is used, it is press-triggered, non-modal, Escape returns focus.
2. Do not use hover-only inspect. Full strip if inspect stays inside the inspector.
