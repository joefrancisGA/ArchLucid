# AS-093 — Share grant/revoke/restrict: Required durable audit co-commit

**Wave:** architecture-spine (**AS**). **Cluster:** share-security. **Depends on:** AS-089.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Same-tx Required audit as approve/reject / ADR 0083 pattern. Fail closed if audit write fails.

## Why

Who could see a package is a career fact.
...
1. Event types + tests orphan probe does not fire.
2. Do not best-effort log.

## Acceptance criteria

- Share change without audit row is impossible.
