# AS-094 — Architectures hub and search honor RestrictToShares

**Wave:** architecture-spine (**AS**). **Cluster:** share-ui. **Depends on:** AS-091.

Do not implement from the wave index. Implement only *What to build*.

## Goal

List/search/global search (WA-19) omit restricted architectures the user cannot View. Counts on Home/hubs must match (HCP parity spirit — do not fork HCP).

## Why

A card in the hub is a leak.

## Context

- Architectures hub
- global search
- HCP-01–05 shipped — apply counting honesty, don’t re-run

## What to build

1. Query filters + tests.
2. Count parity test.

## Acceptance criteria

- Hub and search cannot leak titles of restricted packages.
