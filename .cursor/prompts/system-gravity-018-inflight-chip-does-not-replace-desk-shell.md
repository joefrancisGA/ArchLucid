# SG-018 — In-flight chip does not replace the desk shell

**Wave:** system-gravity (livelihood UX wave 32) (**SG**). **Cluster:** desk. **Depends on:** SG-016, DW leftover locator only.

Do not implement from the wave index. Implement only *What to build*.

## Goal

In-flight analysis is a chip on the architecture. It must not swap the shell to review-detail as Home. DW owns wait; this prompt owns locator/shell.

## Why

The day’s object is still a job if this leftover still screenshots as review-home.

## Context

- RunDetailInFlightDeskChrome · system-not-job-in-flight-review-on-desk · ADR 0096

## What to build

1. Implement only the leftover vs the named owner. Grep the inventory row first.
2. Vitest: Working fixture cannot treat this surface as Monday morning when ArchitectureId is known.
3. Guided/demo/trial may keep peer review URLs (AO-12 / SY-06).
