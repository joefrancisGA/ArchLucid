# AS-008 — SVG sanitize + structured parse (never execute as HTML)
...
Ingest SVG as structured diagram when `<rect>/<ellipse>/<text>/<line>/<path>` plus text labels can be grouped. Sanitize: strip script, foreignObject, event handlers. Do not render uploaded SVG as HTML in preview (ESI-04 already forbids executing SVG as HTML — keep that).

## Why

SVG is the common export from draw tools. Executing it is XSS. Parsing labels is architecture.

## Context

- ESI-04 preview constraints
- AS-006 schema

## What to build

1. Server-side sanitize + parse; fail-soft.
2. Preview continues to use a safe path (rasterize or sanitized img), never inline scriptable SVG.
3. Tests: SVG with onclick stripped; simple labeled boxes → nodes; XXE disabled if XML parser used.

## Acceptance criteria

- No XSS via diagram upload.
- Labeled SVG contributes nodes.
