# AS-059 — Support band enum on the finding wire

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-wire. **Depends on:** AS-056.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Closed enum: Supported, Unchecked, Unsupported, NotScored. NotScored = checklist/advisory exempt or Simulator unlabeled. OpenAPI + TS.

## Why

UI and export need one word.

## Context

- FindingClassification existing pattern
- OPENAPI_CONTRACT_DRIFT.md

## What to build

1. Contracts enum + snapshot.
2. Default Unchecked for decision-grade until scored.
3. Checklist → NotScored.
