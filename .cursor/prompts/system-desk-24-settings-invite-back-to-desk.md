# SY-24 — Settings roles invite success link is nested

Companion to SY-23 for `SettingsRolesInvitePanel.tsx`.

## Goal

`reviewDetailPath(inviteSentForReviewId)` → resolve with architecture id from the invite payload or a follow-up GET. Do not invent ACL.

## Why

Same leak as SY-23 on the roles panel.

## Context

- `archlucid-ui/src/app/(operator)/administration/users/_sections/SettingsRolesInvitePanel.tsx`
...
- Success link is not a peer review URL when architectureId known.
