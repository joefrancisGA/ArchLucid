# IR-008 — Finding card shortcuts default-focus first finding

**Wave:** inhabit remain leftovers (**IR**). **Not wave 35.** **Cluster:** keyboard. **Depends on:** IH-010, IH-059.

Do not implement from the remain index. Implement only *What to build*.

## Goal

On Working inhabited findings, `focusAdjacentFindingCard` / Alt+1/2/3 work when no card is focused by starting from the first `[data-finding-id]`. Nested page already default-focuses on load (IH-059); the hook must not no-op when unfocused.

## Why

`useFindingCardShortcuts.ts` returns if unfocused unless `startFromFirstWhenUnfocused` is true. Inventory: `defaultFocusFirstFinding: false`, `paletteWorkActionsFirst: false`, owner IH-010. `FindingKeyboardTriageHost` already passes the flag for J/K. Alt+1/2/3 still fail until the operator clicks a card — casual-tool keyboard, not all-day.

## Context

- `hooks/useFindingCardShortcuts.ts` · `FindingKeyboardTriageHost.tsx` (already `startFromFirstWhenUnfocused: true` for adjacent) · IH-059 nested page · no single-letter shortcuts

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. On Working inhabited findings, default `startFromFirstWhenUnfocused` for finding-card shortcuts (or always pass true from every inhabit host, including Alt+1/2/3). Do not steal focus from a dialog. No new single-letter shortcuts. Do not change Guided unless it shares the hook unsafely.
2. Do not invent a shortcut overlay trap (IH-062). Do not collapse tabs.
3. Flip the finding-card-shortcuts inventory row to `defaultFocusFirstFinding: true` (and `paletteWorkActionsFirst: true` if this hook is also the palette bridge). Owner `IR-008`. Focused Vitest: unfocused + cards present + inhabit host → first card focuses / first Alt+1 targets first finding.
