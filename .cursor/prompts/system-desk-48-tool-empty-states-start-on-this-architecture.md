# SY-48 — Nested tool empty states start a review on this architecture, never samples

Do not fork AO-19 / LD-02 live no-sample. Empty CTAs only.

## Goal

Nested Ask/Compare/Graph/Findings/Search empty: “Start review on {displayName}” → nested start. No showcase run ids. No Claims Intake.

## Why

Empty Insights islands were the evaluator recovery path. They must not return.

## Context

- Nested tool pages’ empty components

## What to build

1. Shared empty CTA helper + Vitest.
2. Live recovery context still forbids sample hrefs.

## Acceptance criteria

- Empty nested Ask CTA is nested start, not `SHOWCASE_STATIC_DEMO_RUN_ID`.
