# IH-066 — Pins / recents server sync leftover

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** continuity. **Depends on:** LY-093 leftover, LW out-of-wave.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working pins/recents that change which architecture you inhabit persist on the user-preferences API (localStorage cache). Last write wins.

## Why

Cross-device Monday is otherwise a different product.

## Context

- `GET /v1/user/preferences` · WA-14 leftover · LW residual
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. If pins/recents exist in UI, persist them on the account for Working. Theme may stay device-local. OpenAPI if the wire changes.
2. If server sync is still out of wave, record it in `INHABIT_OUT_OF_WAVE_RESIDUALS.md` — do not silently no-op without the residual row.
