# AS-029 — Help: structured diagrams are analyzed; pixels need extract

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-copy. **Depends on:** AS-003.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Update in-app `/help` evidence intake topics (not GitHub blob URLs). Say: mermaid, vsdx, draw.io, SVG labels feed the review graph; PNG/JPEG are stored and NotVerifiable until structured extract or Working opt-in vision.

## Why

Operator guide currently pushes PNG export — that is the defect we are reversing.

## Context

- PRODUCT_DOCUMENTATION_PRESENTATION.md
- evidence-intake-help-guide-content.ts
- EVIDENCE_INTAKE_OPERATOR_GUIDE.md (AS-030 may be same PR if small — prefer this prompt for /help, AS-030 for operator guide)

## What to build

1. Help topic + Vitest on key sentences.
2. TB-645.

## Acceptance criteria

- Help no longer tells Working architects that PNG is the preferred architecture input.
