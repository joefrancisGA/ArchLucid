...
IDP-02 dashes **Declared** only (`stroke-dasharray="4 3"`). ObservedFact, DerivedFact, DeterministicInference, and AiInference stay **solid** until IDP-04.

**IDA-06 collision (locked):** aesthetics prompts dash **peering** with `stroke-dasharray="6 4"` and may **hide** the on-path `peering` pill. That is a different meaning (“this fan is all peering”). Declared edges must keep the `declared` label even if IDA-06 would collapse duplicate pills. Do not reuse `6 4`. Do not start IDA from an IDP chat.
...
Suggested Cloud Agent branch per prompt: `cursor/inventory-diagram-provenance-<short-name>-41fd`. Implementation sessions use a **new** feature branch per prompt. This prompt-set PR lives on `cursor/idp-declared-edge-prompts-41fd`.
