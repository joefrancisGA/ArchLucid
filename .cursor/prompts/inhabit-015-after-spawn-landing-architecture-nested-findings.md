# IH-015 — After spawn, Working lands on architecture-nested findings

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** document. **Depends on:** IH-001, IH-004.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working post-spawn continue / last-open / Start-review success lands on architecture-nested findings when ArchitectureId is known — not review Overview and not `/governance/findings`.

## Why

0098 made the inspector not-Home. Inhabit needs the findings document as the afternoon landing.

## Context

- `resolveWorkingRunReviewLocator` · `architectureNestedFindingsPath` · SG-043 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working spawn success, continue-last, and unfinished-work hrefs prefer nested findings.
2. Guided keeps peer review URLs. Full tab strip if the inspector is opened.
