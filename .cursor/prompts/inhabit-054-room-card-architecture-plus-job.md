# IH-054 — Room card is this architecture plus the open job

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** room. **Depends on:** IH-053, SG-033 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Room card identity is architecture display name plus child job subtitle — same inhabit chrome as findings.

## Why

A room overlay that names only the run ID is a job again.

## Context

- LY-064 leftover · AO-38 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Room card header uses architecture identity. Job is subtitle. Share/print from room includes architecture URL (IH-056).
