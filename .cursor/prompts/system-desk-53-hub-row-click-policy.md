# SY-53 — Hub row: architecture name → desk; job title → nested job

Do not fork AD-07 columns. Click policy only.

## Goal

Verify/fix row click vs name link. Sticky identity columns at 1280px remain (AD-12).

## Why

Clicking a row into a peer run URL undoes nesting.

## Context

- Reviews hub table client

## What to build

1. Nested job hrefs + name → `architectureIdentityPath`.
2. Vitest if href helper exists.

## Acceptance criteria

- Job cell href nested when architectureId on the row.
