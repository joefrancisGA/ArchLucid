# SY-50 — Working Ask refuses to run unscoped

Do not fork WA-07 answer honesty. Gate the **session**, not the model.

## Goal

Working Ask without architectureId (path or open-context) does not call the Q&A API. Show desk picker / last-open. Guided may keep unscoped teaching.

## Why

Unscoped Ask is how the tool becomes a chatbot and the system stops being the object.

## Context

- Ask client submit path
- `resolve-open-architecture-job-run-id.ts`

## What to build

1. Client guard + optional API 400 for Working header if you already have a shell flag — prefer client+resolver only if API cannot see Working.
2. Vitest.

## Acceptance criteria

- Working Ask submit disabled until architecture scoped.
