# IH-025 — Record execute names Simulator incompleteness before start

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** default-day. **Depends on:** IH-005, IH-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

When Working Record is selected and host Mode is Simulator, the primary execute/Start CTA (or an adjacent readiness line) states that this workspace cannot produce a Career-complete sealed record until Real is provisioned. The CTA may remain enabled.

## Why

Blocking start would hide Practice. Unlabeled start is livelihood fraud. Honesty before start, gate at finalize (already CG).

## Context

- ADR 0091 · ADR 0086 · `shouldSuppressReadyToFinalizeForCareerHonesty`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working Record + Simulator: inline readiness near Start/execute. Not toast-only. Do not disable Start solely for Simulator.
2. Do not flip host Mode. Practice path stays unlabeled-complete-as-Practice.
