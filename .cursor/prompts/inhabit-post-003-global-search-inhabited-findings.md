# IP-003 — Global search run and finding stay on inhabited findings

**Wave:** inhabit post-IR leftovers (**IP**). **Not wave 35.** **Cluster:** dual-place. **Depends on:** IP-001, IR-004.

Do not implement from the post-IR index. Implement only *What to build*.

## Goal

On Working, global search resume for a run with known `architectureId` opens nested findings. Finding hits open nested findings with `focusedFinding` (reuse `governanceFindingInspectHref`), not `{reviewHref}/findings/{findingId}`.

## Why

`use-global-search-bar.ts` `navigateToRun` uses `resolveWorkingRunReviewLocator().href`. `navigateToFinding` appends `/findings/{id}` to the review inspector. Search is a daily resume verb; it currently exiles the afternoon document.

## Context

- `components/use-global-search-bar.ts` · `governanceFindingInspectHref` · `resolveWorkingInhabitedFindingsLandingHref` · package results panels that hard-code review-detail finding paths

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working + known architecture: `navigateToRun` → inhabited findings landing. `navigateToFinding` → `governanceFindingInspectHref` with `{ architectureId, isWorkingMode: true }`. Guided/unknown architecture keep current review locator.
2. Fix package/global search result panels that still hard-code `/architecture/reviews/{runId}/findings/{findingId}` on Working when architecture is known. Do not make search land on Overview.
3. Flip inventory search rows. Owner `IP-003`. Focused Vitest on the two navigators (and any panel href helper) for Working vs Guided.
