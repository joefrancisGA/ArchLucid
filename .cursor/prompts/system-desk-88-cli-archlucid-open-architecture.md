# SY-88 — `archlucid open` (or equivalent) opens the architecture locator

Do not invent a new CLI verb if `open` exists — change the URL it prints/opens. If no command exists, document the printed URL only (SY-29) and skip a new verb.

## Goal

Default open target is architecture identity when ArchitectureId is known.

## Why

CLI is the expert’s Start button.

## Context

- ArchLucid.Cli commands named open/print/url

## What to build

1. C# test + compile check if code changes.

## Acceptance criteria

- Command with architecture id does not print peer review Home.
