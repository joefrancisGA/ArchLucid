# SY-13 — Shift+? overlay lists architecture-desk work before inbox / Insights

Do not fork AD-10 / AO-43 overlay structure. Reorder Working groups only.

## Goal

Working shortcut help: group **This architecture** (start review, save draft, Ask, Compare, Graph, findings) above **Inbox** and **Workspace**. Do not list “Open packages list” as a primary item.

## Why

Discoverability still trains the old atlas even after hrefs change.

## Context

- `archlucid-ui/src/components/KeyboardShortcutProvider.tsx` (or help overlay consumer)
- `archlucid-ui/src/lib/shortcut-registry.ts` `ARCHITECTURE_DESK_PAGE_SHORTCUTS`

## What to build

1. Working overlay section order + copy.
2. Vitest on group order or fixture labels.

## Acceptance criteria

- Overlay first group is desk work, not reviews hub.
