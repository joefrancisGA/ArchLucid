# SG-005 — Inventory reviewDetailPath when ArchitectureId is known
...
Produce `docs/architecture/SYSTEM_GRAVITY_REVIEW_DETAIL_PATH_INVENTORY.md`: Working call sites that still mint `reviewDetailPath` although `ArchitectureId` is known (AO-08 / SY-77 leftovers). Shrink-only. Do not mutate copy yet (SG-016+).
...
- architecture-routes.ts · working-review-detail-path-import-guard.test.ts · resolve-working-run-review-locator.ts

## What to build

1. Grep Working UI/docs for the leak class. Write `SYSTEM_GRAVITY_REVIEW_DETAIL_PATH_INVENTORY.md` with path, string/call, whether AO/SY/SN already covers it, leak class.
