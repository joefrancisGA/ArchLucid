# AS-009 — draw.io / diagrams.net XML parse
...
Parse uncompressed draw.io XML (`mxfile` / `mxGraphModel`) into AS-006 nodes and edges. `.drawio` and `.xml` drawio files. Do not require a headless Electron draw.io runtime.

## Why

Working architects use diagrams.net constantly. XML is structured; PNG export is the lossy path we must stop depending on.

## Context

- AS-006
- draw.io mxCell edge/source/target

## What to build

1. Fail-soft parser; ignore unknown styles.
2. Wizard accepts `.drawio`.
3. Tests: small mxfile fixture → 2+ nodes and 1+ edge; compressed `mxfile` with zlib payload: either decompress or honesty “compressed draw.io not extracted” (do not pretend).

## Acceptance criteria

- Uncompressed draw.io participates in decide.
- Compressed-without-decompress is labeled, not silent.
