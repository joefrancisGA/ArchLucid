# AS-071 — JSON/ADR/print export includes support band

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-export. **Depends on:** AS-059.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Career JSON and ADR export include the enum + scorer version. Print uses the same counts as the stamp.

## Why

External ARB packets must carry the band.

## Context

- Career export paths
- FC validators — call evaluateCareerArtifactHonesty; do not rewrite FC-01–80
...
- Export round-trip includes band.
