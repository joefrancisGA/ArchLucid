# SY-47 — Durable open-architecture context drives nested redirects

Do not fork AO-48 server last-open. Reuse that id for SY-37/39/41/07.

## Goal

Single reader `getWorkingOpenArchitectureId()` used by shortcuts and Insights redirects. Source: URL nest → server pref → session last-open. Never infer by SystemName (AO-49).

## Why

Redirects that only see the URL will fail when the user is on Home or Inbox.

## Context

- `desk-continuity-architecture-locator.ts`
- `use-working-start-href.ts`

## What to build

1. Shared reader module + Vitest.
2. Do not store Bearer in JS.

## Acceptance criteria

- Home + last-open id ⇒ Ask redirect nests even from `/`.
