# IH-012 — Inventory: Working Home still teaches a pipeline
...
Inventory Working Home / first-session / empty desk copy that is create → execute → finalize rather than inhabit this architecture.

## Why

SG-056/057 and LY-039/040 named this. Inhabit fails if Home is still a stepper.

## Context

- SG-056 leftover · LY-039 leftover · PC-04
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. List Home heroes, steppers, sample CTAs, and Finish setup rows visible on Working production.
2. Guided may keep eval chrome.
