# IH-051 — Palette Sketch / new version when spawn-locked

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** exploration. **Depends on:** SG-081 leftover, CE-013 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working command palette includes Sketch a change / new version when the draft is spawn-locked. It does not unlock the Career editor.

## Why

Keyboard-only afternoon must still explore.

## Context

- `system-not-job-palette-clone-new-version.ts` · LY-097 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Palette work actions: Sketch a change when spawn-locked. Confirm clone-from-snapshot. No edit-anyway.
