# SY-21 — Room / presenter URL is architecture + job, not `?presenter=1` on a peer run

Do not fork AO-38 / DR-16 / PC-09. Leftover peer `reviewDetailPath` in `review-room-elicitation-url.ts`.

## Goal

Working room href uses nested review path + existing presenter/room query. Guided may keep peer.

## Why

Conference-room R4 still bookmarks a run if the room URL is peer.
...
1. Pass architectureId through room URL builder.
2. Vitest.

## Acceptance criteria

- Working room URL contains `/architecture/architectures/{id}/reviews/`.
