# IH-004 — Inventory: findings editor still lives on review-detail

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** inventory. **Depends on:** IH-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Inventory call sites where disposing or inspecting a finding requires treating nested review-detail as the place you live, not a tool on the architecture.

## Why

SG-019 nested the queue path. The live editor can still be the inspector.

## Context

- SG-019 leftover · LY-021 leftover · `GovernanceFindingsQueueClient.tsx` · architecture-nested findings path
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Table: surface, href, whether ArchitectureId is known, whether the editor is nested findings vs review-detail Overview.
2. Shrink-only. Do not re-run SG-019 body. IH-015–020 own the close.
