# AS-066 — Support band must not be a DeterministicInsightDensityGate term

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-hold. **Depends on:** AS-057.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Ratchet test: InsightDensityGateCandidate / Score does not read support band. Density stays ADR 0070. Band is a sibling signal.

## Why

Fusing would demote cited-but-paraphrased rows and hide the semantic problem inside “checklist.”

## Context

- DeterministicInsightDensityGate.cs
- ADR 0070 — do not change demotion predicate

## What to build

1. Architecture or unit test that Score ignores support band.
2. Comment in ADR 0085 follow-up.

## Acceptance criteria

- Gate file has no support-band dependency.
