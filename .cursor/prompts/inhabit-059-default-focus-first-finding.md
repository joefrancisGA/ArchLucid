# IH-059 — Default focus the first finding on inhabited findings

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** keyboard. **Depends on:** IH-010, PC-11 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working nested findings lands keyboard focus on the first `[data-finding-id]` (or a skip link to it) so Alt+1/2/3 work immediately.

## Why

A map of shortcuts is not a work surface until focus is on the work.

## Context

- `useFindingCardShortcuts.ts` · LY-091 leftover
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. On Working nested findings load (when cards exist), focus first card or provide a visible skip-to-findings control that focuses it.
2. Do not steal focus from a dialog. No single-letter shortcuts.
