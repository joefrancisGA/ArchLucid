# AS-073 — C# tests: citation present but quote mismatch → Unsupported

**Wave:** architecture-spine (**AS**). **Cluster:** semantic-ratchet. **Depends on:** AS-057.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Fixture: EvidenceRefs point at a real ARM excerpt; Message asserts the opposite. Band Unsupported. Structural provenance still Kind B present so persist as decision-grade (0082) with Unsupported band.

## Why

This is the livelihood exhibit.
...
1. C# test named for AS-100.
2. Do not demote to checklist solely for mismatch (AS-066).

## Acceptance criteria

- Mismatch is visible as Unsupported, not deleted.
