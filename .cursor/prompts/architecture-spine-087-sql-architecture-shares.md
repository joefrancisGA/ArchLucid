# AS-087 — SQL ArchitectureShares + RestrictToShares flag

**Wave:** architecture-spine (**AS**). **Cluster:** share-data. **Depends on:** AS-086.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Columns/tables: Architectures.RestrictToShares (bit default 0); ArchitectureShares (architectureId, actorOid or user id already used by UserRoles, role, grantedBy, grantedUtc, rowversion). Unified DDL + migration + rollback.
...
- dbo.UserRoles / Entra oid (ADR 0034)
- Single DDL file

## What to build

1. DDL in ArchLucid.sql + numbered migration.
2. Indexes for list-by-user.
3. Tests: default RestrictToShares = 0.

## Acceptance criteria

- Schema landed; default open.
