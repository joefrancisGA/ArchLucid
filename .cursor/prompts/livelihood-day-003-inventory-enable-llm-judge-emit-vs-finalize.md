# LY-003 — Inventory EnableLlmJudge vs EnableLlmJudgeOnFinalize

**Wave:** livelihood-day (livelihood UX wave 33) (**LY**). **Cluster:** inventory. **Depends on:** LY-001.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Inventory every bind of semantic-support LLM flags so emit cannot silently inherit finalize-on.

## Why

A single bool would put sync LLM on merge (TB-1228).
...
1. Grep EnableLlmJudge and EnableLlmJudgeOnFinalize. Table: path, emit vs finalize, default.
2. Do not merge the flags.
