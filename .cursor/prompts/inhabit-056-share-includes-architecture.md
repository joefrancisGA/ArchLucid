# IH-056 — Share from inhabited work includes the architecture URL

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** room. **Depends on:** SG-034 leftover, LY-063 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Copy link / invite / share from nested findings or inspector includes architectureId locator, not a peer review URL, on Working.

## Why

A shared job URL trains the recipient that the job is the object.

## Context

- `resolveInviteReviewerReviewHref` · SG-046 Guided split
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Working share/copy-link uses architecture nested locator when ArchitectureId is known. Guided may keep peer review URLs.
