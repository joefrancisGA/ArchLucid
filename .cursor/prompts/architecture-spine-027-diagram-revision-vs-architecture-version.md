# AS-027 — Diagram files are submitted evidence; they do not unseal or fork the architecture identity

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-desk. **Depends on:** AS-014.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Replacing a diagram is a new stored-file version on the draft/review, not a new `dbo.Architectures` row and not an unseal. After spawn lock, follow existing new-object CTA (WA-10). Do not merge kernels.

## Why

Architects iterate drawings all week. That must not look like a second product.

## Context

- ADR 0074 Architectures
- ADR 0068 two kernels
- WA-10 new draft from snapshot

## What to build

1. Document + if needed draft mutator: replace attachment on draft.
2. Sealed reviews: cannot replace; start new review (existing empty-state copy).
3. Tests: sealed package rejects diagram replace.

## Acceptance criteria

- Iteration stays on the architecture locator.
