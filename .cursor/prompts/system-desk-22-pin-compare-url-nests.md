# SY-22 — Pinned second context nests under the architecture

Do not fork AO-37 / DR-11. Leftover `reviewDetailPath` in `review-pin-run-url.ts`.

## Goal

Working pin URLs use nested paths. Do not collapse desktop review tabs into More.

## Why

Pin that mints a peer run tab re-creates two pipeline products.

## Context

- `archlucid-ui/src/lib/reviews/review-pin-run-url.ts`
- `archlucid-ui/src/lib/reviews/review-pin-run-url.test.ts`

## What to build

1. Nested hrefs + Vitest.
2. Empty pin still does not mint a blank peer review URL as Home.

## Acceptance criteria

- Pin href with architectureId is nested.
