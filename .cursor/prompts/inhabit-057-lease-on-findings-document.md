# IH-057 — Work-lease banner on the findings document

**Wave:** inhabit (livelihood UX wave 34) (**IH**). **Cluster:** room. **Depends on:** ADR 0090, LY-056 leftover.

Do not implement from the wave index. Implement only *What to build*.

## Goal

If a draft work-lease is active, Working nested findings still shows honest lease chrome (not presence) when the operator is in findings after spawn-lock — or explains that the draft is locked and the job is shared via CAS.

## Why

Lease that exists only on the draft editor disappears for the afternoon.

## Context

- ADR 0090 · `ArchitectureDraftWorkLeaseBanner.tsx`
- Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. Honest copy on Working findings: spawn-locked (no draft lease) vs CAS on findings. Steal remains confirm+audit on draft only.
2. No occupancy heartbeats. No occupancy avatars.
