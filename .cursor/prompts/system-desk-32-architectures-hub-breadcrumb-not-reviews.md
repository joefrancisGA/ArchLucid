# SY-32 — Architectures hub parent is Overview/portfolio, not the reviews list

Do not restore system-wide breadcrumbs (TB-2090). Fix the constant that still parents architectures to reviews.

## Goal

`ARCHITECTURES_HUB_BREADCRUMB_PARENT_HREF = REVIEWS_LIST_PATH` is inverted IA. Parent should be `/` or the architectures list itself — never the inbox.

## Why

The named systems list currently claims to be a child of jobs.

## Context

- `archlucid-ui/src/lib/architectures-hub-copy.ts`

## What to build

1. Change constant + tests + any header `navHref`.

## Acceptance criteria

- Architectures hub does not set reviews list as parent href.
