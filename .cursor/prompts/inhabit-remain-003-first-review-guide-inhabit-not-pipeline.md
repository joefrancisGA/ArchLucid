# IR-003 — First-review guide is inhabit-first

**Wave:** inhabit remain leftovers (**IR**). **Not wave 35.** **Cluster:** dual-spine. **Depends on:** IH-012, IH-027, IR-002.

Do not implement from the remain index. Implement only *What to build*.

## Goal

Working first-review guide does not present a first-session evaluator pipeline as the product. Findings work on this architecture is the afternoon; seal is a verb; sharing the package is after seal.

## Why

`FIRST_REVIEW_GUIDE_STEPS` / `FirstReviewGuideWalkthrough.tsx` still teach define → evidence → evaluate → review findings → address → seal → share. Better than core-pilot (findings before seal) but still a walkthrough of a job, not “you live in this system’s findings document.” Inventory: `teachesPipeline: true`, owner IH-027.

## Context

- `lib/first-review-guide-steps.ts` · `app/(operator)/architecture/first-review-guide/_sections/FirstReviewGuideWalkthrough.tsx` · IH-070 Guided/demo/trial keep eval · IR-002 owns core-pilot

Leftover owners named in Depends on — do not re-run those bodies.

## What to build

1. On Working, retitle/reorder first-review guide copy so the operator inhabits architecture-nested findings after spawn. Keep seal and share as later verbs. Do not send “Evaluate the architecture” to review-detail Overview as Home. Guided/demo/trial may keep the existing eval walkthrough (IH-070).
2. Do not collapse desktop review tabs. Do not add a pipeline stepper to Working Home (unfinished-work rail and identity desk already `teachesPipeline: false`).
3. Flip the first-review-guide inventory row to `teachesPipeline: false` and `ownerPrompt` to `IR-003`. Add/extend focused Vitest on step titles/hrefs for Working vs Guided.
