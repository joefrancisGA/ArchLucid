# AS-044 — Architecture desk shows diagram sources and extract status

**Wave:** architecture-spine (**AS**). **Cluster:** diagram-desk. **Depends on:** AS-015, AS-036.

Do not implement from the wave index. Implement only *What to build*.

## Goal

Working architecture desk (not a new Insights peer page — nest on the identity per ADR 0079): compact strip of diagram sources + analyzed/not extracted. Carbon density.

## Why

Monday morning: did my drawings feed the last review?
...
1. Strip component + tests.
2. Empty: honest empty, not a buyer teaching card on Working (ADR 0080).

## Acceptance criteria

- Visible on the architecture locator without opening a review tab hunt.
