# SY-51 — Reviews hub is absent from Working Home, Start, and empty heroes

Do not fork AO-26 hub layout. Role enforcement.

## Goal

Grep Working Home / empty / Start modules for `REVIEWS_LIST_PATH`. Remaining hits must be labeled Inbox or removed. Start resolver already forbids hub (AO-15) — keep it.

## Why

Any remaining hero button to the hub competes with the portfolio.

## Context

- Operator home Working modules
- Empty states

## What to build

1. Grep pass + fixes + extend AO-13/AO-50 guards.

## Acceptance criteria

- Working Home source has no `REVIEWS_LIST_PATH` as primary CTA.
