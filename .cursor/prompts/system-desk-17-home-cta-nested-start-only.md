# SY-17 — Working Home resume/start never uses peer start helper

Do not fork AO-13. Companion to SY-16 for the Home module specifically.

## Goal

`OperatorHomeWorkingPrimaryCta` and recents use identity path or nested start only.

## Why

Home is the other Monday-morning teacher besides Alt+R.
...
1. Fix remaining peer imports.
2. Extend the guard.

## Acceptance criteria

- Guard fails if Home Working source contains `REVIEWS_NEW_PATH` or `reviewDetailPath`.
