...
public sealed class InMemoryTenantSettingsRepositoryTests
{
    ...
    [Fact]
    public async Task UpsertAsync_rejects_whitespace_only_setting_value()
    {
        InMemoryTenantSettingsRepository repository = new();

        Func<Task> act = () => repository.UpsertAsync(Guid.NewGuid(), "feature.flag", "   ", CancellationToken.None);

        await act.Should().ThrowAsync<ArgumentException>();
    }

    [Fact]
    public async Task DeleteAsync_rejects_whitespace_only_setting_key()
    {
        InMemoryTenantSettingsRepository repository = new();

        Func<Task> act = () => repository.DeleteAsync(Guid.NewGuid(), "   ", CancellationToken.None);
