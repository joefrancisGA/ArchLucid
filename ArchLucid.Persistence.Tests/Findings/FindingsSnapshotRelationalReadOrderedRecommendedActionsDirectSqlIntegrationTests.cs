...
///     <c>dbo.FindingRecommendedActions</c> (<c>ORDER BY FindingRecordId, SortOrder</c>).
/// </summary>
    ...
    SqlServerPersistenceFixture fixture)
{
    ...
    public async Task LoadRelationalSnapshotAsync_returns_recommended_actions_in_SortOrder()
    {
        ...
        await connection.ExecuteAsync(
            new CommandDefinition(
                ...
                new
                {
                    ...
                    SortOrder = 0,
                    FindingId = "action-order-finding",
                    FindingSchemaVersion = 1,
                    FindingType = "ActionOrder",
                    Category = "Cat",
        ...
        await connection.ExecuteAsync(
            new CommandDefinition(
                insertAction,
                new { FindingRecordId = findingRecordId, SortOrder = 1, ActionText = "second-action-by-sort" },
                cancellationToken: CancellationToken.None));

        await connection.ExecuteAsync(
            new CommandDefinition(
                insertAction,
                new { FindingRecordId = findingRecordId, SortOrder = 0, ActionText = "first-action-by-sort" },
                cancellationToken: CancellationToken.None));
        ...
        f.FindingId.Should().Be("action-order-finding");
        f.RecommendedActions.Should().Equal("first-action-by-sort", "second-action-by-sort");
    }
}
