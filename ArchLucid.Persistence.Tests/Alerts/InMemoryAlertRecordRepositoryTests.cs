...
public sealed class InMemoryAlertRecordRepositoryTests
{
    ...
    public async Task CreateAsync_then_GetByIdAsync_returns_same_row()
    {
        ...
        AlertRecord alert = BuildAlert(alertId, AlertStatus.Open, BaseUtc, deduplicationKey: "k1");
    ...
    public async Task UpdateAsync_replaces_existing_alert_by_AlertId()
    {
        ...
        AlertRecord original = BuildAlert(alertId, AlertStatus.Open, BaseUtc, deduplicationKey: "k");
        await repo.CreateAsync(original, CancellationToken.None);

        AlertRecord updated = BuildAlert(alertId, AlertStatus.Resolved, BaseUtc.AddHours(1), deduplicationKey: "k");
    ...
    public async Task CreateAsync_trims_oldest_when_exceeding_MaxEntries_500()
    {
        ...
        AlertRecord first = BuildAlert(firstId, AlertStatus.Open, BaseUtc, deduplicationKey: "first");
        ...
        IReadOnlyList<AlertRecord> scope = await repo.ListByScopeAsync(TenantId, WorkspaceId, ProjectId, null, 600, CancellationToken.None);
    ...
    public async Task GetOpenByDeduplicationKeyAsync_returns_newest_Open_or_Acknowledged_in_scope_only()
    {
        ...
        await repo.CreateAsync(
            BuildAlert(Guid.Parse("20000000-0000-0000-0000-000000000002"), AlertStatus.Acknowledged, BaseUtc.AddHours(1), key),
            CancellationToken.None);
    ...
    public async Task GetOpenByDeduplicationKeyAsync_ignores_Resolved_and_wrong_scope()
    {
        ...
        AlertRecord? noResolved = await repo.GetOpenByDeduplicationKeyAsync(TenantId, WorkspaceId, ProjectId, key, CancellationToken.None);
        ...
        AlertRecord? noOtherTenant = await repo.GetOpenByDeduplicationKeyAsync(TenantId, WorkspaceId, ProjectId, key, CancellationToken.None);
    ...
    public async Task ListByScopeAsync_orders_newest_CreatedUtc_first_and_clamps_take()
    {
        ...
        IReadOnlyList<AlertRecord> list = await repo.ListByScopeAsync(TenantId, WorkspaceId, ProjectId, null, take: 2, CancellationToken.None);
        list.Should().HaveCount(2);
        list[0].CreatedUtc.Should().Be(BaseUtc.AddHours(3));
        list[1].CreatedUtc.Should().Be(BaseUtc.AddHours(1));

        IReadOnlyList<AlertRecord> defaultTake = await repo.ListByScopeAsync(TenantId, WorkspaceId, ProjectId, null, take: 0, CancellationToken.None);
        defaultTake.Should().HaveCount(3);

        IReadOnlyList<AlertRecord> maxCap = await repo.ListByScopeAsync(TenantId, WorkspaceId, ProjectId, null, take: 900, CancellationToken.None);
    ...
    {
        InMemoryAlertRecordRepository repo = new();
        await repo.CreateAsync(BuildAlert(Guid.Parse("31000000-0000-0000-0000-000000000001"), AlertStatus.Open, BaseUtc, "a"), CancellationToken.None);
        await repo.CreateAsync(BuildAlert(Guid.Parse("31000000-0000-0000-0000-000000000002"), AlertStatus.Resolved, BaseUtc, "b"), CancellationToken.None);

        IReadOnlyList<AlertRecord> openOnly =
            await repo.ListByScopeAsync(TenantId, WorkspaceId, ProjectId, status: "open", take: 50, CancellationToken.None);
    ...
    public async Task ListByScopePagedAsync_returns_total_and_respects_skip_take_with_MaxPageSize_clamp()
    {
        ...
        Task[] batch = Enumerable
            ...
            .Select(n => repo.CreateAsync(
                BuildAlert(Guid.Parse($"32000000-0000-0000-0000-0000000000{n:D2}"), AlertStatus.Open, BaseUtc.AddMinutes(n), $"k{n}"),
                CancellationToken.None))
            .ToArray();
        ...
        (IReadOnlyList<AlertRecord> page, int total) = await repo.ListByScopePagedAsync(
            ...
            null,
            skip: 2,
            take: PaginationDefaults.MaxPageSize + 50,
            CancellationToken.None);
