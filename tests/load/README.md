...
| **`large-evidence-package.js`** | — | `K6_LARGE_EVIDENCE_RATE` (`4` iters/s), `K6_LARGE_EVIDENCE_VUS` (`3`), `K6_LARGE_EVIDENCE_MAX_VUS`, `K6_LARGE_EVIDENCE_DURATION` (`45s`), `K6_LARGE_EVIDENCE_CONSTRAINTS` (`50`, API max), `K6_LARGE_EVIDENCE_CAPABILITIES` (`50`, API max), `K6_SUMMARY_PATH` | `k6 run tests/load/large-evidence-package.js` | **`handleSummary`** → `tests/load/results/large-evidence-package.json` |
...
Production-like profile for **`POST /v1/architecture/request`** with a large JSON body (max **50** constraints + **50** capabilities per **`ArchitectureRequestValidator`**, plus a near-max description). Uses **`constant-arrival-rate`** pacing (default **4** iters/s for **45s**) so a normal Development API (**`RateLimiting:FixedWindow:PermitLimit=2000`/min**) is not exhausted by a tight VU loop. This is **not** merge-blocking; scheduled workflow **`.github/workflows/k6-production-like-scheduled.yml`** runs it after **`post-commit-operator-path.js`** with API startup via **`scripts/ci/start_api_for_k6.sh`** (raised permit limit).
