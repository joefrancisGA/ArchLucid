...
/// <summary>HTTP coverage for <c>POST /v1/admin/aws-extractor/hosted/run</c>.</summary>
[Trait("Suite", "Core")]
[Trait("Category", "Slow")]
public sealed class HostedAwsExtractorRunEndpointTests(ApiKeyReaderAndAdminArchLucidApiFactory factory)
    : IClassFixture<ApiKeyReaderAndAdminArchLucidApiFactory>
{
    private const string EndpointPath = "/v1/admin/aws-extractor/hosted/run";
    ...
    public async Task Post_with_reader_key_returns_403()
    {
        ...
        using StringContent content = new("{\"connectionId\":\"aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa\"}", Encoding.UTF8, "application/json");
    ...
    [SkippableFact]
    public async Task Post_with_admin_key_and_missing_connection_id_returns_400()
    {
        using HttpClient client = factory.CreateClient();
        client.DefaultRequestHeaders.Add("X-Api-Key", ApiKeyReaderAndAdminArchLucidApiFactory.IntegrationTestAdminApiKey);
        IntegrationTestBase.WireDefaultSqlIntegrationScopeHeaders(client);
