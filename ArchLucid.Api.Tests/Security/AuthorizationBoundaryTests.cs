    ...
    : IClassFixture<ApiKeyReaderAndAdminArchLucidApiFactory>
{
    ...
    public async Task Reader_key_cannot_POST_run_commit_returns_403()
    {
        using HttpClient client = CreateReaderClient();
        using HttpResponseMessage response = await client.PostAsync(
            "/v1/architecture/review/00000000-0000-0000-0000-000000000000/finalize",
            null);
    ...
    [SkippableFact]
    public async Task Reader_key_cannot_GET_admin_config_lint_returns_403()
    {
        using HttpClient client = CreateReaderClient();
        using HttpResponseMessage response = await client.GetAsync("/v1/admin/config-lint");
