...
| **Runtime connections** | **SN-RT-01–SN-RT-13** (core) + **SN-RT-14** (prompt-ready) | Container Apps env → `app-settings-hosts.json`, parsers, SQL catalog grain, PaaS stages, RBAC allowlist, opt-in `dependency-observations.json` + `ObservedRuntime`, SQL principals companion, upload parsers, operator confirm (`HumanAssertion`), inference questionnaire API + workbench panel. **SN-RT-14** adds the missing OP-01 map row so Diagrams GETs stop 500ing `Unmapped API controller`. |
...
| SN-RT-14 | D/E | OP-01 map row for operator-inferred connections controller | **Prompt-ready** |
