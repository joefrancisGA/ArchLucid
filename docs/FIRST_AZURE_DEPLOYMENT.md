...
Provide a **preflight checklist**, **backend configuration** for Terraform remote state, and **apply order** aligned with [`REFERENCE_SAAS_STACK_ORDER.md`](REFERENCE_SAAS_STACK_ORDER.md). This doc does not replace each root’s `README.md` — read those for variable semantics.
...
Follow the **numbered table** in [`REFERENCE_SAAS_STACK_ORDER.md`](REFERENCE_SAAS_STACK_ORDER.md). Summary:
...
## Per-root workflow (repeat for each root in order)
