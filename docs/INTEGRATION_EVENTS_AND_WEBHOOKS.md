...
**Additional optional Logic App subscriptions (same module):** trial lifecycle email (`com.archlucid.notifications.trial-lifecycle-email.v1`), incident ChatOps (`alert.fired` **or** `alert.resolved`), and prod-only promotion customer notify (`event_type` + user property **`promotion_environment` = `prod`**). The API/worker sets `promotion_environment` on Service Bus messages for `com.archlucid.governance.promotion.activated` so SQL filters can target production without parsing the JSON body.

**Promotion customer channel preferences (read API):** `GET /v1/notifications/customer-channel-preferences` (Read authority, tenant-scoped JWT) returns `TenantNotificationChannelPreferencesResponse` — booleans for email / Teams / outbound-webhook customer notifications and `isConfigured`. If there is no row in `dbo.TenantNotificationChannelPreferences` (migration **082**), the API returns conservative defaults with `isConfigured: false`. Logic Apps can call this (HTTP + managed identity or API key as you wire connectors) before fan-out; there is no write API yet — operators insert/update SQL or use a future admin surface.
