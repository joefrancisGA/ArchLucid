...
**Live API E2E (Tier 2+ — CI `ui-e2e-live` + `ui-e2e-live-apikey`, merge-blocking):** `archlucid-ui/playwright.live.config.ts` uses **`testMatch: ["live-api-*.spec.ts"]`**. **`ui-e2e-live`** runs the **full** live suite under **DevelopmentBypass** (`timeout-minutes: 35`). **`ui-e2e-live-apikey`** starts the API with **`ArchLucidAuth:Mode=ApiKey`**, sets **`LIVE_API_KEY`** / **`LIVE_API_KEY_READONLY`**, and runs a **subset** (`live-api-apikey-auth`, `live-api-journey`, `live-api-negative-paths`) for production-like auth. Specs include: **`live-api-apikey-auth`** (skipped unless `LIVE_API_KEY` is set), **`live-api-journey`**, **`live-api-conflict-journey`**, **`live-api-governance-rejection`**, **`live-api-negative-paths`**, **`live-api-error-states`**, **`live-api-advisory-flow`**, **`live-api-replay-export`**, **`live-api-analysis-report`**, **`live-api-policy-pack-lifecycle`**, **`live-api-compare-runs`**, **`live-api-alert-rules`**, **`live-api-search-ask-graph`**, **`live-api-digest-webhook`**, **`live-api-concurrency`**, **`live-api-archival`**. **Nightly** (not a PR gate): **[`live-e2e-nightly.yml`](../.github/workflows/live-e2e-nightly.yml)** runs the **full** live suite under **DevelopmentBypass** and under **ApiKey** (forks skipped). Auth assumptions: **[LIVE_E2E_AUTH_ASSUMPTIONS.md](LIVE_E2E_AUTH_ASSUMPTIONS.md)**; parity matrix: **[LIVE_E2E_AUTH_PARITY.md](LIVE_E2E_AUTH_PARITY.md)**. Default **`playwright.config.ts`** uses **`testIgnore: "**/live-api-*.spec.ts"`** so mock smoke never calls the real API. Helpers: **`e2e/helpers/live-api-client.ts`**. Narrative: **[LIVE_E2E_HAPPY_PATH.md](LIVE_E2E_HAPPY_PATH.md)**. Run locally: `npx playwright test -c playwright.live.config.ts` (API on **`LIVE_API_URL`**; optional **`LIVE_API_KEY`** for ApiKey mode). CI sets **`LIVE_E2E_SKIP_NEXT_BUILD=1`**. See **[TEST_EXECUTION_MODEL.md](TEST_EXECUTION_MODEL.md)**.
