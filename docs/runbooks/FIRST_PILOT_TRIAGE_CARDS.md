...
**Last reviewed:** 2026-05-28 Â· Full spine: [`FIRST_PILOT_TROUBLESHOOTING.md`](FIRST_PILOT_TROUBLESHOOTING.md)
...
| --- | --- | --- | --- | --- |
| FP-T001 | Auth failure / 401 loop | Wrong `ArchLucidAuth:Mode`, missing API key/JWT | Correlation id from failed request | `archlucid auth diagnostics` Â· `/auth/signin` |
| FP-T002 | SQL / migration failure | Connection string, DbUp error | API startup log excerpt | `/health/ready` Â· [`SQL_SCRIPTS.md`](../library/SQL_SCRIPTS.md) |
...
| FP-T006 | Missing artifact | Commit not completed | Run id, manifest id | `GET /v1/architecture/review/{runId}` |
...
| FP-T011 | OpenAPI contract missing | Host not exposing canonical `GET /openapi/v1.json` | HTTP status and response body | `GET /openapi/v1.json` Â· [`API_CONTRACTS.md`](../library/API_CONTRACTS.md) |
...
| FP-T020 | Mutating route idempotency drift | New POST route lacks idempotency posture classification | `mutating-route-idempotency-posture.md` | [`MUTATING_ROUTE_IDEMPOTENCY_POSTURE.md`](../library/MUTATING_ROUTE_IDEMPOTENCY_POSTURE.md) Â· `python scripts/ci/detect_mutating_route_idempotency_drift.py` |
...
| FP-T024 | Decision-change ledger incomplete | Paid pilot closeout missing attributed decision changes or sponsor acceptance | `pilot-decision-ledger-report.json`, `pilot-decision-ledger.json` | Copy [`pilot-decision-ledger.template.json`](../go-to-market/templates/pilot-decision-ledger.template.json) to `artifacts/pilot-decision-ledger/<runId>/ledger.json` |
| FP-T025 | Paid-pilot baseline not kickoff-ready | Missing `baselineReviewCycleHours`, blocked source label, or waiver rationale before paid pilot start | `paid-pilot-baseline-readiness-report.json`, `paid-pilot-baseline.json` | Copy [`paid-pilot-baseline.template.json`](../go-to-market/templates/paid-pilot-baseline.template.json) to `artifacts/paid-pilot-baseline/<runId>/baseline.json` |
| FP-T026 | First non-obvious moment not captured | Pilot debrief missing first confirmed non-obvious finding moment or explicit not-yet-observed rationale | `first-non-obvious-moment-report.json`, `first-non-obvious-moment.json` | Copy [`first-non-obvious-moment.template.json`](../go-to-market/templates/first-non-obvious-moment.template.json) to `artifacts/first-non-obvious-moment/<runId>/moment.json` |
| FP-T027 | Dismissal-trigger not captured | Pilot debrief missing dismissal trigger category/evidence or explicit no-dismissal record | `pilot-dismissal-trigger-report.json`, `pilot-dismissal-trigger.json` | Copy [`pilot-dismissal-trigger.template.json`](../go-to-market/templates/pilot-dismissal-trigger.template.json) to `artifacts/pilot-dismissal-triggers/<runId>/dismissal.json` |
| FP-T028 | Top-severity finding challenge incomplete | Review close missing counter-argument, adjudication, rationale, or reviewer identity for top-severity finding | `top-severity-finding-challenge-report.json`, `sponsor-packet-appendix-top-severity-finding-challenge.md` | Copy [`top-severity-finding-challenge.template.json`](../go-to-market/templates/top-severity-finding-challenge.template.json) to `artifacts/top-severity-finding-challenge/<runId>/challenge.json` |
| FP-T029 | Reuse cohort tracker incomplete | Day-7/14/30 voluntary usage checkpoints missing or invalid | `pilot-reuse-cohort-tracker-report.json`, `pilot-reuse-cohort-tracker.json` | Copy [`pilot-reuse-cohort-tracker.template.json`](../go-to-market/templates/pilot-reuse-cohort-tracker.template.json) to `artifacts/pilot-reuse-cohort/<runId>/tracker.json` |
