...
| 3 | Run **Azure extractor Tier 1** in the customer subscription (PowerShell, read-only, no ArchLucid secrets in customer tenant). First pilots may use a one-time ZIP; production cadence is the customer-owned scheduled agent. | Script or scheduled job completes; ZIP contains `manifest.json` and cost/inventory payloads. | [`AZURE_EXTRACTOR.md`](../library/AZURE_EXTRACTOR.md) · [`AZURE_EXTRACTOR_SCHEDULED_AGENT.md`](AZURE_EXTRACTOR_SCHEDULED_AGENT.md) |
