...
| Routes normalized | Dynamic ids collapsed (e.g. `/reviews/[runId]`) ΓÇö see `telemetry-route-normalizer.ts` |
...
- `/reviews/[runId]` (or `/architecture/reviews/[runId]` after renames ΓÇö use whatever `route` dimension emits)
...
## 3. Interpretation ΓåÆ backlog mapping

Use **p75** (not a single outlier). Prefer routes with **ΓëÑ20 samples** after sampling.

| Dominant field signal | Likely cause | Prioritize (do this) | Do not start first |
|----------------------|--------------|----------------------|--------------------|
| **TTFB high** and `ttfbShareOfLcp` Γë│ **0.4**, or LCP poor while First Load JS CI green | API / SQL / SSR / proxy wait | **TB-2022** (slim first-paint / server path), **TB-2027** (loader/proxy waterfalls), older **TB-929**/**TB-930** peers if still open | Another client `dynamic()` wave |
...
| **INP poor** on list/inbox/queue routes | Main-thread / large DOM / fan-out on interaction | **TB-935** (list Query + virtualization), **TB-2023** (alerts inbox mount fan-out) | Marketing `next/image` (**TB-2032** waived ΓÇö no raster LCP today) |
| **CLS poor** | Layout shift (late fonts, unsized media, deferred chrome) | Fix the shifting surface; marketing image LCP ΓåÆ reopen **TB-2032** only when Lighthouse/field show **image-bound** LCP ([waiver](../architecture/tb2032_marketing_lcp_image_waiver.md)) | Bundle cut alone |
...
### Rough ΓÇ£good enoughΓÇ¥ bands (field, desktop-ish)
...
1. Run ┬º2.1 for the last 14 days (or since last cut).
2. Pick the worst **priority route ├ù metric**.
3. Apply ┬º3 mapping ΓåÆ open/prioritize **one** TB cluster (or confirm the already-open peer).
...
Monthly owner pass (**G-QA-06**): same queries; file or bump backlog when field CWV regresses ΓÇö do not invent new GTM IDs.
...
| `archlucid-ui/src/lib/telemetry/server-request-timing.ts` | BFF `Server-Timing` + slow proxy logs (UIΓåöAPI wait) |
| [`tb2032_marketing_lcp_image_waiver.md`](../architecture/tb2032_marketing_lcp_image_waiver.md) | **TB-2032** waived ΓÇö no marketing raster LCP |
