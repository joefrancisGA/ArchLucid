...
### Orientation
- **Lost-write Composer prompts (livelihood UX wave 23 — ready to run after AS close)** — [`LOST_WRITE_COMPOSER_PROMPTS.md`](LOST_WRITE_COMPOSER_PROMPTS.md) · [`.cursor/prompts/lost-write-00-index.md`](../../.cursor/prompts/lost-write-00-index.md) (**LW-001–LW-100**). Owner 2026-09-10: fail-close **in-flight writes** so a Working architect cannot silently lose or overwrite work. ADR **0088** (draft PATCH CAS mandatory unless `forceOverwrite`); ADR **0089** (401 resume, `localStorage`, inventoried livelihood mutates); ADR **0090** (work-lease without live presence). Offline queue must carry `expectedUpdatedUtc` and 409 into Keep mine. **Do not re-run** AS / FP / LP / V12-01. Distinct from Hasher robustness wave 23 ([`../library/ARCHITECTURE_REVIEW_ROBUSTNESS_WAVE23.md`](../library/ARCHITECTURE_REVIEW_ROBUSTNESS_WAVE23.md) items 221–230). No desktop **More** menu; no live presence avatars; no finding-comment chat; no G-REAL-06 host Mode flip.
- **v8 quality-ROI Composer prompts (ready to run — after DX-76 / assessment v8)** — [`V8_QUALITY_ROI_COMPOSER_PROMPTS.md`](V8_QUALITY_ROI_COMPOSER_PROMPTS.md) · [`.cursor/prompts/v8-quality-roi-00-index.md`](../../.cursor/prompts/v8-quality-roi-00-index.md) (**QR-01–QR-05**). Best credit ROI after insight density 66→70: compile `Decisioning.Tests`, align `typed-engine-scored` guard, OpenAPI snapshot, `merge_group` draft, then `security-baseline` evidence refs. **Do not** start DX-77. **Do not** re-run DX-01–DX-76 or WK-01–WK-22. No G-REAL-06 fake runs; no desktop **More** menu.
- **Architecture-spine Composer prompts (wave 22 — shipped on `cursor/as-prompt-queue-close-97a4`)** — [`ARCHITECTURE_SPINE_COMPOSER_PROMPTS.md`](ARCHITECTURE_SPINE_COMPOSER_PROMPTS.md) · [`.cursor/prompts/architecture-spine-00-index.md`](../../.cursor/prompts/architecture-spine-00-index.md) (**AS-001–AS-100**). Close audit: [`ARCHITECTURE_SPINE_ACCEPTANCE_2026-09-09.md`](ARCHITECTURE_SPINE_ACCEPTANCE_2026-09-09.md). Owner 2026-09-09: diagrams and bound inventory become **review decide inputs** (ADR **0084**); semantic support is a Working **career band** not a sync commit gate (ADR **0085** / TB-1228); Working **Career vs Rehearsal** doors without flipping host `AgentExecution:Mode` (ADR **0086**); optional **RestrictToShares** inside the tenant (ADR **0087**). **Do not re-run** FP / LP / ESI / IE collector bodies. No desktop **More** menu; no live presence; no finding-comment chat; no G-REAL-06 host Mode flip; no second Azure collector; vision default off. Share **API/UI** (AS-089–AS-099) shipped on the AS close audit. Concurrent work-lease + silent overwrite is livelihood UX **wave 23** ([`LOST_WRITE_COMPOSER_PROMPTS.md`](LOST_WRITE_COMPOSER_PROMPTS.md) **LW-001–LW-100**).
...
- **Evidence source inspect Composer prompts (ready to run)** — [`EVIDENCE_SOURCE_INSPECT_COMPOSER_PROMPTS.md`](EVIDENCE_SOURCE_INSPECT_COMPOSER_PROMPTS.md) · [`.cursor/prompts/evidence-source-inspect-00-index.md`](../../.cursor/prompts/evidence-source-inspect-00-index.md) (**ESI-01–ESI-08**). Review-screen open/preview + Download for **stored** attachments (documents, pictures, text). Catalog + authorized GET first; intake originals retained beside extracted text. Do **not** make citation-only inventory rows downloadable; do **not** call source files the sealed review record; no desktop **More** menu; no blob SAS URLs.
...
- **SecureNow architect Composer prompts (ready to run)** — [`SECURENOW_ARCHITECT_COMPOSER_PROMPTS.md`](SECURENOW_ARCHITECT_COMPOSER_PROMPTS.md) (**SA-01–SA-22** — live-inventory privilege / reachability / capability-to-flow paths on the operational stream; index [`.cursor/prompts/securenow-architect-00-index.md`](../../.cursor/prompts/securenow-architect-00-index.md)). Contract: [`../library/SECURENOW_ARCHITECT_PLANE.md`](../library/SECURENOW_ARCHITECT_PLANE.md). Do **not** add `IFindingEngine`, apply customer Azure, or a second collector. **SA-22** is a written hold.
