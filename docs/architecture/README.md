...
- **Option-preserving API split Composer prompts (ready to run)** — [`OPTION_PRESERVING_API_SPLIT_COMPOSER_PROMPTS.md`](OPTION_PRESERVING_API_SPLIT_COMPOSER_PROMPTS.md) (**OP-01–OP-08** — map, ratchet, product-line signal, 403 gate, composition facades, Worker inventory; second host and catalog splits are holds). Index [`.cursor/prompts/option-preserving-api-00-index.md`](../../.cursor/prompts/option-preserving-api-00-index.md). Do **not** add `SecureNow.Api` or a second `ArchLucid.sql` from the index. **PL-05** stays in force until **OP-07** is explicitly reopened.
