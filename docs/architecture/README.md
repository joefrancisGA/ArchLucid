...
### Orientation
- **Livelihood-desk Composer prompts (run these)** — [`LIVELIHOOD_DESK_COMPOSER_PROMPTS.md`](LIVELIHOOD_DESK_COMPOSER_PROMPTS.md) · [`.cursor/prompts/livelihood-desk-00-index.md`](../../.cursor/prompts/livelihood-desk-00-index.md) (**LD-01–15** — production desk identity, live never-sample, honesty beyond review-detail, every seal path honors MUST, amend on every registry path, Working empty desk, sealed-record nav home, palette first keydown, keyboard Finalize/save, remaining ops leakage, in-flight hub, remaining dirty editors, meeting elicitation, Insights graph list-first, teaching-chrome inventory). Each file names the LI/PT/WD owner; do not fork.
