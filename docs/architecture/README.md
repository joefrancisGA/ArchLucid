...
### Orientation
- **Finding-pointer CAS Composer prompts (wave 21 — run after LP persist gates)** — [`FINDING_POINTER_CAS_COMPOSER_PROMPTS.md`](FINDING_POINTER_CAS_COMPOSER_PROMPTS.md) · [`.cursor/prompts/finding-pointer-00-index.md`](../../.cursor/prompts/finding-pointer-00-index.md) (**FP-01–FP-24**). Owner 2026-09-08: attach ADR 0076 `expectedCurrentDispositionRowVersionBase64` on every production write that can move the current finding pointer. Inspect save, keyboard undo, restore, and bulk currently omit the token; keyboard apply and ITSM inbound already send it. Close audit is **FP-24**. **Do not re-run LP.** Do **not** implement LP-19 (401 resume) in this set. No desktop **More** menu; no live presence; no new ADR.
