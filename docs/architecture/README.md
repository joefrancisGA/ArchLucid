...
### Orientation
- **Livelihood-desk Composer prompts (run these first)** — [`LIVELIHOOD_DESK_COMPOSER_PROMPTS.md`](LIVELIHOOD_DESK_COMPOSER_PROMPTS.md) · [`.cursor/prompts/livelihood-desk-00-index.md`](../../.cursor/prompts/livelihood-desk-00-index.md) (**LD-01–15**). Each file names the LI/PT/WD owner; do not fork.
- **Repeat-seat Composer prompts (wave 3 after LD)** — [`REPEAT_SEAT_COMPOSER_PROMPTS.md`](REPEAT_SEAT_COMPOSER_PROMPTS.md) · [`.cursor/prompts/repeat-seat-00-index.md`](../../.cursor/prompts/repeat-seat-00-index.md) (**RS-01–15** — quiet-engine wizard CTA, coverage on seal, infeasible pending empty, one object after spawn, document-while-running, ops inversion, dirty-guard inventory, keyboard from open package, print opener keepalive, account-scoped ROI prefs, conflict desk, Unknown sentinel, workbench first paint, merge-conflict on list, Working empty presets). Each file names the LD/LI/PT/WD owner (or notes it is unique); do not fork.
