...
### Orientation
- **Working-seat Composer prompts (wave 19 — run after SY desk URLs)** — [`WORKING_SEAT_COMPOSER_PROMPTS.md`](WORKING_SEAT_COMPOSER_PROMPTS.md) · [`.cursor/prompts/working-seat-00-index.md`](../../.cursor/prompts/working-seat-00-index.md) (**WS-01–WS-24**). Owner 2026-09-07: **stop buyer polish on the Working seat.** ADR **0080**; Accept **0078** / **0079**; evict eval chrome and `/al-ui-rate` buyer remediations from Working; expert start; career defaults (show low-confidence; degraded coverage blocks finalize); continuity leftovers. Guided / demo / trial **keep** eval skin. **Do not re-run SY/FC/DX.** No desktop **More** menu; no finding-comment chat; no 40th engine.
