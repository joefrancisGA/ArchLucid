...
| `/snapshot/[runId]` | Read-only executive leave-behind (redirect) | R | Generated links | Retain as-is (redirect) |
...
| `/evolution-review` | Impact preview of proposed changes | P (extended) | Retain as-is; name is opaque — rename candidate (IA-021) |
...
| Route | Purpose | Nav | Disposition |
|---|---|---|---|
| `/settings` | Searchable settings hub | D (nav "Settings" targets `/settings/tenant`) | Promote: make `/settings` the nav target (IA-016; D5 resolved 2026-07-14 — hub-first) |
...
| `/settings/ai-usage` → rewrite → `/settings/cost-reporting` | Tenant LLM spend | P ("AI usage") | Retain alias; remove legacy `/admin/ai-usage-cost` page file (IA-014) |
...
40+ `next.config.ts` redirect rules (`/runs*`→`/reviews*`, `/manifests*`→`/signed-records*`, `/policy-packs*`→`/governance/policy-packs*`, `/audit*`, `/alerts*`, `/admin/users*`→`/settings/users*`, `/executive/dashboard`→`/dashboard`, `/portfolio`→`/dashboard`, etc.), 4 rewrite aliases, and 18 page-level redirects were enumerated and verified `[PI]`. All customer-visible links checked resolve — **except** the `/signed-records` index (Finding 1) and the deliberately legacy `/policy-packs` empty-state CTA (resolves via redirect; normalize anyway, IA-012).
...
**F. Administrator.** Path: Administration group → settings/users/identity/integrations health. Coherent; internal ops correctly separated. Failure points: `/settings` hub vs. nav-target `/settings/tenant` ambiguity; `/health` vs. `/admin/health` naming; internal rank names in error messages (Finding 9).
...
| `/scorecard` | `/executive/scorecard` | Finalized-review metrics vs. sponsor scorecard | A is operator analysis; B is sponsor-facing leave-behind | **High** — same word, different shells | Rename one (e.g., B → "Sponsor scorecard") and cross-link | Low — label change |
...
| Settings | Nav "Settings" → `/settings/tenant` | Breadcrumb "Workspace settings"; separate `/settings` hub | IA-016 |
...
│   ├── Impact preview              /evolution-review
...
**IA-004 · Fix the hub includes-list redundancy** — **P1 · XS**
...
**IA-008 · Hide Pattern library nav until content threshold** — **P1 · XS**
...
**IA-010 · Executive naming reconciliation** — **P1 · XS**
- **Problem:** Nav "Executive dashboard" → `/dashboard`; page vocabulary "Executive summary"; i18n key `portfolioOverview`; `/executive/scorecard` vs. `/scorecard` share a word across shells. `[PUI]`
...
- **Change (per D5 — hub-first):** Point the sidebar "Settings" item at `/settings` (the searchable index); `/settings/tenant` remains a first-class hub destination. Update the breadcrumb parent chain accordingly.
...
- **Problem:** "Impact preview" (`/evolution-review`), "Programs" (group), "Advisory scans" are labels a first-time user cannot map to intent; URL `evolution-review` does not match its label. `[PRV]` — validate with beta users before renaming.
...
| **D5** | Settings landing | **Hub-first** — the sidebar "Settings" item targets the searchable `/settings` index | IA-016 unblocked; breadcrumbs and nav label updated together |
