...
What the assessment does **not** recommend: wholesale restructuring. The nav group model, the tenantΓåÆworkspaceΓåÆproject scoping, the authority-rank gating, and the review-detail tab workspace are coherent and recently hardened by drift-guard tests. The brief's product framing (creation, evidence, governance, patterns, reporting as peer capabilities) is *ahead of* the data model ΓÇö evidence and architecture content exist only inside reviews ΓÇö and the IA should stay truthful to the shipped model rather than manufacture peer inventories the backend cannot support (consistent with `architecture_review_object_model_assessment.md` ┬º3ΓÇô4).

**Bottom line for private beta:** fix the four P0 items (all XSΓÇôS), land the eight P1 items opportunistically, instrument navigation, and defer structural changes (evidence hub, reports consolidation) until telemetry exists.
...
| 6 | Page-boundary quality | **62** | Review detail is one route with 8 tabs mixing object views (Architecture, Evidence, Findings), lifecycle actions (Review/finalize), and meta (Activity) ΓÇö workable but the "Review" tab (id `review-package`) is a label that does not describe its finalize/export content (`review-detail-workspace-tabs.ts:24`) `[PI]`. Executive reporting is split across six routes where tabs or one hub would serve (┬º6). Alerts vs. Alert rules as two nav items is correct (inbox vs. config). |
| 7 | Discoverability | **58** | Five interacting visibility gates (tier, authority, unlock phase, demo omissions, consolidated omissions) mean the same user sees different navs at different lifecycle moments `[PI]`. `/architectures` list, `/signed-records` list, `/settings/baseline`, `/digests`, `/value-report/roi` reachable only by deep link `[PI]`. Help is not a nav item (top-bar icon + contextual only) `[PI]`. Command palette lists destinations the sidebar hides `[PI]`. |
...
| 9 | Product-model clarity | **66** | One durable object, one lifecycle, one sequence ΓÇö and the nav caption states it `[PI]`. Deductions: home dual-path cards ("Create an architecture" / "Review an existing architecture") still frame two peer objects when both terminate in a `Run` (`buyer-polish-copy.ts`) `[PI]`; the signed record has a detail page but no home/inventory `[PI]`; draft vs. in-review architecture content has two live editors `[PI]`. |
| 10 | Administrative separation | **75** | Administration (customer) vs. Internal Operations (employee, feature-flagged, hidden in buyer shell) is a clean two-surface split `[PI]`. Deductions: `/health` ("System health", buyer-safe) vs. `/admin/health` ("Diagnostics dashboard") naming does not convey the distinction; nav "Settings" targets `/settings/tenant` while a `/settings` hub index also exists; cost reporting has three URLs (`/settings/ai-usage` rewrite ΓåÆ `/settings/cost-reporting`; legacy `/admin/ai-usage-cost`) `[PI]`. |
| 11 | Scalability | **60** | Nav config is centralized with drift guards (`NAV_CONFIG_CONTRACT.md`, `nav-config.structure.test.ts`) ΓÇö good `[PI]`. But the mode matrix (buyer-polished ├ù full-operator ├ù demo ├ù static-demo ├ù CTO tour ├ù governance view ├ù unlock phases ├ù dev overrides) is already at the edge of testability; the governance-view nav filter is implemented and tested but never wired into the live shell (`filterNavGroupsForGovernanceMode` unused) `[PI]`. |
| 12 | Private-beta readiness | **70** | Golden path, onboarding hub, sample workspace, and role gating are beta-ready `[PI]`. Blocked by: the P0 dead ends (┬º10); Pattern library nav that will render an empty aggregate for beta tenants (requires ΓëÑ5 contributing tenants, `PatternLibraryPageClient.tsx`) `[PI]`; absence of navigation telemetry to validate the progressive-unlock bet `[SMI]`. |

---

## 3. Top ten findings

1. **`/signed-records` (list) is a customer-visible 404.** `next.config.ts` rewrites `/signed-records` ΓåÆ `/manifests`, but no `manifests/page.tsx` index exists ΓÇö only `[manifestId]`. Breadcrumb map and command palette reference "Signed review records" as a parent concept. Any user who trims the URL from a signed-record detail, or follows a generated parent crumb, lands on a 404. `[PI]`
2. **Architecture drafts can be stranded.** `/architectures` (draft list) is not a configured nav item; it is reachable only from within the create flow ("Save and exit"). A user who saves a draft, signs out, and returns has no visible navigation path to resume it ΓÇö "Create architecture" starts a *new* draft. `[PI]`
3. **The signed record ΓÇö the product's core proof artifact ΓÇö has no home.** It is reachable only from a review detail or a governance link. For an audit/governance-lead persona whose primary question is "show me every signed decision," the closest surface is `/governance/decision-register`, whose relationship to signed records is not stated on either page. `[PI]`
4. **Six executive/reporting surfaces overlap.** `/dashboard` (canonical), `/executive/dashboard` (redirected but page file retained), `/executive/scorecard` (separate shell, not in operator nav), `/scorecard` ("Review scorecard", Insights group), `/governance/dashboard` (deep-link only, redirects home in demo), and the `/value-report` trio. Each has a defensible purpose; nothing in the IA tells a user which one answers which question. `[PI]`
...
Notation ΓÇö **Nav:** P = primary sidebar (when gates pass), S = secondary/in-page, D = deep-link/card/palette only, R = redirect, X = orphaned/unreachable. **Disposition** uses the classification set from ┬º9.1. Route groups `(operator)`, `(marketing)`, `(executive)` do not appear in URLs. Full per-route guard detail was verified against page files and `nav-config.ts`; guards summarized here.

### 4.1 Core review lifecycle (persona: architect, reviewer)

| Route | Purpose | Nav | Entry points ΓåÆ next actions | Disposition |
|---|---|---|---|---|
| `/` | Workspace overview: metrics, setup cards, recent reviews, next-best-action | P ("Overview") | Landing ΓåÆ `/reviews/new`, `/reviews`, `/dashboard`, `/onboarding` | Retain with revised purpose (fix dual-path cards, IA-005) |
| `/architectures/new` | Bootstrap a new architecture draft | P ("Create architecture") | Nav, home card ΓåÆ draft editor | Retain as-is |
| `/architectures/[architectureId]` | Long-lived draft editor (autosave) | D | From bootstrap/list ΓåÆ "Start architecture review" | Retain with revised purpose (hand-off lock, IA-007) |
| `/architectures` | Draft list/resume | **X (near-orphan)** | Only "Save and exit" from editor | **Promote to reachable** (IA-002) |
| `/reviews` | Reviews hub: summary, start CTA, samples, inventory | P ("Reviews") | Nav (Alt+R) ΓåÆ detail, new | Retain as-is (post-consolidation) |
| `/reviews/new` | Start-review intake (quick / guided / templates paths) | P ("Start review") | Nav (Alt+N), hub, home ΓåÆ `/reviews/[runId]` | Retain as-is; `?intent=create-architecture` redirect retained |
| `/reviews/[runId]` | Review detail workspace, 8 tabs (`?reviewTab=`) | D | Hub table, home, palette ΓåÆ findings, evidence, finalize, governance | Retain with revised purpose (rename "Review" tab, IA-003) |
...
| `/signed-records` (list) | **404 ΓÇö rewrite target has no index page** | **X** | Breadcrumb parent, palette concept | **Add index page in Governance group** (IA-001; D1 resolved 2026-07-14) |
...
| `/evolution-review` | Impact preview of proposed changes | P (extended) | Retain as-is; name is opaque ΓÇö rename candidate (IA-021) |
| `/scorecard` | Review scorecard (finalized-review metrics) | P (extended) | Move/merge into reporting family (IA-009, deferred) |
| `/patterns`, `/patterns/[patternKey]` | Anonymized pattern library (needs ΓëÑ5 tenants) | P (flagged) | **Hide from private beta** until data exists (IA-008) |
| `/planning`, `/planning/plans/[planId]` | Improvement themes and plans | P (advanced) | Retain as-is |
| `/product-learning` | Pilot feedback themes | P (advanced; demo ΓåÆ `/`) | Retain as-is |
| `/recommendation-learning` | Recommendation tuning profiles | P (advanced; demo ΓåÆ `/`) | Retain as-is |
| `/replay` | Validate stored review output integrity | Internal Ops nav | Retain as-is (system-admin surface) |
| `/operate/architecture-graph` | Legacy redirect ΓåÆ `/graph` | R | **Remove** page file after redirect verified in config (IA-014) |

### 4.3 Governance (persona: governance/risk lead, reviewer)

| Route | Purpose | Nav | Disposition |
|---|---|---|---|
| `/governance` | Approval queue / workflow (approve, reject, promote) | P ("Approval queue", phase ΓëÑ1) | Retain as-is |
...
| `/governance/resolution` | Effective policy / precedence diagnostics | P ("Standards & rules") | Retain as-is |
...
| `/governance/first-30-days` | Governance operating-rhythm guide | P ("Governance setup guide") | Convert to help topic or keep ΓÇö Requires product decision (IA-022) |
| `/governance/dashboard` | Workspace-health KPI tiles | D only (consolidated omission) | **Remove now** ΓÇö redirect to `/dashboard`, fold unique tiles in (IA-009 D6 carve-out; resolved 2026-07-14) |
| `/governance/approval-requests/[id]/lineage` | Approval lineage graph | D | Retain; add parent detail route (IA-015) |
| `/advisory` (`?tab=scans\|schedules`) | Recommendations from finalized reviews | P ("Advisory scans", Governance group) | Move within navigation? Borderline ΓÇö retain, monitor (IA-022) |
| `/alert-routing`, `/advisory-scheduling` | Legacy redirects | R | Retain redirects; remove page files where config covers them (IA-014) |
| `/settings/alerts` | Retired pre-release path | ΓÇö | Removed (no redirect); use left nav Alert rules (`/governance/alert-rules`) |
...
| `/executive/scorecard` | Sponsor scorecard, standalone executive shell | D (from dashboard) | Retain with revised purpose; label relationship to `/scorecard` (IA-010) |
| `/value-report` | Sponsor DOCX value report (Execute+) | P (advanced) | Retain as-is |
| `/value-report/pilot` | Evaluation value report | D (consolidated omission) | Convert to tab of `/value-report` ΓÇö defer (IA-009) |
| `/value-report/roi` | ROI hours estimate | D (consolidated omission) | Convert to tab of `/value-report` ΓÇö defer (IA-009) |
...
| `/settings/exec-digest` | Retired pre-release path | ΓÇö | Removed (no redirect); use Digests ΓåÆ Schedule (`/digests?tab=schedule`) |
| `/example-roi-bulletin` | Synthetic ROI bulletin sample | D (marketing) | Hide from private beta operator space ΓÇö Requires product decision |
...
| Route | Purpose | Nav | Disposition |
|---|---|---|---|
| `/settings` | Searchable settings hub | D (nav "Settings" targets `/settings/tenant`) | Promote: make `/settings` the nav target (IA-016; D5 resolved 2026-07-14 ΓÇö hub-first) |
| `/settings/tenant` (+ `recycle-bin`) | Workspace settings | P ("Settings") | Retain; nav label vs. breadcrumb "Workspace settings" mismatch (IA-006 family) |
| `/settings/users` (+ `invite-reviewer`, `?tab=users\|roles\|keys`) | Users, roles, keys | P | Retain as-is |
| `/settings/identity-providers` (+ saml, oidc, role-mapping, diagnostics), `/settings/identity/sso-wizard`, `/settings/scim-provisioning` | Identity administration | P (advanced) | Retain as-is |
| `/settings/api-keys` | Automation keys | P (advanced; omitted in buyer shell) | Retain as-is |
| `/settings/billing` | Billing and plans | P | Retain as-is |
| `/settings/ai-usage` ΓåÆ rewrite ΓåÆ `/settings/cost-reporting` | Tenant LLM spend | P ("AI usage") | Retain alias; remove legacy `/admin/ai-usage-cost` page file (IA-014) |
| `/settings/security-trust` | Procurement/trust materials | P | Retain as-is |
| `/settings/support` | Support and diagnostics bundle | P | Retain as-is |
| `/settings/preferences` | Personal preferences | S (settings hub) | Retain as-is |
| `/settings/baseline` | ROI baseline methodology | D (intentional deep link from ROI surfaces) | Retain as-is |
| `/settings/extract-upload` | Azure extractor upload | D (pilot checklist deep link) | Retain as-is |
| `/settings/developer` | Internal dev tools (redirects unless system-admin) | X-ish | Retain as-is (internal) |
...
| `/demo/explain` | Internal demo provenance tooling ΓÇö no inbound nav links | **Hide from private beta** / move behind demo tooling env (IA-014) |
| `/why-archlucid` | Internal proof page; redirects away in buyer shell | **Remove or fold into `/why`** (IA-014) |

### 4.9 Redirect ledger

40+ `next.config.ts` redirect rules (`/runs*`ΓåÆ`/reviews*`, `/manifests*`ΓåÆ`/signed-records*`, `/policy-packs*`ΓåÆ`/governance/policy-packs*`, `/audit*`, `/alerts*`, `/admin/users*`ΓåÆ`/settings/users*`, `/executive/dashboard`ΓåÆ`/dashboard`, `/portfolio`ΓåÆ`/dashboard`, etc.), 4 rewrite aliases, and 18 page-level redirects were enumerated and verified `[PI]`. All customer-visible links checked resolve ΓÇö **except** the `/signed-records` index (Finding 1) and the deliberately legacy `/policy-packs` empty-state CTA (resolves via redirect; normalize anyway, IA-012).
...
**A. Enterprise/solution architect creating an architecture.** Goals: capture a design, refine over sessions, get it evaluated. Path: "Create architecture" ΓåÆ draft editor ΓåÆ "Start architecture review" ΓåÆ guided intake ΓåÆ run detail. Works end-to-end `[PI]`. Failure points: no way back to a saved draft from navigation (Finding 2); after the review spawns, the draft remains editable in parallel with the review's own Architecture tab, with no lock or hand-off message beyond an "Open linked review" link `[PI]`; no diagram/modeling canvas exists ΓÇö creation is form + text + import, which the marketing framing ("architecture creation platform") over-promises `[SMI]`.

**B. Architect preparing an architecture for review.** Path: `/reviews/new` (three intake paths) ΓåÆ evidence tab ΓåÆ pipeline ΓåÆ findings. Works; the path switcher (quick / guided / templates) is the clearest multi-path intake in the app `[PI]`. Terminology stays consistent post-consolidation. Weak point: evidence completeness expectations appear inside the wizard but there is no pre-flight "what evidence will I need" summary reachable before starting (help topic `evidence-intake` exists but is contextual only) `[PRV]`.

**C. Reviewer examining evidence and asking questions.** Path: hub ΓåÆ run detail ΓåÆ Findings/Evidence tabs ΓåÆ finding inspect ΓåÆ `/ask`. Works. Weak points: `/ask` is nav-visible pre-finalization but requires a finalized record (subtitle states it) ΓÇö an invitation to a dead end for a mid-review reviewer `[SMI]`; the graph/trail naming split (Finding 7) forces the reviewer to learn that "Evidence graph," "Evidence trail," and "Provenance" are one capability `[PUI]`.

**D. Governance or risk leader.** Path: Governance group ΓåÆ approval queue ΓåÆ decision register ΓåÆ audit trail. Coherent once unlocked. Failure points: at unlock phase 0/1 most of this group does not exist in nav (Finding 5); the signed-record inventory gap (Finding 3) means "show me all signed decisions" lands on the decision register, which does not explain its relation to signed review records; approval request has lineage but no detail page `[PI]`.

**E. Executive seeking status/value/portfolio insight.** Path: `/dashboard` (sponsor-only users are hard-redirected there). Strong single landing `[PI]`. Failure point: the six-surface reporting overlap (Finding 4) ΓÇö an executive who is *also* an operator sees "Executive dashboard," "Review scorecard," "Value report," and (via palette) "Digests," with no surface explaining which answers what.

**F. Administrator.** Path: Administration group ΓåÆ settings/users/identity/integrations health. Coherent; internal ops correctly separated. Failure points: `/settings` hub vs. nav-target `/settings/tenant` ambiguity; `/health` vs. `/admin/health` naming; internal rank names in error messages (Finding 9).

**G. First-time evaluator (demo/private-beta workspace).** Strongest persona support in the app: sample workspace badge, seeded sample review, golden-journey strip, dual-path home cards, onboarding hub `[PI]`. Failure points: the dual-path cards mislead about the object model (Finding 6); demo-blocked route prefixes silently trim most of the app, which is intended but means beta feedback on IA breadth requires the *non*-demo shell; Pattern library will render a below-threshold empty aggregate (Finding 10 ΓåÆ IA-008).

### 5.2 Core workflows

| Workflow | Natural start | Current start | Verdict and defects |
|---|---|---|---|
| Architecture creation | "Create architecture" | Same `[PI]` | Continuous. Defects: draft-list orphan (P0); dual live editors post-spawn; creation terminates in a `Run` displayed via a query-param-switched workspace view (`fromGeneration=1`) ΓÇö invisible to users but a fragile page boundary `[PI]` |
...
| Governance and approval | Finalize ΓåÆ approval queue | Same | Continuous post-unlock. Defects: unlock gating pre-first-commit; no approval-request detail route; governance-mode vocabulary half-wired |
| Evidence | Review intake / connectors | Same | Works review-scoped. Defect: no cross-review evidence landing; naming split; classification is implicit (policy packs / severity), with no classification surface ΓÇö acceptable for the current model, should be stated in help `[SMI]` |
| Patterns and reuse | `/patterns` | Same | Will be empty for beta tenants (threshold ΓëÑ5 tenants) ΓÇö hide nav until populated |
...
|---|---|---|---|---|---|---|
| `/dashboard` | `/executive/dashboard` | Identical dashboard component (`surface="executive"`) | Chrome only; A is canonical, B config-redirected | Low (redirect live) | **Remove B's page file**; keep redirect | None ΓÇö redirect already permanent `[PI]` |
...
| `/scorecard` | `/executive/scorecard` | Finalized-review metrics vs. sponsor scorecard | A is operator analysis; B is sponsor-facing leave-behind | **High** ΓÇö same word, different shells | Rename one (e.g., B ΓåÆ "Sponsor scorecard") and cross-link | Low ΓÇö label change |
| `/value-report` | `/value-report/pilot`, `/value-report/roi` | Value reporting family | DOCX generator vs. read-only reports | Medium ΓÇö two of three are nav-invisible | Convert pilot/roi to tabs of `/value-report` after usage data | Medium ΓÇö deep links must redirect |
| `/health` | `/admin/health` | System health signals | Buyer-safe liveness vs. employee diagnostics | Medium ΓÇö identical concept name | Rename nav labels to state audience ("Service status" vs. "Internal diagnostics") | None |
...
| `/settings/cost-reporting` | `/settings/ai-usage` (rewrite), `/admin/ai-usage-cost` (redirect) | One page, three URLs | Aliases only | Low | Keep one alias; delete legacy admin page file | None |
| `/governance/alerts` | `/governance/alert-rules` | Alerting | Inbox vs. configuration | Low ΓÇö correct split | Retain both | ΓÇö |
| `/reviews/[runId]` Architecture tab | `/architectures/[id]` draft editor | Editable architecture content | Pre-review draft vs. in-review content | **High** ΓÇö two live editors for one narrative | Hand-off lock when draft spawns a review (IA-007) | Low |
...
| `/onboarding` | `/governance/first-30-days` | Guided setup checklists | First review vs. governance rhythm | Medium ΓÇö two "getting started" surfaces in different groups | Requires product decision: fold B into help or into `/onboarding` phase 2 | Low |
...
| `/reviews` hub summary | `/` home recent reviews | Same run rows, different framing | Home = next action; hub = inventory | Low ΓÇö intentional | Retain both | ΓÇö |
...
| `/settings/baseline`, `/settings/extract-upload` | Intentional deep links | Linked from ROI surfaces / pilot checklist `[PI]` | Retain; document in help |
...
| Product dev state | ΓÇö | "V1 is sold through guided evaluationΓÇª" on public `/pricing`; "pilot" in customer copy | Copy pass (IA-013) |
| Settings | Nav "Settings" ΓåÆ `/settings/tenant` | Breadcrumb "Workspace settings"; separate `/settings` hub | IA-016 |

**Product-model conflicts:** the home dual-path cards (two peer objects) vs. the nav caption (one sequence) is the load-bearing conflict (Finding 6). The data layer supports one durable object; the IA should consistently present *one lifecycle with two entry doors*, per `architecture_review_object_model_assessment.md` ┬º4 ΓÇö that document's conclusion is confirmed by this pass and by the working tree's own direction of travel (`/reviews/new?intent=create-architecture` already redirects into `/architectures/new`).
...
```text
ArchLucid (workspace scope: tenant ΓåÆ workspace ΓåÆ project)
Γö£ΓöÇΓöÇ Architecture  (lifecycle group; caption states the sequence)
Γöé   Γö£ΓöÇΓöÇ Overview                    /
Γöé   Γö£ΓöÇΓöÇ Create architecture         /architectures/new
Γöé   Γöé   ΓööΓöÇΓöÇ Drafts (resume)         /architectures            ΓåÉ promoted to reachable
Γöé   Γö£ΓöÇΓöÇ Start review                /reviews/new
Γöé   Γö£ΓöÇΓöÇ Reviews                     /reviews
Γöé   Γöé   ΓööΓöÇΓöÇ Review detail           /reviews/[runId]  (tabs: Overview ┬╖ Findings ┬╖ Evidence ┬╖
Γöé   Γöé        Policies & standards ┬╖ Decisions & remediation ┬╖ Finalize & exports ┬╖
Γöé   Γöé        Architecture ┬╖ Activity)
Γöé   Γö£ΓöÇΓöÇ Executive dashboard         /dashboard
Γöé   ΓööΓöÇΓöÇ First review guide          /onboarding (demoted post-commit)
Γö£ΓöÇΓöÇ Insights
Γöé   Γö£ΓöÇΓöÇ Evidence graph              /graph
Γöé   Γö£ΓöÇΓöÇ Ask review questions        /ask
Γöé   Γö£ΓöÇΓöÇ Search review evidence      /search
Γöé   Γö£ΓöÇΓöÇ Compare two reviews         /compare
Γöé   Γö£ΓöÇΓöÇ Impact preview              /evolution-review
Γöé   ΓööΓöÇΓöÇ Review scorecard            /scorecard
Γöé   ΓööΓöÇΓöÇ (Pattern library            /patterns ΓÇö hidden until data threshold)
Γö£ΓöÇΓöÇ Governance
Γöé   Γö£ΓöÇΓöÇ Approval queue              /governance
Γöé   Γö£ΓöÇΓöÇ Risk register               /governance/findings
Γöé   Γö£ΓöÇΓöÇ Risk exceptions ┬╖ Policy packs ┬╖ Standards & rules ┬╖ Decision register
Γöé   Γö£ΓöÇΓöÇ Signed review records       /signed-records            ΓåÉ new index (or de-referenced)
Γöé   Γö£ΓöÇΓöÇ Audit trail ┬╖ Alerts ┬╖ Alert rules ┬╖ Recurrence schedules
Γöé   ΓööΓöÇΓöÇ Advisory scans              /advisory
Γö£ΓöÇΓöÇ Reports
Γöé   Γö£ΓöÇΓöÇ Value report                /value-report  (future tabs: evaluation ┬╖ ROI)
Γöé   ΓööΓöÇΓöÇ Digests                     /digests (palette + nav aligned)
Γö£ΓöÇΓöÇ Integrations                    (unchanged)
Γö£ΓöÇΓöÇ Administration                  (unchanged; audience-labeled health)
ΓööΓöÇΓöÇ Internal Operations             (employee only; unchanged)
```
...
|---|---|---|---|
| Architect (create) | `/` | Create architecture ΓåÆ Drafts ΓåÆ Start review ΓåÆ Review detail | Evidence graph, Compare |
| Architect (prepare/review) | `/` | Start review ΓåÆ Review detail (Findings/Evidence) | Ask, Search |
| Reviewer | `/reviews` | Review detail ΓåÆ Finding inspect ΓåÆ Ask | Graph, Compare |
| Governance lead | `/governance` | Approval queue ΓåÆ Decision register ΓåÆ Signed records ΓåÆ Audit trail | Policy packs, Risk register |
| Executive / sponsor | `/dashboard` (hard redirect for sponsor-only) | Dashboard ΓåÆ Value report | Sponsor scorecard |
| Administrator | `/settings/tenant` | Users, Identity, Connection status, Billing | Service status, Support |
| First-time evaluator | `/` (sample workspace) | Sample review ΓåÆ Start review ΓåÆ First review guide | Help topics |

### 9.4 Current-route ΓåÆ proposed-route mapping

All routes map 1:1 except: `/signed-records` (new index page or removed references); page-file deletions behind existing redirects (`/integrations/itsm`, `/executive/dashboard`, `/admin/ai-usage-cost`, `/operate/architecture-graph`, `/why-archlucid`, `/demo/explain` ΓÇö files removed, config redirects preserved); future `/value-report/pilot|roi` ΓåÆ `/value-report?tab=` (deferred, redirects required at cut-over).
...
**Administration / help / demo treatments:** unchanged in structure. Demo state remains badge-differentiated (Sample workspace); demo-only tooling routes move fully behind the demo-tooling env. Evidence remains a workflow input inside reviews *and* is acknowledged as a reusable asset only through Insights tools ΓÇö a standalone evidence library is explicitly deferred until telemetry shows cross-review evidence-seeking behavior (see ┬º13).
...
1. **IA-001** ΓÇö `/signed-records` index 404 (users hit a dead end from a product-canonical concept).
2. **IA-002** ΓÇö draft list unreachable (users lose work they were told is "saved and resumable").
3. **IA-005** ΓÇö home dual-path cards reframed as one lifecycle (otherwise beta users form the wrong product model on day one and their feedback tests the wrong mental model).
4. **IA-008** ΓÇö hide Pattern library nav (an empty flagship "reuse" destination poisons the reuse narrative).

Safe to test **during** beta: progressive unlock phases (instrumented, IA-019), Insights naming, `/ask` pre-finalization gating. (The governance-view question was resolved by D2 ΓÇö delete ΓÇö so IA-020 is now ordinary cleanup, not a beta experiment.)

Wait for **usage evidence**: value-report tab consolidation (IA-009), evidence hub (┬º13), any route renames. (IA-016 no longer waits ΓÇö D5 resolved it hub-first.)
...
**IA-001 ┬╖ Resolve the `/signed-records` index dead end** ΓÇö **Done (2026-07-14)** ΓÇö **P0 ┬╖ S**
- **Problem:** `next.config.ts` rewrites `/signed-records` ΓåÆ `/manifests`, but no `manifests/page.tsx` index exists; only `[manifestId]` detail. Breadcrumb map and command palette use "Signed review records" as a parent concept. `[PI]`
...
**IA-002 ┬╖ Make architecture drafts reachable** ΓÇö **Done (2026-07-14)** ΓÇö **P0 ┬╖ S**
...
- **Change:** (a) Add a "Resume a draft" affordance: a drafts strip or link on the Reviews hub when ΓëÑ1 draft exists; (b) `/architectures/new` bootstrap should offer "resume existing draft" when drafts exist instead of silently creating a new one; (c) breadcrumb for `/architectures/[id]` gains parent "Drafts" ΓåÆ `/architectures`. Do **not** add a top-level "Architectures" nav item (consistent with the object-model assessment ┬º4).
...
**IA-003 ┬╖ Rename the review-detail "Review" tab** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
- **Problem:** Tab id `review-package` renders label "Review" (`review-detail-workspace-tabs.ts:24`) on a page that *is* the review ΓÇö the label carries no information; the tab contains finalize + exports. `[PUI]`
...
- **Change:** label ΓåÆ "Finalize & exports" (or "Deliverables" post-finalize; keep it one static label for now).
...
**IA-004 ┬╖ Fix the hub includes-list redundancy** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
- **Problem:** `REVIEWS_HUB_INCLUDES_ITEMS` lists both "Review record" and "Signed review record" (`reviews-hub-copy.ts:54-61`) ΓÇö two near-identical nouns in a six-item list defining the product. `[PUI]`
...
**IA-005 ┬╖ Reframe home dual-path cards as one lifecycle** ΓÇö **Done (2026-07-14)** ΓÇö **P0 ┬╖ S**
...
- **Change:** Copy-only: reposition cards as "Step 1 ΓÇö describe or import your architecture (start from a draft)" and "Step 2 ΓÇö run a governed review" (or a single card with two entry buttons). No route changes; CTAs keep their targets.
...
**IA-006 ┬╖ Converge evidence-tool and governance naming** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ S**
...
- **Implementation notes:** extend the terminology drift-guard test to assert nav label Γåö breadcrumb segment equality for these keys.
...
**IA-007 ┬╖ Draft ΓåÆ review hand-off lock** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ S**
...
- **Change (per D4, resolved 2026-07-14 ΓÇö soft gate):** When `spawnedRunId` exists, the draft workspace renders a hand-off banner ΓÇö "This draft became review {title}; continue editing there" ΓÇö and the editor is disabled until the user explicitly acknowledges that further draft edits will not flow into the existing review. A hard permanent lock was considered and rejected by the owner.
...
**IA-008 ┬╖ Hide Pattern library nav until content threshold** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
- **Problem:** `/patterns` requires ΓëÑ5 contributing tenants and ΓëÑ3 live cards for live aggregates; private-beta tenants will see an empty/below-threshold flagship "reuse" destination. Nav visibility flag exists (`isPatternLibraryNavVisible()`). `[PI]`
- **Change (per D3, resolved 2026-07-14 ΓÇö data-driven):** Make visibility data-driven: the nav item appears only when the aggregates endpoint reports the threshold met. Route stays live for deep links with an honest below-threshold empty state. The flag-off shortcut was considered and rejected by the owner.
...
**IA-009 ┬╖ Reports family consolidation (deferred design)** ΓÇö **P2 ┬╖ M**
- **Problem:** `/value-report`, `/value-report/pilot`, `/value-report/roi`, `/scorecard`, `/governance/dashboard`, `/digests` fragment reporting; two of six are nav-invisible. `[PI]`
- **Change (post-telemetry, except the D6 carve-out):** Convert `/value-report/pilot` and `/value-report/roi` into tabs of `/value-report` (redirects for old URLs); rename `/executive/scorecard` label to "Sponsor scorecard" and cross-link with `/scorecard`.
- **D6 carve-out (resolved 2026-07-14 ΓÇö immediately actionable, not telemetry-gated):** remove `/governance/dashboard` now ΓÇö redirect it to `/dashboard` and fold any unique workspace-health KPI tiles into `/dashboard`. This portion can ship in Wave 4 independently of the rest of IA-009.
...
- **Priority/complexity:** P2/M. **Risk if deferred:** manageable ΓÇö palette covers power users; confusion cost grows with adoption.

**IA-010 ┬╖ Executive naming reconciliation** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
- **Problem:** Nav "Executive dashboard" ΓåÆ `/dashboard`; page vocabulary "Executive summary"; i18n key `portfolioOverview`; `/executive/scorecard` vs. `/scorecard` share a word across shells. `[PUI]`
...
**IA-011 ┬╖ Align command palette with nav visibility** ΓÇö **P1 ┬╖ S**
- **Problem:** Palette curates tasks including consolidated-omission destinations (e.g., "Digests & subscriptions") while nav deliberately omits them; palette is documented as gated to `visibleOperatorShellHrefSet` but drift exists on omission-set entries. `[SMI]` ΓÇö verify each palette entry against the omission set during implementation.
...
**IA-012 ┬╖ Normalize legacy CTA targets** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
...
- **Shipped:** `GOVERNANCE_WORKFLOW_IDLE` policy packs CTA ΓåÆ `/governance/policy-packs`; `auditTrailNavHref` ΓåÆ `/governance/audit`; `empty-state-preset-cta-guard.test.ts` enforces disjointness from `next.config.ts` permanent redirect sources.

**IA-013 ┬╖ Internal-concept leakage copy pass** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ XS**
- **Problem:** "AdminAuthority"/"ExecuteAuthority" in forbidden-state messages (`SettingsRolesPageView.tsx`, `TrialFunnelOpsPageClient.tsx`, cloud wizard); "V1 is sold through guided evaluationΓÇª" on `/pricing`; help slug `creating-runs` in customer URLs; breadcrumb "pilot" for `/value-report/pilot`. `[PUI]`
- **Change:** Replace rank names with role phrasing ("Requires a workspace admin"); reword pricing note without version labels; register slug alias `starting-reviews` (keep `creating-runs` as redirect alias in the help registry); breadcrumb label for `pilot` segment already maps to "Review value report" ΓÇö verify and keep.
...
**IA-014 ┬╖ Dead page-file and internal-route cleanup** ΓÇö **P2 ┬╖ S**
...
**IA-015 ┬╖ Approval-request detail route** ΓÇö **P2 ┬╖ M**
...
- **Change:** Add `/governance/approval-requests/[id]` (request summary, status, decision history, link to review and lineage) or relocate lineage as a tab of the queue's detail panel. Depends on how often approvals are shared by link ΓÇö instrument first (IA-019).
...
**IA-016 ┬╖ Settings entry point: hub-first** ΓÇö **P2 ┬╖ XS (D5 resolved 2026-07-14)**
- **Problem:** Nav "Settings" targets `/settings/tenant` while a searchable `/settings` hub index exists, reachable only by URL. `[PI]`
- **Change (per D5 ΓÇö hub-first):** Point the sidebar "Settings" item at `/settings` (the searchable index); `/settings/tenant` remains a first-class hub destination. Update the breadcrumb parent chain accordingly.
- **Acceptance:** exactly one settings landing pattern (hub-first); breadcrumbs consistent.

**IA-017 ┬╖ Health-surface audience naming** ΓÇö **P2 ┬╖ XS**
- **Problem:** `/health` ("System health", customer Administration) vs. `/admin/health` ("Diagnostics dashboard", Internal Operations) ΓÇö same concept name, different audiences. `[PI]`
...
**IA-018 ┬╖ Help breadcrumb and entry normalization** ΓÇö **P2 ┬╖ XS**
...
- **Change:** Standardize crumbs on "Help"; keep top-bar entry (adding a sidebar footer "Help" link is optional ΓÇö defer to beta feedback).
- **Acceptance:** all help crumbs share one parent label.

**IA-019 ┬╖ Navigation and route-entry telemetry** ΓÇö **Done (2026-07-14)** ΓÇö **P1 ┬╖ S**
...
- **Priority:** P1 ΓÇö it gates every deferred structural decision. **Risk if deferred:** post-beta IA decisions stay opinion-based.
...
**IA-020 ┬╖ Remove the governance-view mode branch** ΓÇö **P1 ┬╖ S (D2 resolved 2026-07-14 ΓÇö delete)**
...
- **Change (per D2 ΓÇö delete):** Remove `GovernanceModeToggle`, `filterNavGroupsForGovernanceMode`, the `archlucid_governance_mode_enabled` storage key handling, and the dual-vocabulary branches in `governance-mode-vocabulary.ts`; collapse to one vocabulary using the already-canonical terms ("Signed review record", "Authority chain" per the canonical-product-terms module ΓÇö confirm final noun choice against the D7 baseline during implementation). Delete the associated mode tests rather than leaving them asserting dead code.
...
**IA-021 ┬╖ Opaque nav label pass** ΓÇö **P3 ┬╖ XS**
- **Problem:** "Impact preview" (`/evolution-review`), "Programs" (group), "Advisory scans" are labels a first-time user cannot map to intent; URL `evolution-review` does not match its label. `[PRV]` ΓÇö validate with beta users before renaming.
- **Change:** After beta feedback: candidate renames ("Change impact preview"; group "Improvement programs"). No URL changes.

**IA-022 ┬╖ Guide-content placement review** ΓÇö **P3 ┬╖ S**
...
|---|---|---|
| **Wave 1 ΓÇö before beta invites (P0)** | IA-001 Γ£ô, IA-002 Γ£ô, IA-005 Γ£ô, IA-008 Γ£ô | Dead ends, stranded work, wrong mental model, empty flagship |
| **Wave 2 ΓÇö with wave 1 or first beta patch (P1 copy/labels)** | IA-003 Γ£ô, IA-004 Γ£ô, IA-010 Γ£ô, IA-012 Γ£ô, IA-013 Γ£ô | XS copy items; batch into one terminology PR so drift guards update once |
| **Wave 3 ΓÇö early beta (P1 structural-lite)** | IA-019 Γ£ô, IA-006 Γ£ô, IA-007 Γ£ô, IA-011, IA-020 | Telemetry must precede the still-deferred decisions; naming convergence, hand-off gate, and governance-view removal ride behind it |
| **Wave 4 ΓÇö mid-beta hygiene (P2)** | IA-014, IA-016, IA-017, IA-018, IA-009 D6 carve-out (`/governance/dashboard` removal) | No user-facing risk; reduces engineering drag |
| **Wave 5 ΓÇö post-telemetry (P2/P3)** | IA-009, IA-015, IA-021, IA-022 | Each is explicitly gated on usage evidence |

Sequencing constraints: IA-019 before IA-009/IA-015/IA-021; D1ΓÇôD5 (┬º14) before their dependent items; the Wave 2 copy batch updates `review-terminology-guard.test.ts` and breadcrumb tests in the same change, never separately (the guards are designed to fail otherwise).
...
**Beta-facilitated checks (no telemetry needed):** first-session task tests ΓÇö "find your saved draft" (IA-002), "export the finalized review" (IA-003), "show me every signed decision" (IA-001/IA-015), "what does ArchLucid do besides review?" (IA-005 framing).

**Regression net:** the existing drift-guard pattern (terminology guard, nav structure test, onboarding hub contract) is the right mechanism; Waves 2ΓÇô3 extend it to breadcrumb-segment Γåö nav-label equality and preset-CTA Γåö redirect-source disjointness.

---

## 14. Product decisions ΓÇö RESOLVED (owner, 2026-07-14)
...
| **D2** | Governance view mode | **Delete the branch** ΓÇö remove the unmounted toggle, the unused nav filter, and collapse to one vocabulary | IA-020 becomes a removal task; IA-006 converges on the single (governance-view-on) canonical nouns already marked canonical ("Signed review record") |
| **D3** | Pattern library in beta | **Data-driven visibility** ΓÇö nav item appears only when the aggregates endpoint reports the threshold met; route stays live for deep links with an honest below-threshold state | IA-008 implemented as the S-complexity data-driven option, not the XS flag-off option |
| **D4** | Draft post-spawn policy | **Acknowledged divergence** ΓÇö draft stays editable, but only after the user explicitly acknowledges that edits will not flow into the already-spawned review | IA-007 implements the soft gate, not the hard lock; keep the post-spawn-edit telemetry to revisit |
| **D5** | Settings landing | **Hub-first** ΓÇö the sidebar "Settings" item targets the searchable `/settings` index | IA-016 unblocked; breadcrumbs and nav label updated together |
| **D6** | `/governance/dashboard` fate | **Remove now** ΓÇö redirect to `/dashboard` and fold any unique KPI tiles into it; do not wait for telemetry | The `/governance/dashboard` portion of IA-009 is carved out as immediately actionable (see IA-009 note); disposition changes to Remove (with redirect) |
| **D7** | Terminology baseline | **Commit the "Reviews" consolidation as-is** ΓÇö it is the settled terminology baseline for all backlog waves and supersedes the TB-738 "Architecture packages" state and the "package" conflict flagged in `architecture_review_object_model_assessment.md` | Waves 2ΓÇô3 unblocked once the working-tree consolidation is committed (owner names the branch per repo convention) |

---

## Appendix A ΓÇö Mandatory challenge questions (direct answers)

1. **Navbar: customer goals or internal modules?** Customer goals ΓÇö the pilot group is a task sequence, not module names `[PI]`. "Programs" is the exception (module-ish name).
2. **Is "Reviews" carrying foreign responsibilities?** Less than before: the hub now carries lifecycle stages (Architecture definition ΓåÆ ΓÇª ΓåÆ Approval, `reviews-hub-review-status.ts`), which correctly absorbs creation/evidence/governance *status* without absorbing their workflows. Acceptable.
...
4. **Governance / Audit / Evidence / Executive non-overlapping?** Governance and Audit: yes (queue vs. ledger, audit nested under governance ΓÇö defensible). Evidence: no single home (accepted, deferred). Executive: overlapping (Finding 4).
5. **System-health/admin placement?** Yes ΓÇö Administration group is where admins expect it; naming defect only (IA-017).
6. **Top-level ΓåÆ secondary candidates?** "First review guide" already auto-demotes post-commit (good pattern). "Governance setup guide" should follow it (IA-022).
7. **Secondary/hidden ΓåÆ top-level candidates?** Drafts list (to reachable, not top-level); signed-records index (to Governance group).
8. **Pages existing because features shipped at different times?** Yes: the six reporting surfaces; three cost-reporting URLs; two ITSM hubs (one redirected); `/scorecard` vs. `/executive/scorecard`.
9. **Same object, multiple presentations?** `Run`: hub row, home card, detail workspace, showcase, snapshot, compare picker ΓÇö all intentional views. The problematic pair is draft editor vs. Architecture tab (IA-007).
10. **Tabs where routes would be clearer?** No ΓÇö the 8-tab review workspace is right for one object. **Routes where tabs would be clearer?** `/value-report/{pilot,roi}` (IA-009).
11. **Users taken out of the shell unnecessarily?** `/executive/scorecard` uses a separate executive chrome; acceptable for a sponsor leave-behind, but label it (IA-010). Help stays in-shell (good, TB-143ΓÇô148).
12. **Demo/sample/setup/production differentiated?** Yes ΓÇö badges, seeded-sample flows, strict demo nav gates `[PI]`. Residual: internal demo tooling in customer route space (IA-014).
...
15. **Does the IA imply review-only?** Largely yes ΓÇö and that is currently *truthful*. The correction is the lifecycle framing (creation ΓåÆ review ΓåÆ governance ΓåÆ reporting as one loop), not manufacturing peer inventories.
16ΓÇô21. **Coherent homes:** architecture work ΓÇö yes (`/` + create/drafts once IA-002 lands); review work ΓÇö yes (`/reviews`); governance ΓÇö yes post-unlock (`/governance`); evidence ΓÇö **no** (accepted gap, telemetry-gated); administration ΓÇö yes.
22. **Can users understand tenant/workspace/project/architecture/review/package/evidence scope?** Tenant/workspace/project: yes via ScopeSwitcher + `/help/scope`. Architecture-vs-review: only after IA-005/IA-007. "Package": retired from lists in the working tree; residuals are compound export nouns ("evidence package ZIP") ΓÇö acceptable if D7 commits the consolidation.

## Appendix B ΓÇö Disposition summary
...
| Remove (page files) | `/integrations/itsm`, `/executive/dashboard`, `/admin/ai-usage-cost`, `/operate/architecture-graph`, `/why-archlucid` (fold into `/why`) |
| Requires product decision | `/example-roi-bulletin` beta visibility; `/governance/first-30-days` placement (IA-022, P3). D1ΓÇôD7 were resolved by the owner on 2026-07-14 (┬º14) |
