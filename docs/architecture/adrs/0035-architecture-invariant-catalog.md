...
4. **Concurrency with OpenAPI**: any INV that changes externally visible behavior must follow the HTTP surface rule (canonical `GET /openapi/v1.json`, snapshot + client regen) — see [`docs/library/API_CONTRACTS.md`](../../library/API_CONTRACTS.md) and [`docs/library/OPENAPI_CONTRACT_DRIFT.md`](../../library/OPENAPI_CONTRACT_DRIFT.md).

5. **`INV-003` default interpretation** honors **`TB-001`**: transactional vs informational audit paths remain distinct until a superseding backlog item merges.
