...
- **Status note (2026-06-06, [ADR 0042](0042-canonical-run-write-surface.md) / TB-305):** The code-level run-pipeline write surface is now collapsed onto the canonical `v1/architecture/*` family with deprecated-but-routable `v1/runs/*` + `v1/requests` aliases (single shared MVC action → unified idempotency + audit). Gate **(iv)**'s waiver is unchanged: it stays owner/customer-traffic gated and is restored automatically when V1 ships to a paying customer.
- **Status note (2026-07-20, TB-919):** The `v1/runs/*` + `v1/requests` aliases are **deleted** (pre-release product — no traffic to soak, so gate (iv)'s waiver precondition is moot rather than satisfied). See [ADR 0021](0021-coordinator-pipeline-strangler-plan.md) for the migration's full closure record.
