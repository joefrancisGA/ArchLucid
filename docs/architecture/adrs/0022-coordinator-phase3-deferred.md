...
- **Status note (2026-07-20, TB-919):** The `v1/runs/*` + `v1/requests` aliases described above are **deleted** (product is pre-release, so the 14-day traffic-soak precondition for gate (iv) no longer applies — there is no coordinator write traffic to soak). [ADR 0021](0021-coordinator-pipeline-strangler-plan.md) records the migration as fully closed with no open items.
...
- **No** edits to historical SQL migrations **001–028**; behavior changes continue via new migrations + `ArchLucid.Persistence/Scripts/ArchLucid.sql` when schema work resumes.
