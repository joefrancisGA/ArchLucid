...
2. **Demotion predicate (DX-01, shipped 2026-09-07):** demote when `(score < DemotionThreshold || genericAdviceWithoutEvidence || falsifiableWithoutEvidence) && !hasConcreteEvidence`. Supersedes the pre–DX-01 triple-AND shape (`score < DemotionThreshold && !hasArchitectureAnchor && !hasConcreteEvidence` plus category veto). **`InsightDensityAgentCategoryRules.IsDemotionEligibleCategory`** always returns **true** and is **unused** by the gate — category name does not block demotion; resolvable package evidence does.
