...
| `typescript` | `^7.0.2` | Type-checking (`tsc --noEmit`), build. **Peer-conflict note:** `openapi-typescript@7.13.0` still declares `typescript@^5.x`; `archlucid-ui/package.json` uses an `overrides` entry (`openapi-typescript` → `$typescript`) so `npm ci` succeeds without `legacy-peer-deps`. |
