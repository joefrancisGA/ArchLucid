...
- **IDA-06** may already dash peering (`stroke-dasharray="6 4"`, often no on-path pill). Declared uses `4 3` and **always** paints `declared` on the path. Never suppress a declared label in an IDA-06 collapse.
