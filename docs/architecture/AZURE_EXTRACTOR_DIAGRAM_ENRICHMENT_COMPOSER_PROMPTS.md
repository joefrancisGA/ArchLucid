> **Scope:** Copy-paste Composer/Cloud Agent prompts that extended the **existing** Azure extractor so diagrams get more ARM platform wiring, PaaS children, and authorization hops. Internal engineering only. **Shipped (engineering)** — **do not re-run** or implement from the tables. Consumption follow-on: [`AZURE_EXTRACTOR_DIAGRAM_CONSUMPTION_COMPOSER_PROMPTS.md`](AZURE_EXTRACTOR_DIAGRAM_CONSUMPTION_COMPOSER_PROMPTS.md) (**AX-DC-01–08**).
