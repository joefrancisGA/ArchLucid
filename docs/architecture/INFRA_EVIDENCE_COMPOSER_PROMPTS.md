...
| [`DIAGRAM_REGRESSION_REPAIR_COMPOSER_PROMPTS.md`](DIAGRAM_REGRESSION_REPAIR_COMPOSER_PROMPTS.md) | **DRR-01–DRR-05** + hold — repair #3586/#3587: data-flow stage columns and gutters, edges out of other cards, light/dark edge ink, icon honesty, outline sort; **prompts only** |
...
| Subscription-frame restore, new icon zip, or Home hub copy from **DRR** chats | **DRR-HOLD** — repair #3586/#3587 only |
...
| **DRR-01** | Data-flow stage labels, gutter, sky lane | trunk (#3587 columns) |
| **DRR-02** | Data-flow edges in gutter and sky lane | DRR-01 |
| **DRR-03** | Light `#111827` / dark `#e2e8f0` edge and arrowhead | trunk (parallel with 01) |
| **DRR-04** | Icon kind, drop duplicate PNGs, CDN ≠ Front Door | trunk (parallel with 01) |
| **DRR-05** | Outline counts before the 200 cap; sort inside each section | trunk (parallel with 01) |
| **DRR-HOLD** | No subscription-frame restore / new icon zip / IDA greenfield | — |
...
**Diagram regressions after #3586 and #3587 (2026-09-23):** run **DRR-01** first for the data-flow canvas, then **DRR-02**. **DRR-03**, **DRR-04**, and **DRR-05** may run in parallel with **DRR-01**. **DRR-HOLD** is not implementation. Prompts: [`DIAGRAM_REGRESSION_REPAIR_COMPOSER_PROMPTS.md`](DIAGRAM_REGRESSION_REPAIR_COMPOSER_PROMPTS.md). Paste one numbered file from [`.cursor/prompts/diagram-regression-repair-00-index.md`](../../.cursor/prompts/diagram-regression-repair-00-index.md). Do **not** implement from this index. Do **not** restore the subscription frame. Do **not** download a new Azure icon pack. Do **not** re-run IDA/IDF/IDX/SN-DF as greenfield. Do **not** rewrite the SecureNow Home “seven destinations” sentence in a DRR session.
