...
**Status:** Prompt 11 **drafted** below (not yet run). Prompt 10 **done** (see report below). Prompt 9 **done** (see report below). Prompt 8 **done** (see report below). Prompt 7 **done** (`e7eca24395`). Prompt 6 **done** (`faf3500c6d`). Prompt 5 **done** (`e89ceffdfb`). Prompt 4 **done** (`80ad003d60`). Prompt 3 **done** (`c23ec43b4d`). Prompt 1 **done** (`daaa784505`). Prompt 2 **done** (`599b51c74a`) — see reports below.
...
| 11 | D.9 | Golden-corpus consistency scenarios in CI | **Drafted, not run** |

Prompts 1–11 are written out below. **Run Prompt 11** when ready.
...
## Prompt 11 — Golden-corpus technology consistency scenarios (CI)

```
Read docs/architecture/ARCHITECTURE_GENERATION_TECHNOLOGY_CONSISTENCY_ASSESSMENT_2026_07_07.md in full for context before starting, specifically §D fix 9 (golden-corpus consistency scenarios in CI), §E step 6 (regression re-runs the whole path on every pipeline change), §F (uniqueness / closed-world / warn-then-enforce semantics), and §I acceptance criteria #5 (golden-corpus scenarios pass on every generation-pipeline change). Also read "Prompt 6 — Report", "Prompt 7 — Report", and "Prompt 10 — Report" in this same file for the shipped finding-engine rule titles, artifact lint rule ids, and UI scope boundaries — **assert against those engines as-is**; do not change rule logic in this prompt.

Work directly on the current branch (`master`) — no feature branch. Confirm `git status` is clean of unrelated changes before starting; if pre-existing unrelated unstaged changes are present in the working tree, leave them untouched and do not stage or commit them alongside this task's changes.

## Goal

Prompts 1–10 closed the technology-consistency defect in product code (ledger, agents, deterministic finding engine, artifact prose lint, API, UI). This prompt closes fix **D.9** — **regression safety in CI**:

1. Add a **fixed, committed fixture corpus** of Technology Ledger snapshots (and optional artifact prose snippets) representing coherent baselines and known drift violations.
2. Add **table-driven golden tests** that run `TechnologyConsistencyFindingEngine` and `TechnologyLedgerArtifactLinter` against those fixtures on every PR.
3. Wire a **blocking CI step** (plus a small Python drift guard) so pipeline edits cannot silently drop the corpus or its CI hook.

This is **CI + test fixtures only** for v1. Do **not** change `TechnologyConsistencyFindingEngine`, `TechnologyLedgerArtifactLinter`, agent handlers, seeders, merge policies, API/UI (Prompts 9–10), system templates (Prompt 8), or buyer positioning (D.8). Do **not** implement D.10 evidence-linking closure. Do **not** add live Azure OpenAI / real-mode agent runs — fixtures are **deterministic** and PR-safe (same posture as adversarial simulator rows in `tests/eval-corpus/adversarial/`).

## Execution-order context (read before coding)

### Shipped engines to assert (do not re-specify rule logic)

**`TechnologyConsistencyFindingEngine`** (Prompt 6) — finding **titles** today:

| Title | Meaning (summary) |
| --- | --- |
| `DuplicateChosenLedgerRole` | More than one `Chosen` row for the same role |
| `ConflictingChosenProviderFamily` | `Chosen` role provider family conflicts with `Chosen` CloudPlatform |
| `CloudNeutralProviderLeak` | `Chosen` CloudPlatform is `None` but another `Chosen` row is a hyperscaler |
| `MissingChosenCloudPlatform` | `Chosen` platform-dependent rows exist without a `Chosen` CloudPlatform |
| `LockedChosenOverriddenByAssumed` | Locked `Chosen` row contradicted by a conflicting `Assumed` proposal |

Default options: `ArchLucid:TechnologyConsistency:FindingEngine` — `Enabled: true`, `Mode: WarnOnly` (maps to `FindingSeverity.Warning`).

**`TechnologyLedgerArtifactLinter`** (Prompt 7) — lint **rule ids** today:

| RuleId | Meaning (summary) |
| --- | --- |
| `ProseHyperscalerFamilyMismatch` | Exported prose names a hyperscaler family inconsistent with ledger |
| `CloudNeutralProseProviderLeak` | Cloud-neutral ledger but prose asserts a hyperscaler |
| `UnledgeredHyperscalerToken` | Prose names a hyperscaler token with no corroborating ledger row |

Default options: `ArchLucid:TechnologyConsistency:ArtifactLint` — `Enabled: true`, `Mode: WarnOnly`.

### Repo patterns to mirror

- **Adversarial eval corpus:** `tests/eval-corpus/adversarial/*/scenario.json` + committed recordings — see `scripts/ci/assert_hallucination_resistance.py` and `scripts/ci/tests/test_assert_hallucination_resistance.py` for the **nightly + pytest drift-guard** pattern (TB-257).
- **PR agent corpus:** `.github/workflows/ci.yml` runs `python3 scripts/ci/eval_agent_corpus.py --baseline --enforce-quality-gate` — add a sibling **dotnet** step for technology-consistency golden tests (fast, no secrets).
- **Existing unit tests:** `ArchLucid.Application.Tests/Governance/TechnologyConsistencyFindingEngineTests.cs` and `ArchLucid.ArtifactSynthesis.Tests/TechnologyLedgerArtifactLinterTests.cs` — extend via **file-driven** golden corpus tests; do not duplicate inline cases already covered unless the corpus adds a new scenario shape (e.g. full multi-role Azure baseline JSON).

## 1. Fixture corpus layout

Create `tests/technology-consistency-corpus/` (new top-level test data folder — **not** under `tests/eval-corpus/`, because these fixtures target deterministic ledger/lint engines, not agent finding recall):

```
tests/technology-consistency-corpus/
  manifest.json                 # ordered scenario ids
  README.md                     # authoring checklist (short)
  finding-engine/
    azure-coherent-baseline/
      ledger.json
      expected.json             # { "findingTitles": [], "mode": "WarnOnly" }
    aws-coherent-baseline/
      ...
    cloud-neutral-coherent-baseline/
      ...
    cross-family-drift-azure-rds/
      ledger.json
      expected.json             # { "findingTitles": ["ConflictingChosenProviderFamily"], ... }
    duplicate-chosen-primary-datastore/
    missing-chosen-cloud-platform/
    cloud-neutral-hyperscaler-leak/
    locked-chosen-overridden-by-assumed/
    revision-coherent-to-drift/   # two-step revision scenario (see below)
      ledger-v1.json
      ledger-v2.json
      expected-v1.json            # { "findingTitles": [] }
      expected-v2.json            # non-empty — drift introduced in v2
  artifact-lint/
    azure-prose-aligned/
      ledger.json
      artifact.md                 # short narrative prose
      expected.json               # { "ruleIds": [] }
    prose-hyperscaler-mismatch/
    cloud-neutral-prose-leak/
    unledgered-hyperscaler-token/
```

### `ledger.json` shape

Array of objects deserializable to `TechnologyLedgerEntry` — use **string enum names** matching API JSON (`Role`, `Status`, `Source`, `ProviderFamily`). Include minimal required fields: `entryId`, `runId`, `role`, `technologyName`, `providerFamily`, `status`, `source`, `isLocked`, `createdUtc`, `updatedUtc`; `evidenceRef` / `rationale` optional.

Use a fixed canonical `runId` per scenario (e.g. `cccccccccccccccccccccccccccccccc` without dashes) for stable `FindingId` suffixes if tests assert full ids — prefer asserting **titles** and **count** only.

### `expected.json` shape (finding engine)
...
For coherent baselines: `"findingTitles": []`, `"maximumCount": 0`.

### Revision scenario (`revision-coherent-to-drift`)

Models assessment language ("no technology drift across a known run's revisions") without live agents:

- **v1** ledger: coherent Azure greenfield (CloudPlatform + Identity + Datastore + Compute all Azure `Chosen`).
- **v2** ledger: same run after a bad revision — e.g. Cost agent proposal leaves `PrimaryDatastore` as `Chosen` Aws **or** adds conflicting `Assumed` row — engine must emit at least one expected title on v2 while v1 stays clean.

### `manifest.json`

List every scenario directory path (relative to corpus root) in deterministic order. Pytest drift guard and the C# loader both read this manifest — **do not** hardcode scenario lists in two places without sharing the manifest file.

## 2. Golden test harness (C#)

### Finding engine corpus tests

Add `ArchLucid.Application.Tests/Governance/TechnologyConsistencyGoldenCorpusTests.cs`:

- `[Trait("Category", "Unit")]` / `[Trait("Suite", "Core")]`
- At test time, resolve corpus root relative to repo root (copy pattern from other file-based tests if one exists; otherwise `Path.Combine` from `AppContext.BaseDirectory` walking up to `tests/technology-consistency-corpus` — document the resolution approach in `README.md`).
- For each `finding-engine/*` scenario in `manifest.json`:
  - Deserialize `ledger.json` → `List<TechnologyLedgerEntry>`
  - Deserialize `expected.json` → options + expected titles
  - Call `TechnologyConsistencyFindingEngine.Evaluate(runId, entries, options)`
  - Assert titles (order-independent), min/max counts, and `FindingSeverity.Warning` when mode is `WarnOnly`
- For `revision-coherent-to-drift`, evaluate v1 and v2 in one `[Fact]` or two chained assertions.

### Artifact lint corpus tests

Add `ArchLucid.ArtifactSynthesis.Tests/TechnologyConsistencyArtifactGoldenCorpusTests.cs`:

- Load `artifact-lint/*` scenarios from the same manifest (filter by prefix or manifest section — keep one manifest or split `artifact-manifest.json` if cleaner; prefer **one** manifest with typed entries `{ "kind": "finding-engine" | "artifact-lint", "path": "..." }`).
- Build minimal `ArtifactBundle` (reuse helpers from `TechnologyLedgerArtifactLinterTests`).
- Call `TechnologyLedgerArtifactLinter.Lint(bundle, ledger, options)` and assert `ruleIds` per `expected.json`.

Keep helpers in shared test utility classes only if duplication exceeds ~40 lines; one file per test class.

## 3. CI wiring

### Blocking PR step

In `.github/workflows/ci.yml`, near the existing `eval_agent_corpus.py` step (agent eval / golden corpus section), add:

```bash
dotnet test ArchLucid.Application.Tests/ArchLucid.Application.Tests.csproj \
  -c Release --no-restore \
  --filter "FullyQualifiedName~TechnologyConsistencyGoldenCorpus"

dotnet test ArchLucid.ArtifactSynthesis.Tests/ArchLucid.ArtifactSynthesis.Tests.csproj \
  -c Release --no-restore \
  --filter "FullyQualifiedName~TechnologyConsistencyArtifactGoldenCorpus"
```

Use the same job's existing `dotnet build` outputs if those projects are already built; otherwise add restore/build lines consistent with neighboring steps. Step must **fail the job** on test failure.

### Python drift guard

Add `scripts/ci/assert_technology_consistency_corpus.py`:

- Verify `tests/technology-consistency-corpus/manifest.json` exists and lists **at least** the nine finding-engine + four artifact-lint scenarios named in section 1 above (allow additional scenarios; fail if any required id is missing).
- Verify each listed scenario directory contains required files (`ledger.json` + `expected.json`, or revision/v1/v2 files).
- Print `technology consistency corpus OK (N scenarios)` on success.

Add `scripts/ci/tests/test_assert_technology_consistency_corpus.py` (unittest) asserting:

- Script exists.
- `ci.yml` contains `TechnologyConsistencyGoldenCorpus` filter (or the assert script path if you wire the script directly in CI instead of duplicate filters — pick one approach and test it).

**Do not** wire this prompt's corpus into `golden-cohort-expanded-nightly.yml` unless the nightly job already builds Application.Tests — PR blocking is the v1 requirement per assessment §I #5 ("every change to the generation pipeline"). Nightly optional follow-up is out of scope unless trivial (<10 lines).

## 4. Documentation

Update `docs/library/AGENT_EVAL_CORPUS.md` — add a short **"Technology consistency golden corpus"** section pointing at `tests/technology-consistency-corpus/README.md`, explaining how it differs from `tests/eval-corpus/` (deterministic ledger/lint regression vs agent finding recall).

Keep `tests/technology-consistency-corpus/README.md` concise: how to add a scenario, how to run locally:
...
Stage only `tests/technology-consistency-corpus/**`, `ArchLucid.Application.Tests/Governance/TechnologyConsistencyGoldenCorpusTests.cs`, `ArchLucid.ArtifactSynthesis.Tests/TechnologyConsistencyArtifactGoldenCorpusTests.cs`, `scripts/ci/assert_technology_consistency_corpus.py`, `scripts/ci/tests/test_assert_technology_consistency_corpus.py`, `.github/workflows/ci.yml`, and doc touches this prompt requires. Do not stage unrelated dirty files. Commit directly to `master` with a descriptive message (e.g. "Add technology consistency golden corpus and CI regression tests"). Push only if explicitly requested.

## 7. Report

Stop and report:

- Corpus root path and scenario count (finding-engine vs artifact-lint).
- Revision scenario description (`revision-coherent-to-drift` v1 vs v2).
- CI job/step name where golden tests run.
- Python drift guard path.
- `dotnet test` pass/fail counts for both test classes.
- Commit hash.
- Confirm finding-engine rule logic, artifact linter, agents, API/UI, and D.8/D.10 scope were **not** touched.
```
