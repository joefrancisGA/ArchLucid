...
| **`editSourceGuidedIntakeRerun`** (Edit source) | N/A (not draft workspace) | `canEditSource={!manifestId}` → `/architecture/reviews/new?path=guided-intake&rerun={runId}` | N/A — separate intake writer | **Yes** when manifest open — **alternate writer** to draft PATCH (IA-007 leftover) |
...
| **Review “Edit source” alternate writer** | Pre-seal reviews expose guided-intake rerun while `DraftRequests` row still exists | **SN-003** (URL honesty) · **SN-004** (one writer) |
| **Legacy draft URL bookmark** | `/architecture/architectures/{draftId}` without identity desk redirect | **SN-003** · **SN-005** |
...
3. **Alternate-writer** row is the IA-007 residual — remove only when SN-004 blocks or reroutes guided-intake rerun honestly.
