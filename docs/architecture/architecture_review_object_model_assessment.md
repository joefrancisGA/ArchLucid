...
| **Evidence package** | A specific *export/trace bundle* (ZIP) of evidence for one review — distinct product noun from "review package," not the whole review object | ZIP export keyed by `RunId` | `glossary-definitions.ts:36-40`; `RunDetailRunActionsSection.tsx:50` ("Download evidence package (ZIP)") |
...
| **Signed review record / Signed decision record** | The finalized, versioned authority closure produced when a review is committed | `dbo.GoldenManifests` (`ManifestId`, FK `RunId`) | `ManifestDetailPageView.tsx:249` ("Signed decision record", buyer) vs. line 250 ("Finalized architecture review package", operator) |
| **Deliverables / Artifact bundle / Review bundle / Exports** | Generated output files (DOCX, PDF, ZIP) derived from a signed record or committed review, for different audiences | `ArtifactBundle`, `EvidenceBundle` (backend types) | `buyer-polish-copy.ts:467-482` (`BUYER_MANIFEST_DELIVERABLES_HEADING`) |
| **Executive dashboard / Executive summary** | A workspace-wide rollup across committed reviews (dashboard), or a per-review summary panel | Aggregation of `Run` rows | `ExecutiveDashboardPageHero.tsx:31-35`; `RunDetailExecutiveSummaryCtaCard.tsx:22-27` |
...
| **Package** (bare noun) | Not used as a standalone customer-facing noun; always appears as a compound ("review package," "architecture package," "evidence package," "proof package," "diligence package") | — | See §2.3 |
...
| **Review output** | Not currently a shipped term; recommended as the umbrella customer-facing label for exports/deliverables generated from a review (DOCX, PDF, ZIP) if a term broader than "deliverable" is needed | Proposed for exports/artifacts section headings | "Architecture package (DOCX)," "review package bundle," "artifact bundle" (as customer copy) | No |
...
| 8 | **Medium** | Signed record detail page dual H1 | `ManifestDetailPageView.tsx:249-250` | Collapse "Signed decision record" (buyer) / "Finalized architecture review package" (operator) to one buyer-safe term consistently across modes |
| 9 | **Medium** | Export/deliverables surfaces | `RunDetailArtifactsExportsSection.tsx`, `buyer-polish-copy.ts:467-482`, `GoldenManifestExportMenu.tsx`, `ManifestBuyerBundleDownloadSection.tsx`, `RunDetailRunActionsSection.tsx:50` | Rename "Architecture package (DOCX)," "Download evidence package (ZIP)," "Export review package bundle," "Download finalized review package" to the resolved vocabulary |
