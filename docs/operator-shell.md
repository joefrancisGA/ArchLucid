...
1. **Start** — Open the app root (`/`). First-time users: expand **First-run workflow** on Home for a guided V1 checklist (create run → pipeline → commit → artifacts → compare/replay → export); **Hide checklist** collapses it (preference in browser `localStorage`). The header row **Start here · runs & review** mirrors that path: **Home**, **Onboarding**, **New run**, **Runs**, **Graph**, **Compare two runs**, **Replay a run**.
2. **Runs** — `Runs` → pick a project (default `default`) → **Open run** on a row (empty list shows **Create your first run (wizard)**).
3. **Run detail** — After commit, you see manifest summary, **Artifacts** (table with **Review** / **Download**), and shortcuts to **Compare two runs** (base = this run) and **Replay this run**.
