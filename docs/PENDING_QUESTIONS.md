...
35. **Coordinator → Authority pipeline unification — sequenced multi-PR plan ([ADR 0030](adr/0030-coordinator-authority-pipeline-unification.md))** — Phase 3 PR A's grounding read (2026-04-21) found three structural mismatches that block a single-session deletion. The ADR splits the work into PRs **A0 → A4**; the items below are the **per-sub-PR owner decisions** that have to land before the corresponding sub-PR can merge. Each is **owner-only** — the assistant cannot answer them from repository state.
    ...
    - **a. PR A0 — Authority engine projection shape.** Authority's "one-shot rule engine" must learn to emit a `Contracts.Manifest.GoldenManifest` projection alongside its native `Decisioning.Models.GoldenManifest`. Question: **does the projection live (i) inside the Authority engine itself, behind an opt-in flag, or (ii) in a new mapper class consumed by `RunCommitOrchestratorFacade`?** Trade-off: (i) keeps section ordering authoritative inside the engine but couples Authority to a Coordinator-shaped contract; (ii) keeps Authority pure but pushes the mapping responsibility to the application layer. ADR 0030 § Component breakdown / PR A0 records both options without picking one.

    - **b. PR A1 — `IGoldenManifestRepository` overload return shape.** Owner already approved **overload** vs new writer port in answer to in-session question `q_pra_authority_writes`. Open: **does the new `SaveAsync(Contracts.Manifest.GoldenManifest, ...)` overload return `Task` (write-only, like Coordinator's `CreateAsync`) or `Task<Guid>` (returning the generated `ManifestId`, matching the rest of `IGoldenManifestRepository`)?** The latter forces every call site that has a string `ManifestVersion` to reason about the GUID surface. ADR 0030 § PR A1 leaves this for the implementing PR.

    - **c. PR A2 — feature-flag scope for facade target swap.** `RunCommitOrchestratorFacade` switches from driving `ArchitectureRunCommitOrchestrator` (Coordinator) to driving the Authority engine + new write port. Question: **(i) per-tenant feature flag** so a single problem tenant can be rolled back without redeploy, or **(ii) global config switch** with full redeploy as the rollback?** Pre-release context (no paying customers) argues for (ii), but (i) is the only path that survives a partial-cutover incident if a paying customer onboards mid-rollout. ADR 0030 § PR A2 records this as the highest-risk sub-PR.

    - **d. PR A4 — `dbo.GoldenManifestVersions` table drop — backfill / archival policy.** The legacy table is dropped after PR A3 deletes the Coordinator concretes. Question: **(i) hard drop** (no historical Coordinator-shape rows preserved — pre-release acceptable per the same waiver as ADR 0029 gates (i)/(iv)), **(ii) archive to Blob** (`{tenantId}/coordinator-golden-manifest-versions/{utc-yyyyMMdd}.jsonl` before drop), or **(iii) backfill into `dbo.GoldenManifests`** (requires the inverse of the PR A0 mapper — likely lossy, since Coordinator rows lack section-level structure)? ADR 0030 § PR A4 calls option (ii) the default unless owner picks otherwise.

    - **e. Phase 3 PR B placeholder tracker (`docs/architecture/PHASE_3_PR_B_TODO.md`).** Owner answer to in-session question `q_pra_audit_pr_b_scope` was **`prb_create_tracker`**, but the ADR 0030 re-scope consumed this session's scope. Open: **standalone `PHASE_3_PR_B_TODO.md`** vs **inline checklist on ADR 0029 § Lifecycle**. Both fit the "PR B inherits the 2026-05-15 deadline" framing already recorded in ADR 0029 + ADR 0030.
