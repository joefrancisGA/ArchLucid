...
**Last updated:** 2026-04-22 (owner Q&A on items **35a** + **35b** — six sub-decisions resolved, PR A0 + PR A1 drafting unblocked, new sub-bullets **35f** opened — see *Resolved 2026-04-22 (ADR 0030 owner sub-decisions — 35a + 35b)* table below; supersedes the prior `Last updated` entry).
...
35. **Coordinator → Authority pipeline unification — sequenced multi-PR plan ([ADR 0030](adr/0030-coordinator-authority-pipeline-unification.md))** — Phase 3 PR A's grounding read (2026-04-21) found three structural mismatches that block a single-session deletion. The ADR splits the work into PRs **A0 → A4**; the items below are the **per-sub-PR owner decisions** that have to land before the corresponding sub-PR can merge. Each is **owner-only** — the assistant cannot answer them from repository state.
    ...
    - **c. PR A2 — feature-flag scope for facade target swap. (Still open.)** `RunCommitOrchestratorFacade` switches from driving `ArchitectureRunCommitOrchestrator` (Coordinator) to driving the Authority engine + new write port. Two coupled questions remain open: **(c.1) flag scope:** **(i) per-tenant feature flag** so a single problem tenant can be rolled back without redeploy, or **(ii) global config switch** with full redeploy as the rollback? Pre-release context (no paying customers) argues for (ii), but (i) is the only path that survives a partial-cutover incident if a paying customer onboards mid-rollout. **(c.2) default value of the `legacy:true` flag** by environment — recommended `on` in production, `off` in dev / CI for the duration of PR A2 → PR A3 so prod has a one-flag rollback while dev exercises the new path on every test run. ADR 0030 § PR A2 records this as the highest-risk sub-PR.

    - **d. PR A4 — `dbo.GoldenManifestVersions` table drop — backfill / archival policy. (Still open.)** The legacy table is dropped after PR A3 deletes the Coordinator concretes. Question: **(i) hard drop** (no historical Coordinator-shape rows preserved — pre-release acceptable per the same waiver as ADR 0029 gates (i)/(iv)), **(ii) archive to Blob** (`{tenantId}/coordinator-golden-manifest-versions/{utc-yyyyMMdd}.jsonl` before drop), or **(iii) backfill into `dbo.GoldenManifests`** (requires the inverse of the PR A0 mapper — likely lossy, since Coordinator rows lack section-level structure)? ADR 0030 § PR A4 calls option (ii) the default unless owner picks otherwise.

    - **e. Phase 3 PR B placeholder tracker (`docs/architecture/PHASE_3_PR_B_TODO.md`). (Still open.)** Owner answer to in-session question `q_pra_audit_pr_b_scope` was **`prb_create_tracker`**, but the ADR 0030 re-scope consumed this session's scope. Open: **standalone `PHASE_3_PR_B_TODO.md`** vs **inline checklist on ADR 0029 § Lifecycle**. Both fit the "PR B inherits the 2026-05-15 deadline" framing already recorded in ADR 0029 + ADR 0030.

    - **f. PR A0.5 — typed-services source for `ManifestService.ServiceType` / `RuntimePlatform`. (New — opened 2026-04-22.)** PR A0.5 grows first-class typed `ManifestService` and `ManifestDatastore` collections in `Decisioning.Models.GoldenManifest`, populated from the rule-engine resource graph (per 35a.2 = `empty-with-guard`). **Open:** the rule engine today emits resources keyed by string, not by typed (service-type, runtime-platform) tuples — does PR A0.5 (i) extend the rule engine's resource metadata to carry these typed fields directly, (ii) introduce a separate classifier service that runs after the rule engine and infers (service-type, runtime-platform) from resource string + Knowledge Graph context, or (iii) require the operator to tag resources at design time? Trade-off: (i) keeps the data path narrow but couples rule authoring to typed enums; (ii) keeps the rule engine generic but introduces a new classification step that can disagree with operator intent; (iii) is the most accurate but adds operator friction at the moment of resource creation. ADR 0030 § Component breakdown / PR A0.5 row records that this question must be answered before PR A0.5 can start (PR A0 + PR A1 are not blocked on it).
