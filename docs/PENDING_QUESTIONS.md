...
39. **"AI Architecture Review Board" rebrand workstream — schedule.** Assessment §4 cross-cutting q11 was resolved 2026-04-23 as "open to repositioning" toward "AI Architecture Review Board". The rebrand is a **multi-doc + multi-route** change (marketing site `/why`, `/pricing`, `/get-started`; sponsor brief; competitive landscape; per-vertical briefs; Trust Center; in-product copy on the operator-shell governance pages). **Owner-only:** schedule (V1, V1.1, or post-V1.1?) and rebrand owner (assistant proposes the rename, but final brand approval and any trademark / domain check is owner). Assistant safe default if not scheduled: do not change current product copy.
...
36. **Buyer-facing first-30-minutes doc — copy approval.** *(Partially resolved 2026-04-23 — see decisions C, D, E in the table above. **Still open:** the actual customer-facing prose / brand voice / screenshot selection for both the repo stub and the marketing route.)* The corresponding **buyer-facing** first-30-minutes walkthrough — "from `archlucid.com` landing page → signed in → first sample run → first finding, with **no local install of any kind**" — is now scheduled for **both** `docs/BUYER_FIRST_30_MINUTES.md` and the marketing `(marketing)/get-started/` route, with a **vertical-picker-first** preset. **Still needed from owner / marketing:** the actual copy. Do not draft customer copy without explicit go-ahead.

37. **In-product support-bundle download.** *(Partially resolved 2026-04-23 — see decisions F, G in the table above. **Still open:** the redaction policy when the bundle is later attached to a support ticket — what gets stripped before download, what gets stripped before forwarding to support.)* UI placement and role are decided (`/admin/support` page, gated on `ExecuteAuthority`). **Still needed from owner:** sign-off on the redaction rules (defaults: strip secrets, strip tenant PII unless the requester has Admin and explicitly opts in, keep config snapshot + recent run summaries + last 200 audit events).
