...
| **3c** | **`ui-e2e-live`** | After **`dotnet-full-regression`**. SQL service + **`docker run … sqlcmd`** creates empty **`ArchLucidLiveE2e`** (API does not auto-create the catalog). Then **ArchLucid.Api** + Playwright **`live-api-*.spec.ts`** via **`playwright.live.config.ts`**: happy path (**`live-api-journey`**) plus conflict journey (**200** idempotent repeat-commit, **404** missing run), governance **rejection** flow, and lightweight **error-state** UI checks. **Merge-blocking.** See **[LIVE_E2E_HAPPY_PATH.md](LIVE_E2E_HAPPY_PATH.md)**. |
