...
| **3c** | **`ui-e2e-live`** | After **`dotnet-full-regression`**. SQL service + **`docker run … sqlcmd`** creates empty **`ArchLucidLiveE2e`** (API does not auto-create the catalog). Then **ArchLucid.Api** + Playwright **`live-api-journey`** (`playwright.live.config.ts`): full operator happy path (create → execute → commit → manifest UI → run ZIP export → governance approval via API + dashboard **Load** → audit API + **`/audit`** UI). **Merge-blocking.** See **[LIVE_E2E_HAPPY_PATH.md](LIVE_E2E_HAPPY_PATH.md)**. |
