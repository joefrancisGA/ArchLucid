...
| **2d — Doc scope header** | Added **`scripts/ci/check_doc_scope_header.py`** (requires first non-empty line `> **Scope:**` under `docs/**` except `docs/archive/`; README may use `<!-- **Scope:** ... -->`). **`test_check_doc_scope_header.py`** + non-blocking CI step after `check_doc_links`. Root **`README.md`** carries the HTML scope line. | [`scripts/ci/check_doc_scope_header.py`](../scripts/ci/check_doc_scope_header.py), [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) | Done (warn-only until docs back-fill) |
...
| **2d — Scope-header back-fill** | The checker and CI step are **shipped** (see Section B). The remaining cost is adding `> **Scope:** …` as the first non-empty line to each active `docs/**/*.md` file (excluding `docs/archive/`). Until that lands, the CI step exits non-zero under `continue-on-error: true` and prints a capped file list every PR. | Incremental doc PRs (batch by directory), then remove `continue-on-error` from the workflow step. |
