...
| **2d — `check_doc_scope_header.py`** | The script itself is small; the real cost is **back-filling `> **Scope:**`** headers across ~150 existing `docs/*.md` files. Doing the script without the back-fill produces noise on every PR. | A separate doc-hygiene PR that ships the script + a one-time scope-header back-fill. |
