...
`ArchLucid Assessment – (A) Headline Readiness: 74.27%`. Readiness excludes deferred (V1.1/V2) items per the governing scope docs. Base pass computed fresh from the Weighted Quality Model in §2; **TB-654** rescore bumps Time-to-Value on operator stack answers + `archlucid stack init`.
...
| | **(A) Headline readiness** | | **100** | **74.27** | |
...
**Runtime & First-Review Reliability — score 82, weight 7, deficiency 126.** All six ship-gate items PASS. **TB-657** (2026-07-06) resolves CD vs Terraform Container App image drift — CD (`cd.yml` `az containerapp update`) owns runtime tags; `infra/terraform-container-apps` uses `lifecycle { ignore_changes = [template[0].container[0].image] }` on API, worker, and UI (primary + secondary) so routine `terraform apply` does not revert CD rollouts; drift guard in `Assert-TerraformDeploymentDriftPreflight.ps1` and Vitest. Affects: runtime reliability, deployability.
