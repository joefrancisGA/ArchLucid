...
2. **OAuth gap on newly-promoted connectors** — **TB-600 partial 2026-07-04** (Jira/ServiceNow/Confluence OAuth + migration runbook shipped); operator auth-code consent UI still open.
...
**TB-600 — OAuth 2.0 upgrade for Jira/ServiceNow/Confluence (partial shipped 2026-07-04).** Why it matters: basic-auth/API-token-only blocks enterprise buyers whose vendor security policy mandates OAuth. **Shipped:** tenant-scoped ITSM OAuth (`ItsmConnectorAuthMode`, migration `268`, token exchange, Bearer outbound); Confluence publish OAuth (`ConfluencePublishingHttpAuthenticator`); deployment-wide `Integrations:ItsmOutbound` OAuth fields; migration runbook `docs/runbooks/ITSM_CONFLUENCE_OAUTH_MIGRATION.md` (`BasicApiToken` default unchanged). **Remainder:** operator auth-code consent UI for Atlassian refresh-token acquisition. Expected impact: narrows §14 enterprise adoption blocker; full closure removes the named monetization blocker. Actionability: medium for remainder. Classification: **V1 GA tightening (in progress)**.
