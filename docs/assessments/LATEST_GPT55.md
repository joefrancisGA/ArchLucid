...
**Executed this pass (runtime evidence, not doc claims):** `dotnet test` on `ArchLucid.Decisioning.Tests` (808 passed / 0 failed), `ArchLucid.Core.Tests` (1242 passed / **4 failed**), `ArchLucid.Application.Tests` (3832 passed / **7 failed**, 3 skipped). Verified counts by direct inspection: **39** registered finding engines, **45** bundled policy-pack content files, **791** rules in `ga-starter-compliance.rules.json`, **6** engines in the golden-corpus harness.
...
**Capped by ship gate.** Ship Gate 4 is **FAIL** (export generation has failing tests on `master` — see §4). Per the rubric, a FAIL caps headline readiness regardless of the weighted score, so **74.02% is the uncapped weighted figure and should be read as "high-70s product with a currently red trunk," not as a shippable state.** Resolving the 11 failing tests is a mechanical prerequisite to claiming the weighted number.
...
The deduction is empirical and fresh: **11 tests fail on `master` right now.** In `ArchLucid.Core.Tests` (4): a ServiceBus app-property/operator-documentation guard, and three `CorePackageCoverageBatchRc27Tests` failures around legacy vendor alias resolution and a governance-promotion webhook schema sample. In `ArchLucid.Application.Tests` (7): three E2E comparison export formatter assertions, `CommitPathRedundantLoadContractTests` (commit orchestrator preloaded-governance contract), `BaselineMutationAuditDualWritePairingTests` (audit dual-write pairing guard), `TechnologyLedgerTopologyProposalSeederTests`, and `KnowledgeModelClarificationAnswerApplicatorTests`. Two of these — the audit pairing guard and the commit-path contract — are precisely the class of guard whose failure matters for evidence integrity.
...
2. **Red trunk — 11 failing tests on `master`.** Design uncertainty, trivially reducible. **V1 blocker** and a ship-gate FAIL. Fastest path: reconcile the three export formatter assertions with commit `3ebf8a7c78`, then the four Core and four remaining Application failures.
