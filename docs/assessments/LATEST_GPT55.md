...
| 4 | **M-17** — Build outreach list of 20 architects/CTOs/security leaders | Independent prep work; can run in parallel with #1–#3 | Partial — an agent can help structure/dedupe a candidate list the human supplies, not source real contacts | **Composer** — low-stakes list formatting/dedup |
| 5 | **M-18** — Send 20 outreach messages offering a 10-minute demo | Depends on #2 (landing page live) and #3 (demo video) | Partial — an agent can draft the outreach message template | **Sonnet** — persuasive, buyer-facing copy benefits from more careful phrasing than Composer-tier drafting |
| 6 | **M-19** — Run 5–10 live demos against Workspace A/B | Depends on #5 (outreach must land replies first) | No — live human-led demo calls | N/A — human only |
| 7 | **M-20** — Track objections from demos; refine positioning and demo script | Depends on #6 producing real objections to synthesize | Partial — an agent can synthesize raw notes into a structured objection log and suggest copy edits | **Opus** — synthesizing qualitative buyer feedback into strategic copy changes benefits from deeper reasoning than routine drafting |
| 8 | **M-39** — Apply `PROOF_PACKET_RUN_LOG_OPERATING_CHECKLIST.md` on every real pilot; reach ≥3 qualifying G4 rows | Depends on G-REAL-06 pilots existing; checklist itself already shipped | No — requires live pilot execution | N/A — human only |
| 9 | **G-REAL-06** — Execute three committed real-mode pilot runs | Stage 1 exit gate; both workspace smokes signed off 2026-07-03; ideally #6 (a demoed prospect willing to pilot) | No — real customer-facing pilot execution | N/A — human only |
| 10 | **G-REAL-07** — Collect proof packets per pilot run (`collect-first-pilot-proof.ps1`) | Directly depends on #9; script already exists | Partial — an agent can pre-validate the script's flags/output shape before the human runs it live | **Composer** — mechanical script-invocation verification |
