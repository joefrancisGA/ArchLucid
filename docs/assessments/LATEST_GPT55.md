...
# ArchLucid Assessment – (A) Headline Readiness: 82.04%
...
Rescore note: Through **81.98%** (5Z); batch **5AA TB-210** (pilot prerequisites page + `Test-ArchLucidPrerequisites.ps1`) → **82.04%** (+1 Adoption Friction). G-REAL/TB-140 owner-blocked.
...
| Adoption Friction | 74 | 6 | 3.86% | 156 |
...
Score: 74. Weight: 6. Weighted impact: 3.86%. Weighted deficiency signal: 156.

Justification: There is a strong first-pilot operator path, 20-minute first-value guide, proof packets, release smoke, and a clear Pilot vs Operate boundary. Batch **5X** closes **TB-207** — admin `POST /v1/admin/auth/diagnose-token` and `archlucid auth test-token --bearer` for JWT role-claim triage after SSO 403s. Batch **5Y** closes **TB-208** — `PackAsTool` CLI, **Publish CLI** workflow (NuGet + self-contained win/linux/osx binaries), and `CLI_INSTALL.md` linked from operator quickstart. Batch **5Z** closes **TB-209** — `archlucid request create --from-file` posts template JSON to `/v1/architecture/request` and prints `RunId`, replacing hand-rolled curl in operator quickstart. Batch **5AA** closes **TB-210** — `PILOT_PREREQUISITES.md` (per-profile Azure checklist, illustrative cost table, Azure AI Search blocking callout) plus `Test-ArchLucidPrerequisites.ps1` linked from operator path and configuration reference. But the path still demands SQL/auth/API/worker/proof/quality/procurement awareness. Product help still exposes repo paths and GitHub links in user-visible places. Some local failure states are confusing, including API/proxy outage toasts framed as AI assistant failures.
