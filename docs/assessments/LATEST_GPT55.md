...
| 8 | CodeQL C# SARIF gate | **FAIL** — **4** unresolved findings across two completed runs; suppressions were first anchored on the wrong lines, then anchored correctly but pushed out of range by a two-line justification |
...
- **CodeQL suppression placement is now understood and documented, after two failed attempts.** The prior pass annotated enclosing methods, so the gate reported all four findings. Re-anchoring onto the reported lines was still not enough: run [33026460733](https://github.com/joefrancisGA/ArchLucid/actions/runs/33026460733) reported the same four, with line numbers shifted by exactly the number of comment lines inserted — which proved the alerts had not moved and that a **two-line justification pushes the `// codeql[rule-id]` directive out of binding range**. The directive must be the bare line immediately above the alert, with prose above it. Both rules are now in `CODEQL_TRIAGE.md` with the run links, so the next person does not spend two CI cycles rediscovering them. The corrected form is pushed and awaiting its own confirming run — this pass does **not** claim the gate is green.
...
**The CodeQL fix is partial, and the residue is the sharpest single measurement in this assessment.** `cancel-in-progress: false` protects the *in-progress* run, so a scan can now finish. It does not give per-commit coverage: GitHub cancels any *pending* run in the concurrency group when a newer one queues. Runs [33026790482](https://github.com/joefrancisGA/ArchLucid/actions/runs/33026790482) and [33026802032](https://github.com/joefrancisGA/ArchLucid/actions/runs/33026802032) each hold **zero jobs** and were cancelled at the exact second the next run was created (00:25:44 against a 00:25:43 creation; 00:28:38 against 00:28:36), confirming they never started.

Then the measurement: across the **40** most recent `codeql.yml` runs on `master`, **34 were cancelled, 2 were still queued, and 4 concluded** — a 10% completion rate — while **51 commits landed on `master` in one hour**. A C# CodeQL analysis on this repository takes roughly 13–18 minutes. At a commit every ~70 seconds, the scanner cannot converge on any particular commit, and this pass could not obtain a confirming run for its own suppression fix despite the fix being present on `master`. **This is not a CodeQL configuration problem and no workflow edit solves it.** It is the same root cause as §8 weakness 1: an unguarded trunk absorbing direct pushes faster than verification can run. Every gate discussed in this assessment inherits that ceiling.
...
1. **The trunk absorbs direct pushes faster than any gate can verify them.** `ci.yml` runs only on `pull_request` and `workflow_dispatch`, while `master` takes direct pushes from many concurrent agents: **51 commits in one hour** during this pass. The only push-time gate is a thin UI typecheck corset that reported `skipped` on every run until this pass. The consequence is measurable, not theoretical — of the 40 most recent CodeQL runs on `master`, **34 were cancelled and 4 concluded**, because a 13–18 minute analysis cannot survive a commit every ~70 seconds. This is the **root cause** of weaknesses 2, 5 and 7 rather than a peer of them: v3 found one broken file, this pass found a different set, and the mechanism that allows both is unchanged. Process uncertainty. Fastest path: an owner policy decision — require PRs on `master`, or promote a minimal build+typecheck+fast-core job to a `push` trigger and accept latest-wins coverage. Note that a `push` gate alone does not fix verification *latency*; only reducing direct-push velocity does.
...
Tier 1 · **Why it matters:** the highest-leverage item in this assessment. `ci.yml` is `pull_request` + `workflow_dispatch` only; `master` took **51 commits in one hour** during this pass, and 34 of the last 40 CodeQL runs were cancelled as a direct result. Item 1 will be undone within hours without this. · **Expected impact:** Gate 5 stops regressing; §8 weaknesses 2, 5 and 7 stop recurring. · **Affected qualities:** Correctness (72), Runtime (66), Time-to-Value (71). · **Evidence:** `ci.yml` `on:` block; `ui-typecheck-on-push.yml` was the only push gate and reported `skipped` on every run until this pass. · **Actionability:** high for the workflow; the branch-protection half is an owner decision. · **Design uncertainty reduced: 9** · **Market uncertainty reduced: 1** · **Classification: V1 (process).**
...
**5. Confirm the CodeQL SARIF gate is green after the suppression-adjacency fix.**
Tier 1 · **Why it matters:** it is the only automated security signal that reaches a verdict on `master` push, and it has now failed twice for placement reasons rather than security reasons. · **Expected impact:** C# SARIF gate exit 0. · **Affected qualities:** Runtime (66), Correctness (72). · **Evidence:** run [33024730786](https://github.com/joefrancisGA/ArchLucid/actions/runs/33024730786) listed 4 unresolved findings against enclosing-method comments; run [33026460733](https://github.com/joefrancisGA/ArchLucid/actions/runs/33026460733) listed the same 4 with line numbers shifted by the inserted comment lines, proving a two-line justification breaks binding; commit `a46869cdd9` leaves a bare `// codeql[rule-id]` immediately above each alert. · **Actionability:** verification only, unless still red. · **Design uncertainty reduced: 6** · **Market uncertainty reduced: 1** · **Classification: V1.**

> **Cursor prompt.** **Current problem:** the C# SARIF gate has failed twice on suppression placement, not on security substance. All four findings are defensible false positives (2 × `SqlConnectionStringBuilder` construction where the returned string always passes through `EnsureSqlClientEncryptMandatory`; a `pageNumber < 1` pagination guard; a `PublishToProduct` request flag gated by `publishDecision.PublishBlocked`). **Desired behavior:** confirm `scripts/ci/assert_codeql_sarif_clean.py csharp-sarif` exits 0 on a completed `master` run at or after `a46869cdd9`. If any finding is still unresolved, take the line number from the gate output and ensure the **bare** `// codeql[rule-id]` line — no trailing prose, no second comment line between it and the code — sits immediately above that line; put justification prose above the directive. **Scope boundaries:** prefer structural fixes; do not disable the gate; do not add blanket suppressions. The JavaScript job fails at `Install and build UI`, which is item 1, not a CodeQL problem. **Acceptance criteria:** C# SARIF gate green on a completed run; `docs/library/CODEQL_TRIAGE.md` matches the final anchors. **Non-goals:** reopening TB-135/TB-136.
