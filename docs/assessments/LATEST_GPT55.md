...
Rescore note: Through **87.75%** (5CP). Batches **5CY-demo** through **5DC-demo** (**TB-273**: full BDA-001…150 remediation on the buyer-polished golden path; P2 polish in 5DC closes copy/hierarchy smells except deferred BDA-135/139/146) improve buyer-demo credibility under **`(B)`** and **Trustworthiness** narrative — **no `(A)` headline change** per `Assessment-Scope-V1_1.mdc` (TB-273 engineering scope complete). Batch **5DE–5DI** (TB-274 P0): route `{tenantId}` IDOR guards (executive-summary, reference-evidence, metering), redacted run traces list, trial register omits verification token, demo static fallback gated on env flags, `Demo:Enabled` fail-fast on production profile — **no `(A)` headline change** (trust/safety hardening under narrative). Batch **5DG-export-p0** (TB-274): Azure Blob SAS export push SSRF guard (HTTPS + blob host allowlist + DNS re-check), reference-evidence ZIP uses configured `ArchLucid:PublicApiBaseUrl` instead of raw `Host` header — **no `(A)` headline change**. Batch **5DH-evidence-p0** (TB-274): master DDL parity for curated-evidence tables/columns, tenant-scoped proposal list/read via `dbo.Runs`, atomic promote in unit-of-work, `EvidenceProposalPromotedUtc`, and unique `(TenantId, SourceResultId)` guard — **no `(A)` headline change** (data-integrity hardening under **Data consistency** narrative). Batch **5DJ-trust-p0** (TB-274): trial local JWT ignores caller-supplied tenant scope, value-report route uses `RouteTenantScopeAuthorization`, production fail-fast for `Demo:AnonymousViewer:Enabled`, IDOR regression for value-report generate — **no `(A)` headline change** (trust/safety under **Trustworthiness** narrative). Batch **5DK-sec-remainder** (TB-274): `SourceDocumentUrl` DNS re-check on architecture ingest, rate limits on search/billing webhooks/evidence admin routes, `registration` policy on marketing early-access and anonymous client-error funnel telemetry — **no `(A)` headline change** (SSRF/abuse hardening under **Trustworthiness** narrative). Batch **5DL-trust-p1** (TB-274): consolidated `DestinationSasUrl` DNS guard on export push API + service, `registration` rate limit on exec-digest unsubscribe, `fixed` on Slack interactivity webhook — **no `(A)` headline change**. Batch **5DM-usability** (**TB-270–272**): review-creation path disambiguation, client correlation id fallback on API errors, empty-state/loading consistency on dashboard and governance — **no `(A)` headline change** (operator friction under **Usability** narrative). Batch **5DL-trust-p2** (TB-274): SQL IDOR regression for `GET /v1/artifacts/runs/{runId}/export` and `POST …/export/push`, ApiKey scope-binding forbidden on artifact export probe, CI matrix drift guard for export routes — **no `(A)` headline change** (trust/safety under **Trustworthiness** narrative). Batch **5DN-demo-deferred** (**BDA-135**, **BDA-139**, **BDA-146**): home hero/section type scale, unified executive dashboard metric typography, manifest summary `topDecisionSynopses` with operator excerpt list — **no `(A)` headline change** (buyer-demo **(B)** / **Usability** narrative; **TB-273** BDA register engineering-complete). Batch **5DO-trust-remainder-p0** (TB-274): cross-tenant SQL IDOR for run-scoped `POST …/analysis-report` and `POST …/evidence/bulk`; `idempotency-posture` annotations on export push, trial registration, marketing early-access, and client-error telemetry — **no `(A)` headline change** (trust/safety under **Trustworthiness** narrative). Batch **5DO-trust-remainder-p1** (TB-274): `POST …/terraform-pr` cross-tenant IDOR regression; `idempotency-posture` on ingest/quick-scan/request-import/terraform-pr; mutating-route baseline sync for 11 hardened routes — **no `(A)` headline change**. Batch **5DP-mutating-posture-p1** (TB-274 / INV-009): `idempotency-posture` on ask, finding-ask, authority/comparison/export replay, analysis-report export family, advisory scheduling, and end-to-end compare summary; committed mutating-route baseline refresh — **no `(A)` headline change** (trust/reliability narrative under **Trustworthiness** / **Reliability**). Batch **5DQ-trust-ingest-p0** (TB-274): `idempotency-posture` on Azure extractor upload/sessions/connections; SQL IDOR regression for foreign `runId` upload and cross-tenant package download — **no `(A)` headline change**. Batch **5DR-trust-webhooks-p0** (TB-274 / INV-009): `idempotency-posture` on alert routing/rules/simulation/tuning/actions, digest subscriptions, advisory recommendation actions, Teams webhook connections, exec-digest preferences, and marketing quote-request; CI guard on alert/digest controllers — **no `(A)` headline change**. Batch **5DT-demo-revalidate-p0** (**TB-275**): buyer audit completion copy (**BDA-008**), unified finalize anchor (**BDA-001**), illustrative cost KPI label (**BDA-015**), explicit-demo-only curated audit inject (**BDA-006–007**) — **no `(A)` headline change** (buyer-demo **(B)** narrative). Batch **5DS-trust-internal-p0** (TB-274 / INV-009): `idempotency-posture` on internal architecture replay/determinism/seed, SCIM user/group create, governance approval batch/reject, stickiness dispositions/risk/recurrence, and governance/pre-commit simulate/draft routes; CI guard on internal/governance/SCIM controllers; mutating-route baseline **63→48** grandfathered unclassified — **no `(A)` headline change** (trust/reliability under **Trustworthiness** / **Reliability** narrative). Batch **5DU-route-tenant-p0** (**TB-276–278**): global `RouteTenantScopeBindingFilter` replaces per-controller `{tenantId}` guards; `assert_route_tenant_scope_guard.py` + matrix drift guard; unit tests for filter — **no `(A)` headline change** (trust/testability under **Trustworthiness** / **Testability** narrative). Batch **5DU-mutating-posture-p2** (TB-274 / INV-009): `idempotency-posture` on policy-pack, tenant, pilot, register, and billing-checkout mutators; CI guard on commercial controllers; mutating-route baseline **48→26** grandfathered unclassified — **no `(A)` headline change** (trust/reliability under **Trustworthiness** / **Reliability** narrative). Batch **5DV-mutating-posture-p3** (TB-274 / INV-009): final grandfathered slice — diagnostics, demo, evolution, MCP, governance-preview, ITSM, value-report generate; mutating-route baseline **26→0** unclassified — **no `(A)` headline change** (INV-009 posture register complete under **Reliability** narrative). Batch **5DW-trust-pilot-p0** (**TB-283–284**, **TB-289–294**): `BuyerRunDetailSummaryDto` + buyer-polished loader path; audience-tier Problem Details (`x-archlucid-audience: buyer`); live buyer golden-path E2E; SQL audit-trail commit integrity; reference-evidence ZIP integration; route-tenant positive-path matrix; production demo fail-fast host integration; sponsor value-report demo-run isolation API test — **no `(A)` headline change** (DTO boundary + trusted-pilot coverage under **Trustworthiness** / **Testability** / **Architectural integrity** narrative). Batch **5DW-trust-paid-p1a** (**TB-285–287**): `ProofSurfaceForbiddenPropertyDriftTests` + CI forbidden-property allowlist; OpenAPI `x-archlucid-audience` tiers + `buyer-contract.openapi.snapshot.json`; internal operator forensics route + `RequireOperatorRole` on tool-invocation-forensics + reader partition integration tests — **no `(A)` headline change** (DTO boundary P1 under **Testability** / **Architectural integrity** / **Trustworthiness** narrative). Batch **5DW-trust-paid-p1b** (**TB-295–300**): SQL audit export tenant isolation; export-push SSRF API integration; governance HTTP negative-path matrix + audit assertions; committed-run artifact download success path; executive board-pack live E2E + orphan/freshness contract checks; scope identity auth permutation table (ApiKey/JWT/DevBypass) documented in owner pen-test matrix — **no `(A)` headline change** (paid-pilot trust/testability coverage under **Trustworthiness** / **Testability** narrative). Batch **5DX-trust-p2** (**TB-288**, **TB-301**): `BuyerFacingDtoBoundaryArchitectureTests` mechanical guard against `ArchLucid.Persistence.*` return types on buyer-classified routes; five named Persistence tenant-read SQL scope-isolation probes (`SqlRunRepository`, `DapperAuditRepository.GetFilteredAsync`, golden manifest, governance approval, decision trace) + CI drift guards — **no `(A)` headline change** (DTO boundary P2 + repository defense-in-depth under **Testability** / **Architectural integrity** narrative). G-REAL/TB-140 owner-blocked.
