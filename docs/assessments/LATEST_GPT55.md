...
`ArchLucid Assessment – (A) Headline Readiness: 74.79%`. Readiness excludes deferred (V1.1/V2) items per the governing scope docs. Base pass computed fresh from the Weighted Quality Model in §2; **TB-668** rescore bumps Adoption Friction on the policy packs section tab semantics.
...
| 10 | Adoption Friction | 96 | 5 | 4.80 | 20 |
...
| **Iterative retrieve-critique-retry loop (RAG-V2-002 remainder)** | Owner **overrode** TB-598's pilot-signal-first deferral | Multi-round retrieval with a critique step deciding whether to re-query, beyond today's single-pass expansion | **V1 engineering — promoted, overrides validation-first hold; needs new TB number at implementation** |
| **Wire promoted fine-tuned deployments into agent completion routing (RAG-V2-003 / TB-594 follow-up)** | Owner **overrode** the Tier 3 hold below — full Cursor prompt already drafted (§ "Tier 3 detail" retained for the prompt text) | Tenant-opted-in fine-tuned completions actually get used once promoted, closing the gap between `IFineTunedModelRegistry` and `AzureOpenAiCompletionClient` | **V1 engineering — promoted, overrides Tier 3 hold; see Cursor prompt below** |
| **RAG-V1.1-001 (exemplar library + fingerprint matching), RAG-V1.1-003 (pilot-feedback retrieval), RAG-V1.1-004 (cross-tenant pattern library)** | Owner promoted all three from V1.1 during the same backlog review | Rounds out the RAG-V2 pull-forward already in V1 scope | **V1 engineering — promoted from V1.1, active** |
