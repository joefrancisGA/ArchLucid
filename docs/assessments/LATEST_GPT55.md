...
| 7 | `codeql.yml` on `master` | reaches a **conclusion** (run [33024730786](https://github.com/joefrancisGA/ArchLucid/actions/runs/33024730786)); three prior runs were **cancelled** |
...
7. **CodeQL C# SARIF gate is red.** 4 unresolved findings — 2 × `cs/insecure-sql-connection` on `SqlConnectionStringBuilder` construction, 2 × `cs/user-controlled-bypass` on a pagination guard and a publish-request flag. All four are defensible false positives; the suppressions were simply anchored on enclosing methods instead of the reported lines. Re-anchored this pass, pending CI confirmation. Process uncertainty.
