...
`ArchLucid Assessment – (A) Headline Readiness: 72.42%`. Readiness excludes deferred (V1.1/V2) items per the governing scope docs. Computed fresh this pass from the Weighted Quality Model in §2 — no score carried forward from any prior assessment file.
...
| 10 | Adoption Friction | 68 | 5 | 3.40 | 160 |
| | **(A) Headline readiness** | | **100** | **72.42** | |
...
2. **OAuth gap on newly-promoted connectors** — **TB-600 partial 2026-07-04** (Jira/ServiceNow OAuth foundation shipped); Confluence OAuth + auth-code UI still open.
...
**TB-600 — OAuth 2.0 upgrade for Jira/ServiceNow/Confluence (partial foundation shipped 2026-07-04).** Why it matters: basic-auth/API-token-only blocks enterprise buyers whose vendor security policy mandates OAuth. **Shipped this pass:** `ItsmConnectorAuthMode` on `TenantItsmConnectorConnections`, OAuth KV secret-name fields, `ItsmConnectorOAuthTokenExchanger` (ServiceNow client-credentials + Atlassian refresh-token grants), and `IItsmOutboundHttpAuthenticator` Bearer wiring on Jira/ServiceNow outbound + health probes (`BasicApiToken` default unchanged). **Remainder:** Confluence OAuth, operator auth-code consent UI, deployment-wide OAuth config, migration runbook. Expected impact: narrows §14 enterprise adoption blocker; full closure still removes the named monetization blocker. Actionability: medium-high for remainder. Classification: **V1 GA tightening (in progress)**.
