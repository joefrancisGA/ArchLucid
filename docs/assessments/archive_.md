> **Scope:** Rolling weighted readiness assessment for ArchLucid — canonical `(A)` headline score and improvement backlog for coding agents and the owner; not a buyer deliverable or historical archive.
> **Status:** current
...
### 2. Expose Data Archival Health in Operator UI *(Completed 2026-05-19)*
...
### 3. Implement Client-Side Handling for 402 Trial Limits *(Completed 2026-05-19)*
...
### 4. Add Markdown Export for Governance Resolution *(Completed 2026-05-19)*
...
- **Completed.** `Export to Markdown` on `/governance-resolution` downloads a UTC-timestamped snapshot (notes, conflicts, decisions, effective JSON).
```text
...
### 6. Optimize Knowledge Graph Snapshot Serialization *(Completed 2026-05-19)*
...
- **Completed.** `GraphJsonSerializerContext` + `GraphJsonSerialization` (source-generated metadata, canonical node/edge converters); `DefaultGraphBuilder` pre-sizes node lists; distributed projection cache uses the same JSON contract as SQL persistence.
```text
...
### 7. Add "Test Connection" Button for Webhook Subscriptions *(Completed 2026-05-19)*
...
- **Completed.** `POST /v1/webhooks/subscriptions/{id}/test` (alias `/integrations/webhooks/{id}/test`) signs with stored secret; `/settings/webhooks` shows HTTP status + response body.
```text
...
- **Completed.** `ArchLucid.Api/appsettings.Production.json` sets `DataArchival:PurgeUncommittedRunsAfterDays` to **7** with archival enabled; `DataArchivalCoordinator` batches `HardDeleteStaleUncommittedRunsBatchAsync`; `dbo.Archival_PurgeStaleUncommittedRunsBatch` excludes `IsDemoWelcomeRun` and `IsPublicShowcase`.
```text
...
- **Completed.** `AlwaysSampleActivitySourceSampler` + `ObservabilityTraceSamplingConfigurator` keep `ArchLucid.AuthorityRun` spans (`authority.*`, root `authority.run`) at `RecordAndSample` when `SamplingRatio` is below 1.0; other spans use the configured ratio; `ParentBasedSampler` delegates preserve authority children under unsampled HTTP parents.
```text
...
- **Completed.** `RunsController.CreateRun` accepts optional `Idempotency-Key`; `ArchitectureRunCreateOrchestrator` + `dbo.ArchitectureRunIdempotency` persist `(tenant, workspace, project, SHA-256(key))` → `runId` with payload fingerprint; same key + body → **200** + `Idempotency-Replayed: true`, fingerprint mismatch → **409**; `IDistributedCreateRunIdempotencyLock` and PK uniqueness for concurrent requests; omitting the header preserves **201** for legacy clients.
```text
...
Files: `ArchLucid.Api/Controllers/Authority/RunsController.cs`, `ArchLucid.Application/Runs/Orchestration/ArchitectureRunCreateOrchestrator.cs`, `ArchLucid.Persistence/Data/Repositories/ArchitectureRunIdempotencyRepository.cs`.
Constraints: Do not break existing non-idempotent clients.
```

### 14. Add Redis Cache Coherency Checks *(Completed 2026-05-19)*
...
- **Completed.** `HotPathCacheRules.CollectMultiReplicaCoherencyErrors` resolves the effective provider via `HotPathCacheProviderResolver`; when `HotPathCache:ExpectedApiReplicaCount` > 1 and the effective provider is Memory, startup fails outside Development; Development emits a warning via `HotPathMemoryReplicaCoherenceHostedLogger`. Single-replica deployments unchanged.
```text
...
- **Affected qualities:** Performance, Scalability.
- **Completed.** Migration **174** applies PAGE compression to `dbo.GoldenManifests` and `dbo.GoldenManifestDecisions`; `dbo.DecisioningTraces` already had **088**; repositories document `NVARCHAR(MAX)` + blob offload via `ManifestPayloadBlobUri` / `ArtifactLargePayload` (OPENJSON unchanged).
```text
...
- **Completed.** `CircuitBreakingContentSafetyGuard` + keyed `ContentSafetyAzure` `CircuitBreakerGate` (OTel metrics); circuit-open honors `FailClosedOnSdkError`.
```text
...
Files: `ArchLucid.AgentRuntime/Safety/CircuitBreakingContentSafetyGuard.cs`, `ArchLucid.AgentRuntime/Safety/AzureContentSafetyGuard.cs`.
...
- **Completed.** `IRunStateTransitionService` / `RunStateTransitionService` (`ArchLucid.Core/Runs`, `ArchLucid.Application/Runs`) centralizes commit/result/execute lifecycle rules; wired into `AuthorityRunOrchestrator`, `AuthorityDrivenArchitectureRunCommitOrchestrator`, `ManifestFinalizationService`, `ArchitectureRunExecuteOrchestrator`, `ArchitectureApplicationService` (result submit), and `ArchitectureRunAuthorityCoordination`. API contracts unchanged.
```text
...
Files: `ArchLucid.Persistence/Orchestration/AuthorityRunOrchestrator.cs`, `ArchLucid.Application/ArchitectureApplicationService.cs`, `ArchLucid.Application/Runs/Orchestration/*Orchestrator.cs`.
...
### 21. Add Export-Only Trial State Warning *(Completed 2026-05-19)*
...
### 22. Implement Configurable Alert Routing Rules *(Completed 2026-05-19)*
...
### 23. Add Policy Pack Conflict Resolution UI *(Completed 2026-05-19)*
...
### 25. Add System Health Dashboard *(Completed 2026-05-19)*
...
- **Affected qualities:** Observability, Supportability.
- **Completed.** `/health` dashboard (ReadAuthority nav) consumes `/health/live`, `/health/ready`, and `/version` (`processUptimeSeconds`).
```text
...
- ~~Implement API Key Rotation UI for Enterprise Trust (11)~~ *(completed 2026-05-19)*
- ~~Configure Tail-Based Trace Sampling for Authority Runs (12)~~ *(completed 2026-05-19)*
- ~~Implement Full Idempotency for Create Run Endpoint (13)~~ *(completed 2026-05-19)*
- ~~Add UI Toggle for AgentOutput:QualityGate:Mode (9)~~ *(completed 2026-05-19)*
- ~~Add Redis Cache Coherency Checks (14)~~ *(completed 2026-05-19)*
...
- ~~Simplify Domain Vocabulary in Operator UI (16)~~ *(completed 2026-05-19)*
- ~~Add Bulk Evidence Upload Progress Indicator (19)~~ *(completed 2026-05-19)*
- ~~Add Export-Only Trial State Warning (21)~~ *(completed 2026-05-19)*
- ~~Implement Visual Indicator for Trial Expiration in PDF Exports (8)~~ *(completed 2026-05-19)*

**Batch 3: Governance, Integration & Orchestration (Full Stack)**
- ~~Add "Test Connection" Button for Webhook Subscriptions (7)~~ *(completed 2026-05-19)*
- ~~Add Markdown Export for Governance Resolution (4)~~ *(completed 2026-05-19)*
- Unify Authority and Coordinator Pipeline Logic (18)
- ~~Enhance Advisory Scan Scheduling UI (20)~~ *(completed 2026-05-19)*
- ~~Implement Configurable Alert Routing Rules (22)~~ *(completed 2026-05-19)*
- ~~Add Policy Pack Conflict Resolution UI (23)~~ *(completed 2026-05-19)*

**Batch 4: Data Management, Safety & Observability (Backend)**
- Optimize SQL Server JSON Payload Storage (15)
- ~~Optimize Knowledge Graph Snapshot Serialization (6)~~ *(completed 2026-05-19)*
- ~~Add Content Safety API Circuit Breaker (17)~~ *(completed 2026-05-19)*
- Optimize Audit Log CSV Export (24)
- ~~Expose Data Archival Health in Operator UI (2)~~ *(completed 2026-05-19)*
- ~~Add System Health Dashboard (25)~~ *(completed 2026-05-19)*
