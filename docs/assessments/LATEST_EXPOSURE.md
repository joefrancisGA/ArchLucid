> **Scope:** Canonical **broader exposure readiness** assessment. Companion to [`LATEST_GPT55.md`](LATEST_GPT55.md) (strategic release/market readiness, `(A)` headline). This file answers a different question: *not* "is ArchLucid V1-ready," but "should ArchLucid be shown to people outside the founder's direct control, and under what constraints." Generated from [`assessment_prompt_v4.md`](assessment_prompt_v4.md). `/ship-next-improvement` and `/show-next-improvement` read this file's §20 Tier 1/2 rows as a secondary assessment-implementable source, after `LATEST_GPT55.md` §17.
...
| 9 | Core review package flow is understandable | **PASS** | `/onboarding` renamed "First review guide" (TB-674), lifecycle-derived Core Pilot checklist (TB-675) removes manual/misleading checkbox state. |
...
| 5 | Operator UI does not break on first-review path | **PASS** | First-review route smoke, unchanged. |
...
**Top confusion points (hypothesized, not observed live):** the distinction between "review package" and "architecture creation" (the exact ambiguity the v4 prompt names) — mitigated by the TB-645 buyer-vocabulary pass but not independently re-verified word-for-word on every surface this pass; the Architect/Team/Enterprise pricing tiers' relationship to workspace vs. seat count, which was not fully traced end-to-end.
...
> **Current problem:** Admin-visible AI budget/wallet infrastructure exists (`LlmBudgetStatusPill`, wallet cap/refill fields, `LlmJudgeDailyTokenBudgetTracker`), but this pass could not independently confirm whether a **hard, automatic** per-tenant AI-spend ceiling applies to a brand-new self-serve trial signup (as opposed to a founder-provisioned demo/trial tenant) before any admin has looked at a dashboard.
>
> **Desired behavior:** Trace the tenant-provisioning path for a self-serve signup (once the Team checkout flag is enabled) and confirm a default budget ceiling is applied at creation time, not just made visible after the fact. If a gap is found, add one, defaulting to a conservative ceiling that requires an explicit upgrade/admin action to raise.
>
> **Scope boundaries:** Do not change the ceiling value for existing founder-provisioned demo/beta tenants — this is specifically about the self-serve signup path, which does not exist in production traffic yet (checkout flag is off).
> * **Non-goals:** UI changes to the budget dashboard; changes to Admin-tier budget management.
>
> **Acceptance criteria:** A new self-serve tenant (simulated in a test) cannot exceed its default ceiling without an explicit, auditable upgrade action; the ceiling is enforced server-side, not merely displayed client-side.
>
> **Tests to add/update:** a new integration test asserting a freshly-provisioned trial tenant's AI spend is hard-capped at the default ceiling.
