...
# ArchLucid Assessment – (A) Headline Readiness: 93.64%
...
**Headline check:** `(90×8 + 96×8 + 94×6 + 91×7 + 94×5 + 97×4 + 99×2 + 100×2 + 99×1) ÷ 43 = **93.64%** (documented +0.83% from shipped **#1–#18**). Total weighted deficiency **283** (informational; headline uses scores, not deficiency sum).
...
4. **Custom policy pack authoring GTM — SHIPPED 2026-05-25 (Improvement #7):** Public `/pricing` Custom Policy Pack Authoring section, SoW + Order Form Addendum C links, quote form tier **Custom policy pack (professional services)**, and PROCUREMENT_FAQ Q12.
...
10. ~~**Tier-1 extractor ZIP upload errors opaque — post-GA (Improvement #18):**~~ **Mitigated 2026-05-25** — semantic error codes, remediation copy, and doc links on **`/settings/extract-upload`** via **`AzureExtractorUploadFailureCallout`** + **`azure-extractor-upload-error-resolver`**.
...
4. **Custom policy pack authoring GTM:** Shipped (Improvement **#7**, 2026-05-25).
...
5. **Azure OpenAI circuit breakers may trip too aggressively:** ~~Latency brownouts can fail entire authority runs instead of degrading gracefully — **#15** (post-GA).~~ **Mitigated 2026-05-25** — Production/Staging breaker thresholds, **`HalfOpenSuccessThreshold`**, LLM retry backoff tuning, and health **`openReason`** labels (`consecutive_failures` / `half_open_probe_failed`).
...
ArchLucid is **ready to ship V1 GA at 93.64%**. **Improvements #1–#18 shipped 2026-05-25**. The **V1 GA gate set is closed**. Post-GA **#19–#25** do not move headline.
...
**Additional V1 actionable (post-GA or parallel):** **#19–#25** (7 items).
...
### 7. Publish Custom Policy Pack Authoring GTM Before GA — **SHIPPED 2026-05-25**
- **Why it matters:** Improvement **archived #6** committed the SKU matrix, SoW template, and order-form addendum in repo docs — but sales and buyers still could not discover the offer from the **public pricing surface**. Enterprise PS is the highest-margin V1 lever; leaving it docs-only forced internal escalations and weakened the sales-led motion. Owner 2026-05-25: publish all GTM assets before GA (option A).
- **Expected impact:** Directly improves Adoption Friction (+2 pts when shipped) and Proof-of-ROI Readiness (+1 pt). Weighted readiness impact: **+0.08%** when shipped. **Shipped 2026-05-25.**
- **Affected qualities:** Adoption Friction, Proof-of-ROI Readiness, Time-to-Value.
- **Actionable now:** ~~Yes — **V1 GA gate** (publication layer on shipped #6 docs).~~ **Shipped 2026-05-25.**
...
### 15. Tune Azure OpenAI Circuit Breaker for Latency Brownouts — **shipped 2026-05-25**
...
- **Expected impact:** Reliability (+1 pt), AI/Agent Readiness (+1 pt). Weighted readiness impact: **+0.02%** when shipped.
...
### 18. Parse Tier-1 Extractor ZIP Upload Errors in UI — **shipped 2026-05-25**
...
- **Expected impact:** Adoption Friction (+1 pt), Supportability (+1 pt). Weighted readiness impact: **+0.02%** when shipped.
- **Affected qualities:** Adoption Friction, Supportability, Time-to-Value.
- **Shipped:** **`azure-extractor-upload-error-resolver`** semantic codes (unsupported schema, missing manifest/resources, corrupt ZIP, run scope mismatch, size/no-file); **`AzureExtractorUploadFailureCallout`** surfaces code + API **`VALIDATION_FAILED`** + troubleshooting doc link on **`/settings/extract-upload`**; Vitest in **`azure-extractor-upload-failure.test.ts`** and **`ExtractUploadSettingsPageClient.test.tsx`**.
...
Run ~~**#18** (Tier-1 ZIP errors)~~ **shipped 2026-05-25**, **#25** (SAML claim-mapping checklist). FinOps cluster: **#19**, **#20** (LlmCostEstimator fixes + tests). ~~**#17** (Tier-2 SP wizard)~~ **shipped 2026-05-25**.
...
### Custom policy pack authoring GTM — **SHIPPED 2026-05-25**
...
- **Score posture:** **Shipped 2026-05-25** (+0.08% weighted impact documented in improvement).

No other open `(A)`-blocking owner questions — **V1 GA gate set closed 2026-05-25.** **#1–#7 shipped 2026-05-25.** Items below were removed because they pertain to V1.1 / V2 scope per `Assessment-Scope-V1_1.mdc` and therefore must not penalize `(A)` or appear as `(A)` pending questions:
