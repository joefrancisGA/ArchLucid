...
**Score change 2026-05-24 (afternoon):** Rescored upward from 87.74% to 89.93% after removing latent V1.1/V2 penalties. **2026-05-24 (later):** 89.93% → **90.07%** (bulk-CSV → V2, [`V1_DEFERRED.md` §6r](../library/V1_DEFERRED.md)). **90.07% → 90.21%** (capacity guide → V2; later absorbed into §6t). **90.21% → 90.35%** (full **self-hosted Enterprise deals** motion → V2, [`V1_DEFERRED.md` §6t](../library/V1_DEFERRED.md); **V1 GA Enterprise = ArchLucid-hosted SaaS only**). Custom policy pack authoring SKU decided — see Improvement #6. Cross-tenant ROI deduplication tests shipped — see Improvement #3. **90.35% → 90.49%** (Batch 4 partial: vector-store health check #10, Ask RAG SQL fallback #13, executive ROI cache warmup #16, SAML SP startup validation #18). **90.49% → 90.85%** (Batch 1 complete: policy-pack indexing #1, LLM faithfulness evaluator #2, prior-manifest chunks #23). **90.85% → 90.94%** (Batch 2 complete: RAG retrieval telemetry #7, Grafana ROI panel #12, integration outbox dashboard #19, LLM redaction telemetry #25). **90.94% → 91.12%** (Batch 3 #24 board-pack ROI export; Batch 4 complete #8 agent-trace blob cleanup, #22 quality-gate reject thresholds in base config). **91.12% → 91.34%** (Batch 6 complete: custom policy pack PS SKUs #6, pricing quote SLA #11). **91.34% → 91.39%** (Batch 8 complete: hot-path list indexes #26, RunId UNIQUEIDENTIFIER migration #27, archive cascade TVP #28). **91.39% → 91.40%** (Improvement #29: trust 14 core-chain FK constraints). **91.40% → 91.41%** (Improvement #30: missing alerting/advisory/governance FK constraints). **91.41% → 91.42%** (Improvement #31: enumeration CHECK constraints). **91.42% → 91.44%** (Improvement #32: scope column NOT NULL enforcement). **91.44% → 91.45%** (Improvement #33: filtered GoldenManifests unique index + deferred constraint rationale). **91.45% → 91.47%** (Improvement #34: route analytics metrics readers through IReadOnlyDbConnectionFactory). **91.47% → 91.48%** (Improvement #35: WITH (NOLOCK) on display-layer and analytics reads). **91.48% → 91.49%** (Improvement #36: analytics columnstore indexes on AuditEvents, FindingRecords, GoldenManifests). **91.49% → 91.50%** (Improvement #37: PurgeCascade_Core consolidation + elastic pool tempdb vCore validation). **91.50% → 91.51%** (Improvement #39: OpenTelemetry SQL client instrumentation). **91.51% → 91.53%** (Improvement #40: SQL connection pool Prometheus gauges). **91.53% → 91.54%** (Improvement #41: SqlConnectionHealthCheck latency brownout detection). **91.54% → 91.55%** (Improvement #42: Azure SQL Intelligent Insights diagnostics + platform metric alerts). **91.55% → 91.57%** (Improvement #43: PITR 35d + geo-redundant backups + LTR policy). **91.57% → 91.59%** (Improvement #44: production failover group precondition + SqlServer:FailoverGroupListenerFqdn validation). **91.59% → 91.60%** (Improvement #45: GRS artifact storage example + backup restore drill runbook). **91.60% → 91.62%** (Improvement #46: P0-critical action group SMS/voice/PagerDuty + Prometheus P0 routing). **91.62% → 91.63%** (Improvement #47: SQL extended auditing + Microsoft Defender for SQL). **91.63% → 91.64%** (Improvement #48: Production SQL MI enforcement + TrustServerCertificate block). **91.64% → 91.65%** (Improvement #49: SQL TDE CMK Terraform + immutable al_rls_bypass session context). **91.65% → 91.67%** (Improvement #50: RetrievalIndexingOutbox lease/dead-letter + Prometheus gauge). **91.67% → 91.69%** (Improvement #51: Terraform advisory C# inline documentation). **91.69% → 91.71%** (Improvement #52: infra Terraform inline documentation pass). **91.71% → 91.73%** (Improvement #53: DependencyConstraintTests gap closure). **91.73% → 91.75%** (Improvement #54: prune dead Api.csproj Decisioning/KnowledgeGraph refs). **91.75% → 91.77%** (Improvement #57: OpenAPI ↔ Api.Client ↔ UI contract parity). **91.77% → 91.79%** (Improvements #55–#56: lateral coupling policy closure + Host.Composition DI lifetime fixes).

## Executive Summary

**`(A)` Overall Headline Readiness**
ArchLucid is well past the pilot-credible bar for V1 GA at 91.39%. The core architecture, observability, audit, governance, and trial-funnel plumbing are production-ready. Batch 8 hardened SQL persistence: hot-path run-list indexes (#26), typed RunId FK integrity (#27), and consolidated archive cascade (#28). The remaining `(A)` deficit is concentrated in RAG-V1 follow-ons (tenant-assigned pack filter, cross-run golden-manifest history, citation formatter, optional USD savings gauge). None of these block V1 GA; they raise operator confidence and output trustworthiness ceilings.
...
- **Justification:** The agent runtime is solid — circuit breakers, content safety, prompt-redaction telemetry with Grafana panels and `ArchLucidHighPromptRedactionRate` alert (#25, Batch 2, 2026-05-24), trace blob storage with SQL inline fallback, four agent types (Topology, Cost, Compliance, Critic), and a quality-gate scaffold with configurable reject floors. Ask RAG SQL fallback (#13, 2026-05-24) keeps Ask partially functional when the vector store is down. Batch 1 (2026-05-24) wired `AgentOutputFaithfulnessEvaluator` with `archlucid_agent_output_llm_faithfulness_score` histogram (opt-in via `ArchLucid:Agents:LlmFaithfulness:Enabled`, default off). Batch 4 (#22, 2026-05-24) documents zero reject floors in base `appsettings.json` (warn-only posture) while production/staging bind explicit `StructuralRejectBelow` / `SemanticRejectBelow` with optional `EnforceOnReject` / `BlockRunOnReject`.
...
- **Recommendations:** Document and promote LLM faithfulness opt-in for production tenants; consider agent retry on quality-gate reject (future orchestrator enhancement).
...
- **Weight:** 4
- **Weighted Deficiency:** 8
- **Justification:** Executive ROI Summary endpoint, operator-shell `ExecutiveRoiSummarySection`, CSV export CLI (#9, 2026-05-24), board-pack Markdown/PDF export API + CLI (#24, Batch 3, 2026-05-24), leader-elected cache warmup hosted service (#16, 2026-05-24), and Grafana **Business Value** row on `dashboard-archlucid-authority.json` with severity-weighted findings proxy (#12, Batch 2, 2026-05-24) are live. Remaining V1 deficit: no `archlucid_tenant_estimated_savings_usd` background gauge.
...
- **Recommendations:** Emit optional USD savings gauge from a background job (#12 follow-on).
- **Status:** Fixable in V1.
...
- **Recommendations:** Relational integrity hardening (Improvements #29–#33, Batch 9).
- **Status:** Fixable in V1.
...
- **Justification:** Central Package Management, warnings-as-errors, `EnforceCodeStyleInBuild`, strict CI, dependency vulnerability scanning, SBOM publication, gitleaks, merged coverage gates, and a clear bounded-context layout make this codebase exceptionally maintainable. Batch 4 (#8, 2026-05-24) added leader-elected `AgentResultBlobCleanupHostedService` + `AgentTraceOrphanBlobCleanupService` with `DataArchival:BlobCleanup:Enabled`. SQL persistence layer has two notable maintainability debts: (1) the archive cascade logic — eight `IF COL_LENGTH` / `UPDATE … SET ArchivedUtc` blocks — is duplicated verbatim in both `ArchiveRunsCreatedBeforeAsync` and `ArchiveRunsByIdsAsync`; (2) `ArchiveRunsByIdsAsync` makes two sequential SQL round trips (SELECT state, then UPDATE cascade) where one is sufficient. Architecture test coverage has 13 identified gaps: two hexagonal guard omissions (`Provenance` and `Capabilities.Cost` are not protected from `Persistence`), two `Api` boundary tests that enforce only at the type level rather than the stricter assembly-metadata level, and nine unguarded lateral domain couplings spanning `Decisioning→Notifications`, `Provenance→{ArtifactSynthesis,Decisioning,KnowledgeGraph}`, `Retrieval→{Decisioning,ArtifactSynthesis,Provenance}`, and `AgentRuntime→{Decisioning,Provenance}` (see Improvement #53).
...
2. **LLM faithfulness opt-in default off:** Evaluator ships opt-in via `ArchLucid:Agents:LlmFaithfulness:Enabled`; production tenants must explicitly enable for faithfulness histograms.
...
4. **Optional USD savings Grafana gauge not shipped:** Batch 2 added severity-weighted findings proxy (#12); runtime `EstimatedUsdSavings` is still not emitted as a Prometheus gauge.
5. **Custom policy pack authoring SKU decided but not yet published:** SKU matrix approved (Improvement #6) but pricing pages, SoW template, and order-form line items have not landed.
6. **No in-product expansion CTA when tenants approach tier limits:** Existing Team tenants nearing seat or workspace caps see no in-app prompt to upgrade or request a quote.
7. **Marketing pricing quote-request follow-up has no measured SLA:** Rows are written and email goes out, but no committed response-time SLA, aging dashboard, or escalation rule.
8. **Quality gate agent retry on reject not implemented:** Reject floors and run-blocking paths ship (#22); orchestrator does not yet automatically retry agents on reject.
...
1. **No in-product upgrade nudge from trial to paid:** Usage-based trial upgrade nudge shipped (#14, 2026-05-24); remaining gap is Team→Professional expansion CTA.
2. **No in-product expansion CTA for Team → Professional or seat / workspace adds:** Existing tenants nearing tier ceilings see no expansion prompt; expansion revenue depends entirely on the CSM motion.
3. **Marketing pricing quote-request follow-up has no measured SLA:** `dbo.MarketingPricingQuoteRequests` rows are written and an email goes out, but no committed response-time SLA, no aging dashboard, no escalation rule.
4. **Custom policy pack authoring SKU is decided but not yet published:** Per Improvement #6 — the SKU matrix is approved (Starter / Standard / Program with shared-IP discount) but pricing pages, SoW template, and order-form line items have not landed.
...
3. **Custom policy pack authoring SKU not yet published on pricing pages:** SKU matrix approved (Improvement #6) but GTM publication incomplete.
...
9. ~~**Unguarded lateral domain couplings allow hidden transitive dependency creep**~~ **Resolved 2026-05-25 (#55):** Option B recorded; all listed edges pinned with `_by_design` csproj facts in `DependencyConstraintTests` (port inversion deferred).
...
11. ~~**Public HTTP contract under-documents wire JSON**~~ **Resolved 2026-05-25 (#57):** `RunSummaryResponse` shadow fields, `required` arrays on high-traffic schemas, generated client/TS regen, and `API_CONTRACTS.md` run-shape routing documented.
...
### 8. Implement Automated Cleanup for Orphaned AgentResult Blobs (completed 2026-05-24)
...
- **Actionable now:** No (completed).
- **Completed (Batch 4, 2026-05-24):** `AgentResultBlobCleanupHostedService` (leader-elected daily loop), `AgentTraceOrphanBlobCleanupService`, `DataArchival:BlobCleanup:Enabled` / `MinAgeDays`, metric `archlucid_data_archival_blobs_deleted_total`.
```cursor
...
### 22. Add Explicit Reject Thresholds to AgentOutputQualityGate (completed 2026-05-24)
...
- **Affected qualities:** AI/Agent Readiness, Reliability.
- **Actionable now:** No (completed).
- **Completed (Batch 4, 2026-05-24):** `StructuralRejectBelow` / `SemanticRejectBelow` documented with zero defaults in base `appsettings.json`; production/staging bind non-zero floors with optional `EnforceOnReject` / `BlockRunOnReject`. Gate evaluation and orchestrator blocking paths were already implemented; **deferred:** automatic agent retry on reject.
```cursor
...
### 24. Implement Board-Pack ROI One-Pager Export (completed 2026-05-24)
...
- **Affected qualities:** Executive Value Visibility, Proof-of-ROI Readiness.
- **Actionable now:** No (completed).
- **Completed (Batch 3, 2026-05-24):** `GET /v1/roi/executive-summary/board-pack?format=md|pdf`, `ExecutiveRoiBoardPackExporter` + Markdown/PDF builders, CLI `archlucid roi board-pack`, audit `ExecutiveRoiBoardPackExported`.
```cursor
...
### 26. Add Filtered Covering Indexes for Hot-Path Run List Correlated Subqueries (completed 2026-05-24)
...
### 27. Migrate NVARCHAR(64) Run-ID Columns to UNIQUEIDENTIFIER for FK Integrity (completed 2026-05-24)
...
### 28. Consolidate Archive Cascade to TVP Stored Procedure and Reduce Round Trips (completed 2026-05-24)
...
### 29. Trust the 14 WITH NOCHECK FK Constraints on the Core Execution Chain (completed 2026-05-24)
...
### 30. Add Missing FK Constraints on Alerting, Advisory, and Governance Tables (completed 2026-05-24)
...
### 31. Add CHECK Constraints to 13 Enumeration Columns (completed 2026-05-24)
...
### 32. Enforce NOT NULL on Nullable Scope Denormalization Columns (completed 2026-05-24)
...
### 33. Fix Unfiltered Unique Index on GoldenManifests and Resolve Deferred Unique Constraints (completed 2026-05-24)
...
### 34. Route Analytics and Metrics Readers Through IReadOnlyDbConnectionFactory (completed 2026-05-24)
...
### 35. Add WITH (NOLOCK) to Remaining Display-Layer and Analytics Reads (completed 2026-05-24)
...
### 36. Add Non-Clustered Columnstore Indexes on Analytics-Heavy Tables (completed 2026-05-24)
...
### 37. Codify tempdb File Configuration in Terraform and Consolidate Duplicate Purge Procedures (completed 2026-05-24)
...
### 39. Add OpenTelemetry SQL Client Instrumentation (completed 2026-05-24)
...
### 40. Publish Connection Pool Metrics to Prometheus (completed 2026-05-24)
...
### 41. Enhance SqlConnectionHealthCheck with Latency Measurement (completed 2026-05-24)
...
### 42. Enable Azure SQL Intelligent Insights and Wire Alerts into Prometheus (completed 2026-05-24)
...
### 43. Harden SQL Backup: PITR 35 Days, Geo-Redundant Backup Storage, and Long-Term Retention Policy (completed 2026-05-24)
...
### 44. Enforce Failover Group Is Enabled for Production and Wire the Primary Connection String to the Listener FQDN (completed 2026-05-24)
...
### 45. Enforce GRS for Production Artifact Storage and Create a Backup Restore Drill Runbook (completed 2026-05-24)
...
### 46. Wire Two-Tier Alert Routing with SMS, Voice, and PagerDuty Free for Escalating Wakeup (completed 2026-05-24)
...
### 47. Enable Microsoft Defender for SQL and SQL Server Auditing in Terraform (completed 2026-05-24)
...
### 48. Enforce Managed Identity Authentication and Block TrustServerCertificate in Production (completed 2026-05-24)
...
### 49. Add TDE with Customer-Managed Key in Terraform and Fix al_rls_bypass Session Context Read-Only Flag (completed 2026-05-24)
...
**Status:** **Completed 2026-05-24** — migration 219, leased dequeue, dead-letter path, and `archlucid_retrieval_indexing_outbox_dead_lettered_total` gauge shipped. KEDA SQL scaler deferred per Part 2.
...
**Assessment date:** 2026-05-24
**Status:** **Completed 2026-05-24** — method-level XML docs added across 13 Terraform advisory C# files; malformed `TerraformGitHubPrOptions.PersonalAccessToken` doc fixed.
...
**Assessment date:** 2026-05-24
**Status:** **Completed 2026-05-24** — inline `#` comments on priority roots (entra, container-apps, edge, alerts), all `versions.tf` provider pinning notes, and `terraform.tfvars.example` mandatory-plan headers; `terraform fmt -check` green.
...
**Assessment date:** 2026-05-24
**Status:** **Completed 2026-05-24** — 13 new facts in `DependencyConstraintTests.cs` (M1–M13); lateral couplings pinned with `_by_design` / csproj checks per #55 Option B.
...
**Assessment date:** 2026-05-24
**Status:** **Completed 2026-05-24** — removed dead `Decisioning` and `KnowledgeGraph` `ProjectReference` entries from `ArchLucid.Api.csproj`; Api builds via transitive Application references.
...
**Status:** **Completed 2026-05-25** — **Option B (pin as intentional)** recorded for all eight listed edges; `_by_design` csproj facts cover Decisioning→Notifications, Provenance→{ArtifactSynthesis, Decisioning, KnowledgeGraph}, Retrieval→{Decisioning, ArtifactSynthesis, Provenance}, AgentRuntime→{Decisioning, Provenance}, and Backfill.Cli→Persistence. Contracts port inversion deferred to a future backlog item.
...
**Recorded decisions (Option B — 2026-05-25):**

| Edge | Decision | Pinning test |
|------|----------|--------------|
| Decisioning→Notifications | Pin | `Decisioning_csproj_references_Notifications_by_design` |
| Provenance→ArtifactSynthesis | Pin | `Provenance_csproj_references_ArtifactSynthesis_by_design` |
| Provenance→Decisioning | Pin | `Provenance_csproj_references_Decisioning_by_design` |
| Provenance→KnowledgeGraph | Pin | `Provenance_csproj_references_KnowledgeGraph_by_design` |
| Retrieval→Decisioning | Pin | `Retrieval_csproj_references_Decisioning_by_design` |
| Retrieval→ArtifactSynthesis | Pin | `Retrieval_csproj_references_ArtifactSynthesis_by_design` |
| Retrieval→Provenance | Pin | `Retrieval_csproj_references_Provenance_by_design` |
| AgentRuntime→Decisioning | Pin | `AgentRuntime_references_Decisioning_by_design` |
| AgentRuntime→Provenance | Pin | `AgentRuntime_references_Provenance_by_design` |
| Backfill.Cli→Persistence | Pin | `BackfillCli_references_Persistence_by_design` |
...
**Status:** **Completed 2026-05-25** — `IAzureDevOpsPullRequestDecoratorFactory` creates per-message decorators from named `HttpClient`; `SqlScopedResolutionDbConnectionFactory` disposal contract documented in XML + `ARCHITECTURE_COMPONENTS.md`.
...
- [x] `AuthorityRunCompletedAzureDevOpsIntegrationEventHandler` no longer captures `IAzureDevOpsPullRequestDecorator` in its constructor; `HttpClient` is obtained per-message via `IHttpClientFactory` or the handler's registration lifetime matches the typed-client lifetime.
- [x] `SqlScopedResolutionDbConnectionFactory.CreateOpenConnectionAsync` XML doc updated to require `using` at all call sites; a code-review checklist entry or analyzer rule enforces this.
- [x] `ValidateScopes = true` (already the default in Development) passes after the handler fix; no new Singleton→Scoped captures introduced.
...
**Status:** **Completed 2026-05-25** — OpenAPI snapshot, Api.Client, and `api-types` regen; `OpenApiContractInvariantsTests` required-field assertions; UI wire-extension cleanup; `API_CONTRACTS.md` run DTO routing table.
...
- [x] `RunSummaryResponse` OpenAPI includes `isSample`, `hasWarnings`, `hasGovernanceWarnings`; snapshot + NSwag + TS regen committed together.
- [x] `OpenApiContractInvariantsTests` asserts `required` on `RunSummaryResponse`, `RunRecord`, and `ManifestSummaryResponse` high-signal fields.
- [x] `docs/library/API_CONTRACTS.md` documents `ArchitectureRun` vs `RunRecord` route mapping.
- [x] `CreateArchitectureRunResponsePayload` removed in favor of generated types; stale `RunSummaryWireExtensions` snapshot-id fields removed or promoted to OpenAPI.
- [x] `OpenApiContractBackwardCompatibilityChecker` and full Core CI suite green.
...
**Batch 3 — CLI surfaces & operator export tools** — **completed 2026-05-24**
...
**Shipped:** #24 board-pack Markdown/PDF API + CLI (prior items #4, #5, #9, #15, #21 were already complete).

**Batch 4 — Reliability, hosted services, startup validation** — **completed 2026-05-24**
...
**Shipped:** #8 orphan agent-trace blob cleanup + metric; #22 reject floors in base config with production overrides; #10, #13, #16, #18 from earlier partial pass.
...
**Batch 8 — SQL Server persistence quality** — **completed 2026-05-24**
...
**Shipped:** #26 migration `202_HotPathListIndexes`; #27 migrations `203`–`210` (eight RunId columns + FKs); #28 migration `211` (`ArchivedRunIdList` TVP + `Archival_CascadeFromArchivedRuns` proc) with `SqlRunRepository` single-batch archive refactor.

**Batch 9 — Relational integrity hardening** — **completed 2026-05-24**
...
**Shipped:** #29 migration `212_TrustCoreChainForeignKeys`; #30 migration `213_MissingForeignKeys`; #31 migration `214_EnumerationCheckConstraints`; #32 migration `215_ScopeColumnsNotNull`; #33 migration `216_GoldenManifests_UniqueIndex_Filtered` (GraphSnapshot/ArtifactBundle multi-row retries documented in master DDL).

**Batch 10 — Read-path concurrency hardening** — **completed 2026-05-24**
...
**Shipped:** #34 — analytics readers on `IReadOnlyDbConnectionFactory`; #35 — NOLOCK on audit timeline, advisory lists, policy catalog/version/assignment lists, and metrics reader SELECT shapes; audit hot-path constants renamed `*NoLock`.

**Batch 11 — tempdb hardening** — **completed 2026-05-24**
...
**Batch 13 — SQL observability** — **completed 2026-05-24**
...
**Shipped:** #39 `AddArchLucidSqlClientInstrumentation`; #40 `SqlConnectionPoolMetricsHostedService` + `SqlPoolNearExhaustion`; #41 `SqlConnectionHealthCheck` SELECT 1 latency + production readiness wiring; #42 `infra/modules/azure-sql-serverless-app/monitoring.tf` + authority/integration outbox Prometheus alerts.

**Batch 14 — Backup, DR, and RTO/RPO hardening** — **completed 2026-05-24**
...
**Shipped:** #43 PITR 35d + Geo backup + LTR in `backup.tf`; #44 production failover group precondition + `SqlServer:FailoverGroupListenerFqdn` validation; #45 GRS storage validation + `BACKUP_RESTORE_DRILL.md` runbook.

**Batch 15 — On-call alerting (wake-up routing)** — **completed 2026-05-24**
...
**Shipped:** #46 `azurerm_monitor_action_group.critical` + Key Vault secret resolution + `prometheus_p0_rules.tf` + P0 labels in `archlucid-alerts.yml` + `production.tfvars.example`.

**Batch 16 — SQL security hardening** — **completed 2026-05-24**
...
**Shipped:** #48 Production SQL password rejection + `TrustServerCertificate=False` in factories; #47 SQL auditing + Defender Terraform; #49 TDE CMK module + `@read_only = 1` on `al_rls_bypass`.

**Batch 17 — SQL-as-queue hardening** — **completed 2026-05-24**
...
**Shipped:** #50 migration 219 (`LockedUntilUtc`, `AttemptCount`, `DeadLetteredUtc`, backoff columns); leased `READPAST/UPDLOCK/ROWLOCK` dequeue; `RecordDeadLetterAsync` / `CountDeadLetteredAsync`; processor retry/dead-letter; `archlucid_retrieval_indexing_outbox_dead_lettered_total` gauge; in-memory lease tests.

**Batch 18 — Terraform documentation** — **completed 2026-05-24**
...
**Shipped:** #51 XML docs on Terraform advisory C# emit pipeline; #52 inline docs on entra/container-apps/edge/alerts roots, all `versions.tf` files, and mandatory-plan headers on key `terraform.tfvars.example` files.

**Batch 19 — Architecture boundary hardening** — **completed 2026-05-25**
...
**Shipped (#53–#55):** M1–M13 facts; Api.csproj prune; Option B decision table with Retrieval `_by_design` pins.
**#55 decision (Option B — pin as intentional):** Decisioning→Notifications, Provenance→{ArtifactSynthesis, Decisioning, KnowledgeGraph}, Retrieval→{Decisioning, ArtifactSynthesis, Provenance}, AgentRuntime→{Decisioning, Provenance}, Backfill.Cli→Persistence remain pinned until Contracts port inversion is scheduled.

**Batch 20 — Public HTTP contract hardening** — **completed 2026-05-25**
...
**Shipped:** #57 Phases A–E in one pass — `PublicHttpContractSchemasOpenApiDocumentTransformer`, snapshot/client/TS regen, invariant tests, UI type cleanup, `API_CONTRACTS.md` run-shape table, snapshot test DeepEquals fast path.

**Batch 21 — Host.Composition DI lifetime hardening** — **completed 2026-05-25**
Apply **#56** as a focused composition-root PR: per-message Azure DevOps decorator factory (named `HttpClient`), `SqlScopedResolutionDbConnectionFactory` disposal contract in XML + architecture docs. No schema changes.
**Shipped:** #56 `IAzureDevOpsPullRequestDecoratorFactory`, named `HttpClient`, disposal contract documentation.
