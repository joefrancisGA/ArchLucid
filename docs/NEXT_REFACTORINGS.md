...
| **MN-1** | Magic numbers / named bounds | **Session 1 (2026-04-19):** `PaginationDefaults.MaxListingTake`, `ManifestSummaryLimits.MaxRelationships`, commit-retry backoff constant, OpenAPI snapshot walk depth, replay diagnostics cap; list `take` clamps aligned with `MaxPageSize` / `MaxListingTake`. **Session 2 (2026-04-19):** `ProvenanceQueryLimits` (UI vs authority neighbourhood depth), `ClientErrorTelemetryIngestLimits` + test uses `MaxContextEntries + 2`, `DependencyConstraintTests` repo-root walk depth. **Next:** remaining test `for` iteration literals, NSwag-generated client, optional `IOptions` for commit backoff. |
