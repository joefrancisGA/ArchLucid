...
- [x] **Persistence split** (7 April 2026): Split **`ArchLucid.Persistence`** (was 234 .cs files) into **`ArchLucid.Persistence`** (164 files — pure data access: Dapper repos, SQL queries, models, connection factories, type handlers, migrations, serialization, blob envelope models, caching abstractions) + **`ArchLucid.Persistence.Coordination`** (70 files — service coordination: Compare, Replay, Evolution, ProductLearning, Retrieval, Backfill, Compliance, Diagnostics, Caching options/resolver). **`ArchLucid.Persistence.Runtime`** (25 files, unchanged) now references both. All 1,391 unit tests pass.
