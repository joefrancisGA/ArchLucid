...
To verify **CI parity**, run **`assert_merged_line_coverage_min.py`** on merged **`Cobertura.xml`** with **`78`**, **`--min-branch-pct 61`**, **`--min-package-line-pct 84`** (same as CI; include the workflow **`--skip-package-line-gate`** list). For the **strict-profile / V1.1** dry run, use **`95`** instead of **`78`** and **`assert_coverage_floor_ratchet.py`**.
...
**Advisory (non-blocking):** the advisory band is **inactive** today — **`--warn-below-package-line-pct 70`** sits below the **84%** package floor, so no package can pass the floor and still land in the band (see workflow).
