...
| **LLM token quotas** | **`LlmTokenQuota:Enabled`** true in production; limits still generous enough for peak tenants | `ArchiForge.Api/appsettings.Production.json`; correlate with Prometheus `archiforge_llm_*_tokens_total` |
