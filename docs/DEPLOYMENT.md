...
2. **Run database migrations** with **`ArchiForge.DatabaseMigrator`** (DbUp) against the target database **before** or **in lockstep** with rolling out the API version that requires new schema. See **`docs/runbooks/MIGRATION_ROLLBACK.md`** for failure handling.
