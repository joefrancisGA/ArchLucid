> **Scope:** Security and accessibility operations owners provisioning and routing the `accessibility@archlucid.com` alias; not WCAG engineering guidance (see root [`ACCESSIBILITY.md`](../../ACCESSIBILITY.md) and marketing **`/accessibility`**).

> **Spine doc:** [Five-document onboarding spine](../FIRST_5_DOCS.md). Read this file only if you have a specific reason beyond those five entry documents.


# Accessibility mailbox (`accessibility@archlucid.com`)

## Decision (2026-04-22)

**`accessibility@archlucid.com`** exists as a **public-facing alias** for reporting **accessibility barriers** (WCAG / usability / assistive-technology friction). Mail routes to the **same operational custodian** as **`security@archlucid.com`** so one accountable owner triages inbound mail; the subject line and message body should make **accessibility vs coordinated security disclosure** obvious.
...
| Mailbox | Role |
| ------- | ---- |
| `security@archlucid.com` | Canonical coordinated disclosure + security inquiries (see [`SECURITY.md`](../../SECURITY.md)). |
| `accessibility@archlucid.com` | Accessibility barrier reports; **same custodian / same tenant** as `security@`, separate alias for clarity and filtering. |

## Public surfaces

- Marketing WCAG self-attestation: **`/accessibility`** (source `archlucid-ui/src/app/(marketing)/accessibility/page.tsx`).
- [`archlucid-ui/public/.well-known/security.txt`](../../archlucid-ui/public/.well-known/security.txt) lists **`Contact: mailto:accessibility@archlucid.com`** alongside the security contact.

<!-- TODO(owner): confirm provider — pick the checklist below that matches the live `security@archlucid.com` tenant (Microsoft 365 vs Google Workspace vs other). Remove this comment once confirmed. -->
...
2. **Add alias / proxy address:** on the **same recipient object** that receives `security@`, add **`accessibility@archlucid.com`** as a secondary SMTP proxy (`Set-Mailbox -EmailAddresses` or Admin UI **Email aliases**).
3. **DNS:** ensure **MX** for `archlucid.com` points at the same Microsoft 365 endpoint already used for `security@` (no split routing unless intentionally designed).
...
3. **DNS:** **MX** records for `archlucid.com` must continue to point at Google’s inbound servers (same as `security@`).
