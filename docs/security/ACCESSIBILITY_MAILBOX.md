...
| Mailbox | Role |
| ------- | ---- |
| `security@archlucid.net` | Canonical coordinated disclosure + security inquiries (see [`SECURITY.md`](../library/contributor-reference/SECURITY.md)). |
...
- **Volume expectation:** low initially; still warrants the same **48-hour acknowledgment** target as security mail where feasible (not a legal SLA — see [`SECURITY.md`](../library/contributor-reference/SECURITY.md) for security-specific timelines).
