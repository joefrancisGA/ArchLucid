...
| `tenant-only-covered` | `dbo.IdempotencyRecords`, `dbo.LlmDailyTenantTokenWindowState`, `dbo.LlmMonthlyTenantBudgetState`, `dbo.SentEmails`, `dbo.TenantLifecycleTransitions`, `dbo.TenantTrialSeatOccupants`, `dbo.TenantOnboardingState`, `dbo.TenantSettings` | Tenant-id predicate variant; no workspace/project columns are present on these rows (includes HTTP `Idempotency-Key` replay cache keyed by tenant). |
