...
| `operational-table` | `dbo.BackgroundJobs`, `dbo.HostLeaderLeases`, `dbo.DataConsistencyQuarantine`, `dbo.FirstTenantFunnelEvents`, `dbo.AuthorityPipelineTenantExecutionLease` | Operational scheduling, leader election, concurrency leases, quarantine, or funnel telemetry where app/job identities and monitoring are the primary controls. |
