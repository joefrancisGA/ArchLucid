...
| `tenant-only-covered` | `dbo.IdempotencyRecords`, `dbo.LlmDailyTenantTokenWindowState`, `dbo.LlmMonthlyTenantBudgetState`, `dbo.LlmTenantWalletState`, `dbo.LlmTenantWalletLedger`, `dbo.SentEmails`, `dbo.TenantLifecycleTransitions`, `dbo.TenantTrialSeatOccupants`, `dbo.TenantOnboardingState`, `dbo.TenantSettings`, `dbo.TenantCostSettings` | Tenant-id predicate variant; no workspace/project columns are present on these rows (includes HTTP `Idempotency-Key` replay cache keyed by tenant and LLM wallet state/ledger rows filtered by tenant repository methods). |
