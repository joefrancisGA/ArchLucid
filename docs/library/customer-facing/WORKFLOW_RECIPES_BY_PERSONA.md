...
2. **Track pipeline** — UI: `/reviews/{runId}` · API: `GET /v1/architecture/run/{runId}` · CLI: `… status <runId>`.
3. **Finalize** — UI: review detail **Finalize** · API: `POST /v1/architecture/run/{runId}/commit` · CLI: `… commit <runId>` (signed review record must exist before downstream exports).
