...
**Operator UI (V1 vs V1.1):** V1 proxy/types work and the V1.1-only UI architecture backlog are summarized in [UI_ARCHITECTURE_V1_1.md](UI_ARCHITECTURE_V1_1.md). Progressive disclosure Batch 3 (run-detail IA refactor) is tracked as **TB-401** in the V1.1 backlog.
...
| **CI — merged line ≥ 95% + coverage ratchet** | **Shipped in V1 GA** (owner reconciliation **2026-06-06**): **`.github/workflows/ci.yml`** (`dotnet-coverage-merge`) enforces merge-blocking **merged line ≥ 95%** (branch **63%**, per-product-package line **63%**); **`assert_coverage_floor_ratchet.py`** + **`.coverage-floor`** are **enabled**. Keep **`docs/engineering/BUILD.md`**, **`docs/library/coverage-exclusions.md`**, and **`docs/library/COVERAGE_GAP_ANALYSIS.md`** aligned with **`ci.yml`** — do not regress docs to stale 75%/V1.1 wording. |
...
**Owner decision (2026-05-29):** Assessment improvements **#23** (SOC 2 CPA) and **#25** (third-party pen test) are **V1.1 backlog** items tracked as **TB-135** and **TB-136** in [`TECH_BACKLOG.md`](TECH_BACKLOG.md). They are **not** V1 assessment implementation prompts — see **`.cursor/rules/V1_1-assurance-backlog.mdc`**.
...
**Third-party** pen-test execution, vendor SoW award, assessor deliverables, and optional Trust Center row refresh for that engagement are **V1.1 backlog** (**TB-136**) — use [`2026-Q2-SOW.md`](../security/pen-test-summaries/2026-Q2-SOW.md) and [`2026-Q2-REDACTED-SUMMARY.md`](../security/pen-test-summaries/2026-Q2-REDACTED-SUMMARY.md) as procurement templates when the owner picks up the backlog item. Prior **V2** framing for external pen test is **superseded** by this decision where it conflicts.
...
- V1 assessments **must not** penalize the solution for absent **third-party** pen-test publication; that work is **V1.1 backlog (TB-136)**, not a recurring assessment implementation item. The **PGP** row remains **V1.1** as above. Pre-2026-05-01 docs that referenced a named third-party vendor for Q2 2026 are **superseded** by this owner decision where they conflict.
