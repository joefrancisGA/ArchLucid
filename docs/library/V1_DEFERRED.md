...
| **CI — merged line ≥ 95% + coverage ratchet** | **Interim (RC27, 2026-08):** **`.github/workflows/ci.yml`** (`dotnet-coverage-merge`) enforces **merged line ≥ 76%** (branch **60%**, per-product-package line **86%**; **`ArchLucid.Api`** package gate skipped until Integration HTTP tests collect Coverlet, plus RC interim package skips). The job is **warn-only** (`continue-on-error`). RC27 `*PackageCoverageBatchRc27Tests` clear prior package FAILs; merged floors track measured ~**75.36%** / ~**60.17%** with uplift. **`assert_coverage_floor_ratchet.py`** + **`.coverage-floor`** (**78.00** anchor → **76%** with **2%** slack) are **enabled**. **V1.1 target:** ratchet **76 → 78 → 95%** merged line. Keep **`docs/engineering/BUILD.md`**, **`docs/library/coverage-exclusions.md`**, and **`docs/COVERAGE_GAP_ANALYSIS.md`** aligned with **`ci.yml`**. |
