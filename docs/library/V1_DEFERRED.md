...
| **CI — merged line ≥ 95% + coverage ratchet** | **Interim (RC26, 2026-08):** **`.github/workflows/ci.yml`** (`dotnet-coverage-merge`) enforces **merged line ≥ 78%** (branch **61%**, per-product-package line **84%**; **`ArchLucid.Api`** package gate skipped until Integration HTTP tests collect Coverlet, plus RC interim package skips). The job is **warn-only** (`continue-on-error`) and all three floors sit **above** measured CI **#2938** Cobertura (~**74.94%** line, ~**59.67%** branch, ~**81.57%** lowest gated package), so they fail until coverage rises. **`assert_coverage_floor_ratchet.py`** + **`.coverage-floor`** (**80.00** anchor → **78%** with **2%** slack) are **enabled**. **V1.1 target:** ratchet **78 → 95%** merged line. Keep **`docs/engineering/BUILD.md`**, **`docs/library/coverage-exclusions.md`**, and **`docs/COVERAGE_GAP_ANALYSIS.md`** aligned with **`ci.yml`**. |
