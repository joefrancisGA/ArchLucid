...
| **CI — merged line ≥ 95% + coverage ratchet** | **`.github/workflows/ci.yml`** (`dotnet-coverage-merge`): **`assert_merged_line_coverage_min.py`** merged-line positional was relaxed to **0** (branch **63%** and per-package line **63%** remain merge-blocking); **`assert_coverage_floor_ratchet.py`** + **`.coverage-floor`** are not invoked. **V1.1:** restore **95%** merged line, re-enable the ratchet step, and keep docs (**`docs/engineering/BUILD.md`**, **`docs/library/coverage-exclusions.md`**, **`docs/library/COVERAGE_GAP_ANALYSIS.md`**) in sync. |
