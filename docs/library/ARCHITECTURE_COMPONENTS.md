> **Scope:** Contributor-reference — ArchLucid architecture (Components) - full detail, tables, and links in the sections below.
...
| **Rest of `ArchLucid.Persistence`** | **Data access** ports: unit of work, snapshot repos for context/graph/findings/manifests, caching decorators (`CachingRunRepository`), archival, retrieval outbox, authority pipeline **work adapters** (outbox enqueue, tenant concurrency), RLS session context. Authority orchestration lives in **`ArchLucid.Application/Runs/Orchestration/`** (**TB-302**). | `IRunRepository` (`Models.RunRecord`), `IArchLucidUnitOfWork`, `SqlContextSnapshotRepository` |
