...
| Content preference | Prefer Category-1 short answers from `contextualHelpForPathname` (what is this page / what to do next / why empty / where to configure, plus optional task steps). When short answers are not written yet, still open the drawer with a generic fallback and **Open full help page**. Do not navigate away from the current page on Help. |
...
| Panel semantics | Use the shared `HelpDrawerContent` primitive (`components/help/HelpDrawerContent.tsx`) as a **non-modal** right-side drawer (`<Dialog modal={false}>`). It supplies `role="dialog"`, focus movement into the panel, Escape / close-button dismissal, and focus return to the page-header trigger via `returnFocusRef`. Do not dim or block the page; the operator must be able to keep working the form behind the drawer. Do **not** use `HelpPopover` for page-header Help — that primitive remains for field-level `ContextualHelp` only. |
| Tooltip vs popover | `FieldHelpTooltip` for a short, **non-interactive** hint on a single control. `HelpPopover` for field-level help that contains a link, CTA, or more than one idea. Page-header Help uses the drawer, not a popover. Never put interactive content in a tooltip. |
...
**Code touchpoints:** `PageContextualHelpButton` / `PageScopedContextualHelpPanel`, `HelpDrawerContent`, `page-help-topic-map.ts`, `contextual-help-registry.ts`. Field-level `HelpPopover` remains at `components/ui/help-popover.tsx`.
...
`PageContextualHelpButton` / Category-1 drawer copy teach the **current page job**. The **Open full help page** escape hatch must not send the operator to a generic Getting started / How it works guide when the page is a secondary hub.

| Rule | Required behavior |
|------|-------------------|
| Job match | Open full help page targets `/help/{slug}` whose **primary job** matches the route (same job as the Category-1 answers), **or** the link is **omitted** when no honest job-matched guide exists. |
...
**Code touchpoints (apply in follow-on rows):** `page-help-topic-map.ts` (`pageHelpTopicForPathname`), `PageContextualHelpButton` / `PageScopedContextualHelpPanel` Open full help page href. Remaps: Digests golden **TB-2049** Done; secondary-hub sweep **TB-2050** Done (2026-08-05); popover deep-link CTAs **TB-2051** Done (2026-08-05); Vitest suite **TB-2052** Done (2026-08-06) — `learn-more-job-match.test.ts` + `learn-more-job-match-inventory.ts`.

**Good exemplar (target state):** Digests Category-1 answers discuss Schedule/recipients; Open full help page must open digests-relevant help (or omit) — not `/help/getting-started`. Category-1 what-to-do-next / where-to-configure fields that name a tab or operator route should expose optional `{ label, href }` actions (`PageContextualHelpAction`) rendered in `PageScopedContextualHelpPanel`.
...
- Page-scoped help **mount + interaction** contract: this file § *Operator page contextual help — mount + interaction contract* (**TB-1666** Done) — press-only triggers, non-modal `HelpDrawerContent` right drawer, `title`-as-help banned (sweep **TB-2147**); mount waves **TB-1667**–**TB-1670** Done
