...
**Status:** Active (V1)  
**Backlog:** **TB-975** (this contract) · **TB-976** (durable per-reservation leases + orphan reclaim — open) · **TB-977** (SQL-owned UTC period + admission fairness — open)  
...
Name what **INV-004** reserve/settle guarantees under live multi-replica concurrency — and which lifecycle failures remain until **TB-976** / **TB-977** ship.

**One line:** **Hard cap is cap-correct under concurrent writers**; **settle is best-effort on the happy path**; **orphan reserved pressure, app-clock period keys, and assumed-max races are documented residuals**.
...
| “No race soft-DoS on the budget path.” | N concurrent calls each reserving **assumed-max** can fill the reserved pool and deny later work **without** exceeding the hard cap (**TB-977**). |
| “Month boundary is always SQL-authoritative today.” | Period keys are computed from injected `TimeProvider` at the tracker/repository boundary; skew across replicas can split one UTC month across two rows until **TB-977**. |
...
| Fail-closed admission when `Total*Pressure + assumed > effectiveMax` | Fairness when many workers reserve assumed-max concurrently (soft denial without cap breach) | **TB-977** |
| Single monthly SQL row per `(TenantId, UtcYear, UtcMonth)` when period key is consistent | Two rows for one UTC month when app clocks disagree at boundary | **TB-977** |
| Run-scoped batch admit before agent batch (**TB-939**) | Run-scoped store does not replace pooled monthly orphan semantics | **TB-976** / **TB-977** |
...
## 7. Period key and clock (today vs **TB-977**)

| Today | Residual |
|-------|----------|
| Monthly key `yyyy-MM` from `TimeProvider` in `LlmMonthlyTenantDollarBudgetTracker.GetUtcYearMonth()` | Replica with skewed clock can mint a different `periodKey` near UTC month rollover |
| Daily key `yyyy-MM-dd` from same clock in `LlmDailyTenantBudgetTracker` | Same boundary risk on UTC day |
| SQL `SYSUTCDATETIME()` on `LastUpdatedUtc` only | Period **membership** not decided inside reserve/settle SQL yet |

**TB-977** must: decide period inside reserve/settle SQL (or single trusted clock at repository boundary), define cross-period settle rules, and add admission fairness under assumed-max races.
...
| N parallel calls each reserve assumed-max | No | Reserved pool saturated; deny without spend | **TB-977** fairness |
...
| Month-boundary skew (two period keys) | Possible **split-budget** illusion (two rows) | Cap may appear reset or fragmented | **TB-977** |
...
3. **Month-boundary skew:** Clocks straddling UTC month end cannot admit spend that exceeds **one** intended monthly hard cap **once TB-977 ships**; until then, document split-row risk explicitly in buyer materials.
4. **Optimistic retry exhaustion:** After `MaxOptimisticRetries` (12), reserve throws — ops alert target for **TB-977** metrics.
...
4. Ask how month-boundary time is chosen — answer must cite app `TimeProvider` today and **TB-977** residual.
...
| Soft scarcity possible without cap breach | No race-induced denial pressure |
...
| **TB-977** | SQL-owned UTC period membership; admission fairness; month-boundary + race integration tests |
