...
**TB-114 â€“ TB-120** were added 2026-05-30 from the owner-ratified UI design standard (decision date 2026-05-27). They close the gap between the current Tailwind/shadcn default aesthetics and the **IBM Carbonâ€“inspired enterprise visual language** mandated for V1 GA. Canonical standard: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md). **TB-114** (design tokens) and **TB-115** (surface/card pass) are foundational and should be done first; **TB-116** (status tags) and **TB-117** (data tables) unlock governance credibility; **TB-118** (spacing) and **TB-119** (typography) are polish; **TB-120** (agent rule) ensures future AI-written code stays conformant.

**Recently shipped (IDs kept for grep, ADRs, and code comments â€” spec text removed below):** **TB-001** (informational async audit best-effort + counter), **TB-002** (`archlucid_startup_config_warnings_total`), **TB-003** (named-query p95 allowlist + `archlucid_query_p95_ms`), **TB-006** (`ComparisonRecords` run id GUID + FK migration), **TB-022** (long-safe run token aggregation), **TB-024** (reasoning-token test coverage), **TB-026** (`LlmCostEstimationOptions` negative-rate validation + runtime guard).

**TB-022 â€“ TB-026** were added 2026-05-24 from an audit-grade correctness review of `LlmCostEstimator` (see `ArchLucid.AgentRuntime/LlmCostEstimator.cs` and `ArchLucid.Application/Agents/AgentExecutionTraceRunLlmCostAggregator.cs`). They form a single thematic cluster: TB-022 + TB-026 are correctness fixes; TB-024 is test coverage; TB-023 + TB-025 are documentation/annotation.

**TB-027 â€“ TB-032** were added 2026-05-26 from a full dependency-graph audit across all 59 `.csproj` files (239 edges). They address violations against the intended `Contracts â†’ Core â†’ Application â†’ Host/Adapters` layering and close gaps in `ArchLucid.Architecture.Tests` / `DependencyConstraintTests`.

**TB-033 â€“ TB-038** were added 2026-05-26 from a replay / provenance completeness audit (`ArchLucid.Provenance` decision lineage vs `AgentRuntime` `AgentExecutionTrace` forensics). They close gaps where a single agent task cannot be fully reconstructed from durable storage. Retrieval grounding enrichment is also tracked as **RAG-V1-006** in [`RAG_QUALITY_TECHNICAL_BACKLOG.md`](RAG_QUALITY_TECHNICAL_BACKLOG.md).

**TB-039 â€“ TB-044** were added 2026-05-26 from an AgentRuntime determinism and idempotency audit (retry, fan-out, partial-failure, and authority-pipeline replay paths). They close gaps where LLM calls, token metering, or graph snapshots can be applied more than once without supersession. **TB-039** and **TB-043** are FinOps / economics fixes; **TB-041** + **TB-042** are authority-pipeline replay guards; **TB-040** and **TB-044** are metering honesty and trace deduplication. Cross-ref **TB-012** (**INV-009** idempotency) and **TB-035** (remediation attempt forensics â€” complementary, not duplicate).

**TB-045 â€“ TB-049** were added 2026-05-26 from a retrieval correctness & drift audit (`ArchLucid.Retrieval` â€” embedding model drift, index staleness, chunking invalidation, tenancy bleed, IR eval harness). Authoritative sub-IDs **RAG-V1-007** through **RAG-V1-011** in [`RAG_QUALITY_TECHNICAL_BACKLOG.md`](RAG_QUALITY_TECHNICAL_BACKLOG.md). **TB-048** (tenancy) is security-critical; **TB-049** (IR eval) blocks silent retrieval regressions.

**TB-050 â€“ TB-056** were added 2026-05-27 from a Decisioning explainability and uncertainty audit (`ArchLucid.Decisioning` â€” authority `RuleBasedDecisionEngine` / `RuleAuditTracePayload` vs coordinator `DecisionEngineV2` / `DecisionNode`). They close gaps where operators cannot trace manifest decisions to inputs, rules/prompts, and honest confidence. Cross-ref **TB-036** (provenance â†” agent trace correlation), **TB-037** (provenance snapshot materialization). Canvas audit: `canvases/decisioning-explainability-audit.canvas.tsx` (IDE-only).

**TB-057 â€“ TB-063** were added 2026-05-27 from a commercial stickiness review. They do **not** create a parallel GRC product. They consolidate existing signed review package primitives â€” findings, monitored risks, manifest decisions, governance approvals, digests, ROI, compare/drift, audit, and integration correlations â€” into a recurring operating workflow. **TB-063** is explicitly **V1.1** because first-party ITSM productization is release-windowed there per [`V1_SCOPE.md`](V1_SCOPE.md) Â§2.13 and [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6.

**TB-064 â€“ TB-070** were added 2026-05-27 from a DDL hygiene and migration-safety audit (`ArchLucid.Persistence` DbUp + `Scripts/ArchLucid.sql` + `Persistence.MigrateVerify`). They close gaps against the repo **one DDL file per DB** rule, journal-only verification, IaC/generated-schema drift, and rolling-deploy risk from non-additive migrations. **TB-065** and **TB-068** are deploy-safety critical; **TB-064** closes the system-catalog DDL gap; **TB-066**â€“**TB-067** are CI/docs parity; **TB-069**â€“**TB-070** are maintainability hygiene. Canvas audit: `canvases/ddl-hygiene-audit.canvas.tsx` (IDE-only).

**TB-071 â€“ TB-078** were added 2026-05-27 from a multi-tenancy and blast-radius audit (API ingress â†’ Application â†’ Persistence â†’ Retrieval / knowledge graph â†’ operator UI). They close gaps where `tenantId` is derived but not enforced at the query layer, and where cross-tenant data could leak via ID-only snapshot reads, unbound auth schemes, client-controlled scope headers, or retrieval index writes. **TB-071** and **TB-072** are security-critical (P0); **TB-073**â€“**TB-075** are high (P1); **TB-076**â€“**TB-078** are defense-in-depth (P2). Cross-ref **TB-048** / **RAG-V1-010** (retrieval query filter â€” partial; production Azure client still missing), **TB-010** (**INV-001** tenant boundary), **TB-005** (owner pen-test). Canvas audit: `canvases/multitenant-blast-radius-audit.canvas.tsx` (IDE-only).

**TB-079 â€“ TB-084** were added 2026-05-27 from a secrets, identity, and tool-sandboxing audit (`Integrations.AzureDevOps`, `Integrations.AzureExtractor`, agent tool surfaces, prompt-injection paths). No WIQL/LLMâ†’ADO API injection path exists; the integration is event-driven with config-fixed targets. Identified gaps: unescaped markdown from compare data echoed into ADO PR bodies (**TB-079**, Lowâ€“Med); Azure OpenAI still using symmetric `ApiKey` instead of Entra/MI (**TB-080**, Info); Service Bus raw connection string permitted in production with no safety rule (**TB-081**, Info); `AgentTask.AllowedTools` advisory-only with no runtime enforcer at handler dispatch (**TB-082**, Med); `ArchLucidApiKey` header secret has no production Key Vault reference requirement (**TB-083**, Info); `SubscriptionId` not validated as GUID before ARM URL construction (**TB-084**, Low). Cross-ref **TB-005** (pen-test), **TB-072** (scope-to-identity binding).
...
**TB-103 â€“ TB-105** were added 2026-05-27 from a cross-layer domain-term audit (executive dashboard, orphan candidates, governance). They close gaps where business logic defined once in the backend has been reconstituted independently in the UI layer, causing KPI values to diverge silently from server-computed truth. **TB-103** is the highest-priority item: orphan-candidate count and savings are computed by two separate pipelines (different inputs, different algorithms) with no shared API contract. **TB-104** closes the 14-day expiring-waiver window living only in the client. **TB-105** pushes business-impact category bucketing to the server so `BusinessImpactSummaryWidget` becomes a pure display component. Cross-ref **TB-062** (executive dashboard live KPI replacement â€” these items are scoped sub-tasks of that broader effort).

**TB-106 â€“ TB-113** were added 2026-05-27 from a `RunDetailPageView` operator fidelity audit (does the run detail page surface everything needed to approve, reject, or remediate a run?). Root cause is a split API contract: the operator loader calls `GET /v1/authority/runs/{runId}` but the UI reads `agentExecutionLlmCostEstimate`, `trustEvidenceCard`, and `results[]` that exist only on the architecture endpoint â€” those fields are null on every live run. Additional gaps: retrieval hits and tool calls have no dedicated UI surface anywhere; `findingCoverageSummary.hasCommitBlockingFailures` and `dispositionCoverage` are computed in `GetRunDetailAsync` but dropped before render; `hasGovernanceWarnings` and `lastFailureReason` from `RunRecord` are never shown. **TB-106**â€“**TB-108** are correctness/operator-visibility P0s; **TB-109**â€“**TB-111** are P1 operator-visibility additions; **TB-112** is P2 workflow; **TB-113** is P2 schema hygiene. Canvas audit: `canvases/run-detail-operator-fidelity.canvas.tsx` (IDE-only).

**TB-114 â€“ TB-118** were added 2026-05-29 from a Template and Accelerator Richness review. The conclusion was **not** to add template volume for its own sake. The V1 opportunity is to make existing starter proof packs easier to choose, validate, trust, and dry-run. **TB-114** is the highest leverage because it maps buyer jobs to existing accelerators; **TB-115** prevents stale/unsafe pack metadata; **TB-116** and **TB-117** add deterministic validation and proof dry-run coverage; **TB-118** creates one golden walkthrough that sales/operators can use without multiplying templates.

**TB-119 â€“ TB-134** were added 2026-05-29 from a Policy/Governance, Auditability, and Commercial Packaging review. The theme is proof discipline: make governance packs, audit trails, and sales-led packaging harder to misread, drift, or overclaim. **TB-119 â€“ TB-123** cover policy/governance alignment; **TB-124 â€“ TB-128** cover auditability; **TB-129 â€“ TB-134** cover commercial packaging readiness. Owner-gated items such as live commerce and named customer references remain deferred outside this cluster.

**TB-135 â€“ TB-142** were added 2026-05-29 per owner decision. **TB-135 â€“ TB-136** (SOC 2 CPA and third-party pen test) remain V1.1 assurance backlog organizational / vendor programs per workspace rule. **TB-137 â€“ TB-140** capture follow-on real-LLM engineering after assessment improvement **#1** local evidence shipped. **TB-141 â€“ TB-142** were later promoted on 2026-06-01 to near-term GTM backlog priorities for real pilot proof packets and market-facing demo assets. See **`.cursor/rules/V1_1-assurance-backlog.mdc`**, `.cursor/rules/Assessment-Scope-V1_1.mdc`, and [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6c.

**TB-143 â€“ TB-148** were added 2026-05-30 from owner-ratified product documentation presentation guidance (decision date 2026-05-27). Customer-facing help must not dump buyers or operators into raw GitHub repository browsing. **TB-143** (in-app markdown renderer + `/help/{topic}` routes) and **TB-144** (documentation registry) are foundational; **TB-145** migrates existing GitHub blob links; **TB-146** bans redirect stubs; **TB-147** adds CI drift guard; **TB-148** adds role-gated optional source links. Canonical standard: [`PRODUCT_DOCUMENTATION_PRESENTATION.md`](PRODUCT_DOCUMENTATION_PRESENTATION.md).

**TB-149 â€“ TB-155** were added 2026-05-31 from a cross-layer **data consistency** audit (executive KPIs, governance decisions-needed summary, waiver/disposition state, recurrence trigger idempotency). They extend **TB-103â€“105** and partially close **TB-104** (canonical 14-day waiver window). **TB-149** unifies two non-equivalent server implementations of the expiring-waiver window. **TB-150** fixes `TotalDecisionItems` double-counting overlapping finding categories. **TB-151** and **TB-152** correct inverted or aliased fields on `ExecutiveSummaryResult`. **TB-153** prevents duplicate recurring review runs on ACA restart. **TB-154** enforces waiver â†” disposition invariants. **TB-155** stops cached ROI waiver counts from diverging from live decisions-needed. Cross-ref **TB-062**, **TB-012** (**INV-009**), **TB-089** (digest retry â€” different surface).

**TB-156 â€“ TB-157** were added 2026-05-31 from local-dev triage: operators running `start-local-api-and-ui.ps1` (or UI-only) saw repeated Sonner warnings **â€œReview assistant unavailable / AI assistant service is not reachableâ€** while the root cause was **ArchLucid.Api not running** or **UI proxy â†’ API misconfiguration** (502), not Azure OpenAI / Ask. **Both are P0 â€” pick up in the next available engineering thread** before other backlog polish. **TB-156** fail-closes the startup script on a full browser â†’ UI â†’ `/api/proxy` â†’ API chain. **TB-157** reframes connectivity toasts so proxy/API outages say **API unreachable**, reserving assistant wording for Ask/SSE-only failures. Cross-ref [`docs/runbooks/TROUBLESHOOTING.md`](../runbooks/TROUBLESHOOTING.md), [`docs/library/customer-facing/OPERATOR_QUICKSTART.md`](customer-facing/OPERATOR_QUICKSTART.md), `scripts/env-readiness.ps1`, `scripts/demo-start-local.ps1`.

**TB-158 â€“ TB-167** were added 2026-06-01 from `docs/assessments/LATEST_GPT55.md` human-input score-limiter triage and rescore. They avoid duplicating existing engineering tasks by extending **TB-131 â€“ TB-134**, promoting existing **TB-141 â€“ TB-142** to near-term GTM priority, and adding only missing owner-reviewable GTM/procurement/support/release-claim/sponsor-evidence work. Formal SOC 2 CPA and third-party pen-test programs remain parked in **TB-135 â€“ TB-136** per the V1.1 assurance backlog rule; do not reclassify them from normal assessment passes.

**TB-168** was added 2026-06-01 from the GPT-5.5 assessment correctness follow-up. It does not duplicate **TB-103 â€“ TB-105** or **TB-149 â€“ TB-155**; those fix known KPI/waiver/recurrence defects. **TB-168** adds a regression guard so future changes cannot silently reintroduce UI KPI heuristics, stale cache fallbacks, or duplicate business semantics.

**TB-169** was added 2026-06-01 from the GPT-5.5 adoption-friction follow-up. It does not duplicate **TB-156 â€“ TB-157** (API/proxy diagnostics) or **TB-143 â€“ TB-148** (in-app docs). It targets first-run branching and progressive disclosure: Pilot-first onboarding should keep Operate surfaces out of the primary path until a committed review exists.

**TB-085 â€“ TB-090** were added 2026-05-27 from a Backfill.Cli and Jobs.Cli operational review (idempotency on rerun, bounded memory, checkpointing, poison-message handling, observability). **TB-089** is operator-visible (duplicate digest emails on ACA retry); **TB-087** closes a concurrent-rerun duplicate-`FindingRecords` window; **TB-088** prevents whole-job failure on one bad tenant/schedule; **TB-085** + **TB-086** harden large-catalog backfill runs; **TB-090** enables CI/pipeline assertions. Neither CLI writes cost rows; provenance child inserts are count-guarded (**TB-087** adds DB-level defense). Cross-ref **TB-012** (**INV-009** idempotency), **TB-067** (migration/backfill docs), **TB-061** (digest recurrence), [`SqlRelationalBackfill.md`](SqlRelationalBackfill.md), [`CONTAINER_APPS_JOBS.md`](../runbooks/CONTAINER_APPS_JOBS.md).
...
| TB-010 | Architecture invariant enforcement â€” Wave A (INV-001, INV-005, INV-006) | Done (Improvement **#21**, 2026-05-25) â€” INV-001 Roslyn analyzer; INV-005 catalog/fail-fast parity; INV-006 composition-root scan | S |
| TB-011 | Architecture invariant enforcement â€” Wave B (INV-002, INV-004, INV-012, INV-013) | Partial (Batch H, 2026-05-26) â€” **INV-002** persisted mode + trust card + operator UI badge; INV-004/012/013 remain | L |
| TB-033 | Agent execution trace â€” persist LLM sampling params + reasoning token count | **Done (2026-05-31)** â€” `AgentExecutionTraceRecorder` persists sampling params + `ReasoningTokenCount`; `AgentExecutionTraceRecorderSamplingParamsTests` | XS |
| TB-071 | Azure Search production client â€” wire tenant OData filter on every search/delete | **Done (2026-05-31)** â€” `AzureSearchSdkClient` + scoped delete; `AzureSearchTenantScopeFilterBuilderTests` | Sâ€“M |
| TB-072 | Scope-to-identity binding at API ingress (ApiKey, DevBypass, header/claim reconciliation) | **Done (2026-05-31)** â€” `ScopeIdentityBindingMiddleware` + ApiKey scope claims; `ScopeIdentityBindingIntegrationTests` | M |
| TB-073 | Scoped snapshot repository reads (findings / graph / context + relational child loads) | **Done (2026-05-31)** â€” scoped `GetByIdAsync(ScopeContext)` + `SqlFindingsSnapshotRepositoryScopeIsolationSqlIntegrationTests` | M |
| TB-074 | Retrieval indexing write-path tenant validation | **Done (2026-05-31)** â€” `RetrievalIndexingScopeValidator` on index writes; scoped in-memory delete; tests | S |
| TB-075 | Operator UI server-side scope (proxy strips client headers; SSR from session) | **Done (2026-05-31)** â€” `resolveProxyUpstreamScopeHeaders` + production-like proxy posture; value-report tenant from `/api/auth/me` | Sâ€“M |
| TB-082 | Agent `AllowedTools` â€” runtime enforcement at handler dispatch | **Done (2026-05-31)** â€” `AgentTaskAllowedToolsDispatchGuard` at dispatch; `AgentTaskAllowedToolsDispatchGuardTests` | S |
| TB-079 | ADO PR markdown â€” sanitize `SummaryHighlights` + deep-link fields before writing PR comment body | **Done (2026-05-31)** â€” `AdoPullRequestMarkdownEscaper` + safe links in compare + run-summary Markdown | XS |
| TB-083 | Service Bus â€” production safety rule: require namespace FQDN, disallow raw connection string | **Done (2026-05-31)** â€” `CollectIntegrationEventsServiceBusConnectionStringKeyVaultReference`; `ProductionSecretSourceRulesTests` | XS |
| TB-081 | `ArchLucidApiKey` â€” production safety rule: require Key Vault reference | **Done (2026-05-31)** â€” `CollectAzureDevOpsArchLucidApiKeyKeyVaultReference`; `ProductionSecretSourceRulesTests` | XS |
| TB-080 | Azure OpenAI â€” migrate from `ApiKey` config key to `DefaultAzureCredential` (Entra auth) | **Done (2026-05-31)** â€” `AuthenticationMode=ManagedIdentity` for completion, embeddings, judge; `AzureOpenAiConfigurationProbe`; KV sample + startup lint | S |
| TB-084 | AzureExtractor â€” validate `SubscriptionId` as GUID before ARM URL construction | **Done (2026-05-31)** â€” `HostedAzureExtractorGuidValidator` on client + ARM reader; unit tests | XS |
| TB-091 | Key Vault private endpoint + private DNS zone (`privatelink.vaultcore.azure.net`) | **Done (2026-06-01)** â€” `terraform-private` KV DNS zone + PE when `key_vault_id` set; `key_vault_private_endpoint_id` output; `terraform validate` | XS-S |
| TB-092 | Key Vault Secrets User RBAC for API + Worker managed identities | **Done (2026-06-01)** â€” `terraform-keyvault/workload_rbac.tf` + principal ID vars; `terraform-private/keyvault_rbac.tf`; `apply-saas.ps1` TB-092 second pass; `terraform.tfvars.example` | XS |
| TB-093 | Compose Azure OpenAI existing-resource consumption into hosted Terraform stack | **Done (2026-06-01)** â€” `openai_compose_mode=existing` (eastus); hosted-prod + container-apps env/RBAC; `terraform-openai` consumed contract; pilot_essential | M |
| TB-094 | Create `terraform-redis` root --- Azure Cache for Redis hot-path cache | **Done (2026-06-01)** â€” `infra/terraform-redis` + container-apps `hot_path_cache_redis_connection_string`; PE/DNS optional; `terraform validate` | S |
...
| TB-090 | Backfill.Cli â€” `--output-json` report + per-stage timing | **Done (2026-05-31)** â€” `--output-json [path]`, `stages[]` timings on report, `BackfillCliJsonReportSerializerTests` | XS |
| TB-069 | Simplify `GreenfieldBaselineMigrationRunner` sparse-stamp path | Maintainability â€” complex drift-repair runner with no post-stamp schema verification | M |
| TB-070 | `PersistenceContractSupplement.sql` stale refs + test catalog parity | Test hygiene â€” supplement references retired `ArchiForge.sql`; can drift from latest migrations | XS |
| TB-156 | `start-local-api-and-ui.ps1` â€” strict preflight + `/api/proxy/health/live` E2E gate; no browser on failure | **P0** â€” local dev / operator diagnostics â€” script declares success when UI `/` loads but proxy returns 502; contributors misdiagnose as LLM outage | S |
| TB-157 | API connectivity toasts â€” distinguish ArchLucid API unreachable vs Ask/assistant stream failures | **P0** â€” operator diagnostics â€” `api-error-toast-policy.ts` maps all proxy 502/fetch failures to â€œAI assistant not reachableâ€ | XS |
| TB-106 | RunDetailPageView â€” enrich authority `RunDetailDto` with cost estimate, trust evidence card, and `results[]` | **Done (2026-05-31)** â€” `AuthorityRunDetailOperatorEnricher` on `GetRunDetail`; explanation-trace fallback label when `results[]` empty | M |
| TB-107 | RunDetailPageView â€” surface `lastFailureReason` + `hasGovernanceWarnings` from `RunRecord` | **Done (2026-05-31)** â€” `RunDetailGovernanceAlerts` + metadata `retryCount` when &gt; 0 | S |
| TB-108 | RunDetailPageView â€” render `findingCoverageSummary.dispositionCoverage` + `hasCommitBlockingFailures` | **Done (2026-05-31)** â€” `FindingCoverageDispositionPanel` + `commitBlockedReason` on `CommitRunButton` | S |
| TB-103 | Orphan candidate count + savings â€” expose backend-computed values via API; remove heuristic parser from UI | Customer-visible correctness â€” `OrphanedResourceClassifier` and `run-potential-savings-parser.ts` use different inputs; KPI can silently diverge | M |
| TB-104 | 14-day expiring waiver KPI â€” server-compute the window; remove client-side date rule | Customer-visible correctness â€” `countExpiringWaivers` filter in `ExecutiveRoiDashboardLiveKpiCards.tsx` uses a frontend-defined 14-day cutoff; not returned by any backend metric | S |
| TB-105 | Business-impact category buckets â€” add pre-bucketed counts to `ExecutiveRoiSummaryResponse`; remove substring matcher | Customer-visible correctness â€” `BusinessImpactSummaryWidget.sumIssueCounts` uses `category` substring matching; brittle and not validated against backend classification | S |
| TB-149 | Canonical 14-day expiring-waiver window â€” single server implementation; delete `CountExpiringWaivers` duplicate | Customer-visible correctness â€” `ExecutiveRoiSummaryService` counts expired waivers; `BuildSummaryAsync` uses `[now, now+14d]`; dashboard prefers stale ROI field | S |
| TB-150 | Decisions-needed `TotalDecisionItems` â€” union cardinality across buckets, not sum | Customer-visible correctness â€” same finding can increment stale + needs-evidence + deferred + waiver buckets; KPI overcounts | S |
| TB-151 | `ExecutiveSummaryResult.TotalRiskReductionScore` â€” rename or map to pending-decision count | Customer-visible correctness â€” field maps to `TotalDecisionItems` (high = more work); semantic inversion for exports/consumers | XS |
| TB-152 | `ExecutiveSummaryResult.CostWasteUsd` â€” stop aliasing `TotalEstimatedUsdSavings` | Semantic debt â€” waste vs recoverable savings conflated; silent break when ROI distinguishes them | XS |
| TB-153 | Recurring architecture review trigger â€” idempotency before `ExecuteRunAsync` | Reliability / correctness â€” ACA restart after `CreateRunAsync` but before `UpdateAsync` can duplicate runs per schedule period | M |
| TB-154 | Waiver â†” disposition state machine â€” bidirectional invariants | Governance correctness â€” active waiver on remediated finding; expiry without disposition event; stale risk double-count with waiver | M |
| TB-155 | ROI cache TTL vs live decisions-needed â€” canonical expiring-waiver source | Customer-visible correctness â€” `CachingExecutiveRoiSummaryService` can serve stale `ExpiringWaiversCount14Days` while decisions-needed is fresh | S |
| TB-109 | RunDetailPageView â€” add retrieval-hit / RAG grounding panel | Operator visibility (P1) â€” no UI surface anywhere shows which chunks were retrieved, their scores, or whether any retrieval step was degraded; critical when `faithfulnessWarning` is true | M |
| TB-110 | RunDetailPageView â€” add tool-call / function-invocation log panel | Operator visibility (P1) â€” no dedicated tool-call panel; agent forensics shows trace rows but not function-call lists; full prompt/response in blob storage is not rendered | M |
| TB-111 | RunDetailPageView â€” inline provenance summary card (collapse from sibling route) | Operator visibility (P1) â€” provenance requires full-page navigation to a sibling route using a different API; operator loses run context while reviewing | S |
| TB-112 | RunDetailPageView â€” add run-level approve / reject / request-remediation actions | Operator workflow (P2) â€” `CommitRunButton` (finalize manifest) is the only run-level action; all finding disposition lives on per-finding sub-routes with no run-level governance action | M |
| TB-113 | Fix OpenAPI schema drift on `RunDetailDto` â€” expose `degradedFindingCoverage` + `findingCoverageSummary` in generated TypeScript types | Schema hygiene (P2) â€” C# `RunDetailDto` has both fields; generated `api-types.generated.ts` may omit them; silent type-level omission makes it impossible to add UI without bypassing type safety | XS |
| TB-114 | Accelerator chooser â€” map buyer job â†’ starter proof pack â†’ expected proof output | Template richness / time-to-value â€” existing accelerators are useful but not easy enough to choose in the first 10 minutes | S |
| TB-115 | Starter proof pack metadata contract | Template trust â€” packs need owner, last-reviewed, buyer persona, required inputs, expected outputs, V1/V1.1/deferred scope, and "do not use when" fields | S |
| TB-116 | Starter proof pack static validation gate | Correctness / template safety â€” every pack should parse, include required files, avoid placeholders/secrets, and have valid scope labels before release | S |
| TB-117 | Template-to-proof dry-run harness | Time-to-value / regression safety â€” dry-run each starter pack through minimal request + policy/context path without live cloud dependencies | M |
| TB-118 | Golden accelerator walkthrough (one pack only) | Marketability / enablement â€” one canonical walkthrough with expected artifacts/screenshots improves trust without adding template sprawl | S |
| TB-119 | Policy pack metadata and buyer-safe caveat validation | Governance alignment â€” every policy pack needs scope, owner, last reviewed, sample finding, and explicit "not certification" language | S |
| TB-120 | Policy pack dry-run index | Time-to-value / governance usability â€” list packs by buyer job, required inputs, expected findings, and V1/V1.1/deferred scope | S |
| TB-121 | Route/tier/policy/nav parity release gate hardening | Governance drift prevention â€” changes to routes, tiers, policy surfaces, or nav must refresh parity proof before release | S |
| TB-122 | Governance outcome summary in sponsor proof | Trust / buyer clarity â€” sponsor proof should summarize applied policies, approvals, waivers, unresolved governance items, and buyer-safe status | S |
| TB-123 | Policy-pack freshness report in proof/procurement artifacts | Governance freshness â€” proof packets should show policy-pack last-reviewed posture and stale-pack warnings | S |
| TB-124 | Audit coverage drift gate for critical workflows | Auditability â€” mutating routes, governance actions, and proof actions must map to expected audit event types or explicit allowlist rationale | M |
| TB-125 | Buyer-safe audit evidence summary in proof bundles | Auditability / trust â€” proof bundles need audit categories, correlation IDs, run/manifest traceability, and omitted sensitive-field notes | S |
| TB-126 | Audit event catalog metadata | Supportability â€” event types need owner, purpose, actor, scope fields, retention sensitivity, and buyer-safe/export posture | M |
| TB-127 | Audit tests for sponsor/procurement proof actions | Correctness â€” critical proof generation and commercial handoff actions should emit audit rows or document informational-only rationale | M |
| TB-128 | Support/audit triage one-pager | Supportability â€” given runId/correlationId, operators need one artifact-open order for audit and support investigation | XS-S |
| TB-129 | Quote-to-proof readiness checklist | Commercial packaging â€” quote, tier, proof status, ROI basis, deferred asks, and next commercial action should be one checklist | S |
| TB-130 | Quote aging export and follow-up SLA report | Decision velocity â€” open quote requests need aging buckets, follow-up status, warn/breach counts, and AdminAuthority-only export | M |
| TB-131 | Commercial closeout artifact hardening | Monetization â€” generated PASS/HOLD/DEFERRED_SCOPE, next ask, owner, and caveats should agree with proof JSON | S |
| TB-132 | Tier fit validation matrix | Packaging correctness â€” tiers should map to buyer jobs, included evidence outputs, excluded capabilities, and deferred capabilities | S |
| TB-133 | AI & Cloud Architecture Readiness Review offer pack aligned to pricing | GTM execution â€” one-page offer, pilot scope, pricing bands, week 1 / week 2 outcomes, proof outputs, exclusions, and buyer prerequisites | S |
| TB-134 | Commercial copy overclaim guard + public claim-boundary guide | Trust / packaging safety â€” CI should catch copy implying live commerce, Marketplace transactability, SOC 2 CPA, public references, unsupported ROI, or unlabeled simulator/local-owner-dev/prototype claims | S |
| TB-158 | Pilot success thresholds and acceptance criteria | Customer-success / ROI proof â€” model-assisted owner-reviewable PASS/HOLD thresholds for proof quality, ROI confidence, false positives, and time-to-first-value | S |
| TB-141 | Near-term GTM backlog: real pilot proof packet cohort | GTM proof â€” owner-selected scenarios, approved data boundaries, and buyer-safe proof packets for Azure cost / orphan / governance review and adjacent starter cohorts | Owner/program |
| TB-142 | Near-term GTM backlog: market-facing demo asset production | GTM proof â€” approved screenshots/video/copy and evidence-labeling rules for channel-specific demo assets | Owner/GTM |
| TB-159 | Buyer security/procurement packet | Procurement readiness â€” approved security questionnaire answers, trust-center caveats, support/SLA posture, data-retention answers, and not-yet-certified language | S-M |
| TB-160 | Legal/procurement terms packet | Procurement readiness â€” MSA/DPA posture, support/SLA language, data-retention commitments, liability boundaries, and redline approval path | M |
| TB-161 | Design partner / pilot recruiting pipeline | GTM execution â€” target accounts, qualification criteria, founder-led outreach, pilot acceptance terms, and proof-capture permission path | S-M |
| TB-162 | Support and pilot operating model | Operations â€” support hours, escalation path, response targets, incident communications, owner availability, and white-glove vs self-serve pilot posture | S |
| TB-163 | Transactable procurement path | Commercial conversion â€” invoice/services SOW/private offer/Stripe/Marketplace decision tree, payment terms, legal/tax readiness, and claim boundaries | M |
| TB-164 | V1.1 backlog: first named public reference customer | GTM proof â€” customer permission, logo/case-study approval, reference-call terms, and claim update process | Owner/GTM |
| TB-165 | Assessment score consistency guard | Documentation quality â€” keep weighted tables, per-quality sections, and headline score synchronized after rescores | XS-S |
| TB-166 | Release claim gate for full real-mode AI evidence | Release safety â€” RC packaging must attach passing Topology/Cost/Compliance/Critic evidence or automatically downgrade release copy and proof artifacts to simulator-only / partial-real-mode claims | S |
| TB-167 | Sponsor AI readiness posture artifact | Sponsor proof â€” one simple release/proof artifact showing execution mode, quality gate, retrieval grounding, and budget/cost posture for every sponsor packet | S-M |
| TB-168 | Executive KPI semantic contract and UI heuristic regression guard | Customer-visible correctness â€” one machine-checkable contract for ROI/governance KPI fields plus a UI guard that blocks reintroduced substring/date/local fallback heuristics | S |
| TB-169 | Pilot-first onboarding and Operate-surface progressive disclosure | Adoption friction â€” first-run UI should show one guided Pilot path and hide recurring/Operate surfaces until the first committed review/proof artifact exists | M |
| TB-143 | In-app markdown documentation renderer + `/help/{topic}` routes | Customer-visible UX â€” product help must render inside ArchLucid shell, not GitHub blob pages | M |
| TB-144 | Customer-facing documentation registry | Correctness / maintainability â€” stable map from product topics to in-app routes and repo source paths | S |
| TB-145 | Migrate operator/product help links from GitHub blob to in-app routes | Customer-visible UX â€” HelpPanel, contextual help, doc index, hard-coded marketing/operator GitHub links | M |
| TB-146 | Redirect-stub ban + canonical target resolution in registry | Trust â€” never link compatibility stubs such as `PILOT_GUIDE.md`; resolve final markdown internally | XS |
| TB-147 | CI drift guard â€” no customer-facing GitHub blob links in product UI | Regression prevention â€” lint/CI fails on new `github.com/.../blob/` links in operator/marketing UI paths | S |
| TB-148 | Role-gated optional â€œView source on GitHubâ€ footer | Developer ergonomics â€” source link only in admin/developer/diagnostics mode, not buyer/operator primary UI | XS |
| TB-019 | Signup marketing attribution + server-side conversion (UTM survive funnel â†’ provision success â†’ telemetry/SQL) | Paid + organic honesty â€” **`SEO_AND_PAID_ACQUISITION.md`** data flow requires measurable **`TenantProvisioningService`** outcomes; avoids raw-UTM metric cardinality explosions | M |
| TB-020 | Public marketing SEO â€” `SoftwareApplication` + trust `FAQPage` JSON-LD; consent-gated Clarity (`NEXT_PUBLIC_ARCHLUCID_CLARITY_PROJECT_ID`); CSP (`clarity.ms`, `c.bing.com`); privacy Â§2.4 â€” DPIA / server kill-switch mirror optional | SERP + honest analytics posture | Sâ€“M |

---

## TB-009 â€” Architecture invariant catalog + ADR 0035

**Status:** **Documentation landed** (2026-05-09) â€” [`docs/library/ARCHITECTURE_INVARIANTS.md`](ARCHITECTURE_INVARIANTS.md) (IDs `INV-001`â€“`INV-015`), authoring skeleton [`docs/architecture/adrs/adr-template-full.md`](../architecture/adrs/adr-template-full.md), governance ADR **`docs/architecture/adrs/0035-architecture-invariant-catalog.md`** (**Status: Proposed** â€” flip to Accepted when owner reviews).
...
## TB-114 â€” Accelerator chooser â€” buyer job â†’ starter proof pack â†’ expected proof output
...
- Add a chooser artifact that maps buyer jobs to existing starter proof packs, for example:
  - AI governance / LLM risk review â†’ `templates/starter-proof-packs/ai-llm-workload/`
  - Regulated SaaS procurement â†’ `templates/starter-proof-packs/regulated-saas-soc-procurement/`
  - Azure cost governance â†’ `templates/starter-proof-packs/azure-cost-governance/`
  - Healthcare workflow review â†’ `templates/starter-proof-packs/healthcare-data-workflow/`
...
## TB-115 â€” Starter proof pack metadata contract
...
## TB-116 â€” Starter proof pack static validation gate
...
## TB-117 â€” Template-to-proof dry-run harness
...
## TB-118 â€” Golden accelerator walkthrough (one pack only)
...
## TB-119 â€” Policy pack metadata and buyer-safe caveat validation
...
## TB-120 â€” Policy pack dry-run index
...
## TB-121 â€” Route/tier/policy/nav parity release gate hardening
...
## TB-122 â€” Governance outcome summary in sponsor proof
...
## TB-123 â€” Policy-pack freshness report in proof/procurement artifacts
...
## TB-124 â€” Audit coverage drift gate for critical workflows
...
## TB-125 â€” Buyer-safe audit evidence summary in proof bundles
...
## TB-126 â€” Audit event catalog metadata
...
## TB-127 â€” Audit tests for sponsor/procurement proof actions
...
## TB-128 â€” Support/audit triage one-pager
...
## TB-129 â€” Quote-to-proof readiness checklist
...
## TB-130 â€” Quote aging export and follow-up SLA report
...
## TB-131 â€” Commercial closeout artifact hardening
...
## TB-132 â€” Tier fit validation matrix
...
## TB-133 â€” AI & Cloud Architecture Readiness Review offer pack aligned to pricing
...
## TB-134 â€” Commercial copy overclaim guard + public claim-boundary guide
...
## TB-135 â€” V1.1 backlog: SOC 2 CPA attestation program kickoff (assessment #23)

**Window:** **V1.1 backlog** â€” owner / organizational program, not V1 engineering. **Do not** re-prompt from assessment batches; see **`.cursor/rules/V1_1-assurance-backlog.mdc`**.
...
3. Update Trust Center rows only when posture changes â€” no implied â€œSOC 2 issuedâ€ language before CPA delivery.

**Explicit limits:** Not a coding-agent default task; **`(A)` assessments must not** treat absent CPA SOC 2 as a V1 defect.

**Refs:** [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6c, assessment improvement **#23**.

**Size estimate:** Organizational â€” multi-week / multi-month; no repo SLA.

---

## TB-136 â€” V1.1 backlog: third-party pen-test program (assessment #25)

**Window:** **V1.1 backlog** â€” funded vendor engagement, not V1 engineering. **Do not** re-prompt from assessment batches; see **`.cursor/rules/V1_1-assurance-backlog.mdc`**. V1 relies on **owner-conducted** testing (**TB-005**).
...
**Refs:** [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6c, assessment improvement **#25**, **TB-005**.

**Size estimate:** Organizational â€” vendor-led; calendar not pinned here.

---

## TB-137 â€” Real-LLM evidence: full quad-agent live pipeline gate
...
**Pick up when:** live gate still HOLD after owner re-run â€” capture `merge.Errors` from test output and extend normalizer/converters. Also update release checklist/proof docs if the RC evidence attachment rules drift from the owner decision above.
...
**Size estimate:** M (~4â€“8 h) â€” **closed**.

---

## TB-138 â€” Real-LLM evidence: GitHub golden-cohort secrets + required PR check (TB-007 Gap A remainder)

**Status:** **Shipped (2026-05-30)** â€” `ARCHLUCID_GOLDEN_COHORT_REAL_LLM=true` repo variable, `cohort-real-llm-gate` runs on **pull_request** (fork-safe no-op), **`environment: dev`** for Azure federated login + Cost Management probe, CI AOAI secret fallbacks in `real-llm-golden-cohort.yml` / `Invoke-RealLlmGoldenCohort.ps1`, and **`cohort-real-llm-gate`** added to branch ruleset required checks. Owner confirms one green **`golden-cohort-nightly`** run on **`master`** after merge.
...
**Refs:** **TB-007** Gap A, `docs/runbooks/GOLDEN_COHORT_REAL_LLM_GATE.md` Â§ 2, `.github/workflows/golden-cohort-nightly.yml`.

---

## TB-139 â€” Real-LLM evidence: capture token usage and cost in gate metrics
...
**Size estimate:** S (~2â€“4 h).

---

## TB-140 â€” Real-LLM eval corpus: real-mode scenarios + nightly scoring (TB-007 Gap C)
...
**Refs:** `docs/assessments/LATEST.md` Â§9, `docs/runbooks/FIRST_PILOT_OPERATOR_PATH.md`, `docs/go-to-market/QUOTE_TO_PROOF_READINESS_CHECKLIST.md`.

**Size estimate:** Owner/program work â€” depends on selected scenarios and environment access.

---

## TB-142 â€” Near-term GTM backlog: market-facing demo asset production

**Window:** **Near-term GTM backlog priority** â€” brand, audience, and publication approval required. Channel resolved 2026-05-30: optimize **Upwork** first; website, sales email, LinkedIn, and live demo can reuse/adapt later. Evidence policy resolved 2026-05-30: real-mode output may be shown in public assets when authorized, redacted, source-labeled, and caveated; synthetic/demo-labeled assets remain allowed. Owner triage on 2026-06-01 promoted this from V1.1 to near-term GTM work.
...
**Size estimate:** Owner/GTM production work â€” depends on channel and approval path.

---

## TB-158 â€” Pilot success thresholds and acceptance criteria
...
## TB-159 â€” Buyer security/procurement packet
...
## TB-160 â€” Legal/procurement terms packet
...
## TB-161 â€” Design partner / pilot recruiting pipeline
...
## TB-162 â€” Support and pilot operating model
...
## TB-163 â€” Transactable procurement path
...
## TB-164 â€” V1.1 backlog: first named public reference customer
...
**Window:** **V1.1 GTM backlog** â€” not current release work and not a current `(A)` headline-readiness blocker.
...
## TB-165 â€” Assessment score consistency guard
...
## TB-166 â€” Release claim gate for full real-mode AI evidence
...
**Why this is not a duplicate:** **TB-137 â€“ TB-140** generate and improve real-mode evidence. This item controls release claims and release-candidate packaging: if the evidence is not attached and passing, product/proof/release materials must narrow themselves to simulator-only or partial-real-mode language.
...
## TB-167 â€” Sponsor AI readiness posture artifact
...
## TB-168 â€” Executive KPI semantic contract and UI heuristic regression guard
...
## TB-169 â€” Pilot-first onboarding and Operate-surface progressive disclosure
...
- Add empty states that explain what unlocks after first commit, using in-app help routes from **TB-143 â€“ TB-145**.
...
## TB-143 â€” In-app markdown documentation renderer + `/help/{topic}` routes

**Status:** Open. Partial shell exists (`archlucid-ui/src/app/(operator)/help/`) but product users still land on GitHub blob pages for â€œLearn moreâ€ and documentation index links.
...
**Size estimate:** M (~1â€“2 days).

---

## TB-144 â€” Customer-facing documentation registry
...
- Registry is the only authoritative map from product topic id â†’ `/help/{slug}` â†’ markdown source.
...
## TB-145 â€” Migrate operator/product help links from GitHub blob to in-app routes
...
- `HelpPanel.tsx` â€” topic links and core pilot guide CTA use registry in-app hrefs.
- `contextual-help-content.ts` / `ContextualHelp` â€” `learnMoreUrl` resolves to `/help/{slug}` not `toDocsBlobUrl()`.
- `help-topics.ts` â€” add optional `inAppSlug` (or derive from registry); stop defaulting `getDocHref()` to GitHub for operator audiences.
- `archlucid-ui/public/doc-index.json` generation â€” emit in-app URLs for customer-facing entries.
...
- No product â€œLearn moreâ€ link targets `docs/library/PILOT_GUIDE.md` stub on GitHub.

**Refs:** **TB-143**, **TB-144**, **TB-146**, `WizardStepPreset.tsx`, `security-trust-content.ts`, `PilotStartHereStrip.tsx`.

**Size estimate:** M (~4â€“8 h).

---

## TB-146 â€” Redirect-stub ban + canonical target resolution in registry
...
- Registry entries must not use stub-only files as rendered source (e.g. `docs/library/PILOT_GUIDE.md` â†’ [`docs/library/customer-facing/PILOT_GUIDE.md`](customer-facing/PILOT_GUIDE.md) and/or merged `CORE_PILOT.md` sections).
- Add validation script or unit test that fails when a customer-facing registry entry points at a file whose H1/title matches â€œMoved â€”â€ or â€œredirectâ€ without a resolved `canonicalSourcePath`.
- Update any remaining UI/doc-index references that still point at stub paths.

**Acceptance criteria:**

- Clicking pilot guide help from product UI never shows a four-line â€œMoved â€” pilot guideâ€ page (in-app or external).
...
**Size estimate:** XS (~1â€“2 h).

---

## TB-147 â€” CI drift guard â€” no customer-facing GitHub blob links in product UI
...
## TB-148 â€” Role-gated optional â€œView source on GitHubâ€ footer
...
- On in-app doc pages (**TB-143**), render a collapsed footer link â€œView source documentationâ€ only when user is in admin/developer/diagnostics mode (reuse existing admin/diagnostics env or role signals â€” do not expose to buyer-polished shell).
...
0. **Offline verification (no Azure CLI):** `python scripts/report_observability_export_readiness.py --environment Production --out artifacts/observability-export-readiness.md` â€” see `docs/library/OBSERVABILITY.md` (values from process env are detected but never printed; use `--no-process-environment` for committed JSON only).
...
4. **Alerts:** **Shipped (Improvement #22, 2026-05-25)** â€” Terraform **`prometheus_agent_output_rules.tf`** + committed Prometheus YAML. Staging: **`terraform apply`**, one execute smoke, Azure Portal **Test** on a rule. Eval baseline CI remains warn-soak until merge-blocking flip (Improvement **#1** exit criterion).
...
## TB-007 â€” LLM correctness boundary: three remaining gaps after 2026-05-01 session
...
### Gap A â€” Promote cohort-real-llm-gate to a required PR status check

**Status (2026-05-29):** **Local path shipped** for assessment improvement **#1** â€” `secrets/local-real-aoai.env` (gitignored), `scripts/Import-LocalRealAoaiEnv.ps1` (Foundry URL â†’ classic host), `scripts/Invoke-RealLlmEvidenceGate.ps1` (topology-only profile), tolerant parsers (`AgentTypeJsonConverter`, `EvalCorpusFindingSeverityJsonConverter`), and `AzureOpenAiEndpointNormalizer` in `AzureOpenAiCompletionClient`. **CI promotion** remains blocked on owner task: Azure OpenAI golden-cohort deployment secrets / federated identity in the protected GitHub Environment. See **TB-138** and `docs/runbooks/GOLDEN_COHORT_REAL_LLM_GATE.md` Â§ 2 and Â§ 6.

**Prerequisite checklist (Improvement #20, Batch J):** Run **`.\scripts\ci\verify_real_mode_prereqs.ps1 -Profile GoldenCohortGate`** locally (names only). With GitHub CLI: **`-UseGitHubCli`**. Documented in **`docs/engineering/BUILD.md`** Â§ *Real-mode LLM CI and golden cohort*.
...
### Gap B â€” Enable EnforceOnReject after product decision
...
### Gap C â€” Eval corpus has no real-mode scenarios
...
**Size estimate:** Gap A ~1 h (operational, no code). Gap B â€” closed (see Gap B status above). Gap C ~4 h (scenario authoring + workflow wiring).

---

## TB-008 â€” Context ingestion connectors: Phases 3â€“4 after typed stages + orchestrator
...
1. **Phase 3 â€” Meaningful delta + typed enrichers** â€” Introduce `IConnectorDeltaComputer` (shared default + optional per-connector overrides). Replace literal-string deltas where useful (e.g. set-diff on `SourceId`). Split `CanonicalInfrastructureEnricher` into per-`ObjectType` enrichers behind a composite.

2. **Phase 4 â€” Cross-connector coupling** â€” Resolve `PolicyReferenceConnector` / topology stable-ID duplication via a shared resolver service consumed by policy + topology stages so overlap logic is not replicated.
...
**Size estimate:** Phase 3 ~4â€“8 h (delta semantics + enricher split + regression). Phase 4 ~2â€“4 h (extract shared topology resolution + tests).

---

## TB-013 â€” Documentation library audience reorganisation (remaining phases)

**Status:** **Phase 1 shipped** â€” subtrees **`docs/library/customer-facing/`** and **`docs/library/contributor-reference/`** plus README indexes; persona recipes canonical at [`customer-facing/WORKFLOW_RECIPES_BY_PERSONA.md`](customer-facing/WORKFLOW_RECIPES_BY_PERSONA.md); bookmark stub [`WORKFLOW_RECIPES_BY_PERSONA.md`](WORKFLOW_RECIPES_BY_PERSONA.md) under **`docs/library/`**.
...
**Size estimate:** Phase 2 ~2â€“6 h (script plus stubs plus CI green). Phase 3 ~1â€“2 days (freeze, batch rewire, smoke).

---

## TB-014 â€” LLM token wallet (non-expiring auto-replenish)

**Progress (2026-05-10):** Operator path shipped â€” persistent bump column **`PurchasedCapBumpUsd`** on **`dbo.LlmMonthlyTenantBudgetState`** (migration **`155_LlmMonthlyTenantBudgetPurchasedCapBump.sql`**) + effective cap in **`LlmMonthlyTenantDollarBudgetTracker`**; runbook **[`LLM_BUDGET_TOP_UP.md`](LLM_BUDGET_TOP_UP.md)**; test hook **`InMemoryLlmTenantBudgetRepository.ApplyMonthlyPurchasedCapBumpAsync`**.

**Progress (2026-05-26):** **Shipped (Batch F)** â€” `221_LlmTenantWallet.sql`, `LlmTenantWalletService`, `StripeWalletGateway`, wallet webhook handling on existing Stripe route, `GET/PUT /v1/billing/wallet`, operator billing wallet panel, metrics, unit tests. Operator **`PurchasedCapBumpUsd`** path unchanged.

**Remaining follow-ons:** Stripe Elements card collection in UI (TEST uses manual `cus_`/`pm_` ids today); optional live-key flip per [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6b.

**Decision (operator, 2026-05-11):** **Greenlit in principle.** There is **no** target cost-per-run budget â€” runs are bounded by **`LlmMonthlyTenantDollarBudget`** + **`LlmTokenQuota`**, not by a per-run prompt-design ceiling. Tenants who legitimately exhaust their **`HardCutoffUsdPerUtcMonth`** before the UTC month rolls should be able to **buy more LLM headroom** self-serve, rather than waiting or contacting sales.
...
| Balance expiry | **Never** â€” carries forward indefinitely |
...
- Tenant monthly budget still governs **included** envelope â€” see [`ArchLucid.Core/Configuration/LlmMonthlyTenantDollarBudgetOptions.cs`](../../ArchLucid.Core/Configuration/LlmMonthlyTenantDollarBudgetOptions.cs) and `LlmCompletionAccountingClient` enforcement.
- Wallet covers **overage only** â€” after effective cap would be exceeded, debit wallet; do not inflate **`PurchasedCapBumpUsd`** for self-serve purchases.
- Stripe **TEST** keys on staging (confirmed 2026-05-25); live keys flip per [`docs/library/V1_DEFERRED.md`](V1_DEFERRED.md) Â§6b.
- **Azure Marketplace plan-add-on** for wallet refills is **deferred** â€” ship Stripe-only self-serve for V1; Marketplace alignment when commerce un-holds.
...
- **Real-time settlement.** Charge the card **at refill time** (when balance drops below **$10**), not at UTC month end â€” bounds unbilled exposure to one refill increment (~**$50**) per tenant.
- **No balance expiry.** Wallet balance **never** expires; rejects the earlier â€œuse it or lose it within UTC monthâ€ draft.
...
flowchart LR
  ...
  Cap -->|no| Wallet{Wallet balance<br/>â‰¥ estimated cost?}
  Wallet -->|yes| Allow
...
- **`dbo.LlmTenantWalletState`** â€” balance, auto-replenish flag, refill increment/trigger defaults, monthly cap, UTC-month refill counter, Stripe customer/payment-method refs.
- **`dbo.LlmTenantWalletLedger`** â€” append-only **`Refill` | `Consume` | `OperatorAdjustment`** with **`BalanceAfterUsd`**, optional **`StripePaymentIntentId`**.
- **`LlmTenantWalletService`** â€” `GetBalanceAsync`, `ConsumeAsync`, `TryAutoRefillAsync` (cap + threshold checks).
- **`IStripeWalletGateway`** â€” Stripe.net PaymentIntent; config **`Billing:Stripe:SecretKey`**.
- **`LlmCompletionAccountingClient`** â€” after monthly cap check fails, consult wallet; queue consume + optional refill via background task.
- **`WalletController`** â€” `GET/PUT /v1/billing/wallet`, `POST /v1/billing/stripe/webhook`.
- **UI** â€” balance, cap slider (**$0â€“$500**, step **$50**), auto-replenish toggle, Stripe Elements.

**Out of scope for this item**

- Azure Marketplace wallet SKU (follow-on at commerce un-hold).
- Replacing per-tier **`LlmMonthlyTenantDollarBudget`** defaults â€” wallet is **additive overage**, not a tier change.
- Per-run dollar ceilings (explicitly rejected).
- Refunding wallet balance on tenant cancellation.

**Security model**

Wallet config requires **`Admin`** (same as billing today). Webhook validates Stripe signature and tenant binding. Payment method stored as Stripe **`PaymentMethodId`** only â€” no raw PAN in ArchLucid SQL.

**Operational considerations**

- Reconciliation: Stripe PaymentIntents â†” **`LlmTenantWalletLedger`** **`Refill`** rows; nightly script once volume justifies it.
...
- [`docs/library/V1_DEFERRED.md`](V1_DEFERRED.md) Â§6b (commerce un-hold sequencing)
- [`docs/OPERATIONS_LLM_QUOTA.md`](../OPERATIONS_LLM_QUOTA.md)
- [`docs/assessments/LATEST.md`](../assessments/LATEST.md) â€” Improvement **#27** (implementation prompt)

**Size estimate:** **M** â€” ~1â€“2 days end-to-end (wallet tables + service + Stripe gateway + webhook + UI + metrics + tests + doc sync). Gating piece is **`LlmCompletionAccountingClient`** wallet fallback path.

---

## TB-015 â€” Per-agent/per-invoke-kind LLM token dimensions + CI export
...
- **`LlmCompletionAccountingClient`** aggregates **`ArchLucidInstrumentation.LlmPromptTokensTotal`** / **`LlmCompletionTokensTotal`** (**`MeterName`** = **`ArchLucid`**) â€” optional tags are **`tenant_id`**, **`llm_provider`**, **`llm_deployment`** via **`RecordLlmTokenUsage`** only â€” **`not`** `agent_type` or invoke role.
...
- OTel **`Activity`** **does** tag the agent handler (**`AgentHandler`**): **`archlucid.agent.type_enum`** (**`RealAgentExecutor`** line ~362), and **`AzureOpenAiCompletionClient`** tags **`AgentLlmCompletion`** spans with **`gen_ai.usage.*`** â€” downstream Azure Monitor / Application Insights traces can correlate **when** exporters are wired, but Prometheus counters are **flat** unless we add dimensions.

Until **TB-015** ships, â€œaverages without a live deploymentâ€ mean **estimated** bounds (**`AgentExecutionCostPreviewController`** + **`PER_TENANT_COST_MODEL.md`**), **not measured** envelopes.

---

### Phase A â€” Bounded dimensions on token counters/histogram
...
**Histogram vs counter:** Prefer **adding** **`Histogram<long>`** (`archlucid.llm.completion_tokens`) with the same bounded tags plus **distribution** queries (percentiles); keep **additive counters** so existing golden-cohort Grafana tiles keep working â†’ **dual emit** counters + histogram for Phase A if cost is negligible (one AOAI invoke = one histogram point).

---

### Phase B â€” Unit + integration tests without Azure
...
**Integration (optional smoke):** `WebApplicationFactory` + echo mode proves tags survive the full **`IAgentCompletionClient`** decorator chain (accounting â†’ cache â†’ cost guardrail).

---

### Phase C â€” Capture in CI (`golden-cohort` + optional nightly)

**When real AOAI is available:**

1. After **`golden-cohort drift --strict-real`** (**`.github/workflows/golden-cohort-nightly.yml`** â†’ **`cohort-real-llm-live`**) scrape **`GET /metrics`** **if** the API host exposes Prometheus (**`Observability:Prometheus:Enabled`** for that environment), **or** export OTLP traces to a temp sink and sum **`gen_ai.usage.*`** by correlated **`archlucid.agent.type_enum`** (**more fragile â€” prefer Prometheus parse**).
2. Check in **`scripts/ci/aggregate_llm_token_metrics.py`** (new): parse text exposition format; aggregate **per-consumer / per-invoke_kind** deltas for the workflow window; emit **`golden-cohort-llm-token-report.md`** GH Actions artifact (+ optional **`tests/golden-cohort/telemetry-snapshots/last-real-run-tokens.sample.json`** for doc examples â€” **never** commit secrets).

**Frequency:** Weekly live job is sufficient for trend; rerun when **`MaxCompletionTokens`**, prompts, or model SKU changes.

---

### Phase D â€” Product doc + estimator alignment

Roll forward measured **p50 / p95** ranges into **`docs/library/PER_TENANT_COST_MODEL.md`** (â€œmeasured cohort 2026-â€¦â€) distinct from **`GET /v1/agent-execution/cost-preview`** hypothetical bounds â€” until then, **`cost-preview`** remains explicitly **estimated**.
...
- Labels are **literal enums** bounded to **Architecture agent types Ã— invoke kinds**, not free text or tenant-derived strings (tenant stays on **`RecordPerTenantTokens`** paths only).

**Size estimate:** **M** â€” ~2â€“4 eng days (**Phase A+B** dominating C); **Phase C** is ops + scripting once metrics exist.

---

## TB-016 â€” ITSM + Slack vendor sandbox accounts (provision + secrets + inbound URLs)

**Status (operator question, resolved for scope 2026-05-11):** The repo **does not** ship tenant credentials or long-lived sandbox URLs â€” **those are operator-owned**. Free / trial programs exist for **Jira Cloud**, **ServiceNow Developer instances**, **Confluence Cloud**, and **Slack developer workspaces**. Use **separate pilot projects / spaces / channels** away from production knowledge bases â€” never reuse brittle automation credentials against prod SOX systems.
...
Recurrence aligns with **`ArchLucid_Assessment_Weighted_Readiness_2026_05_10`** â€” Improvement 8 (**scheduled + `workflow_dispatch`**, not one-off).
...
2. Enable **Jira**; create a **pilot project** â€” note **`project key`**.

**Outbound auth ArchLucid expects (MVP)** â€” **`ArchLucid.Core/Configuration/IntegrationsItsmOutboundOptions.cs`** (**`Integrations:ItsmOutbound:Jira`**):

| Binding | Contents |
|---------|----------|
| `CloudBaseUrl` | **`https://{site}.atlassian.net`** (no trailing slash) |
| `ServiceAccountEmail` | Atlassian-account email |
| `ApiToken` | **Profile â†’ Security â†’ API tokens** (Key Vault / deploy secret â€” **never git**) |
| `DefaultProjectKey` | Fallback when tenant SQL override empty |

**Inbound**

- **`Integrations:ItsmInbound:JiraWebhookSecret`**; vendor POST validates shared header â€” **`ItsmInboundWebhooksController`**, **`docs/integrations/smoke/CONNECTOR_SMOKE_JIRA.md`**.
- Jira **Automation** â†’ **`POST`** your API **`â€¦/integrations/webhooks/jira`** (staging hostname or authenticated tunnel).

---

### B. ServiceNow (personal developer instance â†’ Table API)

**Provision**

1. **[ServiceNow Developer Program](https://developer.servicenow.com/)** portal â†’ **Request / open Personal Developer Instance** â†’ record **`InstanceBaseUrl`** (instances may sleep unless kept warm).
2. Integration user + **`incident`** Table API create (+ CMDB reads if testing **`cmdb_ci`** paths).

**Outbound** â€” **`Integrations:ItsmOutbound:ServiceNow`**: **`InstanceBaseUrl`**, **`Username`**, **`Password`** (basic auth MVP).

**Inbound** â€” **`Integrations:ItsmInbound:ServiceNowWebhookSecret`** + **`CONNECTOR_SMOKE_SERVICENOW.md`** business-rule HTTP.

Developer instances reset / sleep â€” cron smoke should **`continue-on-error`** with clear stderr when unreachable.

---

### C. Confluence Cloud (typically same Atlassian site as Jira)

**Publishing** â€” **`ArchLucid.Core/Configuration/ConfluencePublishingOptions.cs`** (**`Integrations:ConfluencePublishing`**): **`Enabled`**, **`CloudBaseUrl`**, **`SpaceKey`**, **`ServiceAccountEmail`**, **`ApiToken`** (Basic + Cloud token until OAuth ships per catalog).

**OAuth follow-on:** **`INTEGRATION_CATALOG.md`** â€” MVP is Basic.
...
2. **Incoming Webhooks** Slack app installation â†’ **`#integrations-smoke`**.

**Auth model** â€” store webhook URL as **opaque secret** (GitHub Action secret `SLACK_INCOMING_WEBHOOK_URL`); bind to **`SlackWebhook`** **`AlertRoutingSubscription`** / digest route per **`CONNECTOR_SMOKE_SLACK.md`** (SQL **`destination`** is sensitive â€” tighten operator RBAC).
...
**Suggested secret inventory (names only â€” values never committed)**
...
Map into host **`Azure Key Vault`** / GitHub Actions **staging environment** (`Integrations:ItsmOutbound`, `Integrations:ItsmInbound`, `Integrations:ConfluencePublishing`) â€” authoritative key list **`docs/library/CONFIGURATION_REFERENCE.md`**.
...
2. Metric / ops query: dormant trials by phase + **`TenantDatabaseBindings`** state â€” alert when elasticity pool SKU pressure climbs.
...
**Refs:** [`docs/go-to-market/TRIAL_AND_SIGNUP.md`](../go-to-market/TRIAL_AND_SIGNUP.md) Â§4, [`docs/runbooks/TRIAL_LIFECYCLE.md`](../runbooks/TRIAL_LIFECYCLE.md).

---

## TB-018 â€” Warm tenant catalogs in elastic pool (signup latency)

**Context:** Hosted **`SystemWithPerTenantCatalogs`** signup currently runs **`SqlTenantSqlCatalogProvisioner.ProvisionTenantCatalogAsync`** which **always executes** **`DatabaseMigrator.RunTenant`** before mirroring **`dbo.Tenants`**. Migrating hundreds of migrations on-demand adds seconds to tens of seconds under load â€” acceptable at low signup rate, poor for bursts.

**Target behavior**

1. **Terraform/IaC** â€” configure **warm pool depth** (**N**) of empty product catalogs attached to an **elastic pool** (reuse existing pool per environment or dedicated small pool â€” FinOps spreadsheet).
...
3. **Claim:** On signup, dequeue warm DB â†’ **`UpsertPending` / MarkActive fast path**: **skip RunTenant when schema stamp matches deployed version** â†’ **`MirrorTenantRowFromSystemAsync`** â†’ **`MarkActive`**; optional **`ALTER DATABASE â€¦ MODIFY NAME`** to canonical **`TenantDatabaseNaming`** form.
4. **Post-claim** â€” enqueue replenish to restore **N**.

**Safety / correctness**

- No resolver cache until **`MarkActive`**; invalidate after claim (**`InvalidateCachedTenantConnectionString`** already exists).
- Orphan warm DB teardown if tenant insert fails mid-claim â†’ align with **`TB-017`** teardown SOP.
- Pool capacity alert when **warm depth &lt; threshold** â€” avoid empty pool at peak signup.
...
## TB-019 â€” Signup marketing attribution + server-side conversion
...
1. **Capture** normalized first-touch attribution on marketing entry (**`utm_source`**, **`utm_medium`**, **`utm_campaign`**, **`utm_content`**, optional **`utm_term`**) â€” **`httpOnly`**, **`Secure`**, **`SameSite=Lax`** cookie or equivalent first-party KV with short TTL (**â‰¤ 90 days**) and sanitization (**max lengths**, strip PII-ish junk).
2. **Propagate** into the signup API boundary (authenticated provisioning unchanged; anonymous trial signup is the MVP scope). If the signup flow spans **`archlucid-ui` â†’ `/v1/...`** only, threading may be **`x-archlucid-first-touch`** header derived from cookie server-side â€” choose one transport, document in **`PUBLIC_MARKETING_SITE_TOPOLOGY.md`** or API notes alongside OpenAPI-touching edits per **`docs/library/API_CONTRACTS.md`** if request DTO grows.
3. **Persist durability** optional but recommended: **`dbo.TenantMarketingAttribution`** (or widen **`AuditEvents`** with typed payload) keyed by **`TenantId`** + **`CapturedUtc`** with **immutable insert** â€” supports SQL cohort reports without exploding OTel label cardinality.
4. **Telemetry** â€” increment **low-cardinality** counters/histogram **after provision succeeds end-to-end** (tenant row **and** tenant catalog **`Active`** if per-tenant mode). Example coarse buckets: **`attribution.medium`** âˆˆ **`{organic, paid_direct, referral, unknown}`**, **`attribution.platform`** âˆˆ **`{linkedin, google, bing, internal, unknown}`** (map from raw **`utm_*`** in fixed code tables). **Never** attach raw **`utm_campaign`** strings to Prometheus-style metrics â€” keep raw values in SQL/Audit only.

**Safety / correctness**

- **Privacy / consent** â€” first-party technical attribution should stay documented in **`PRIVACY_POLICY.md`** companion change when semantics ship (legal owns final wording â€” especially EU traffic).
- **Idempotency** â€” first-touch wins; ignore naive rewrite spikes except telemetry alerts.
- **Tests** â€” unit map raw UTM tuples â†’ coarse buckets + integration asserting provision path emits conversion once only.
...
## TB-020 â€” Public marketing structured data + consent-gated third-party analytics
...
**Status (2026-05-10):** **`JSON-LD`** â€” `SoftwareApplication` on `(marketing)` + **`FAQPage`** on **`/trust`** (`TrustCenterFaqJsonLd`). **Microsoft Clarity** â€” consent banner + loader; CSP allows `https://www.clarity.ms` + Bing pixel host; **`PRIVACY_POLICY.md`** updated (Â§2.4). **Config:** `NEXT_PUBLIC_ARCHLUCID_CLARITY_PROJECT_ID` in **`archlucid-ui/.env.example`**. **Remaining:** DPIA text for EU-heavy traffic if legal requests; optional CONFIGURATION_REFERENCE row if ops wants server-side kill-switch mirror.

**What to ship**

1. **`JSON-LD`** â€” inject **`@type: SoftwareApplication`** (and minimal **`publisher`**) from **`(marketing)`** shells with narrative aligned **`POSITIONING.md`** â€” **never** mint **`aggregateRating`** / **`reviewCount`** unless tied to audited real survey data. *(Done for marketing shell.)*
2. **FAQ blocks only where copy supports them** â€” e.g. discrete Q/A on **`/compliance-journey`** or **`/trust`** excerpts; reject spam-tier FAQ schema stuffing. *(Partial: factual **`FAQPage`** on **`/trust`**.)*
3. **Analytics gated** â€” optional **Microsoft Clarity** (or chosen vendor) activates only when (**a**) optional server kill-switch (exact subtree TBD **`docs/library/CONFIGURATION_REFERENCE.md`**) and (**b**) client consent UX exists where jurisdictions require opt-in (**`(marketing)`** subtree only until a separate DPIA says otherwise for logged-in shells). *(Partial: consent + `NEXT_PUBLIC_ARCHLUCID_CLARITY_PROJECT_ID`.)*
4. **CSP** â€” extend **`archlucid-ui/next.config.ts`** **`script-src`** / **`connect-src`** minimally per vendor subdomain allowlist + changelog entry in **`PRIVACY_POLICY.md`** noting active vendors. *(Done for Clarity + Bing image host used by Clarity.)*
...
## TB-021 â€” RAG quality program â€” V1 foundation

**Decision (engineering, 2026-05-23):** **Greenlit in principle** â€” RAG infrastructure already ships (`ArchLucid.Retrieval`, `AskService` retrieval, ADR 0004 outbox, ADR 0005 LLM pipeline). V1 work **extends** that stack to raise agent **faithfulness** and **citation density** without new vector stores or agentic multi-hop retrieval.

**Authoritative task breakdown:** [`RAG_QUALITY_TECHNICAL_BACKLOG.md`](RAG_QUALITY_TECHNICAL_BACKLOG.md) â€” stable sub-IDs **RAG-V1-000** through **RAG-V1-011**.

**First implementation slice (approved design):** [`RAG_CORPUS_KIND_POLICY_PACK_DESIGN.md`](RAG_CORPUS_KIND_POLICY_PACK_DESIGN.md) â€” implement **RAG-V1-000 partial** + **RAG-V1-001** as one PR (~3â€“5 eng days). Remaining **RAG-V1-000** items (`RetrievalGroundingTrace`, citation formatter, architecture test) follow in a second PR.

**Why assessments should schedule this:** Directly targets **AI/Agent Readiness**, compliance finding honesty, cost citation contract ([`V1_SCOPE.md`](V1_SCOPE.md) Â§2.16), and Ask grounded answers â€” not net-new product surfaces.

**Recommended pick-up order**

| Sub-ID | Title | Size | Design |
|--------|-------|------|--------|
| **RAG-V1-000** (partial) + **RAG-V1-001** | `CorpusKind` seam + policy-pack indexer + compliance retrieval | Sâ€“M | [`RAG_CORPUS_KIND_POLICY_PACK_DESIGN.md`](RAG_CORPUS_KIND_POLICY_PACK_DESIGN.md) |
| **RAG-V1-005** | Faithfulness eval harness + citation coverage CI | M | Backlog only â€” output-side; IR metrics â†’ **RAG-V1-011** |
| **RAG-V1-006** | `RetrievalGroundingTrace` forensic enrichment â€” **TB-038** | Sâ€“M | [`RAG_QUALITY_TECHNICAL_BACKLOG.md`](RAG_QUALITY_TECHNICAL_BACKLOG.md) Â§RAG-V1-006 |
| **RAG-V1-010** | Tenancy isolation hardening â€” **TB-048** | S | Retrieval audit 2026-05-26 â€” P0 security |
| **RAG-V1-007** | Embedding model drift guard â€” **TB-045** | Sâ€“M | Retrieval audit 2026-05-26 |
| **RAG-V1-011** | Retrieval IR eval (recall@k, MRR) â€” **TB-049** | Done (Batch E, 2026-05-26) | Retrieval audit 2026-05-26 â€” P0 correctness |
| **RAG-V1-008** | Index freshness + ContentHash skip â€” **TB-046** | Sâ€“M | Retrieval audit 2026-05-26 |
| **RAG-V1-009** | Chunking strategy fingerprint â€” **TB-047** | S | Retrieval audit 2026-05-26 |

**Hard constraints (summary)**

- Deterministic rules still fire in code â€” RAG enriches narrative only.
- Retrieved chunks are **prompt context** â€” excluded from manifest canonical fingerprint unless a future ADR adds snapshotting.
- Tenant-bound corpora: mandatory scope filters on index and query.
- Cross-tenant text RAG is **out of scope** â€” see ADR 0031 for k-anon aggregates only.

**V1.1 / V2 follow-ons:** **RAG-V1.1-*** and **RAG-V2-*** live in [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6q â€” not `(A)` V1 GA obligations.
...
**Size estimate:** **Mâ€“L** phased â€” ~2â€“3 weeks eng if executed sequentially; **first slice** (`RAG_CORPUS_KIND_POLICY_PACK_DESIGN.md`) is ~3â€“5 eng days.

---

## TB-022 â€” `LlmCostEstimator` â€” `int` overflow in aggregator token-count fields â€” **Done (Improvement #19, 2026-05-25)**
...
int completionSum = 0;
// ...
promptSum += inTok;   // overflows at int.MaxValue â‰ˆ 2.1 B tokens
...
3. Update `RunLlmTokenCountsResponse` fields (`Prompt`, `Completion`) and any callers that downcast to `int` â€” check `RunAgentExecutionLlmCostEstimateAppender` and any frontend DTO mapping.
4. Update `AgentExecutionTraceRunLlmCostAggregatorTests` with assertions that would have caught the overflow (e.g. token counts > `int.MaxValue` across multiple traces â€” or at minimum add a comment warning for future large-scale tests).
...
**Size estimate:** **XS** â€” ~30 min mechanical change + test annotation.

</details>

---

## TB-026 â€” `LlmCostEstimator` â€” negative-rate guard on `LlmDeploymentUsdRates` â€” **Done (Improvement #19, 2026-05-25)**
...
**Size estimate:** **XS** â€” ~1 h including annotation, IValidateOptions check, and test.

</details>

---

## TB-024 â€” `LlmCostEstimator` â€” reasoning-token test coverage

**Status:** **Done** (Improvement **#20**, 2026-05-25) â€” explicit reasoning rate, output-rate fallback, per-deployment reasoning override, persisted override + reasoning fallback, and OTel `archlucid_llm_cost_usd_total` alignment covered in **`LlmCostEstimatorTests`**.
...
- Global rate override (`ILlmCostEstimationUsdRateOverride`) combined with reasoning fallback â€” the fallback uses the *overridden* output rate, not the config output rate; this is correct but currently invisible in tests.
...
**Size estimate:** **XS** â€” ~30 min.

---

## TB-023 â€” `LlmCostEstimator` â€” document replay-rate semantics (live rate vs. stored-per-trace divergence) â€” **Done (Improvement #18, Batch J, 2026-05-26)**

**Source:** Cost estimator audit-grade correctness review (2026-05-24).

**Shipped:** XML remarks on **`ILlmCostEstimator`**, **`LlmCostEstimator`**, and **`AgentExecutionTraceRunLlmCostAggregator.Compute`**; operator table in **`docs/library/PER_TENANT_COST_MODEL.md`** Â§ *Rate changes and replay*; TB-023 class summary on **`LlmCostEstimatorTests`**.
...
1. ~~Add remarks to `ILlmCostEstimator.EstimateUsd`~~ â€” done.
2. ~~Add remarks to `AgentExecutionTraceRunLlmCostAggregator.Compute`~~ â€” done.
3. ~~Add operator note in `PER_TENANT_COST_MODEL.md`~~ â€” done.
...
**Size estimate:** **XS** â€” ~30 min (comments + one paragraph in ops doc).

---

## TB-025 â€” `LlmCostEstimator` â€” annotate OTel `double` cast and pretax nature

**Source:** Cost estimator audit-grade correctness review (2026-05-24).

**Problem:** Two undocumented correctness caveats exist in the metrics emission path:

1. **`decimal â†’ double` precision loss.** `ArchLucidInstrumentation.RecordLlmCostUsd` casts the `decimal` estimate to `double` before adding to the `Counter<double>` OTel instrument (`archlucid_llm_cost_usd_total`). Values like `$0.000003` are not exactly representable in IEEE 754 `double`, introducing rounding error that accumulates in the Prometheus counter. The in-process `decimal` and any SQL-persisted values are unaffected.

2. **Pretax only, not labeled as such.** The counter description ("Estimated LLM USD from token counts Ã— rates") does not state that the value is pretax. Operators reconciling the counter against an Azure invoice (which includes VAT/GST depending on jurisdiction) will see unexplained discrepancies.

**What to do:**

1. Update the `LlmCostUsdTotal` counter description to read: *"Pre-tax estimated LLM spend in USD from token counts Ã— configured per-million rates (label tenant). Monitoring-grade only â€” not invoice-reconciliation-grade; the decimal-to-double cast introduces sub-microdollar IEEE 754 rounding. Does not include VAT/GST."*
...
**Affected files:**
- [`ArchLucid.Core/Diagnostics/ArchLucidInstrumentation.cs`](../../ArchLucid.Core/Diagnostics/ArchLucidInstrumentation.cs) â€” `LlmCostUsdTotal` counter definition and `RecordLlmCostUsd` method
- [`ArchLucid.Core/Configuration/ILlmCostEstimator.cs`](../../ArchLucid.Core/Configuration/ILlmCostEstimator.cs)

**Size estimate:** **XS** â€” ~20 min (comments + description string updates).

---

## TB-027 â€” Introduce `IAgentExecutor` port â€” eliminate AgentSimulator coupling from production assemblies

**Status (2026-05-31):** **Done** â€” `IAgentExecutor` in Core; `AgentRuntime`/`Capabilities.Cost`/`Host.Core` do not reference `AgentSimulator`; composition-root binding + `DependencyConstraintTests` positive-list (`Host.Composition`, `*.Tests` only).

**Source:** Dependency graph audit (2026-05-26). Three production assemblies â€” `ArchLucid.AgentRuntime`, `ArchLucid.Capabilities.Cost`, and `ArchLucid.Host.Core` â€” directly reference `ArchLucid.AgentSimulator`. Because `Application` depends on `Capabilities.Cost`, the simulator is a transitive runtime dependency of every production code path through Application. The existing `AgentRuntime_references_AgentSimulator_by_design` test documents the coupling without resolving it.
...
- The coupling is invisible to callers who depend on `Application` â€” it emerges transitively via `Capabilities.Cost`.
...
3. Update `ArchLucid.Capabilities.Cost` â€” audit which simulator types are used directly and replace with the port. Remove the `<ProjectReference>` to `ArchLucid.AgentSimulator`.
4. Update `ArchLucid.Host.Core` â€” move any simulator registration or conditional wiring to `ArchLucid.Host.Composition`. Remove the `<ProjectReference>` to `ArchLucid.AgentSimulator` from `Host.Core.csproj`.
5. In `ArchLucid.Host.Composition`, bind `AgentSimulator`'s concrete type to `IAgentExecutor` for non-production environments (the composition root is the correct place for this â€” it already references `AgentSimulator`).
...
- No behavioural change to simulation paths â€” `AgentSimulator` is still wired by Host.Composition in non-production; callers just see the port.
- All existing `AgentRuntime.Tests` and `Application.Tests` that use the simulator directly through project references are unaffected â€” test projects may still reference `AgentSimulator` directly.
- Run the full Architecture.Tests suite and compile-check all affected projects before closing.

**Affected files / projects:**

- `ArchLucid.Core` or `ArchLucid.Contracts` â€” new `IAgentExecutor.cs`
- `ArchLucid.AgentRuntime/ArchLucid.AgentRuntime.csproj` â€” remove `AgentSimulator` reference
- `ArchLucid.Capabilities.Cost/ArchLucid.Capabilities.Cost.csproj` â€” remove `AgentSimulator` reference
- `ArchLucid.Host.Core/ArchLucid.Host.Core.csproj` â€” remove `AgentSimulator` reference; move wiring to `Host.Composition`
- `ArchLucid.Host.Composition/Startup/ServiceCollectionExtensions.*.cs` â€” bind `IAgentExecutor`
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” delete `_by_design` tests; add hard-fail + positive-list guards

**Size estimate:** **M** â€” ~1â€“2 eng days (interface definition + reference removal + host composition wiring + Architecture.Tests updates + test regression).

---

## TB-028 â€” Move `Integrations.AzureExtractor` wiring out of `Api.csproj` into Host.Composition

**Status (2026-05-31):** **Done** â€” `ArchLucid.Api.csproj` has no direct `Integrations.AzureExtractor` reference; wiring remains in `Host.Composition`; `Api_csproj_must_not_reference_Integrations_AzureExtractor_assembly` in `DependencyConstraintTests`.

**Source:** Dependency graph audit (2026-05-26). `ArchLucid.Api.csproj` carries a direct `<ProjectReference>` to `ArchLucid.Integrations.AzureExtractor`. This violates the single-composition-root rule enforced by `SingleCompositionRootServiceCollectionExtensionsTests` â€” adapter wiring belongs exclusively in `Host.Composition`.
...
1. Audit `ArchLucid.Api` source files for all usages of types from `ArchLucid.Integrations.AzureExtractor`. Identify which, if any, are referenced from controller code (unlikely â€” should only be DI registration).
...
4. Verify that `ArchLucid.Host.Composition` already references `ArchLucid.Integrations.AzureExtractor` â€” it does; no new reference is needed there.
...
- `Api â†’ Host.Composition â†’ Integrations.AzureExtractor` is already the transitive path; removing the direct reference does not change what is registered at runtime.
- Compile-check `ArchLucid.Api` after removing the reference to confirm no direct type usages remain.

**Affected files / projects:**

- `ArchLucid.Api/ArchLucid.Api.csproj` â€” delete `AzureExtractor` project reference
- `ArchLucid.Api/Program.cs` or startup code â€” move any direct registration calls
- `ArchLucid.Host.Composition/Startup/ServiceCollectionExtensions.ApplicationPipeline.cs` â€” receive the registration (likely already there)
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” add hard-fail assertion

**Size estimate:** **XS** â€” ~30 min (delete one csproj line, possibly move one registration call, add one test).

---

## TB-029 â€” Replace `Decisioning â†’ Notifications` with domain events
...
- All existing notification behaviour must be preserved â€” verify end-to-end in `Decisioning.Tests` (use a fake `IDomainEventPublisher`) and in integration smoke.
...
- `ArchLucid.Core` â€” new `IDomainEventPublisher.cs` (if not already present), domain event records
- `ArchLucid.Decisioning/ArchLucid.Decisioning.csproj` â€” remove `Notifications` reference
- `ArchLucid.Decisioning` source â€” replace direct notification calls with `IDomainEventPublisher.Publish`
- `ArchLucid.Notifications` or adapter â€” new domain event handler
- `ArchLucid.Host.Composition` â€” register handler
- `ArchLucid.Decisioning.Tests` â€” update to use fake publisher; remove Notifications test doubles
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” replace `_by_design` test with hard-fail assertion

**Size estimate:** **Mâ€“L** â€” ~1â€“3 eng days depending on how widely Notifications types are used within Decisioning and whether a domain event bus is already in place.

---

## TB-030 â€” Architecture.Tests gap closure â€” add Mcp, AzureExtractor, AgentSimulator, Jobs.Cli coverage + 10 missing `[Fact]`s
...
| Assembly | Risk |
|---|---|
| `ArchLucid.Mcp` | No guard against `Mcp â†’ Application` or `Mcp â†’ Persistence` â€” can silently gain prohibited dependencies |
| `ArchLucid.Integrations.AzureExtractor` | No guard against `AzureExtractor â†’ Application` |
...
2. Add the following `[Fact]` methods to `DependencyConstraintTests.cs` (use existing patterns â€” `ReadProjectReferenceAssemblyNames` + `Types.InAssembly(...).ShouldNot().HaveDependencyOn(...)`):
...
- Do not add `AgentSimulator` or `Jobs.Cli` to `SingleCompositionRootArchitectureTestConstants.CompositionRootScannedProductAssemblyNames` â€” they are not product assemblies subject to the composition-root scan.

**Affected files / projects:**

- `ArchLucid.Architecture.Tests/ArchLucid.Architecture.Tests.csproj` â€” four new `<ProjectReference>` entries
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” 10+ new `[Fact]` methods

**Size estimate:** **S** â€” ~2â€“4 h (mostly mechanical: reference adds + test method authoring + one test run to catch any pre-existing violations).

---

## TB-031 â€” Disambiguate ArtifactSynthesis / Decisioning layer position
...
**Source:** Dependency graph audit (2026-05-26). `ArchLucid.ArtifactSynthesis` depends directly on `ArchLucid.Decisioning` (`ArtifactSynthesis â†’ Decisioning`). Both are positioned at the same nominal layer (analysis / domain services, below Application), yet the dependency is unidirectional. Neither assembly has an Architecture.Tests assertion stating which is "above" the other, making future bidirectional coupling a silent regression.

**Problem:**

Without an explicit layer ordering, it is valid to ask: can Decisioning depend on ArtifactSynthesis? The current graph says no (there is no such edge), but there is no test enforcing that. Over time, a developer could add `Decisioning â†’ ArtifactSynthesis` and create a cycle.
...
**Affected files / projects (Option A â€” minimal):**

- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” two new facts
- `docs/library/SYSTEM_MAP.md` â€” layer clarification

**Affected files / projects (Option B â€” full):**

- `ArchLucid.Contracts` â€” new type(s) moved from Decisioning
- `ArchLucid.Decisioning/ArchLucid.Decisioning.csproj` â€” no change (already references Contracts)
- `ArchLucid.ArtifactSynthesis/ArchLucid.ArtifactSynthesis.csproj` â€” remove Decisioning reference
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” two new hard-fail assertions

**Size estimate:** **XSâ€“S** â€” Option A ~30 min; Option B ~2â€“4 h depending on how many types need to move.

---

## TB-032 â€” Replace `Mcp â†’ Retrieval` direct coupling with a query port

**Source:** Dependency graph audit (2026-05-26). `ArchLucid.Mcp` (a protocol adapter â€” infrastructure layer, L4) depends directly on `ArchLucid.Retrieval` (application orchestration layer, L3). `Retrieval` itself depends on `Decisioning`, `Provenance`, and `ArtifactSynthesis`, giving the Mcp adapter a wide transitive footprint into the application layer. This violates the principle that infrastructure adapters depend on port *interfaces*, not on application layer *implementations*.
...
4. In `ArchLucid.Host.Composition`, bind `IMcpRetrievalPort` â†’ the Retrieval implementation.
...
- No behavioural change â€” the Retrieval implementation is still wired at runtime; the port is a compile-time boundary only.
...
- Coordinate with TB-030 (Architecture.Tests gap closure) â€” the `Mcp_must_not_depend_on_*` assertions added in TB-030 should turn green when this item ships.

**Affected files / projects:**

- `ArchLucid.Contracts` or `ArchLucid.Application` â€” new `IMcpRetrievalPort.cs`
- `ArchLucid.Retrieval` â€” new `McpRetrievalPortAdapter.cs` (implements the port)
- `ArchLucid.Mcp/ArchLucid.Mcp.csproj` â€” swap `Retrieval` reference for port-defining assembly
- `ArchLucid.Mcp` source â€” replace concrete Retrieval usages with port calls
- `ArchLucid.Host.Composition` â€” bind `IMcpRetrievalPort`
- `ArchLucid.Architecture.Tests/DependencyConstraintTests.cs` â€” assertions (pair with TB-030)

**Size estimate:** **M** â€” ~1â€“2 eng days (port definition + Retrieval adapter + Mcp refactor + composition binding + tests).

---

## TB-033 â€” Agent execution trace â€” persist LLM sampling params + reasoning token count
...
- `temperature` (handler default **0.1** today â€” not stored)
- `maxTokens` / `max_completion_tokens`
- `top_p`, presence/frequency penalties (if ever enabled)

`ReasoningTokenCount` (or equivalent) is consumed when estimating cost in `LlmCompletionAccountingClient` / `LlmCostEstimator` but is **not** written to the trace row â€” only input/output token totals are stored.
...
3. Populate fields in `AgentExecutionTraceRecorder.RecordAsync` from `AgentCompletionModelMetadata` / completion result DTO â€” single source at record time.
4. Update OpenAPI/codegen if trace detail DTOs are customer-visible; extend `docs/library/AGENT_TRACE_FORENSICS.md` Â§Model metadata.
...
**Out of scope:** LLM tool-call loops (architecture is single-shot JSON completion today â€” no tool persistence layer).

**Depends on:** none (orthogonal to **TB-011** replay *scope* isolation â€” **INV-013**).
...
**Size estimate:** **XS** â€” ~2â€“4 h.

---

## TB-034 â€” Degraded-handler minimal `AgentExecutionTrace` rows
...
1. On degradation (before returning placeholder), record a **minimal** trace row: `FailureReasonCode` / `degradation_reason`, `AgentType`, `RunId`, `TaskId`, optional **truncated** system/user prompt hashes or template metadata (no full blob requirement if degradation happened pre-LLM â€” document which fields are best-effort).
...
- Do **not** block degradation return on trace insert failure â€” same best-effort contract as blob persistence (**TB-001** informational posture for secondary writes).
...
**Size estimate:** **S** â€” ~4â€“8 h.

---

## TB-035 â€” Persist intermediate LLM attempts on schema-remediation retries
...
Only the **final** attempt is passed to `IAgentExecutionTraceRecorder.RecordAsync`. Intermediate prompts and raw responses are discarded; `RecordAgentSchemaRemediationRetry` is a **metric counter only**. Support cannot answer â€œwhat did the model return on attempt 1 vs 3?â€ for schema drift incidents.

**What to do:**

1. **Option A (preferred):** Child trace rows per attempt â€” same `RunId`/`TaskId`, distinct `TraceId`, `AttemptIndex` column (migration), `ParentTraceId` nullable for final consolidated row OR final row references `AttemptCount`.
2. **Option B:** Append `RemediationAttempts[]` JSON array on a single trace row (bounded size; truncate with audit if exceeded).
3. Record each attemptâ€™s `RawResponse`, parse error, and token/cost slice when a completion occurred.
...
**Size estimate:** **M** â€” ~1â€“2 eng days.

---

## TB-036 â€” Correlate `DecisionProvenanceGraph` with `AgentExecutionTrace`

**Status:** Done (Batch F, 2026-05-26).

**Source:** Replay / provenance completeness audit (2026-05-26). `ArchLucid.Provenance` builds decision lineage; `AgentRuntime` stores LLM forensics â€” no link between them.

**Problem:**

`ProvenanceBuilder` / `GET â€¦/provenance` answer â€œwhich findings and decisions contributed to this run?â€ but not â€œwhich agent trace produced this decision narrative?â€ `AgentExecutionTrace` rows have `RunId` + `TaskId` + `AgentType` but no provenance node IDs. Cross-navigation requires manual correlation by timestamp and agent type.

**What to do:**

1. Product/engineering agree correlation grain: **per agent task** (`TaskId` + `AgentType`) vs **per decision key** vs **per finding id**.
2. Add stable correlation fields â€” e.g. `ProvenanceCorrelationId` on trace row; optional `AgentExecutionTraceId` on `ProvenanceNode.Metadata` for `Decision` / `Finding` nodes when builder can infer mapping.
...
**Size estimate:** **M** â€” ~2â€“3 eng days.

---

## TB-037 â€” Production write path for `DecisionProvenanceSnapshot`
...
`AuthorityQueryController` / provenance query paths invoke `ProvenanceBuilder` on demand. **`SaveAsync` has no production callers** â€” snapshots are never materialized. Every provenance read recomputes from findings, manifest, and decision trace artefacts (higher latency, harder point-in-time audit).
...
3. Wire invalidation when run artefacts are superseded (align with replay scope rules â€” **INV-013** / **TB-011**).
4. Metrics: `archlucid_provenance_snapshot_writes_total`, rebuild fallback counter.
5. Tests: commit â†’ snapshot exists; second read does not call builder when snapshot valid.

**Out of scope:** Changing graph semantics or node types.

**Affected files / projects:**

- `ArchLucid.Application` (or worker) â€” post-commit hook
...
## TB-038 â€” `RetrievalGroundingTrace` forensic enrichment (+ non-Compliance agents)
...
**Size estimate:** **Sâ€“M** â€” ~1â€“2 eng days (schema + writers + tests).

---

## TB-039 â€” Agent execute retry â€” per-`(RunId, TaskId)` skip before handler dispatch
...
When execute is retried after a partial batch (some handlers succeeded, run still `TasksGenerated`), the orchestrator calls `agentExecutor.ExecuteAsync` with the **full** task list. There is no per-task â€œalready executedâ€ gate. `AgentResultRepository.CreateManyAsync` delete-then-insert prevents duplicate SQL rows, but every handler is re-invoked and **LLM tokens are charged again** via `LlmCompletionAccountingClient` (accounting in `finally` on each successful `CompleteJsonAsync`). The completion cache (`CachingLlmCompletionClient`) deduplicates identical prompts only â€” not `(RunId, TaskId)` identity.
...
5. Tests: partial batch persisted â†’ retry executes only missing tasks; full batch idempotent early return unchanged; degraded placeholder does not skip unless product says so.
...
## TB-040 â€” `LlmCompletionAccountingClient` â€” await metering with `CancellationToken.None`
...
In `LlmCompletionAccountingClient.CompleteJsonAsync`, `TryRecordLlmUsageMeteringAsync` is invoked as fire-and-forget (`_ = â€¦`) with the **original** `cancellationToken`. When linked cancellation fires immediately after a successful LLM response, metering can be silently skipped while `_dailyTenantBudgetTracker` / `_monthlyDollarBudgetTracker` still record usage (they use `CancellationToken.None`). Retry then bills LLM again â€” budget ledgers and `IUsageMeteringService` event logs diverge.
...
4. Tests: simulate cancelled token after inner completion returns â€” assert metering `RecordAsync` still called once; budget and metering counts align.
...
## TB-041 â€” Authority pipeline â€” per-stage completion checkpoint on retry

**Status:** Done (Batch F, 2026-05-26).

**Source:** AgentRuntime determinism and idempotency audit (2026-05-26). `AuthorityPipelineStagesExecutor.ExecuteStageAsync` has no â€œstage already completedâ€ guard.

**Problem:**

If the authority pipeline throws mid-run (e.g. after context ingestion and graph save, before findings completes), a retry restarts from **stage 1** (`context_ingestion`). Each stageâ€™s `SaveAsync` is insert-oriented; cross-stage work is not atomically rolled back. Retries can produce duplicate context snapshots, duplicate findings snapshots, and duplicate connector fetches â€” even when earlier stage outputs are already durable on `RunRecord` (`ContextSnapshotId`, `GraphSnapshotId`, etc.).

**What to do:**

1. Define stage completion predicates from persisted run header + snapshot FKs (e.g. `ContextSnapshotId` set â‡’ skip `context_ingestion`; `GraphSnapshotId` set â‡’ skip `graph` unless fingerprint changed).
...
5. Integration tests: fail after graph stage â†’ retry skips ingestion + graph, continues at findings.
...
## TB-042 â€” Graph snapshot supersession â€” skip rebuild when `RunRecord.GraphSnapshotId` set
...
If `SaveAsync` succeeds but `UpdateRunAsync` fails (transient SQL), retry builds and saves a **second** graph snapshot. When Cosmos graph storage is enabled, graph save runs **outside** the SQL authority transaction â€” orphaned snapshots are not rolled back. Run header eventually points at the latest id; earlier snapshots remain as orphans (storage + lineage noise).

**What to do:**

1. At graph stage entry: if `run.GraphSnapshotId` is non-null and load-by-id succeeds, reuse that snapshot (set `ctx.GraphSnapshot`, resolution mode `reused_from_run_header`) â€” skip `GraphSnapshotReuseEvaluator` rebuild/save.
...
3. For Cosmos path: write run header pointer only after successful blob/document save, or implement compensating delete of orphan on header update failure (product choice â€” document).
4. Align with **TB-041** checkpoint rules.
5. Tests: simulate failure after `SaveAsync` before `UpdateRunAsync` â†’ retry does not create second snapshot id.
...
## TB-043 â€” Schema remediation â€” non-retried completion client (decouple from Polly stack)
...
Maximum billed LLM calls per handler task scales as **`MaxCompletionAttempts Ã— (1 + LlmCallMaxRetryAttempts)`** (e.g. 3 Ã— 4 = 12). Each Polly retry that reaches Azure and returns usage is charged in `LlmCompletionAccountingClient`. Remediation attempts use **different user prompts** (remediation text appended), so completion cache does not dedupe across attempts. This is intentional retry behaviour for reliability but unbounded for FinOps unless capped.

**What to do:**

1. Register a dedicated **`IAgentCompletionClient`** for remediation attempts (same Azure endpoint, **no** Polly retry wrapper â€” or max 1 attempt) and pass as `remediationCompletionClient` into `LlmAgentSchemaCompletion.CompleteAsync` (parameter already exists).
...
4. Tests: schema violation triggers remediation â†’ assert Polly retry count applies only to first attempt; token accounting call count bounded.
5. Coordinate with **TB-035** if persisting intermediate attempts (forensics) â€” billing and trace rows should align per attempt.
...
**Size estimate:** **XSâ€“S** â€” ~4â€“8 h.

---

## TB-044 â€” `AgentExecutionTraces` â€” unique index on `(RunId, TaskId, AgentType)` + upsert semantics
...
Full execute retry (see **TB-039**) appends additional trace rows for the same logical agent task. Forensics queries (`GetByTaskIdAsync`) return multiple rows ordered by `CreatedUtc` â€” ambiguous â€œcanonicalâ€ trace for support. Not a direct double-charge risk, but violates one-trace-per-task-per-run expectation and complicates cost aggregation.

**What to do:**

1. DbUp migration + consolidated `Scripts/ArchLucid.sql`: unique index on `(RunId, TaskId, AgentType)` (confirm cardinality â€” one row per agent type per task per run).
2. Change `CreateAsync` to **MERGE** or delete-then-insert for that key (mirror `AgentResultRepository` pattern), or skip insert when row exists unless **TB-035** multi-attempt model requires child rows (if so, use `(RunId, TaskId, AgentType, AttemptIndex)` unique key instead â€” coordinate with **TB-035**).
3. Backfill / dedupe strategy for existing duplicates (keep latest `CreatedUtc` per key).
4. Tests: retry execute â†’ single trace row per task (or explicit attempt index set if **TB-035** shipped first).
...
## TB-045 â€” Embedding model identity and drift guard
...
3. Startup compare config vs index metadata â†’ full re-embed on change.
...
**Size estimate:** **Sâ€“M** â€” ~1â€“2 eng days.

---

## TB-046 â€” Index freshness, ContentHash skip, and indexer observability
...
**What to do:** See **RAG-V1-008**. Priority within item: **ContentHash skip (P0)** â†’ last-indexed-at + health signal â†’ optional scheduled re-index.
...
## TB-047 â€” Chunking strategy fingerprint and invalidation
...
**Size estimate:** **S** â€” ~1 eng day.

---

## TB-048 â€” Tenancy isolation hardening (retrieval)

**Status:** Done (Batch G, 2026-05-26). **Remaining gap:** production Azure Search client + delete path â€” see **TB-071** / **RAG-V1-010** P1.
...
**What to do:** See **RAG-V1-010**. **P0:** safe default on null assignment list + integration test. **P1:** Azure Search tenant `$filter` when client ships â€” tracked as **TB-071**.

**Schedule under:** **TB-021** â€” treat as **security** item; pick up before broad MCP retrieval exposure (**TB-032**).
...
## TB-049 â€” Retrieval IR eval harness (recall@k, MRR)
...
**Schedule under:** **TB-021** alongside **RAG-V1-005** â€” complementary gates (retrieval quality vs output faithfulness).
...
## TB-050 â€” Manifest `ResolvedArchitectureDecision` â€” confidence + `ConfidenceSource`
...
`ResolvedArchitectureDecision` (`ArchLucid.Core/Manifest/ResolvedArchitectureDecision.cs`) exposes category, title, selected option, rationale, and supporting finding IDs â€” but **no numeric confidence** and no indication of whether confidence was measured, defaulted, or absent. Finding-level `ConfidenceScore` / `EvaluationConfidenceScore` may exist on upstream findings yet are **not projected** onto the decision row an operator reads in the manifest or governance UI. This is the highest-impact explainability gap for the authority production path.

**What to do:**

1. Add nullable `double? Confidence` and `DecisionConfidenceSource` (or string enum) to `ResolvedArchitectureDecision` and contract DTOs â€” values such as `RuleEngine`, `FindingAggregate`, `LlmAgent`, `Calibrated`, `Unknown`, `NotComputed`.
2. Populate in `DefaultGoldenManifestBuilder` from the winning findingâ€™s `EvaluationConfidenceScore` (preferred) or `ConfidenceScore`, with explicit `Unknown` when both are null â€” **never** silently substitute a constant.
3. OpenAPI snapshot + codegen per [`API_CONTRACTS.md`](API_CONTRACTS.md); UI types if manifest decision surfaces expose the field.
4. Tests: accepted finding with score â†’ decision carries score + source; null scores â†’ `Unknown` not `0`.
...
## TB-051 â€” Decisioning V2 merge â€” consume `CalibratedConfidence`
...
Operators and replay tooling cannot distinguish calibrated uncertainty from raw model self-report or strategy literals. Calibrated values are **dead data** in the decisioning path â€” a silent loss of uncertainty signal after calibration runs.
...
3. Replace or gate hardcoded `BaseConfidence` literals â€” either derive from evaluation evidence or mark `strategyPrior` explicitly in trace metadata so operators know the score is not measured.
4. Tests: calibrated present â†’ merge uses it; absent â†’ raw; neither â†’ `Unknown` / explicit prior with source label.
...
- `ArchLucid.AgentRuntime` (read-only â€” calibration already exists)
...
## TB-052 â€” `RuleAuditTracePayload` â€” snapshot IDs + prompt refs
...
2. Add bounded `PromptRefs` collection (template id + version + optional agent type) aggregated from accepted findingsâ€™ `PromptTemplateId` / `PromptTemplateVersion`.
...
5. Tests: authority run â†’ trace contains snapshot IDs; LLM finding with prompt refs â†’ refs appear on trace.
...
## TB-053 â€” `FindingConfidenceCalculator` â€” typed unknown/failed (no bare catch)
...
- `FindingConfidenceCalculator.cs` (~line 47): null `traceCompletenessRatio` is treated as **`0.0`**, not â€œunknownâ€ â€” depresses scores as if the trace were empty.
- `FindingConfidenceCalculator.cs` (~lines 66â€“68): bare `catch { return null; }` swallows arithmetic failures; callers cannot distinguish **not computed** vs **failed**.
- `FindingFactory.CreateFromAgentArchitectureFinding` (~line 178): `ConfidenceScore ?? agentResult.Confidence` coerces null to agent aggregate â€” drops explicit unknown.
...
3. Map null completeness ratio â†’ `Unknown`, not `0.0`; document contract on `IFindingConfidenceCalculator`.
...
## TB-054 â€” Unified run decision explainability API (authority audit + V2 nodes)
...
Operators hitting `GET â€¦/decisions` or trace endpoints see **either** rule-audit semantics **or** V2 weighted nodes â€” not a single explainability view per `RunId`. `DecisionTraceManifestAttachment` wires run-event trace IDs (merge/replay path) only. Provenance (**TB-036**) links graph nodes to agent traces but does not unify the two decision pipelines.
...
5. Integration test: authority run â†’ single response contains audit + materialized V2 nodes when applicable.

**Out of scope:** Merging the two pipelines into one engine (architectural); **TB-029** notifications decoupling.

**Depends on:** **TB-050**â€“**TB-052** for field completeness; **TB-036** for trace deep links.

**Affected files / projects:**

- `ArchLucid.Application` run/decision query services
- `ArchLucid.Api` controllers + response models
- `ArchLucid.Application/Runs/â€¦/AuthorityDrivenArchitectureRunCommitOrchestrator.cs` (materialization timing docs)
...
## TB-055 â€” Propagate `AgentResult.ReasoningTrace` into `Finding` explainability
...
1. Add optional `ReasoningTrace` (bounded length) to `ExplainabilityTrace` or `Finding` contract â€” truncate with hash reference to blob if over limit.
...
4. Tests: agent finding â†’ trace contains reasoning substring; over-limit â†’ truncated + stable hash id.

**Out of scope:** Storing full prompt/response blobs (already on `AgentExecutionTrace` â€” **TB-033**, **TB-034**).
...
## TB-056 â€” Decisioning partial-failure surfacing + sentinel trace inflation guard
...
| Location | Behaviour |
|----------|-----------|
| `FindingsOrchestrator.cs` (~88â€“105) | Per-engine `catch` â†’ log + `FindingEngineFailure` + **continue**; partial snapshot saved with no manifest-level summary |
| `FindingsSnapshotEvaluationConfidenceEnricher.cs` (~107â€“115) | Enrichment failure â†’ warning only; snapshot lacks `EvaluationConfidenceScore` |
| `NullFindingsSnapshotEvaluationConfidenceEnricher` | No-op registered via `TryAdd` in lightweight hosts â€” indistinguishable from â€œnot runâ€ |
...
| `DefaultGoldenManifestBuilder` | `payload is null` â†’ `continue` â€” finding omitted from manifest section silently |
...
3. Register explicit â€œenricher skippedâ€ flag on findings snapshot metadata when `NullFindingsSnapshotEvaluationConfidenceEnricher` is active (host profile), not silent no-op.
4. Exclude sentinel `AlternativePathsConsidered` from `ExplainabilityTraceCompletenessAnalyzer` ratio (or remove sentinel â€” prefer real empty + `Unknown` in **TB-053**).
...
- `ArchLucid.Host.Composition/â€¦/ServiceCollectionExtensions.CorePersistencePortCompatibility.cs`

**Size estimate:** **Sâ€“M** â€” ~1â€“2 eng days.

---

## TB-057 â€” Architecture risk register framing over governance findings
...
**Size estimate:** **Sâ€“M** â€” mostly UI/query projection if **TB-058** supplies disposition metadata.

---

## TB-058 â€” Finding disposition workflow API + UI
...
**Size estimate:** **M** â€” API, persistence, UI, audit, and tests.

---

## TB-059 â€” First-class waiver / exception records
...
**Size estimate:** **M** â€” new governed object, but can reuse approval/audit patterns.

---

## TB-060 â€” Review-package decision register consolidation
...
**Size estimate:** **S** â€” mostly query/view consolidation if existing lineage endpoints are sufficient.

---

## TB-061 â€” Decision-needed governance digest
...
**Size estimate:** **Sâ€“M** â€” projection and template work over existing data.

---

## TB-062 â€” Executive dashboard live KPI replacement
...
**Size estimate:** **S** â€” UI cleanup plus data-source alignment.

---

## TB-063 â€” ITSM one-click issue creation from findings â€” **V1.1**

**Source:** Commercial stickiness review (2026-05-27) plus owner scope: first-party Jira / ServiceNow productization is **V1.1**, not V1 GA. See [`V1_SCOPE.md`](V1_SCOPE.md) Â§2.13 and [`V1_DEFERRED.md`](V1_DEFERRED.md) Â§6.
...
- `docs/library/V1_SCOPE.md` Â§2.13 if scope changes

**Size estimate:** **M** â€” UI productization over existing backend, plus sync-state display and tests.

---

## TB-064 â€” System catalog consolidated DDL (`ArchLucid.System.sql`)
...
1. Author a full idempotent consolidated `ArchLucid.Persistence/Scripts/ArchLucid.System.sql` mirroring the tenant pattern (`IF OBJECT_ID â€¦ IS NULL` + inline indexes).
2. Keep `Migrations/System/001â€“003` as the authoritative DbUp upgrade path for brownfield; update consolidated DDL whenever those migrations change.
3. Wire `SqlSchemaBootstrapper` (or a dedicated system bootstrapper) into the system-catalog startup path after `DatabaseMigrator.RunSystem`, matching tenant **DbUp-first â†’ bootstrap** order documented in [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§1.
4. Update [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§2 inventory and the schema-change checklist in Â§5.
...
## TB-065 â€” MigrateVerify â€” deployed schema vs DDL drift detection
...
3. Optionally: provision two empty catalogs â€” DbUp-only vs DbUp+bootstrap â€” and assert zero structural drift between them (closes the two-pathway gap in [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§1).
4. Extend Tier 1.5 GitHub Actions job (or add a sibling job) to run the drift check after existing MigrateVerify.
5. Document operator interpretation in [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§6 troubleshooting.
...
## TB-066 â€” CI gate â€” `ArchLucid_Unified_Schema.sql` matches generator output
...
3. Document regenerate command in [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§2 and schema-change checklist Â§5.
...
## TB-067 â€” `SQL_SCRIPTS.md` migration catalog â€” backfill 051â€“227 + automation

**Source:** DDL hygiene and migration-safety audit (2026-05-27). Â§4.2 catalog stops around migration ~050 plus a handful of later entries; migrations **051â€“227** are largely undocumented.

**Problem:**

Operators and contributors cannot rely on [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§4.2 for deploy history or intent. The schema-change checklist requires updating Â§4.2 for every migration, but enforcement has lapsed (~170 migrations missing).

**What to do:**

1. Backfill Â§4.2 with one-line summaries for migrations **051â€“227** (parse migration file headers where present; otherwise derive from filename and first comment block).
2. Add a CI script that fails when the highest `Migrations/NNN_*.sql` number exceeds the highest documented entry in Â§4.2 (or when a new forward migration lands without a catalog line).
...
- Â§4.2 documents every forward migration through the current highest number.
...
**Size estimate:** **S** â€” ~4â€“8 h for backfill + gate.

---

## TB-068 â€” DbUp migration rolling-deploy guardrails (CI lint + runbook)
...
| Migration | Risk |
|-----------|------|
| **215** `ScopeColumnsNotNull` | `ALTER NOT NULL` after backfill â€” old app writing NULL fails |
| **223** `AgentExecutionTraces_RunTaskAgentType_Unique` | Deletes duplicates then `CREATE UNIQUE INDEX` â€” old app can recreate duplicates |
| **214** / **116** | `CHECK` constraints reject legacy values |
| **216** | Drops old unique index before adding filtered replacement â€” uniqueness gap |

There is no CI lint forbidding these patterns in new migrations and no operator runbook for coordinated deploy order.

**What to do:**

1. Author `docs/runbooks/ROLLING_DEPLOY_MIGRATIONS.md` with required patterns: nullable add â†’ backfill â†’ NOT NULL; `WITH NOCHECK` for CHECK/FK; add-new-index-before-drop-old; deploy app before enforcing UNIQUE.
2. Add CI static analysis over new/changed `Migrations/NNN_*.sql` files flagging: bare `ALTER COLUMN â€¦ NOT NULL` without prior nullable-add migration in same PR; `DELETE` before `CREATE UNIQUE`; `DROP INDEX` before replacement `CREATE UNIQUE` in same script.
3. Annotate known historical migrations in Â§4.2 (via **TB-067**) with **rolling-deploy: coordinated** tags.
...
- Runbook linked from [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§5 checklist and [`MIGRATION_ROLLBACK.md`](../runbooks/MIGRATION_ROLLBACK.md).
...
## TB-069 â€” Simplify `GreenfieldBaselineMigrationRunner` sparse-stamp path

**Source:** DDL hygiene and migration-safety audit (2026-05-27). Baseline runner stamps migrations **001â€“050** into `SchemaVersions` without always executing them, with complex drift-repair branches.
...
1. Document current runner behavior and all drift branches in [`SQL_SCRIPTS.md`](SQL_SCRIPTS.md) Â§4.0 with a sequence diagram.
2. Evaluate replacing multi-branch stamp logic with a single idempotent migration that records **001â€“050** as applied when sentinel tenant tables exist and journal is empty/inconsistent â€” without re-executing DDL that would duplicate objects.
...
**Size estimate:** **M** â€” ~2â€“3 eng days (refactor + test matrix).

**Depends on:** **TB-065** recommended before simplifying stamp paths.

---

## TB-070 â€” `PersistenceContractSupplement.sql` stale refs + test catalog parity
...
**Size estimate:** **XS** â€” ~1â€“2 h.

---

## TB-071 â€” Azure Search production client â€” wire tenant OData filter on every search/delete

**Status (2026-05-31):** **Done** â€” `AzureSearchSdkClient` applies `AzureSearchTenantScopeFilterBuilder` on search/delete; registered when `Retrieval:AzureSearch:Endpoint` is set; `AzureSearchTenantScopeFilterBuilderTests`.
...
## TB-072 â€” Scope-to-identity binding at API ingress

**Status (2026-05-31):** **Done** â€” `ScopeIdentityBindingMiddleware` + `ScopeIdentityBindingValidator`; ApiKey scope claims; `ScopeIdentityBindingIntegrationTests` / `ScopeIdentityBindingValidatorTests`.

**Source:** Multi-tenancy and blast-radius audit (2026-05-27). `HttpScopeContextProvider` resolves tenant from JWT claims â†’ `x-*` headers â†’ dev defaults, but **no middleware validates that the authenticated principal may use the resolved tenant**.

**Problem:**

- **`ApiKeyAuthenticationHandler`** â€” roles/permissions only; zero scope claims. Valid key + arbitrary `x-tenant-id` â‡’ any tenant's data (especially in SingleCatalog mode).
- **`DevelopmentBypassAuthenticationHandler`** â€” no `tenant_id` / `workspace_id` / `project_id` claims; headers fully control scope.
- **SCIM bearer** â€” `tenant_id` only; workspace/project fall through to headers/defaults.
- **`TenantOrProjectCapabilityAuthorizationHandler`** â€” augments project capabilities but does not prove caller âˆˆ `scope.TenantId` for tenant-wide JWT roles.

Production safety currently depends on per-tenant catalog routing (`ScopedRoutingSqlConnectionFactory`) plus repository SQL filters â€” not on identity binding.
...
2. **ApiKey:** Embed scope in key record or issue per-tenant keys; derive scope server-side from key metadata â€” never trust client headers alone.
...
4. Optional middleware after auth: `scope.TenantId != Guid.Empty` for tenant APIs; SCIM user âˆˆ tenant for non-platform roles.
...
## TB-073 â€” Scoped snapshot repository reads (findings / graph / context)

**Status (2026-05-31):** **Done** â€” scoped repository reads + relational filters; `SqlFindingsSnapshotRepositoryScopeIsolationSqlIntegrationTests`; `ScopedSnapshotReadIdorIntegrationTests`.
...
|------------|--------|------|
| `SqlFindingsSnapshotRepository` | `GetByIdAsync`, `ListFindingRecordsKeysetAsync`, `UpdatePriorityRanksAsync` | Leaked `FindingsSnapshotId` â†’ read/mutate another tenant's findings |
...
| `SqlGraphSnapshotRepository` | `GetByIdAsync` | Leaked `GraphSnapshotId` â†’ entire knowledge graph |
| `SqlContextSnapshotRepository` | `GetByIdAsync` | Leaked `ContextSnapshotId` â†’ context snapshot |
| `InMemoryFindingsSnapshotRepository` | `GetByIdAsync` | Global `Dictionary<Guid, string>` â€” no tenant key |

Safe in **per-tenant catalog** mode only. Vulnerable in **SingleCatalog** / dev / test when a GUID is known. `DapperAuthorityQueryService` loads snapshots by bare GUID after a scoped run gate â€” indirect risk if run row is corrupt or repository called elsewhere.

**What to do:**

1. Add `ScopeContext` parameter to `IFindingsSnapshotRepository.GetByIdAsync`, `ListFindingRecordsKeysetAsync`, `UpdatePriorityRanksAsync` â€” mirror `SqlGoldenManifestRepository` (`WHERE TenantId = @TenantId AND WorkspaceId = @WorkspaceId AND ProjectId = @ProjectId`).
...
## TB-074 â€” Retrieval indexing write-path tenant validation

**Status (2026-05-31):** **Done** â€” `RetrievalIndexingScopeValidator.ValidateDocuments` in `RetrievalIndexingService`; `InMemoryVectorIndex.RemoveChunksForDocumentAsync` scopes delete by tenant/workspace/project; `RetrievalIndexingScopeValidatorTests`.
...
4. Tests: document with mismatched `TenantId` â†’ rejected; matching scope â†’ indexed.

**Out of scope:** Per-tenant index partitioning; platform corpora indexing (`PlatformSentinelTenantId` â€” by design).
...
## TB-075 â€” Operator UI server-side scope (proxy + SSR)

**Status (2026-05-31):** **Done (V1 posture)** â€” `resolveProxyUpstreamScopeHeaders` strips client scope in production-like mode; env `ARCHLUCID_PROXY_*` trusted scope; `proxy-scope-resolution.test.ts`; sponsor value report resolves tenant from `/api/auth/me`. Full Entra cookie-bound SSR scope remains paired with **TB-072**.
...
- **`proxy/[...path]/route.ts`** â€” forwards browser `x-tenant-id` / `x-workspace-id` / `x-project-id` when present; `localStorage` (`archlucid_operator_scope_v1`) is the authoritative tenant source in the browser.
- **`scope.ts` `getScopeHeaders()`** â€” hardcoded `DEV_SCOPE_*` GUIDs for all SSR requests.
- **`middleware.ts`** â€” no auth or tenant gate (demo alias redirects only).
- **`downloadValueReportDocx`** â€” `tenantId` in URL path can disagree with scope headers.
...
## TB-076 â€” Run-child SQL scope predicates + in-memory repository tenant keys
...
Residual risk in **SingleCatalog** / dev if run gate is skipped or connection targets wrong catalog. `TenantErasureQuarantineMiddleware` skips `/v1/admin` entirely â€” quarantined tenants with Admin credentials still reach admin routes.
...
3. Audit callers of `GetByRunIdAdminAsync` (`FindingPriorityReranker`, archival jobs) â€” ensure system/background context only.
...
## TB-077 â€” Operator UI resource ownership checks + governance mutation hardening
...
- Routes `[runId]`, `[manifestId]`, `[findingId]` call APIs with ID alone â€” URL manipulation is IDOR if API returns cross-scope data.
- `recordFindingDisposition(findingId, â€¦)` â€” `findingId` in URL; `runId` optional in body.
- `getArchitectureDecisionRegister()` â€” no `projectId` from active scope.
- `compareRuns(leftRunId, rightRunId)` â€” two run IDs, no scope validation in UI.
- SSR `/reviews?projectId=â€¦` can mismatch hardcoded `x-project-id` on server renders.

**What to do:**

1. After loading run/manifest/finding detail, compare resource `projectId` (and `tenantId` if API returns it) to effective scope; call `notFound()` on mismatch (defense in depth â€” API remains authoritative).
...
**Depends on:** Ships alongside **TB-071**â€“**TB-077** (tests added per item; this item tracks the consolidated matrix and CI wiring).
...
**Size estimate:** **S** â€” ~1 eng day (incremental; spread across sibling items).
...
## TB-079 â€” ADO PR markdown â€” sanitize `SummaryHighlights` and deep-link fields

**Status (2026-05-31):** **Done** â€” `AdoPullRequestMarkdownEscaper` (bullet escape, dangerous-content rejection, length cap, https/http link targets) used by compare + run-summary formatters; `AdoPullRequestMarkdownEscaperTests` / `AzureDevOpsRunSummaryMarkdownTests`.

**Source:** Secrets, identity, and tool-sandboxing audit (2026-05-27).

**Problem:**

`GoldenManifestCompareMarkdownFormatter.cs` (L34â€“36) echoes `SummaryHighlights` from the compare response verbatim into the Azure DevOps PR comment body. `AzureDevOpsRunSummaryMarkdown` similarly embeds `operatorRunDeepLink` into a Markdown link without format validation. If compare metadata or run configuration is poisoned (e.g. via a malicious architecture run, a compromised evidence package, or a supply-chain attack on the compare endpoint), arbitrary Markdown â€” including HTML rendered in ADO's PR UI â€” can appear in PR comment threads visible to all reviewers.
...
## TB-080 â€” Azure OpenAI â€” migrate from `ApiKey` config key to `DefaultAzureCredential`

**Status (2026-05-31):** **Done** â€” `AzureOpenAI:AuthenticationMode=ManagedIdentity` registers completion, embedding, and semantic-judge clients via `DefaultAzureCredential`; `AzureOpenAiConfigurationProbe` + `CriticalConfigurationValidator` skip ApiKey when MI is set; `ProductionLikeSecretTransportConfigurationLint` advises on plaintext keys; `appsettings.KeyVault.sample.json` documents MI-first posture. Content Safety remains ApiKey/KV until a managed-identity guard ships.
...
## TB-081 â€” `ArchLucidApiKey` â€” production safety rule: require Key Vault reference

**Status (2026-05-31):** **Done** â€” `ProductionSafetyRules.CollectAzureDevOpsArchLucidApiKeyKeyVaultReference`; `ProductionSecretSourceRulesTests`.
...
## TB-082 â€” Agent `AllowedTools` â€” runtime enforcement at handler dispatch

**Status (2026-05-31):** **Done** â€” `AgentTaskAllowedToolsDispatchGuard` in `RealAgentExecutorSingleHandlerExecution`; `AgentToolNotAllowedException`; `AgentTaskAllowedToolsDispatchGuardTests`.
...
## TB-083 â€” Service Bus â€” production safety rule: require namespace FQDN, disallow raw connection string

**Status (2026-05-31):** **Done** â€” `ProductionSafetyRules.CollectIntegrationEventsServiceBusConnectionStringKeyVaultReference`; `ProductionSecretSourceRulesTests`.
...
## TB-084 â€” AzureExtractor â€” validate `SubscriptionId` as GUID before ARM URL construction

**Status (2026-05-31):** **Done** â€” `HostedAzureExtractorGuidValidator` on collection request + ARM list path; `HostedAzureExtractorGuidValidatorTests`.
...
## TB-085 â€” SqlRelationalBackfill â€” paged entity scans + durable checkpoint table
...
**Size estimate:** **M** â€” ~1â€“2 days.

---

## TB-086 â€” SqlRelationalBackfill â€” poison-row quarantine (`BackfillFailures` + `--max-retries`)
...
## TB-087 â€” Findings backfill slice â€” DB-level idempotency (remove double COUNT race)
...
2. Add a DbUp migration + consolidated DDL constraint preventing duplicate slice materialization â€” prefer `UNIQUE` on the natural child key or `MERGE`/upsert semantics in `InsertFindingRecordsRelationalAsync`.
...
**Size estimate:** **XSâ€“S** â€” ~2â€“4 h.

---

## TB-088 â€” Container App jobs â€” per-entity error isolation in multi-tenant loops
...
## TB-089 â€” Digest delivery scanners â€” record delivery before send (ACA retry idempotency)
...
- `ArchLucid.Application/` (digest scanner implementations â€” locate via `ExecDigestWeeklyDeliveryScanner`, `WeeklyExecutiveSummaryDeliveryScanner`)
...
## TB-090 â€” Backfill.Cli â€” `--output-json` report + per-stage timing

**Status (2026-05-31):** **Done** â€” `SqlRelationalBackfillStageRunner` records per-stage `ElapsedMilliseconds` and delta counts; `--output-json [path]` writes `archlucid.backfill.cli.report.v1` to stdout or file; documented in `SqlRelationalBackfill.md`; `BackfillCliJsonReportSerializerTests`, `SqlRelationalBackfillStageRunnerTests`.
...
**Status (2026-06-01):** **Done** â€” `azurerm_private_dns_zone` + VNet link + `azurerm_private_endpoint` for `privatelink.vaultcore.azure.net` when `enable_private_data_plane` and `key_vault_id` are set; `key_vault_private_endpoint_id` output; ARM ID check in `checks.tf`; documented in `terraform.tfvars.example` and `README.md`.
...
## TB-103 â€” Orphan candidate count + savings â€” expose via backend API; remove UI heuristic parser

**Status (2026-05-31):** **Done** â€” `ExecutiveOrphanCandidateKpiCalculator` + `orphanCandidates` on `ExecutiveRoiSummaryResponse`; `ExecutiveOrphanCandidatesCard` reads API only; orphan heuristics removed from `run-potential-savings-parser.ts` (run detail uses cost artifact only).
...
2. Expose the new field on `GET /v1/roi/executive-summary` response (`ExecutiveRoiSummaryResponse`) â€” or as a dedicated `GET /v1/roi/orphan-candidate-summary` endpoint if the data source is a separate analysis pipeline.
...
**Cross-ref:** **TB-062** (executive dashboard live KPI replacement â€” this item is a scoped sub-task).

**Size estimate:** **M** â€” ~1â€“2 days (backend query + contract + API surface + UI replacement + tests).

---

## TB-104 â€” 14-day expiring waiver KPI â€” server-compute the window; remove client-side date rule

**Status (2026-05-31):** **Done** â€” `GovernanceWaiverExpiryWindow` + `ExpiringWaiversCount14Days` on ROI; dashboard uses `waiversExpiringWithin14Days`; `ExecutiveReviewPacketPortfolioSignalsFactory` maps ROI expiring count (not `ActiveWaiversCount`).
...
2. Add `ExpiringWaiversCount14Days: int` to `ExecutiveRoiSummaryResponse` (it may already exist as a stub â€” verify and populate it).
...
**Cross-ref:** **TB-062** (executive dashboard live KPI replacement); **TB-057** (governance stickiness review packet); **TB-149** (canonical window â€” supersedes duplicate `CountExpiringWaivers` logic); **TB-155** (cached ROI vs live decisions-needed).

**Size estimate:** **S** â€” ~4â€“6 h (backend field population + UI simplification + tests).

---

## TB-105 â€” Business-impact category buckets â€” add pre-bucketed counts to `ExecutiveRoiSummaryResponse`; remove substring matcher

**Status (2026-05-31):** **Done** â€” six pillars (`Security`, `Compliance`, `Reliability`, `Cost`, `Governance`, `Other`) via `ExecutiveBusinessImpactPillarMatchers` + classifier; widget + `StaleArchitectureRiskCount` on ROI aligned with decisions-needed and review packet.
...
- A backend category rename (e.g., `"resiliency"` â†’ `"resilience"`) silently zeroes the reliability bucket without a compilation error.
...
- `ArchLucid.Core` or `ArchLucid.Contracts` (category â†’ pillar mapping constant)
- `archlucid-ui/src/components/BusinessImpactSummaryWidget.tsx`
- `ArchLucid.Application.Tests/Roi/` (new unit tests)

**Cross-ref:** **TB-062** (executive dashboard live KPI replacement); **TB-103** (orphan savings â€” same root cause pattern).

**Size estimate:** **S** â€” ~4â€“8 h (backend aggregation + contract change + UI simplification + tests).

---

## TB-149 â€” Canonical 14-day expiring-waiver window â€” single server implementation

**Status (2026-05-31):** **Done** â€” `GovernanceWaiverExpiryWindow.CountExpiringWithinDays` (`[now, now+14d]` UTC inclusive); `ExecutiveRoiSummaryService.CountExpiringWaivers` removed; composer + ROI share helper; unit tests in `GovernanceWaiverExpiryWindowTests`.

**Source:** Cross-layer data consistency audit (2026-05-31). Extends **TB-104**.

**Problem:**

Two server paths compute â€œwaivers expiring within 14 daysâ€ with **different predicates**:

- `ExecutiveRoiSummaryService.CountExpiringWaivers` â€” `ExpiresAtUtc <= now.AddDays(14)` with **no lower bound** (includes already-expired rows that missed `MarkExpiredAsync`).
- `GovernanceDigestDecisionNeededComposer.BuildSummaryAsync` â€” `ExpiresAtUtc >= now && ExpiresAtUtc <= now.AddDays(14)` (correct inclusive window).
...
3. Update `ExecutiveRoiDashboardLiveKpiCards.tsx` to read **only** `decisionsNeeded.waiversExpiringWithin14Days` or a single ROI field sourced from the same helper â€” remove dual-source `??` when both are populated.
...
**Size estimate:** **S** (~4â€“6 h).

---

## TB-150 â€” Decisions-needed `TotalDecisionItems` â€” union cardinality, not sum

**Status (2026-05-31):** **Done** â€” `GovernanceDecisionsNeededSummaryCalculator.ComputeTotalDecisionItems` (distinct finding union + pending approvals); contract XML on `TotalDecisionItems`; unit tests.
...
4. Add unit tests: one finding in two buckets â†’ total increments by 1, not 2.
...
## TB-151 â€” `ExecutiveSummaryResult.TotalRiskReductionScore` â€” semantic fix

**Status (2026-05-31):** **Done** â€” `TotalRiskReductionScore` maps to `ResolvedFindingsCount30Days`; added `PendingGovernanceDecisionCount` on `ExecutiveSummaryResult`; `ExecutiveReportsSummaryServiceTests`.
...
1. **Rename** to `PendingGovernanceDecisionCount` (breaking â€” coordinate OpenAPI snapshot + consumers), or
...
- No field name implies â€œreductionâ€ while monotonically increasing with outstanding decisions.
...
**Size estimate:** **XS** (~2â€“3 h).

---

## TB-152 â€” `ExecutiveSummaryResult.CostWasteUsd` â€” stop aliasing savings
...
## TB-153 â€” Recurring architecture review trigger â€” idempotency before execute

**Status (2026-05-31):** **Done** â€” `RecurringArchitectureReviewTriggerService` persists `LastTriggeredRunId` + advanced `NextRunUtc` immediately after `CreateRunAsync`, before `ExecuteRunAsync`; failure before checkpoint still advances `NextRunUtc` in catch. Unit test asserts `create` â†’ `update` â†’ `execute` order.

**Source:** Cross-layer data consistency audit (2026-05-31). Cross-ref **TB-062**, **TB-012** (**INV-009**).

**Problem:**

`RecurringArchitectureReviewTriggerService.TriggerScheduleAsync` order: `CreateRunAsync` â†’ `ExecuteRunAsync` â†’ `scheduleRepository.UpdateAsync` (advance `NextRunUtc`). If the host process dies after create but before update, the next poll treats the schedule as still due and creates a **second run** for the same period. Explicit exceptions advance `NextRunUtc` in the catch block; **OOM / ACA eviction** does not.
...
## TB-154 â€” Waiver â†” disposition state machine â€” bidirectional invariants

**Status (2026-05-31):** **Done (V1 strict create/renew)** â€” `RiskExceptionDispositionGuard` blocks waivers when latest disposition is `Remediated`; `ArchitectureRiskRegisterStaleEvaluator` in Core suppresses stale signal under active waiver; register reader uses shared evaluator. Expiry disposition events remain V1.1 optional.
...
- **C:** Risk register `IsStale` ignores active waiver â€” same finding contributes to stale-risk and expiring-waiver buckets (**TB-150** makes this worse).
...
4. Integration tests for scenarios Aâ€“C.

**Acceptance criteria:**

- No finding appears as both â€œstale riskâ€ and â€œcovered by active waiverâ€ in `BuildSummaryAsync` for the same snapshot.
...
## TB-155 â€” ROI cache vs live decisions-needed â€” canonical expiring-waiver source

**Status (2026-05-31):** **Done** â€” `CachingExecutiveRoiSummaryService` refreshes `ExpiringWaiversCount14Days` on every read; dashboard tile uses `waiversExpiringWithin14Days` only (no ROI `??` fallback).
...
3. Dashboard: single source â€” `waiversExpiringWithin14Days` only.

**Acceptance criteria:**

- Create waiver â†’ refresh dashboard within one request cycle: expiring count matches decisions-needed without waiting for ROI TTL.
...
## TB-156 â€” `start-local-api-and-ui.ps1` â€” strict preflight + UI proxy E2E gate

**Status (2026-05-31):** **Done** â€” preflight (toolchain, node_modules, port/env alignment), `/health/live` + `/health/ready`, UI root, blocking `GET /api/proxy/health/live` before browser; `-SkipPreflight`, `-EnsureSql`.

**Priority:** **P0** â€” pick up in the next available thread (local dev blocker; misleads every contributor who starts UI without a healthy API chain).
...
`scripts/start-local-api-and-ui.ps1` waits for `GET /health/ready` on ArchLucid.Api and for the UI root (`GET /`) to return 200. Neither proves **browser â†’ Next.js â†’ `/api/proxy/*` â†’ API**. The homepage can load while every proxied API call returns **502 Upstream API unreachable**. The script exits success and opens the browser anyway.
...
2. **Config validation:** read `.env.local` (or `.env.example` fallback warning); assert `ARCHLUCID_API_BASE_URL` host/port matches `-ApiPort` (default 5128); do **not** auto-edit â€” print expected vs actual and exit non-zero.
...
6. **Optional switches:** `-EnsureSql` â†’ `dev up --sql-only` (default off); `-NoBrowser` unchanged.
7. Reuse patterns from `scripts/env-readiness.ps1`, `scripts/demo-start-local.ps1`, `scripts/v1-rc-drill.ps1` where practical â€” avoid duplicating probe logic.
...
- Optional cross-link in `docs/library/customer-facing/OPERATOR_QUICKSTART.md` or `docs/runbooks/TROUBLESHOOTING.md` (one line only â€” no new doc file required)

**Cross-ref:** **TB-157** (in-app toast copy); `archlucid-ui/src/app/api/proxy/[...path]/route.ts`; `archlucid-ui/.env.example`.

**Size estimate:** **S** (~4â€“8 h).

---

## TB-157 â€” API connectivity toasts â€” accurate API-down vs assistant-stream messaging

**Status (2026-05-31):** **Done** â€” `classifyApiConnectivityFailure` routes 502/upstream to **ArchLucid API unreachable**, 503 misconfig, UseStream to assistant; tests in `api-error-toast-policy.test.ts`.

**Priority:** **P0** â€” pick up in the next available thread (same local-dev incident cluster as **TB-156**; can ship independently but pair for best UX).

**Source:** Local dev triage (2026-05-31). `resolveApiRequestErrorToastPlan` treats any connectivity-shaped 5xx as â€œReview assistant unavailable / AI assistant service is not reachableâ€, including proxy **502 Upstream API unreachable** when ArchLucid.Api is down.

**Problem:**

`archlucid-ui/src/lib/api-error-toast-policy.ts` â€” `isConnectivityOrAssistantFailure` buckets UseStream, fetch failed, upstream unreachable, etc. into one assistant-focused toast. IT-savvy local operators misdiagnose API/proxy/config failures as Azure OpenAI / Ask outages. Local dev defaults to **Simulator**; conflating API down with LLM down is incorrect.
...
1. Replace `isConnectivityOrAssistantFailure` with a classifier (e.g. `classifyApiConnectivityFailure(message, httpStatus, problem?)`) that returns distinct toast plans:
   - **â€œUpstream API unreachableâ€** / proxy 502 â†’ title **ArchLucid API unreachable**; detail: proxy could not reach backend; verify API process and `ARCHLUCID_API_BASE_URL`; link or cite `docs/runbooks/TROUBLESHOOTING.md`.
   - **503 + invalid upstream config** â†’ **API URL not configured**; set `ARCHLUCID_API_BASE_URL` in `.env.local`; restart `npm run dev`.
   - **UseStream** / Ask SSE path only â†’ keep **Review assistant unavailable** (stream endpoint failed; core navigation may still work).
   - Generic fetch/network â†’ **Cannot reach ArchLucid API** (include correlation id when present).
2. Prefer `problem.supportHint` from proxy Problem Details when present (`route.ts` already emits hints).
3. Severity: **warning** for 502/API down (degraded shell); **error** for 503 misconfiguration (optional â€” document choice in tests).
4. **Buyer-polished / demo shell:** keep suppressing connectivity toasts (`buyerPolishedShell` path unchanged).
5. Update `api-error-toast-policy.test.ts`; grep for stale â€œAI assistant service is not reachableâ€ expectations.

**Acceptance criteria:**

- Simulated `ApiRequestError` with message `Upstream API unreachable: fetch failed` and status 502 â†’ toast title mentions **API**, not assistant.
- UseStream-only failure â†’ assistant wording retained.
- Buyer-polished shell still suppresses connectivity failures.
- No change to non-connectivity 5xx (â€œServer errorâ€ + detail) behavior.
...
**Size estimate:** **XS** (~2â€“4 h).

---

## TB-106 â€” RunDetailPageView â€” enrich authority `RunDetailDto` with cost estimate, trust evidence card, and `results[]`

**Status (2026-05-31):** **Done** â€” `IAuthorityRunDetailOperatorEnricher` / `AuthorityRunDetailOperatorEnricher` invoked from `AuthorityQueryController.GetRunDetail`; `AuthorityRunDetailOperatorEnricherTests`; quick-decision shows explicit explanation-trace fallback when `results[]` empty.
...
Consequence: on every live operator run review, the operator sees "Cost estimate unavailable", an empty trust evidence section, and no quick-decision confidence summary â€” three of the six critical operator signals identified in the fidelity audit.

**What to do (preferred â€” enrich authority endpoint):**
...
- `ArchLucid.Application/` (cost / trust service interfaces â€” determine shared extraction point)
- `archlucid-ui/src/app/(operator)/reviews/[runId]/_sections/load-run-detail-page-model.ts`
- `archlucid-ui/src/lib/api-types.generated.ts`
- `archlucid-ui/src/lib/api/architecture-runs.ts`
- `archlucid-ui/src/lib/quick-decision-summary-derive.ts`

**Cross-ref:** **TB-107**, **TB-108** (same root-cause audit cluster); **TB-011** (INV-002 trust card + operator UI badge â€” partially done).

**Size estimate:** **M** â€” ~1â€“2 days (authority endpoint enrichment + TS type update + loader simplification; parallel-fetch alternative is ~S).

---

## TB-107 â€” RunDetailPageView â€” surface `lastFailureReason` + `hasGovernanceWarnings` from `RunRecord`

**Status (2026-05-31):** **Done** â€” `RunDetailGovernanceAlerts` (warnings + last failure); `RunDetailRunMetadataSection` shows `retryCount` when &gt; 0; `RunDetailGovernanceAlerts.test.tsx` + metadata section tests.
...
- `LastFailureReason (string?)` â€” free-text reason written when a run fails or falls back.
- `HasGovernanceWarnings (bool)` â€” flag set when at least one governance warning is present on the run.
...
2. Add a `HasGovernanceWarnings` indicator in `RunDetailManifestSummaryAlerts.tsx` or `RunDetailPageHeader` â€” either a warning pill on the header or a `Callout` that fires when `run.hasGovernanceWarnings === true`. The wording should be "This run has governance warnings â€” review all findings before committing."
...
- `ArchLucid.Core/Persistence/ApplicationPorts/Models/RunRecord.cs` (read-only â€” verify field names)

**Cross-ref:** **TB-106** (same audit cluster â€” split API contract); **TB-108** (`hasCommitBlockingFailures` â€” complementary operator signal).

**Size estimate:** **S** â€” ~3â€“5 h (UI changes + type verification).

---

## TB-108 â€” RunDetailPageView â€” render `findingCoverageSummary.dispositionCoverage` + `hasCommitBlockingFailures`

**Status (2026-05-31):** **Done** â€” `FindingCoverageDispositionPanel` in `RunDetailOutcomeCards`; `commitBlockedReason` blocks commit UX; `RunDetailOutcomeCards.test.tsx`.
...
- `HasCommitBlockingFailures (bool)` â€” true when at least one finding engine failure prevents a reliable commit.
- `DispositionCoverage (RunFindingDispositionCoverage)` â€” breakdown of how many findings have been dispositioned (accepted, remediated, deferred, rejected, needs-evidence, open).
- `FailedEngineLabels (string[])` â€” the only field currently rendered (as a degraded banner).
...
- `ArchLucid.Contracts/Findings/RunFindingCoverageSummary.cs` (read-only â€” verify field names)

**Cross-ref:** **TB-106**, **TB-107** (same audit cluster); **TB-113** (OpenAPI drift may hide these fields in generated types).

**Size estimate:** **S** â€” ~4â€“6 h (callout + commit guard + disposition summary panel + type verification).

---

## TB-109 â€” RunDetailPageView â€” add retrieval-hit / RAG grounding panel

**Status (2026-05-31):** **Done** â€” `GET /v1/authority/runs/{runId}/retrieval-grounding` + `RunDetailRetrievalGroundingSection` (collapsed) after explanation; faithfulness banner links `#run-retrieval-grounding`; forensics duplicate panel removed.

**Source:** `RunDetailPageView` operator fidelity audit (2026-05-27). Canvas: `canvases/run-detail-operator-fidelity.canvas.tsx`.

**Problem:**

There is no UI surface anywhere on the run detail page (or any sub-route) that shows what documents were retrieved during RAG execution, their similarity scores, or whether any retrieval step was skipped or degraded. The retrieval infrastructure exists (`ArchLucid.Retrieval`, `IRetrievalQueryService`) and finding trust labels reference weak retrieval, but the operator has no way to judge whether the model had good grounding without examining raw trace blobs. This gap is most consequential when `RunExplanationSummary.faithfulnessWarning = true` â€” the operator sees a faithfulness warning but cannot inspect the underlying retrieval to understand it.
...
4. Link the section from the `faithfulnessWarning` banner in `RunExplanationSection` ("View retrieval hits â†’").
...
- `ArchLucid.Application/` (retrieval hit query â€” interface in `IRetrievalQueryService` or new port)
...
3. Surface the `blobUploadFailed` warning as a named issue in the same section ("Full trace unavailable â€” blob upload failed") so the operator understands completeness limits.
...
- `ArchLucid.Application/Agents/` (agent execution trace projection â€” add tool-call sub-records)
- `archlucid-ui/src/components/RunAgentForensicsSection.tsx`
- `archlucid-ui/src/app/(operator)/reviews/[runId]/_sections/RunDetailPageView.tsx`
- `archlucid-ui/src/lib/api/architecture-runs.ts`

**Cross-ref:** **TB-033** (persist LLM sampling params + reasoning token count â€” forensic completeness); **TB-082** (`AllowedTools` runtime enforcement â€” security); **TB-035** (remediation attempt forensics).

**Size estimate:** **M** â€” ~1.5â€“2 days (trace endpoint extension + UI section + blob-failure surfacing).

---

## TB-111 â€” RunDetailPageView â€” inline provenance summary card (collapse from sibling route)

**Source:** `RunDetailPageView` operator fidelity audit (2026-05-27). Canvas: `canvases/run-detail-operator-fidelity.canvas.tsx`.

**Problem:**

Provenance (which context snapshot was used, what inputs were fed to the architecture request, and what the coordinator graph source was) is accessible only by navigating to `reviews/[runId]/provenance` â€” a full-page sibling route that uses `GET /v1/architecture/runs/{runId}/provenance`. This route is linked from `RunDetailOutcomeCards` but only as an external link. An operator reviewing the run must navigate away, losing their scroll position and the run-detail context they were building.
...
2. The card should show a compact summary sourced from the authority provenance endpoint: context snapshot ID + description, architecture request ID, graph snapshot ID, and "View full provenance â†’" link to the sibling route.
...
**Cross-ref:** **TB-033**â€“**TB-038** (provenance completeness audit â€” backend side); **TB-036** (provenance â†” agent trace correlation).

**Size estimate:** **S** â€” ~3â€“5 h (inline card + authority provenance call if needed + link update).

---

## TB-112 â€” RunDetailPageView â€” add run-level approve / reject / request-remediation actions

**Source:** `RunDetailPageView` operator fidelity audit (2026-05-27). Canvas: `canvases/run-detail-operator-fidelity.canvas.tsx`.

**Problem:**

The only run-level action on `RunDetailPageView` is `CommitRunButton` (finalize the golden manifest). Governance actions â€” accept, reject, waive, defer, request remediation â€” all live on individual `reviews/[runId]/findings/[findingId]` sub-routes and are backed by `GovernanceStickinessController`. There is no run-level approve or reject.
...
5. Scope the actions to the `operator` role â€” buyer users should not see them.

**Note:** This is explicitly V1 scope â€” check [`V1_SCOPE.md`](V1_SCOPE.md) Â§2 before implementation to confirm run-level governance disposition is within the current release window. If it is release-windowed to V1.1, downgrade to V1.1 and note it here.
...
**Cross-ref:** **TB-057**â€“**TB-063** (commercial stickiness / governance review workflow); **TB-108** (`hasCommitBlockingFailures` gate for Approve action).

**Size estimate:** **M** â€” ~1.5â€“2 days (API + command/handler + UI action panel + dialog).

---

## TB-113 â€” Fix OpenAPI schema drift on `RunDetailDto` â€” expose `degradedFindingCoverage` + `findingCoverageSummary` in generated TypeScript types
...
- `ArchLucid.Contracts/Findings/RunFindingCoverageSummary.cs` (read â€” verify public surface)
- `scripts/` (OpenAPI generation script â€” verify CI integration)
...
**Size estimate:** **XS** â€” ~1â€“2 h (regeneration + annotation fix + cast cleanup).

---

## TB-114 â€” Establish enterprise design-token layer (Carbon-aligned neutral palette and accent scale)

**Status (2026-05-31):** **Done** â€” `design-tokens.ts`, `--al-*` in `globals.css`, Tailwind `al-*` colors, `UI_DESIGN_SYSTEM.md` Â§Tokens. Operator surface migration completed in **TB-115**.

**Source:** Owner-ratified UI design standard, 2026-05-27. Canonical doc: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md).

**Problem:**

The current UI inherits Tailwind's default palette and shadcn component defaults. The result is `teal-500` borders, `neutral-100` card backgrounds, and accent colors with a startup-dashboard feel rather than a Carbon-grade enterprise visual language. There is no single place that defines the ArchLucid surface palette â€” changes to brand-significant tokens scatter across dozens of component files.

**What to do:**

1. Create `archlucid-ui/src/lib/design-tokens.ts` (or a CSS custom-properties file) that declares the authoritative token set:
   - **Neutral surface scale:** `--al-surface-base`, `--al-surface-raised`, `--al-surface-overlay` â€” mapped to Carbon's `$layer-01` / `$layer-02` semantics (cool gray range: `gray-50` â†’ `gray-100` for light mode).
   ...
   - **Status semantic tokens:** `--al-status-ready`, `--al-status-warn`, `--al-status-blocked`, `--al-status-approved`, `--al-status-approved-monitoring` â€” each with a background, foreground, and border variant.
...
3. Document the token map in `docs/library/UI_DESIGN_SYSTEM.md` Â§Tokens.
...
**Size estimate:** **S** â€” ~4â€“6 h (token definition + Tailwind wiring + docs; excludes component migration which is TB-115/116/117).

---

## TB-115 â€” Surface and card audit: remove pastel cards; apply Carbon-style neutral surfaces

**Status (2026-05-31):** **Done** â€” `operatorSemanticSurface` / `operatorSemanticBadge` / `operatorConfidenceSurface` in `design-tokens.ts`; shared proof/confidence cards migrated; `WelcomeBanner` compact enterprise banner; bulk pass via `archlucid-ui/scripts/migrate-tb115-operator-surfaces.ps1` (~95 files across operator, executive, marketing, `lib`, `global-error`). **Verified 2026-05-31:** `design-tokens.test.ts`, `PolicyPackDiffView.test.tsx`, demo-explain and `/why` snapshots green; decorative `bg-*-50` grep clear in app TS/TSX (remaining hits: primary CTAs `bg-teal-700`, chart/meter fills `bg-*-500`, 6px status dots). **TB-117â€“TB-120** design-system wave complete (tables, spacing, typography, Cursor rule).

**Source:** Owner-ratified UI design standard, 2026-05-27. Canonical doc: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md).

**Problem:**

Operator surfaces contain large pastel cards with colored backgrounds (`bg-teal-50`, `bg-amber-50`, `bg-blue-50`, `bg-green-50`) used decoratively rather than to communicate actionable status. These read as consumer SaaS, not enterprise governance. Carbon's aesthetic uses flat neutral surfaces (`$layer-01`) with structured borders and restrained elevation â€” color appears only on status indicators and interactive elements.
...
- `archlucid-ui/src/app/(operator)/` â€” all page views and section components
- `archlucid-ui/src/components/` â€” shared cards (e.g. `CorePilotNextStepsCard.tsx`, `OperatorNextActionsCard.tsx`, `WelcomeBanner.tsx`, `SampleFirstReviewPackageCard.tsx`)

**Cross-ref:** **TB-114** (tokens must exist before this pass); **TB-116** (status cards get canonical tags, not ad-hoc coloring).

**Size estimate:** **M** â€” ~1.5 days (audit + systematic replacement across ~40â€“60 component files).

---

## TB-116 â€” Implement canonical status tag component and replace ad-hoc status badges

**Status (2026-05-31):** **Wave 1 landed** â€” `StatusTag`, `SeverityTag`, `RunStatusBadge` monitoring chip. Follow-up: replace remaining `StatusPill` ad-hoc pipeline colors (**TB-115**).

**Source:** Owner-ratified UI design standard, 2026-05-27. Canonical doc: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md).

**Problem:**

Run status, governance approval state, and finding severity are communicated through a mix of ad-hoc colored `span` elements, `Badge` variants, and inline `cn()` conditionals scattered across the codebase. There is no single canonical set of status tokens. The result is visual inconsistency and copy inconsistency â€” the same state appears as "READY", "Ready", "ready", or "âœ“" depending on where the user looks.

**What to do:**

1. Create `archlucid-ui/src/components/ui/StatusTag.tsx` â€” a single component accepting a `status` prop with the canonical value set:
   - `ready` Â· `needs-attention` Â· `blocked` Â· `approved` Â· `approved-with-monitoring` Â· `in-progress` Â· `draft`
...
- `archlucid-ui/src/components/` â€” any component rendering status/severity badges

**Cross-ref:** **TB-114** (status tokens); **TB-115** (card status colors â†’ tags).

**Size estimate:** **S** â€” ~4â€“6 h (component creation + replacement pass).

---

## TB-117 â€” Operator data tables: Carbon-style structured tables for runs, findings, and audit

**Status (2026-05-31):** **Done** â€” `EnterpriseTable` on reviews list, governance findings desktop queue (`GovernanceFindingsQueueDesktopTable`), operator audit log (`AuditEventsOperatorTable`). Buyer-polished audit keeps milestone timeline cards; operator mode uses structured table + row payload disclosure.

**Source:** Owner-ratified UI design standard, 2026-05-27. Canonical doc: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md).

**Problem:**

The reviews list, findings list, and audit timeline use a mix of card-stacked rows, custom `div`-based grids, and minimal `table` elements. None conform to Carbon's structured data table pattern: column headers with sort indicators, row hover states using a single `$layer-hover` tint, compact row height (~40px), and clear primary/secondary column hierarchy. Procurement and CIO reviewers evaluate enterprise products on how their data tables look â€” a mature table communicates governance credibility; a card stack does not.

**What to do:**

1. Create `archlucid-ui/src/components/ui/EnterpriseTable.tsx` â€” a typed generic table component:
...
**Size estimate:** **M** â€” ~1.5 days (component + three migration targets).

---

## TB-118 â€” Spacing audit: replace marketing-scale spacing with enterprise-compact spacing in operator views

**Status (2026-05-31):** **Done (acceptance scope)** â€” operator home, run detail, executive dashboard (`py-4`), governance findings + decision register, finding detail/inspect, audit page (`space-y-4`). Admin/settings wizards may still use `space-y-6` where forms need breathing room; avoid new `space-y-8` / `py-8` on `(operator)/` routes.
...
1. Define a spacing convention in `docs/library/UI_DESIGN_SYSTEM.md`:
   - **Operator page section gap:** `space-y-4` (16px) â€” not `space-y-6` or `space-y-8`.
...
3. Apply the convention systematically â€” prioritize pages that procurement/CIO reviewers land on first: operator home, review detail, executive dashboard, and findings.
...
**Size estimate:** **S** â€” ~3â€“4 h (convention doc + four-page audit and fix).

---

## TB-119 â€” Typography audit: enforce accessible enterprise type hierarchy across operator surfaces

**Status (2026-05-31):** **Done** â€” `OPERATOR_TYPOGRAPHY` + `DESIGN_TOKENS.typography`; `cardTitle` token; migration script `archlucid-ui/scripts/migrate-tb119-operator-typography.ps1` (~106 files). Operator `h1` capped at `text-xl`; KPI tiles use `kpiValue` (`text-4xl` mono); run-detail section labels use `sectionTitle`. Acceptance surfaces (home banner, run detail, executive dashboard, findings, audit) pass grep for `text-2xl`/`text-3xl` page titles.
...
3. Verify that all text distinguishing meaning uses at least two signals (size + weight, not color alone) â€” accessibility requirement.
...
- `archlucid-ui/src/app/(operator)/` â€” page heading and section heading elements

**Cross-ref:** `.cursor/rules/UI-Accessibility-Baseline.mdc`; **TB-115** (cards); **TB-118** (spacing).

**Size estimate:** **S** â€” ~3â€“4 h (convention doc + targeted fixes).

---

## TB-120 â€” Add Carbon-standard Cursor rule so AI-generated UI code stays conformant

**Status (2026-05-31):** **Done** â€” `.cursor/rules/UI-Enterprise-Design-Standard.mdc` (globs `archlucid-ui/src/**`), wave completion table, migration script anchor; `archlucid-ui/AGENTS.md` + `UI_DESIGN_SYSTEM.md` cross-refs.

**Source:** Owner-ratified UI design standard, 2026-05-27. Canonical doc: [`docs/library/UI_DESIGN_SYSTEM.md`](UI_DESIGN_SYSTEM.md).

**Problem:**

There is no Cursor rule that instructs the AI assistant to follow the Carbon-inspired enterprise visual standard when writing or modifying UI code. Without a rule, every new component risks defaulting to Tailwind/shadcn aesthetic defaults â€” pastel cards, teal borders, marketing spacing â€” which directly contradicts the V1 GA requirement.
...
1. Create `.cursor/rules/UI-Enterprise-Design-Standard.mdc` with the following content (adapt format to match existing rules in `.cursor/rules/`):
   ...
   - Include the full ratified agent instruction block verbatim (Â§ "Ratified instruction for AI coding agents" in `UI_DESIGN_SYSTEM.md`).
...
**Cross-ref:** **TB-114**â€“**TB-119** (the design system items this rule enforces).

**Size estimate:** **XS** â€” ~1 h (rule file + AGENTS.md update).
