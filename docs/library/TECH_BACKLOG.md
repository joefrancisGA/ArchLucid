...
**TB-207 – TB-213** were added 2026-06-02 from an independent first-principles **Adoption Friction** quality assessment. The source assessment file (`docs/assessments/AdoptionFriction_06022026.MD`) was not persisted from the generating session and requires recreation. Items address: CLI dotnet global tool packaging (**TB-208**, P0), `archlucid request create --from-file` CLI command (**TB-209**, P1), pilot prerequisites page with `Test-ArchLucidPrerequisites.ps1` (**TB-210**, P1), SAML SP certificate rotation runbook (**TB-211**, P1), Terraform for Azure OpenAI + AI Search + Content Safety (**TB-212**, P1), and QualityGate WarnOnly lint rule (**TB-213**, P2). **TB-214** (reserved) and **TB-215–TB-221** (Time-to-Value assessment — source file also not persisted) are deferred to a future session when those assessment files are recreated.

**TB-222 – TB-228** were added 2026-06-02 from an independent first-principles **Stickiness** quality assessment (`docs/assessments/Stickiness_06022026.MD`, weight 6/116 — COMMERCIAL, score 63/100, weighted contribution 3.26%). Items address the highest stickiness gaps by weighted deficiency signal: recurrence scheduling UI post-commit nudge + management page (**TB-222**, P1), "Decisions needed" KPI card in governance dashboard (**TB-223**, P1), AI conversational compare-two-runs narrative (**TB-224**, P1), CS-06 RLS fix on `SqlOperatorStickinessSnapshotReader` (**TB-225**, P1 — security), risk exceptions management page (**TB-226**, P2), second-review proof script update (**TB-227**, P2), and internal tenant health score surface (**TB-228**, P3). None duplicate TB-057–TB-063 (commercial stickiness review, prior batch).

**TB-229 – TB-237** were added 2026-06-02 from an independent first-principles **Marketability** quality assessment (`docs/assessments/Marketability_06022026.MD`, weight 8/116 — COMMERCIAL, score 65/100, weighted contribution 4.48%). Items address the highest marketability gaps by weighted deficiency signal: reference-customer first-contact workflow (**TB-229**, P1), GTM placeholder audit + CI warn script (**TB-230**, P1), Stage 0→1 claim-readiness tracker and proof-packet run log (**TB-231**, P1), LinkedIn publishing schedule (**TB-232**, P2), demo video storyboard (**TB-233**, P2), `SHOULD_YOU_EVALUATE.md` ICP enrichment (**TB-234**, P2), `EXECUTIVE_ONE_EMAIL_KIT.md` creation (**TB-235**, P2), demo video production — DEFERRED owner action (**TB-236**), and discount-stack buyer-facing pricing framing (**TB-237**, P2). Pending questions: PQ-MKT-01 (canonical contact URL), PQ-MKT-02 (LinkedIn cadence), PQ-MKT-03 (demo video tooling).
...
| TB-207 | Token-claims diagnostic — `POST /v1/admin/auth/diagnose-token` + `archlucid auth test-token --bearer` — decode JWT payload without signature validation; pipe through `ArchLucidRoleClaimsTransformation`; return `resolvedRoles[]`, `unmappedValues[]`, `warnings[]`; Admin auth required; durable `Auth.TokenDiagnosticRequested` audit event | Adoption Friction P0 — 403-after-SSO is the #1 documented identity adoption blocker; operators have no programmatic path to diagnose claim mapping | M |
| TB-208 | CLI as dotnet global tool + self-contained binaries — `<PackAsTool>true</PackAsTool>` in `ArchLucid.Cli.csproj`; GitHub Actions publish job for win-x64/linux-x64/osx-x64 `PublishSingleFile=true`; `docs/engineering/CLI_INSTALL.md`; update `OPERATOR_QUICKSTART.md` | Adoption Friction P0 — evaluators without .NET SDK cannot use `archlucid` CLI | S |
| TB-209 | `archlucid request create --from-file <path>` — reads JSON file, POSTs to `POST /v1/architecture/request`, prints run ID; optional `--request-id` override; replace `curl` example in `OPERATOR_QUICKSTART.md` reference architecture section | Adoption Friction P1 — explicitly documented as missing in `OPERATOR_QUICKSTART.md` | S |
...
| TB-222 | Recurrence scheduling UI — `RecurrenceSchedulePostCommitCard` post-commit nudge + `/governance/recurrence-schedules` management page; calls `createArchitectureReviewRecurrenceSchedule` / `listArchitectureReviewRecurrenceSchedules`; enable/disable toggle | Stickiness P1 — `GovernanceStickinessController` recurrence endpoints fully built but no UI; operators must call API directly | M |
| TB-223 | "Decisions needed" KPI card in governance dashboard — 7-tile `DecisionsNeededSummaryCard` from `GET /v1/governance/decisions-needed-summary`; per-tile deep-links; caution highlight on expiring waivers; auto-refresh | Stickiness P1 — summary endpoint and DTO implemented but not surfaced in dashboard | S |
| TB-224 | AI conversational compare-two-runs narrative — `GET /v1/compare/{id}/narrative` LLM-generated context for structured delta; narrative panel in compare UI; `ComparisonNarrativeService` | Stickiness P1 — compare UI renders diff table only; no narrative explaining "what changed and why does it matter" | M |
| TB-225 | CS-06 RLS fix on `SqlOperatorStickinessSnapshotReader` — add `SESSION_CONTEXT` scope filter to all queries; add SQL integration tests verifying cross-tenant isolation | Stickiness P1 (security) — `SqlOperatorStickinessSnapshotReader` identified in CUSTOMER_SUCCESS_PERSISTENCE_DESIGN.md as not scoped by tenant | S |
| TB-226 | Risk exceptions management page — `/governance/risk-exceptions` list + create UI; calls `GovernanceStickinessController` risk exception endpoints; `RiskExceptionCard` component | Stickiness P2 — `FindingInspectGovernanceStickinessPanel` creates exceptions per-finding but no standalone management surface exists | M |
| TB-227 | Second-review proof script update — extend `collect-first-pilot-proof.ps1` with `--include-comparison` flag; outputs delta summary, governance activity since first run, and manifest lineage to `pilot-proof-packet/repeat-review/` | Stickiness P2 — proof-packet collection is first-run only; second review has no automated evidence collection path | S |
| TB-228 | Internal tenant health score surface — `GET /v1/admin/tenant-health` endpoint returning `TenantHealthScore`; admin operator UI widget `/admin/tenant-health` | Stickiness P3 — `OperatorStickinessSignals` and `TenantHealthScores` designed in CUSTOMER_SUCCESS_PERSISTENCE_DESIGN.md but no admin surface exists | S |
| TB-229 | Reference-customer first-contact capture workflow — `REFERENCE_CUSTOMER_FIRST_CONTACT_TEMPLATE.md` (email template + 3 commitment tiers) + `REFERENCE_CUSTOMER_TRACKING_CHECKLIST.md` (pilot-complete→Published steps); update `reference-customers/README.md` §3 | Marketability P1 — zero published references; first-contact workflow absent; reference infrastructure complete but operationally idle | XS |
| TB-230 | GTM collateral placeholder audit — fix `PRODUCT_DATASHEET.md` contact placeholder + `SHOULD_YOU_EVALUATE.md` Q2 dead-end; CI warn script `check_gtm_placeholder_tokens.py`; `PLACEHOLDER_AUDIT.md` tracking all `<<...>>` tokens in GTM docs | Marketability P1 — buyer-facing docs contain unfilled `[placeholder]` tokens; dead CTA on datasheet | S |
| TB-231 | Stage 0→1 claim-readiness tracker — `CLAIM_READINESS_STATUS.md` (G1–G6 PASS/HOLD + evidence links) + `PROOF_PACKET_RUN_LOG.md` (real run log for G4); linked from `GTM_BACKLOG.md` | Marketability P1 — G1–G6 gate framework exists in docs only; no operational status page for Stage 1 progress | XS |
| TB-232 | LinkedIn publishing schedule — `LINKEDIN_PUBLISHING_SCHEDULE.md` with date assignment for M-10 through M-15, hashtag set, comment-seed prompts, and status tracking column | Marketability P2 — 5 posts + 1 article drafted and ready; no publishing calendar | XS |
| TB-233 | Demo video storyboard — `DEMO_VIDEO_STORYBOARD.md` mapping each `DEMO_VIDEO_SCRIPT.md` segment to screen state, action, narration extract, annotation, and duration; pre/post production checklists | Marketability P2 — video script exists; storyboard removes production friction; video itself is owner-action | XS |
| TB-234 | `SHOULD_YOU_EVALUATE.md` ICP enrichment — add Q5 (team size gate), replace archlucid.net signup link with conditional quote-request fallback, add "Strong fit signals" section | Marketability P2 — Q2 dead-ends on multi-cloud with no contact link; no ICP strong-fit signals section | XS |
| TB-235 | `EXECUTIVE_ONE_EMAIL_KIT.md` — verify existence or create; 3 subject variants, 120-word email body, 4-artifact checklist, follow-up timing guidance | Marketability P2 — file referenced from `EXECUTIVE_SPONSOR_BRIEF.md` but not found in repo | XS |
| TB-236 | Demo video production (DEFERRED — owner) — requires screen recording with voiceover using storyboard from TB-233; upload to Loom/Wistia; link from datasheet and brief | Marketability P1 (owner) — no demo video exists; buyers have no "show me what this does" artifact | DEFERRED |
| TB-237 | Discount stack buyer-facing framing — add "Why is the price this low?" early-adopter explanation below tier grid in `/pricing` UI; add "Transparent early-access framing" pricing principle row to `PRICING_PHILOSOPHY.md §1` | Marketability P2 — pricing discount stack is internally documented; buyers have no buyer-visible framing for aggressive early pricing | S |
...
1. Post-commit nudge (RecurrenceSchedulePostCommitCard):
   In archlucid-ui/src/app/(operator)/reviews/[runId]/_sections/RunDetailPageView.tsx, after CommitRunButton succeeds and hasGoldenManifest is true, render a new RecurrenceSchedulePostCommitCard component that:
   - Shows "Schedule next review" heading using OPERATOR_TYPOGRAPHY.pageTitle.
   - Pre-fills a cron expression field defaulting to "0 8 * * 1" (weekly Monday 8am).
   - Includes a display name field (default: "Weekly architecture review").
   - Submits via createArchitectureReviewRecurrenceSchedule from governance-stickiness-api.ts with sourceRunId = runId.
   - On success: shows StatusTag "Scheduled" + nextRunUtc in human-readable format.
   - On error: shows inline error message.
   - Wraps in a collapsible; open by default only when hasStickinessPrompt (new boolean prop) is true.

2. Recurrence management page (/governance/recurrence-schedules):
   Create archlucid-ui/src/app/(operator)/governance/recurrence-schedules/page.tsx and RecurrenceSchedulesClient.tsx. The page should:
   ...
   - Provide enable/disable toggle (call PUT /v1/governance/recurrence-schedules/{id} if the endpoint exists; note gap if not).
   - Add route to governance nav in nav-config.ts after "Decision register".
   - Add LayerHeader: "Operate (governance and trust)".

Add unit tests for RecurrenceSchedulePostCommitCard: submitted state, success state, error state.
```

**Acceptance criteria:**
- `/governance/recurrence-schedules` renders a list of schedules when API returns data.
- Post-commit flow shows "Schedule next review" card after first manifest commit.
- Enable/disable toggle calls the correct endpoint.
- Unit tests pass.
...
**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-02)
**Priority:** P1
**Size:** S (~2–3 h)

**Problem:** `GET /v1/governance/decisions-needed-summary` returns a fully specified `GovernanceDecisionsNeededSummary` DTO with 7 fields: `pendingApprovals`, `staleRisks`, `unownedHighSeverityRisks`, `findingsAwaitingEvidence`, `waiversExpiringWithin14Days`, `deferredFindingsDue`, `totalDecisionItems`. The client method `getGovernanceDecisionsNeededSummary` is implemented in `governance-stickiness-api.ts`. The governance dashboard (`/governance/dashboard`) does NOT surface this summary. Operators have no "what needs my attention today?" view.
...
2. Fetch the summary alongside existing getGovernanceDashboard call (parallel fetch).
3. Create DecisionsNeededSummaryCard in archlucid-ui/src/components/governance/ with:
   - 2×3 grid of KPI tiles: "Pending approvals", "Stale risks", "Unowned High risks", "Awaiting evidence", "Waivers expiring (14d)", "Deferred items due".
   - If totalDecisionItems === 0: OperatorEmptyState "No decisions needed — all risks are current."
   - If waiversExpiringWithin14Days > 0: subtle left-border accent using --al-caution.
   - Each tile links to relevant route (pending approvals → /governance, stale risks → /governance/findings?filter=stale, waivers → /governance/risk-exceptions).
4. Place card at top of governance dashboard before pending approval requests list.
5. Auto-refresh alongside existing 30-second dashboard refresh.
6. Add Vitest test for DecisionsNeededSummaryCard: renders 7 tiles, empty state when totalDecisionItems is 0, highlights waivers expiring tile.
```

**Acceptance criteria:**
- KPI card renders with all 7 tiles from the API response.
- Empty state shown when `totalDecisionItems === 0`.
- Expiring waivers tile shows caution accent.
- All tile links navigate to the correct route.
- Vitest tests pass.
...
**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-03)
**Priority:** P1
**Size:** M (~4–6 h)

**Problem:** `IComparisonService` produces structured deltas. The compare UI renders a diff table. But "what changed between run A and run B, and why does it matter?" requires human interpretation of raw delta rows. This gap reduces the "value compounds over time" story that makes buyers renew. `AI_LEVERAGE_ROADMAP.md` item 11 identifies this as a priority.

**Cursor prompt:**
```
Add LLM-generated narrative to the comparison flow.

Backend:
1. Create ComparisonNarrativeService : IComparisonNarrativeService in ArchLucid.Core.Services with a single method:
   Task<ComparisonNarrativeResult> GenerateAsync(ComparisonRecord comparison, CancellationToken ct)
   - Build a prompt from the structured delta (findings added/removed/changed, manifest delta, compliance changes).
   - Call ILlmProvider with the prompt and return a ComparisonNarrativeResult { NarrativeMarkdown, GeneratedUtc, ModelDescriptor }.
   - Persist the result to a new dbo.ComparisonNarratives table (ComparisonId PK FK, NarrativeMarkdown, GeneratedUtc, ModelDescriptor).
2. Expose via GET /v1/compare/{comparisonId}/narrative. Return 204 if not yet generated, 200 with the narrative when present.
3. Add a POST /v1/compare/{comparisonId}/narrative endpoint that triggers async generation (returns 202 Accepted).

Frontend:
1. In the comparison detail page, below the delta table add a ComparisonNarrativePanel component.
2. On mount, call GET /v1/compare/{comparisonId}/narrative.
3. If 204: show a "Generate narrative" button that calls POST; poll every 5s until 200.
4. If 200: render the NarrativeMarkdown in a collapsible using ReactMarkdown.

Add unit tests for ComparisonNarrativeService (mock ILlmProvider), repository, and the narrative endpoint.
```

**Acceptance criteria:**
- `GET /v1/compare/{id}/narrative` returns 200 with markdown after generation.
- UI renders narrative in compare view after clicking "Generate narrative."
- Narrative is persisted; regeneration is possible but not triggered automatically.
- Unit tests pass.
...
**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-04)
**Priority:** P1 (security)
**Size:** S (~2 h)

**Problem:** `SqlOperatorStickinessSnapshotReader` was identified in `CUSTOMER_SUCCESS_PERSISTENCE_DESIGN.md` as gap CS-06: queries are not scoped by `SESSION_CONTEXT` tenant filter, meaning a multi-tenant deployment could return cross-tenant stickiness data to an authenticated operator.

**Cursor prompt:**
```
Fix SqlOperatorStickinessSnapshotReader to enforce SQL RLS tenant scope.

1. In SqlOperatorStickinessSnapshotReader, add a ScopeContext parameter to all public query methods.
2. Ensure every Dapper query includes a WHERE TenantId = @TenantId clause (or relies on SESSION_CONTEXT set by the repository base class pattern used in other scoped readers).
3. Verify the repository sets SESSION_CONTEXT before executing queries — see SqlFindingsSnapshotRepository for the established pattern.
4. Add SQL integration tests:
   - Two tenants each with stickiness data.
   - Verify that a query scoped to tenant A returns zero rows from tenant B's stickiness data.
   - Verify that SESSION_CONTEXT is set before each query.

Cross-ref: ADR 0003 (SQL RLS), SqlFindingsSnapshotRepository (pattern reference), CUSTOMER_SUCCESS_PERSISTENCE_DESIGN.md §CS-06.
```

**Acceptance criteria:**
- SQL integration test confirms no cross-tenant data leakage.
- `SESSION_CONTEXT` is set on every query path.
- Existing functionality is unchanged for single-tenant queries.

---

## TB-226 — Risk exceptions dedicated management page

**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-05)
**Priority:** P2
**Size:** M (~3–5 h)

**Problem:** `FindingInspectGovernanceStickinessPanel` allows per-finding risk exception creation. `GovernanceRiskRegisterClient.tsx` renders the architecture risk register. But there is no standalone page where governance leads can see all risk exceptions across all findings, filter by status, and manage expiry. Governance review sessions require jumping through each finding individually to review risk exceptions.

**Cursor prompt:**
```
Create a risk exceptions management page at /governance/risk-exceptions.

1. Create archlucid-ui/src/app/(operator)/governance/risk-exceptions/page.tsx and RiskExceptionsClient.tsx.
2. On load, call the appropriate governance-stickiness-api endpoint for listing risk exceptions.
3. Render exceptions in an EnterpriseTable with columns: Finding title, Exception type, Justification (truncated), Expiry date, Status.
4. Add a StatusTag per exception: Active / Expiring soon (< 14 days) / Expired.
5. Highlight Expiring soon rows with --al-caution left-border accent.
6. Add a LayerHeader: "Operate (governance and trust)".
7. Add the route to governance nav in nav-config.ts after "Risk register".
8. Add unit tests: renders exception list, shows expiring-soon accent, handles empty state.
```

**Acceptance criteria:**
- Page renders all risk exceptions with correct status tags.
- Expiring-soon exceptions display accent.
- Empty state renders when no exceptions exist.
- Route appears in governance navigation.

---

## TB-227 — Second-review proof script update (`collect-first-pilot-proof.ps1`)

**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-06)
**Priority:** P2
**Size:** S (~2–3 h)

**Problem:** `collect-first-pilot-proof.ps1` collects evidence for the first run only. There is no automated path for a second review evidence bundle. Proof packets for repeat reviews require manual artifact surgery — a barrier to G4 gate clearance for Stage 1 GTM.

**Cursor prompt:**
```
Extend collect-first-pilot-proof.ps1 to support repeat review collection.

1. Add a -RunNumber parameter (int, default 1). When -RunNumber 2 or higher:
   a. Require -CompareBaseRunId parameter (Guid — the previous committed run ID).
   b. Call GET /v1/architecture/run/{CompareBaseRunId}/compare/{RunId}. Write response JSON to pilot-proof-run{N}/compare-run{N-1}-to-run{N}.json.
   c. Call GET /v1/governance/decisions-needed-summary. Append result to pilot-proof-run{N}/decisions-needed-summary-run{N}.json.
   d. Call GET /v1/governance/risk-register. Write to pilot-proof-run{N}/risk-register-run{N}.json.
   e. Add a stickinessSignals section to the output manifest with: findingsReduced, findingsAdded, governanceComplianceDelta.

2. Rename output folder from pilot-proof/ to pilot-proof-run{N}/ when -RunNumber > 1 to avoid overwriting first-run evidence.

3. Update script header comment to state support for first and repeat review collection.
...
**Acceptance criteria:**
- `-RunNumber 2` with `-CompareBaseRunId` produces a repeat-review proof bundle.
- `-RunNumber 2` without `-CompareBaseRunId` exits with a descriptive error.
- Pester tests pass.
- `REPEAT_REVIEW_LOOP.md §5` references the new invocation pattern.

---

## TB-228 — Internal tenant health score surface

**Source:** Stickiness quality assessment (`docs/assessments/Stickiness_06022026.MD` — S-07)
**Priority:** P3
**Size:** S (~2–3 h)

**Problem:** `TenantHealthScoringCalculator` computes engagement, governance, and pilot completion scores. `dbo.TenantHealthScores` stores them. No endpoint or admin UI surfaces this data. The SaaS CS team cannot identify at-risk accounts during the post-pilot conversion window.

**Cursor prompt:**
```
Expose tenant health scores via an admin endpoint and admin UI widget.

Backend:
1. Create GET /v1/admin/tenant-health returning a list of TenantHealthScoreDto { TenantId, TenantName, EngagementScore, GovernanceScore, PilotCompletionScore, OverallScore, LastComputedUtc, RiskBand (Green/Yellow/Red) }.
2. Require AdminAuthority scope on the endpoint.
3. Add unit tests for the endpoint handler.

Frontend (admin surface):
1. Add an admin-only nav link "Tenant health" visible only to users with Admin role.
2. Create archlucid-ui/src/app/(admin)/tenant-health/page.tsx with an EnterpriseTable of tenant health scores.
3. Columns: Tenant, Engagement, Governance, Pilot Completion, Overall, Risk Band (StatusTag: Green / Yellow / Red).
4. Sort by OverallScore descending by default.
5. Add unit test: renders table, shows StatusTag per risk band.
```

**Acceptance criteria:**
- `GET /v1/admin/tenant-health` returns 401 for non-admin callers.
- Admin UI renders tenant health table.
- Risk band StatusTag correctly reflects the computed band.

---

## TB-229 — Reference-customer first-contact capture workflow

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-01)
**Priority:** P1
**Size:** XS (~1 h)

**Problem:** Zero published reference customers. The CI guard warns on every build. The reference-customer infrastructure (templates, lifecycle, pricing discount trigger) is fully built. The operational gap is that there is no written workflow to go from "pilot completed" to "first-contact email sent." Without a template, the ask gets deferred indefinitely.

**Cursor prompt:**
```
Create docs/go-to-market/REFERENCE_CUSTOMER_FIRST_CONTACT_TEMPLATE.md with:
- Subject line variants: short (8 words max), outcome-first, meeting-request
- 150-word email body template with: opening hook, 2-sentence ArchLucid value, pilot outcome summary placeholder (<<PILOT_OUTCOME_SENTENCE>>), reference ask (logo / written quote / full case study — buyer chooses level)
- Clear statement of the 15% standing discount per PRICING_PHILOSOPHY.md §4.1
- Objection-handling postscript for "we have a non-disclosure policy" (suggest logo-only level)
- Required placeholders: <<CUSTOMER_NAME>>, <<TIER>>, <<PILOT_OUTCOME_SENTENCE>>
...
- Steps: (a) gather pilot metrics from PILOT_ROI_MODEL.md §5, (b) send first-contact template, (c) get approval in writing, (d) receive logo file, (e) complete case study draft, (f) get customer-approved copy, (g) update CHANGELOG.md, (h) set README row to Published

Update reference-customers/README.md "How to add a real reference" section (after step 1) to link to both new documents.
```

**Acceptance criteria:**
- Both new files exist with all required sections.
- `reference-customers/README.md` §3 links to both files.
- Template contains no unfilled `<<...>>` tokens other than the documented buyer-fill variables.

---

## TB-230 — GTM collateral placeholder audit and CI warn script

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-02)
**Priority:** P1
**Size:** S (~2–3 h)

**Problem:** Several customer-facing GTM documents contain `[placeholder]`, `<<placeholder>>`, or `TBD` tokens in buyer-visible positions (contact info, SLA targets, reference names). The most critical instance: `PRODUCT_DATASHEET.md` ends with `Contact: [placeholder — add sales contact or URL]`. A buyer receiving this document ends at a dead contact entry.

**Cursor prompt:**
```
1. In docs/go-to-market/PRODUCT_DATASHEET.md, replace the final line:
...
2. In docs/go-to-market/SHOULD_YOU_EVALUATE.md, replace:
...
3. Create scripts/ci/check_gtm_placeholder_tokens.py that:
   ...
   - Identifies lines containing <<[A-Z_]+>> patterns (not buyer-fill variables — skip lines with "Required placeholders:" context)
   - Prints file:line:token for each match
   - Exits 0 (non-blocking warn only)

4. Create docs/go-to-market/PLACEHOLDER_AUDIT.md listing each known unfilled <<...>> token with: document path, token name, owner, and target fill date.

5. Add check_gtm_placeholder_tokens.py to .github/workflows/ci.yml as a warn-only step "Check GTM placeholder tokens" with continue-on-error: true.
```

**Acceptance criteria:**
- `PRODUCT_DATASHEET.md` has no `[placeholder]` in the final line.
- `SHOULD_YOU_EVALUATE.md` Q2 links to `/contact`.
- `check_gtm_placeholder_tokens.py` runs in CI without failing the build.
- `PLACEHOLDER_AUDIT.md` lists all known tokens.

---

## TB-231 — Stage 0→1 claim-readiness tracker and proof-packet run log

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-03)
**Priority:** P1
**Size:** XS (~1 h)

**Problem:** The G1–G6 proof-gate framework and rollout stage criteria exist in `GTM_BACKLOG.md` but there is no operational status page a founder can update after each pilot. Without a per-gate status tracker, the Stage 1 gate lives only in documentation, not as a working checkpoint.

**Cursor prompt:**
```
Create docs/go-to-market/CLAIM_READINESS_STATUS.md:
- Header section: "Current authorized stage: Stage 0 — Controlled pilots (last reviewed: 2026-06-02)"
- Table with columns: Gate | Signal | Status (PASS/HOLD) | Evidence Link | Blocking Dep | Who Unblocks
- Populate from CLAIM_READINESS_CHECKLIST.md gate definitions with initial statuses:
  G1: HOLD — run-detail fidelity verification pending (TB-106–108)
  G2: PASS — RoiMetricSourceKind implemented
  G3: PASS — TB-071/072/073 completed
  G4: HOLD — 0 of 3 qualifying real runs logged
  G5: HOLD — requires owner credentials (owner action)
  G6: PASS — trust pack current; deferred items stated as deferred
- Section: "Stage 1 authorized when G1 and G4 both PASS for ≥3 distinct real committed runs"
- Instructions: update Status and Evidence Link after each pilot run

Create docs/go-to-market/PROOF_PACKET_RUN_LOG.md:
- Table: Run date | Tenant | Run ID | Mode (Real/Simulator) | Proof packet generated (Y/N) | Clean without manual surgery (Y/N) | Notes
- One placeholder row showing required format
- Header note: "G4 requires 3 PASS rows with Mode=Real, Proof packet=Y, Clean=Y"

Update GTM_BACKLOG.md "Proof-gated rollout criteria" section to add links to CLAIM_READINESS_STATUS.md and PROOF_PACKET_RUN_LOG.md after the "How to use this" subsection.
```

**Acceptance criteria:**
- Both files created with all required sections.
- `GTM_BACKLOG.md` links to both files.
- G1–G6 initial statuses match the current engineering state from `TECH_BACKLOG.md`.

---

## TB-232 — LinkedIn publishing schedule with content sequencing calendar

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-04)
**Priority:** P2
**Size:** XS (~30 min)

**Problem:** Five LinkedIn posts (M-10 through M-14) and one long-form article (M-15) are drafted and ready in `LINKEDIN_CONTENT_V1.md`. No publishing calendar exists. Without a date assignment and sequencing plan, publishing is indefinitely deferred.

**Cursor prompt:**
```
Create docs/go-to-market/LINKEDIN_PUBLISHING_SCHEDULE.md:

Weekly cadence (Monday 8:00 AM recommended):
- Week 1: M-10 (The bottleneck nobody talks about)
- Week 2: M-11 (Why "AI for architecture" keeps disappointing teams)
- Week 3: M-12 (extract title from LINKEDIN_CONTENT_V1.md)
- Week 4: M-13 (extract title from LINKEDIN_CONTENT_V1.md)
- Week 5: M-14 (extract title from LINKEDIN_CONTENT_V1.md)
- Week 7: M-15 long-form article (2-week gap for audience warmup)

For each post include:
- Recommended hashtags: max 3-4 from: #EnterpriseArchitecture, #SoftwareArchitecture, #CloudGovernance, #AIGovernance, #ArchitectureReview
- Comment seed: 1 sentence to comment within 30 minutes of posting
- Status: Published (Yes/No date TBD)

Add "Publishing tips" section: post at 8 AM Mon/Tue, reply to comments within 2h on day 1, do not edit post within 30 min, tag max 1 person per post.

Cross-reference LINKEDIN_CONTENT_V1.md for full copy.
```

**Acceptance criteria:**
- File created with all 6 schedule entries.
- Hashtags and comment seeds present for each post.
- Tips section included.

---

## TB-233 — Demo video storyboard from existing demo script

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-05)
**Priority:** P2
**Size:** XS (~1 h)

**Problem:** `DEMO_VIDEO_SCRIPT.md` exists with full script. No video has been produced. The storyboard removes 90% of production decision burden for the person who records the video.
...
- Target length: under 3 minutes for buyer-facing cut
- For each script segment, produce a table row:
  | Segment | URL/Screen | Mouse Action | Narration extract (≤25 words) | Annotation suggestion | Duration (s) |
- Pre-production checklist:
  - [ ] Staging/local env with seeded Contoso demo tenant running
  - [ ] Browser at 100% zoom, full screen, clean bookmark bar
  - [ ] Recording started before narration begins
  - [ ] All non-ArchLucid tabs closed
  - [ ] Audio quality tested
- Post-production checklist:
  - [ ] Trim dead air at start/end
  - [ ] Add title card: "ArchLucid — Defensible architecture, on demand"
  - [ ] Add captions for accessibility
  - [ ] Upload to Loom or Wistia (not YouTube for sales demos — avoids competitor ads)
  - [ ] Add link in PRODUCT_DATASHEET.md and EXECUTIVE_SPONSOR_BRIEF.md after creation
```

**Acceptance criteria:**
- Storyboard table covers every segment in `DEMO_VIDEO_SCRIPT.md`.
- Pre/post production checklists are complete.
- Total estimated duration sums to under 180 seconds.

---

## TB-234 — `SHOULD_YOU_EVALUATE.md` ICP enrichment

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-06)
**Priority:** P2
**Size:** XS (~30 min)

**Problem:** Q2 dead-ends with "Contact us for multi-cloud roadmap" but provides no contact link. No "strong fit signals" section exists to help self-qualifying buyers understand whether they are ideal early customers.
...
1. Add Q5 after Q4:
   ...
   - No → ArchLucid may be early for your team — try a single pilot review to validate fit.
...
2. Replace the 15-minute evaluation path section with:
   ...
   **Self-hosted:** archlucid doctor && archlucid new --quick-scan → review findings (~15 minutes).

3. Add "Strong fit signals" section after the evaluation path:
   ## Strong fit signals
   You are likely a strong fit if:
   - Your last architecture review involved ≥2 weeks of preparation time
   - A compliance gap has surfaced in production rather than during design
   - Your workloads are Azure-primary or planned to be within 6 months
   - You have a formal architecture review board, CAB, or governance committee
   - You need audit-trail evidence for a regulator, insurer, or CTO sign-off
```

**Acceptance criteria:**
- Q5 added with both answer paths.
- Q2 "Contact us" line includes a link to `/contact`.
- "Strong fit signals" section present.

---

## TB-235 — `EXECUTIVE_ONE_EMAIL_KIT.md` — verify or create

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-07)
**Priority:** P2
**Size:** XS (~1 h)

**Problem:** `EXECUTIVE_SPONSOR_BRIEF.md` §Related references `EXECUTIVE_ONE_EMAIL_KIT.md` as "one-email sponsor/procurement copy (subject, ~120-word summary, four-artifact checklist)." The file is not present in the go-to-market directory. A dead link in the executive brief is a credibility gap.

**Cursor prompt:**
```
Check whether docs/go-to-market/EXECUTIVE_ONE_EMAIL_KIT.md exists. If not, create it:

Content:
1. Three subject line variants:
   a. Short: "Architecture review results — [your company name]"
   b. Outcome-first: "Your <<PILOT_OUTCOME_SENTENCE>> — next steps from ArchLucid"
   c. Meeting-request: "15 minutes to review <<N>> architecture findings?"

2. 120-word email body template:
   - Opening: acknowledge pilot context (<<PILOT_CONTEXT>>)
   - 2-sentence ArchLucid summary (use POSITIONING.md §1 tagline)
   - Pilot outcome summary: "In <<N>> architecture reviews, we found <<FINDING_SUMMARY>>."
   - CTA: "I'd welcome 30 minutes to walk through the top findings. [Scheduling link]"
   - Placeholders: <<PILOT_CONTEXT>>, <<N>>, <<FINDING_SUMMARY>>, <<SCHEDULING_LINK>>

3. Four-artifact checklist:
   1. Executive Sponsor Brief PDF (generate from docs/go-to-market/EXECUTIVE_SPONSOR_BRIEF.md)
   ...
   4. ROI estimate (PILOT_ROI_MODEL.md §5 benchmark)

4. Follow-up timing: 48h first follow-up, 5-day second follow-up

Cross-reference EXECUTIVE_SPONSOR_BRIEF.md as canonical sponsor narrative.
```

**Acceptance criteria:**
- File exists with all four sections.
- All `<<...>>` placeholders are documented with fill instructions.
- Cross-reference link to `EXECUTIVE_SPONSOR_BRIEF.md` present.

---

## TB-236 — DEFERRED: Demo video production

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-08)
**Priority:** P1 (owner action)
**Status:** DEFERRED — requires owner screen recording and voiceover. Storyboard (TB-233) is a prerequisite. No Cursor action until storyboard is complete and owner is ready to record.

**Pending question PQ-MKT-03:** Who records the demo video, with which tool (Loom/Camtasia/OBS), and is voiceover overlay acceptable or must it be a live recording?

---

## TB-237 — Discount stack buyer-facing framing in `/pricing` UI

**Source:** Marketability quality assessment (`docs/assessments/Marketability_06022026.MD` — MKT-09)
**Priority:** P2
**Size:** S (~1–2 h)

**Problem:** ArchLucid's pricing is 50% below fair value due to the trust/reference/self-serve discount stack. Buyers who notice the aggressive pricing may read it as a distress signal rather than an early-adopter benefit. A 2-sentence buyer-visible framing resolves this ambiguity.

**Cursor prompt:**
```
1. In the archlucid-ui pricing page component (archlucid-ui/src/app/(marketing)/pricing/ or equivalent):
   Locate the tier grid section and add below it:

   <PricingPhilosophyNote> component that renders:
   "**Early adopter pricing.** Current prices are set at approximately 50% of fair value to reward
   early adopters who help shape the product. Prices increase as we publish independent security
   attestations and customer case studies — your rate is locked for your initial subscription term."

   Style: muted text (--al-text-secondary), max-width 600px, centered, margin-top: 24px.

2. In docs/go-to-market/PRICING_PHILOSOPHY.md §1 Pricing principles table, add a new row:
   | **Transparent early-access framing** | Buyers who understand why the price is low are more likely to convert. The −50% discount stack is an honest early-adopter signal, not a distress indicator. Surfacing this in the UI removes the implicit "why so cheap?" objection before it is raised. |

3. Add unit test for PricingPhilosophyNote: renders with correct text, uses muted text class.
```

**Acceptance criteria:**
- Pricing page renders the early-adopter note below the tier grid.
- `PRICING_PHILOSOPHY.md §1` has the new principle row.
- Unit test passes.
