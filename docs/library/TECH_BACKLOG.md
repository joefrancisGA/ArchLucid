...
|----|-------|----------------|------|
|| TB-196 | Reasoning token cost underreporting fix — update `AgentExecutionTraceRunLlmCostAggregator.Compute()` to pass `trace.ReasoningTokenCount ?? 0` (not literal `0`) to `costEstimator.EstimateUsd`; add test covering reasoning-token cost path | Correctness P0 — silent monetary underreporting for reasoning-model deployments since TB-033 added the column; fix is one-line in aggregator + test | XS |
| TB-197 | `GovernanceWorkflowService.PromoteAsync` — wrap `approvalRepo.UpdateAsync` + `promotionRepo.CreateAsync` in single `IArchLucidUnitOfWork`; add atomicity test | Correctness P1 — current code can leave approval permanently `Promoted` with no promotion record if `CreateAsync` fails; irrecoverable without manual DB repair | S |
| TB-198 | `CostConstraintFindingEngine` unit tests — add 7 scenarios covering severity, budget parsing, null properties, empty graph, multi-node | Correctness P1 — 0% coverage on cost-engine business logic (14.75% assembly line coverage) | S |
| TB-199 | `TenantOrProjectCapabilityAuthorizationHandler` unit tests — 6 authorization scenarios (succeed/fail/no-identity/tenant-miss/project-scope) | Correctness P1 — 0% coverage on security-critical 98-line authorization handler | M |
| TB-200 | `HttpScopeContextProvider` unit tests — 5 scenarios (valid claims, missing tenant, missing workspace, unauthenticated, malformed GUIDs) | Correctness P1 — 0% coverage on HTTP auth scope boundary; scope drives every downstream tenant query | S |
...
| TB-205 | `BuildHistoryAsync` / `BuildExportAsync` / `CollectCommittedRunsForTrendsAsync` pagination safety — add `maxPages = 500` guard + warning log to all three `while(true)` pagination loops in `ExecutiveRoiSummaryService` | Correctness P2 — infinite loop risk if `ListRunSummariesKeysetAsync` returns `hasMore=true` indefinitely | XS |
| TB-206 | `ResolveEnvironmentLabel` null guard — null-coalesce `service.Tags ?? []`; add unit tests for null tags, empty tags, valid env tag | Correctness P3 — `NullReferenceException` if `ManifestService.Tags` is null | XS |
