...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-09-03
- **last-bug:** 2026-09-03 — `on`/`off` boolean-synonym `schemaVersion` rejected in extractor manifest upgrader
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `AzureExtractorManifestSchemaUpgrader.TryReadSchemaVersion` — `on`/`off` boolean-synonym strings rejected — **hit 2026-09-03 (#617):** `"schemaVersion":"off"` failed upgrade from v0 while `"schemaVersion":"on"` failed current-v1 acceptance; private `TryParseBooleanString` used bare `bool.TryParse` instead of shared synonym coercion already used by `AzureExtractorPackageZipValidator`; fixed by delegating to `RunExplanationAggregateJsonReader.TryParseBooleanString` (`TryUpgradeManifestJson_upgrades_off_synonym_for_legacy_zero_schema_version`, `TryUpgradeManifestJson_accepts_on_synonym_for_current_schema_version`).

2026-09-03 seed hunt #617: reseeded from `AzureExtractorManifestSchemaUpgrader`; proved on/off boolean-synonym schemaVersion parity gap after #595 string-zero/PascalCase fix.
