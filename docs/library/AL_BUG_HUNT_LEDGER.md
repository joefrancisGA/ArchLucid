...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string — persisted JSON with numeric `1` failed deserialization or left `NotRequired`; fixed 2026-08-25 via `ReadHumanReviewStatus` accepting string or defined ordinal (`FindingJsonConverterTests`).
