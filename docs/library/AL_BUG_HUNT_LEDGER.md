...
- [x] (proven) `ContractJson.Default` registered `JsonStringEnumConverter<AgentType>` instead of `AgentTypeJsonConverter`, so ordinal `99` deserialized on canonical contract presets while dedicated converter tests reject it — fixed 2026-08-25 (`ContractJsonDefaultAgentTypeTests`).
