...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — `FindingJsonConverter` numeric `humanReviewStatus` ordinals
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string; persisted JSON with numeric `1` leaves the default `NotRequired`, silently downgrading pending review state on round trip — **hit 2026-08-25:** numeric `humanReviewStatus` called `GetString()` and failed deserialization (or skipped assignment); fixed with `ReadHumanReviewStatus` accepting defined ordinals and strings (`FindingJsonConverterTests.Deserialize_numeric_humanReviewStatus_maps_to_pending`).
