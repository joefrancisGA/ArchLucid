...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — `RecentPilotRunDeltasService` treated `CurrentManifestVersion` as committed; ReadyForCommit runs inflated before/after panel medians
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `RecentPilotRunDeltasService.IsCommitted` keyed on `CurrentManifestVersion` while `PilotScorecardBuilder` requires `LegacyRunStatus == Committed` — **hit 2026-08-25:** ReadyForCommit runs with manifest version appeared in recent-deltas medians (`GetRecentDeltasAsync_ReadyForCommitRunWithManifest_IsExcludedFromCommittedPanel`).
- [ ] (hunt-ready) `PilotValueReportService.IsCommittedSummary` ORs `Status == Committed` with non-empty `CurrentManifestVersion` — ReadyForCommit runs with manifest version inflate `TotalRunsCommitted` and committed-run timeline the same way scorecard/recent-deltas disagreed before 2026-08-25 partial fix.
