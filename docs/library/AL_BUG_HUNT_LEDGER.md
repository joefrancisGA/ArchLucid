...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-29
- **last-bug:** 2026-08-29 — `InMemorySelfServiceTrialAbuseRepository` email-claim keys were case-sensitive vs SQL CI PK
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `InMemorySelfServiceTrialAbuseRepository` email-claim dictionary — mixed-case `NormalizedEmail` keys bypassed lifetime cap in dev/test (`HasEmailClaimAsync` / `TryInsertEmailClaimAsync` used `StringComparer.Ordinal` while SQL `PK_PlatformSelfServiceTrialEmailClaims` is CI) — **hit 2026-08-29 (#768, supersedes #603):** `_emailClaims` uses `OrdinalIgnoreCase`; regression in `HasEmailClaimAsync_treats_email_keys_as_case_insensitive_like_sql_pk` and `EvaluateAsync_denies_repeat_email_when_only_casing_differs`.

- [ ] (hunt-ready) `InMemoryEmailOtpChallengeRepository` — `InvalidateActiveChallengesForEmailAsync` iterates `_byId` outside `_completionLock` while `TryCompleteAsync` completes under the lock, so concurrent invalidate+verify can return `Completed` for a challenge that should be `Invalidated` (`InMemoryEmailOtpChallengeRepository.cs` L122–141 vs L144–215).

- [ ] (hunt-ready) `InMemoryPlatformUserRepository.InsertAsync` — duplicate explicit `Id` silently overwrites the existing row (`_byId[row.Id] = row`) instead of surfacing SQL PK violation like `DapperPlatformUserRepository` (`InMemoryPlatformUserRepository.cs` L38; `DapperPlatformUserRepository.cs` L40–52).

2026-08-29 seed hunt #170 (supersedes #768/#603): proved self-service trial abuse email-claim casing parity gap; reseeded OTP invalidate/complete race and platform-user duplicate-insert parity candidates.
...
- [x] (proven) `FindingJsonConverter.EnumReaders` string paths for `severity`, `enforcementTier`, `humanReviewStatus`, `treatment`, and `classification` accept numeric-string `"99"` via bare `Enum.TryParse` while JSON number paths guard with `Enum.IsDefined` — **hit 2026-08-29:** post-refactor partial split preserved the gap on top-level string tokens; fixed with `Enum.IsDefined` after `TryParse`; regressions in `Deserialize_severity_numeric_string_out_of_range_throws`, `Deserialize_enforcementTier_numeric_string_out_of_range_throws`, `Deserialize_humanReviewStatus_numeric_string_out_of_range_throws`.
- [x] (proven) `FindingEnforcementTierClassifier.TryReadTierFromProperties` accepts undefined `properties.enforcementTier` via bare `Enum.TryParse` while `ReadEnforcementTierFromString` guards numeric strings — **hit 2026-08-29:** numeric-string `"99"` short-circuited classification; fixed with `Enum.IsDefined` guard (`ClassifyFinding_ignores_undefined_enforcement_tier_property_value`).
...
- **last-bug:** 2026-08-28 — narrative/DOCX parity and DOCX posture-string sanitization
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `ArchitectureNarrativeArtifactGenerator` omitted `manifest.Decisions` — reference-architecture and DOCX included decisions while narrative jumped to unresolved issues — **hit 2026-08-28:** emit `## Decisions` section (reference-architecture parity); regression in `ArchitectureNarrativeArtifactGenerator_GenerateAsync_emits_committed_decisions`.
- [x] (proven) `MermaidDiagramArtifactGenerator` ignored typed golden topology — AST built only from `manifest.Decisions`, not `Topology.Services` / `Datastores` / `Relationships` — **hit 2026-08-28:** materialize service/datastore nodes and relationship edges; regression in `MermaidDiagramArtifactGeneratorTests.GenerateAsync_includes_typed_topology_services_datastores_and_relationships`.
- [x] (proven) `DocxExportService.BuildDocumentAsync` omitted Constraints and Assumptions sections present in markdown artifacts — **hit 2026-08-28:** emit Assumptions and Constraints headings with manifest content; regression in `DocxExportServiceGoldenTests.ExportAsync_emits_assumptions_and_constraints_sections`.
- [x] (proven) `DocxExportService` skipped `LlmArtifactFreeTextSanitizer` on manifest posture strings (topology gaps, security/compliance gaps, cost risks) — **hit 2026-08-28:** route posture strings through `SanitizeArtifactText`; regression in `DocxExportServiceGoldenTests.ExportAsync_sanitizes_topology_gap_bidi_override_characters`.
...
2026-08-28 thorough hunt #157: proved narrative decisions parity, mermaid typed-topology materialization, DOCX assumptions/constraints sections, and DOCX posture-string sanitization; zone candidate backlog cleared.
...
- [x] (invalid) `GovernanceController.DryRunPolicyPack` — `pageSize`/`page` silently clamped instead of HTTP 400 — **cheap-disproof 2026-08-29:** intentional owner Q38 server clamp in `IPolicyPackDryRunService` / `PolicyPackDryRunService`; regression in `GovernanceControllerSimulateTests.DryRunPolicyPack_delegates_page_size_to_service_for_documented_server_side_clamp` and `PolicyPackDryRunServiceTests.EvaluateAsync_ClampsPageSizeToServerMaximum`.
...
- [x] (proven) `GovernanceController.BatchReviewApprovalRequests` — case-variant duplicate `approvalRequestIds` (`apr-1` / `APR-1`) double-processed because dedupe `HashSet` used `Ordinal` comparer — **hit 2026-08-29 (#246):** `StringComparer.OrdinalIgnoreCase` dedupe (`RecordBulkDisposition` parity); regression in `BatchReviewApprovalRequests_returns_validation_failed_per_item_when_list_contains_case_variant_duplicate_id`.
- [x] (invalid) `GovernanceController.DryRunProposedPolicyPack` — sibling dry-run endpoint may accept duplicate run/manifest id lists without controller dedup guard — **cheap-disproof 2026-08-29 (#246):** endpoint accepts single `targetRunId` / `targetManifestId`, not id lists; validator rejects both targets set.
- [x] (proven) `PolicyPacksController.SimulateBulk` — case-variant duplicate `runIds` silently deduped — **hit 2026-08-29 (#248):** `OrdinalIgnoreCase` duplicate guard (`SimulateBulk_returns_bad_request_when_run_ids_differ_only_by_case`).
- [x] (proven) `GovernanceStickinessController.RecordBulkDisposition` — mixed `findingIds` with whitespace-only entries silently dropped in facade when at least one in-scope id processed — **hit 2026-08-29 (#250):** controller rejects any whitespace id before facade call; strengthened regression with in-scope finding mock in `RecordBulkDisposition_returns_bad_request_when_mixed_finding_ids_include_whitespace`.
...
2026-08-29 seed hunt #197 (hit): proved simulate-bulk null `runIds` and dry-run null `evaluateAgainstRunIds` HTTP 400 guards (batch-review list-null parity).

2026-08-29 thorough hunt #196 (hit): proved bulk-disposition duplicate findingId HTTP 400 guard and dry-run duplicate evaluateAgainstRunIds dedupe (SimulateBulk parity); zone candidate backlog cleared.

2026-08-29 thorough hunt #195 (hit): proved batch-review duplicate-id per-item validation; cheap-disproved compare-manifest padded-query candidate.
