...
- [x] (proven) `TerraformShowJsonInfrastructureDeclarationParser.TryAddResource` copied `values` without `ShouldRedactKey(prop.Name)` — **hit 2026-08-26:** `admin_secret` plaintext landed in `tf.admin_secret` when `sensitive_values` was absent; fixed by key-name redaction before `CanonicalInfrastructureJsonValue.CanonicalizeText` (`TerraformShowJsonInfrastructureDeclarationParserTests.ParseAsync_redacts_top_level_sensitive_key_without_sensitive_values`).
- [x] (proven) `TerraformShowJsonInfrastructureDeclarationParser` nested JSON serializer omitted key-name redaction without `sensitive_values` — **hit 2026-08-26:** `site_config.connection_string` leaked inside `tf.site_config` blob; fixed by routing show-json values through shared `CanonicalInfrastructureJsonValue` (`TerraformShowJsonInfrastructureDeclarationParserTests.ParseAsync_redacts_nested_sensitive_json_key_without_sensitive_values`).
