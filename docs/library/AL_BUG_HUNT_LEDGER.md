...
- [x] (proven) `deriveOperatorHomeWorkspaceMetrics` treats a paginated runs page as workspace-wide KPI totals — **hit 2026-08-25:** when `totalCount` exceeds loaded `items`, open findings and committed/active counts were summed from the first page only; fixed by skipping page-local aggregates until every run row is loaded (`operator-home-workspace-metrics.test.ts`).
