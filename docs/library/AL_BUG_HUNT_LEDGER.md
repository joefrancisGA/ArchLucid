...
- [ ] (hunt-ready) `PilotValueReportService.IsCommittedSummary` ORed `Status == Committed` with non-empty `CurrentManifestVersion` (same predicate shape as pre-fix scorecard/recent-deltas).
