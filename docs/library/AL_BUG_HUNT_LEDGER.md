...
- [x] (invalid) `ManifestsController.CompareManifests` — padded `leftVersion` / `rightVersion` query params may 404 despite `GetManifestInScopeAsync` trim parity — **cheap-disproof 2026-08-28:** `LoadAndCompareManifestPairAsync` delegates to `GetManifestInScopeAsync` which trims before lookup (summary/export padded regressions already existed); added `ManifestsControllerTests.CompareManifests_returns_ok_with_diff_when_query_params_are_padded`.

2026-08-28 thorough hunt #193 (hit): proved batch-review duplicate-id per-item validation; cheap-disproved compare-manifest padded-query candidate; zone candidate backlog cleared.
