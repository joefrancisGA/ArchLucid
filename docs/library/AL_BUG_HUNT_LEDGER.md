...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — golden fixture on deep-linked review reset product context to idle and showed "Scoped to run"
- **related-pd-tb:** none
- **code-changed-since:** no
...
- **last-bug:** 2026-08-28 — negated intent phrase false premise-conflict signals
- **related-pd-tb:** none
- **code-changed-since:** yes
...
2026-08-28 thorough hunt #210 (hit): proved negated intent phrase false premise-conflict signals, sub-threshold PROTECTS edge severity downgrade, and portfolio recurrence missing in-flight current identities on first pass.

- [x] (proven) `DeclarationPremiseConflictClassifier.IntentMatchesConflictKind` with negated intent phrases (`"do not disable public"`) — substring phrase list matched inside negated requirements and emitted false premise-conflict signals — **hit 2026-08-28:** skip phrase matches immediately preceded by `do not` / `don't` / `never`; regression in `DeclarationPremiseConflictClassifierTests.Classify_does_not_fire_private_network_conflict_for_negated_disable_public_phrase`.
- [x] (proven) `DeclarationPremiseConflictFindingEngine.ResolveApplicableIntentNodes` with PROTECTS/APPLIES_TO edge weight just below `GraphEdgeDecisioningThresholds.MinWeightForSemanticLink` — sub-threshold edges were dropped and graph-wide intent fallback downgraded severity from Error to Warning — **hit 2026-08-28:** treat sub-threshold explicit intent edges as narrow applicability before graph-wide fallback; regression in `DeclarationPremiseConflictFindingEngineTests.AnalyzeAsync_emits_error_when_protects_edge_weight_is_just_below_semantic_link_threshold`.
- [x] (proven) `PortfolioRecurrenceFindingEngine.ResolveCurrentScopeIdentities` when the current system's persisted findings snapshot is empty on first pass — recurrence scan only read persisted snapshots, missing cross-system identities during in-flight generation — **hit 2026-08-28:** defer `portfolio-recurrence` until after primary engines, supply in-flight identities via `IPortfolioRecurrenceCurrentReviewIdentitySource`, and count current system from in-flight identities when persisted snapshot is absent; regression in `PortfolioRecurrenceFindingEngineTests.AnalyzeAsync_when_current_snapshot_missing_uses_in_flight_identities` and orchestrator phased invoke.
- [ ] (candidate) `DeclarationPremiseConflictClassifier` — negation guard covers `do not` / `don't` / `never` only; other negation forms (`"must not block public"`) may still false-match phrase list.
- [ ] (candidate) `PortfolioRecurrenceFindingEngine` — in-flight identity merge uses representative finding from first persisted system only; current-only identities with no portfolio snapshot row never qualify.
...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-28
- **last-bug:** 2026-08-28 — `GovernanceController.BatchReviewApprovalRequests` case-variant duplicate `approvalRequestIds` lacked per-item `ValidationFailed` row
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `GovernanceController.BatchReviewApprovalRequests` — duplicate non-whitespace `approvalRequestIds` silently deduped with no per-item result row — **hit 2026-08-28 (#252):** emit per-item `ValidationFailed` for duplicate ids while processing first occurrence; `StringComparer.OrdinalIgnoreCase` dedupe (`RecordBulkDisposition` parity); regression in `BatchReviewApprovalRequests_returns_validation_failed_per_item_when_approval_request_id_is_duplicated` and `..._case_variant_duplicate_id`.
- [x] (invalid) `ManifestsController.CompareManifests` — padded `leftVersion` / `rightVersion` query params may 404 despite `GetManifestInScopeAsync` trim parity — **cheap-disproof 2026-08-28 (#252):** `LoadAndCompareManifestPairAsync` delegates to `GetManifestInScopeAsync` which trims before lookup; sibling regression in `CompareManifestsSummary_returns_trimmed_manifest_versions_when_query_params_are_padded` and `CompareManifestsExport_returns_trimmed_manifest_versions_when_query_params_are_padded`.
...
2026-08-28 thorough hunt #252: cheap-disproved picker `CompareManifests` padded-version candidate; added case-variant duplicate-id regression and aligned duplicate message text.

2026-08-28 thorough hunt #232: proved risk-exception and bulk-disposition whitespace `findingId` guards; re-shipped batch-review duplicate-id fix (#222 carry); cheap-disproved manifest-compare padded-version (#222 carry).
...
- [x] (proven) `GovernanceController.Approve` / `Reject` — workflow `InvalidOperationException` returned `ProblemTypes.BadRequest` while submit/promote use `ValidationFailed` — **hit 2026-08-28:** map workflow `InvalidOperationException` to `ValidationFailed` on approve and reject; regression in `GovernanceControllerRunHistoryScopeTests`.
- [x] (proven) `GovernanceController.Activate` — workflow `ArgumentException` validation failures were uncaught (HTTP 500) while promote/submit map workflow validation exceptions to HTTP 400 — **hit 2026-08-28:** catch workflow `ArgumentException` and return `ValidationFailed`; regression in `GovernanceControllerRunHistoryScopeTests`.
- [x] (invalid) `GovernanceController.Activate` — workflow `InvalidOperationException` catch missing — **cheap-disproof 2026-08-28:** `GovernanceWorkflowActivateStage` throws `RunNotFoundException`, `GoldenManifestVersionNotFoundException`, and `ArgumentException` only; no `InvalidOperationException` path.
...
- [x] (proven) `GovernanceController.GetComplianceDriftTrend` — range validation errors (`fromUtc`/`toUtc`/`bucketMinutes`) returned `ProblemTypes.BadRequest` while sibling governance query caps use `ValidationFailed` — **hit 2026-08-28:** align drift-trend query validation problem type with dashboard caps; regression in `GovernanceControllerDashboardTests`.
- [x] (invalid) `GovernancePreviewController.Preview` — `InvalidOperationException` from preview service surfaces `ProblemTypes.BadRequest` while `ArgumentException` validation errors use `ValidationFailed` — **cheap-disproof 2026-08-28:** `GovernancePreviewService.PreviewActivationAsync` and preview dependencies throw `ArgumentException`/`RunNotFoundException`/`GoldenManifestVersionNotFoundException` only; `InvalidOperationException` catch is defensive with no reachable preview activation path.

2026-08-28 thorough hunt #202 (hit): cheap-disproved compare-same-env BadRequest candidate (already ValidationFailed); cheap-disproved padded-valid-guid not-found candidate; proved pre-commit malformed run id error detail trim parity; seeded governance insights BadRequest vs ValidationFailed and preview InvalidOperationException problem-type candidates.

- [x] (invalid) `GovernancePreviewController.CompareEnvironments` — `ArgumentException` for identical source/target after normalization surfaces `ProblemTypes.BadRequest` while sibling validation inputs use `ValidationFailed` — **cheap-disproof 2026-08-28:** controller catch block already maps `ArgumentException` to `ValidationFailed`; regression in `GovernancePreviewControllerUnitTests.CompareEnvironments_returns_validation_failed_when_source_equals_target`.
- [x] (invalid) `GovernancePreCommitSimulationController.GetChecklistAsync` — padded valid guid route value may echo untrimmed `runId` in error detail while parse path trims before lookup — **cheap-disproof 2026-08-28:** out-of-scope not-found uses `runIdNormalized` in detail; regression in `GovernancePreCommitSimulationControllerTests.GetChecklist_returns_not_found_with_trimmed_run_id_when_padded_valid_guid_out_of_scope`.
- [x] (proven) `GovernancePreCommitSimulationController.GetChecklistAsync` / `SimulateAsync` — padded malformed run id echoed whitespace in validation error detail while parse path trims before lookup (`RequireScopedRunAsync` trim parity gap) — **hit 2026-08-28:** trim run id in malformed-parse error detail; regression in `GovernancePreCommitSimulationControllerTests`.
...
- [x] (proven) `GovernanceController.SubmitApprovalRequest` / `LogGovernanceApprovalRequestedAuditAsync` — padded `manifestVersion` may echo into `GovernanceApprovalRequested` audit `DataJson` while workflow trims for processing (run-id audit trim parity gap) — **hit 2026-08-28:** trim manifest/environment fields in audit payload (run-id trim parity); regression in `GovernanceControllerRunHistoryScopeTests`.
- [x] (invalid) `GovernanceController.DryRunPolicyPack` — out-of-range `pageSize`/`page` query params silently clamped in service instead of HTTP 400 (`GetDashboard` rejects invalid caps explicitly) — **cheap-disproof 2026-08-28:** intentional Q38 server-clamp documented on `IPolicyPackDryRunService`; controller delegates to service; regression in `GovernanceControllerSimulateTests.DryRunPolicyPack_delegates_page_size_to_service_for_documented_server_side_clamp` and `PolicyPackDryRunServiceTests.EvaluateAsync_ClampsPageSizeToServerMaximum`.
