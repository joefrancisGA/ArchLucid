...
- [x] (valid-no-repro) `AzureExtractorManifestSchemaUpgrader.TryUpgradeManifestJson` — string `"true"` / boolean `true` schemaVersion at current version — #612 coercion maps boolean synonyms to v1; current-version path returns true without mutation (`TryUpgradeManifestJson_accepts_on_synonym_for_current_schema_version`; boolean `true` same code path).
- [x] (invalid) `PrivateNetworkAddressGuard.IsForbiddenIpAddress` / `IsForbiddenHostLiteral` — decimal IPv4 literals (`3232235777`) — `IPAddress.TryParse` accepts decimal notation on .NET 10 and `IsForbiddenIpAddress` already blocks mapped private address (#628 cheap-disproof).
...
- [x] (proven) `CloudInventoryExtractorPackageZipValidator.Validate` — zip-slip / unsafe entry paths accepted — **hit 2026-09-03 (#628):** sibling `AzureExtractorPackageZipValidator` calls `ZipArchiveSafety.ValidateArchive` before manifest lookup; AWS/GCP cloud-inventory validator skipped safety checks and returned `IsValid = true` for archives containing `../outside/evil.txt`; fixed with shared `ZipArchiveSafety` gate (`Validate_rejects_zip_slip_entry_paths`).
- [x] (invalid) `RunAuthorityPipelineDeadLetterDetection.TryDeserialize` — boolean / string-boolean `schemaVersion` rejected — duplicate of #616; boolean/`on` synonyms already accepted.
...
- [ ] (candidate) `TeamsNotificationTriggerCatalog.IsKnown` — uppercase canonical `com.archlucid.*` trigger names rejected — `AllSet` uses `StringComparer.Ordinal`; cheap-disproof before repro.

2026-09-03 seed hunt #628: reseeded cloud-inventory ZIP safety parity; proved zip-slip acceptance gap; cheap-disproved upgrader boolean-at-current-version, decimal IPv4, and stale dead-letter boolean candidate; added graph-reuse trim and Teams trigger casing candidates.
