...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — AgentResult structured claim evidenceRefs dropped at deserialize
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `AgentResultClaimListJsonConverter` flattens structured claim text but ignores an entry-level `evidenceRefs` array — **hit 2026-08-25:** `{"detail":"Subnet missing","evidenceRefs":["pol-123"]}` lost linkage at parse time; fixed with `AgentResultJsonConverter.MergeClaimEvidenceRefs` (`AgentResultClaimListJsonConverterEvidenceRefsTests.Deserialize_merges_structured_claim_evidence_refs_into_result_evidence_refs`).
