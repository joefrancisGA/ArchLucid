...
- [ ] (hunt-ready) `RunHeaderAnchorJsonComparer.TryAbsentEmptyObjectEquivalent` — empty JSON object property vs omitted property treated as anchor mutation — locus `TryCrossKindEquivalent` / `PropertiesEquivalent`; input `{"scope":{}}` vs `{}`; wrong outcome `RunEvidenceAnchorImmutableException` on committed reload; mechanism #895 added `TryAbsentEmptyStringEquivalent` only, not empty-object↔absent bridge; repro `EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_object_equivalent_to_omitted_on_committed_run` (probe seed #896).
- [ ] (hunt-ready) `RunHeaderAnchorJsonComparer.TryAbsentEmptyArrayEquivalent` — empty JSON array property vs omitted property treated as anchor mutation — locus `TryCrossKindEquivalent` / `PropertiesEquivalent`; input `{"packAssignments":[]}` vs `{}`; wrong outcome `RunEvidenceAnchorImmutableException`; mechanism #888 null↔empty-array bridge skips absent↔`[]`; repro `EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_array_equivalent_to_omitted_on_committed_run` (probe seed #896).
- [ ] (hunt-ready) `ConfigurationSensitiveConfigPathMatcher` — `Certificate` segment not redacted vs `AzureExtractorSensitivePropertyRedactor` `certificate` fragment — locus `IsSensitiveConfigSegment` / `IsExplicitCredentialConfigSegment`; input `ArchLucid:Auth:SigningCertificatePath` or `ArchLucid:Saml:SigningCertificate`; wrong outcome operator summary exposes PFX path literals; mechanism #895 credential parity added `ClientSecret`/`PrimaryKey`/`AccountKey` only; repro `Resolve_redacts_certificate_config_paths` (probe seed #896).
- [ ] (hunt-ready) `ConfigurationSensitiveConfigPathMatcher` — `SigningKey` segment not redacted — locus `IsSensitiveConfigSegment`; input `ArchLucid:Auth:SigningKey`; wrong outcome operator summary exposes signing key material; mechanism segment lacks `SigningKey` explicit credential and no `Key` suffix on multi-part paths; repro `Resolve_redacts_signing_key_config_path` (probe seed #896). (`SecondaryKey` already covered — valid-no-repro probe #896.)
- [ ] (hunt-ready) `RequestConstraintTokenMatcher.IsNegatedPhraseSuffix` — post-phrase `need not` / `cannot` suffix negation gap — locus `IsNegatedPhraseSuffix`; input `managed identity need not be used for storage`, `private networking need not be required…`, `encryption need not be used at rest`, `cannot use managed identity for storage`; wrong outcome `RequestConstraintClassifier` false positives on starter evidence refs; mechanism #893–#895 suffix list covers `not necessary` / `isn't required` but not `need not` or `cannot`; repro `HasManagedIdentityConstraint_does_not_false_positive_on_need_not_managed_identity_phrasing`, `HasPrivateNetworkingConstraint_does_not_false_positive_on_need_not_private_networking_phrasing`, `HasEncryptionConstraint_does_not_false_positive_on_need_not_encryption_phrasing`, `HasManagedIdentityConstraint_does_not_false_positive_on_cannot_use_managed_identity_phrasing` (probe seed #896).
- [ ] (hunt-ready) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `won't need` / `must not` suffix gap — locus `IsObviousGenericAdvice` imperative branch; input `enable mfa won't need to be configured…`, `use https must not be required…`; wrong outcome obvious-advice demotion on negated checklist phrasing; mechanism #895 suffix parity on fragment path missed imperative-regex matches; repro `IsObviousGenericAdvice_does_not_flag_wont_need_or_must_not_suffix_phrasing` (probe seed #896).
- [ ] (hunt-ready) `DeclarationSecurityPropertyKeyResolver.TryGet` — missing `tf.network_acls` snake_case alias for `IngressBlob` — locus `LogicalNameToCandidateKeys[IngressBlob]`; input property bag `tf.network_acls` from Terraform HCL `network_acls` blocks (`TryAddTfBlockProperty` preserves underscores, sibling `tf.public_network_access`); wrong outcome open-admin ingress heuristic misses Key Vault / storage network ACL bags; mechanism #892–#893 added compact `tf.networkacls` / `tf.ip_security_restrictions` only; repro `TryGet_resolves_tf_network_acls_for_ingress_blob` (probe seed #896).

2026-09-05 seed hunt #896 (seed-only): reseeded after #895 closure; promoted seven hunt-ready rows with failing repro probes on `816caf32fd` — anchor absent↔empty object/array, Certificate/SigningKey config redaction parity, constraint `need not`/`cannot` suffix negation, advice imperative suffix negation, and `tf.network_acls` resolver alias.
