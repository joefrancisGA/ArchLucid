...
- [x] (proven) `PilotValueReportService.IsCommittedSummary` ORs `Committed` status with manifest version — **hit 2026-08-25:** `ReadyForCommit` runs with `CurrentManifestVersion` inflated `TotalRunsCommitted`; fixed to require `RunSummary.Status == Committed` (`PilotValueReportServiceTests.BuildAsync_ReadyForCommitRunWithManifest_IsNotCountedAsCommitted`).
