...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — buyer proof ZIP used PascalCase deltas JSON and omitted extractor timestamp for ROI freshness
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `BuyerProofPackBuilder` serialized deltas with PascalCase JSON and omitted extractor timestamp — **hit 2026-08-25:** default `JsonSerializer.Serialize` failed `BuyerProofPackCommitGuard` camelCase checks (ZIP returned null) and `extractorCollectionTimestampUtc: null` made stale extractor ROI freshness PASS vs API HOLD; fixed via `BuyerProofPackDeltasJsonFormatter` + `RoiCostEvidenceCollectionResolver` (`BuyerProofPackDeltasJsonFormatterTests`).
