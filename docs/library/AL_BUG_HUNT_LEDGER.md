...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — FindingJsonConverter ignored numeric humanReviewStatus ordinals
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string — **hit 2026-08-25:** persisted JSON with numeric `1` left default `NotRequired` because `GetString()` was invoked on a number token; fixed with `ReadHumanReviewStatus` accepting defined ordinals (`FindingJsonConverterTests.Deserialize_numeric_humanReviewStatus_maps_pending`).
