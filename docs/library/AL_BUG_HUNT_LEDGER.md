...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string — **hit 2026-08-25:** persisted JSON with numeric `1` threw on `GetString()` and left default `NotRequired`; fixed with `ReadHumanReviewStatus` accepting defined ordinals and enum names (`FindingJsonConverterTests.Deserialize_numeric_humanReviewStatus_maps_pending`).
