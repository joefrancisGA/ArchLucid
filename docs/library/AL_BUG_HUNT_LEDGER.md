...
2026-08-31 seed hunt #376 (hit): reseeded from zone files; proved ghost-workspace posture/coverage reads and product-feedback omitted score on master; seeded environment-catalog ghost-tenant, empty `projectId` validation, and checklist `isCompleted` omission candidates.
