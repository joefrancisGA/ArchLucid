...
- [x] (proven) `CanonicalInfrastructurePropertyBag.TryAddTfJsonProperty` preserved array/object `tf.*` JSON via `GetRawText()` — **hit 2026-08-26:** inner key casing (`Environment` vs `environment`) false-modified arm-json infra declaration deltas; fixed with shared `CanonicalInfrastructureJsonValue` canonical rewrite (`ArmJsonInfrastructureDeclarationParserTests.ParseAsync_CanonicalizesComplexTfJsonCasing`).
- [x] (proven) `CanonicalInfrastructurePropertyBag.TryAddTfJsonProperty` redacted only top-level sensitive ARM property names — **hit 2026-08-26:** nested `siteConfig.connectionString` leaked plaintext inside `tf.siteconfig` blob; fixed by redacting nested sensitive keys during canonical JSON serialization (`ArmJsonInfrastructureDeclarationParserTests.ParseAsync_redacts_nested_sensitive_object_values`).
- [x] (proven) `CanonicalInfrastructurePropertyBag.TryAddTfBlockProperty` stored nested HCL block bodies without sensitive-key redaction — **hit 2026-08-26:** `site_config { connection_string = "..." }` leaked password text in `tf.site_config`; fixed with `RedactSensitiveAssignmentsInHclBody` (`SimpleTerraformDeclarationParserTests.ParseAsync_redacts_sensitive_nested_block_assignments`).
- [x] (proven) `SimpleTerraformResourceBlockParser.ScalarAssignmentRegex` captured trailing `#` comments in scalar values — **hit 2026-08-26:** `location = "eastus" # primary` false-modified infra declaration deltas; fixed by stripping HCL comments before unquoting (`SimpleTerraformDeclarationParserTests.ParseAsync_StripsTrailingCommentsFromScalarAssignments`).
- [x] (proven) `KubernetesYamlInfrastructureDeclarationParser` split multi-doc manifests only on exact `\n---` tokens — **hit 2026-08-26:** leading-space document separators (` ---`) treated file as one YAML doc and dropped trailing resources; fixed with `^\s*---\s*$` split (`KubernetesYamlInfrastructureDeclarationParserTests.ParseAsync_MultiDocWithPaddedDocumentSeparator_MapsAllDocuments`).

- [ ] (hunt-ready) `TerraformShowJsonInfrastructureDeclarationParser.TryAddResource` copies `values` without `CanonicalInfrastructurePropertyBag.ShouldRedactKey(prop.Name)` — input: terraform-show-json resource with `values.admin_secret: "hidden"` and no `sensitive_values` sidecar — wrong outcome: plaintext lands in `tf.admin_secret` (and false modified if a later export adds `sensitive_values`) — mechanism: unlike arm-json `CopyBoundedProperties` → `TryAddTfProperty`, show-json calls `CanonicalizeTerraformValueText` on every value key and only redacts via `RedactTopLevelSensitiveTfValues` when `sensitive_values` marks the key — test: `TerraformShowJsonInfrastructureDeclarationParserTests.ParseAsync_redacts_top_level_sensitive_key_without_sensitive_values`, `InfrastructureDeclarationConnectorTests.DeltaAsync_TerraformShowJsonSensitiveKeyWithoutMarker_ReportsUnchanged`
- [ ] (hunt-ready) `TerraformShowJsonInfrastructureDeclarationParser.WriteCanonicalJsonValue` omits nested key-name redaction — input: `values.site_config.connection_string` plaintext with no `sensitive_values` entry — wrong outcome: password text inside `tf.site_config` JSON blob — mechanism: local `WriteCanonicalJsonValue` never passes property names into `ShouldRedactKey` unlike shared `CanonicalInfrastructureJsonValue.WriteCanonicalJsonValue(writer, value, propertyName)` used by arm-json after hunt #55 — test: `TerraformShowJsonInfrastructureDeclarationParserTests.ParseAsync_redacts_nested_sensitive_json_key_without_sensitive_values`
- [ ] (hunt-ready) `CanonicalInfrastructureJsonValue.WriteCanonicalJsonValue` preserves JSON array element order — input: arm-json `properties.ipSecurityRestrictions` array with rules `[allow-a, allow-b]` vs `[allow-b, allow-a]` — wrong outcome: false modified on infrastructure declaration connector delta — mechanism: object keys are sorted but array items are serialized in enumeration order with no stable sort (depends_on sorting fixed separately in show-json) — test: `ArmJsonInfrastructureDeclarationParserTests.ParseAsync_IpSecurityRestrictionsArrayOrder_ReportsUnchanged`, `InfrastructureDeclarationConnectorTests.DeltaAsync_ArmJsonIpSecurityRestrictionsArrayOrder_ReportsUnchanged`
- [ ] (hunt-ready) `SimpleTerraformResourceBlockParser.ExtractNestedBlockBody` retains `#` comment lines inside nested HCL block bodies — input: `site_config { # primary region\n location = "eastus" }` vs identical block without the comment line — wrong outcome: different `tf.site_config` strings and false modified infra declaration delta — mechanism: nested block collector appends full lines; `RedactSensitiveAssignmentsInHclBody` treats `# …` lines as literal body text rather than stripping HCL comments — test: `SimpleTerraformDeclarationParserTests.ParseAsync_NestedBlockCommentOnlyChange_ReportsUnchanged`, `InfrastructureDeclarationConnectorTests.DeltaAsync_SimpleTerraformNestedBlockCommentChange_ReportsUnchanged`
- [ ] (hunt-ready) `BicepInfrastructureDeclarationParser.ParseAsync` does not skip `Microsoft.Resources/deployments` wrapper resources — input: bicep `resource nestedDeployment 'Microsoft.Resources/deployments@2022-09-01' = { }` alongside real resources — wrong outcome: spurious `TopologyResource` node and false add/remove when nested ARM template wrappers are ingested — mechanism: `ArmJsonInfrastructureDeclarationParser.TryAddResource` filters deployment wrappers but Bicep line parser emits every `resource` regex match — test: `BicepInfrastructureDeclarationParserTests.ParseAsync_SkipsMicrosoftResourcesDeployments`, `InfrastructureDeclarationConnectorTests.NormalizeAsync_BicepNestedDeploymentWrapper_DoesNotEmitTopologyResource`

2026-08-26 seed hunt #56: reseeded terraform-show-json key-name redaction gaps (top-level + nested JSON), arm-json array order, simple-terraform nested-block comment noise, Bicep deployment-wrapper filter parity; five hunt-ready rows added (no repro).

2026-08-26 seed hunt #55: reseeded arm-json blob canonicalization / nested sensitive redaction / simple-terraform nested block + comment paths / K8s YAML separator spacing; proved all five promoted rows.
