...
- [x] (proven) Global API enum conversion still permits out-of-range numeric `StructuralExecutionMode`, `FindingEnforcementTier`, `FindingHumanReviewStatus`, and `FindingTreatment` — **hit 2026-08-25:** ordinal `99` on `ArchitectureRun.structuralExecutionMode` deserialized without `Enum.IsDefined`; fixed with dedicated converters (`StructuralExecutionModeJsonConverterTests`, `FindingHumanReviewStatusJsonConverterTests`, `FindingEnforcementTierJsonConverterTests`, `FindingTreatmentJsonConverterTests`).
