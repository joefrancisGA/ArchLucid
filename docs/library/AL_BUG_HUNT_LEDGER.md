...
- [ ] (hunt-ready) `resolveOperatorBillingCurrentPlan` checks `trialStatus` before `isTrialUsage`; stale `trialStatus: Active` with `isTrialUsage: false` and a commercial tier shows tenant trial instead of paid plan.
- [ ] (hunt-ready) `recentViewLabelFromPathname` maps canonical `/governance/audit` to `"governance · audit"` because only legacy `/audit` is labeled `"Audit trail"`.
- [ ] (hunt-ready) `deriveOperatorHomeWorkspaceMetrics` treats a paginated runs page as workspace-wide KPI totals when `totalCount` exceeds loaded `items`.
- [x] (proven) `recentViewLabelFromPathname` maps `/governance/findings/assigned-to-me` to `"governance · findings · assigned-to-me"` instead of the sidebar label — **hit 2026-08-25:** subroute fell through to generic path prettifier; fixed by matching `GOVERNANCE_ASSIGNED_TO_ME_FINDINGS_PATH` before parent findings route (`operator-recent-views.test.ts`).
