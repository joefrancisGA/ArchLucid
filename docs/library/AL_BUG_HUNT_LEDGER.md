...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-27
- **last-bug:** 2026-08-27 — Api role registered AuditEventChangeFeedHostedService with Cosmos audit enabled
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `AuditEventChangeFeedHostedService` registered on Api role with Cosmos audit — **hit 2026-08-27:** `RegisterCosmosPolyglotPersistence` lacked `ArchLucidHostingRole` gate; split Api+Worker deployments competed for Cosmos change-feed leases; fixed to Worker+Combined; regression in `AddArchLucidApplicationServices_Api_role_with_cosmos_audit_does_not_register_AuditEventChangeFeedHostedService` and `AddArchLucidApplicationServices_Worker_with_cosmos_audit_registers_AuditEventChangeFeedHostedService`.
- [x] (invalid) `DraftIntakeCompositionRegistrar` registers `AdvisoryDraftOperationHostedService` without hosting-role gate — `AdvisoryDraftOperationQueue` is a per-process bounded channel; Api must drain work accepted on that host.
- [x] (invalid) `RunLifecycleOrchestrationCompositionRegistrar` registers `ArchitectureRunAsyncOperationHostedService` without hosting-role gate — `ArchitectureRunAsyncOperationQueue` is per-process in-memory; Worker cannot dequeue work enqueued on Api replicas.
- [x] (valid-no-repro) `RegisterDurableBackgroundJobInfrastructure` registers `BackgroundJobStuckRunningWatchdogHostedService` for Api durable enqueue-only hosts — watchdog uses `HostLeaderElectionCoordinator` and reclaims stale SQL rows cluster-wide; beneficial without local queue processor.
...
- [x] (proven) `ManifestsController.GetManifestSummary` — out-of-range `maxRelationships` silently clamped via `Math.Clamp` instead of HTTP 400 — **hit 2026-08-27:** explicit bounds check before summary generation (register `maxRows` parity); regression in `ManifestsControllerTests`.
...
- [x] (proven) `ManifestsController` manifest route reads — whitespace-only `manifestVersion` returned HTTP 404 instead of HTTP 400 (`CompareManifests` already required `leftVersion`/`rightVersion`) — **hit 2026-08-27:** `ValidateManifestVersionRoute` on all route-based manifest GET/export paths; regression in `ManifestsControllerTests`.
...
- [x] (proven) `GovernanceController.DryRunProposedPolicyPack` — null or whitespace `policyPackContentJson` reached `PolicyPackGovernanceDryRunService.EvaluateAsync` (`ArgumentException`, HTTP 500) instead of HTTP 400 (`PolicyPackGovernanceDryRunRequestValidator` not invoked on this action) — **hit 2026-08-27:** explicit `IsNullOrWhiteSpace` guard before service call; regression in `GovernanceControllerSimulateTests`.

2026-08-27 seed hunt #176: proved proposed policy-pack dry-run null content JSON validation.
...
- [x] (proven) `GovernanceController.DryRunProposedPolicyPack` — out-of-range `blockCommitMinimumSeverity` reached `PolicyPackGovernanceDryRunService.EvaluateAsync` (HTTP 200 with invalid ordinal) instead of HTTP 400 (`PolicyPackGovernanceDryRunRequestValidator` range rule not invoked on this action) — **hit 2026-08-27:** reject values outside 0–3 before service call; regression in `GovernanceControllerSimulateTests`.

2026-08-27 seed hunt #179: proved proposed policy-pack dry-run severity ordinal validation.
...
- [x] (proven) `GovernanceCoverageController.GetScopeCoverage` / `PreviewCoverage` — missing workspace membership preflight; foreign workspace returned HTTP 200 coverage payload instead of HTTP 404 — **hit 2026-08-27:** workspace preflight before coverage query/preview (posture parity); regression in `GovernanceCoverageControllerScopeTests`.
