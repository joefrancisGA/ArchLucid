...
- [x] (proven) `ArchitectureFindingJsonConverter` reads `sourceAgent` and `enforcementTier` only as strings — **hit 2026-08-25:** numeric ordinals such as `"sourceAgent": 1` were ignored, leaving default agent and tier; fixed with `ReadDefinedEnum` accepting defined ordinals and labels (`ArchitectureFindingJsonConverterTests.Deserialize_numeric_sourceAgent_maps_topology`, `Deserialize_numeric_enforcementTier_maps_advisory`).
