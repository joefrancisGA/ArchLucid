...
- **consecutive-dry-hunts:** 1
- **last-hunt:** 2026-08-28
- **last-bug:** 2026-08-27 — `TenantPilotValueReportController` ghost workspace returned HTTP 200 instead of workspace 404
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `TenantPilotValueReportController` (`GetPilotValueReport`, `GetRoiSummaryPageBundle`) — ghost workspace → HTTP 200 pilot/ROI report payload instead of workspace 404; `PilotValueReportService.BuildAsync` checks tenant row only while audit export and run aggregation use ambient workspace (`TenantPilotValueReportController.cs` L69–76; `PilotValueReportService.cs` L77–90) — **hit 2026-08-27:** shared `TenantWorkspaceScopePreflight` preflight (core-pilot-checklist parity); regression in `GetPilotValueReport_returns_not_found_when_workspace_missing` and `GetRoiSummaryPageBundle_returns_not_found_when_workspace_missing`.

2026-08-27 thorough hunt #169: proved pilot-value-report foreign-workspace preflight gap; ghost-workspace hunt-ready backlog cleared.
