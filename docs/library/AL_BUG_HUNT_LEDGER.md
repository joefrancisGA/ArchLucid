...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — recent-deltas panel counted ReadyForCommit runs with manifest ids as committed
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `RecentPilotRunDeltasService.IsCommitted` keys on manifest version instead of run status — **hit 2026-08-25:** `ReadyForCommit` runs with `CurrentManifestVersion` inflated recent-deltas panel/medians while `PilotScorecardBuilder` already required `Committed`; fixed to filter on `ArchitectureRunStatus.Committed` (`RecentPilotRunDeltasServiceTests.GetRecentDeltasAsync_ReadyForCommitRunWithManifest_IsExcludedFromCommittedPanel`).
- [ ] (hunt-ready) `PilotValueReportService.IsCommittedSummary` treats `ReadyForCommit` runs with manifest ids as committed for totals/timeline (same predicate shape as pre-fix scorecard).
