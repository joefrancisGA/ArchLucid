...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — sponsor narrative ignored findings snapshot fallback while deltas showed non-zero severity counts
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) First-value / buyer-proof sponsor narrative ignores FindingsSnapshot fallback — **hit 2026-08-25:** `SponsorDecisionDeltaNoveltyResolver` and `FirstValueReportBuilder.FormatSponsorTopFindings` read only `ArchitectureRunDetail.Results` while `PilotRunDeltaComputer` populated severity counts from snapshot; fixed via `PilotRunDeltas.SnapshotFallbackFindings` and `PilotMaterialFindingsCollector` (`SponsorDecisionDeltaNoveltyResolverTests.Resolve_when_snapshot_fallback_findings_present_uses_deltas_for_decision_delta`).
