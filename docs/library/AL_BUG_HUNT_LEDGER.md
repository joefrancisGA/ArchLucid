...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — stale `trialStatus: Active` overrode paid plan when `isTrialUsage` is false
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `resolveOperatorBillingCurrentPlan` treated stale shell `trialStatus: Active` as tenant trial before checking `isTrialUsage: false` with a commercial tier — **hit 2026-08-25 seed hunt:** plan card showed Trial while usage/subscription reported Team paid plan; fixed by skipping active-trial branch when `isTrialUsage === false` (`operator-billing-current-plan.test.ts`).
- [ ] (hunt-ready) `recentViewLabelFromPathname` imports `GOVERNANCE_AUDIT_PATH` (`/governance/audit`) but only maps legacy `/audit`; canonical audit visits persist as `governance · audit` instead of `Audit trail` in Home recent views.
