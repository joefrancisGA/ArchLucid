...
- [x] (proven) `AgentResultClaimListJsonConverter` flattens structured claim text but ignores an entry-level `evidenceRefs` array — **hit 2026-08-25:** `{"detail":"Subnet missing","evidenceRefs":["pol-123"]}` preserved claim text but dropped linkage from `AgentResult.EvidenceRefs`; fixed with `AgentResultJsonConverter` post-merge of claim-level refs (`AgentResultClaimListJsonConverterEvidenceRefsTests`).
