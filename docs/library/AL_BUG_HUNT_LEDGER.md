...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — `PilotValueReportService.IsCommittedSummary` ORed manifest version with committed status; ReadyForCommit runs inflated value-report totals
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) `PilotValueReportService.IsCommittedSummary` ORed `Status == Committed` with non-empty `CurrentManifestVersion` — **hit 2026-08-25:** ReadyForCommit runs with manifest version inflated `TotalRunsCommitted` and timeline (`BuildAsync_ReadyForCommitRunWithManifest_IsNotCountedAsCommitted`).
- [ ] (hunt-ready) `RecentPilotRunDeltasService.IsCommitted` keys on `CurrentManifestVersion` while `PilotScorecardBuilder` requires `LegacyRunStatus == Committed` — ReadyForCommit runs with manifest version appear in recent-deltas medians the same way value-report totals did before 2026-08-25 fix.
