...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string — **hit 2026-08-25:** persisted JSON with numeric `1` threw on read or left default `NotRequired`; fixed with `ReadHumanReviewStatus` accepting defined ordinals (`FindingJsonConverterTests.Deserialize_numeric_humanReviewStatus_maps_pending`).
