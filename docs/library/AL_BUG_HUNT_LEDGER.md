...
- [x] (proven) `RecentPilotRunDeltasService.IsCommitted` treats `ReadyForCommit` runs with `CurrentManifestVersion` as committed — **hit 2026-08-25:** predicate keyed on manifest version only; fixed to require `RunSummary.Status == Committed` (`RecentPilotRunDeltasServiceTests.GetRecentDeltasAsync_ReadyForCommitRunWithManifest_IsExcludedFromCommittedPanel`).
- [ ] (hunt-ready) `PilotValueReportService.IsCommittedSummary` ORs `Committed` status with manifest version, so `ReadyForCommit` runs that already carry a manifest id inflate `TotalRunsCommitted` in the value report window.
