...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-08-25
- **last-bug:** 2026-08-25 — sponsor narrative sections ignored findings-snapshot fallback when agent results empty
- **related-pd-tb:** none
- **code-changed-since:** no
...
- [x] (proven) Sponsor narrative sections ignored findings-snapshot fallback — **hit 2026-08-25:** `SponsorDecisionDeltaNoveltyResolver` and `FirstValueReportSponsorStatusSectionFormatter` read only `detail.Results[*].Findings` while `PilotRunDeltaComputer` populated severity buckets from snapshot; fixed via `SponsorNarrativeFindings` on `PilotRunDeltas` plus `PilotSponsorMaterialFindingsResolver` (`Resolve_when_agent_results_empty_uses_sponsor_narrative_findings_from_deltas`, `BuildMarkdownAsync_WhenFindingsOnlyInSnapshot_SponsorNarrativeMatchesDeltas`).
