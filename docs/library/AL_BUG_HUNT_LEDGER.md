...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-09-03
- **last-bug:** 2026-09-03 — whitespace-padded failure-summary `failureClass` rejected dead-letter detection
- **related-pd-tb:** none
- **code-changed-since:** yes

### Hypotheses

- [x] (proven) `RunAuthorityPipelineDeadLetterDetection.TryDeserialize` — whitespace-padded `failureClass` rejected — **hit 2026-09-03 (#626):** `" PipelineDeadLetter "` deserialized but `OrdinalIgnoreCase` compare missed padded token so run list showed not dead-lettered; fixed with `TryReadNonEmptyTextToken` + trim (`IsDeadLettered_returns_true_for_whitespace_padded_failure_class`).

2026-09-03 seed hunt #626: proved padded failureClass dead-letter detection gap.
