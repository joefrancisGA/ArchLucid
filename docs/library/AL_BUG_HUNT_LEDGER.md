...
- **consecutive-dry-hunts:** 0
- **last-hunt:** 2026-09-05
- **last-bug:** 2026-09-05 — PrimaryStorageAccessKey/SecondaryStorageAccessKey/RotatedAccessKey redaction, would/shall/cannot/should/ought-not-maintain negation, advice would-not-provision and cannot-provision/enforce and need-not-ensure suffix
- **related-pd-tb:** none
- **code-changed-since:** yes
...
- [x] (proven) `FakeScenarioFactory.CreateComplianceResult` — negated `encryption` constraint still adds `Encryption At Rest` control — **hit 2026-09-05 (#882):** raw `Contains("encryption")` ignored `non-encryption` negation already handled by `RequestConstraintClassifier.HasEncryptionConstraint`; simulator compliance proposals overstated required controls; fixed by reusing classifier (`CreateComplianceResult_does_not_add_encryption_control_for_negated_constraint`).
- [x] (proven) `AzureExtractorSensitivePropertyRedactor.IsSensitiveKey` — `accesskeyless` substring false positive — **hit 2026-09-05 (#882):** #880 `secretless` suffix guard missed `accesskey`+`less`; benign ARM metadata redacted; fixed with `accesskeyless` suffix guard (`IsSensitiveKey_detects_secret_like_names` with `accesskeyless`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher.IsSensitiveConfigPath` — `ApiKeylessAuth` segment substring false positive — **hit 2026-09-05 (#882):** #881 segment matcher missed `ApiKey`+`less` suffix on config paths; operator summary redacted `ApiKeylessAuth` to `***`; fixed with `ApiKeyless` suffix guard (`Resolve_returns_scalar_for_non_secret_segment_substrings` with `ArchLucid:ApiKeylessAuth:Mode`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsObviousGenericAdvice` — `without {phrase}` prohibitive intent still matches affirmative fragments — **hit 2026-09-05 (#882):** #880 `not required to` negation missed `without enable mfa`; architecture-specific findings demoted as generic checklist advice; fixed with `without` negation prefix (`IsObviousGenericAdvice_does_not_flag_negated_checklist_phrasing` with `workload without enable mfa for service accounts`).
- [x] (proven) `AzureExtractorSensitivePropertyRedactor.IsSensitiveKey` — `primarykeyless` / `secondarykeyless` suffix false positives — **hit 2026-09-05 (#883):** #882 `accesskeyless` suffix guard missed `primarykey`+`less` and `secondarykey`+`less`; benign ARM metadata redacted; fixed with matching suffix guards (`IsSensitiveKey_detects_secret_like_names` with `primarykeyless` / `secondarykeyless`).
...
- [x] (proven) `RunHeaderAnchorJsonComparer.TryAbsentEmptyObjectEquivalent` — empty JSON object property vs omitted property treated as anchor mutation — **hit 2026-09-05 (#896):** #895 added absent↔empty-string only; fixed with undefined↔empty-object bridge (`EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_object_equivalent_to_omitted_on_committed_run`).
- [x] (proven) `RunHeaderAnchorJsonComparer.TryAbsentEmptyArrayEquivalent` — empty JSON array property vs omitted property treated as anchor mutation — **hit 2026-09-05 (#896):** #888 null↔empty-array bridge skipped absent↔`[]`; fixed with undefined↔empty-array bridge (`EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_array_equivalent_to_omitted_on_committed_run`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher.IsCertificateCredentialSegment` — `Certificate` segment not redacted vs Azure redactor — **hit 2026-09-05 (#896):** #895 credential parity missed certificate paths; fixed with explicit certificate segment detection (`Resolve_redacts_certificate_config_paths`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` — `SigningKey` segment not redacted — **hit 2026-09-05 (#896):** segment lacked explicit signing-key credential detection; fixed with `SigningKey` explicit credential parity (`Resolve_redacts_signing_key_config_path`).
- [x] (proven) `RequestConstraintTokenMatcher.IsNegatedPhraseSuffix` — post-phrase `need not` / `cannot` suffix negation gap — **hit 2026-09-05 (#896):** #893–#895 suffix list missed `need not` and `cannot use`; fixed with suffix/prefix negation guards including mid-phrase ` need not ` for standalone `private` token (`HasManagedIdentityConstraint_does_not_false_positive_on_need_not_managed_identity_phrasing`, `HasPrivateNetworkingConstraint_does_not_false_positive_on_need_not_private_networking_phrasing`, `HasEncryptionConstraint_does_not_false_positive_on_need_not_encryption_phrasing`, `HasManagedIdentityConstraint_does_not_false_positive_on_cannot_use_managed_identity_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `won't need` / `must not` suffix gap — **hit 2026-09-05 (#896):** #895 suffix parity missed imperative-regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_wont_need_or_must_not_suffix_phrasing`).
...
- [x] (proven) `RunHeaderAnchorJsonComparer.TryObjectSingleElementArrayEquivalent` — JSON object scalar vs single-element `[{}]` array treated as anchor mutation — **hit 2026-09-05 (#897):** after #891–#892 scalar/array bridges, `{"scope":{}}` vs `{"scope":[{}]}` threw `RunEvidenceAnchorImmutableException`; fixed with object↔single-element-object-array bridge (`EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_object_scalar_equivalent_to_single_element_object_array_on_committed_run`).
- [x] (proven) `RunHeaderAnchorJsonComparer.TryNestedEmptyArrayEquivalent` — empty JSON array vs single-element `[[]]` nested empty array treated as anchor mutation — **hit 2026-09-05 (#897):** `{"packAssignments":[]}` vs `{"packAssignments":[[]]}` threw on committed lifecycle retry; fixed with array↔single-nested-empty-array bridge in `ArraysEquivalent` (`EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_array_equivalent_to_single_element_empty_array_on_committed_run`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `SharedAccessKey` compound credential segment not redacted — **hit 2026-09-05 (#897):** embedded `AccessKey`/`accesskey` fragment guard skipped `SharedAccessKey` / `sharedAccessKey`; fixed with explicit shared-access-key credential detection (`Resolve_redacts_shared_access_key_config_path`, `IsSensitiveKey_detects_shared_access_key_property_names`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` — `CertificateThumbprint` segment not redacted vs Azure redactor — **hit 2026-09-05 (#897):** Azure `certificate` fragment matched at index 0; config lacked certificate thumbprint credential parity; fixed with explicit thumbprint segment detection (`Resolve_redacts_certificate_thumbprint_config_path_matching_azure_redactor`).
- [x] (proven) `AzureExtractorSensitivePropertyRedactor.IsSensitiveKey` — `SigningCertificate` not redacted vs config explicit credential segment — **hit 2026-09-05 (#897):** embedded `certificate` fragment guard skipped `signingCertificate` while config redacted `SigningCertificate`; fixed with explicit signing-certificate credential parity (`IsSensitiveKey_detects_signing_certificate_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` / `ContainsMidSentenceNegation` — `cannot require` and `need not enable` negation gaps — **hit 2026-09-05 (#897):** #896 `cannot use`/`need not` suffix parity missed prohibitive `cannot require {token}` and leading `need not enable {token}`; fixed with mid-phrase and suffix negation guards including `before.EndsWith("cannot require")` (`HasEncryptionConstraint_does_not_false_positive_on_cannot_require_phrasing`, `HasEncryptionConstraint_does_not_false_positive_on_need_not_enable_encryption_phrasing`).
...
- [x] (proven) `RunHeaderAnchorJsonComparer.TryAbsentEmptyArrayEquivalent` — omitted array property vs nested empty array treated as anchor mutation — **hit 2026-09-05 (#900):** #899 null↔`[[]]` bridge missed absent property vs `{"packAssignments":[[]]}`; fixed by extending absent↔empty-array bridge with `IsDeeplyNestedEmptyArray` parity (`EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_omitted_array_equivalent_to_nested_empty_array_on_committed_run`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` — `AccountSharedAccessKey` compound credential segment not redacted — **hit 2026-09-05 (#900):** compound access-key class after #897–#899; embedded `AccessKey` fragment guard skipped `AccountSharedAccessKey`; fixed with explicit account-shared-access-key credential detection (`Resolve_redacts_account_shared_access_key_config_path`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `StorageAccountKey` not redacted — **hit 2026-09-05 (#900):** config/Azure lacked explicit `StorageAccountKey`/`storageaccountkey` parity while embedded `accountkey` fragment guard skipped compound segment; fixed with explicit credential detection (`Resolve_redacts_storage_account_key_config_path`, `IsSensitiveKey_detects_storage_account_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not require` negation gap — **hit 2026-09-05 (#900):** #899 added `shall`/`should not require` only; missed prohibitive `will not require {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_require_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not require` negation gap — **hit 2026-09-05 (#900):** same mid-sentence family as `will not require`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_require_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `does not need` prefix negation gap — **hit 2026-09-05 (#900):** prefix list had `does not require` only; missed expanded `does not need {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_does_not_need_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `shall not require` suffix gap — **hit 2026-09-05 (#900):** #899 added `should not require` suffix only; missed trailing `shall not require` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_shall_not_require_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `will not require` suffix gap — **hit 2026-09-05 (#900):** same imperative suffix family; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_will_not_require_suffix_phrasing`).

2026-09-05 seed hunt #900 (hit): reseeded after #899 closure; proved eight hunt-ready rows — absent nested-empty array bridge, AccountSharedAccessKey/StorageAccountKey redaction parity, will/would/does-not-need constraint negation, and advice shall/will-not-require suffix; cheap-disproved nested empty-object array bridge (already recurses via `TryObjectSingleElementArrayEquivalent`).

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `BlobAccessKey` not redacted — **hit 2026-09-05 (#901):** compound access-key class after #900; embedded `AccessKey`/`accesskey` fragment guard skipped `BlobAccessKey`/`blobaccesskey`; fixed with explicit credential detection (`Resolve_redacts_blob_access_key_config_path`, `IsSensitiveKey_detects_blob_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not need` negation gap — **hit 2026-09-05 (#901):** #900 added `will not require` only; missed prohibitive `will not need {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_need_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not need` negation gap — **hit 2026-09-05 (#901):** same mid-sentence family as `will not need`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_need_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not adopt` prefix negation gap — **hit 2026-09-05 (#901):** prefix list had `need not use`/`need not have` only; missed leading `need not adopt {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_adopt_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `is not required to` prefix negation gap — **hit 2026-09-05 (#901):** prefix list had `not required to` only; missed leading `is not required to {token}` phrasing; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_is_not_required_to_use_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `would not require` suffix gap — **hit 2026-09-05 (#901):** #900 added `will not require` suffix only; missed trailing `would not require` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_would_not_require_suffix_phrasing`).
...
2026-09-05 seed hunt #902 (hit): reseeded after #901 closure; proved six hunt-ready rows — FileAccessKey redaction parity, ought-not-require/need-not-implement constraint negation, and advice would/will/does-not-need suffix; cheap-disproved null-object-vs-omitted anchor parity and `must not need` prefix (already covered by `must not` guard).

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `QueueAccessKey` not redacted — **hit 2026-09-05 (#903):** compound access-key class after #902; embedded `AccessKey`/`accesskey` fragment guard skipped `QueueAccessKey`/`queueaccesskey`; fixed with explicit credential detection (`Resolve_redacts_queue_access_key_config_path`, `IsSensitiveKey_detects_queue_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `TableAccessKey` not redacted — **hit 2026-09-05 (#903):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_table_access_key_config_path`, `IsSensitiveKey_detects_table_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not deploy` prefix negation gap — **hit 2026-09-05 (#903):** prefix list had `need not implement`/`need not adopt` only; missed leading `need not deploy {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_deploy_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not mandate` prefix negation gap — **hit 2026-09-05 (#903):** same prefix family; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_mandate_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `is not needed for` prefix negation gap — **hit 2026-09-05 (#903):** prefix list had `is not required to` only; missed leading `is not needed for {token}` phrasing; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_is_not_needed_for_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `ought not require` suffix gap — **hit 2026-09-05 (#903):** #902 added `would/will not need` suffix only; missed trailing `ought not require` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_ought_not_require_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `is not required for` / `need not adopt` suffix gaps — **hit 2026-09-05 (#903):** suffix list had `is not required`/`need not` prefix matches only; missed trailing `is not required for` and explicit `need not adopt` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_is_not_required_for_suffix_phrasing`, `IsObviousGenericAdvice_does_not_flag_need_not_adopt_suffix_phrasing`).

2026-09-05 seed hunt #903 (hit): reseeded after #902 closure; proved seven hunt-ready rows — QueueAccessKey/TableAccessKey redaction parity, need-not-deploy/mandate/is-not-needed-for constraint negation, and advice ought/is-not-required-for/need-not-adopt suffix; cheap-disproved empty-object-vs-omitted anchor parity (already covered at line 388).

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `DiskAccessKey` not redacted — **hit 2026-09-05 (#904):** compound access-key class after #903; embedded `AccessKey`/`accesskey` fragment guard skipped `DiskAccessKey`/`diskaccesskey`; fixed with explicit credential detection (`Resolve_redacts_disk_access_key_config_path`, `IsSensitiveKey_detects_disk_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `WebAccessKey` not redacted — **hit 2026-09-05 (#904):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_web_access_key_config_path`, `IsSensitiveKey_detects_web_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not enforce` prefix negation gap — **hit 2026-09-05 (#904):** prefix list had `need not deploy`/`need not mandate` only; missed leading `need not enforce {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_enforce_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not configure` prefix negation gap — **hit 2026-09-05 (#904):** same prefix family; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `is not necessary for` prefix negation gap — **hit 2026-09-05 (#904):** prefix list had `is not needed for` only; missed leading `is not necessary for {token}` phrasing; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_is_not_necessary_for_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `ought not need` negation gap — **hit 2026-09-05 (#904):** #903 added `ought not require` only; missed prohibitive `ought not need {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_ought_not_need_encryption_phrasing`).
...
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `DataLakeAccessKey` not redacted — **hit 2026-09-05 (#907):** compound access-key class after #906; embedded `AccessKey`/`accesskey` fragment guard skipped `DataLakeAccessKey`/`datalakeaccesskey`; fixed with explicit credential detection (`Resolve_redacts_data_lake_access_key_config_path`, `IsSensitiveKey_detects_data_lake_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `ServiceAccessKey` not redacted — **hit 2026-09-05 (#907):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_service_access_key_config_path`, `IsSensitiveKey_detects_service_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `does not configure` prefix negation gap — **hit 2026-09-05 (#907):** prefix list had `does not enforce`/`does not mandate` only; missed leading `does not configure {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_does_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `does not apply` prefix negation gap — **hit 2026-09-05 (#907):** same prefix family; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_does_not_apply_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not mandate` negation gap — **hit 2026-09-05 (#907):** #906 added `will not mandate` only; missed prohibitive `would not mandate {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_mandate_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not mandate` negation gap — **hit 2026-09-05 (#907):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_mandate_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `does not enforce` suffix gap — **hit 2026-09-05 (#907):** suffix list had `does not mandate` only; missed trailing `does not enforce` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_does_not_enforce_suffix_phrasing`).
...
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `AdminAccessKey` not redacted — **hit 2026-09-05 (#909):** compound access-key class after #908; embedded `AccessKey`/`accesskey` fragment guard skipped `AdminAccessKey`/`adminaccesskey`; fixed with explicit credential detection (`Resolve_redacts_admin_access_key_config_path`, `IsSensitiveKey_detects_admin_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `SasAccessKey` not redacted — **hit 2026-09-05 (#909):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_sas_access_key_config_path`, `IsSensitiveKey_detects_sas_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `BackupAccessKey` not redacted — **hit 2026-09-05 (#909):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_backup_access_key_config_path`, `IsSensitiveKey_detects_backup_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not enforce` negation gap — **hit 2026-09-05 (#909):** #908 added mandate-family mid-sentence negation only; missed prohibitive `will not enforce {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_enforce_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not enforce` negation gap — **hit 2026-09-05 (#909):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_enforce_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not enforce` negation gap — **hit 2026-09-05 (#909):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_enforce_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `need not maintain` prefix negation gap — **hit 2026-09-05 (#909):** prefix list had `need not ensure`/`need not enforce` only; missed leading `need not maintain {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_need_not_maintain_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `shall not mandate` suffix gap — **hit 2026-09-05 (#909):** suffix list had `shall not require`/`shall not need` only; missed trailing `shall not mandate` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_shall_not_mandate_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `should not mandate` suffix gap — **hit 2026-09-05 (#909):** #908 added constraint mid-sentence `should not mandate` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_should_not_mandate_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `does not apply` suffix gap — **hit 2026-09-05 (#909):** #907 added constraint prefix `does not apply` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_does_not_apply_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `does not provision` suffix gap — **hit 2026-09-05 (#909):** #908 added constraint prefix `does not provision` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_does_not_provision_suffix_phrasing`).

2026-09-05 seed hunt #909 (hit): reseeded after #908 closure; proved eleven hunt-ready rows — AdminAccessKey/SasAccessKey/BackupAccessKey redaction parity, will/would/shall-not-enforce and need-not-maintain constraint negation, and advice shall/should-not-mandate and does-not-apply/provision suffix.

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `MasterAccessKey` not redacted — **hit 2026-09-05 (#910):** compound access-key class after #909; embedded `AccessKey`/`accesskey` fragment guard skipped `MasterAccessKey`/`masteraccesskey`; fixed with explicit credential detection (`Resolve_redacts_master_access_key_config_path`, `IsSensitiveKey_detects_master_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `RootAccessKey` not redacted — **hit 2026-09-05 (#910):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_root_access_key_config_path`, `IsSensitiveKey_detects_root_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `AccountAccessKey` not redacted — **hit 2026-09-05 (#910):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_account_access_key_config_path`, `IsSensitiveKey_detects_account_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `cannot mandate` negation gap — **hit 2026-09-05 (#910):** #909 added enforce-family mid-sentence negation only; missed prohibitive `cannot mandate {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_cannot_mandate_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.IsAdviceStyleNegation` — `does not ensure` prefix negation gap — **hit 2026-09-05 (#910):** prefix list had `need not ensure` only; missed leading `does not ensure {token}`; fixed with prefix negation guard (`HasEncryptionConstraint_does_not_false_positive_on_does_not_ensure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not configure` negation gap — **hit 2026-09-05 (#910):** #907 added `does not configure` prefix only; missed prohibitive `will not configure {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not configure` negation gap — **hit 2026-09-05 (#910):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not configure` negation gap — **hit 2026-09-05 (#910):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_configure_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `ought not mandate` suffix gap — **hit 2026-09-05 (#910):** #908 added constraint mid-sentence `ought not mandate` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_ought_not_mandate_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `will not enforce` suffix gap — **hit 2026-09-05 (#910):** #907 added constraint `does not enforce` suffix only; missed trailing `will not enforce` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_will_not_enforce_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `would not enforce` suffix gap — **hit 2026-09-05 (#910):** same suffix family; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_would_not_enforce_suffix_phrasing`).
...
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `GlobalAccessKey` not redacted — **hit 2026-09-05 (#912):** compound access-key class after #911; embedded `AccessKey`/`accesskey` fragment guard skipped `GlobalAccessKey`/`globalaccesskey`; fixed with explicit credential detection (`Resolve_redacts_global_access_key_config_path`, `IsSensitiveKey_detects_global_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `SharedKeyAccessKey` not redacted — **hit 2026-09-05 (#912):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_shared_key_access_key_config_path`, `IsSensitiveKey_detects_shared_key_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `CustomAccessKey` not redacted — **hit 2026-09-05 (#912):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_custom_access_key_config_path`, `IsSensitiveKey_detects_custom_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not apply` negation gap — **hit 2026-09-05 (#912):** #911 added `will not apply` only; missed prohibitive `would not apply {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_apply_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not apply` negation gap — **hit 2026-09-05 (#912):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_apply_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `should not configure` negation gap — **hit 2026-09-05 (#912):** #910 added `shall not configure` only; missed prohibitive `should not configure {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_should_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `ought not configure` negation gap — **hit 2026-09-05 (#912):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_ought_not_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `cannot apply` negation gap — **hit 2026-09-05 (#912):** #910 added `cannot mandate` only; missed prohibitive `cannot apply {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_cannot_apply_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `ought not enforce` suffix gap — **hit 2026-09-05 (#912):** #911 added constraint mid-sentence `ought not enforce` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_ought_not_enforce_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `will not configure` suffix gap — **hit 2026-09-05 (#912):** #908 added constraint prefix `does not configure` only; missed trailing `will not configure` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_will_not_configure_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `would not configure` suffix gap — **hit 2026-09-05 (#912):** #910 added constraint mid-sentence `would not configure` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_would_not_configure_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `does not maintain` suffix gap — **hit 2026-09-05 (#912):** #911 added constraint prefix `does not maintain` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_does_not_maintain_suffix_phrasing`).

2026-09-05 seed hunt #912 (hit): reseeded after #911 closure; proved twelve hunt-ready rows — GlobalAccessKey/SharedKeyAccessKey/CustomAccessKey redaction parity, would/shall-not-apply and should/ought-not-configure/cannot-apply constraint negation, and advice ought-not-enforce and will/would-not-configure and does-not-maintain suffix.

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `LocalAccessKey` not redacted — **hit 2026-09-05 (#913):** compound access-key class after #912; embedded `AccessKey`/`accesskey` fragment guard skipped `LocalAccessKey`/`localaccesskey`; fixed with explicit credential detection (`Resolve_redacts_local_access_key_config_path`, `IsSensitiveKey_detects_local_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `RemoteAccessKey` not redacted — **hit 2026-09-05 (#913):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_remote_access_key_config_path`, `IsSensitiveKey_detects_remote_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `TempAccessKey` not redacted — **hit 2026-09-05 (#913):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_temp_access_key_config_path`, `IsSensitiveKey_detects_temp_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `should not apply` negation gap — **hit 2026-09-05 (#913):** #912 added `would/shall not apply` only; missed prohibitive `should not apply {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_should_not_apply_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `ought not apply` negation gap — **hit 2026-09-05 (#913):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_ought_not_apply_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `cannot configure` negation gap — **hit 2026-09-05 (#913):** #912 added `should/ought not configure` only; missed prohibitive `cannot configure {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_cannot_configure_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not provision` negation gap — **hit 2026-09-05 (#913):** #908 added `does not provision` prefix only; missed prohibitive `will not provision {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_provision_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not provision` negation gap — **hit 2026-09-05 (#913):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_provision_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `ought not configure` suffix gap — **hit 2026-09-05 (#913):** #912 added constraint mid-sentence `ought not configure` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_ought_not_configure_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `shall not apply` suffix gap — **hit 2026-09-05 (#913):** #909 added constraint prefix `does not apply` only; missed trailing `shall not apply` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_shall_not_apply_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `should not apply` suffix gap — **hit 2026-09-05 (#913):** same suffix family; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_should_not_apply_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `cannot mandate` suffix gap — **hit 2026-09-05 (#913):** #910 added constraint mid-sentence `cannot mandate` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_cannot_mandate_suffix_phrasing`).

2026-09-05 seed hunt #913 (hit): reseeded after #912 closure; proved twelve hunt-ready rows — LocalAccessKey/RemoteAccessKey/TempAccessKey redaction parity, should/ought-not-apply and cannot-configure/will/would-not-provision constraint negation, and advice ought-not-configure and shall/should-not-apply and cannot-mandate suffix.

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `TemporaryAccessKey` not redacted — **hit 2026-09-05 (#914):** compound access-key class after #913; embedded `AccessKey`/`accesskey` fragment guard skipped `TemporaryAccessKey`/`temporaryaccesskey`; fixed with explicit credential detection (`Resolve_redacts_temporary_access_key_config_path`, `IsSensitiveKey_detects_temporary_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `ExternalAccessKey` not redacted — **hit 2026-09-05 (#914):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_external_access_key_config_path`, `IsSensitiveKey_detects_external_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `InternalAccessKey` not redacted — **hit 2026-09-05 (#914):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_internal_access_key_config_path`, `IsSensitiveKey_detects_internal_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not provision` negation gap — **hit 2026-09-05 (#914):** #913 added `will/would not provision` only; missed prohibitive `shall not provision {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_provision_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `cannot provision` negation gap — **hit 2026-09-05 (#914):** #912 added `cannot apply` only; missed prohibitive `cannot provision {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_cannot_provision_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `should not provision` negation gap — **hit 2026-09-05 (#914):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_should_not_provision_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `ought not provision` negation gap — **hit 2026-09-05 (#914):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_ought_not_provision_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `will not maintain` negation gap — **hit 2026-09-05 (#914):** #911 added `need not maintain` prefix only; missed prohibitive `will not maintain {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_will_not_maintain_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `shall not provision` suffix gap — **hit 2026-09-05 (#914):** #908 added constraint prefix `does not provision` only; missed trailing `shall not provision` on imperative regex matches; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_shall_not_provision_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `should not provision` suffix gap — **hit 2026-09-05 (#914):** same suffix family; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_should_not_provision_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `cannot configure` suffix gap — **hit 2026-09-05 (#914):** #913 added constraint mid-sentence `cannot configure` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_cannot_configure_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `need not maintain` suffix gap — **hit 2026-09-05 (#914):** #911 added constraint prefix `need not maintain` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_need_not_maintain_suffix_phrasing`).

2026-09-05 seed hunt #914 (hit): reseeded after #913 closure; proved twelve hunt-ready rows — TemporaryAccessKey/ExternalAccessKey/InternalAccessKey redaction parity, shall/cannot/should/ought-not-provision and will-not-maintain constraint negation, and advice shall/should-not-provision and cannot-configure and need-not-maintain suffix.

- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `PrimaryStorageAccessKey` not redacted — **hit 2026-09-05 (#915):** compound access-key class after #914; embedded `AccessKey`/`accesskey` fragment guard skipped `PrimaryStorageAccessKey`/`primarystorageaccesskey`; fixed with explicit credential detection (`Resolve_redacts_primary_storage_access_key_config_path`, `IsSensitiveKey_detects_primary_storage_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `SecondaryStorageAccessKey` not redacted — **hit 2026-09-05 (#915):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_secondary_storage_access_key_config_path`, `IsSensitiveKey_detects_secondary_storage_access_key_property_names_matching_config_redactor`).
- [x] (proven) `ConfigurationSensitiveConfigPathMatcher` / `AzureExtractorSensitivePropertyRedactor` — `RotatedAccessKey` not redacted — **hit 2026-09-05 (#915):** same compound access-key class; fixed with explicit credential detection (`Resolve_redacts_rotated_access_key_config_path`, `IsSensitiveKey_detects_rotated_access_key_property_names_matching_config_redactor`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `would not maintain` negation gap — **hit 2026-09-05 (#915):** #914 added `will not maintain` only; missed prohibitive `would not maintain {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_would_not_maintain_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `shall not maintain` negation gap — **hit 2026-09-05 (#915):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_shall_not_maintain_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `cannot maintain` negation gap — **hit 2026-09-05 (#915):** #911 added `need not maintain` prefix only; missed prohibitive `cannot maintain {token}`; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_cannot_maintain_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `should not maintain` negation gap — **hit 2026-09-05 (#915):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_should_not_maintain_encryption_phrasing`).
- [x] (proven) `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — `ought not maintain` negation gap — **hit 2026-09-05 (#915):** same mid-sentence family; fixed with mid-sentence negation guard (`HasEncryptionConstraint_does_not_false_positive_on_ought_not_maintain_encryption_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `would not provision` suffix gap — **hit 2026-09-05 (#915):** #913 added constraint mid-sentence `would not provision` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_would_not_provision_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `cannot provision` suffix gap — **hit 2026-09-05 (#915):** #914 added constraint mid-sentence `cannot provision` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_cannot_provision_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `cannot enforce` suffix gap — **hit 2026-09-05 (#915):** #911 added constraint mid-sentence `cannot enforce` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_cannot_enforce_suffix_phrasing`).
- [x] (proven) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — imperative-path `need not ensure` suffix gap — **hit 2026-09-05 (#915):** #908 added constraint prefix `need not ensure` only; missed imperative-path suffix; fixed with suffix guard parity (`IsObviousGenericAdvice_does_not_flag_need_not_ensure_suffix_phrasing`).

2026-09-05 seed hunt #915 (hit): reseeded after #914 closure; proved twelve hunt-ready rows — PrimaryStorageAccessKey/SecondaryStorageAccessKey/RotatedAccessKey redaction parity, would/shall/cannot/should/ought-not-maintain constraint negation, and advice would-not-provision and cannot-provision/enforce and need-not-ensure suffix.
...
- [x] (invalid) `MermaidDiagramRenderer.Render` node id unsanitized claim — **cheap-disproof 2026-09-05 (#890):** `AppendNodeLine` already calls `MermaidIdSanitizer.Sanitize(node.NodeId)`; label/bracket gap was the real defect.
- [x] (proven) `MermaidDiagramRenderer.EscapeLabel` — bracket/newline characters in node/edge labels break `flowchart TD` syntax — **hit 2026-09-05 (#890):** quote-only escaping left `]` and embedded newlines in labels; fixed with `#91;`/`#93;` entity substitution and newline flattening (`MermaidDiagramRenderer_Render_escapes_brackets_and_newlines_in_labels`).
- [x] (proven) `DocxExportService.BuildDocumentAsync` — security/compliance posture table cells bypass `SanitizeArtifactText` — **hit 2026-09-05 (#890):** gap lines sanitized in #732 but four-column control rows were raw; fixed with `SanitizeArtifactText` on all table cell values (`ExportAsync_strips_control_chars_from_security_posture_table_cells`).
...
- [x] (proven) `PolicyPacksController.Create` / `PolicyPacksAppService.CreatePackAsync` — operator retry with pack `packType` differing only by casing allocated fresh `PolicyPackId` and logged duplicate `PolicyPackCreated` audit (`Ordinal` packType match in create dedupe matcher; API accepts case-insensitive pack types via `ValidPackTypes`) — **hit 2026-09-05 (#865):** case-insensitive packType comparison in create dedupe matcher; regression in `CreatePackAsync_returns_existing_pack_and_skips_duplicate_audit_when_pack_type_differs_only_by_casing`.
...
2026-09-05 thorough hunt #854 (hit): proved legal-hold reason case-insensitive idempotent retry gap seeded in #853.
