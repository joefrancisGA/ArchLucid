...
- [x] (proven) `deriveOperatorHomeWorkspaceMetrics` mixed workspace `totalCount` with page-local aggregates when the runs dashboard slice was partial — KPI strip could show `20 (5 finalized)` while findings summed only the first page; fixed 2026-08-25 by clamping total to loaded count when `items.length < totalCount` (`operator-home-workspace-metrics.test.ts`).
