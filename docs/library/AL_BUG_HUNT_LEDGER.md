...
- [ ] (hunt-ready) `RunHeaderAnchorJsonComparer.PropertiesEquivalent` — empty JSON string property vs omitted property treated as anchor mutation — locus `RunHeaderAnchorJsonComparer.PropertiesEquivalent` / `IsNullOrAbsentProperty`; input `{"label":""}` vs `{}` on committed `GovernanceScopeJson`; wrong outcome `RunEvidenceAnchorImmutableException` on lifecycle retry; mechanism #888 null/empty-string coercion does not bridge absent vs `""` (string kind vs undefined); repro `EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_empty_string_equivalent_to_omitted_on_committed_run` — **fails today** (seed #895).
- [ ] (hunt-ready) `RunHeaderAnchorJsonComparer.TryNullEmptyStringEquivalent` — whitespace-only JSON string vs `null` treated as anchor mutation — locus `TryNullEmptyStringEquivalent` uses `string.IsNullOrEmpty` not `IsNullOrWhiteSpace`; input `{"label":" "}` vs `{"label":null}`; wrong outcome `RunEvidenceAnchorImmutableException`; mechanism #885 trims compared strings but never coerces whitespace-only to null/empty; repro `EnsureAnchorsUnchangedIfCommitted_allows_governance_scope_whitespace_string_equivalent_to_null_on_committed_run` — **fails today** (seed #895).
- [ ] (hunt-ready) `ConfigurationSensitiveConfigPathMatcher.IsSensitiveConfigSegment` — `ClientSecret` / `PrimaryKey` / `AccountKey` paths not redacted while `AzureExtractorSensitivePropertyRedactor` flags them — locus `SensitiveSegmentFragments` + `IsEmbeddedSensitiveFragment`; input `ArchLucid:Auth:ClientSecret`, `Azure:Storage:PrimaryKey`, `Azure:Storage:AccountKey`; wrong outcome operator summary returns plaintext not `***`; mechanism embedded-letter guard blocks `Secret` inside `ClientSecret` and matcher omits `accesskey`/`primarykey`/`clientsecret`/`certificate` fragments present on Azure path; repro `Resolve_redacts_client_secret_config_paths`, `Resolve_redacts_primary_key_config_paths`, `Resolve_redacts_account_key_config_paths` — **fail today** (seed #895).
- [ ] (hunt-ready) `GenericArchitectureAdvicePatterns.IsSuffixNegatedAdviceFragment` — `isn't required` / `isn't needed` suffix negation missing vs `RequestConstraintTokenMatcher.IsNegatedPhraseSuffix` — locus `IsSuffixNegatedAdviceFragment`; input `Enable MFA isn't required for batch workloads.` / `Use HTTPS isn't needed for legacy clients.`; wrong outcome `IsObviousGenericAdvice` true demotes negated architecture finding; mechanism #894 added `is not` / contraction prefixes but not apostrophe suffixes already on constraint matcher; repro extend `IsObviousGenericAdvice_does_not_flag_negated_checklist_phrasing` with isn't variants — **fails today** (seed #895).
- [ ] (hunt-ready) `GenericArchitectureAdvicePatterns.IsNegatedAdviceFragment` — mid-sentence `doesn't require {phrase}` not negated vs `RequestConstraintTokenMatcher.ContainsMidSentenceNegation` — locus `ContainsAffirmativeAdviceFragment` prefix scan only; input `This workload doesn't require enable tls for internal hops.`; wrong outcome `IsObviousGenericAdvice` true; mechanism advice matcher lacks `doesn't require` mid-sentence guard present on constraint path since #894; repro `IsObviousGenericAdvice_does_not_flag_doesnt_require_mid_sentence_phrasing` — **fails today** (seed #895).
...
2026-09-05 seed hunt #895 (seed-only): reseeded after #894 closure; promoted five hunt-ready parity rows with failing repro probes; disproved string `"1"`/boolean anchor coercion gap.
