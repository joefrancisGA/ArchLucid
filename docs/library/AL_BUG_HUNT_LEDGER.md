...
- [x] (proven) `resolveOperatorBillingCurrentPlan` returned tenant-trial when `trialStatus` was Active but `isTrialUsage` was false with a commercial tier — stale trial flag overrode paid plan headline; fixed 2026-08-25 by gating tenant-trial branch on `isTrialUsage !== false` (`operator-billing-current-plan.test.ts`).
