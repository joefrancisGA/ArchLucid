...
- [x] (proven) `resolveOperatorBillingSubscriptionLoadState` returned unavailable when subscription finished with null before usage-status loaded — billing cards could flash unavailable during staggered query completion; fixed 2026-08-25 by staying pending until both sides fetch or one side has signal (`operator-billing-subscription-resolution.test.ts`).
