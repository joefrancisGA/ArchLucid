...
- [x] (proven) `RecentPilotRunDeltasService` counted `ReadyForCommit` runs with manifest ids as committed — **hit 2026-08-25 (seed):** `IsCommitted` keyed on `CurrentManifestVersion` instead of `ArchitectureRunStatus.Committed`; inflated recent-deltas medians (`RecentPilotRunDeltasServiceTests.GetRecentDeltasAsync_ReadyForCommitRunWithManifest_IsExcludedFromCommittedPanel`).
