...
- [x] (proven) `FindingJsonConverter` reads `humanReviewStatus` only when the token is a string; persisted JSON with numeric `1` left the default `NotRequired` or threw on deserialize — **hit 2026-08-25:** added `ReadHumanReviewStatus` with numeric ordinal + string parity (`FindingJsonConverterTests.Deserialize_numeric_humanReviewStatus_maps_pending`).
