...
- **BFF Origin:** browser session-bootstrap requests must use the same host as the Next server URL (`localhost` vs `127.0.0.1`). The BFF validates `Origin` against `request.nextUrl.origin`; a browser document opened on the other loopback host is correctly rejected as cross-site. CI therefore sets `PLAYWRIGHT_BASE_URL=http://localhost:3000` for JwtBearer lanes.
...
- **Private-beta gate:** **`ui-e2e-live-beta-access`** in **`ci.yml`** runs **`live-api-private-beta-access.spec.ts`** only, with **`NEXT_PUBLIC_ARCHLUCID_AUTH_MODE=jwt-bearer`** at build time and scope claims on the minted JWT — required before sending beta invites (TB-797). That `ci.yml` job is **`if: github.event_name != 'pull_request'`** and **`ci.yml` has no `push` trigger**, so it does **not** run on trunk merge. **`.github/workflows/private-beta-access-on-push.yml`** runs the same job name on **`push`** to **`main`/`master`** (and `workflow_dispatch`) without waiting for full regression, so the invite-wave path fires on trunk.
- **Simulator API startup:** `appsettings.Pilot.json` ships partial `AzureOpenAI` endpoint/deployment without an API key. Simulator live-API jobs must clear those keys via env (`AzureOpenAI__Endpoint=""`, `AzureOpenAI__ApiKey=""`, `AzureOpenAI__DeploymentName=""`, `AzureOpenAI__EmbeddingDeploymentName=""`) so `AzureOpenAiOptionsValidator` does not fail startup.
...
- **Expired invite recovery (wave 2):** harness **`POST /v1/e2e/invitations/set-expires`** forces **`ExpiresUtc`** in the past so **`GET /v1/auth/invitations/validate`** returns **`Expired`** without waiting for TTL.
...
- **Email-OTP invite path (not in jwt-bearer CI):** the private-beta push workflow builds with **`NEXT_PUBLIC_ARCHLUCID_AUTH_MODE=jwt-bearer`** only — it does **not** set **`NEXT_PUBLIC_ARCHLUCID_EMAIL_OTP_ENABLED`** or **`Auth:EmailOtp:Enabled`**. **`live-api-private-beta-wave-3.spec.ts`** gates the OTP placeholder on **`archlucid-ui/e2e/helpers/live-email-otp-harness.ts`**: enable both flags, supply a challenge-code capture sink, and set **`LIVE_E2E_EMAIL_OTP_CHALLENGE_CODE`** before running that lane locally or in a dedicated workflow.
