...
- **Email-OTP invite path (not in jwt-bearer CI):** the private-beta push workflow builds with **`NEXT_PUBLIC_ARCHLUCID_AUTH_MODE=jwt-bearer`** only — it does **not** set **`NEXT_PUBLIC_ARCHLUCID_EMAIL_OTP_ENABLED`** or **`Auth:EmailOtp:Enabled`**. **`live-api-private-beta-wave-3.spec.ts`** documents a skipped placeholder for OTP invite E2E; enabling it requires both flags plus a challenge-code capture harness (no OTP sink in CI today).
