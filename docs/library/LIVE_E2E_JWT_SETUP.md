...
- **Simulator API startup:** `appsettings.Pilot.json` is a scale-honest overlay and must **not** ship `AgentExecution:Mode=Real` or a partial `AzureOpenAI` endpoint/deployment without an API key (that combination fails local `dotnet run`). Simulator live-API jobs still clear those keys via env (`AzureOpenAI__Endpoint=""`, `AzureOpenAI__ApiKey=""`, `AzureOpenAI__DeploymentName=""`, `AzureOpenAI__EmbeddingDeploymentName=""`) so a future overlay regression cannot fail `AzureOpenAiOptionsValidator`.
