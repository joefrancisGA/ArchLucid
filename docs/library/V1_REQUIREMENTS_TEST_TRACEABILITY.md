...
| **§4 Happy path** | [`FIRST_RUN_WALKTHROUGH.md`](FIRST_RUN_WALKTHROUGH.md), [`LIVE_E2E_HAPPY_PATH.md`](LIVE_E2E_HAPPY_PATH.md), [`operator-shell.md`](operator-shell.md) | `archlucid-ui` (wizard + E2E); `ArchLucid.Api.Tests` (architecture flows) | `FullyQualifiedName~NewRunWizardClient` OR `FullyQualifiedName~ArchitectureIngestion` | First-run wizard route **`/reviews/new`** (legacy **`/runs/new`** may redirect). HTTP spine narrative (request → execute → commit → retrieval → ask): **[`onboarding/day-one-developer.md`](../onboarding/day-one-developer.md#following-the-request-past-create-execute--commit--retrieval--ask)**. |
