...
**Maintenance map:** which TypeScript files correspond to which packaging layer is summarized in **PRODUCT_PACKAGING.md** §3 *Code seams (operator UI — maintenance map)*; which **UI concern** maps to which module is enumerated in **PRODUCT_PACKAGING.md** §3 *Two UI shaping surfaces*—update both when you add groups or split rank vs mutation wiring on a page.
