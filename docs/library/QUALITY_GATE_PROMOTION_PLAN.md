...
| Hosted quality gate + judges + reranking (**TB-684**) | **Staging promoted 2026-07-08** — `EnforceOnReject`/`BlockRunOnReject` already true; `LlmJudge` + `Retrieval:Reranking` enabled; advanced retrieval (query rewrite, HyDE, Graph-RAG) enabled in Staging only | Production advanced-retrieval promotion after **TB-683** trend + **TB-595** ablation show no floor breach for 2 weeks; attach nightly eval artifact to sign-off | AI platform | Rollback: `ArchLucid:Agents:LlmJudge:Enabled=false`, `Retrieval:Reranking:Enabled=false`, `Retrieval:Advanced:Enabled=false` |
