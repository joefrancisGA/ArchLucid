...
| **ArchLucid DEV** (optional) | **`8aa56f3b-18bc-43ca-ad45-bad9e811d33b`** | (env secret `AZURE_TENANT_ID`) | **`centralus`** (same primary region as staging/production; runtime RG example **`rg-ArchLucid-dev-cus`** — ACR may remain in legacy **`rg-ArchLucid-dev`**) | (env secret `AZURE_CLIENT_ID` — same or separate app registration as staging; see §5.1.1) | `dev` | **Optional** — CD target `dev` in [`cd.yml`(../../.github/workflows/cd.yml); use for images + Terraform + Container Apps against a personal/squad sandbox. |
...
3. **Region:** Sandbox primary region is **`centralus`**. Set Environment variable/secret **`EXPECTED_AZURE_LOCATION`** = `centralus` when using deploy-target preflight. Prefer a new runtime RG in Central US (example **`rg-ArchLucid-dev-cus`** with `create_resource_group = true` in [`dev.tfvars.example`](../../infra/terraform-container-apps/dev.tfvars.example)) when the legacy **`rg-ArchLucid-dev`** metadata location is still **eastus2** — with `create_resource_group = false`, Terraform follows the existing RG location, not only the `location` tfvar.
4. **ACR:** Registry **`acrarchluciddev`** may remain in legacy **`rg-ArchLucid-dev`** while Container Apps run in **`rg-ArchLucid-dev-cus`**. Set **`ACR_LOGIN_SERVER`** = `acrarchluciddev.azurecr.io`, **`ACR_NAME`** = `acrarchluciddev`. Grant the federated identity **AcrPush** (and **Reader** on the RG if required by your org) on that registry.
5. **Container Apps:** After first Terraform apply creates apps in the centralus runtime RG, set **`AZURE_RESOURCE_GROUP`** (e.g. **`rg-ArchLucid-dev-cus`**), **`CONTAINER_APP_API_NAME`**, optional **`CONTAINER_APP_WORKER_NAME`**, **`CONTAINER_APP_UI_NAME`** to match Terraform outputs.
6. **Terraform:** Set **`TF_WORKING_DIRECTORY`** = `infra/terraform-container-apps`. Copy [`dev.tfvars.example`](../../infra/terraform-container-apps/dev.tfvars.example) locally (`location` / `content_safety_location` = **`centralus`**), fill **artifact** storage URIs (apply [`infra/terraform-storage`](../../infra/terraform-storage) in this subscription first if needed), then paste the full file into GitHub Environment secret **`DEV_TFVARS`** so the workflow can materialize `dev.tfvars` before `terraform plan` (see [`DEPLOYMENT_CD_PIPELINE.md`](DEPLOYMENT_CD_PIPELINE.md)).
7. **Content Safety (required on Staging/Production-like API hosts):** Prefer account **`archlucid-dev-contentsafety`** in the **centralus** runtime RG. Set Environment secrets **`ARCHLUCID_CONTENT_SAFETY_ENDPOINT`** and **`ARCHLUCID_CONTENT_SAFETY_API_KEY`**. CD heals the API Container App env + secret **`al-cs-key`** on each deploy. Do **not** use **`rg-longevity-dev-01`** / `longevity-safety` — that RG is a separate lab stack, not ArchLucid.
8. **Smoke:** Set **`SMOKE_TEST_BASE_URL`** to the public API URL (e.g. Front Door or default Container Apps FQDN) when you want post-deploy checks.
9. **Run:** Actions → **CD** → `workflow_dispatch` → target **`dev`**. Image tags also receive **`latest-dev`**.
