...
6. **Content Safety (required on Staging/Production-like API hosts):** Own account **`archlucid-dev-contentsafety`** in **`rg-ArchLucid-dev`** (eastus2). Set Environment secrets **`ARCHLUCID_CONTENT_SAFETY_ENDPOINT`** = `https://archlucid-dev-contentsafety.cognitiveservices.azure.com/` and **`ARCHLUCID_CONTENT_SAFETY_API_KEY`** (key1 from that account). CD heals the API Container App env + secret **`al-cs-key`** on each deploy. Do **not** use **`rg-longevity-dev-01`** / `longevity-safety` — that RG is a separate lab stack, not ArchLucid.
7. **Smoke:** Set **`SMOKE_TEST_BASE_URL`** to the public API URL (e.g. Front Door or default Container Apps FQDN) when you want post-deploy checks.
8. **Run:** Actions → **CD** → `workflow_dispatch` → target **`dev`**. Image tags also receive **`latest-dev`**.
