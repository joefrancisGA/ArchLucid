...
| **ArchLucid DEV** (optional) | **`8aa56f3b-18bc-43ca-ad45-bad9e811d33b`** | (env secret `AZURE_TENANT_ID`) | **`centralus`** (same primary region as staging/production; RG / ACR / Content Safety for the sandbox must live in Central US) | (env secret `AZURE_CLIENT_ID` — same or separate app registration as staging; see §5.1.1) | `dev` | **Optional** — CD target `dev` in [`cd.yml`(../../.github/workflows/cd.yml); use for images + Terraform + Container Apps against a personal/squad sandbox. |
...
3. **Region:** Sandbox primary region is **`centralus`**. Set Environment variable/secret **`EXPECTED_AZURE_LOCATION`** = `centralus` when using deploy-target preflight. If **`rg-ArchLucid-dev`** (or ACR / Content Safety) still live in **eastus2**, recreate or migrate them into **centralus** before expecting CD to land there — with `create_resource_group = false`, Terraform follows the existing RG location, not only the `location` tfvar.
4. **ACR:** Registry **`acrarchluciddev`** should be in **`rg-ArchLucid-dev`** in **centralus** (or a documented centralus successor RG). Set **`ACR_LOGIN_SERVER`** = `acrarchluciddev.azurecr.io`, **`ACR_NAME`** = `acrarchluciddev`. Grant the federated identity **AcrPush** (and **Reader** on the RG if required by your org) on that registry.
5. **Container Apps:** After first Terraform apply creates apps in your chosen RG, set **`AZURE_RESOURCE_GROUP`**, **`CONTAINER_APP_API_NAME`**, optional **`CONTAINER_APP_WORKER_NAME`**, **`CONTAINER_APP_UI_NAME`** to match Terraform outputs.
...
7. **Content Safety (required on Staging/Production-like API hosts):** Own account **`archlucid-dev-contentsafety`** in **`rg-ArchLucid-dev`** (**centralus**). Set Environment secrets **`ARCHLUCID_CONTENT_SAFETY_ENDPOINT`** = `https://archlucid-dev-contentsafety.cognitiveservices.azure.com/` and **`ARCHLUCID_CONTENT_SAFETY_API_KEY`** (key1 from that account). CD heals the API Container App env + secret **`al-cs-key`** on each deploy. Do **not** use **`rg-longevity-dev-01`** / `longevity-safety` — that RG is a separate lab stack, not ArchLucid.
