...
**Normative decisions** that conflict with this catalog belong in a new or amended [Architecture Decision Record](../adr/README.md); this file is the **checklist and ID registry** for enforcement work tracked in [`TECH_BACKLOG.md`](TECH_BACKLOG.md).
...
**Why:** Cross-tenant data access is irreversible reputational failure; aligns with mental model of [ADR 0003](../adr/0003-sql-rls-session-context.md) (RLS as defense in depth, not the only control).
...
**Intent:** All outbound calls inherit correlation, circuit breaking, and retry policy from [ADR 0005](../adr/0005-llm-completion-pipeline.md) composition.
...
- [ADR 0035 (Proposed)](../adr/0035-architecture-invariant-catalog.md) — governance and acceptance  
