...
|--------|-----------|--------|--------|------------------|
| Merged **line** | **74%** (matches CI Cobertura ~**74.99%** on run [30882961646](https://github.com/joefrancisGA/ArchLucid/actions/runs/30882961646); target V1.1 **95%**) | [`scripts/ci/assert_merged_line_coverage_min.py`](../../scripts/ci/assert_merged_line_coverage_min.py) (positional **`74`** in **`.github/workflows/ci.yml`** `dotnet-coverage-merge`) | `.NET: merge coverage + gates` (`dotnet-coverage-merge`) | Exit **1** if root `line-rate` × 100 is below the floor. |
| Merged **branch** | **59%** (matches CI Cobertura ~**59.67%** on the same run) | same (`--min-branch-pct 59`) | same | Exit **1** if root `branch-rate` × 100 is below the floor. |
