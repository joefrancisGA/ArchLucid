...
|--------|-----------|--------|--------|------------------|
| Merged **line** | **76%** (owner ratchet; prior CI Cobertura ~**74.99%** on run [30882961646](https://github.com/joefrancisGA/ArchLucid/actions/runs/30882961646); target V1.1 **95%**) | [`scripts/ci/assert_merged_line_coverage_min.py`](../../scripts/ci/assert_merged_line_coverage_min.py) (positional **`76`** in **`.github/workflows/ci.yml`** `dotnet-coverage-merge`) | `.NET: merge coverage + gates` (`dotnet-coverage-merge`) | Exit **1** if root `line-rate` × 100 is below the floor. |
| Merged **branch** | **61%** (owner ratchet; prior CI Cobertura ~**59.67%** on the same run) | same (`--min-branch-pct 61`) | same | Exit **1** if root `branch-rate` × 100 is below the floor. |
| Per-product **line** | **81%** (tracks lowest gated package ~**81.42%** on the same run) | same (`--min-package-line-pct 81`; script default **60**) | same | Exit **1** if any gated **`ArchLucid.*`** product package is below the floor or has coverable lines but missing `line-rate`. **`--skip-package-line-gate`** for **`ArchLucid.Api`** (Integration HTTP tests without Coverlet on integration shards), **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, and RC interim skips for **`ArchLucid.Cli`**, **`ArchLucid.Host.Core`**, **`ArchLucid.Persistence`**, **`ArchLucid.Application`**, **`ArchLucid.AgentRuntime`**, **`ArchLucid.Host.Composition`** until coverage batches close the gap. |
