...
|--------|-----------|--------|--------|------------------|
| Merged **line** | **77%** (interim; target V1.1 **95%**) | [`scripts/ci/assert_merged_line_coverage_min.py`](../../scripts/ci/assert_merged_line_coverage_min.py) (positional **`77`** in **`.github/workflows/ci.yml`** `dotnet-coverage-merge`) | `.NET: merge coverage + gates` (`dotnet-coverage-merge`) | Exit **1** if root `line-rate` × 100 is below the floor. |
| Merged **branch** | **59%** | same (`--min-branch-pct 59`) | same | Exit **1** if root `branch-rate` × 100 is below the floor. |
...
**Ratchet (merged line):** **[`.coverage-floor`](../../.coverage-floor)** is the committed anchor (**79.00**; effective minimum **77%** with default **2%** slack); **`assert_coverage_floor_ratchet.py`** runs in **`dotnet-coverage-merge`** after the merged-line floor. Target ratchet path: **80 → 95%** (V1.1). See **[`V1_DEFERRED.md`](V1_DEFERRED.md)**.
