...
- **Shipped default (`ArchLucid.Api/appsettings.json`):** **`Enabled: true`** with conservative **warn** thresholds (**`StructuralWarnBelow` 0.3**, **`SemanticWarnBelow` 0.2**) and **reject floors at `0`** (no automatic **`rejected`** outcome for normal 0.0–1.0 scores — warn-only V1). Raise **`StructuralRejectBelow` / `SemanticRejectBelow`** and set **`EnforceOnReject: true`** when product wants hard failures.
- **Local Development (`appsettings.Development.json`):** matches the same block so **`dotnet run`** sees the same **`warned`** metrics and logs as staging—set **`Enabled: false`** only when you intentionally silence gate noise.
...
When enabled, **`AgentOutputEvaluationRecorder`** increments **`archlucid_agent_output_quality_gate_total`** (labels `agent_type`, `outcome`) and logs **warn** for **warned**/**rejected** outcomes. A **`warned`** outcome also sets **`AgentExecutionTrace.QualityWarning`** (persisted in **`TraceJson`**) so **`GET …/agent-evaluation`** and trace reads expose the flag. **`EnforceOnReject`** remains **`false`** by default so runs are not aborted unless operators opt in.
