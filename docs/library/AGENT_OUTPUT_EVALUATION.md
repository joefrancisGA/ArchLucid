...
- **Production-like / production (`appsettings.Staging.json`, `appsettings.Production.json`):** **`Mode: PilotStrict`** with **`EnforceOnReject: true`**, **`BlockRunOnReject: true`**, and aligned **`PilotStrict*`** / warn/reject values so weak traces cannot complete as **`ReadyForCommit`**.
