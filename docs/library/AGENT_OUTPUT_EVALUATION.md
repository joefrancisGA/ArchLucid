...
**Release cohort green bar (planning baseline, 2026-05-09):** Tiered targets on the committed **release cohort** under **`gpt-4o`** (structural completeness, quality-gate rejects, semantic score percentiles, explainability trace completeness mean, adversarial qualitative bar) are summarized in [`GOLDEN_COHORT_REAL_LLM_GATE.md`](../runbooks/GOLDEN_COHORT_REAL_LLM_GATE.md) §10 — **not** merge-blocking unless you wire automation to enforce them.
