...
| Tenant architecture review board cover logo upload | `AdminController` (`POST /v1/admin/tenant/logo`) | — | Tenant from ambient scope | PNG/JPEG validated via `ArchitectureReviewBoardCoverLogoValidator`; persisted by `TenantReviewBoardCoverLogoStore`; **no** durable audit row (tenant branding blob overwrite) |
