...
| LLM completion output truncated (`finish_reason=length`) | `AuditLlmCompletionOutputTruncationReporter` | `LlmCompletionOutputTruncated` | Tenant/Workspace/Project from ambient scope | `{ deploymentName, maxOutputTokens, outputTokenCount, reasoningTokenCount }` — fire-and-forget; run processing continues. |
