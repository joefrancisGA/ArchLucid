...
| Architecture request draft overview rewrite (advisory, no persistence) | `RunsController` (`POST /v1/architecture/request/draft/overview-rewrite`); `ArchitectureOverviewRewriteService` | — | — | Execute-auth gated LLM rewrite assist; returns rewritten overview text only — **no** durable audit row (`[MutatingAuditExcluded]` on controller) |
| Architecture request draft clarification rephrase (advisory, no persistence) | `RunsController` (`POST /v1/architecture/request/draft/clarification-answers/rephrase`); `ClarificationAnswerRephraseService` | — | — | Execute-auth gated LLM rephrase assist; returns rephrased clarification answers only — **no** durable audit row (`[MutatingAuditExcluded]` on controller) |
| Architecture request draft suggestion explain (advisory, no persistence) | `RunsController` (`POST /v1/architecture/request/draft/suggestion/explain`); `StructuredBriefSuggestionExplainService` | — | — | Execute-auth gated LLM explain assist; returns narrative only — **no** durable audit row (`[MutatingAuditExcluded]` on controller) |
| Architecture request async create (Tier C) | `RunsController` (`POST /v1/architecture/request/async`); `ArchitectureRunAsyncOperationAcceptor` | `RequestCreated` (async worker after 202) | Tenant/Workspace/Project from ambient scope | 202 + `Location: /v1/operations/run:{runId}`; controller `[MutatingAuditExcluded]` — durable `RequestCreated` emitted by worker |
...
| Socratic intake drafts (ADR 0048 / 0051) | `DraftRequestsController` (`POST /v1/architecture/draft`; `POST /v1/architecture/draft/{draftId}/abandon`; `POST /v1/architecture/draft/{draftId}/admit`; `POST /v1/architecture/draft/{draftId}/answer`; `POST /v1/architecture/draft/{draftId}/branch`; `POST /v1/architecture/draft/{draftId}/reason`; `POST /v1/architecture/draft/{draftId}/reopen`; `POST /v1/architecture/draft/{draftId}/skip`; `POST /v1/architecture/draft/{draftId}/submit`) | `DraftIntakeCreated`, `DraftIntakePatched`, `DraftIntakeQuestionAnswered`, `DraftIntakeQuestionSkipped`, `DraftIntakeBranched`, `DraftIntakeReasoned`, `DraftIntakeReopened`, `DraftIntakeAdmissionEvaluated`, `DraftIntakeSubmitted`, `DraftIntakeAbandoned`, `DraftIntakeTerminalPurged` | Tenant/Workspace/Project from ambient scope | `{ draftId, status?, questionKey?, parentDraftId?, branchDraftId?, overrideKind? }` |
...
| Governance coverage preview (read-only) | `GovernanceCoverageController` (`POST /v1/governance/coverage/preview`); `CoveragePreviewService` | — | — | Read-auth gated what-if coverage disclosure; **no** durable audit row (`[MutatingAuditExcluded]` on controller) |
...
| `FindingMergeConflictResolved` | `FindingMergeConflictResolved` | `GovernanceStickinessFacade` (`POST /v1/governance/runs/{runId}/finding-merge-conflicts/{findingId}/resolve`) |
