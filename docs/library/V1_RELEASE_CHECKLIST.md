...
- [ ] **Release confidence rollup** present for RC signoff: `release-confidence-rollup.json` / `.md` summarizes validation lanes (full regression, outbox tests, release evidence tests, doc links, UI unit) with explicit MISSING/STALE — does not run full regression locally by default.
