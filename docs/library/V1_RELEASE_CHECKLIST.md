...
- [ ] **Strict RC mode** for buyer-facing signoff: pass `-StrictRc` to `scripts/Emit-ReleaseReadinessEvidence.ps1` (or set `ARCHLUCID_STRICT_RC=1`) so release-blocking lanes must be PASS; inspect `strictDisposition` and `strictBlockingReasons` in the rollup JSON.
