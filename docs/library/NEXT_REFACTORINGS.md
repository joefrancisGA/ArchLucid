...
**2026-09-03 pass 10 — suggestions only; do not treat this list as in-progress implementation.** Evidence from current line counts after PR **#1265** (pass-9 items 1–10). Same constraints as prior passes: keep HTTP routes, OpenAPI wire shapes, and tenant isolation unchanged unless a follow-up explicitly says otherwise.

1. **Split leftover `ManifestsController.Get` and `UserPreferencesController` / `TenantWorkspacesController` route families** — Pass-9 split PolicyPacks CRUD/assignment and RunComparison Agents/Replay; leftovers: `ManifestsController.Get` **323** (manifest GET, diagram, summary, feasibility in one partial), `UserPreferencesController` **313** (appearance, cloud visibility, notification prefs — no partials), `TenantWorkspacesController` **309** (workspace list, project CRUD, retention purge). Split Get into `Get.Manifest` / `Get.Diagram` / `Get.Summary` partials; split preferences and workspaces by route family; keep facade delegation; routes and auth stay equivalent. **Impact:** High (manifest reads / tenant workspace APIs) · **Effort:** Low–Medium · **Paths:** `ArchLucid.Api/Controllers/Governance/ManifestsController.Get.cs`, `ArchLucid.Api/Controllers/User/UserPreferencesController.cs`, `ArchLucid.Api/Controllers/Tenancy/TenantWorkspacesController.cs`

2. **Decompose `TopologyProposalRelationshipEndpointIndex` and `GoldenArchitectureTestRunner` into stages/partials** — Pass-9 staged dry-run / prepared-zip ingest; leftovers: topology index **361** mixes service-endpoint keys, datastore keys, synthetic keys, and relationship validation; golden-architecture runner **351** mixes closed-loop invoke, benchmark comparison, and specialist-review aggregation. Extract `*.ServiceEndpoints` / `*.DatastoreEndpoints` / `*.RelationshipValidation` partials and `IGoldenArchitectureInvokeStage` / `IGoldenArchitectureBenchmarkStage`; parents become sequencers. **Impact:** High (authority topology / closed-loop eval correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Application/Runs/Orchestration/TopologyProposalRelationshipEndpointIndex.cs`, `ArchLucid.Application/ArchitectureIntelligence/GoldenArchitectureTestRunner.cs`

3. **Extend leftover persistence twin cores (`RunRepositoryCore` split + draft adapter thinning + golden-manifest hydrate)** — Pass-9 expanded draft/findings/outbox/ITSM cores; leftovers: `RunRepositoryCore` itself is now a **443**-line god (validation, SQL constants, keyset paging, row mapping); `InMemoryDraftRequestRepository` still **383** despite `DraftRequestRepositoryCore`; `GoldenManifestPhase1RelationalRead.Hydrate` **362** mixes slice-count probes and section hydration; `SqlRunRepository.Query` **346** still mixes committed-run / graph-at-time / list-keyset SQL. Split Run Core into Query/Write/InMemoryOrdering slices; push remaining draft list/map helpers into Core; split Hydrate by manifest section. **Impact:** High (run list / draft / golden-manifest parity) · **Effort:** Medium · **Paths:** `ArchLucid.Persistence/Repositories/RunRepositoryCore.cs`, `ArchLucid.Persistence/Data/Repositories/InMemoryDraftRequestRepository.cs`, `ArchLucid.Persistence/GoldenManifests/GoldenManifestPhase1RelationalRead.Hydrate.cs`, `ArchLucid.Persistence/Repositories/SqlRunRepository.Query.cs`

4. **Split `GcpCloudBillingCatalogClient` and `HeuristicAgentOutputSemanticEvaluator`** — Largest leftover Core/AgentRuntime gods: GCP catalog client **520** mixes HTTP fetch, 24h cache, SKU JSON parse, and monthly-cost heuristics; heuristic evaluator **372** mixes rubric scoring, claim/evidence alignment, and trace-quality checks. Extract `GcpCatalogHttpClient` / `GcpSkuCache` / `GcpSkuPricingParser`; split evaluator into `ClaimEvidence` / `TraceQuality` stages. **Impact:** High (cost catalog / agent-output eval correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Core/Costing/GcpCloudBillingCatalogClient.cs`, `ArchLucid.AgentRuntime/Evaluation/HeuristicAgentOutputSemanticEvaluator.cs`

5. **Split leftover CLI `DraftNewCommandIntakeLoop` and `BuyerProofPackCommand`** — Pass-9 extracted tenant-isolation offline/live and deployment-evidence gates; leftovers: intake loop **378** still mixes connect/auth, draft create/admit, MUST-question loop, submit, and execute; `BuyerProofPackCommand` **331** mixes arg parse leftovers, API fetch, ZIP assembly, and evidence ledger. Extract `DraftNewCommandConnectStage` / `AdmitStage` / `MustQuestionLoop`; split buyer-proof into fetch vs ZIP/ledger writers. **Impact:** Medium (guided intake CLI / GTM proof packets) · **Effort:** Medium · **Paths:** `ArchLucid.Cli/Commands/DraftNewCommandIntakeLoop.cs`, `ArchLucid.Cli/Commands/BuyerProofPackCommand.cs`

6. **Continue UI mega-client split (showcase SSR, tenant cost settings, graph controls, run-detail shell)** — Pass-9 thinned wizard hooks / admin tenants / deferred chunks; remaining gods: marketing `showcase/[runId]/page.tsx` **447** (live vs static-demo SSR + section composition), `TenantCostSettingsCard.tsx` **435** (form state, save, checklist, audit), `GraphPageControls.tsx` **413** (filters, layout toggles, query wiring), `RunDetailPageView.tsx` **403** (shell still orchestrates deferred chunks + tab chrome). Extract resolution/state hooks and control shells; pages become composition only. **Impact:** High (showcase / cost settings / graph / run-detail UX) · **Effort:** Medium · **Paths:** `archlucid-ui/src/app/(marketing)/showcase/[runId]/page.tsx`, `archlucid-ui/src/app/(operator)/administration/workspace-settings/_sections/TenantCostSettingsCard.tsx`, `archlucid-ui/src/app/(operator)/insights/evidence-graph/_sections/GraphPageControls.tsx`, `archlucid-ui/src/app/(operator)/architecture/reviews/[reviewId]/_sections/RunDetailPageView.tsx`

7. **Split leftover UI API catalogs (`policy-governance-api`, `downloads-api`) and runtime diagnostics** — Pass-9 split `http.ts` / design-token shell; leftovers: `policy-governance-api.ts` **404** mixes policy-pack CRUD, dry-run, governance workflow, and alert-routing helpers; `downloads-api.ts` **440** mixes blob download, content-disposition, and export-job polling; `client-runtime-diagnostics.ts` **467** mixes soft-nav stuck detection, overlay probing, long-task stalls, and hard-fallback navigation. Split by surface (`policy-packs-api`, `governance-workflow-api`, `downloads-blob` / `downloads-export-jobs`) and diagnostic concern. **Impact:** High (operator API client / download / diagnostics maintainability) · **Effort:** Medium · **Paths:** `archlucid-ui/src/lib/api/policy-governance-api.ts`, `archlucid-ui/src/lib/api/downloads-api.ts`, `archlucid-ui/src/lib/client-runtime-diagnostics.ts`

8. **Split leftover `authority.ts` composites (wave 11 type modules)** — Wave 10 shipped `authority-run-detail-wire.ts` and **245** OpenAPI alias keys; leftover `authority.ts` **272** still mixes `RunSummaryWireExtensions`, `ManifestSummary` required-picks, `RunDetail` snapshot merges, provenance graph, and pipeline timeline. Split into `authority-run-summary.ts`, `authority-manifest.ts`, `authority-run-detail.ts`; keep documented UI-only extensions; extend `openapi-type-aliases.test.ts` (do not drop the empty-`AgentResult` snapshot guard). **Impact:** Medium (contract drift on run-detail / summary) · **Effort:** Low–Medium · **Paths:** `archlucid-ui/src/types/authority.ts`, `archlucid-ui/src/types/authority-run-detail-wire.ts`, `archlucid-ui/src/types/openapi-type-aliases.test.ts`

9. **Split leftover Host.Composition `DataHealthJobsCompositionModule`** — Pass-9 split CoordinatorArtifacts Artifacts/Explanation and hot-path registrar families; leftover DI god **297** registers health checks, data-consistency reconciliation, and background jobs in one module. Extract `DataHealthJobsCompositionModule.HealthChecks.cs` / `.Reconciliation.cs` / `.BackgroundJobs.cs`; keep `Register` as a thin composer. **Impact:** Medium (DI / hosted-job maintainability) · **Effort:** Low–Medium · **Paths:** `ArchLucid.Host.Composition/Startup/Modules/DataHealthJobsCompositionModule.cs`

10. **Split `SchemaValidationService` schema registry vs cache vs validate** — Decisioning leftover **301** mixes lazy schema load, LRU cache, metrics, and per-schema `Validate*` methods. Extract `SchemaValidationRegistry` (lazy schemas) and `SchemaValidationCache`; service becomes a thin validator over those. Keep validation results and metrics semantics identical. **Impact:** Medium (authority schema-gate correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Decisioning/Validation/SchemaValidationService.cs`
