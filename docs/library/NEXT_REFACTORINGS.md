...
**2026-09-03 pass 12 — suggestions only; do not treat this list as in-progress implementation.** Evidence from current line counts after PR **#1318** (pass-11 items 1–10). Same constraints as prior passes: keep HTTP routes, OpenAPI wire shapes, and tenant isolation unchanged unless a follow-up explicitly says otherwise.

1. **Split leftover `TenantCustomerSuccessController`, `GovernanceController.ApprovalRequests`, and `ArtifactExportController.Export` route families** — Pass-11 split PolicyPacks Catalog/Simulate/DryRun/DraftGenerate and ITSM list/mutate/OAuth; leftovers: `TenantCustomerSuccessController` **313** (health-score, next-actions, funnel/stickiness snapshots, product-feedback — no partials), `GovernanceController.ApprovalRequests` **302** (create, approve, reject, batch-review in one partial), `ArtifactExportController.Export` **294** (decision-receipt, export download/verify, terraform-advisory, blob push, terraform-PR). Split customer-success into `Health` / `Funnel` / `Feedback` partials; split approvals into `Create` / `Review` / `Batch`; split export into `Download` / `Verify` / `Push`. Keep facade delegation; routes and auth stay equivalent. **Impact:** High (customer-success / governance approvals / artifact export APIs) · **Effort:** Low–Medium · **Paths:** `ArchLucid.Api/Controllers/Tenancy/TenantCustomerSuccessController.cs`, `ArchLucid.Api/Controllers/Governance/GovernanceController.ApprovalRequests.cs`, `ArchLucid.Api/Controllers/Authority/ArtifactExportController.Export.cs`

2. **Decompose `ArchitectureReviewPdfBuilder.Sections` and `ArchitectureIntelligenceBenchmark` into stages/partials** — Pass-11 staged run-findings query and payload-complexity partials; leftovers: PDF builder sections **409** mixes sponsor/system-overview/evidence and remaining board-section renderers in one switch; intelligence benchmark **349** mixes visible microcases, held-out cases, and mutation tests. Extract `ArchitectureReviewPdfBuilder.Sections.Sponsor` / `.Evidence` / `.Board` partials; split benchmark into `VisibleMicrocases` / `HeldOut` / `Mutation` sources; parents become sequencers. **Impact:** High (ARB PDF export / extraction-fidelity eval correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Application/Exports/ArchitectureReviewBoard/ArchitectureReviewPdfBuilder.Sections.cs`, `ArchLucid.Application/ArchitectureIntelligence/ArchitectureIntelligenceBenchmark.cs`

3. **Extend leftover persistence (`SqlLlmTenantBudgetRepository.Monthly` + `ContextSnapshotRelationalRead` + `DapperCompositeAlertRuleRepository`)** — Pass-11 split Cosmos trace Query and SqlRun Write; leftovers: monthly LLM budget **333** mixes period-key, get-or-create, reserve, and settle; context-snapshot hydrate **333** mixes slice-presence probes, source hashes, and canonical-object load; composite alert-rule Dapper **330** mixes list, upsert, and enable-state. Split Monthly into `.Period` / `.Reserve` / `.Settle`; Hydrate into `.SlicePresence` / `.CanonicalObjects`; Dapper into list vs mutate partials. **Impact:** High (LLM budget / context-snapshot hydrate / alert-rule parity) · **Effort:** Medium · **Paths:** `ArchLucid.Persistence/Data/Repositories/SqlLlmTenantBudgetRepository.Monthly.cs`, `ArchLucid.Persistence/ContextSnapshots/ContextSnapshotRelationalRead.cs`, `ArchLucid.Persistence/Alerts/DapperCompositeAlertRuleRepository.cs`

4. **Split `MarketplaceWebhookPayloadParser` and `FindingJsonConverter.EnumReaders`** — Pass-11 extracted finding JSON string/numeric/date readers and LLM-judge eligibility/complete-parse; leftovers: Marketplace parser **398** mixes plan-id → tier mapping, ChangePlan/ChangeQuantity field extract, and quantity coerce; enum readers **359** mix finding-status, severity, category, and confidence enums. Extract `MarketplacePlanIdMapper` / `MarketplaceChangePlanReader` / `MarketplaceQuantityReader`; split enum readers by enum family. Keep parse results identical. **Impact:** High (Marketplace webhook / finding deserialize correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Core/Billing/AzureMarketplace/MarketplaceWebhookPayloadParser.cs`, `ArchLucid.Core/Findings/Serialization/FindingJsonConverter.EnumReaders.cs`

5. **Split leftover CLI `ArchLucidCliApiClient.Comparisons` and `ReturnTriggerTelemetrySessionParser`** — Pass-11 extracted Runs Create/ExecuteCommit/FingerprintSeed and golden-cohort drift modules; leftovers: comparisons client **347** mixes search, single replay-to-file, batch ZIP, and drift/diagnostics JSON; `ReturnTriggerTelemetrySessionParser` **323** mixes arg parse, session JSON walk, and trigger-slot scoring. Extract `ArchLucidCliApiClient.Comparisons.Search` / `.Replay` / `.Drift`; split telemetry parser into parse vs score. **Impact:** Medium (CLI comparison replay / return-trigger telemetry) · **Effort:** Medium · **Paths:** `ArchLucid.Cli/ArchLucidCliApiClient.Comparisons.cs`, `ArchLucid.Cli/Commands/ReturnTriggerTelemetrySessionParser.cs`

6. **Continue UI mega-client split (structured brief, audit search, alerts inbox, CTO demo overlay)** — Pass-11 thinned global search, draft list, run-detail tabs, and account security; remaining gods: `architecture-draft-structured-brief.ts` **487** (confirmed/suggested/denied field state + apply helpers), `use-audit-page-search.ts` **474** (URL date-range, CTO-demo filter, query/pagination), `use-alerts-inbox-controller.ts` **470** (inbox queries, batch actions, keyboard shortcuts), `BuyerCtoDemoTourOverlay.tsx` **445** (preflight, presenter notes, navigation chrome). Extract hooks and shells; parents become composition only. **Impact:** High (draft brief / audit / alerts / CTO-demo UX) · **Effort:** Medium · **Paths:** `archlucid-ui/src/lib/architecture/architecture-draft-structured-brief.ts`, `archlucid-ui/src/app/(operator)/governance/audit/_sections/use-audit-page-search.ts`, `archlucid-ui/src/components/alerts/use-alerts-inbox-controller.ts`, `archlucid-ui/src/components/BuyerCtoDemoTourOverlay.tsx`

7. **Split leftover UI API catalogs (`architecture-runs`, `alerts-api`) and `proxy-forward`** — Pass-11 split stickiness barrel and architecture-runs create/lifecycle; leftovers: `architecture-runs.ts` **364** mixes paged list, run-detail/summary, provenance/timeline, traces/forensics, and artifact fetch; `alerts-api.ts` **304** mixes rules CRUD, inbox list/actions, routing subscriptions, and webhook dry-run; `proxy-forward.ts` **428** mixes header copy, body streaming, and error mapping. Split by surface (`architecture-runs-list` / `architecture-runs-read` / `architecture-runs-artifacts`; `alerts-inbox-api` / `alerts-routing-api`) and proxy concern; keep barrel re-exports. **Impact:** High (operator run-read / alerts / Next proxy maintainability) · **Effort:** Medium · **Paths:** `archlucid-ui/src/lib/api/architecture-runs.ts`, `archlucid-ui/src/lib/api/alerts-api.ts`, `archlucid-ui/src/lib/proxy/proxy-forward.ts`

8. **Split leftover type composites (wave 13 type modules)** — Wave 12 shipped explanation-run/structured, draft-intake-actors, and run-detail trust/provenance barrels with **245** OpenAPI alias keys; leftovers: `draft-intake.ts` **156** still mixes lifecycle status, branch overrides, and document shape; `agent-forensics.ts` **135** mixes trace rows, semantic scores, and tool-invocation forensics; `explanation-structured.ts` **134** mixes structured envelope, finding confidence, and narrative helpers. Split into `draft-intake-document.ts`, `agent-forensics-traces.ts` / `agent-forensics-scores.ts`, `explanation-confidence.ts`; keep documented UI-only extensions; extend `openapi-type-aliases.test.ts` (do not drop the empty-`AgentResult` snapshot guard). **Impact:** Medium (contract drift on draft-intake / forensics / explanation) · **Effort:** Low–Medium · **Paths:** `archlucid-ui/src/types/draft-intake.ts`, `archlucid-ui/src/types/agent-forensics.ts`, `archlucid-ui/src/types/explanation-structured.ts`, `archlucid-ui/src/types/openapi-type-aliases.test.ts`

9. **Split leftover Host.Composition agent Azure OpenAI / LLM-support / execution modules** — Pass-11 split RunLifecycle Coverage/CreateHooks/ExecuteStages and ServiceCollectionExtensions probes/HTTP clients; leftovers: `AgentAzureOpenAiExecutorRegistrar` **260** registers completion pipeline, circuit breakers, and tier router in one `Register`; `AgentLlmSupportCompositionModule` **257** and `AgentExecutionCompositionModule` **257** still mix prompt/token, schema-completion, and executor wiring. Extract `AgentAzureOpenAiExecutorRegistrar.Completion` / `.CircuitBreaker` / `.TierRouter`; keep `Register` as a thin composer. **Impact:** Medium (DI / real-mode agent executor maintainability) · **Effort:** Low–Medium · **Paths:** `ArchLucid.Host.Composition/Startup/Modules/Agents/AgentAzureOpenAiExecutorRegistrar.cs`, `ArchLucid.Host.Composition/Startup/Modules/Agents/AgentLlmSupportCompositionModule.cs`, `ArchLucid.Host.Composition/Startup/Modules/Agents/AgentExecutionCompositionModule.cs`

10. **Split `GraphCoverageAnalyzer` section analyzers and `FindingFactory` finding kinds** — Pass-11 extracted ComparisonService section partials and faithfulness token/corpus helpers; leftovers: `GraphCoverageAnalyzer` **286** mixes topology, security, policy, requirement, and baseline-completeness analyzers; `FindingFactory` **274** mixes requirement/topology/security gap and policy-applicability constructors. Extract `IGraphCoverageSlice` (or `GraphCoverageAnalyzer.*` partials per domain); split factory into `RequirementFindingFactory` / `TopologyFindingFactory` / `PolicyApplicabilityFindingFactory`. Keep coverage results and finding payloads identical. **Impact:** Medium (graph coverage / finding-factory correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Decisioning/Analysis/GraphCoverageAnalyzer.cs`, `ArchLucid.Decisioning/Findings/Factories/FindingFactory.cs`
