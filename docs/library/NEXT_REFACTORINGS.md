...
**Note:** The 2026-09-01 items in the completed table shipped in PR #1098. The **2026-09-02** batch (items 1–30) shipped on **master** (2026-09-02). The **2026-09-02 pass-3** batch (items 1–10) shipped in PR **#1173**. The **2026-09-02 pass-4** batch (items 1–10) shipped in PR **#1178**. The **2026-09-02 pass-5** batch (items 1–10) shipped in PR **#1182**.
...
**2026-09-02 pass 6 — suggestions only; do not treat this list as in-progress implementation.** Evidence from current line counts after PR **#1182** (pass-5 items 1–10). Same constraints as prior passes: keep HTTP routes, OpenAPI wire shapes, and tenant isolation unchanged unless a follow-up explicitly says otherwise.

1. **Split leftover `GovernanceStickinessController` server route families** — Pass-5 split the **UI API** (`governance-stickiness-api-*.ts`); the server controller is unchanged at **839** lines (`Registers` **215**, `Dispositions` **235**, `ExceptionsAndSchedules` **317**, shell **72**). Extract one controller partial per register family (`risk` / `decision` / `waivers` / `exceptions` / schedules) delegating to `GovernanceStickinessFacade` + `GovernanceStickinessHttpMapper`; routes and auth attributes stay equivalent. **Impact:** High (buyer governance APIs, IDOR/rate-limit surface) · **Effort:** Medium · **Paths:** `ArchLucid.Api/Controllers/Governance/GovernanceStickinessController.cs`, `GovernanceStickinessController.Registers.cs`, `GovernanceStickinessController.Dispositions.cs`, `GovernanceStickinessController.ExceptionsAndSchedules.cs`

2. **Sub-split `ArchLucidGrowthFunnelMeters` and `ArchLucidLlmMeters` by instrument domain** — Pass-1 extracted meter domains from `ArchLucidInstrumentation`, but the domain files are still gods: growth funnel **643**, LLM **503**. Split into phase-specific meter modules (`ArchLucidTrialSignupMeters`, `ArchLucidEmailOtpMeters`, `ArchLucidPilotRailMeters`, `ArchLucidLlmTokenMeters`, `ArchLucidLlmBatchMeters`, `ArchLucidLlmCircuitBreakerMeters`); `ArchLucidInstrumentation.*.cs` partials forward unchanged metric names. **Impact:** Medium (telemetry cardinality / observability maintainability) · **Effort:** Medium · **Paths:** `ArchLucid.Core/Diagnostics/ArchLucidGrowthFunnelMeters.cs`, `ArchLucid.Core/Diagnostics/ArchLucidLlmMeters.cs`, `ArchLucid.Core/Diagnostics/ArchLucidInstrumentation.GrowthFunnel.cs`, `ArchLucid.Core/Diagnostics/ArchLucidInstrumentation.Llm.cs`

3. **Decompose `AuthenticationIdentityLinkProposalService` into proposal stages** — Identity persistence cores shipped in pass-5; proposal orchestration is still a single service (**370**): create external / create pending / confirm / cancel / email-collision guard / migration-review side effects. Mirror pass-4 identity-link split: `IAuthenticationIdentityLinkProposalCreateStage`, `ConfirmStage`, `CancelStage`, `ExternalKeyEligibilityChecker`; service becomes a thin sequencer. **Impact:** High (auth linking correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Application/Identity/AuthenticationIdentityLinkProposalService.cs`, `ArchLucid.Application/Identity/AuthenticationIdentityLinkingService.cs`

4. **Extract `TenantItsmConnectorConnectionRepositoryCore`** — ITSM connector SQL (**325**) and InMemory twins share no core; SQL owns OAuth/KV secret field mapping, list filters, and upsert merge rules that can drift. Share record mapping, auth-mode normalization, and enabled-state transitions in `TenantItsmConnectorConnectionRepositoryCore`; adapters stay Dapper/in-memory only. **Impact:** High (integration connector correctness) · **Effort:** Medium · **Paths:** `ArchLucid.Persistence/Integrations/SqlTenantItsmConnectorConnectionRepository.cs`, `ArchLucid.Persistence/Integrations/InMemoryTenantItsmConnectorConnectionRepository.cs`

5. **Finish `ArchitectureRunExecuteOrchestrator` sequencer thinning** — Execute stage handlers shipped across pass-1/3/4; orchestrator still **376** lines coordinating scope resolution, telemetry, and tail hooks inline. Extract `ArchitectureRunExecuteScopeResolveStage`, `ArchitectureRunExecuteTelemetryStage`, and `ArchitectureRunExecuteTailHooksStage`; orchestrator delegates only. **Impact:** High (run execute hot path) · **Effort:** Medium · **Paths:** `ArchLucid.Application/Runs/Orchestration/ArchitectureRunExecuteOrchestrator.cs`, `ArchLucid.Application/Runs/Orchestration/Execute/`

6. **Finish CLI registry + pilot-readiness slot extraction** — Pass-5 thinned doctor/scoreboard/proof helpers; leftovers: `CommandRegistry` **428** (`CreateDefault()` descriptor table), `PilotReadinessBundleRunner` **364** with **6** inline `FuncPilotReadinessSlotRunner` delegates. Split registry by command-family partials; promote remaining slots to named runners (`ReturnTriggerTelemetrySlotRunner`, `DecisionOwnerScoreboardSlotRunner`, `FrontierAiBaselineSlotRunner`, `TenantIsolationSlotRunner`, `ItsmPullForwardSlotRunner`, `ShipGateEvidenceSlotRunner`). **Impact:** Medium (pilot proof / doctor / GTM packets) · **Effort:** Medium · **Paths:** `ArchLucid.Cli/CommandRegistry.cs`, `ArchLucid.Cli/Commands/PilotReadinessBundleRunner.cs`, `ArchLucid.Cli/Commands/PilotReadiness/`

7. **Continue `GovernanceFindingsQueueClient` split (table + saved views)** — Pass-5 extracted mode/synopsis hooks; `GovernanceFindingsQueueClient.tsx` is still **393** lines owning saved-view apply, triage target resolution, ITSM prefetch, and table chrome. Extract `GovernanceFindingsQueueTableShell`, `useGovernanceFindingsQueueSavedViews`, and `useGovernanceFindingsQueueTriageTargets`; page becomes composition + routing only. **Impact:** High (governance findings queue UX) · **Effort:** Medium · **Paths:** `archlucid-ui/src/app/(operator)/governance/findings/GovernanceFindingsQueueClient.tsx`, `archlucid-ui/src/app/(operator)/governance/findings/use-governance-findings-queue-*.ts`

8. **Split contextual-help and settings master catalogs** — Operator help topic rows remain monolithic: `help-topic-rows.ts` **494** (sibling `page-help-topic-rows-*.ts` splits exist but contextual-help registry does not). Settings hub catalog `settings-master-catalog.ts` **465** mixes security, integrations, and workspace bands. Extract domain row modules + thin registries (`help-topic-rows-governance.ts`, `help-topic-rows-integrations.ts`, `settings-master-catalog-security.ts`, `settings-master-catalog-integrations.ts`). **Impact:** Medium (help/settings IA maintainability) · **Effort:** Low–Medium · **Paths:** `archlucid-ui/src/lib/contextual-help/help-topic-rows.ts`, `archlucid-ui/src/app/(operator)/administration/_sections/settings-master-catalog.ts`

9. **Alias remaining hand-authored UI types to OpenAPI (wave 7)** — Wave 6 shipped **~193** keys. Still fully or partially hand-authored: `agent-forensics.ts` (**137**), `explanation.ts` (`RunExplanation`, `FindingExplainability*`), `governance-dashboard.ts` (lineage/batch-review interfaces), `draft-intake.ts` (`DraftRequestDocument`, summary/page envelopes), `authority.ts` (`RunTrustEvidence*`, `RunDetailAgentFinding`). Keep `pagination.ts` and `pilot-scorecard.ts` intentional; `auth.ts` role union may stay local. Extend `openapi-type-aliases.test.ts`. **Impact:** Medium (contract drift) · **Effort:** Medium · **Paths:** `archlucid-ui/src/types/agent-forensics.ts`, `explanation.ts`, `governance-dashboard.ts`, `draft-intake.ts`, `authority.ts`, `archlucid-ui/src/types/openapi-type-aliases.test.ts`

10. **Extend persistence cores for draft + usage-event twins** — Pass-2 shipped `DraftRequestRepositoryCore` for Dapper; `InMemoryDraftRequestRepository` still **357** with duplicate filter/sort/reaper logic. Metering twins `DapperUsageEventRepository` **283** / `InMemoryUsageEventRepository` have no shared core. Extend `DraftRequestRepositoryCore` for in-memory query predicates; add `UsageEventRepositoryCore` for aggregation keys and cap rules. **Impact:** Medium (persistence parity / test drift) · **Effort:** Medium · **Paths:** `ArchLucid.Persistence/Data/Repositories/InMemoryDraftRequestRepository.cs`, `DraftRequestRepositoryCore.cs`, `ArchLucid.Persistence/Tenancy/DapperUsageEventRepository.cs`, `InMemoryUsageEventRepository.cs`
