...
| Buyer OpenAPI snapshot | **`ArchLucid.Api.Tests/Contracts/buyer-contract.openapi.snapshot.json`** | **Filtered procurement/UI boundary** — operations tagged `x-archlucid-audience: buyer` only; **must not** include `/v1/internal/*`. Guarded by `OpenApiBuyerContractSnapshotTests`. Regenerate: `ARCHLUCID_UPDATE_BUYER_OPENAPI_SNAPSHOT=1 dotnet test --filter OpenApiBuyerContractSnapshotTests`. Audience tiers on live `/openapi/v1.json`: `buyer`, `operator`, `internal`, `forensics` (TB-286). |
...
**Operator narrative:** `docs/library/LIVE_E2E_HAPPY_PATH.md` (HTTP request → commit → retrieval parity); **`docs/library/CANONICAL_FIRST_RUN_PATH.md#first-architecture-review-walkthrough`** for the operator wizard surface (`FIRST_RUN_WALKTHROUGH.md` alias).
...
**Finding dispositions:** `POST /v1/governance/findings/{findingId}/dispositions` and `POST /v1/governance/findings/bulk-disposition` require `expectedCurrentDispositionRowVersionBase64` (or the bulk per-finding map) when a current pointer exists. Omission after a pointer exists is **409**, not last-write-wins. See [`FINDING_CONCURRENT_DISPOSITION_CONFLICT_CONTRACT.md`](FINDING_CONCURRENT_DISPOSITION_CONFLICT_CONTRACT.md).
...
| `GET` | `/v1/admin/auth/configuration-diagnostics` | **AdminAuthority** | **`AdminAuthConfigurationDiagnosticsResponse`**: **`authMode`**, audience/issuer configuration flags, optional OpenID discovery success, SAML SP entity id and role-claim-source flags, optional tenant **`ClaimMappingJson`** health for the current scope, and bounded **`misconfigurationHints`** (no secrets). CLI: **`archlucid auth diagnostics`**. |
