...
| `run` | Submit an architecture request. Reads `archlucid.json` and `inputs/brief.md` from current directory. Optional **`--idempotency-key <uuid>`** sends the **`Idempotency-Key`** header on **`POST /v1/architecture/request`** (200 replay returns **`X-Idempotency-Replayed: true`** — same semantics as the operator UI wizard). |
