...
| `architectures list` | ReadAuthority | List durable architecture identities in the current tenant/workspace/project scope (`ArchitectureId`, display name, child counts). Not draft ids. |
| `architectures get <architectureId>` | ReadAuthority | Show one architecture identity with child draft and review summaries. Draft ids and review run ids are separate nouns. |
