...
| Orphan golden manifests (missing `dbo.Runs`) | Operator-controlled quarantine | `DataConsistencyOrphanProbeHostedService` + `DataConsistency:Enforcement` | **`Mode=Quarantine`** inserts idempotent rows into **`dbo.DataConsistencyQuarantine`** (bounded by **`MaxRowsPerBatch`**). **`AutoQuarantine=true`** runs the same insert path when **`Mode`** is **`Warn`** or **`Alert`** (still **no-op** when **`Mode=Off`**). Emits **`archlucid_data_consistency_orphans_quarantined_total`**. Does **not** delete authority rows — operators review quarantine then use admin remediation APIs. |
