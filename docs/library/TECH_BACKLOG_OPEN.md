...
| Cluster | IDs |
| --- | --- |
| Persona UX audit + deploy (batch A) | **TB-642** – **TB-657** (Done detail archived 2026-07-06; **TB-655** remains open; **TB-656** closed 2026-07-06) |
...
| TB-658 | `archlucid stack doctor` — unified deployment readiness router over `Test-ArchLucidPrerequisites.ps1`, `config lint`, drift preflight, and post-deploy verification by profile; see `## TB-658` below | Supportability P2 — **V1**; complements **TB-654**; found during setup/deployment complexity review 2026-07-05 |
...
| TB-665 | Shared accessible `Tabs` primitive for operator shell — Carbon line-tab styling, `tablist`/`tab`/`tabpanel`, `aria-selected`/`aria-controls`, Left/Right/Home/End keyboard roving, optional `?tab=` URL sync; see `## TB-665` below | Adoption friction P1 — **V1**; prerequisite for **TB-666**–**TB-670**; found during tab-candidate UX assessment 2026-07-06 |
| TB-666 | Convert `/reviews/new` start-mode selector to proper tabs — Quick start / Guided intake / Templates and imports; remove first-run "More options" disclosure hiding peer modes; see `## TB-666` below | Adoption friction P1 — **V1**; depends on **TB-665**; found during tab-candidate UX assessment 2026-07-06 |
