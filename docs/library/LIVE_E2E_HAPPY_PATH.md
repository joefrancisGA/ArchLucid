...
| `archlucid-ui/e2e/live-api-compare-runs.spec.ts` | `live-api-compare-runs` | Two runs create → execute → commit; **`GET /v1/authority/compare/runs`**; UI **`/compare?leftRunId=…&rightRunId=…`** asserts **`#compare-structured`** + **Comparison request outcome** panel; **404** when second run id is missing. |
