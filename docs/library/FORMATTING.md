> **Scope:** Contributor-reference — C# formatting (blank lines & layout) - full detail, tables, and links in the sections below.
...
> **See also:** [`docs/CSHARP_HOUSE_STYLE.md`](CSHARP_HOUSE_STYLE.md) is the human-readable index of the consolidated Cursor C# bundles (guards/flow, modern language, construction/layout). This page covers only the **mechanical** side — what `dotnet format` and the small Roslyn fixers under `scripts/` actually do.
...
- **Single-statement control flow** (`if` / `else` / `for` / `foreach` / `while` / `lock` / `using`): **omit** `{ }` when syntax allows (`csharp_prefer_braces = false`, IDE0011). Prefer **braceless**, and preferably **same physical line** when short (see **`CSharp-Terse-Guards-And-Flow.mdc`**). Use a block when there are multiple statements or you need a local scope.
...
See `.cursor/rules/CSharp-Terse-Guards-And-Flow.mdc` for the braceless / same-line convention.
