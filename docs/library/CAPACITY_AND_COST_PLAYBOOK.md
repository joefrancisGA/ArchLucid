> **Scope:** Contributor-reference — Capacity and cost playbook - full detail, tables, and links in the sections below.
...
| **SQL** | DTU/vCore maxed, long query store | Scale tier; index/outbox retention; archive cold runs. **Signup latency:** standby **warm** migrated tenant catalogs in an elastic pool (depth **N**) trade **minimum storage idle** vs **provision p95** — see **`TENANT_DATABASE_TOPOLOGY.md`** Operational notes + **TB-018**. |
...
| **Edge WAF** | Front Door WAF **enabled** in production (`posture_tier = production`) | `infra/terraform-edge` — `enable_front_door_waf`, `posture_tier` in **`production.tfvars.example`** |
...
| **Hosted signup trials (AOAI + mode)** | Envelopes keyed off **run caps**, not calendar days alone; SKU + **`MaxCompletionTokens`**; **`AgentExecution:Mode=Real`** for buyer runs; SaaS **`appsettings`** merge order (**`FIRST_REAL_VALUE`**) | [`TRIAL_AND_SIGNUP.md`](../go-to-market/TRIAL_AND_SIGNUP.md) **sections 3.1–3.2**, [`FIRST_REAL_VALUE.md`](FIRST_REAL_VALUE.md), [`PER_TENANT_COST_MODEL.md`](PER_TENANT_COST_MODEL.md) |
