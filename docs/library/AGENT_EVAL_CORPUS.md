...
**claimBoundary:** Production gate (ADR 0070) — `DeterministicInsightDensityGate` applies the demotion predicate to typed-engine and agent findings. The legacy `typed-engine-protected` Promote short-circuit is superseded. The table covers engines that produced ≥1 finding across case-01..case-35; the harness registers **16** engines and **23** built-in product engines are absent from that slice. `WouldDemoteIfUnprotectedCount` matches production demotion when the predicate applies.
