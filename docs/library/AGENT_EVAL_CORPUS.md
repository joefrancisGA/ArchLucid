...
- **`.github/workflows/real-mode-eval-nightly.yml`** (**TB-683**) — **daily** job (05:30 UTC) when repo variable **`ARCHLUCID_GOLDEN_COHORT_REAL_LLM=true`**: scores all 18 committed **`*.real.json`** exemplars with quality-gate + Phase B faithfulness floors, writes **`artifacts/real-mode-eval-nightly/{date}/trend.json`** + **`trend.md`** (`--json-report` / `--markdown-report`), uploads a CI artifact, caches the prior night for **`assert_real_mode_eval_consecutive_regression.py`** two-night baseline-regression warnings. Skips cleanly with a documented message when the variable is unset. **Honesty:** this is **offline exemplar scoring**, not a live model tripwire — see [`LIVE_VS_NIGHTLY_FINDING_QUALITY_TRIPWIRE_MAP.md`](LIVE_VS_NIGHTLY_FINDING_QUALITY_TRIPWIRE_MAP.md) (**TB-1506** Done).
