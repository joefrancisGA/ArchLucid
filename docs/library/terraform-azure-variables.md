> **Scope:** Contributor-reference — Terraform / Azure variables (reference sketch) - full detail, tables, and links in the sections below.
...
| `archlucid_api_backend_url` | API `service_url` | HTTPS URL the Consumption gateway can reach (typically the public App Service origin for ArchLucid API). Greenfield IaC uses **`archlucid`** resource labels only ([V1_DEFERRED.md](V1_DEFERRED.md) §3). |
