...
| **Findings / quality** | `archlucid_findings_produced_total` (label `severity`), `archlucid_finding_engine_failures_total`, `archlucid_findings_engine_partial_failure_total`, `archlucid_explainability_trace_completeness_ratio`, `archlucid_explanation_faithfulness_ratio`, `archlucid_retrieval_faithfulness_ratio` |
| **LLM / RAG** | `archlucid_llm_prompt_tokens_total`, `archlucid_llm_completion_tokens_total` (add `tenant_id` label only when `LlmTelemetry:RecordPerTenantTokens=true`), `archlucid_llm_calls_per_run`, `archlucid_llm_cache_hits_total`/`_misses_total`, `archlucid_llm_quota_exceeded_total`, `archlucid_llm_call_retries_total`, `archlucid_llm_rate_limit_total`, `archlucid_rag_retrieval_duration_ms`, `archlucid_rag_chunks_retrieved_total`, `archlucid_rag_retrieval_fallback_total` |
...
| **Business / GTM** | `archlucid_tenant_estimated_savings_usd` (gauge, label `scope`), `archlucid_trial_signups_total`, `archlucid_trial_conversion_total`, `archlucid_trial_first_run_seconds`, `archlucid_tenant_time_to_first_commit_seconds`, `archlucid_pricing_quote_request_age_hours` (label `breach_status`) |
