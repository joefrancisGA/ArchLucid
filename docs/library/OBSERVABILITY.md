...
| **`archlucid_run_lifecycle_deprecated_alias_requests_total`** | Counter | — | **`operation`** (`create` \| `execute` \| `commit`). Deprecated run-lifecycle alias routes (`v1/requests`, `v1/runs/*/submit`, `v1/runs/*/manifest/finalize`) per ADR 0042 — soak input before alias sunset. |
