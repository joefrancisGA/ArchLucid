...
- **GitHub Actions** job **“.NET: fast core (corset)”** runs **`DOTNET_FAST_CORE_TEST_FILTER`** (fast core subset **without** `OpenApiContractSnapshotTests`). OpenAPI drift is still caught by job **`openapi-contract-snapshot`** (Tier **0.x**) before corset builds.
