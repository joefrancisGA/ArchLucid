...
| **`.NET: OpenAPI v1 contract snapshot (fail-fast)`** | Yes (job on this workflow; owner may add the check name after a green run) | `scripts/ci/check_openapi_contract_snapshot.sh` — `ArchLucid.Api.Tests` OpenAPI v1 + buyer snapshots. Not stuffed into the Core/Decisioning test filter. |
...
| **`CI: beta-readiness wiring guards`** | Yes (JSON lists it; live ruleset may lag) | Python guards for sponsor export, private-beta wiring, OpenAPI-on-push wiring, release-gate Playwright, insight-density honesty, policy-pack offline packet |
| **`Operator UI: typecheck (blocking)`** | Yes | `archlucid-ui`: `npm ci` + `npm run typecheck` |

**Sibling invite-wave workflow:** `.github/workflows/private-beta-access-on-push.yml` runs **`Operator UI: private-beta access-path (JwtBearer)`** on **`push`** to **`main`/`master`** (and `workflow_dispatch`) without waiting for `ci.yml` full regression. `ci.yml` `ui-e2e-live-beta-access` stays `if: github.event_name != 'pull_request'` and `ci.yml` has no `push` trigger, so that job never ran on trunk merge. **Do not** add this Playwright job as a sixth golden-cohort required check until the owner applies it after a green run.

**Tradeoff:** Direct pushes to the default branch get a **thin** corset (gitleaks + OpenAPI snapshot + Active-solution build + Core/Decisioning fast-core tests + UI typecheck + beta-readiness guards) so compile, contract drift, declaration-gating regressions, and truncated Operate help JSX cannot land silently. The full **`ci.yml`** matrix (all fast-core shards, SQL regression, Playwright, k6) still runs on **`pull_request`** and **`workflow_dispatch`** — not on every default-branch push. **`codeql.yml`** already runs on push separately. Add the new job names under branch protection **required status checks** if pushes should be blocked when they fail.
