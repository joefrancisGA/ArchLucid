...
| **3a′** | **`ui-first-load-js-budget`** | `archlucid-ui`: `npm ci`, production **`npm run build`**, **`npm run check:first-load-js`** against committed **`performance/first-load-js-baseline.v1.json`** (TB-573; **+25 kB** tolerance per tracked route; primary gate **`/reviews`**). Refresh baseline: **`npm run write:first-load-js-baseline -- --log .next-build.log`**. |
