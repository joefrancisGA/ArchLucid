...
| **Architecture** | The named, tenant-scoped **durable identity** for a system under active architecture work (ADR 0074). An **Architecture** owns child **architecture drafts** and **architecture reviews**; it is not a sealed record and not a draft document. In **Working** mode, the Architectures hub lists these identities. |
| **Architecture draft** | A saved, resumable **child** description of architecture intent and context, linked to an **Architecture** identity when ensure-on-create has run. Saving or editing a draft does not start an **architecture review**; starting a review is a separate explicit action that creates an **architecture package**. In **Guided** mode, draft inventory teaching may list drafts before identities are emphasized. |
