...
- **Cross-run executive ROI summary (V1 GA)** — tenant-scoped portfolio rollup for sponsor dashboards: **`GET /v1/roi/executive-summary`** ([`ExecutiveRoiSummaryService`](../../ArchLucid.Application/Roi/ExecutiveRoiSummaryService.cs), [`PILOT_SCORECARD_API.md`](PILOT_SCORECARD_API.md)); operator UI panel on Home ([`ExecutiveRoiSummarySection`](../../archlucid-ui/src/app/(operator)/dashboard/_sections/ExecutiveRoiSummarySection.tsx)). **Authoritative aggregation model (owner 2026-05-22, clarified 2026-06-06):** ROI is **not** one monolithic algorithm across every surface — it is a **layered model of single-source primitives plus explicitly-scoped totals**:
  ...
  - **Known consolidation target (2026-06-06):** the **cross-tenant portfolio** rollup ([`ExecutiveRoiSummaryService.GetCrossTenantPortfolioSummaryAsync`](../../ArchLucid.Application/Roi/ExecutiveRoiSummaryService.cs)) currently uses a **raw per-system sum** rather than the disposition-aware basis; align it to the basis (or label it `TotalPotentialUsd`) before quoting cross-tenant totals next to single-tenant headlines.
