...
**Owner unblocker — ServiceNow engineering tenant (2026-05-15):** **`V1` GA** includes bidirectional ServiceNow sync (**ServiceNow row below**). Engineering validation assumes the owner provisions a **cost-free** ServiceNow **Developer Program** / personal developer-style instance — **not** a paid sandbox SKU. Until that instance exists, connector validation may be **queued** — track owner / engineering status in **[PENDING_QUESTIONS.md](../PENDING_QUESTIONS.md)**; **[V1_SCOPE.md](V1_SCOPE.md)** §2.13 stays authoritative unless explicitly amended there.
