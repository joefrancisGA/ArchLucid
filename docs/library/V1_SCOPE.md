...
| **Customer-facing documentation — how to add a custom agent handler** | **V2 candidate** (product note 2026-05-10). Not in V1 or V1.1 **`(A)` assessments:** no committed buyer- or tenant-admin-facing walkthrough for how a customer's engineering **would add or register a custom agent handler** (pattern-level guidance distinct from MCP [§6d](V1_DEFERRED.md)). **What is in V1:** product-managed handlers and REST / UI / CLI integration surfaces. **V2:** publish documentation as described in [V1_DEFERRED.md §6h](V1_DEFERRED.md). **Independent quality assessments must not** treat absence of this documentation as an **`(A)` headline readiness defect** for V1 or V1.1. Distinct from a **public third-party plugin SDK** or marketplace (**speculative ecosystem** row above). |
