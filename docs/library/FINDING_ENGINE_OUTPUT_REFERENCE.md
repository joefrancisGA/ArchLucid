...
| `topology-security-drift` | `TopologySecurityDriftFindingEngine` | Security | Security-semantic prior-vs-current graph deltas (public inbound, replica removal, admin inbound widening, write/admin role paths). Requires prior graph snapshot; records `PriorRunSnapshot` held-check when absent. |
...
| `checklist-cluster-synthesis` | `ChecklistClusterSynthesisFindingEngine` | Insight | Post-gate synthesis of demoted checklist rows sharing a root cause (DX-22). `AnalyzeAsync` is a catalog placeholder. |
| `decision-grade-fusion` | `DecisionGradeFusionFindingEngine` | Insight | Post-gate join of **Decision-grade** findings from distinct preferred engines that share a `RelatedNodeIds` node (DX-51). After DX-71, constituents remain on the snapshot as **checklist**; the fusion row stays Decision-grade. Fusion copies constituent `EvidenceRefs` and does not add synthetic `graph-node:` citations. Not a named-model beat; not live inventory; not SOC 2 Type II. `AnalyzeAsync` is a catalog placeholder. |
...
| `diagram-declaration-omission` | `DiagramDeclarationOmissionFindingEngine` | When a structured diagram asserts completeness (named trust boundary + explicit structured-parse connectors), flags cited declaration resources omitted from the diagram connector set. Not a missing-node-type coverage nag (AS-042). |
...
| `portfolio-shared-topology` | `PortfolioSharedTopologyFindingEngine` | Cross-system contradiction when two committed architectures in the same tenant catalog share a product-shaped inventory resource id (ARM/ARN/GCP) but declare incompatible security or DR posture. Tenant-catalog scoped (ADR 0037). **Default off** — enable per tenant for Real pilot. Not live cloud polling. |
