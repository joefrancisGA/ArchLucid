...
| `decision-grade-fusion` | `DecisionGradeFusionFindingEngine` | Insight | Post-gate join of **Decision-grade** findings from distinct preferred engines that share a `RelatedNodeIds` node (DX-51). Constituents stay on the package. Fusion copies constituent `EvidenceRefs` and does not add synthetic `graph-node:` citations. Not a named-model beat; not live inventory; not SOC 2 Type II. `AnalyzeAsync` is a catalog placeholder. |
