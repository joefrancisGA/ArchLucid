...
## Real-mode end-to-end benchmark {#real-mode-e2e-benchmark}

Former standalone body: `docs/library/REAL_MODE_BENCHMARK.md` → this section (filename kept as a path-stable alias). Contributor-reference — how to run, interpret, and compare wall-clock time from architecture request submission to a committed manifest in real mode. Complements sponsor assembly in [`PILOT_SUCCESS_SCORECARD.md#proof-of-value-snapshot-assembly`](../go-to-market/PILOT_SUCCESS_SCORECARD.md#proof-of-value-snapshot-assembly).

**Path-stable alias:** [`REAL_MODE_BENCHMARK.md`](REAL_MODE_BENCHMARK.md).
...
2. For **real-mode** results, the API stack must be started with `AgentExecution__Mode=Real` and Azure OpenAI credentials configured per this document (sections above).
3. Optionally set **`ARCHLUCID_REAL_AOAI=1`** in the shell — the value is recorded in the benchmark JSON artifact (metadata only); **real-mode execution depends on API host configuration** (`AgentExecution__Mode=Real` plus Azure OpenAI).
...
### K6 summary export JSON {#k6-summary-export-json}
