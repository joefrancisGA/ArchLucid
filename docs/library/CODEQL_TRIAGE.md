...
| **`ArchLucid.Application/Tenancy/TrialTenantBootstrapService.cs`** | Email verification policy block logs **`LogSanitizer.EmailDomainForLogs(auditActorEmail)`** (domain only, never mailbox local-part). **`barrierModel`** on **`EmailDomainForLogs`** should clear **`cs/exposure-of-sensitive-information`**. |
