...
| SCIM provisioning | — | — | ✓ | **SCIM token administration:** `[RequiresCommercialTenantTier(TenantTier.Enterprise)]`; SCIM bearer routes remain protected by `ScimWrite` after token issuance | `ScimTokensAdminController` is Enterprise-gated; `ScimUsersController` / `ScimGroupsController` require the tenant-bound `ScimBearer` token | **Aligned for token issuance:** non-Enterprise tenant admins cannot mint/manage SCIM bearer credentials. |
...
3. **SCIM:** **Resolved for token administration** — `ScimTokensAdminController` is Enterprise-gated. Continue to evaluate whether SCIM bearer controllers need an additional runtime tier check for already-issued legacy tokens.
