> **Scope:** Contributor-reference — API controller area map - full detail, tables, and links in the sections below.
...
| **`(Controllers root)`** | `ArchLucid.Api.Controllers` | `VersionController` (`GET /version`, `[AllowAnonymous]`, build identity probe) |
| **`Admin/`** | `ArchLucid.Api.Controllers.Admin` | `AdminController`, `AdminAuthDiagnosticsController`, `DiagnosticsController`, `JobsController`, `DemoController`, `ScopeDebugController`, `AuthDebugController`, `DocsController`, `AuditController` — plus `AdminArchiveRunsByIdsRequest`, `AdminArchiveRunsBatchRequest` |
