...
| **`Admin/`** | `ArchLucid.Api.Controllers.Admin` | `AdminController`, `DiagnosticsController`, `JobsController`, `DemoController`, `ScopeDebugController`, `AuthDebugController`, `DocsController`, `AuditController` — plus `AdminArchiveRunsByIdsRequest`, `AdminArchiveRunsBatchRequest` |
