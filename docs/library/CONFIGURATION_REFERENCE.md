...
| ArchLucid | `ArchLucid:Persistence:ReadOnlyConnectionStringTemplate` | appsettings, env | empty | Optional (not mode-gated) | All (Api, Worker, Combined) | Read-scale-out template for analytical queries (`Application Intent=ReadOnly`). Empty = primary fallback. See [READ_REPLICA_ROUTING.md](READ_REPLICA_ROUTING.md). |
...
| Retrieval | `Retrieval:AzureSearch:ApiKey` | appsettings, env, KeyVault | empty | **Required when not using Entra/MI auth** | All | API key for Search; prefer managed identity where TB-080 patterns apply. |
| Retrieval | `Retrieval:Reranking:Provider` | appsettings, env | `AzureAiSearchSemantic` | Optional | All | Reranker provider when `Retrieval:Reranking:Enabled=true`. **Does not** replace the production-like requirement for `VectorIndex=AzureSearch` (owner 2026-05-29). |
