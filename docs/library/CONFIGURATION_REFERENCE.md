...
| ArchLucid | `ArchLucid:AgentOutput:QualityGate:PilotStrictMinAgentResultFaithfulnessSupportRatio` | appsettings, env | (unset) | Optional (not mode-gated) | All (Api, Worker, Combined) | Optional PilotStrict minimum deterministic AgentResult-to-evidence grounding ratio when an evidence package exists. |
