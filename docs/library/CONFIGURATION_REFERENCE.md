...
- Server snapshot: `GET /v1/admin/config-summary` or `GET /v1/admin/configuration/summary` (admin API key; same key paths as the catalog; sensitive values return `***`, never connection strings or API key material).
