...
**Declaration path edges — AWS/GCP IAM and segmentation (DX-74–DX-76):** The same mapper promotes AWS **`role`** / **`policy_arn`** and GCP **`member`** / **`role`** (skipping redaction tokens; GCP secrets use **`scope`** as the join key when **`secret_id`** is redacted). **`DeclarationIdentityActorMaterializer`** includes **`google_service_account`**. **`DeclarationIdentityPathEdgeMaterializer`** consumes those keys for **`identity-blast-radius`** on AWS/GCP Terraform (**case-66** / **case-67**). Data-flow backends promoted in DX-69 are exercised parse-through on **case-68** (**case-60** overlay retained for regression). **`DeclarationSegmentationPathEdgeMaterializer`** (stage **`declaration-segmentation-path-edges`**) promotes NSG/SG/firewall rule blobs and association keys, then emits segmentation path hops consumed by **`segmentation-semantics`** (**case-69**; **case-39** / **case-44** overlays retained). **claimBoundary:** not live NSG evaluation in Azure, not a named-model beat, not SOC 2 Type II attestation.
