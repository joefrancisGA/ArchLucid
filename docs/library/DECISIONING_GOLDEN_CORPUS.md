...
Each case is a directory under `tests/golden-corpus/decisioning/` named `case-NN` (two-digit index). **73** directories exist today: **`case-01` … `case-30`** are produced by `GoldenCorpusGraphFactory` / the materializer; **`case-31`** through **`case-73`** are **hand-authored** scenarios (see each folder’s `README.md`).
...
| **`case-72`** (hand-authored) | **AS-042** — completeness-asserting structured diagram (named trust boundary + explicit structured-parse connectors) cites a Key Vault declaration omitted from the diagram connector set — expect **`diagram-declaration-omission`**. See `tests/golden-corpus/decisioning/case-72/README.md`. |
| **`case-73`** (hand-authored) | Assigned **CIS Azure** pack plus declaration/inventory public-network contradiction. Exercises **`policy-declaration-inventory-contradiction`** in the merge harness. See `tests/golden-corpus/decisioning/case-73/README.md`. |
