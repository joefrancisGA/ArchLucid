...
Each case is a directory under `tests/golden-corpus/decisioning/` named `case-NN` (two-digit index). **72** directories exist today: **`case-01` … `case-30`** are produced by `GoldenCorpusGraphFactory` / the materializer; **`case-31`** through **`case-72`** are **hand-authored** scenarios (see each folder’s `README.md`).
