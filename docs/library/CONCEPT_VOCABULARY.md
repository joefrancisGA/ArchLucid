...
| 3 | **Signed review record** (package locked at finalize) | **Signed decision record**, **governance decision record**, or calling the **architecture package** a decision record | The **signed review record** is the finalized package artifact (findings, evidence trail, decisions, exports). A **decision** is a disposition row in the Decision register — not the package itself. **ADR** remains **Architecture Decision Record** for ADR export wording. |
