...
| 4 | **Architecture** (named durable identity) | "Architectures list = drafts"; treating **`DraftId`** as the architecture id in Working copy; describing the Architectures hub as **only** a draft editor | ADR 0074: **`dbo.Architectures`** is the customer-visible anchor. **Architecture drafts** and **architecture reviews** are children. **Working** lists identities on the Architectures hub; **Guided** may still teach with draft-first inventory — label Guided explicitly when draft-only copy is intentional. |
...
| **Architecture** | Durable customer identity (`dbo.Architectures`, `GET /v1/architectures`); Working Architectures hub lists identities — not draft ids |
