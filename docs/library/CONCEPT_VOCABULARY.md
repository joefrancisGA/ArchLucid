...
|---|----------------------|----------------------|-----------|
| 1 | **review** (noun — the product session) | "run" when used as the product noun for a governed architecture review session (e.g. "create a run", "the run's findings", "run ID") | ArchLucid's primary product concept is an architecture *review*, not a pipeline *run*. Buyer-facing copy, docs, and UI labels must use **review**. Legitimate uses of "run" (CI run, test run, script run, dry run, **run** the pipeline as a verb) are **not** rejected — only uses where "run" names the ArchLucid review session itself. **2026-05-18:** Buyer/evaluator spine (`docs/` onboarding, `go-to-market/`, `onboarding/` eval guide) refreshed. **Promote to § 1.1 CI:** before **V1 GA** ship candidate, in the same PR that finishes **`docs/library/**` + runbook cleanup and adds the matching `RULES` entry in `check_concept_vocabulary.py`. |
