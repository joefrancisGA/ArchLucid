...
<<<<<<< HEAD
8. **Diagram enrichment collection (AX-DE):** **In progress** — AX-DE-01–09 shipped on `cursor/ax-de-implementation-80a4` (association catalog, diagnostic `diagnosticToDestination` edges, MI+RBAC `appAuthorizedAccess`, wider ADF hostname index, remaining ADF connectors, triggers, integration runtimes, data flows + dataset location, Synapse factory-style pipelines). Remaining: Event Grid, Logic Apps, Event Hub/Service Bus children, SQL/storage/AKS children, NAT/Firewall/Front Door/Container Apps. Hosted stays GET-only. `config/list` is optional Tier 1 last. Catalog and prompts: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md`](../architecture/AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md). Hold: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md`](AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md).
=======
8. **Diagram enrichment collection (AX-DE):** **In progress** — AX-DE-01–13 shipped on `cursor/ax-de-implementation-80a4` (association catalog, diagnostic destinations + expanded path-relevant diagnostic GET fan-out, app authorized access, wider ADF hostname index, remaining ADF connectors, triggers, integration runtimes, data flows + dataset location, Synapse factory-style pipelines, Event Grid subscriptions, Logic App connections, Event Hub/Service Bus children + capture). Remaining: SQL/storage/AKS children, NAT/Firewall/Front Door/Container Apps. Hosted stays GET-only. `config/list` is optional Tier 1 last. Catalog and prompts: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md`](../architecture/AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md). Hold: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md`](AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md).
>>>>>>> 50105d4113 (docs(azure-extractor): mark AX-DE-10–13 shipped in backlog)
