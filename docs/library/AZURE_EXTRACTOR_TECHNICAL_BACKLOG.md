...
8. **Diagram enrichment collection (AX-DE):** **In progress** — AX-DE-01–04 shipped on `cursor/ax-de-implementation-80a4` (association catalog, diagnostic `diagnosticToDestination` edges, MI+RBAC `appAuthorizedAccess`, wider ADF hostname index). Remaining: type-scoped companions (remaining ADF connectors, triggers, IRs, data flows, Synapse pipelines, Event Grid, Logic Apps, Event Hub/Service Bus children, SQL/storage/AKS children, NAT/Firewall/Front Door/Container Apps). Hosted stays GET-only. `config/list` is optional Tier 1 last. Catalog and prompts: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md`](../architecture/AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md). Hold: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md`](AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md).
