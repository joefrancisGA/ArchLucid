...
6. **Continuous Tier 2 (ArchLucid-hosted auto-pull):** Wire ARM/cost collection in `AzureExtractorAutoPullHostedService` using the approved **cross-tenant WIF** pattern (`WorkloadIdentityHostedAzureExtractorCredentialFactory` → `HostedAzureExtractorClient` → existing ingest). Feature-flagged via `AzureExtractor:AutoPull:Enabled`. Architecture decision: [V1_DEFERRED.md §6p](V1_DEFERRED.md); operator/onboarding context: [AZURE_EXTRACTOR.md](AZURE_EXTRACTOR.md).
