...
8. **Diagram enrichment collection (AX-DE):** **In progress** — AX-DE-01–15 shipped on `cursor/ax-de-implementation-80a4` (association catalog, diagnostic `diagnosticToDestination` edges + expanded path-relevant diagnostic GET fan-out, MI+RBAC `appAuthorizedAccess`, wider ADF hostname index, remaining ADF connectors, triggers, integration runtimes, data flows + dataset location, Synapse factory-style pipelines, Event Grid subscriptions, Logic App connections, Event Hub/Service Bus children + capture, SQL/Cosmos/storage children + AKS property enrichment, NAT/Firewall/Front Door/Container Apps/PE DNS network leftovers). Remaining: Databricks (AX-DE-16), Service Connector (AX-DE-17), Tier 1 app-setting hosts (AX-DE-18). Hosted stays GET-only. `config/list` is optional Tier 1 last. Catalog and prompts: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md`](../architecture/AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_COMPOSER_PROMPTS.md). Hold: [`AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md`](AZURE_EXTRACTOR_DIAGRAM_ENRICHMENT_HOLD.md).
