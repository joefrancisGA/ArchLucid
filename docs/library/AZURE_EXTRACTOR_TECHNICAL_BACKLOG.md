...
7. **Relationship-first topology (IE-RF):** ARG typed relationship projections + hosted type-scoped ARM **list** GETs into `network-associations.json` (VM→NIC, all IP configs, NSG/UDR/peering, AGW/LB/Private DNS/App Service subnet). Catalog and prompts: [`INFRA_EVIDENCE_RELATIONSHIP_FIRST_COMPOSER_PROMPTS.md`](../architecture/INFRA_EVIDENCE_RELATIONSHIP_FIRST_COMPOSER_PROMPTS.md). **Do not** add `Export-AzResourceGroup` or treat ARM `dependsOn` as architecture. Hold: [`INFRA_EVIDENCE_RELATIONSHIP_FIRST_HOLD.md`](INFRA_EVIDENCE_RELATIONSHIP_FIRST_HOLD.md).
