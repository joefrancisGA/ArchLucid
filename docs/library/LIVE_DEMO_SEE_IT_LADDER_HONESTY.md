...
**`/live-demo` title honesty is shipped** (**TB-1265** Done — H1/metadata/Resources use “Guided sample walkthrough”), and ladder UI between `/see-it` and `/live-demo` is shipped (**TB-1267** / **TB-1282** Done). The page remains **conditionally hurting** when mode chip is absent, offline fallback is narrated as a live API session, or Contoso-shaped payload sits under Claims chrome. Keep it as **rung 2** of a labeled see-it ladder with universe/mode pins and sample-illustrative governance copy only.

---

## PA verdict (helping vs hurting — one paragraph)

`/live-demo` **helps** when it teaches the product spine on fabricated data with disclosure, `robots: noindex`, honest title (**TB-1265** Done — “Guided sample walkthrough”), ladder cross-links (**TB-1267** / **TB-1282** Done), and a visible mode chip (`Offline curated sample` vs `Anonymous public sample API`). It **hurts** when offline fallback is narrated as a live API session; when Contoso-shaped payloads sit under Claims chrome; or when “Review integrity” sells sponsor/governance readiness on sample data. **Safe posture:** rung 1 (`/see-it`) for universe-honest quick proof → rung 2 (`/live-demo`) for guided fabricated sample walkthrough → rung 3 (eval/enterprise CTA). Treat “ladder done” while **TB-1029** or hero/mode-chip rows remain open as a **review finding**.
...
- Do **not** claim the see-it ladder is **done** while **TB-1029** remains open, mode chip is absent, or hero-budget rows (**TB-1266** / **TB-1281**) remain open.
...
| **TB-1267** | `/live-demo` journey vs `/see-it` ladder UI | **Done** | Pairs **TB-1282** |
...
| **TB-1282** | `/see-it` single CTA path + ladder + jargon purge | **Done** | Pairs **TB-1267** |
| **TB-1029** | Anti-Claims-banner-Contoso-payload honesty CI | **Open** | After **TB-1028**; pairs **M-178** |
| **TB-1428** | Anti-live-demo-as-live / ladder-closed honesty CI | **Done** | `check_live_demo_see_it_ladder_honesty.py`; GTM **M-259** |

**Recommended next engineering passes among open rows:** **TB-1029** (Contoso-under-Claims CI), then **TB-1266** / **TB-1281** (hero budgets) and mode-chip honesty.

---

## PA review script

1. Ask which rung the buyer will open and confirm ladder UI labels the `/see-it` ↔ `/live-demo` relationship (**TB-1267** / **TB-1282** Done).
...
5. Treat “ladder done” while **TB-1029** or mode-chip / hero-budget rows remain open as a **review finding**.
...
- Honesty CI: **TB-1428** Done — `scripts/ci/check_live_demo_see_it_ladder_honesty.py` (wired in `run_buyer_surface_strict_guards.py`)
