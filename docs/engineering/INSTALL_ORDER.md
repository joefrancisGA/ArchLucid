...
| **6** | **What:** **SQL connection string** for the API. **Why:** DbUp migrations and runtime ([`README.md`](../REPOSITORY_README.md) dev string). **How:** `dotnet user-secrets set "ConnectionStrings:ArchLucid" "..." --project ArchLucid.Api` (see [`OPERATOR_QUICKSTART.md`](../library/customer-facing/OPERATOR_QUICKSTART.md)). **Verify:** secret set (no echo) | **What:** **Entra + app settings** for deployed hosts. **Why:** **`JwtBearer`** / **`ApiKey`** in Azure per [`deployment/PILOT_PROFILE.md`](../deployment/PILOT_PROFILE.md). **How:** [`terraform-entra`](../../infra/terraform-entra/README.md) + Key Vault references. **Verify:** API `/health/ready` **200** on deployed URL |
...
| **8** | **What:** **Node.js 22.x** (CI uses `node-version: "22"` in `.github/workflows/ci.yml`). **Why:** Architect workspace (`archlucid-ui`) **npm ci** / **Vitest**. **How:** [Node.js LTS matching CI](https://nodejs.org/) (use **22**). **Verify:** `node --version` → **v22.x** | **What:** *(optional)* **JWT / E2E lab** parity. **Why:** Live Playwright + **`JwtBearer`** local PEM path. **How:** [`LIVE_E2E_JWT_SETUP.md`](../library/LIVE_E2E_JWT_SETUP.md). **Verify:** job **`ui-e2e-live-jwt`** pattern in **`.github/workflows/ci.yml`** |
