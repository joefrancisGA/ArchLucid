...
- **Shell / process hygiene (always-loaded):** `.cursor/rules/Agent-Shell-Discipline.mdc` — at most one `Shell` per turn, no parallel terminal batches, prefer Read/Grep over shell, chain git in one invocation, call `.\scripts\ci\agent-compile-check.ps1` directly (no nested `powershell -File`).
