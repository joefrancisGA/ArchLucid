...
**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **75%** merged line minimum (aligned to CI Cobertura ~**75.1%** on run [30642343755](https://github.com/joefrancisGA/ArchLucid/actions/runs/30642343755); interim toward V1.1 **95%**); **81%** merged branch and **83%** per-product-package line (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, plus RC interim skips for Cli/Host.Core/Persistence/Application/AgentRuntime/Host.Composition — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **83%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**77.00** anchor → **75%** with **2%** slack) are **enabled** in CI; job is warn-only (`continue-on-error`) (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
