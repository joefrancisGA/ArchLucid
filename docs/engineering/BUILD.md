...
**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **78%** merged line minimum (above CI **#2938** Cobertura ~**74.94%**; interim toward V1.1 **95%**); **61%** merged branch (above ~**59.67%** on that run) and **84%** per-product-package line (above the ~**81.57%** lowest gated package) (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, plus RC interim skips for Cli/Host.Core/Persistence/Application/AgentRuntime/Host.Composition — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **84%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**80.00** anchor → **78%** with **2%** slack) are **enabled** in CI; job is warn-only (`continue-on-error`) (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
