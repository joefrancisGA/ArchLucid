...
**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **74%** merged line minimum (aligned to CI Cobertura ~**74.99%** on run [30882961646](https://github.com/joefrancisGA/ArchLucid/actions/runs/30882961646); interim toward V1.1 **95%**); **59%** merged branch and **81%** per-product-package line (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, plus RC interim skips for Cli/Host.Core/Persistence/Application/AgentRuntime/Host.Composition — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **81%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**76.00** anchor → **74%** with **2%** slack) are **enabled** in CI; job is warn-only (`continue-on-error`) (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
