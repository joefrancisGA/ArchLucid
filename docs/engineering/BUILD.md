...
**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **78%** merged line minimum (interim toward V1.1 **95%**); **63%** merged branch and **80%** per-product-package line remain merge-blocking (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`** skipped at package gate — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **80%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**80.00** anchor → **78%** with **2%** slack) are **enabled** in CI (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
