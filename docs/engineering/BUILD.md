...
**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **77%** merged line minimum (RC28; interim toward V1.1 **95%**); **62%** merged branch and **88%** per-product-package line (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, plus RC interim skips for Cli/Host.Core/Persistence/Application/AgentRuntime/Host.Composition — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **88%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**78.00** anchor → effective slack vs hard floor) are **enabled** in CI; job is warn-only (`continue-on-error`) (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
