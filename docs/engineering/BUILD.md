...
**CI / supply chain:** GitHub Actions workflow **`.github/workflows/ci.yml`** runs **`scripts/ci/assert_nuget_no_high_critical_vulnerabilities.py`**, which invokes **`dotnet list ArchLucid.sln package --vulnerable --include-transitive --format json`** and **fails only on High or Critical** severities (Low, Moderate, and unknown levels are reported for visibility but do not block the job). Run the same script locally from the repo root after dependency changes. The workflow uses **tiered jobs** (fast .NET core, then full .NET regression with SQL, plus **Vitest** and **Playwright** for `archlucid-ui`); see **[TEST_EXECUTION_MODEL.md](../library/TEST_EXECUTION_MODEL.md)**.

**Merged .NET coverage gates:** After **`dotnet-full-regression`**, the workflow runs **`scripts/ci/assert_merged_line_coverage_min.py`** on merged Cobertura with **77%** merged line minimum (RC28; interim toward V1.1 **95%**); **62%** merged branch and **89%** per-product-package line (**`ArchLucid.Api`**, **`ArchLucid.Mcp`**, **`ArchLucid.Backfill.Cli`**, plus RC interim skips for Cli/Host.Core/Persistence/Application/AgentRuntime/Host.Composition — see **`docs/library/coverage-exclusions.md`**). **`--warn-below-package-line-pct 70`** is below the package floor today, so the advisory band is inactive until raised above **89%**. **`assert_coverage_floor_ratchet.py`** and **`.coverage-floor`** (**78.00** anchor → effective slack vs hard floor) are **enabled** in CI; job is warn-only (`continue-on-error`) (see **`.github/workflows/ci.yml`** `dotnet-coverage-merge` job).
...
|------|------|--------------|------------------------|
| **Tier 2d — `dotnet-azure-openai-live-post-regression`** | `RealAzureOpenAIEndToEndTests` | After OpenAPI snapshot, merged coverage, and chaos; **`push` / `workflow_dispatch` / `merge_group` only** (not `pull_request`). Shares **`scripts/golden_cohort_budget_probe.py`** ($15/mo cap): runs live tests on probe exit **0/1**; **`::warning::` + skip** on exit **2/3** (workflow stays green). | Repository variable **`ARCHLUCID_CI_REAL_AOAI_ENABLED=true`**; secrets **`ARCHLUCID_CI_REAL_AOAI_ENDPOINT`**, **`ARCHLUCID_CI_REAL_AOAI_KEY`**; optional var **`ARCHLUCID_CI_REAL_AOAI_DEPLOYMENT`**. Budget probe: GitHub **`dev`** environment secrets **`AZURE_CLIENT_ID`**, **`AZURE_TENANT_ID`**, **`AZURE_SUBSCRIPTION_ID`**, **`ARCHLUCID_GOLDEN_COHORT_AZURE_OPENAI_RESOURCE_ID`** (see [`GOLDEN_COHORT_BUDGET.md`](../runbooks/GOLDEN_COHORT_BUDGET.md)). |
| **`golden-cohort-nightly.yml`** — `cohort-real-llm-gate` | Structural gate + budget probe (no in-job AOAI call) | **PRs + schedule / dispatch.** Required status check. Heavy sibling jobs (faithfulness, RAG, simulator drift, real-mode eval) skip `pull_request`. | See [`docs/runbooks/GOLDEN_COHORT_REAL_LLM_GATE.md`](../runbooks/GOLDEN_COHORT_REAL_LLM_GATE.md); cost cap in **`tests/golden-cohort/budget.config.json`** |
...
**Local live evidence (owner machine):** follow **[`OWNER_REAL_MODE_EVIDENCE_CHECKLIST.md`](../runbooks/OWNER_REAL_MODE_EVIDENCE_CHECKLIST.md)** — copy **`secrets/local-real-aoai.env.example`** to **`secrets/local-real-aoai.env`** (gitignored), set classic **`https://{resource}.openai.azure.com/`** endpoint + key + deployment, set repo variable **`ARCHLUCID_CI_REAL_AOAI_ENABLED=true`**, then run **`.\scripts\Invoke-RealLlmEvidenceGate.ps1`**. Session template: [`docs/quality/REAL_LLM_SESSION_2026-05-29.md`](../quality/REAL_LLM_SESSION_2026-05-29.md).
...
| Metric | Notes |
|--------|--------|
| `archlucid_digest_delivery_succeeded` / `archlucid_digest_delivery_failed` | Tag **`channel`**. |
| `archlucid_alert_evaluation_duration_ms` | Tag **`rule_kind`**: `simple` \| `composite`. |
| `archlucid_governance_resolve_duration_ms` | End-to-end **`EffectiveGovernanceResolver.ResolveAsync`** latency. |
