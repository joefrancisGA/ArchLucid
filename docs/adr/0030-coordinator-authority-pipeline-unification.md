...
- **Status:** Accepted
- **Date:** 2026-04-21 (amended 2026-04-22 — owner Q&A on `PENDING_QUESTIONS.md` item **35a** + **35b**, see § Owner sub-decisions and the new **PR A0.5** row in § Component breakdown)
...
- **PR A4 backfill.** The backfill of historical `dbo.GoldenManifestVersions` rows to blob storage is an owner-decision: keep forever, keep N years, or drop without backfill. Pending question item **35d** captures this.
...
> The sub-decisions below extend `docs/PENDING_QUESTIONS.md` items **35a** and **35b**. They are recorded here (not just in `PENDING_QUESTIONS.md`) because each one binds a concrete shape decision in the **PR A0** / **PR A1** / **PR A0.5** rows of § Component breakdown above, and the next contributor reading this ADR needs the bind on the same page.
...
These six answers fully unblock PR A0 drafting and PR A1 drafting. PR A0.5 needs one more sub-decision (new item **35f** — `ManifestService.ServiceType` / `RuntimePlatform` typed source) before it can start; assistant will surface that as a separate ask. PR A2 still gated on item **35c** (legacy feature flag default + scope). PR A4 still gated on item **35d** (backfill destination). Item **35e** (PR B placeholder tracker location) is independent of A0–A4 and stays open.
...
- [`docs/PENDING_QUESTIONS.md`](../PENDING_QUESTIONS.md) item **16** sub-bullets and item **35** (sub-bullets **35a** + **35b** **resolved 2026-04-22 — see § Owner sub-decisions above**; sub-bullets **35c**, **35d**, **35e** still open; new sub-bullet **35f** — PR A0.5 typed-services source — opened 2026-04-22). *Note: the original draft of this ADR mis-numbered these as "item 34a–d"; that mis-numbering is corrected throughout this 2026-04-22 amendment — there is no item 34a–d in `PENDING_QUESTIONS.md`; item 34 is "Production Simmy / fault-injection game day".*
...
- [`docs/CHANGELOG.md`](../CHANGELOG.md) 2026-04-21 entry recording this ADR + the amendments to 0021 / 0022 / 0029; 2026-04-22 entry recording the 35a / 35b owner Q&A, the PR A0.5 row addition, and the 34→35 numbering correction.
