...
| **DemoLegacyArchitectureRunSynchronizer** | **`CreateAsync`**, **`UpdateStatusAsync`** | **Demo only:** **`DemoSeedService`** seeds **`dbo.Runs`** via **`IRunRepository.SaveAsync`** first; this synchronizer inserts/updates **`dbo.ArchitectureRuns`** so coordinator FKs (tasks, results, traces, etc.) remain valid until those tables reference **`dbo.Runs`**. | **Retire** when demo coordinator data no longer FKs **`ArchitectureRuns`**. | Active |
