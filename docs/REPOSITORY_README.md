...
## Customer delivery vs this repository
...
Start at **[docs/START_HERE.md](START_HERE.md)** (buyers). **Engineering team** onboarding for this repo also starts at **[docs/START_HERE.md](START_HERE.md)** (see the contributor section).
...
**Local smoke (engineering only)** — repo root; requires .NET 10 SDK + Docker — **not** the customer onboarding path:

```bash
dotnet run --project ArchLucid.Cli -- try
```

**Offline demo pack** (sanitized JSON for slides / examples): [`docs/demo/README.md`](demo/README.md) — `dotnet run --project ArchLucid.Cli -- demo export`.

Windows Docker-only helper: `.\scripts\demo-start.ps1`

<details>
<summary><strong>Quick buyer / sponsor links</strong> — START_HERE, sponsor brief, architecture poster</summary>

| Doc | Open this when… |
|-----|-----------------|
| **[`docs/START_HERE.md`](START_HERE.md)** | You need the **single canonical first-30-minutes** buyer / operator path |
| **[`docs/go-to-market/EXECUTIVE_SPONSOR_BRIEF.md`](go-to-market/EXECUTIVE_SPONSOR_BRIEF.md)** | You are a **sponsor, procurement partner, or outward buyer** |
| **[`docs/ARCHITECTURE_ON_ONE_PAGE.md`](ARCHITECTURE_ON_ONE_PAGE.md)** | You want the **architecture poster** (C4-style system map) |
...
**Canonical buyer narrative:** For sponsor-facing and outward buyer messaging, start with **[docs/go-to-market/EXECUTIVE_SPONSOR_BRIEF.md](go-to-market/EXECUTIVE_SPONSOR_BRIEF.md)**. The rest of this repository should stay aligned with that summary rather than competing with it.

**Repository layout:** Source lives under **`ArchLucid.*`** projects, **`archlucid-ui/`**, and **`docs/`**. Local packaging writes to **`artifacts/`** (gitignored). See **[docs/REPO_HYGIENE.md](library/REPO_HYGIENE.md)** for what to commit vs regenerate. User-visible changes are tracked in **[docs/CHANGELOG.md](CHANGELOG.md)**; the breaking-only narrative continues to live in **[BREAKING_CHANGES.md](../BREAKING_CHANGES.md)**.
...
**Pick your engineering onboarding path.** If you have never run ArchLucid on this machine, **Docker-only first-run:** **[docs/engineering/FIRST_30_MINUTES.md](engineering/FIRST_30_MINUTES.md)** needs nothing but Docker.

<details>
<summary><strong>Engineering persona table</strong> — who starts where (maintains legacy doc path)</summary>

Full table (filename retained for links): **[docs/library/CONTRIBUTOR_PERSONA_TABLE.md](library/CONTRIBUTOR_PERSONA_TABLE.md)** — applies to engineers and operators on **this codebase**, not a public OSS contributor pool.
...
Full capability inventory: **[docs/PRODUCT_PACKAGING.md](library/PRODUCT_PACKAGING.md)** (§3 *Two UI shaping surfaces* — **Visibility** via **`useNavSurface()`** (nav + **`LayerHeader`** / layer guidance) and **Capability** via **`useOperateCapability()`** (Execute+ mutation soft-enable + **`OperateCapabilityHints`**); *UI seam / drift guards* (**PRODUCT_PACKAGING** uses historical “Contributor” wording) + *Cross-surface lock* — keep **`nav-config.ts`**, **`nav-shell-visibility.ts`** (**tier → authority**), **`current-principal.ts`** (`/me` read-model), **`layer-guidance.ts` / `LayerHeader`**, **`operate-capability.ts`** / **`useOperateCapability()`**, **Vitest** seam tests including **`authority-seam-regression.test.ts`**, **`authority-execute-floor-regression.test.ts`**, and **`authority-shaped-ui-regression.test.ts`** (catalog **`ExecuteAuthority`** rows + mutation floor invariants), and **API** policies aligned). First-pilot walkthrough: **[docs/CORE_PILOT.md](CORE_PILOT.md)**. **Measurement companion:** **[docs/PILOT_ROI_MODEL.md](library/PILOT_ROI_MODEL.md)**. **Usage guidance:** **[docs/OPERATOR_DECISION_GUIDE.md](library/OPERATOR_DECISION_GUIDE.md)**. **Canonical buyer narrative:** **[docs/go-to-market/EXECUTIVE_SPONSOR_BRIEF.md](go-to-market/EXECUTIVE_SPONSOR_BRIEF.md)**. Future packaging map: **[docs/FUTURE_PACKAGING_ENFORCEMENT.md](library/FUTURE_PACKAGING_ENFORCEMENT.md)**. **Operator UI shaping only:** [archlucid-ui/README.md](../archlucid-ui/README.md#seam-maintenance-anti-drift) — nav and soft-disable follow **`/me`**; **ArchLucid.Api** still returns **401/403**. **Page-level mutation + layout seams:** Vitest **`archlucid-ui/src/app/(operator)/operate-authority-ui-shaping-*.test.tsx`** shards (hook → **`disabled`** / **`readOnly`**), **`archlucid-ui/src/app/(operator)/authority-shaped-layout-regression.test.tsx`** (inspect-first layout when mutation is off).

## Pilot onboarding (56R)

**Product boundary (V1):** [docs/V1_SCOPE.md](library/V1_SCOPE.md). **Pre-handoff checklist:** [docs/V1_RELEASE_CHECKLIST.md](library/V1_RELEASE_CHECKLIST.md). **Commands:** [docs/library/customer-facing/OPERATOR_QUICKSTART.md](library/customer-facing/OPERATOR_QUICKSTART.md). **Measurement companion and success criteria:** [docs/PILOT_ROI_MODEL.md](library/PILOT_ROI_MODEL.md). **Layer decision guidance:** [docs/OPERATOR_DECISION_GUIDE.md](library/OPERATOR_DECISION_GUIDE.md). **Canonical buyer narrative:** [docs/go-to-market/EXECUTIVE_SPONSOR_BRIEF.md](go-to-market/EXECUTIVE_SPONSOR_BRIEF.md). **Narrative (archived):** [docs/library/customer-facing/PILOT_GUIDE.md](library/customer-facing/PILOT_GUIDE.md) (supersedes 56R change-set guide). **Fix issues:** [docs/runbooks/TROUBLESHOOTING.md](runbooks/TROUBLESHOOTING.md). **Package an RC:** [docs/RELEASE_LOCAL.md](library/RELEASE_LOCAL.md).

**Before a handoff or demo:** `scripts\run-readiness-check.cmd` or `.\scripts\run-readiness-check.ps1`. For **API + CLI quick run + artifacts** in one script, set **`ARCHLUCID_SMOKE_SQL`** and run **`scripts\release-smoke.cmd`** ([docs/RELEASE_SMOKE.md](library/RELEASE_SMOKE.md)); optional UI E2E: **`.\scripts\release-smoke.ps1 -RunPlaywright`** ([archlucid-ui/docs/TESTING_AND_TROUBLESHOOTING.md](../archlucid-ui/docs/TESTING_AND_TROUBLESHOOTING.md#8-e2e-tests-playwright)).

> **Release smoke vs live Playwright:** **`scripts/release-smoke.ps1`** (with or without **`-RunPlaywright`**) does **not** replace CI’s **`live-api-*.spec.ts`** (**SQL-backed browser**) gates — **`-RunPlaywright`** runs **mock-backed** **`npm run test:e2e`**, separate from the smoke API process. Table: **[docs/library/RELEASE_SMOKE.md](library/RELEASE_SMOKE.md#release-smoke-ui-sql-parity)** · live path **[docs/library/LIVE_E2E_HAPPY_PATH.md](library/LIVE_E2E_HAPPY_PATH.md)**.

**Hosted SaaS URLs:** staging funnel `https://staging.archlucid.net`; production `https://archlucid.net` when Front Door hostnames are wired (see [docs/REFERENCE_SAAS_STACK_ORDER.md](library/REFERENCE_SAAS_STACK_ORDER.md), `infra/apply-saas.ps1`). **Public liveness (hosted):** `Invoke-RestMethod https://staging.archlucid.net/health/live` (or `/health/ready`). **`scripts/release-smoke.ps1`** still starts a **local** API for the E2E block; use **`-ApiBaseUrl`** / **`-BaseUrl`** only when that process is not on the default `http://localhost:5128` ([docs/RELEASE_SMOKE.md](library/RELEASE_SMOKE.md)).

**Build / version:** **`GET /version`** on the API, or **`dotnet run --project ArchLucid.Cli -- doctor`**. **Diagnostics:** **`dotnet run --project ArchLucid.Cli -- support-bundle --zip`** (review before sharing). **Reporting issues:** [docs/library/customer-facing/PILOT_GUIDE.md#when-you-report-an-issue](library/customer-facing/PILOT_GUIDE.md#when-you-report-an-issue) (version, correlation ID, logs, bundle).

## Operator quick start (HTTP API semantics)

Customers reach these endpoints **through the vendor-hosted SaaS deployment** (`archlucid.net` hostnames — not localhost). Sections below describing `appsettings` and local URLs are aimed at **engineering** running the API on a workstation.

- **Health:** `GET /health/live` (liveness), `GET /health/ready` (readiness: DB when using Sql storage, schema files, compliance rule pack, temp dir), `GET /health` (all checks). See [docs/engineering/BUILD.md](engineering/BUILD.md) for startup vs migration failure behavior.
- **Versioned API:** Routes are under `/v1/...`. Send optional **`X-Correlation-ID`** on requests for support correlation (see [docs/API_CONTRACTS.md](library/API_CONTRACTS.md)).
...
- **Cost-aware pilot / unit economics:** [docs/deployment/PILOT_PROFILE.md](deployment/PILOT_PROFILE.md), [docs/deployment/PER_TENANT_COST_MODEL.md](deployment/PER_TENANT_COST_MODEL.md).
...
- **AsyncAPI spec:** [`docs/contracts/archlucid-asyncapi-2.6.yaml`](contracts/archlucid-asyncapi-2.6.yaml)
- **Full reference:** [`docs/INTEGRATION_EVENTS_AND_WEBHOOKS.md`](library/INTEGRATION_EVENTS_AND_WEBHOOKS.md)
...
Local compose is for **engineering and CI-style validation**, not customer deployment.
...
## Database Setup (engineering / local stack)

Hosted SaaS customers do not provision databases; the vendor-operated planes per `infra/` own SQL topology.

1. Create a database (for example `ArchLucid`, or a pilot-specific name), or use `archlucid dev up` to run SQL Server in Docker **for development**.
...
## Running the API (engineering)

Production traffic for customers is hosted by ArchLucid SaaS infra; **`dotnet run` here targets local development.**
...
## Running Tests (engineering)

Full **54R** tier list, copy-paste commands, SQL variables, and **`archlucid-ui`** Vitest/Playwright: **[docs/TEST_STRUCTURE.md](library/TEST_STRUCTURE.md)**. CI job mapping: **[docs/TEST_EXECUTION_MODEL.md](library/TEST_EXECUTION_MODEL.md)**.
...
**ArchLucid.Api.Tests** integration tests need a reachable **SQL Server**; **`ArchLucidApiFactory`** creates ephemeral databases and runs **DbUp**. See **[docs/engineering/BUILD.md](engineering/BUILD.md)** for CPM, connection strings, and DecisionEngine’s Microsoft.Extensions bundle.

**Notable API behavior:** comparison replay with `replayMode: verify` returns **422** (problem+json with drift fields) when regenerated output does not match the stored comparison—not HTTP 200 with a failure flag. End-to-end run compare uses **`#run-not-found`** when a run ID is missing. See [docs/API_CONTRACTS.md](library/API_CONTRACTS.md).
...
## CLI (ArchLucid.Cli) — automation and engineering workflows

The CLI speaks to the HTTP API (`ARCHLUCID_API_URL`). **Primary operator flows for customers stay in hosted SaaS UIs.** The CLI is retained for CI, demos, diagnostics, seller-led automation, and **engineering** tooling. For reference, see [docs/CLI_USAGE.md](library/CLI_USAGE.md). Run commands with:
...
For details, including replay modes, supported formats, headers, and example curl commands, see [docs/COMPARISON_REPLAY.md](library/COMPARISON_REPLAY.md).
...
See [docs/DECISIONING_TYPED_FINDINGS.md](library/DECISIONING_TYPED_FINDINGS.md).
...
**Canonical poster (start here):** [`docs/ARCHITECTURE_ON_ONE_PAGE.md`](ARCHITECTURE_ON_ONE_PAGE.md) — C4 system context + containers, ownership table, happy-path trace. **Operator action map:** [`docs/OPERATOR_ATLAS.md`](library/OPERATOR_ATLAS.md) — every major UI route with API + CLI + authority hints.
