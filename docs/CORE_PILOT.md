...
| **Verification ladder (truth vs release smoke)** | **A)** `run-readiness-check` / deployment checks → **B)** **`docs/library/RELEASE_SMOKE.md`** (API+CLI) → **C)** CI **live Playwright** against a SQL-backed API (**`live-api-accessibility*.spec.ts`**, **`LIVE_E2E_HAPPY_PATH.md`**) for UI+SQL truth → **D)** Merge-blocking **k6 operator-path smoke** (`tests/load/k6-api-smoke.js`) applies **CI/pilot-only** p95 ceilings on the Core Pilot-shaped API slice (including seed→commit→artifact descriptors in Simulator mode — **not** a throughput or contractual SLO proof). See [PERFORMANCE_TESTING.md](library/PERFORMANCE_TESTING.md), [RELEASE_SMOKE.md](library/RELEASE_SMOKE.md#release-smoke-ui-sql-parity), and [LIVE_E2E_HAPPY_PATH.md](library/LIVE_E2E_HAPPY_PATH.md). |
