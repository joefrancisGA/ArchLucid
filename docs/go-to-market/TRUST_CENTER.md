...
- **Data isolation:** **Database-per-tenant** SQL catalogs are supported for launch (`ArchLucid:SqlTopology:Mode=SystemWithPerTenantCatalogs`) with **`TenantDatabaseBindings`** routing; **row-level security** remains defense-in-depth with per-request **`SESSION_CONTEXT`** (tenant / workspace / project) on covered tables — alongside application authorization; see [TENANT_ISOLATION.md](TENANT_ISOLATION.md), [../library/TENANT_DATABASE_TOPOLOGY.md](../library/TENANT_DATABASE_TOPOLOGY.md), and [../security/MULTI_TENANT_RLS.md](../security/MULTI_TENANT_RLS.md).
