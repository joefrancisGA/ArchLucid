...
- **Data isolation:** **Database-per-tenant** SQL catalogs (`ArchLucid:SqlTopology:Mode=SystemWithPerTenantCatalogs`) with **`TenantDatabaseBindings`** routing — **self-serve trial organizations use the same per-tenant product catalog pattern as paying tenants**; the database boundary is the primary tenant isolation mechanism. Application-layer scope enforcement (`IScopeContextProvider`, policies) provides authorization within a tenant. SQL Row-Level Security was evaluated and not adopted; see ADR 0037. The primary control is database-per-tenant catalogs; see [TENANT_ISOLATION.md](TENANT_ISOLATION.md) and [../library/TENANT_DATABASE_TOPOLOGY.md](../library/TENANT_DATABASE_TOPOLOGY.md).
...
- **Auditability:** Durable **append-only** audit trail in SQL (`dbo.AuditEvents`) with a **typed event catalog** and correlation identifiers; **[channel quick reference](../library/AUDIT_COVERAGE_MATRIX.md#quick-reference-which-channel-logs-what)** (governance vs baseline log vs platform SQL) in [../AUDIT_COVERAGE_MATRIX.md](../library/AUDIT_COVERAGE_MATRIX.md) and [SECURITY.md](../library/contributor-reference/SECURITY.md) (PII / exports). **Enterprise extended retention:** [`AUDIT_RETENTION_EXTENSION.md`](../library/AUDIT_RETENTION_EXTENSION.md).
- **Billing webhook integrity:** Stripe/Marketplace replay guard and idempotency runbook — [`../runbooks/BILLING_WEBHOOK_REPLAY_GUARD.md`](../runbooks/BILLING_WEBHOOK_REPLAY_GUARD.md).
...
**Tier 2 (optional continuous mode — Azure shipped V1; AWS shipped V1.1 TB-402; GCP shipped V1.1 TB-403):** Pull-based collection uses **federated workload identity** — no long-lived secrets stored in ArchLucid. **Azure:** customer provisions a dedicated service principal with **only** **`Reader`** and **`Cost Management Reader`**, scoped to a subscription or management group. **AWS (V1.1):** customer provisions a **read-only IAM role** that trusts ArchLucid's **Azure managed identity** via **OIDC / web identity federation**; ArchLucid polls **AWS Resource Explorer** and ingests the same schema v1 inventory ZIP as Tier 1 manual upload. **GCP (V1.1):** customer binds ArchLucid's Azure managed identity in a **Workload Identity Pool** and grants a read-only service account **Cloud Asset Viewer** (or equivalent); ArchLucid polls **Cloud Asset Inventory** and ingests the same schema v1 inventory ZIP as Tier 1 manual upload. Connection metadata is stored per tenant; **no** access-key pairs or service-account JSON keys on the primary path. See [../library/AZURE_EXTRACTOR_TECHNICAL_BACKLOG.md](../library/AZURE_EXTRACTOR_TECHNICAL_BACKLOG.md) and operator settings **Cloud connections**.
...
## GCP connectivity (extractor posture)

**Tier 1 (default):** Operators run **`scripts/Get-ArchLucidGcpPackage.ps1`**, inspect the artifact, and upload the **`schemaVersion`-versioned ZIP** via **`POST /v1/extractor/gcp/upload`**.

**Tier 2 (optional V1.1 — TB-403):** Same federated-trust model adapted for GCP: customer **Workload Identity Pool** provider bound to ArchLucid's Azure managed identity + read-only service account impersonation; scheduled polling via **`CloudPolling:Gcp`** (default off; 24 h interval when enabled). ArchLucid requests **read-only** inventory via **Cloud Asset Inventory** only — **no** write, mutate, or infrastructure-apply permissions.

### What we will never ask you to assign (GCP)

ArchLucid will **never** ask for **`Owner`**, **`Editor`**, **`roles/iam.serviceAccountKeyAdmin`**, or any role that can modify your GCP infrastructure on your behalf.
...
|----------|-------------|
| [TENANT_ISOLATION.md](TENANT_ISOLATION.md) | Buyer-readable summary of tenant isolation (identity, app layer, database-per-tenant). |
...
| Item | Status | Notes |
|------|--------|--------|
| **Accessibility Conformance Report** — VPAT® **2.5** structure, WCAG **2.1** A / AA (**Operator UI + marketing web**) | **In-repo ACR** — merge-blocking **axe-core** (Playwright + jest-axe) + honest manual gaps | Canonical: [VPAT_2_5_WCAG_2_1_AA.md](../security/VPAT_2_5_WCAG_2_1_AA.md) · evidence map: [VPAT_EVIDENCE_MAP.md](../security/VPAT_EVIDENCE_MAP.md) · public route **`/accessibility`** · [ACCESSIBILITY.md](../../ACCESSIBILITY.md). **Participant AT user testing is not claimed complete.** Superseded draft: [VPAT_2_4_WCAG_2_1_DRAFT.md](../security/VPAT_2_4_WCAG_2_1_DRAFT.md) (historical only). |
...
- **Coordinated disclosure key:** public OpenPGP material is at **`/.well-known/pgp-key.txt`** on the marketing site (committed in-repo at [`../../archlucid-ui/public/.well-known/pgp-key.txt`](../../archlucid-ui/public/.well-known/pgp-key.txt)). Custodian procedure and rotation: [../security/PGP_KEY_GENERATION_RECIPE.md](../security/PGP_KEY_GENERATION_RECIPE.md) — see repository **[`SECURITY.md`](../library/SECURITY.md)** (PGP section) for fingerprint.
...
| [../MARKETABILITY_ASSESSMENT_2026_04_15_SAAS_ONLY.md](../archive/MARKETABILITY_ASSESSMENT_2026_04_15_SAAS_ONLY_PRE_TRUST_CENTER.md) | SaaS-only marketability assessment |
