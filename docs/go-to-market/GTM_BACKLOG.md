...
**Updated:** 2026-07-24 — PA Q4 audit fail-closed follow-up: engineering **TB-999**/**TB-1000** (mutation × audit-failure matrix + honesty CI); GTM stays **M-117**/**M-118** (no new M rows). Prior: **M-148** / **M-149** opened (`(RunId, TaskId)` process vs provider billing — claim honesty + PA one-pager); engineering refs may lag TECH ID remap — prefer live **TB-*** rows in `TECH_BACKLOG.md`. Prior: **M-146** / **M-147** (LLM/evidence telemetry redaction surfaces — claim honesty + PA one-pager); engineering **TB-995**–**TB-997**. Prior: **M-144** / **M-145** (transactional outbox replay-safe vs consumer idempotency); engineering **TB-992**–**TB-994**. Prior: **M-142** / **M-143** (solo-operator pages vs support-email); engineering **TB-989**–**TB-991**. Prior: **M-140** / **M-141** (finding concurrent disposition race); engineering **TB-986**–**TB-988**. Prior: **M-138** / **M-139** (Simulator-derived ROI/savings forbid on sponsor surfaces); engineering **TB-983**–**TB-985**. Prior: **M-133**–**M-137** (showcase scenario portfolio); engineering **TB-978**–**TB-982**. Prior: **M-131** / **M-132** (INV-004); **M-129** / **M-130**; **M-127** / **M-128**; **M-119**–**M-126**; **M-117** / **M-118**; **M-115** / **M-116**; **M-113** / **M-114** **Done**. Does not duplicate **G-REAL-06**/**G-REAL-07**.
...
| **P1** | 98 | **~124–198 h** (≈ half is LinkedIn long-form draft+publish; incl. **G-REAL-09** DOCX visual check; **G-QA-06**/**G-QA-07**/**M-112** UI perf triage; **G-SCALE-01**/**G-SCALE-02** autoscale drills; **M-115**–**M-132** / **M-138**–**M-149** PA claim-honesty / one-pager cluster; **M-133**–**M-136** showcase scenario portfolio) |
...
| **Total unique open** | ~144 | **~212–345 h** active; calendar for cohorts/assurance/commerce dominates wall-clock |
...
| M-146 | LLM/evidence telemetry redaction claim honesty (Do Not Log ≠ no App Insights text) | P1 | **30–45 m** |
| M-147 | PA prompt/evidence telemetry surface matrix one-pager | P1 | **45–90 m** |
| M-148 | `(RunId, TaskId)` process vs provider billing claim honesty (no exactly-once AOAI) | P1 | **30–45 m** |
| M-149 | PA double-billing `(RunId, TaskId)` without provider exactly-once one-pager | P1 | **45–90 m** |
...
**Suggested near-term focus (highest leverage / hour for this profile):** **G-REAL-06 → G-REAL-07** (unlocks Stage 1) · **M-107** + **M-133** → **M-134** (showcase funnel coherence — hours, not weeks) · run **M-113** script before next PA review (docs Done) · **M-115**/**M-116** + engineering **TB-949**–**TB-952** (prompt-injection confinement story) · **M-117**/**M-118** + engineering **TB-953** (Done) / **TB-954**–**TB-956** / **TB-999**–**TB-1000** (audit Required fail-closed story) · **G-COMMERCE-01 → M-34 → G-COMMERCE-02** (first invoice path) · **M-07 → M-16 → M-09** (credibility assets) · **G-QA-01 → M-96–M-98** (founder acceptance loop) · **TB-915 → G-SCALE-01 → G-SCALE-02** (autoscale bake-off before launch load).
...
| M-114 | **Security-reviewer isolation one-pager** — buyer-safe G3 handout; complements [`TENANT_ISOLATION.md`](TENANT_ISOLATION.md). Artifact: [`BUYER_SECURITY_PROCUREMENT_PACKET.md`](BUYER_SECURITY_PROCUREMENT_PACKET.md#isolation-one-pager-m-114) | Content | P1 | **Done** | Authored **2026-07-22**; pairs **TB-925** (Done) |
...
| M-146 | **LLM/evidence telemetry redaction claim honesty** — do not promise “prompts never appear in App Insights” or “deny-list guarantees no PII in LLM traffic”; default is Do Not Log + metadata-only OTel; opt-in GenAI span capture and Real-mode agent-trace forensics are separate surfaces. Touch [`WHAT_NOT_TO_PROMISE.md`](WHAT_NOT_TO_PROMISE.md) / procurement / privacy AI language | Content | P1 | Not started | Engineering **TB-995**–**TB-997**; pairs **M-147**; extends Done **TB-330** / ADR 0053 without overclaiming redaction completeness |
| M-147 | **PA prompt/evidence telemetry surface matrix one-pager** — Serilog/App Insights vs optional `CapturePromptResponseOnSpans` vs agent-trace forensics vs support bundle; what is redacted vs ACL’d. Artifact: `LLM_EVIDENCE_TELEMETRY_REDACTION_PA_ONE_PAGER.md` (new) | Content | P1 | Not started | Engineering **TB-995**–**TB-997**; cite `LoggingPolicy` / `LLM_PROMPT_REDACTION.md` / `AGENT_TRACE_FORENSICS.md`; does not disable Real-mode forensic persistence |
| M-148 | **`(RunId, TaskId)` process vs provider billing claim honesty** — do not promise exactly-once Azure OpenAI billing or zero duplicate provider spend across Worker replicas; promise at-most-once **process** for durable successful results (skip + UNIQUE) only. Touch [`WHAT_NOT_TO_PROMISE.md`](WHAT_NOT_TO_PROMISE.md) / FinOps / procurement language; extend **M-121** bullets with the process-vs-provider distinction | Content | P1 | Not started | Engineering **TB-998**–**TB-1000**; pairs **M-149**; complements **M-121**/**M-122** (ACA interrupt UX) and **M-144** (bus delivery) without conflating layers |
| M-149 | **PA double-billing `(RunId, TaskId)` without provider exactly-once one-pager** — skip before LLM, SQL UNIQUE, accounting charge points, residual in-flight/concurrent race; what FinOps can defend today vs after **TB-943**/**TB-961**. Artifact: `AGENT_TASK_BILLING_IDEMPOTENCY_PA_ONE_PAGER.md` (new) | Content | P1 | Not started | Engineering **TB-998**–**TB-1000**; cite Done **TB-039**/**TB-201** + open **TB-960**; does not claim provider refunds |
