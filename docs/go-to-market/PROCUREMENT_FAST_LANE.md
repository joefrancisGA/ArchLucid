> **Scope:** Five-minute procurement skim — outbound links duplicate **`PROCUREMENT_PACK_INDEX.md`** and the buyer **`docs/go-to-market/trust-center.md`** index only (no artefacts beyond those inventories). Claims here are labels, not attestations; **legal-only** artefacts are explicitly **template-only** until executed.
