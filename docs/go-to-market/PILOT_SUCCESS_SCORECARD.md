...
| **Wizard-to-committed wall-clock** | Minutes from wizard submit to first committed manifest (wizard-sourced runs only) | p50 ≤ 15 min (aspirational) | p50 _______ / p95 _______ min | `archlucid.pilot.wizard_to_committed_minutes` OTel histogram (`execution_mode`, `preset_used`) |
