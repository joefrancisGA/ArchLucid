...
1. **Support role filtering is UI-only** — Severity: High. Dimension: Security/privacy. Evidence: role mapping shows `fieldFiltering: ui-only-v1`.
2. **Field-level PHI minimization is unresolved** — Severity: High. Dimension: Privacy. Evidence: field-level encryption is under discussion and PHI crosses multiple flows.
3. **Adjudication replay/idempotency risk** — Severity: High. Dimension: Reliability. Evidence: replay republishes original message with new queue message ID.
4. **LRS for PHI documents may be weak for durability expectations** — Severity: Medium/High. Dimension: Resilience. Evidence: storage account uses LRS.
5. **Retention/lifecycle policy is unclear** — Severity: Medium. Dimension: Governance. Evidence: 30-day delete retention vs 7-year document retention.
6. **Partner identity deferred but partner pilot wants access** — Severity: Medium. Dimension: Security. Evidence: ADR defers identity design.
7. **Database rollback undefined** — Severity: Medium. Dimension: Operability. Evidence: deployment model lacks DB rollback.
8. **OCR output may become ungoverned PHI** — Severity: Medium. Dimension: Data classification. Evidence: open question on normalized text extraction.
