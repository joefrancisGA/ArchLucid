...
1. **Cloud-only alerting conflicts with outage detection goal** — Severity: High. Dimension: Reliability. Evidence: alerts generated only after telemetry reaches cloud.
2. **Unauthenticated local status page exposes operational data** — Severity: Medium/High. Dimension: Security. Evidence: local UI lacks auth and shows gateway/line/error details.
3. **Certificate rotation/revocation weak** — Severity: Medium/High. Dimension: Device identity. Evidence: manual revocation, no expiration alert.
4. **180-day SAS token for firmware manifest** — Severity: Medium/High. Dimension: Supply chain/secrets. Evidence: long-lived SAS token.
5. **Buffer overflow drops oldest events** — Severity: Medium. Dimension: Data loss. Evidence: config says `drop_oldest`.
6. **Plant operators not notified of telemetry-platform degradation** — Severity: Medium. Dimension: Operability. Evidence: only machine alerts notify plant operators.
7. **Firmware rollback/approval unclear** — Severity: Medium. Dimension: Change safety. Evidence: manual rollback, approval unresolved.
