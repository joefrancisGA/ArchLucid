...
1. **Payment token references stored in Redis and shared with fraud service** — Severity: High. Dimension: Security/payment-adjacent. Evidence: Redis and fraud event samples include `paymentTokenRef`.
2. **Loyalty/customer data cached with unclear residency controls** — Severity: High for EU. Dimension: Privacy/residency. Evidence: Redis holds customerRef/loyaltyTier and EU question unresolved.
3. **Regional failover incomplete for orders** — Severity: High. Dimension: Reliability. Evidence: DB replication undecided and duplicate-order issue open.
4. **Promotion rules lack dual approval** — Severity: Medium. Dimension: Change governance. Evidence: business console can deploy rules without dual approval.
5. **Fraud event may over-share customer data** — Severity: Medium/High. Dimension: Data minimization. Evidence: customer ID, loyalty tier, payment token ref shared.
6. **Support search by payment token reference is risky** — Severity: Medium. Dimension: Access governance. Evidence: support can search by token reference.
7. **20-minute cache TTL may produce stale loyalty/promo decisions** — Severity: Medium. Dimension: Correctness. Evidence: profile changes may lag checkout.
