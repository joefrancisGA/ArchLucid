...
1. **Production data in lower environments before masking** — Severity: High. Dimension: Privacy/governance. Evidence: ADR and lower-environment note.
2. **Support ticket free text may contain PII and is copied into sandbox** — Severity: High. Dimension: Privacy. Evidence: classification issue + sandbox request includes support_tickets_raw.
3. **Sandbox lineage/retention controls weak** — Severity: Medium/High. Dimension: Governance. Evidence: notebook-derived features not cataloged; retention proposed.
4. **Manual BI semantic changes undermine certification** — Severity: Medium. Dimension: Change control. Evidence: BI changes manually applied before review.
5. **Data-quality ownership unclear** — Severity: Medium. Dimension: Operability. Evidence: warnings not always incidents; owner unresolved.
6. **Deletion propagation through derived datasets unresolved** — Severity: Medium. Dimension: Privacy. Evidence: open question.
7. **Broad raw-zone engineer access** — Severity: Medium. Dimension: Least privilege. Evidence: security notes.
