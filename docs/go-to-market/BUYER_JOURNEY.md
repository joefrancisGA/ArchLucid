...
ArchLucid is an **AI-assisted architecture workflow** with three product layers: **Core Pilot** (request → **review** (pipeline) → commit → manifest & artifacts), **Operate (analysis workloads)** (compare, replay, provenance), **Operate (governance and trust)** (governance, policy, alerts). See [../PRODUCT_PACKAGING.md](../library/PRODUCT_PACKAGING.md).
...
Fail-closed API auth (`ArchLucidAuth`), **scope policies** (`ReadAuthority` / `ExecuteAuthority` / `AdminAuthority`), **database-per-tenant** catalog routing with application scope predicates for tenant-scoped reads, **no SMB** on the public internet (private endpoints for Azure Storage). See [SECURITY.md](../library/contributor-reference/SECURITY.md) and [../onboarding/day-one-security.md](../onboarding/day-one-security.md).
