...
**Honest policy-awareness boundary (WK-21):** Bundled packs drive three influence kinds documented in [`POLICY_PACK_EXPECTATION_FACET.md`](../library/POLICY_PACK_EXPECTATION_FACET.md): (1) **compliance + declaration gating** via `complianceRuleKeys` / `priorityFloor`; (2) **coverage + cost extras** only when `advisoryDefaults` contains `expectation.*` or `cost.requireBudgetCap` (today: FinOps `cost.requireBudgetCap`, CIS Azure `identity` topology extra); (3) **pre-finalize gate** severity floors. Assigning SOC 2 alone does **not** stamp topology extras unless that pack's JSON includes the keys. This is **not** "all 39 engines are policy-aware."
