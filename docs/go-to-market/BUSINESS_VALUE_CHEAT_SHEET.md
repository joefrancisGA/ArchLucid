...
| Pre-finalize governance gate | Stops architecture packages from landing when severity or policy thresholds fail | Avoids shipping non-compliant architecture decisions into production; cuts emergency rework |
| Architecture package (versioned, immutable; API: golden manifest) | Locks the authoritative reviewed architecture snapshot tied to findings and traces | Single source of truth for sign-off and handoffs; shortens stakeholder alignment cycles |
...
| Architecture drift detection & comparison | Highlights what changed between two finalized designs | Targets incremental review effort; avoids full re-review cost for small deltas |
