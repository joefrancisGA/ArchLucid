...
**Rationale:** **76%** merged line is the current solution-wide bar (per-assembly Coverlet thresholds are not used—they do not match tree-wide coverage). **60%** merged branch tightens conditional coverage toward the improvement target without matching every branch on generated code. **60%** per product package raises the per-assembly floor so thin or effectively untested production projects surface in CI before merge.
