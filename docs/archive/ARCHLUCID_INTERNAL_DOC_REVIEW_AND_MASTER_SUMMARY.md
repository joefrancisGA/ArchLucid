...
- Summary: Use when you need **symptom → likely cause → first command → deeper doc**. Full flow: **[TROUBLESHOOTING.md](../runbooks/TROUBLESHOOTING.md)** and **[CORE_PILOT.md](../CORE_PILOT.md)**. **Correlation:** Include **`X-Correlation-ID`** (and `correlationId` inside ProblemDetails JSON) in notes whenever you open a thread, grep logs, or attach diagnostics. **Support bundle:** From a machine with CLI access to the tenant, run **`archlucid support-bundle --zip`** (or your deployment’s equivalent). Open **`README.txt`** then **`next-steps.json`**; use **`references.json`** for doc paths from repo root. Inspect contents before external send—see **support bundle** row below and [TROUBLESHOOTING.md](../runbooks/TROUBLESHOOTING.md) (support bundle / redaction). | Symptom | Likely cause | First command / check | Next doc | | --- | --- | --- | --- | | API unreachable / connection refused | Host down, wrong URL/port, ne
- Main headings:
...
#### `runbooks/PILOT_RESCUE_PLAYBOOK.md`
Use when you need **symptom → likely cause → first command → deeper doc**. Full flow: **[TROUBLESHOOTING.md](../runbooks/TROUBLESHOOTING.md)** and **[CORE_PILOT.md](../CORE_PILOT.md)**. **Correlation:** Include **`X-Correlation-ID`** (and `correlationId` inside ProblemDetails JSON) in notes whenever you open a thread, grep logs, or attach diagnostics. **Support bundle:** From a machine with CLI access to the tenant, run **`archlucid support-bundle --zip`** (or your deployment’s equivalent). Open **`README.txt`** then **`next-steps.json`**; use **`references.json`** for doc paths from repo root. Inspect contents before external send—see **support bundle** row below and [TROUBLESHOOTING.md](../runbooks/TROUBLESHOOTING.md) (support bundle / redaction). | Symptom | Likely cause | First command / check | Next doc | | --- | --- | --- | --- | | API unreachable / connection refused | Host down, wrong URL/port, network | `dotnet run --project ArchLucid.Cli -- doctor`; confirm `ARCHLUCID_API_URL` / TLS | [COMMON_ERRORS.md](COMMON_ERRORS.md), [TROUBLESHOOTING.md](../runbooks/TROUBLESHOOTING.md) | | **`/health/ready`** un
...
**Summary:** **Audience:** Customer success, sales engineering, and account management teams onboarding new SaaS customers. **Last reviewed:** 2026-04-17 This playbook aligns with the 6-week pilot timeline in [PILOT_SUCCESS_SCORECARD.md](PILOT_SUCCESS_SCORECARD.md) and the technical quickstart in [../OPERATOR_QUICKSTART.md](../library/customer-facing/OPERATOR_QUICKSTART.md). **Pricing:** Current tier pricing, pilot fee, and design-partner terms are in [PRICING_PHILOSOPHY.md §4–§5](PRICING_PHILOSOPHY.md). Do not restate prices in this playbook. | Item | Owner | Definition of done | |------|-------|--------------------| | Tenant provisioned in ArchLucid SaaS | ArchLucid | Tenant ID confirmed, workspace created | | SSO con
...
#### `integrations/IDP_PROVISIONING.md`
**Scope:** OIDC/JWT SSO configuration for ArchLucid API against secondary identity providers — Auth0 and Okta. Audience: enterprise IT / identity administrators using Auth0 or Okta instead of Entra ID.
**Title:** IdP provisioning — Auth0 and Okta SSO configuration
**Summary:** Merged (2026-07-20) from the retired `integrations/SSO_AUTH0_CONFIGURATION.md` and `integrations/SSO_OKTA_CONFIGURATION.md` into one provider-comparison doc. Shared overview, ArchLucid-side configuration table, and troubleshooting are written once with per-provider variants called out inline; provider-specific setup steps (API/application registration, custom `roles` claim, group/role mapping) stay in their own numbered sections (§2 Auth0, §3 Okta) so nothing was lost in the merge.
**Headings:** IdP provisioning — Auth0 and Okta SSO configuration; 1. Overview; 2. Auth0-side configuration; 3. Okta-side configuration; 4. ArchLucid-side configuration; 5. Verification; 6. Troubleshooting; 7. Role and policy reference
...
**Scope:** Redirect — merged 2026-07-20 into `docs/library/OBSERVABILITY.md` § Authority pipeline remediation runbook. Stub kept at this exact path because Terraform (`prometheus_slo_rules.tf`, `terraform-container-apps` variables/README) and a CI test (`test_prometheus_slo_dead_letter_alert_rule_tf.py`) reference this path by literal string.
**Title:** Authority pipeline metering and Grafana remediation — moved
**Summary:** Full remediation content (architecture, alert-by-alert playbooks, concurrency tuning by environment tier, lease-table growth triage) moved into `docs/library/OBSERVABILITY.md`, which already owned the metric catalog these alerts reference. Kept as a thin redirect rather than deleting outright since Terraform-embedded URLs and a CI test assert this exact path.
**Headings:** Authority pipeline metering and Grafana remediation — moved
...
#### `security/TENANT_TABLE_ISOLATION_CLASSIFICATION.md`
