# Runbook: Terraform `state mv` for Phase 7.5 (`archiforge` resource addresses) — archived

**Archived:** 2026-04-19 — Superseded for **greenfield** ArchLucid deployments (no Azure resources deployed yet). Phase 7.5 was completed in-repo by removing `moved {}` blocks, using `archlucid` resource labels only, and renaming the APIM API Azure name to `archlucid-api`. CI rejects the substring `archiforge` in any `infra/**/*.tf` file.

**Use this document only if** you have **existing** Terraform state in Azure that still references historical `*.archiforge` addresses (brownfield migration).

---

<!-- Original runbook body preserved below for brownfield operators. -->
