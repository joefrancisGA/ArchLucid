...
- [x] 276. **`SqlScopedResolutionDbConnectionFactory`** — singleton **`IDbConnectionFactory`** for SQL storage that opens connections via a **short scope** resolving scoped **`ISqlConnectionFactory`** (resilience + optional RLS) for **`CreateOpenConnectionAsync`**, while **`CreateConnection`** stays an unopened **`SqlConnection`** for readiness probes. SQL mode registers this bridge inside **`AddArchLucidStorage`**. *(2026-04-18 follow-up: removed erroneous **`RegisterDataInfrastructure`** that overwrote InMemory **`UnsupportedRelationalDbConnectionFactory`** with **`Data.SqlConnectionFactory`**, breaking ZAP without **`ConnectionStrings:ArchLucid`**.)*
...
- [x] Documentation & ADRs (155–169): complete (155 XML doc pieces 12–21 done; 156–169 largely addressed via `docs/architecture/adrs`, runbooks, `API_CONTRACTS`, `ALERTS`, `BUILD`, `TEST_STRUCTURE`, `CONTRIBUTOR_ONBOARDING`, `terraform-azure-variables`, `CONTEXT_INGESTION` SMB note).
