# ArchLucid Assessment - (A) Headline Readiness: 81.20%
...
Scoring basis: clean-slate assessment updated after implementation pass 2026-05-29. Weighted readiness uses the user-supplied weights exactly: total weight 119; weighted score 9663; readiness `9663 / 119 = 81.20%`.

## 2. Executive Summary

### `(A)` Overall Headline Readiness

ArchLucid is a credible V1/V1.1-shaped product, not a slideware prototype. The repo shows a coherent pilot path, SQL persistence, API/UI/CLI surfaces, live API + SQL E2E coverage, trust documentation, audit coverage, policy packs, Azure extractor posture, ROI materials, and substantial agent/RAG hardening. The main headline drag is not missing scope that was intentionally deferred; it is the remaining gap between an internally strong engineering system and buyer-proof evidence that real agent outputs, ROI claims, and production-like handoff work reliably under current hosted conditions.

### `(B)` Procurement / Market-Motion Realism

Procurement friction remains materially higher than the `(A)` score implies. SOC 2 CPA attestation, third-party pen-test report, named public reference, live Marketplace publication, and design-partner proof are explicitly outside the headline score where deferred, but buyers will still ask for them. The current Trust Center is honest and useful, but enterprise security/procurement teams will treat self-assessment and templates as interim evidence.

### Commercial Picture

The commercial story is real but still founder-led. Pricing, ROI, service-led offers, quote-request flow, procurement pack, and sponsor materials exist. Monetization risk concentrates around proof: buyers need to see a fast first value moment, credible live output, and a clear path from pilot to paid conversion without feeling they are funding product discovery.

### Enterprise Picture

Enterprise posture is unusually deep for a V1 product: tenant isolation, audit, SCIM, SAML/OIDC, policy packs, trust docs, data residency narrative, support bundles, and runbooks are present. The remaining blocker is confidence, not breadth: an enterprise buyer will need environment-specific proof, clean support artifacts, and careful procurement handling around assurance gaps.

### Engineering Picture

Engineering quality is strong but heavy. The architecture is modular and heavily documented, with visible invariants and many regression gates. The product is also broad enough that cognitive load, configuration complexity, real-LLM validation, and cross-surface consistency are the most serious engineering risks.
...
- **Score:** 76
- **Weight:** 8
- **Weighted impact on readiness:** 608 / 119
- **Weighted deficiency signal:** 192
- **Justification:** The product uses a modern AI substrate: Azure OpenAI configuration, structured output/schema validation, RAG corpora, policy-pack grounding, prior-manifest retrieval, semantic reranking, embedding drift guards, cost budgets, redaction, content safety, and agent output quality gates. The weakness is proof depth: the live Azure OpenAI golden-cohort evidence was blocked by missing credentials in the available environment, and the strongest metrics are exemplar/offline rather than fresh hosted real-mode evidence.
- **Tradeoffs:** The current design favors enterprise constraints and deterministic safety over experimental autonomy. That is commercially sensible, but it makes the product feel less "frontier" than systems that visibly demonstrate agentic planning, graph-RAG, or live copiloting.
- **Improvement recommendations:** Capture fresh real-LLM golden cohort runs; publish a concise AI evidence appendix; strengthen faithfulness trend gates; expose model/deployment provenance clearly in sponsor artifacts.
- **Disposition:** Fixable in v1 for evidence and evaluation hardening; advanced graph-RAG/agentic retrieval remains better suited for V2 unless promoted.

### AI/Agent Readiness

- **Score:** 78
- **Weight:** 8
- **Weighted impact on readiness:** 624 / 119
- **Weighted deficiency signal:** 176
- **Justification:** Agent readiness is above average: simulator/real modes are explicit, schema validation exists, quality gates exist, RAG grounding traces exist, budget limits exist, and CI has eval harnesses. The gap is confidence under real hosted LLM execution. Available evidence says the live AOAI run did not execute in the recorded environment.
- **Tradeoffs:** Simulator-first CI gives deterministic reliability and low cost, but semantic quality cannot be fully proven from simulator lanes.
- **Improvement recommendations:** Run credentialed real-mode evidence weekly; fail or warn on real-mode faithfulness regressions; attach model/version/run metadata to buyer-safe evidence.
- **Disposition:** Mostly fixable in v1, except owner-provided cloud credentials are needed for the actual live evidence run.
...
- **Weighted impact on readiness:** 624 / 119
- **Weighted deficiency signal:** 176
- **Justification:** ArchLucid has a strong marketable wedge: evidence-backed architecture reviews with exportable board/sponsor artifacts. Service-led offers, pricing, trust center, and ROI materials are present. The weakness is market proof: the docs can explain value, but the currently available materials do not provide enough fresh, buyer-verifiable proof that the promise lands in a short pilot.
- **Tradeoffs:** Positioning as an evidence-backed review system is more credible than generic "AI governance platform" positioning, but may sound narrower than the product's full breadth.
- **Improvement recommendations:** Lead with packaged review SKUs, publish concrete demo proof packets, emphasize first-review outcome over platform breadth, and remove any copy that forces buyers to infer value from feature lists.
...
- **Score:** 74
- **Weight:** 6
- **Weighted impact on readiness:** 444 / 119
- **Weighted deficiency signal:** 156
- **Justification:** The repo contains operator paths, quickstarts, live E2E docs, config lint, support bundles, and hosted pilot profiles. Friction remains because meaningful operation still requires SQL, auth posture, Azure/OpenAI choices, evidence ingestion, and careful scope understanding. The docs help, but there are many doors.
- **Tradeoffs:** Enterprise-grade controls increase setup surface. Simplifying too aggressively could hide important security and tenant-isolation decisions.
- **Improvement recommendations:** Create one "20-minute first value" path; reduce duplicate first-pilot entry points; add environment-specific proof packet output; make config lint outcomes sponsor-readable.
- **Disposition:** Fixable in v1.
...
- **Score:** 82
- **Weight:** 8
- **Weighted impact on readiness:** 656 / 119
- **Weighted deficiency signal:** 144
- **Justification:** Correctness is supported by API contract snapshots, SQL-backed E2E lanes, audit matrix guards, data consistency probes, release smoke scripts, schema validation, and many test projects. Remaining correctness concern is concentrated in AI output validity and ROI/cost claims: deterministic system behavior is stronger than semantic output proof.
- **Tradeoffs:** The product separates deterministic decisioning from AI narrative. That reduces risk but means correctness must be evaluated in two layers: pipeline correctness and recommendation faithfulness.
- **Improvement recommendations:** Expand real-mode golden tests; require citation-backed ROI/cost claims; keep OpenAPI/client/UI generation in lockstep; add cross-surface tests for key sponsor artifacts.
- **Disposition:** Fixable in v1.
...
- **Score:** 80
- **Weight:** 6
- **Weighted impact on readiness:** 480 / 119
- **Weighted deficiency signal:** 120
- **Justification:** Stickiness is good because committed reviews, manifests, audit trails, policy packs, prior-manifest retrieval, comparison, replay, governance, and executive ROI rollups create accumulated organizational value. The weakness is that deeper workflow embedding via first-party ITSM/docs/chat connectors is scoped to V1.1, so V1 stickiness relies more on review artifacts and governance loops than native daily-tool integration.
- **Tradeoffs:** V1 avoids overpromising integrations, but that leaves some accounts using ArchLucid as a periodic review tool rather than a daily system of work.
- **Improvement recommendations:** Make repeat-review loops and prior-decision reuse more visible; tighten team/tenant usage nudges; package policy-pack authoring as an expansion motion.
- **Disposition:** Fixable in v1 for artifact-driven stickiness; deeper connector stickiness is V1.1.

### Proof-of-ROI Readiness

- **Score:** 76
- **Weight:** 5
- **Weighted impact on readiness:** 380 / 119
- **Weighted deficiency signal:** 120
- **Justification:** ROI modeling is detailed and mapped to shipped capabilities. First-value reports, pilot deltas, executive ROI summary, pricing philosophy, and service-led offers exist. The weakness is evidence quality: ROI examples remain benchmark/template driven unless a tenant's actual before/after values are captured and cited.
- **Tradeoffs:** Conservative templates are better than unsupported claims, but enterprise sponsors buy quantified outcomes, not just plausible formulas.
- **Improvement recommendations:** Add a first-pilot ROI evidence capture workflow; require source citations for savings lines; generate a sponsor-ready before/after packet from real run data.
- **Disposition:** Fixable in v1.
...
- **Score:** 84
- **Weight:** 7
- **Weighted impact on readiness:** 588 / 119
- **Weighted deficiency signal:** 112
- **Justification:** The first-pilot path is explicit: create review, execute, commit, review artifacts. There are quickstarts, checklists, live E2E paths, and release smoke. The remaining issue is not capability but path clarity: too many docs can make the first path feel heavier than the product itself.
- **Tradeoffs:** Comprehensive docs reduce support risk but can slow evaluators who only need the shortest path.
- **Improvement recommendations:** Promote one canonical operator checklist; add a small proof packet artifact that says exactly what passed, what was skipped, and what the buyer can trust.
- **Disposition:** Fixable in v1.
...
- **Score:** 77
- **Weight:** 4
- **Weighted impact on readiness:** 308 / 119
- **Weighted deficiency signal:** 92
- **Justification:** Differentiation is strongest around traceable architecture review, evidence bundles, manifests, governance, and Azure-safe advisory posture. It is weaker where buyers compare the AI layer to generic copilots: without fresh real-mode examples and concise proof packets, the difference may be described better than it is felt.
- **Tradeoffs:** Narrower evidence-backed positioning is more defensible than broad AI claims but needs sharp demos.
- **Improvement recommendations:** Produce comparison proof against generic AI review; show traceability and replay as differentiators; make "ArchLucid never applies/destroys infra" prominent.
- **Disposition:** Fixable in v1.
...
- **Score:** 82
- **Weight:** 4
- **Weighted impact on readiness:** 328 / 119
- **Weighted deficiency signal:** 72
- **Justification:** Sponsor briefs, first-value reports, board exports, executive ROI summary, dashboards, and DOCX/PDF artifacts exist. The weakness is proof density: executive outputs need sharper "what changed, what saved money, what decision to make next" framing.
- **Tradeoffs:** Detailed traceability can overwhelm executives if not compressed.
- **Improvement recommendations:** Add a one-page sponsor packet generated from a committed review; include explicit limitations and source citations.
- **Disposition:** Fixable in v1.
...
- **Score:** 76
- **Weight:** 3
- **Weighted impact on readiness:** 228 / 119
- **Weighted deficiency signal:** 72
- **Justification:** V1 embeds through REST, CLI, operator UI, SCIM, GitHub/Azure DevOps-style surfaces, exports, audit, and policy workflows. First-party Jira, ServiceNow, Confluence, Slack, Teams, and broader webhooks are V1.1-scoped or otherwise not V1 headline gates.
- **Tradeoffs:** REST/CLI/UI are enough for pilots but not enough to feel native inside all enterprise operating rhythms.
- **Improvement recommendations:** For v1, improve handoff recipes and export-to-existing-workflow patterns; for V1.1, execute committed connectors without inventing parallel schemas.
- **Disposition:** V1 improvement possible through recipes and handoff clarity; first-party workflow depth is V1.1.

### Usability

- **Score:** 78
- **Weight:** 3
- **Weighted impact on readiness:** 234 / 119
- **Weighted deficiency signal:** 66
- **Justification:** The operator UI has live E2E coverage, accessibility sweeps, setup checklists, and role-based routes. Usability risk comes from product breadth, terminology transitions, and configuration choices that can make the first session feel like an implementation project.
- **Tradeoffs:** Power-user surfaces support enterprise needs but increase cognitive load for first-time evaluators.
- **Improvement recommendations:** Continue consolidating first-pilot routing; add task-oriented empty states and error recovery; keep legacy "run" wording from leaking into buyer-facing review language where possible.
- **Disposition:** Fixable in v1.
...
- **Score:** 79
- **Weight:** 3
- **Weighted impact on readiness:** 237 / 119
- **Weighted deficiency signal:** 63
- **Justification:** The system is honest about simulator/real modes, trust posture, SOC self-assessment, data isolation, audit, support, redaction, and advisory-only Terraform. Trustworthiness is reduced by live real-LLM evidence gaps and procurement-grade assurance gaps that remain `(B)` informational.
- **Tradeoffs:** Honest limitations reduce overselling but may slow some procurement cycles.
- **Improvement recommendations:** Attach provenance and mode labels everywhere sponsor-facing; keep self-assessment wording conservative; add buyer-safe real-mode evidence.
- **Disposition:** Mostly fixable in v1; SOC CPA and third-party pen test remain deferred procurement realism.

### Security

- **Score:** 82
- **Weight:** 3
- **Weighted impact on readiness:** 246 / 119
- **Weighted deficiency signal:** 54
- **Justification:** Security posture includes OIDC/SAML, API keys, RBAC, SCIM, tenant catalogs, RLS/session context options, private endpoint/WAF IaC, Key Vault references, content safety, prompt redaction, CI security checks, audit, and safe Azure extractor boundaries. Remaining risk is configuration discipline across environments.
- **Tradeoffs:** Rich auth/config options help enterprise fit but raise misconfiguration risk.
- **Improvement recommendations:** Keep production-like config lint fail-closed; add clearer operator evidence outputs; continue reducing dev-bypass ambiguity in handoff docs.
- **Disposition:** Fixable in v1.
...
- **Score:** 73
- **Weight:** 2
- **Weighted impact on readiness:** 146 / 119
- **Weighted deficiency signal:** 54
- **Justification:** Buyers have pricing, quote requests, order templates, service offers, trust docs, and ROI material. Decision velocity is slowed by assurance questions, broad docs, and the need for guided proof.
- **Tradeoffs:** Sales-led motion is appropriate before live self-serve commerce, but it adds human latency.
- **Improvement recommendations:** Build a "send this after demo" decision packet; map common objections to proof artifacts; clarify pilot-to-contract next steps.
- **Disposition:** Fixable in v1 for sales-led motion; live self-serve commerce remains V1.1/owner-gated.

### Procurement Readiness

- **Score:** 75
- **Weight:** 2
- **Weighted impact on readiness:** 150 / 119
- **Weighted deficiency signal:** 50
- **Justification:** The procurement pack, Trust Center, DPA template, subprocessors, SOC roadmap, CAIQ/SIG, and security docs are substantial. Procurement readiness is limited by interim assurance status and legal/customer-specific completion requirements.
- **Tradeoffs:** Templates are useful but cannot replace signed legal review or third-party assurance.
- **Improvement recommendations:** Keep procurement pack strict mode fresh; add a buyer-safe evidence manifest from the latest validated pilot; maintain sharp `(B)` wording for SOC and pen-test questions.
- **Disposition:** Fixable in v1 for pack quality; third-party assurance is deferred.

### Interoperability

- **Score:** 76
- **Weight:** 2
- **Weighted impact on readiness:** 152 / 119
- **Weighted deficiency signal:** 48
- **Justification:** API, CLI, OpenAPI, AsyncAPI docs, SCIM, GitHub/Azure DevOps references, Azure extractor upload, and export surfaces exist. The current limitation is first-party enterprise connector breadth, much of which is V1.1-scoped.
- **Tradeoffs:** Stable REST/CLI surfaces are safer than rushing brittle connectors.
- **Improvement recommendations:** Improve OpenAPI/client drift discipline; publish recipe-grade examples for current surfaces; do not create target-specific schemas for V1.1 connectors.
- **Disposition:** V1 recipes fixable now; connector breadth V1.1.

### Architectural Integrity

- **Score:** 84
- **Weight:** 3
- **Weighted impact on readiness:** 252 / 119
- **Weighted deficiency signal:** 48
- **Justification:** The architecture is coherent: bounded projects, composition root, persistence split, authority pipeline, invariants, ADRs, contract snapshots, tenant isolation model, and advisory-only IaC posture. The main integrity risk is breadth and legacy compatibility paths increasing reasoning cost.
- **Tradeoffs:** Keeping compatibility and shipped surfaces stable adds some complexity.
- **Improvement recommendations:** Keep enforcing architecture invariants; finish high-value analyzer waves; retire legacy naming only when scheduled and safe.
- **Disposition:** Fixable in v1 as incremental hardening.

### Compliance Readiness

- **Score:** 77
- **Weight:** 2
- **Weighted impact on readiness:** 154 / 119
- **Weighted deficiency signal:** 46
- **Justification:** Compliance readiness has strong internal evidence: SOC self-assessment, roadmap, CAIQ/SIG, compliance matrix, audit export, policy packs, DPA templates, and trust docs. It is not formal certification readiness yet, and that is procurement realism rather than headline deduction where deferred.
- **Tradeoffs:** Self-assessment supports early buyers but does not satisfy rigid SOC 2/ISO requirements.
- **Improvement recommendations:** Maintain conservative wording; keep evidence pack fresh; add operating evidence artifacts from real pilots.
- **Disposition:** V1 pack improvements fixable; CPA attestation deferred.

### Maintainability

- **Score:** 78
- **Weight:** 2
- **Weighted impact on readiness:** 156 / 119
- **Weighted deficiency signal:** 44
- **Justification:** Maintainability benefits from modular projects, Dapper/lightweight persistence, single DDL file discipline, generated clients, architecture tests, and extensive docs. Risk comes from solution size, many options, legacy flows, and broad product surface.
- **Tradeoffs:** Modularity reduces local complexity but increases navigation overhead.
- **Improvement recommendations:** Continue invariant enforcement, dependency constraints, source-of-truth docs, and code map hygiene; avoid new parallel abstractions.
- **Disposition:** Fixable in v1.
...
- **Weighted impact on readiness:** 156 / 119
- **Weighted deficiency signal:** 44
- **Justification:** Pricing, tiers, order form, service-led offers, marketplace alignment, and quote request flow exist. Packaging needs sharper buyer-entry alignment between product subscription, service-led review SKUs, and proof artifacts.
- **Tradeoffs:** Offering both SaaS tiers and founder-led services is commercially flexible but can confuse buyers if not presented as a sequenced motion.
- **Improvement recommendations:** Make "buy this first" explicit; tie each SKU to one output artifact and one acceptance outcome.
- **Disposition:** Fixable in v1.
...
- **Score:** 81
- **Weight:** 2
- **Weighted impact on readiness:** 162 / 119
- **Weighted deficiency signal:** 38
- **Justification:** Reliability has health checks, retries, outbox patterns, data consistency probes, config validation, smoke scripts, live E2E, and budget/circuit-breaker concepts. Weakness remains around environment-specific proof and operational maturity under real hosted traffic.
- **Tradeoffs:** Single-region V1 is intentional and should not be penalized as active/active absence.
- **Improvement recommendations:** Capture staged pilot run evidence; add stronger readiness snapshots for support; keep data consistency probes visible.
- **Disposition:** Fixable in v1.
...
- **Score:** 88
- **Weight:** 3
- **Weighted impact on readiness:** 264 / 119
- **Weighted deficiency signal:** 36
- **Justification:** Traceability is a major strength: manifests, artifacts, provenance graph, audit events, correlation IDs, OpenAPI snapshots, requirements-test traceability, retrieval grounding traces, and export bundles. Remaining work is mostly presentation and keeping source-of-truth drift low.
- **Tradeoffs:** High traceability can increase cognitive load unless summarized well.
- **Improvement recommendations:** Improve sponsor-facing trace summaries and support-bundle triage indexes.
- **Disposition:** Fixable in v1.
...
- **Score:** 82
- **Weight:** 2
- **Weighted impact on readiness:** 164 / 119
- **Weighted deficiency signal:** 36
- **Justification:** Explain endpoints, aggregate summaries, provenance graph, evidence chains, retrieval traces, and first-value reports support explainability. The gap is that explanations must be consistently grounded and concise enough for buyers.
- **Tradeoffs:** Rich explanation data can overrun executive attention.
- **Improvement recommendations:** Strengthen citation floors and short sponsor narratives; make unsupported claims explicit.
- **Disposition:** Fixable in v1.
...
- **Score:** 83
- **Weight:** 2
- **Weighted impact on readiness:** 166 / 119
- **Weighted deficiency signal:** 34
- **Justification:** SQL persistence, DbUp, orphan probes, remediation endpoints, RLS/session context, and consistency runbooks are strong. Risk remains in broad workflow breadth and historical/legacy paths.
- **Tradeoffs:** Automated remediation is safer with dry-run first, but requires operator discipline.
- **Improvement recommendations:** Keep dry-run/execute payload retention; expand consistency checks only where real incidents justify them.
- **Disposition:** Fixable in v1.
...
- **Score:** 70
- **Weight:** 1
- **Weighted impact on readiness:** 70 / 119
- **Weighted deficiency signal:** 30
- **Justification:** Cognitive load is the lowest raw score. The repo is large, docs are numerous, and the product has many modes and surfaces. The mitigations are good, but a new buyer or contributor still has to understand reviews vs runs, Pilot vs Operate, simulator vs real, V1 vs V1.1, and multiple validation lanes.
- **Tradeoffs:** Removing nuance would create false confidence; the answer is better routing, not less truth.
- **Improvement recommendations:** Consolidate first-pilot entry points and add role-specific "ignore this until later" guidance.
- **Disposition:** Fixable in v1.
...
- **Score:** 86
- **Weight:** 2
- **Weighted impact on readiness:** 172 / 119
- **Weighted deficiency signal:** 28
- **Justification:** Governance workflows, policy packs, approval segregation, pre-commit gate, audit, and bundled default policy packs are strong. The weakness is avoiding overclaiming certification-style compliance from starter rules.
- **Tradeoffs:** Starter packs accelerate review, but exhaustive certification automation is out of scope.
- **Improvement recommendations:** Preserve "review prompt, not certification" wording and make pack versioning visible.
- **Disposition:** Fixable in v1.
...
- **Weighted impact on readiness:** 172 / 119
- **Weighted deficiency signal:** 28
- **Justification:** Azure-first design is strong: Azure SQL, Container Apps, Blob, Key Vault, Azure OpenAI, private endpoints, WAF, Terraform, and Azure extractor posture. V1 intentionally avoids non-Azure hosting and active/active guarantees.
- **Tradeoffs:** Azure-native focus improves cohesion but narrows buyer fit until V1.1 multi-cloud analysis matures.
- **Improvement recommendations:** Keep Terraform examples current; capture production-like deploy evidence; clarify what is advisory vs applied.
- **Disposition:** Fixable in v1 for Azure proof; AWS/GCP target analysis is V1.1.

### Availability

- **Score:** 76
- **Weight:** 1
- **Weighted impact on readiness:** 76 / 119
- **Weighted deficiency signal:** 24
- **Justification:** Health checks, SLO targets, probes, and runbooks exist. Actual production availability evidence is not established from the materials; staging probes are explicitly not production SLA proof.
- **Tradeoffs:** Honest availability language is better than pretending synthetic staging proves production SLA.
- **Improvement recommendations:** Attach environment-specific uptime/probe summaries when available.
- **Disposition:** Fixable in v1 operationally; multi-region active/active is deferred.

### Customer Self-Sufficiency

- **Score:** 78
- **Weight:** 1
- **Weighted impact on readiness:** 78 / 119
- **Weighted deficiency signal:** 22
- **Justification:** Docs, CLI, runbooks, config lint, support bundles, and first-pilot paths support self-sufficiency. First deployments will still benefit from guided help because setup spans identity, SQL, LLM, and Azure posture.
- **Tradeoffs:** Full enterprise self-service would require more productized onboarding than V1 needs.
- **Improvement recommendations:** Improve proof packet and troubleshooting flow; keep "call support with these artifacts" clear.
- **Disposition:** Fixable in v1.
...
- **Weighted impact on readiness:** 78 / 119
- **Weighted deficiency signal:** 22
- **Justification:** Custom handler documentation, modular agents, DI composition, policy packs, OpenAPI, and CLI support extensibility. Public plugin SDK, MCP, and marketplace are explicitly not V1 headline gates.
- **Tradeoffs:** Code-level extensibility is useful for advanced integrators but not the same as a third-party ecosystem.
- **Improvement recommendations:** Keep custom handler guide aligned with current composition and tests.
- **Disposition:** V1 docs fixable; ecosystem surfaces deferred.

### Deployability

- **Score:** 79
- **Weight:** 1
- **Weighted impact on readiness:** 79 / 119
- **Weighted deficiency signal:** 21
- **Justification:** Docker, compose, Terraform, release smoke, config lint, and CD post-deploy verification exist. Deployment remains non-trivial because enterprise hosted pilots need SQL/auth/LLM/telemetry decisions.
- **Tradeoffs:** More automation would help but could accidentally imply ArchLucid owns customer infra changes.
- **Improvement recommendations:** Improve minimal Azure pilot deployment proof and operator handoff outputs.
- **Disposition:** Fixable in v1.
...
- **Score:** 80
- **Weight:** 1
- **Weighted impact on readiness:** 80 / 119
- **Weighted deficiency signal:** 20
- **Justification:** Config catalog, admin diagnostics, config lint, settings routes, policy/governance admin, audit export, and support runbooks are strong. The main issue is number of knobs.
- **Tradeoffs:** Enterprise configurability raises admin burden.
- **Improvement recommendations:** Add opinionated profiles and clearer PASS/WARN/HOLD outputs.
- **Disposition:** Fixable in v1.
...
- **Score:** 82
- **Weight:** 1
- **Weighted impact on readiness:** 82 / 119
- **Weighted deficiency signal:** 18
- **Justification:** The repo is highly testable: many test projects, contract tests, architecture tests, live E2E, smoke scripts, eval harnesses, and CI guards. Remaining gap is live real-LLM semantic validation.
- **Tradeoffs:** Full end-to-end fidelity is expensive and credential-dependent.
- **Improvement recommendations:** Continue using simulator for deterministic gates while adding scheduled real-mode evidence where credentials exist.
- **Disposition:** Fixable in v1, with owner/env input for live LLM.
...
- **Weighted impact on readiness:** 82 / 119
- **Weighted deficiency signal:** 18
- **Justification:** LLM budgets, cost estimates, caching, ContentHash embedding skip, pricing model, and Azure cost posture are strong. The weakness is that actual production COGS evidence is not visible from current materials.
- **Tradeoffs:** Conservative budget caps can limit output quality if tuned too tightly.
- **Improvement recommendations:** Capture per-run cost traces and compare against pricing assumptions after real pilots.
- **Disposition:** Fixable in v1.
...
- **Score:** 83
- **Weight:** 1
- **Weighted impact on readiness:** 83 / 119
- **Weighted deficiency signal:** 17
- **Justification:** Policy packs, demo workspaces, proof packets, ROI models, order forms, trust templates, and service offers give a rich accelerator base. The weakness is less about inventory and more about which accelerators a first buyer should use first.
- **Tradeoffs:** Richness can become clutter without routing.
- **Improvement recommendations:** Promote a compact starter accelerator path and hide deep catalogs until after first commit.
- **Disposition:** Fixable in v1.
...
- **Score:** 84
- **Weight:** 1
- **Weighted impact on readiness:** 84 / 119
- **Weighted deficiency signal:** 16
- **Justification:** Health/version endpoints, support bundle, diagnostics, troubleshooting, correlation IDs, audit search, runbooks, and redaction work make supportability a strength. Further improvements are mostly triage ergonomics.
- **Tradeoffs:** Support bundles must balance completeness with redaction.
- **Improvement recommendations:** Keep redaction manifest current and add "known-good evidence bundle" examples.
- **Disposition:** Fixable in v1.
...
- **Score:** 88
- **Weight:** 1
- **Weighted impact on readiness:** 88 / 119
- **Weighted deficiency signal:** 12
- **Justification:** Documentation volume and specificity are exceptional: scope, deferrals, API contracts, trust, onboarding, runbooks, architecture, and release checks are all present. The weakness is discoverability, not absence.
- **Tradeoffs:** High documentation depth can increase first-read cost.
- **Improvement recommendations:** Continue centralizing routing through `START_HERE.md`, `V1_SCOPE.md`, and first-pilot operator path.
- **Disposition:** Fixable in v1.

## 4. Top 12 Most Important Weaknesses

1. Real-LLM buyer-grade evidence is not yet fresh enough; available live AOAI evidence was blocked by missing credentials.
2. Commercial proof depends too much on modeled ROI and not enough on tenant-specific before/after evidence.
3. First-pilot path is documented but still cognitively heavy because the repo has many overlapping doors.
4. Adoption still requires meaningful configuration decisions across SQL, auth, LLM, telemetry, and evidence ingestion.
5. AI output trust is stronger in scaffolding than in continuously measured hosted real-mode results.
6. Buyer-facing differentiation needs more proof artifacts and fewer feature-list explanations.
7. Procurement readiness is honest but interim: self-assessment and templates will not satisfy rigid SOC 2/ISO buyers.
8. Workflow embeddedness is adequate for V1 but not yet native in the tools where enterprises live daily.
9. Executive outputs need sharper source-backed "decision and impact" compression.
10. Production-like environment evidence is not yet as easy to package as code-level test evidence.
11. Broad modularity creates maintenance and onboarding load even though the architecture is coherent.
12. Assurance and commercial proof gates rely on owner/business actions that engineering cannot fully complete alone.

## 5. Top 6 Monetization Blockers

1. Insufficient fresh real-run proof that the AI review output is reliable enough to justify paid evaluation.
2. ROI proof is still too template-based unless a pilot captures customer-specific before/after baselines.
3. Sales-led packaging can confuse buyers unless the first purchasable offer and deliverable are obvious.
4. Procurement assurance gaps will slow or block larger enterprises even though they are excluded from `(A)`.
5. Live self-serve commerce / Marketplace publication is deferred, so founder-led conversion must carry revenue.
6. Reference and case-study proof is deferred, so early buyers must rely on demos, trust docs, and founder credibility.

## 6. Top 6 Enterprise Adoption Blockers

1. Security reviewers will ask for SOC 2 CPA / ISO / third-party pen-test evidence that is not yet available.
2. Operators need production-like deployment evidence, not just repository-level CI evidence.
3. Identity and tenant setup are powerful but complex enough to require careful implementation support.
4. Toolchain embedding beyond REST/CLI/UI is incomplete until the V1.1 connector tranche lands.
5. The product has many admin/config knobs; enterprise teams need opinionated profiles and clear PASS/HOLD results.
6. Buyers may struggle to distinguish current V1 guarantees from V1.1/V2 roadmap unless scope boundaries stay prominent.

## 7. Top 6 Engineering Risks

1. Real LLM semantic quality can drift without enough live, credentialed evidence runs.
2. Broad surface area increases cross-contract drift risk across API, generated clients, UI, docs, and CLI.
3. Configuration mistakes can undermine otherwise strong security posture.
4. Cognitive load can cause operators to choose the wrong validation lane or overtrust mock/simulator evidence.
5. AI cost and faithfulness controls must remain durable across replicas and environments.
6. Legacy-compatible paths and terminology can obscure the canonical authority pipeline and review semantics.

## 8. Most Important Truth

ArchLucid is closest to being blocked by proof, not by product breadth: the system already contains most of the right pieces, but buyers will pay only when the first live review produces a source-backed, trustworthy, sponsor-ready result quickly.

## 9. Top Improvement Opportunities

### 1. COMPLETED - Capture Fresh Real-LLM Golden Cohort Evidence

- **Why it matters:** This is the highest-leverage proof gap for AI trust and commercial confidence.
- **Expected impact:** Converts "the harness exists" into "the product produced credible real outputs."
- **Affected qualities:** AI/Agent Readiness, Cutting-Edge AI Technology, Correctness, Trustworthiness, Marketability.
- **Actionable now or DEFERRED:** **COMPLETED** (2026-05-29).
- **Evidence:** `artifacts/release/real-llm-evidence-gate.md`, `artifacts/release/real-llm-last-run-metrics.json`, [REAL_LLM_SESSION_2026-05-29.md](../quality/REAL_LLM_SESSION_2026-05-29.md). Live topology smoke against `oai-archlucid-dev` / `gpt-4o`; credentials in gitignored `secrets/local-real-aoai.env`.
- **Follow-up:** Full quad-agent pipeline merge on live output; GitHub golden-cohort nightly secrets (TB-007 Gap A); token usage surfacing on trace recorder.

### 2. COMPLETED - Build a First-Pilot Proof Packet Generator

- **Why it matters:** Buyers need one artifact that says what passed, what ran live, what was simulated, and what evidence supports the first review.
- **Expected impact:** Directly improves Time-to-Value (+4-6 pts), Adoption Friction (+3-5 pts), Procurement Readiness (+3-4 pts), Marketability (+2-4 pts). Weighted readiness impact: +0.5-0.8%.
- **Affected qualities:** Time-to-Value, Adoption Friction, Procurement Readiness, Executive Value Visibility, Supportability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Implement a first-pilot proof packet generator. Add a CLI command or script wrapper that gathers existing artifacts from a completed review and writes a buyer-safe folder containing: `proof-summary.md`, `environment.json` with redacted mode/config facts, `run-evidence.json`, `audit-sample.json`, `artifact-manifest.json`, and `limitations.md`. Prefer reusing existing support bundle, release smoke result, audit search, version, and first-value report code instead of duplicating logic. Update `docs/runbooks/FIRST_PILOT_OPERATOR_PATH.md` and `docs/library/V1_READINESS_SUMMARY.md` to point to the packet. Acceptance criteria: generated packet never includes secrets; explicitly labels simulator vs real mode; includes run id, manifest id, version, correlation id examples, and skipped gates; has unit tests for redaction and output shape. Constraints: do not change product scoring, do not add new external services, do not weaken existing support-bundle redaction.

### 3. COMPLETED - Add Citation-Backed ROI Evidence to Sponsor Outputs

- **Why it matters:** ROI proof is persuasive only when every savings claim points to source data or a declared assumption.
- **Expected impact:** Directly improves Proof-of-ROI Readiness (+6-8 pts), Executive Value Visibility (+3-5 pts), Trustworthiness (+2-3 pts). Weighted readiness impact: +0.4-0.6%.
- **Affected qualities:** Proof-of-ROI Readiness, Executive Value Visibility, Correctness, Trustworthiness.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Extend sponsor/first-value ROI outputs so each dollar or time-savings line carries a source classification: `CustomerProvided`, `ExtractorZip`, `RetailPriceRow`, `BenchmarkAssumption`, or `NotEstimated`. Update the first-value report and executive ROI summary path to display the source classification and citation details where available. Add tests proving unsupported ROI/cost claims are labeled as assumptions rather than facts. Start with existing ROI/report services and docs under `docs/go-to-market/ROI_MODEL.md` and pilot report endpoints; do not invent new pricing numbers. Acceptance criteria: no sponsor-facing ROI amount is emitted without source metadata; existing reports remain backward compatible; docs explain how to interpret assumptions. Constraints: do not alter pricing source of truth, do not claim invoiced Azure cost from estimates.

### 4. COMPLETED - Create a 20-Minute First Value Operator Path

- **Why it matters:** Current documentation is rich but broad; first value should feel obvious.
- **Expected impact:** Directly improves Time-to-Value (+3-5 pts), Cognitive Load (+6-8 pts), Adoption Friction (+2-4 pts). Weighted readiness impact: +0.3-0.5%.
- **Affected qualities:** Time-to-Value, Cognitive Load, Usability, Adoption Friction.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Create or update a concise `docs/runbooks/FIRST_VALUE_20_MINUTES.md` that is explicitly the shortest operator path from evidence to sponsor artifact. It must include prerequisites, exact commands or UI actions, expected outputs, failure triage, and links to deeper docs only after the main path. Update `docs/START_HERE.md` and `docs/runbooks/FIRST_PILOT_OPERATOR_PATH.md` to make this the preferred first read for time-boxed evaluators. Acceptance criteria: one page, no duplicate checklist that conflicts with existing canonical path, labels simulator/real expectations, and points to support bundle/proof packet generation. Constraints: do not remove existing depth docs; do not imply live LLM or production SLA proof unless actually run.

### 5. COMPLETED - Add Production-Like Config Evidence Snapshots

- **Why it matters:** Enterprise operators need proof that a staged environment is configured safely, not just code that can be configured safely.
- **Expected impact:** Directly improves Security (+3-5 pts), Deployability (+4-6 pts), Procurement Readiness (+2-3 pts), Reliability (+2-3 pts). Weighted readiness impact: +0.25-0.45%.
- **Affected qualities:** Security, Deployability, Manageability, Procurement Readiness, Reliability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Extend the existing config lint/profile output so production-like hosted pilot runs can emit a buyer-safe Markdown and JSON evidence snapshot. Include auth mode, storage topology, telemetry export disposition, LLM mode, content safety disposition, billing safety disposition, and redacted secret presence only. Update `CONFIGURATION_REFERENCE.md` and first-pilot runbooks with the exact command. Add tests for secret masking and PASS/WARN/HOLD mapping. Constraints: do not print raw connection strings, API keys, SAML material, or Key Vault secret values; do not make optional V1.1 features blocking.

### 6. COMPLETED - Strengthen Real-vs-Simulator Labels in Sponsor-Facing Artifacts

- **Why it matters:** Trust collapses if a buyer mistakes simulator output for real model output.
- **Expected impact:** Directly improves Trustworthiness (+4-5 pts), AI/Agent Readiness (+2-3 pts), Correctness (+1-2 pts). Weighted readiness impact: +0.25-0.4%.
- **Affected qualities:** Trustworthiness, AI/Agent Readiness, Correctness, Explainability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Audit sponsor-facing exports, first-value reports, executive summaries, review detail UI, and proof packet docs for structural execution mode labeling. Ensure every buyer-visible artifact clearly states `Real`, `Simulator`, `Fallback`, or `Mixed` where the data supports it. Add tests for at least one API response, one UI component, and one export/report path. Constraints: do not change run semantics; do not hide simulator mode; do not add ambiguous labels like "AI-powered" without execution mode context.

### 7. COMPLETED - Add Buyer-Safe AI Evidence Appendix

- **Why it matters:** AI quality needs a concise, shareable evidence story, not only scattered eval scripts.
- **Expected impact:** Directly improves Cutting-Edge AI Technology (+3-4 pts), AI/Agent Readiness (+3-4 pts), Marketability (+2-3 pts). Weighted readiness impact: +0.3-0.5%.
- **Affected qualities:** Cutting-Edge AI Technology, AI/Agent Readiness, Marketability, Trustworthiness.
- **Actionable now or DEFERRED:** Partially actionable now.
- **Cursor prompt:** Create `docs/go-to-market/AI_EVIDENCE_APPENDIX.md` as a buyer-safe index of current AI evidence: simulator coverage, RAG faithfulness report, retrieval IR harness, quality gates, budget controls, content safety, and the known status of live real-LLM evidence. Link to existing internal runbooks but keep the appendix concise. Acceptance criteria: explicitly says which evidence is offline/exemplar vs live; includes no secrets or raw prompts; does not claim third-party validation; links from Trust Center or buyer orientation only where appropriate. Constraints: do not inflate real-mode claims; do not reference prior assessment scores.

### 8. COMPLETED - Tighten First-Pilot Error Recovery UX

- **Why it matters:** Failed setup is a commercial event; fast recovery preserves buyer confidence.
- **Expected impact:** Directly improves Usability (+3-5 pts), Adoption Friction (+2-4 pts), Supportability (+2-3 pts). Weighted readiness impact: +0.2-0.35%.
- **Affected qualities:** Usability, Adoption Friction, Supportability, Time-to-Value.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Review the operator first-pilot UI and API error handling for the create/execute/commit/artifact path. Add or improve actionable `supportHint` text, UI empty/error states, and troubleshooting links for common failures: SQL not ready, auth missing, trial/quota exhausted, no artifacts, run not found, quality rejected, and LLM budget exhausted. Add tests for representative problem details and UI rendering. Constraints: do not expose stack traces or secrets; do not suppress existing problem types; do not change HTTP status contracts without updating OpenAPI snapshots and clients.

### 9. COMPLETED - Build a Compact Commercial Decision Packet

- **Why it matters:** Founder-led sales needs one packet that moves a buyer from demo to paid next step.
- **Expected impact:** Directly improves Decision Velocity (+6-8 pts), Commercial Packaging Readiness (+3-5 pts), Marketability (+2-3 pts). Weighted readiness impact: +0.2-0.35%.
- **Affected qualities:** Decision Velocity, Commercial Packaging Readiness, Marketability, Proof-of-ROI Readiness.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Create `docs/go-to-market/COMMERCIAL_DECISION_PACKET.md` that combines the shortest buyer-facing next step: offer menu, pilot deliverables, pricing source links, ROI proof requirements, trust-pack link, and procurement objections. Keep prices linked to `PRICING_PHILOSOPHY.md` instead of duplicating numbers unless already permitted. Acceptance criteria: one clear "recommended first purchase" path for founder-led motion; explicitly separates SaaS subscription from service-led review offers; includes what proof the buyer receives at the end. Constraints: do not change official list prices; do not imply Marketplace/live Stripe checkout is available if not enabled.

### 10. COMPLETED - Add Cross-Surface Sponsor Artifact Consistency Tests

- **Why it matters:** The same run should not tell different stories in UI, PDF, DOCX, API, and CLI.
- **Expected impact:** Directly improves Correctness (+2-4 pts), Executive Value Visibility (+2-3 pts), Trustworthiness (+2-3 pts). Weighted readiness impact: +0.25-0.4%.
- **Affected qualities:** Correctness, Executive Value Visibility, Testability, Documentation.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Add focused tests that generate or inspect the same committed review through API, CLI first-value report, sponsor PDF/DOCX path, and operator UI summary fixture. Assert consistent run id/review id, manifest id, execution mode, findings counts, and top ROI/source labels. Reuse existing fixtures and live/smoke helpers where possible. Constraints: keep scope narrow; do not require live Azure OpenAI; do not rewrite report generation architecture unless tests reveal a concrete bug.

### 11. COMPLETED - Reduce First-Pilot Documentation Duplication

- **Why it matters:** Too many equivalent entry points increase evaluator confusion.
- **Expected impact:** Directly improves Cognitive Load (+6-8 pts), Time-to-Value (+2-3 pts), Customer Self-Sufficiency (+2-3 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Cognitive Load, Documentation, Time-to-Value, Customer Self-Sufficiency.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Audit first-pilot docs linked from `docs/START_HERE.md`, `CORE_PILOT.md`, `FIRST_PILOT_OPERATOR_PATH.md`, `EVALUATION_GUIDE.md`, and related runbooks. Make one file the canonical checklist and mark others as narrative, depth, or troubleshooting. Add short "read this only if..." labels. Acceptance criteria: no conflicting step order; links point to the canonical checklist; depth docs are preserved. Constraints: do not delete useful content; do not change product scope.

### 12. COMPLETED - Add Retrieval and Faithfulness Trend Artifact to CI

- **Why it matters:** AI faithfulness should be tracked over time, not rediscovered during assessments.
- **Expected impact:** Directly improves AI/Agent Readiness (+3-4 pts), Cutting-Edge AI Technology (+2-3 pts), Reliability (+1-2 pts). Weighted readiness impact: +0.25-0.4%.
- **Affected qualities:** AI/Agent Readiness, Cutting-Edge AI Technology, Reliability, Testability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Wire the existing retrieval IR and RAG faithfulness evaluators into a CI artifact or documented local command that emits Markdown and JSON trend outputs. If enforcement is too noisy, start as warn-only with explicit floors and artifact upload. Update `AGENT_OUTPUT_EVALUATION.md` and `RAG_QUALITY_TECHNICAL_BACKLOG.md` with the current command. Acceptance criteria: per-corpus recall/MRR and support-ratio metrics are visible in one generated report; report distinguishes retrieval quality from output citation faithfulness. Constraints: do not require live LLM; do not make unstable metrics merge-blocking without owner approval.

### 13. COMPLETED - Improve Support Bundle Triage Index

- **Why it matters:** Supportability is strong, but first-line triage should be faster.
- **Expected impact:** Directly improves Supportability (+3-5 pts), Reliability (+1-2 pts), Adoption Friction (+1-2 pts). Weighted readiness impact: +0.1-0.2%.
- **Affected qualities:** Supportability, Reliability, Adoption Friction, Manageability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Extend the support bundle manifest/readme/triage index so operators can quickly see environment mode, execution mode, redaction manifest, version, health snapshot, latest failed gate, and relevant correlation ids. Add tests around redaction and manifest shape. Constraints: preserve existing support bundle file names where consumers may depend on them; do not include secrets or raw LLM prompts unless already redacted by existing policy.

### 14. COMPLETED - Add Proof Packet Acceptance to Release Checklist

- **Why it matters:** Release readiness should include buyer-proof output, not only build/test success.
- **Expected impact:** Directly improves Procurement Readiness (+2-3 pts), Time-to-Value (+1-2 pts), Marketability (+1-2 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Procurement Readiness, Time-to-Value, Marketability, Supportability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Update `docs/library/V1_RELEASE_CHECKLIST.md`, `docs/library/RELEASE_SMOKE.md`, and related release docs to include an optional but recommended "buyer proof packet generated and reviewed" gate. Include acceptance criteria for execution mode label, redaction, version, run id, manifest id, audit sample, and skipped-gates section. Constraints: do not make buyer proof generation mandatory for every developer build; keep it as release/handoff evidence.

### 15. COMPLETED - Clarify Current vs V1.1 Integration Story in Buyer Copy

- **Why it matters:** Misunderstanding connector scope can create false objections or false promises.
- **Expected impact:** Directly improves Workflow Embeddedness (+3-4 pts), Interoperability (+2-3 pts), Trustworthiness (+1-2 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Workflow Embeddedness, Interoperability, Trustworthiness, Procurement Readiness.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Review buyer-facing integration docs and ensure current V1 paths are described as REST, CLI, UI, SCIM, Azure extractor, exports, and available CI/workflow surfaces, while V1.1 first-party Jira/ServiceNow/Confluence/Slack/Teams/webhooks are clearly labeled by release window. Acceptance criteria: no buyer-facing table implies deferred connectors are V1 GA; no assessment or proof packet penalizes their absence. Constraints: do not remove V1.1 roadmap detail; do not add new connector commitments.

### 16. COMPLETED - Add Production-Like Auth Handoff Checklist

- **Why it matters:** Auth setup is one of the most likely enterprise adoption friction points.
- **Expected impact:** Directly improves Adoption Friction (+2-4 pts), Security (+2-3 pts), Customer Self-Sufficiency (+2-3 pts). Weighted readiness impact: +0.2-0.3%.
- **Affected qualities:** Adoption Friction, Security, Customer Self-Sufficiency, Usability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Create a concise production-like auth handoff checklist covering Entra/OIDC, SAML SP, API key automation, SCIM, claim mapping, tenant/workspace/project claims, and diagnostics endpoints. Link to existing detailed docs rather than duplicating all setup. Acceptance criteria: includes PASS/HOLD checks; tells operators what evidence to capture; warns against dev bypass in production-like handoff. Constraints: do not add new auth modes; do not include secrets or real tenant IDs in examples.

### 17. COMPLETED - Make Executive Summary Outputs More Decision-Oriented

- **Why it matters:** Executives need "approve / fund / remediate" more than technical trace detail.
- **Expected impact:** Directly improves Executive Value Visibility (+3-5 pts), Decision Velocity (+2-3 pts), Marketability (+1-2 pts). Weighted readiness impact: +0.2-0.3%.
- **Affected qualities:** Executive Value Visibility, Decision Velocity, Marketability, Usability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Update executive/sponsor report templates so the top section always includes: decision requested, top three risks, top three savings/value levers, confidence level, evidence coverage, and next action owner. Reuse existing report models where possible. Acceptance criteria: generated sample or test fixture shows the new structure; detailed trace sections remain available lower in the artifact. Constraints: do not remove technical appendices; do not invent ROI numbers.

### 18. COMPLETED - Add "What Not to Promise" Guardrails to GTM Pack

- **Why it matters:** Overpromising deferred assurance or integration scope creates procurement and trust failures.
- **Expected impact:** Directly improves Trustworthiness (+2-3 pts), Procurement Readiness (+2-3 pts), Marketability (+1-2 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Trustworthiness, Procurement Readiness, Marketability, Compliance Readiness.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Add a concise `docs/go-to-market/WHAT_NOT_TO_PROMISE.md` or equivalent section in the GTM playbook. Include clear statements for SOC 2 CPA, third-party pen test, live Marketplace/Stripe, design partner/reference status, MCP/plugin marketplace, multi-region active/active, and V1.1 connectors. Acceptance criteria: each item links to the canonical scope/deferred doc; wording separates `(A)` product readiness from `(B)` procurement realism. Constraints: do not weaken existing trust posture; do not hide roadmap items.

### 19. COMPLETED - Add Cost Trace Review to Pilot Retro

- **Why it matters:** Cost-effectiveness and pricing confidence require observed per-run economics.
- **Expected impact:** Directly improves Cost-Effectiveness (+3-5 pts), Proof-of-ROI Readiness (+1-2 pts), Manageability (+1-2 pts). Weighted readiness impact: +0.1-0.2%.
- **Affected qualities:** Cost-Effectiveness, Proof-of-ROI Readiness, Manageability, AI/Agent Readiness.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Add a pilot retro section and optional script/report that summarizes per-run LLM token estimates, estimated USD, budget warnings, cache/retrieval activity, and pricing assumption deltas. Link it from ROI and pilot success docs. Acceptance criteria: output is tenant-scoped, no secrets, and clearly labels estimates as estimates. Constraints: do not treat estimated Azure OpenAI cost as invoice truth; do not alter billing logic.

### 20. COMPLETED - Strengthen OpenAPI / Client / UI Drift Guidance

- **Why it matters:** Integration trust depends on keeping API, generated client, UI types, and docs synchronized.
- **Expected impact:** Directly improves Correctness (+1-2 pts), Interoperability (+2-3 pts), Maintainability (+1-2 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Correctness, Interoperability, Maintainability, Testability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Review current API contract change guidance and CI guards. Add a compact contributor checklist that ties route/DTO changes to OpenAPI snapshot, generated .NET client, UI API types, docs, and audit matrix updates. Acceptance criteria: checklist lives near `API_CONTRACTS.md` or change impact docs; includes exact commands; avoids duplicate conflicting instructions. Constraints: do not regenerate clients unless needed for actual drift.

### 21. COMPLETED - Add First-Run Evidence Fixture for Sales Demos

- **Why it matters:** Sales demos should not depend on live improvisation.
- **Expected impact:** Directly improves Marketability (+2-3 pts), Time-to-Value (+1-2 pts), Differentiability (+1-2 pts). Weighted readiness impact: +0.15-0.25%.
- **Affected qualities:** Marketability, Time-to-Value, Differentiability, Usability.
- **Actionable now or DEFERRED:** Fully actionable now.
- **Cursor prompt:** Create a curated first-run evidence fixture and demo script for one narrow buyer scenario, using existing demo workspace and proof packet patterns. Include expected findings, artifact names, sponsor narrative, and failure fallback. Acceptance criteria: demo can run in simulator mode without external credentials; copy clearly says simulator where applicable; fixture does not include customer data. Constraints: do not add fake claims that could be mistaken for real customer outcomes.

### 22. DEFERRED - Publish a Named Reference Customer Proof Row

- **Why it matters:** Reference proof materially improves marketability and procurement confidence.
- **Expected impact:** Improves Marketability, Differentiability, Proof-of-ROI Readiness, Decision Velocity; `(B)` procurement realism strongly improves, while `(A)` should not be penalized before this deferred item.
- **Affected qualities:** Marketability, Proof-of-ROI Readiness, Differentiability, Procurement Readiness.
- **Actionable now or DEFERRED:** DEFERRED.
- **Reason deferred:** Requires a real customer, permission to publish, approved logo/case study, and commercial/legal approval.
- **Input needed later:** Customer name, permission scope, approved metrics, logo/case-study approval status, and any NDA constraints.

### 23. V1.1 BACKLOG — SOC 2 CPA Attestation Program (TB-135)

- **Why it matters:** It will unblock larger enterprise procurement even though it is excluded from `(A)`.
- **Expected impact:** Improves `(B)` procurement realism materially; also supports Compliance Readiness and Trustworthiness narratives.
- **Affected qualities:** Compliance Readiness, Procurement Readiness, Trustworthiness.
- **Actionable now or DEFERRED:** **V1.1 backlog** — not a V1 assessment or engineering batch item.
- **Tracking:** [`TECH_BACKLOG.md`](../library/TECH_BACKLOG.md) **TB-135**; **`.cursor/rules/V1_1-assurance-backlog.mdc`**. Pick up only when owner directs.
- **Input needed later:** Budget ceiling, desired SOC 2 scope, auditor/consultant preference, observation window, target customer requirements.

### 24. DEFERRED - Unhold Live Commerce and Marketplace Publication

- **Why it matters:** Self-serve revenue and procurement channel velocity improve once Stripe live keys, Marketplace publication, payout/tax profile, and DNS are complete.
- **Expected impact:** Improves Decision Velocity, Commercial Packaging Readiness, Adoption Friction, and monetization realism; not a current `(A)` deduction where deferred.
- **Affected qualities:** Decision Velocity, Commercial Packaging Readiness, Adoption Friction, Marketability.
- **Actionable now or DEFERRED:** DEFERRED.
- **Reason deferred:** Owner-only Partner Center, payout, tax, domain/DNS, and Stripe live configuration steps are required.
- **Input needed later:** Stripe live readiness, Partner Center seller verification status, Marketplace offer status, payout/tax completion, DNS target, and go-live approval.

### 25. V1.1 BACKLOG — Third-Party Pen-Test Program (TB-136)

- **Why it matters:** Some security reviewers will not accept owner-conducted testing as enough.
- **Expected impact:** Improves `(B)` procurement realism, Trustworthiness, Compliance Readiness, and enterprise adoption.
- **Affected qualities:** Trustworthiness, Compliance Readiness, Procurement Readiness, Security.
- **Actionable now or DEFERRED:** **V1.1 backlog** — not a V1 assessment or engineering batch item. V1 owner-conducted testing remains **TB-005**.
- **Tracking:** [`TECH_BACKLOG.md`](../library/TECH_BACKLOG.md) **TB-136**; **`.cursor/rules/V1_1-assurance-backlog.mdc`**. Pick up only when owner directs.
- **Input needed later:** Vendor shortlist/selection, budget, scope, timing, NDA/public summary policy, and remediation SLA expectations.

## 10. Prompt Batching Guidance

Batch 1 - buyer proof and first value: Improvements 2, 4, 6, 14, and 21. These share first-pilot artifacts, execution-mode labeling, and proof packet routing.

Batch 2 - ROI and executive conversion: Improvements 3, 9, 17, and 19. These share sponsor outputs, ROI/cost source labels, and commercial decision materials.

Batch 3 - enterprise handoff and procurement honesty: Improvements 5, 7, 15, 16, and 18. These share trust, config evidence, integration scope, and "what not to promise" language.

Batch 4 - engineering drift and supportability: Improvements 8, 10, 12, 13, and 20. These touch tests, support bundles, API/client drift, UI errors, and CI artifacts.

Batch 5 - owner-blocked commercial actions: Improvements **22 and 24** only.

## 11. Pending Questions for Later

### COMPLETED - Real-LLM golden cohort evidence (2026-05-29)

Resolved for local/dev capture: deployment **`gpt-4o`**, resource **`oai-archlucid-dev`**, classic endpoint **`https://oai-archlucid-dev.openai.azure.com/`**, credentials in gitignored **`secrets/local-real-aoai.env`**. Evidence: **`docs/quality/REAL_LLM_SESSION_2026-05-29.md`**. GitHub nightly cohort secrets remain TB-007 Gap A.

### DEFERRED - Publish a Named Reference Customer Proof Row

- Which customer can be named publicly?
- What metrics, logo use, and quotes are approved?
- Is the case study public, NDA-only, or sales-confidential?

### V1.1 BACKLOG — SOC 2 CPA Attestation Program (TB-135)

Parked on V1.1 backlog — not an assessment implementation prompt. See **`.cursor/rules/V1_1-assurance-backlog.mdc`**.

- What is the budget ceiling for readiness consultant and CPA?
- What Trust Services Criteria scope is intended for the first report?
- What observation window and target date are commercially necessary?

### DEFERRED - Unhold Live Commerce and Marketplace Publication

- Is Stripe live mode approved for production use?
- Is Azure Marketplace seller verification, payout, and tax profile complete?
- What DNS target should `signup.archlucid.net` use?

### V1.1 BACKLOG — Third-Party Pen-Test Program (TB-136)

Parked on V1.1 backlog — not an assessment implementation prompt. See **`.cursor/rules/V1_1-assurance-backlog.mdc`**.

- Which vendor should be selected?
- What scope is in the first engagement?
- Will a redacted summary be public, NDA-only, or internal-only?

## 12. Implementation Update and Rescore (2026-05-29)

### Completed in this pass

- **Improvement #1 (real-LLM evidence):** live topology smoke captured 2026-05-29 — see `docs/quality/REAL_LLM_SESSION_2026-05-29.md`.
- **Improvements 2–21 (engineering batches 1–4):** implemented unless listed as deferred below.
- **CLI `archlucid pilot proof-packet`:** buyer-safe folder (`proof-summary.md`, `environment.json`, `run-evidence.json`, `audit-sample.json`, `artifact-manifest.json`, `limitations.md`).
- **ROI source classification** in first-value reports (`RoiMetricSourceKind` per metric line).
- **Support bundle triage index:** redaction status, structural execution mode, latest failed gate hint.
- **Docs:** `FIRST_VALUE_20_MINUTES.md`, `AI_EVIDENCE_APPENDIX.md`, `COMMERCIAL_DECISION_PACKET.md`, `WHAT_NOT_TO_PROMISE.md`, auth handoff checklist, cost trace retro, demo script.
- **CI:** `scripts/ci/Invoke-FaithfulnessTrendReport.ps1`.
- **Tests:** ROI source + cross-surface sponsor consistency + support bundle triage.

### Still deferred (owner/customer input)

- **Improvements 22, 24:** reference customer, live commerce/Marketplace.

### V1.1 backlog (not assessment batches)

- **Improvements 23, 25 → TB-135, TB-136:** SOC 2 CPA program, third-party pen-test program. See **`.cursor/rules/V1_1-assurance-backlog.mdc`**.

### Rescore note

Headline `(A)` readiness: **81.20%** (9663 / 119), up from 79.46%. Remaining drag is live LLM proof and repeated buyer-specific pilot evidence—not missing V1 scope.

> **Superseded appendix:** The content below is an older duplicate assessment snapshot retained temporarily; sections 1–12 above are authoritative (`81.20%` headline).

<!--ASSESSMENT_DUPLICATE_REMOVED-->
