> **Scope:** ArchLucid Assessment – (A) Headline Readiness: 86.35% - full detail, tables, and links in the sections below.

# ArchLucid Assessment – (A) Headline Readiness: 86.35%

This score represents the `(A)` headline readiness per `Assessment-Scope-V1_1.mdc`, excluding deferred items. Prior scores were revised after Tier 1 (improvements 1–5), Tier 2 security batches (6–7, 9, 12), TB-033 trace forensic completeness, TB-114 accelerator chooser (partial), and Cosmos graph snapshot tenant metadata (2026-05-31).
...
Weighted calculation: total weighted points = `10,275 / 11,900 = 86.35%`.
...
- **Tier 2 closure (2026-05-31):** Improvements **6** (TB-073 SQL/in-memory/Cosmos scope + TB-074), **7** (TB-082), **9** partial (TB-075), **10** (TB-033), **11** partial (TB-114), **12** (TB-079). Remaining: TB-075 SSR/session scope, Improvement **8** `terraform-private` parity, TB-115–117 pack validation harness.
...
ArchLucid is a credible V1 pilot-ready architecture proof system. Tier 1 and Tier 2 batches closed the highest-leverage P0/P1 gaps including trace forensic completeness (TB-033), accelerator chooser (TB-114 partial), and Cosmos graph snapshot tenant metadata (TB-073). The weighted score is now held down primarily by `terraform-private` IaC parity (TB-091–092), UI proxy SSR/session scope remainder (TB-075), starter-pack validation harness (TB-115–117), and client-side run-savings heuristics outside the executive rollup.
...
|---:|---|---:|---:|---:|---:|---|
| 1 | Marketability | 83 | 8 | 5.58% | 136 | Service-led motion, policy packs, claim checklist, and SOW template are in place. Run detail and executive KPI authority improvements reduce demo friction; proof density for broadest claims remains the binding constraint. |
| 2 | Cutting-Edge AI Technology | 84 | 8 | 5.64% | 128 | Multi-agent execution, RAG, grounding traces, eval harnesses, and Azure OpenAI support are real; credentialed full-pipeline gate PASS (TB-138). Not yet an advanced autonomous/agentic or continuously validated AI platform at production scale. |
| 3 | Adoption Friction | 84 | 6 | 4.24% | 96 | Hosted SaaS path, platform-provisioned AOAI, in-app help, path chooser, and accelerator chooser card reduce first-10-minute friction (TB-114 partial **Done**). Configuration branching still creates enterprise ambiguity. |
...
| 5 | Correctness | 92 | 8 | 6.19% | 64 | Run detail enricher + UI close TB-106–108; merge normalizer/converter closes improvement #28 gaps; TB-138 PASS. Run-level savings parser heuristics remain on run detail (outside executive rollup). |
...
| 7 | Time-to-Value | 87 | 7 | 5.12% | 91 | Pilot path is clear. Accelerator chooser maps buyer jobs to starter packs on operator home after first commit (TB-114 partial). Repeat-review prompt shortens second-review path. |
...
| 13 | Trustworthiness | 90 | 3 | 2.27% | 30 | Honest trust posture, claim checklist, accessibility disclosure. Retrieval OData scope, auth scope binding, ADO PR escaping, and proxy scope posture reduce latent trust risk (TB-071, TB-072, TB-079, TB-075 partial **Done**). |
...
| 15 | Security | 97 | 3 | 2.45% | 9 | Strong baseline plus TB-071–074, TB-073 SQL/in-memory/Cosmos graph scope, TB-082, TB-079, TB-075 partial. Open: `terraform-private` KV PE (TB-091–092). |
...
| 27 | Azure Compatibility and SaaS Deployment Readiness | 89 | 2 | 1.50% | 22 | Azure-first architecture, container images, Terraform modules. `AzureSearchSdkClient` + scoped delete path close TB-071. TB-091–102 (IaC parity gaps) remain open but non-blocking for pilot. |
...
| 35 | Deployability | 82 | 1 | 0.69% | 18 | Auth behavior contract, Dockerfiles, compose profiles, Terraform modules. Deployment ambiguity remains for auth mode selection. |
...
| 38 | Template and Accelerator Richness | 88 | 1 | 0.74% | 12 | Policy-pack harness, SOW template, `ACCELERATOR_CHOOSER.md`, operator-home chooser card, starter-pack metadata (TB-114 partial **Done**). TB-117 dry-run harness remains backlog. |
...
| 41 | Testability | 89 | 1 | 0.75% | 11 | Strong test tiers, SQL integration, live UI E2E, mutation testing, k6, ZAP, Schemathesis. Tier 2 added scope-binding, ADO escaper, proxy scope, in-memory snapshot scope, and AllowedTools guard tests. Real AI E2E coverage remains environment-gated. |

---

## 4. Top 12 Most Important Weaknesses

1. **`terraform-private` IaC parity gaps (TB-091–092).** `deploy/hosted-prod-terraform` includes Key Vault PE + Secrets User RBAC, but **`infra/terraform-private` still lacks parity**. Azure OpenAI, Redis, AI Search, and ACR remain partially outside Terraform in some roots.

2. **Operator UI proxy scope remainder (TB-075 partial).** Production-like posture ignores client scope headers and decodes JWT bearer scope. **SSR still falls back to dev GUIDs; `/api/auth/me`-driven scope is not wired.**

3. **Starter proof pack validation harness (TB-115–117).** Chooser doc + UI surface shipped (TB-114 partial), but CI static validation and offline dry-run harness for packs remain backlog.

4. **Run-level savings heuristics remain client-side outside the executive rollup.** Executive KPIs are server-authoritative (TB-103–105 **Done**); run detail may still parse savings locally.

5. **Proof density still lags product breadth.** TB-138 closed merge validation; GTM proof packaging for broadest claims remains a commercial risk.

7. **Documentation drift risk grows with surface area.** The product has enough docs that stale claims are now an engineering risk. Bulk-upload cap text, audit-gap wording, auth-mode descriptions, and trust-center posture can drift faster than manual review catches.

8. **Onboarding has too many decision branches.** Enterprise implementers face ambiguous choices across auth (Entra/OIDC/SAML/API key), storage (SQL topology mode), execution mode (simulator/real), retrieval provider (in-memory/Azure Search), and deployment profile.

9. **Decisioning explainability gaps (TB-050–056).** Operators cannot fully trace manifest decisions from inputs through rules/prompts to confident outputs on every surface; run detail visibility is fixed but deeper explainability fields remain backlog.

10. **Recurring review workflow is not shipped (TB-057–063).** Compare/replay/drift exist, but the monitored-risk / digest cadence that drives stickiness is still backlog.

11. **Accelerator chooser and proof dry-run harness missing (TB-114, TB-117).** First-10-minute buyer experience and repeatable proof activation remain weaker than the underlying engine.

12. **Dependency graph / layering violations (TB-027–032).** Documented architectural debt; non-blocking for pilot but increases maintainability drift pressure.
...
**Status:** **Partial** (2026-05-31). `deploy/hosted-prod-terraform/keyvault_private_endpoints.tf` ships Key Vault PE + Secrets User RBAC. **`infra/terraform-private` and container-apps roots still need parity** (TB-091–092). Review separately before apply.
...
**Status:** **Partial** (2026-05-31). Evidence: `proxy-scope-resolution.ts` (production ignores client scope headers), `proxy-bearer-scope.ts` (JWT claim scope), `proxy-scope-resolution.test.ts`, `proxy-bearer-scope.test.ts`. **Remainder:** SSR dev GUID hardening, `/api/auth/me`-driven scope, env var documentation.
...
**Status:** **Partial** (2026-05-31). Evidence: `docs/library/ACCELERATOR_CHOOSER.md`, `AcceleratorChooserCard` on operator home (post-commit gate), `accelerator-chooser.ts`, help registry slug `accelerator-chooser`. **Remainder:** wizard preset deep-link per pack id, TB-115–117 validation harness.
