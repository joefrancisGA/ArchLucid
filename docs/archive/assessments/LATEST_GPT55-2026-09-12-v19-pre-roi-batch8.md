...
**Pass date:** 2026-09-12, **04:30 UTC**. **Computed fresh** — no carry-forward ratchet; scores recomputed from evidence on this HEAD (prior pass archived for history only).

**Prior rolling snapshot archived:** `docs/archive/assessments/LATEST_GPT55-2026-09-12-v18-pre-roi-batch7.md` (history only; not used for scores).

**Reasoning engine:** Cursor Composer 2.5, code-and-doc desk review. **No live Azure OpenAI call this pass.** Scoped unit tests executed (Vitest classification/treatment delta + spine band + sendable cover; Python RC evidence + batch-6/7 drift guards; C# sendable-cover composer). Real-mode first-review was **not** re-run.

**Inspected HEAD:** `cursor/arch-quality-roi-improvements-0d83` **`c2a7627497`** (ROI batch **7**: dual-channel honesty on spine audit export + server audit CSV; classification `showReason` closure on Working/inspect surfaces; first-review spine semantic counts + WK-21; compare **treatment** band delta; RC `simulator-live-divergence-summary.json` alias; sendable-cover ROI non-summing line; compare-journey + release-gate witnesses).

### Pass delta vs 2026-09-12 pre-batch-7 (`LATEST_GPT55` v18 @ 79.20% → this HEAD)

| Area | Prior | This pass | Evidence |
|---|---|---|---|
| **(A) headline** | **79.20%** | **79.82%** (+0.62 pp) | ROI batch **7**: dual-channel audit honesty on spine export + server CSV; classification `showReason` closure; first-review spine semantic counts + WK-21; compare treatment band delta; RC divergence filename alias; sendable-cover ROI non-summing line; batch-7 drift guards + e2e witnesses |
| Insight Density | 77 | **78** | Classification `showReason` on all Working decision-grade surfaces + inspect body |
| Differentiability | 82 | **82** | unchanged |
| Governed Review Integrity | 85 | **86** | Dual-channel honesty on package-spine run-scoped audit export + server audit CSV preamble |
| Correctness & Evidence | 83 | **84** | Compare-two-reviews **treatment** band delta panel beside classification + semantic deltas |
| AI / Agent Readiness | 78 | **78** | unchanged |
| Time-to-Value | 73 | **73** | unchanged |
| Proof-of-ROI | 80 | **81** | Sendable export cover ROI non-summing headline line (UI + C# composer parity) |
| Comprehension | 74 | **75** | First-review spine semantic support counts + WK-21 chip; classification `showReason` closure |
| Runtime reliability | 80 | **81** | RC evidence index resolves `simulator-live-divergence-summary.json` alias |
...
**ArchLucid Assessment – (A) Headline Readiness: 79.82%**

Readiness **excludes** deferred items per `V1_SCOPE.md` §3, `V1_DEFERRED.md` §6*, and `.cursor/rules/Assessment-Scope-V1_1.mdc`. Reasoning engine: **desk review** (scoped unit tests this pass; **not** a fresh Real-mode Azure OpenAI run). Timestamp: **2026-09-12 04:30 UTC**.
...
**This pass runtime:** Vitest classification/treatment delta + spine band + sendable cover → **15 passed**; Python `test_release_readiness_evidence_script.py` + `test_roi_batch6_drift_guards.py` + `test_roi_batch7_drift_guards.py` + `test_build_rc_evidence_index.py` → **OK**; C# `SendableExportCoverComposerTests` → **3 passed**.
...
|---|---------|------:|-------:|----------------------:|---------------------------:|
| 1 | Decision-Changing Insight Density | 78 | 13 | 10.14 | 286 |
...
| | **(A) Headline Readiness** | | **100** | **79.82** | |
...
**Decision Advantage Score: 63 / 100.** ArchLucid can change a decision a skilled architect using frontier AI would not operationalize — when a policy pack assignment, pre-commit gate, disposition, or citation-backed cost line is in play. ROI batches **5A–7** extend **showReason** semantic-band honesty, compare classification/semantic/treatment deltas, dual-channel audit disclosure, and governance-queue visibility but did **not** add live customer proof. Tension with headline: 79.82% says the **product contract is largely built**; 63 says **decision change in the wild is still unproven** (G4 = 0 of 3). That is consistent: `(A)` scores in-contract engineering, not paid pilots.
...
### 7.1 Decision-Changing Insight Density — Score 78 · Weight 13 · Contribution 10.14 · Deficiency 286

**Outcomes:** 1 (insight), 3 (usage), 5 (survivability).

**Justification:** Typed engines + insight-density gate (ADR 0070) demote generic/unanchored advice instead of deleting it. Golden distribution shows 28 emitting engines on the recorded corpus with **zero** “no evidence / no anchor” cells. **ROI batches 1–6** add first-class **classification/treatment/semanticSupportBand** on inspect API and buyer/Working chips with **`showReason`** on primary surfaces. **Batch 7** closes the **`showReason`** gap on **all Working decision-grade surfaces** (primary/secondary/dense table/governance queue/header) plus **inspect body** — demotion semantics no longer hide on any buyer-facing classification chip. Live customer “I did not think of that” is still **G4-empty**.
...
### 7.4 Sponsor / Operator Comprehension — Score 75 · Weight 8 · Contribution 6.00 · Deficiency 200

**Outcomes:** 3, 4.

**Justification:** Product-language cleanup is extensive. **ROI batches** add first-review spine band, sendable export cover fields (policy pack, gate outcome, execution mode), and classification/semantic-band chips on buyer primary cards. **Batch 6** adds compare classification band delta, inspect showReason, and stamp Lane B async copy. **Batch 7** extends the **first-review spine band** with **semantic support counts**, **Lane B copy**, and **`PolicyPackInfluenceHonestyChip`** — compressing pack-influence honesty into the first viewport without founder narration. Remaining load: seats + workspace + AI credits; governance vs approvals vs alerts.
...
### 7.8 Proof-of-ROI Readiness — Score 81 · Weight 9 · Contribution 7.29 · Deficiency 171

**Outcomes:** 4.

**Justification:** `GET /v1/roi/sponsor-summary` + board-pack + disposition-aware headline **exist and are tested**. **Sendable export covers** (MD/DOCX/PDF/one-pager) stamp policy pack, gate outcome, execution mode, and WK-21 policy-influence boundary on sponsor-facing artifacts (ROI batches 1–6). **Batch 7** adds **`SENDABLE_EXPORT_COVER_ROI_NON_SUMMING_LINE`** on UI markdown covers and **`SponsorRoiNonSummingHeadlineLine`** on C# composer — explicit non-summing headline honesty on every sendable artifact. Credibility gap is **no customer packet**, not missing math.
...
### 7.9 Runtime & First-Review Reliability — Score 81 · Weight 7 · Contribution 5.67 · Deficiency 133

**Outcomes:** 3, 7 (exposure).

**Justification:** Happy-path lifecycle tests pass; k6 CI smoke exists; worker/outbox patterns exist. **`release-smoke.ps1` ship-gate hook** and RC signoff **ship-gate-evidence / G-REAL-08 / faithfulness-nightly-warn** gate rows operationalize evidence collection (ROI batches 1–6). **Batch 7** aliases **`simulator-live-divergence-summary.json`** in `Emit-ReleaseReadinessEvidence.ps1` and resolves it in **`build_rc_evidence_index.py`** — closing RC bundle filename drift without manual rename steps. **`@release-gate`** Playwright asserts first-review spine semantic support + WK-21; **compare-journey** witnesses treatment band delta panel. This pass did not run release-smoke or launch-load. Gate 1 UNKNOWN is the honesty tax.
