> **Scope:** ArchLucid Assessment – (A) Headline Readiness: 87.24% - full detail, tables, and links in the sections below.

# ArchLucid Assessment – (A) Headline Readiness: 87.24%

This score represents the `(A)` headline readiness per `Assessment-Scope-V1_1.mdc`, excluding deferred items. Prior scores were revised after Tier 2 closure (87.00%), post-Tier 2 run-detail server-authoritative savings (87.18%, 2026-05-31), then Tier 2 merge to mainline plus demo-only ROI fallback removal (87.24%, 2026-06-01).
...
Weighted calculation: total weighted points = `10,382 / 11,900 = 87.24%`.
...
- **Tier 2 closure (2026-05-31):** Improvements **6–12** — TB-073 (SQL/in-memory/Cosmos), TB-074, TB-082, TB-075, TB-033, TB-114, TB-079, TB-091–092, TB-115–117 — merged to mainline 2026-06-01.
- **Post-Tier 2 ROI parity (2026-05-31/06-01):** Run-detail savings server-authoritative; client artifact heuristics gated to static demo only.
...
ArchLucid is a credible V1 pilot-ready architecture proof system. Tier 2 is merged to mainline; run-detail savings align with the executive ROI resolver (server-authoritative on `RunDetailDto`); client artifact heuristics are demo-only. Remaining gaps: proof density for broadest GTM claims, IaC remainder (TB-093–102), and backlog clusters (layering, explainability, recurring review).
...
The enterprise posture is serious for a small product: tenant database isolation (catalog-per-tenant), typed audit events (78), governance approvals, policy packs, SAML/OIDC/SCIM, procurement pack content, DPA/subprocessor/SLA templates, and Azure-native deployment assumptions are all present. Tier 1 P0 items for retrieval tenant OData filtering and API-key scope binding are closed. Tier 2 closed retrieval write-path validation (TB-074), graph-reuse snapshot scope (TB-073 SQL/in-memory/Cosmos), AllowedTools dispatch guard (TB-082), ADO PR markdown escaping (TB-079), production-like UI proxy scope (TB-075 **Done**), and Key Vault PE + workload RBAC in Terraform (TB-091–092 **Done**). Remaining enterprise absorption work is IaC remainder (TB-093–102) and onboarding decision-branch simplification.

### Engineering Picture

The engineering system is broad and increasingly mature. Tier 1 batches closed run detail enricher/UI gaps, Azure Search scoped query enforcement, auth scope binding middleware, executive KPI server authority, and credentialed multi-agent merge validation. Tier 2 closed retrieval index write-path tenant validation (TB-074), scoped graph snapshot reuse reads (TB-073), AllowedTools runtime enforcer (TB-082), ADO PR sanitizer (TB-079), proxy scope resolution (TB-075), LLM trace sampling params (TB-033), accelerator wizard deep-links (TB-114), and KV RBAC Terraform (TB-091–092). The biggest remaining engineering risks are: (1) IaC remainder (TB-093–102), (2) decisioning explainability (TB-050–056), and (3) layering violations (TB-027–032).
...
| 5 | Correctness | 94 | 8 | 6.32% | 48 | Run detail enricher + UI close TB-106–108; run-detail savings now server-authoritative via `EstimatedUsdSavingsSummary` (same resolver as executive ROI). |
...
| 8 | Proof-of-ROI Readiness | 90 | 5 | 3.78% | 50 | Cross-surface ROI consistency closed: run detail uses server savings only on live tenants; executive orphan/waiver buckets server-authoritative (TB-103–105 **Done**). |
...
| 25 | Data Consistency | 95 | 2 | 1.60% | 10 | ROI source rows and freshness disposition tested. Run-detail and executive surfaces share the tenant-adjusted findings savings resolver; live runs no longer fall back to client artifact heuristics. |
...
1. **Proof density still lags product breadth.** TB-138 closed merge validation; GTM proof packaging for broadest claims remains a commercial risk.

2. **Documentation drift risk grows with surface area.** The product has enough docs that stale claims are now an engineering risk. Bulk-upload cap text, audit-gap wording, auth-mode descriptions, and trust-center posture can drift faster than manual review catches.

3. **Onboarding has too many decision branches.** Enterprise implementers face ambiguous choices across auth (Entra/OIDC/SAML/API key), storage (SQL topology mode), execution mode (simulator/real), retrieval provider (in-memory/Azure Search), and deployment profile.

4. **Decisioning explainability gaps (TB-050–056).** Operators cannot fully trace manifest decisions from inputs through rules/prompts to confident outputs on every surface; run detail visibility is fixed but deeper explainability fields remain backlog.

5. **Recurring review workflow is not shipped (TB-057–063).** Compare/replay/drift exist, but the monitored-risk / digest cadence that drives stickiness is still backlog.

6. **Remaining IaC coverage gaps (TB-093–102).** Azure OpenAI, Redis, Cosmos provisioning, ACR, and diagnostic settings remain partially out-of-band in some deployment roots.

7. **Dependency graph / layering violations (TB-027–032).** Documented architectural debt; non-blocking for pilot but increases maintainability drift pressure.
...
5. **Onboarding decision branches slow self-directed enterprise rollout.** Auth mode, storage topology, execution mode, and retrieval provider choices remain evaluator-facing friction without a guided enterprise preset path.

---

## 6. Top 6 Enterprise Adoption Blockers

1. **P1 snapshot IDOR hardening largely closed (TB-073).** Scoped primary reads, Cosmos graph snapshot tenant filter, and API IDOR tests exist; residual SingleCatalog child-load edge cases remain low priority.

2. **Onboarding has too many decision branches.** Enterprise implementers face ambiguous choices across auth (Entra/OIDC/SAML/API key), storage (SQL topology mode), execution mode (simulator/real), retrieval provider (in-memory/Azure Search), and deployment profile. This is acceptable for controlled pilots but blocks self-directed enterprise onboarding.

3. **IaC parity gaps create audit failures.** An enterprise InfoSec team that audits the Terraform state will find AOAI, Redis, Cosmos, and AI Search partially managed outside the primary deployment roots. Key Vault PE + workload RBAC are now codified (TB-091–092 **Done**); TB-093–102 remainder still contradicts full IaC-first posture.

4. **Support model is founder-operated.** Support policy exists, but larger enterprise adoption will require evidence that triage, escalation, and incident communication work without heroic founder involvement.

5. **Procurement attestations deferred (V1.1 backlog).** CPA SOC 2 and third-party pen test are honestly disclosed and excluded from `(A)`, but remain `(B)` friction for rigid gatekeepers.

---

## 7. Top 6 Engineering Risks

1. **TB-073 (P1): Residual snapshot repository scoping.** Primary scoped reads, Cosmos tenant filter, and API IDOR tests exist; **`GetLatestByContextSnapshotIdAsync` and some child relational loads** remain low-priority SingleCatalog edge cases.

2. **Docs/code drift under surface area pressure.** The system is large enough that a new route, a new agent type, or a new configuration option can appear in code without being reflected in the audit matrix, the route-tier-policy-nav matrix, the configuration reference, or the trust center. CI guards exist but cover only declared anchor points.

3. **IaC parity (TB-093–102).** AOAI, Redis, Cosmos, ACR, and diagnostic settings remain partially out-of-band in some deployment roots. Key Vault reachability + workload RBAC are codified (TB-091–092 **Done**).

4. **Decisioning explainability (TB-050–056).** Operators cannot fully trace manifest decisions from inputs through rules/prompts to confident outputs on every surface.

5. **Layering violations (TB-027–032).** Documented dependency-graph debt increases maintainability drift pressure.

---

## 8. Most Important Truth

ArchLucid has a functioning multi-agent architecture proof pipeline, strong compliance and governance mechanics, and a clear V1 pilot path. **Tier 1 (improvements 1–5) and Tier 2 (improvements 6–12) are closed on mainline.** Run-detail and executive ROI surfaces now share the same server resolver on live tenants; static demo runs retain illustrative KPIs only. The weighted `(A)` score is **87.24%**. The product's most serious **remaining** risks are IaC remainder (TB-093–102), commercial proof density, and backlog clusters (explainability, recurring review, layering) — not fundamental architecture or concept gaps.
