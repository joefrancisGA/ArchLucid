> **Scope:** ArchLucid Assessment – (A) Headline Readiness: 86.08% - full detail, tables, and links in the sections below.

# ArchLucid Assessment – (A) Headline Readiness: 86.08%

This score represents the `(A)` headline readiness per `Assessment-Scope-V1_1.mdc`, excluding deferred items. It is a clean-slate, first-principles assessment based solely on materials currently available in the repository. Prior scores in this document were revised after Tier 1 improvements 1–5 shipped (2026-05-30/31) and Tier 2 security batches 6–7, 9, 12 (2026-05-31).
...
Weighted calculation: total weighted points = `10,233 / 11,900 = 86.08%`.
...
- **Tier 1 closure (2026-05-30/31, merged to `master`):** Improvements **1–5** shipped — run detail operator visibility (TB-106–108), Azure Search OData tenant scope (TB-071), API-key/DevBypass scope binding (TB-072), executive KPI server authority including business-impact buckets (TB-103–105), and credentialed full-pipeline real-LLM validation PASS (TB-138). Evidence: `AuthorityRunDetailOperatorEnricher`, `AzureSearchSdkClient` + `BuildRequiredScopeFilter`, `ScopeIdentityBindingMiddleware`, `ExecutiveBusinessImpactCategoryClassifier`, `artifacts/release/real-llm-evidence-gate.md`.
- **Tier 2 partial closure (2026-05-31):** Improvements **6** (TB-073 graph reuse scope + TB-074 write validation), **7** (TB-082 AllowedTools dispatch guard), **9** (TB-075 proxy scope posture), **12** (TB-079 ADO markdown escaper). Remaining: TB-073 Cosmos metadata, TB-075 SSR `/api/auth/me` scope, Improvement **8** `terraform-private` parity, Improvements **10–11**.
...
ArchLucid is a credible V1 pilot-ready architecture proof system. The current product boundary includes a hosted SaaS posture, a clear Pilot path, an Operate layer, committed manifests and artifacts, governance workflows, policy packs (23 seeded bundles), audit trails, tenant database isolation, SAML/OIDC/SCIM posture, support bundles, and a broad automated test story. Tier 1 improvements closed the highest-leverage P0 gaps; Tier 2 batches closed snapshot graph-reuse scope, retrieval write validation, AllowedTools runtime dispatch, ADO PR markdown escaping, and production-like UI proxy scope posture. The weighted score is now held down primarily by IaC parity gaps in `terraform-private` (TB-091–092), Cosmos snapshot tenant metadata remainder (TB-073), trace forensic completeness (TB-033), accelerator chooser (TB-114), and client-side run-savings heuristics outside the executive rollup.
...
| 3 | Adoption Friction | 82 | 6 | 4.13% | 108 | Hosted SaaS path, platform-provisioned AOAI, in-app help, and path chooser reduce friction meaningfully. Configuration branching (auth modes, storage, execution mode, retrieval) still creates implementation ambiguity for enterprise operators. |
...
| 7 | Time-to-Value | 85 | 7 | 5.00% | 105 | Pilot path is clear. Repeat-review prompt shortens the second-review path. Accelerator chooser (TB-114) would improve first-10-minute experience. |
...
| 15 | Security | 96 | 3 | 2.42% | 12 | Strong baseline plus TB-071 OData scope, TB-072 scope binding, TB-074 retrieval write validation, TB-073 graph reuse scope, TB-082 AllowedTools guard, TB-079 ADO escaping, TB-075 proxy scope (partial). Open: TB-073 Cosmos remainder, `terraform-private` KV PE (TB-091–092). |
...
| 22 | Traceability | 90 | 3 | 2.27% | 30 | Run IDs, manifests, provenance, 78 typed audit events, correlation IDs, trace bundles. Strong. New merge-time schema validation adds traceable validation checkpoints. |
...
| 26 | Explainability | 89 | 2 | 1.50% | 22 | Execution-mode labels, real-agent triage scenario IDs, provenance, decision traces. TB-050–056 (decisioning explainability gaps) are in backlog. |
...
| 37 | Auditability | 90 | 2 | 1.51% | 20 | One of the strongest areas: 78 typed events, append-only SQL, matrix, CSV export, CI guards, semantic audit invariant guard. |
| 38 | Template and Accelerator Richness | 85 | 1 | 0.71% | 15 | Policy-pack harness and SOW template strengthen curated accelerator quality. Accelerator chooser (TB-114) and proof dry-run harness (TB-117) would materially improve time-to-value. |
| 39 | Supportability | 84 | 1 | 0.71% | 16 | Real-agent failure triage matrix, CLI doctor, support bundle. Operator-visible Backfill.Cli output (TB-090) is backlog. |
...
1. **Cosmos graph snapshot tenant metadata remains unscoped (TB-073 remainder).** SQL and in-memory paths now scope `GetLatestByContextSnapshotIdAsync`, list/mutate finding APIs, and finding-record counts via `ScopeContext`. **`CosmosGraphSnapshotRepository` still queries by context snapshot ID without tenant partition metadata**, leaving residual IDOR risk when Cosmos is the graph store. (TB-073, P1)

2. **Operator UI proxy scope is partially hardened (TB-075 partial).** Production-like posture ignores client `x-tenant-*` headers via `resolveProxyUpstreamScopeHeaders` and JWT bearer scope decode (`proxy-bearer-scope.ts`). **SSR still falls back to dev GUIDs when env vars are unset; session `/api/auth/me`-driven scope is not wired.** Remaining P1 work: document `ARCHLUCID_PROXY_*` env vars and replace dev fallbacks in non-dev environments.

3. **IaC parity gaps in `terraform-private` affect production security posture.** `deploy/hosted-prod-terraform` now includes Key Vault private endpoint + Secrets User RBAC, but **`infra/terraform-private` still lacks Key Vault PE/DNS and Container Apps workload RBAC** (TB-091–092). Azure OpenAI, Redis, AI Search, and ACR remain partially outside Terraform in some roots.

4. **Agent execution trace forensic completeness (TB-033).** Temperature, maxTokens, and reasoning token counts are consumed for cost estimation but were not persisted on `AgentExecutionTrace` until the current batch (in progress). Compliance replay still lacks top_p and degraded-path minimal traces (TB-034).

5. **Proof density still lags product breadth.** The system has many credible surfaces but needs real-mode, source-labeled, buyer-safe proof packets for the broadest commercial claims. TB-138 closed the merge-validation gap; packaging proof for GTM remains a commercial risk.

6. **Run-level savings heuristics remain client-side outside the executive rollup.** Executive orphan/waiver/category KPIs are server-authoritative (TB-103–105 **Done**), but run detail and related surfaces may still parse savings locally with different heuristics than the authority pipeline.
...
**Status:** **Mostly done** (2026-05-31). TB-074 **Done** via `RetrievalIndexingScopeValidator` + tests. TB-073 **SQL/in-memory/API paths done** — scoped `GetLatestByContextSnapshotIdAsync`, finding-record count scope, in-memory scoped reads, IDOR integration tests. **Remainder:** Cosmos graph snapshot tenant metadata on documents/partition key.
...
**Status:** **In progress** (2026-05-31). Evidence: `LlmCompletionRequestParamsAmbient`, `AgentCompletionRequestParams`, trace contract fields (`CompletionTemperature`, `MaxCompletionTokens`, `CompletionTopP`, `ReasoningTokenCount`), recorder ambient consumption, `AgentExecutionTraceRecorderSamplingParamsTests`.
...
**Status:** Fully actionable now. Can be batched with Improvement 10.

**Why ranked #11:** Time-to-value improvement; low risk; directly addressable with docs + UI changes.

**Evidence from repo:** TB-114 documents that existing accelerators are useful but not easy to choose. The 23 policy packs are seeded (`DEFAULT_POLICY_PACKS_V1.md`) but have no buyer-job-to-pack mapping.
...
- **Improvement 10** (TB-033 trace sampling + reasoning tokens): In progress — `LlmCompletionRequestParamsAmbient`, trace contract fields, recorder persistence.
- **Improvement 11** (TB-114 accelerator chooser): First-10-minute buyer experience.
- **Improvement 8** (`terraform-private` TB-091–092): Mirror hosted-prod Key Vault PE + workload RBAC into private-network root.
- **Improvement 6 remainder** (TB-073 Cosmos tenant metadata) + **Improvement 9 remainder** (TB-075 SSR/session scope): Parallel P1 security tail.
