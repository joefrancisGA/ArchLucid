> **Scope:** Independent weighted-readiness snapshot variant from direct codebase inspection (2026-04-25) — not a committed roadmap or sales forecast.

# ArchLucid Assessment – Weighted Readiness 62.18%
...
### 2.1. Marketability — Score: 55 | Weight: 8 | Weighted Deficiency: 3.60

**Justification:** The product has a clear positioning statement, executive sponsor brief, competitive landscape document, and go-to-market folder with 20+ files. The "AI Architecture Intelligence platform" positioning is clear and the three-pillar value story is well-grounded in code. **`www.archlucid.net` is live on a branded domain with valid TLS (2026-04-25).** However, the self-serve signup funnel is not functional at the domain. The buyer first-30-minutes document contains `<<placeholder copy>>` markers. Auth is in dev bypass mode. No live pricing page. Stripe checkout is not production-wired.
...
**Tradeoffs:** The branded domain establishes credibility. The remaining v1 gap is wiring the buyer-facing funnel (auth, signup, pricing, Stripe, placeholder copy).

**Recommendations:**
- Switch auth to JWT/OIDC so the site has real sign-in
- Make `www.archlucid.net/signup` functional with the Stripe test-mode flow
...
### 2.19. Azure Compatibility and SaaS Deployment Readiness — Score: 66 | Weight: 2 | Weighted Deficiency: 0.68

**Justification:** 110 Terraform files across 13+ roots covering the full Azure stack: Container Apps, Front Door, Key Vault, SQL failover, Service Bus, Entra ID, APIM, monitoring, storage, private endpoints, OpenAI. The `terraform-pilot` root provides a single entry point. `apply-saas.ps1` script exists. CD pipeline (`cd.yml`, `cd-staging-on-merge.yml`, `cd-saas-greenfield.yml`). Container images with Docker compose for dev. Managed identity wiring. Consumption budget resources. **`www.archlucid.net` is live on Azure Container Apps with a managed TLS certificate (2026-04-25).**

Gaps: Front Door with WAF is not wired (deferred to v2 for cost reasons). The staging environment probe workflow is conditional on repository variables. The greenfield CD workflow has not been exercised against a clean subscription. Auth is still in dev bypass mode on the live deployment.

**Tradeoffs:** The infrastructure-as-code is comprehensive. The custom domain deployment is live but not yet production-grade (no WAF, no production auth). Front Door deferred to v2 is a reasonable cost-stage tradeoff.

**Recommendations:**
- Switch auth from dev bypass to JWT/OIDC on the live deployment
...
### 2.32. Deployability — Score: 68 | Weight: 1 | Weighted Deficiency: 0.32

**Justification:** Docker compose for dev. Container images. CD pipelines (3 workflows). Terraform across 13+ roots. Post-deploy verification script. Build release scripts. Package release scripts. Greenfield baseline migration runner. **Live deployment at `www.archlucid.net` on Azure Container Apps with managed TLS certificate (2026-04-25) demonstrates deployment works end-to-end.**

Gaps: No blue-green or canary deployment documented. No rollback procedure documented.
...
| Azure SaaS Deployment | 66 | 2 | 132 |
...
**Readiness = 6342 / (100 × 102) × 100 = 62.18%**
...
### 9. Deployment is live but not production-grade

`www.archlucid.net` is live on Azure Container Apps with TLS, but auth is in dev bypass mode, there is no WAF, and the system has not handled real user traffic. The deployment proves the container works; it does not yet prove the system is production-ready.
