...
| **2c — Compress cognitive load** | README "Key documentation" replaced with a **6-row persona table** that puts FIRST_30_MINUTES first; the long deeper-doc list collapsed to one tail paragraph. | [`README.md`](../REPOSITORY_README.md) (Getting started section) | Done |
...
|---|---|---|
| **2b — Bulk archive of stale assessments** to `docs/archive/quality/` | **Shipped 2026-04-21:** five 2026-04-20 quality artifacts (`QUALITY_ASSESSMENT_*`, `QUALITY_IMPROVEMENT_DECISIONS_2026_04_20.md`) moved under `docs/archive/quality/` with synchronized inbound link rewrites (`CHANGELOG`, ADR 0021, Cursor prompt companions, `PENDING_QUESTIONS`, `.github/workflows/ci.yml` comments, `QUALITY_ASSESSMENT.md`, etc.). Archive log: `docs/archive/quality/README.md`. | *(complete)* |
