...
| --- | --- | --- | --- | --- | --- | --- |
| Home `/` | Required | Required | Required | Required | Recommended | First-pilot cockpit, operating rail, and trial widgets. |
...
| Compare `/compare` | Required | Required | Required | Required | Optional | Side-by-side content and overflow behavior. |
| Governance `/governance` | Required | Required | Required | Required | Recommended | Approval workflow cards and status copy. |
...
| Automated evidence | `live-api-accessibility.spec.ts` route remains in `PAGES`; axe gate is separate CI evidence. |
| Keyboard result | Pass: first-pilot cockpit links and operating rail are reachable in logical order. |
