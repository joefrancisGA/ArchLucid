...
The distribution report lists engine types that produced findings on the golden corpus graphs. The harness registers **fourteen** engines (`GoldenCorpusHarness.CreateEngines`); **six** appear in the distribution table today. **33** built-in product engines are absent from that table. `WouldDemoteIfUnprotectedCount` is a **counterfactual** — not production behavior.
