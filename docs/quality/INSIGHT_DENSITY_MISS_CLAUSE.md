...
The distribution report lists engine types that produced findings on the golden corpus graphs. The harness registers **39** engines (`GoldenCorpusHarnessEngineRegistration.RegisteredEngineCount`); **11** built-in product engines are listed absent-with-reason in `GoldenCorpusHarnessEngineInventory` (50 catalog − 39 registered). Only a subset appear in the distribution table on any given record pass. `WouldDemoteIfUnprotectedCount` reflects **production** demotion after ADR 0070 / DX-01. Corpus coverage is **`case-01` … `case-60`** (`LatestGoldenCorpusCaseNumber`); effectful engines that need cloud inventory only fire when a case supplies a pinned fixture (see **`case-37`** for two-way contradiction; **`case-48`–`case-50`** for Azure inventory; **`case-51`–`case-54`** for AWS/GCP inventory; **`case-55`–`case-57`** for Advisor / AWS / GCP cost recommendations; **`case-58`–`case-60`** for DX-42 CloudFormation / Pulumi / CDK ingest feeding path and contradiction engines).
