...
**Status note (2026-04-20).** The Quality Assessment Improvement 4 prompt asked for `ArchLucid.Api` per-package line coverage to reach the strict-profile floor (**≥ 79 %**) without `--skip-package-line-gate`. Two prior sessions (2026-04-19, 2026-04-20) added the under-covered controller and middleware unit tests called out below; the residual gap is small but cannot be honestly stated as "≥ 79 %" without a fresh `dotnet test --collect:"XPlat Code Coverage"` run on the merged Cobertura report (≈ 30 minutes of wall time on CI hardware). Treat the per-package figure as **measurement-pending** until the next full-regression run posts merged Cobertura: this session shipped the Stryker target (`stryker-config.api.json`, baseline **55.0**) and the Marketplace `GaEnabled=true` flip together, and intentionally deferred any *new* speculative coverage tests to a follow-up PR that can verify each addition against measured deltas. See `docs/CHANGELOG.md` 2026-04-20 entry for Improvement 4.
