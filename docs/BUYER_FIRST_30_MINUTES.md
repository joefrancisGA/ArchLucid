...
3. **Run a sample.** Use **Start with sample review** on the operator Home page to open the curated Claims Intake sample package immediately, or run the vertical-shaped sample if your workspace asks you to create one. No upload is required for the first pass. The sample shows the reviewed manifest, findings, evidence trail, and downloadable artifacts so you can see the shape of the output before investing your own data. Sample numbers are demo data, not customer ROI evidence.
