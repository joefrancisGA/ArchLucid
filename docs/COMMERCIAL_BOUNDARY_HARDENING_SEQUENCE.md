...
**More than two signals on one route:** packaging strips (**`LayerHeader`**) and inline cues (**`EnterpriseControlsContextHints`**) use **`useNavCallerAuthorityRank()`** for rank-aware copy; some pages **also** branch paragraphs on **`useEnterpriseMutationCapability()`** (e.g. **Governance resolution** — rank cue vs **Change related controls** supplement). Same **Execute** numeric idea, **different React hooks** — do not collapse them when refactoring. Enumeration: **[PRODUCT_PACKAGING.md](PRODUCT_PACKAGING.md)** §3 *Four UI shaping surfaces*.
