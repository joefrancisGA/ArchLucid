...
- Product language: *architecture package*, *finding*, *evidence trail*, *sealed review record*, *decision* (disposition in Decision register), *approval*, *audit trail* — not *run*, *job*, *alert* (unless it is an alert), *log*. Never call the package a *signed decision record*; *ADR* remains *Architecture Decision Record* for ADR export wording.
