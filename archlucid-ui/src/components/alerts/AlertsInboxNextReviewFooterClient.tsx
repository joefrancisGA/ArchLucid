...
export type AlertsInboxNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review alerts inbox footer when available. */
export function AlertsInboxNextReviewFooterClient(
  props: AlertsInboxNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: alertsInboxNextReviewHref(target.runId),
    };
  }, [props.runId, runs]);
  ...
  return <AlertsInboxNextReviewFooter target={nextReview} />;
}
