...
describe("AcceleratorJobChooserList", () => {
  ...
  it("starts specialty packs when prerequisite is not met", () => {
    useAcceleratorChooserPrerequisitePresentation.mockReturnValue({
      status: "not-met",
      signedRecordHref: null,
      retry: vi.fn(),
    ...
    expect(screen.getByTestId("accelerator-chooser-start-ai-llm-workload")).toBeInTheDocument();
    expect(screen.queryByText(/Follow-up pack/i)).toBeNull();
    expect(screen.getByTestId("accelerator-chooser-row-cost-governance-baseline-recommendation")).toBeInTheDocument();
  });
});
