...
/** Disabled KPI-style cards so the executive dashboard reads as a dashboard before data exists. */
export function ExecutiveDashboardPreviewMetricCards(): React.JSX.Element {
  const v = BUYER_EXECUTIVE_SUMMARY_VOCABULARY;

  return (
    <section
      aria-labelledby="executive-dashboard-preview-metrics-heading"
      className="space-y-3"
      data-testid="executive-dashboard-preview-metrics"
    >
      <h2 id="executive-dashboard-preview-metrics-heading" className={`m-0 ${OPERATOR_TYPOGRAPHY.sectionTitle}`}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {v.metricsPreviewCards.map((metric) => (
          <Card
            ...
            data-testid={`executive-dashboard-preview-metric-${metric.title.toLowerCase().replace(/\s+/g, "-")}`}
          >
