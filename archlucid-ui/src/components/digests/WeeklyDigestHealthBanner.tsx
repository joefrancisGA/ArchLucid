...
export function WeeklyDigestHealthBanner(props: WeeklyDigestHealthBannerProps): ReactElement {
  const { refreshToken = 0, onHealthLoaded } = props;
  ...
  const gaps: DigestSetupGapAction[] = mapDigestSetupGaps(snap.setupGaps.slice(0, 4));
  ...
  return (
    ...
    >
      ...
      {gaps.length > 0 ? (
        <ul className="m-0 mt-4 list-none space-y-3 p-0" data-testid="digest-setup-gaps">
          {gaps.map((gap) => (
            <li
