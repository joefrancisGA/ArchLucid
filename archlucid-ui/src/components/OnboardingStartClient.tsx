...
export function OnboardingStartClient() {
  ...
  return (
    <div className="w-full max-w-3xl space-y-6">
      {loading ? <p className={cn("text-neutral-600 dark:text-neutral-400", OPERATOR_TYPOGRAPHY.body)}>Loading trial workspace…</p> : null}

      {error !== null && !loading && recovery !== null ? (
        <section
          aria-labelledby="onb-error-heading"
          className={cn("rounded-md border border-rose-600/40 bg-al-surface-raised px-3 py-2 text-al-text-primary dark:border-rose-700/50 p-5", OPERATOR_TYPOGRAPHY.body)}
          role="alert"
        >
          <h2 id="onb-error-heading" className={cn("m-0 font-semibold text-al-text-primary", OPERATOR_TYPOGRAPHY.cardTitle)}>
            {recovery.headline}
          </h2>
          <p className={cn("m-0 mt-2 leading-relaxed", OPERATOR_TYPOGRAPHY.body)}>{recovery.detail}</p>
          <p className={cn("m-0 mt-2 font-mono text-red-900/90 dark:text-red-100/90", OPERATOR_TYPOGRAPHY.body)}>{error}</p>
      ...
      {!loading && status !== null ? (
        <section aria-labelledby="onb-status-heading" className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 id="onb-status-heading" className={cn("font-semibold text-al-text-primary", OPERATOR_TYPOGRAPHY.cardTitle)}>
            Trial workspace
          </h2>
          <dl className={cn("mt-3 grid gap-2 text-neutral-800 dark:text-neutral-200 sm:grid-cols-2", OPERATOR_TYPOGRAPHY.body)}>
            <div>
