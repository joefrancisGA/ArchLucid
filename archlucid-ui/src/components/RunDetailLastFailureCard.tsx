...
export function RunDetailLastFailureCard(props: {
  ...
  return (
    <Card
      className={cn("rounded-md border border-rose-600/40 bg-al-surface-raised px-3 py-2 text-al-text-primary dark:border-rose-700/50 shadow-sm", OPERATOR_TYPOGRAPHY.body)}
      data-testid="run-detail-last-failure-card"
    >
      <CardHeader className="pb-2">
        <CardTitle className={cn("font-semibold text-al-text-primary text-red-950 dark:text-red-100", OPERATOR_TYPOGRAPHY.cardTitle)}>
          Last execution failure — HOLD
        </CardTitle>
      </CardHeader>
      <CardContent className={cn("space-y-2 pt-0 text-red-950 dark:text-red-100", OPERATOR_TYPOGRAPHY.body)}>
        <p className="m-0">
        ...
        {reasonCode !== null ? (
          <p className="m-0">
            ...
            <span className={cn("font-mono", OPERATOR_TYPOGRAPHY.helper)}>{reasonCode}</span>
          </p>
        ) : null}
        {typeof props.legacyRunStatus === "string" && props.legacyRunStatus.length > 0 ? (
          <p className={cn("m-0 text-red-900/90 dark:text-red-200/90", OPERATOR_TYPOGRAPHY.helper)}>
