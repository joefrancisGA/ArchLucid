...
export type AdvisorySchedulesNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review advisory schedules footer when available. */
export function AdvisorySchedulesNextReviewFooterClient(
  props: AdvisorySchedulesNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: advisorySchedulesNextReviewHref(target.runId),
    };
  }, [props.runId, runs]);
  ...
  return <AdvisorySchedulesNextReviewFooter target={nextReview} />;
}
