...
export type ProvenanceNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review provenance footer when available. */
export function ProvenanceNextReviewFooterClient(
  props: ProvenanceNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: `/architecture/reviews/${encodeURIComponent(target.runId)}/provenance`,
    };
  }, [props.runId, runs]);
  ...
  return <ProvenanceNextReviewFooter target={nextReview} />;
}
