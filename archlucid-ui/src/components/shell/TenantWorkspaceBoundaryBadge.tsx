...
export function TenantWorkspaceBoundaryBadge(props: TenantWorkspaceBoundaryBadgeProps): React.JSX.Element | null {
  ...
  const tooltip = `Tenant: ${tenantName} · Workspace: ${workspaceName} — click for isolation evidence`;
  ...
  const badgeButton = (
    <button
      ...
      data-testid={variant === "compact" ? "tenant-workspace-boundary-badge-compact" : "tenant-workspace-boundary-badge"}
      title={tooltip}
    >
      {variant === "compact" ? (
  ...
  return <CtoDemoHowItWorksTrigger focusSection="isolation" trigger={badgeButton} />;
}
