...
describe("ApiKeysVsUsersReconciler (TB-2237)", () => {
  it("renders compact strip on API keys with peer link to users", () => {
    ...
    const peer = screen.getByTestId("api-keys-vs-users-peer-link");
  ...
  it("renders compact strip on users with peer link to API keys", () => {
    ...
    expect(screen.getByTestId("api-keys-vs-users")).toHaveAttribute(
  ...
  it("renders full variant with why-two explanation", () => {
    ...
    expect(screen.getByTestId("api-keys-vs-users-current")).toHaveTextContent(
