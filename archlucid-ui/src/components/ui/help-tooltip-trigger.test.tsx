...
describe("HelpTooltipTrigger", () => {
  it("renders an info icon for contextual explanations with a 24px hit target", () => {
    render(<HelpTooltipTrigger aria-label="About workspace scope" size="contextual" icon="info" />);

    const trigger = screen.getByRole("button", { name: "About workspace scope" });

    expect(trigger).toHaveAttribute("data-help-tooltip-trigger");
    expect(trigger).toHaveAttribute("data-help-tooltip-icon", "info");
    expect(trigger).toHaveClass("inline-flex", "h-6", "w-6");
    expect(trigger.className).not.toMatch(/rounded-full/);
    expect(trigger.querySelector("svg")).not.toBeNull();
    expect(trigger.querySelector("svg")?.classList.toString()).toMatch(/h-4/);
