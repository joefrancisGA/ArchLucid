...
export function PostCommitAdvancedAnalysisHint({
  ...
  const sidebarHint = buyerPolishedShell ? (
    <>Deeper passes below are optiona — ost sponsors consume exported deliverables first.</>
  ) : (
    <>
    ...
    <>
      ...
      <p className={cn("m-0 mt-1 text-neutral-800 dark:text-neutral-200", OPERATOR_TYPOGRAPHY.body)}>
        {buyerPolishedShell ? (
          ...
          <>
            This review has a finalized architecture snapshot. None of this is required to judge first-pilot valu — nly when you have a
