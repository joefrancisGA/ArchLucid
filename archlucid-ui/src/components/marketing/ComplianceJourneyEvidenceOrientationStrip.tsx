...
export function ComplianceJourneyEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="mt-6 space-y-3 text-left" data-testid="compliance-journey-orientation">
      <section
        className="rounded-md border border-neutral-200 bg-neutral-50/80 p-3 dark:border-neutral-700 dark:bg-neutral-900/40"
        aria-labelledby="compliance-journey-sources-heading"
        data-testid="compliance-journey-sources"
      >
        <h2
          id="compliance-journey-sources-heading"
        ...
        <ul className={cn("m-0 mt-2 flex list-none flex-wrap gap-x-3 gap-y-1 p-0", MARKETING_TYPOGRAPHY.body)}>
          {COMPLIANCE_JOURNEY_SOURCES.map((link) => (
      ...
      <aside
        ...
        data-testid="compliance-journey-claim-discipline"
      >
        <h2 className={cn("m-0 text-al-text-primary", MARKETING_TYPOGRAPHY.cardTitle)}>
          Posture summary only
        </h2>
        <p className={cn("m-0 mt-2", MARKETING_TYPOGRAPHY.body)}>{COMPLIANCE_JOURNEY_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
