...
export function DemoPreviewEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="mt-8 space-y-3 text-left" data-testid="demo-preview-orientation">
      <section
        className="rounded-md border border-neutral-200 bg-neutral-50/80 p-3 dark:border-neutral-700 dark:bg-neutral-900/40"
        aria-labelledby="demo-preview-sources-heading"
        data-testid="demo-preview-sources"
      >
        <h2
          id="demo-preview-sources-heading"
      ...
      <aside
        ...
        data-testid="demo-preview-claim-discipline"
      >
        <h2 className={cn("m-0 text-al-text-primary", MARKETING_TYPOGRAPHY.cardTitle)}>
          Sample demo only
        </h2>
        <p className={cn("m-0 mt-2", MARKETING_TYPOGRAPHY.body)}>{DEMO_PREVIEW_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
