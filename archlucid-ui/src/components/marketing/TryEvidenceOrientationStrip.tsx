...
import {
  TRY_CLAIM_DISCIPLINE,
  TRY_SOURCES,
  TRY_SOURCES_INTRO,
} from "@/lib/try-evidence-copy";
...
export function TryEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="mt-6 space-y-3 text-left" data-testid="try-orientation">
      <section
        className="rounded-md border border-neutral-200 bg-neutral-50/80 p-3 dark:border-neutral-700 dark:bg-neutral-900/40"
        aria-labelledby="try-sources-heading"
        data-testid="try-sources"
      >
        <h2
          id="try-sources-heading"
      ...
      <aside
        ...
        data-testid="try-claim-discipline"
      >
        <h2 className={cn("m-0 text-al-text-primary", MARKETING_TYPOGRAPHY.cardTitle)}>
          Illustrative sample only
        </h2>
        <p className={cn("m-0 mt-2", MARKETING_TYPOGRAPHY.body)}>{TRY_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
