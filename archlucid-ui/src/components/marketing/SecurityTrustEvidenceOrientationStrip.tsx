...
export function SecurityTrustEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="space-y-3" data-testid="security-trust-orientation">
      <section
        className="rounded-md border border-neutral-200 bg-neutral-50/80 p-3 dark:border-neutral-700 dark:bg-neutral-900/40"
        aria-labelledby="security-trust-sources-heading"
        data-testid="security-trust-sources"
      >
        <h2
          id="security-trust-sources-heading"
        ...
        <ul className={cn("m-0 mt-2 flex list-none flex-wrap gap-x-3 gap-y-1 p-0", MARKETING_TYPOGRAPHY.body)}>
          {SECURITY_TRUST_SOURCES.map((link) => (
      ...
      <aside
        ...
        data-testid="security-trust-claim-discipline"
      >
        <h2 className={cn("m-0 text-al-text-primary", MARKETING_TYPOGRAPHY.cardTitle)}>
          Engagement metadata only
        </h2>
        <p className={cn("m-0 mt-2", MARKETING_TYPOGRAPHY.body)}>{SECURITY_TRUST_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
