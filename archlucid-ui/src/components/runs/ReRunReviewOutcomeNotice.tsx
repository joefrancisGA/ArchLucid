...
const OUTCOME_NOTICE_SHELL_CLASS = "min-w-0 w-full max-w-full break-words";
...
export function ReRunReviewOutcomeNotice(props: ReRunReviewOutcomeNoticeProps): React.JSX.Element {
  ...
  if (phase === "succeeded") {
    return (
      <OperatorSuccessCallout
        ...
        className={cn(OUTCOME_NOTICE_SHELL_CLASS, "mb-0 mt-2", className)}
      />
    );
  }
  ...
  if (phase === "failed") {
    return (
      <div
        className={cn(OUTCOME_NOTICE_SHELL_CLASS, "mt-2", className)}
        data-testid="re-run-review-outcome"
        role="status"
        aria-live="polite"
      >
        <OperatorWarningCallout>
          <p className={cn("m-0 font-medium break-words", OPERATOR_TYPOGRAPHY.body)}>{headline}</p>
          <p className={cn("m-0 mt-2 break-words text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}>
  ...
  if (phase === "canceled") {
    return (
      <div
        className={cn(OUTCOME_NOTICE_SHELL_CLASS, DESIGN_TOKENS.callout.info, "mt-2 p-3", className)}
        data-testid="re-run-review-outcome"
        role="status"
        aria-live="polite"
      >
        <p className={cn("m-0 break-words text-al-text-primary", OPERATOR_TYPOGRAPHY.body)}>{headline}</p>
      </div>
    );
  }
  ...
  return (
    <div
      ...
      aria-busy="true"
    >
      <p className={cn("m-0 break-words font-medium text-al-text-primary", OPERATOR_TYPOGRAPHY.body)}>
        {runningHeadline}
      </p>
      ...
      {runningProgress?.statusRefreshHint !== undefined ? (
        <p className={cn("m-0 mt-1 break-words text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}>
          {runningProgress.statusRefreshHint}
        </p>
      ) : null}
      {runningDetail !== null && runningDetail.length > 0 ? (
        <p
          className={cn("m-0 mt-2 break-words text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}
