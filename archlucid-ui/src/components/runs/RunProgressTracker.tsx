...
export type RunProgressTrackerProps = {
  ...
  /** Create-home Activity: analysis stages finished, signed review record not created yet. */
...
export function RunProgressTracker({
  ...
  const liveStatus = useMemo(() => {
    if (preFinalizeTerminal) {
      return "Ready to finalize — use Finalize review to create the signed review record for this architecture package.";
    }
      ...
      <ul className="m-0 flex flex-col gap-3 p-0 list-none">
        ...
        {buyerAssessmentCopy ? (
          <li className="flex flex-wrap items-center gap-2" data-testid="run-progress-signed-record-row">
            <span className={cn("w-36 font-medium", OPERATOR_TYPOGRAPHY.body)}>Signed review record</span>
            <StatusTag
