export const QUICK_SCAN_PAGE_TITLE = "Quick scan" as const;

export const QUICK_SCAN_HERO_LEAD =
  "Describe a system and receive a concise demonstration architecture risk and improvement summary. Sample-only public path — no account required." as const;

export const QUICK_SCAN_LAST_REVIEWED_LABEL = "2026-08-15" as const;

export const QUICK_SCAN_PRIMARY_CONTENT_ID = "quick-scan-primary-content" as const;

export const QUICK_SCAN_FIRST_VIEWPORT_TEST_ID = "quick-scan-first-viewport" as const;

export const QUICK_SCAN_SKIP_TARGET_ID = QUICK_SCAN_FIRST_VIEWPORT_TEST_ID;

export const QUICK_SCAN_SKIP_LINK_LABEL = "Skip to quick scan content" as const;

export const QUICK_SCAN_HEADER_CLAIM_DISCIPLINE_TEST_ID = "quick-scan-header-claim-discipline" as const;

export const QUICK_SCAN_ORIENTATION_BOTTOM_TEST_ID = "quick-scan-orientation-bottom" as const;

export const QUICK_SCAN_BUYER_OVERVIEW =
  "Describe your system below and run Analyze architecture, then review demonstration limits and privacy handling before treating the summary as workspace evidence." as const;

export const QUICK_SCAN_FORM_ID = "quick-scan-form" as const;

export const QUICK_SCAN_ANALYSIS_TYPICAL_DURATION =
  "Typically 15–30 seconds for a concise demonstration result." as const;
