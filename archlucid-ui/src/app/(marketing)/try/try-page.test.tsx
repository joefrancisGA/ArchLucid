...
describe("TryPage (TB-774)", () => {
  ...
  it("keeps fabricated sample reassurance without Entra-as-default framing", () => {
    render(<TryPage />);

    const text = document.body.textContent ?? "";

    expect(text).toMatch(/fabricated data only/i);
    expect(text).toMatch(/no cloud account setup/i);
    expect(text).toMatch(/no corporate sign-in required/i);
    expect(text).not.toMatch(/\bAzure\b/i);
