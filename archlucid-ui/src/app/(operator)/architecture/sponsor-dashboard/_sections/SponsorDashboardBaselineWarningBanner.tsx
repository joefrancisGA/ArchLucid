...
const SPONSOR_DASHBOARD_BASELINE_WARNING_DISMISSED_SESSION_KEY: string =
...
export const SPONSOR_DASHBOARD_BASELINE_UPLOAD_WIZARD_HREF = "/architecture/reviews/new?baseline=1";

/** Prominent nudge when no cloud inventory baseline artifact exists in the active workspace. */

export type SponsorDashboardBaselineWarningBannerProps = {
  /** `setup` renders a neutral optional-setup card; `banner` keeps the legacy alert strip. */
  readonly variant?: "banner" | "setup";
};

export function SponsorDashboardBaselineWarningBanner({
  variant = "banner",
}: SponsorDashboardBaselineWarningBannerProps = {}) {
  ...
  if (variant === "setup") {
    return (
      <div
        ...
        data-testid="sponsor-baseline-upload-setup-card"
      >
        ...
        <div className="mt-3">
          <Button asChild size="sm" variant="outline" className="border-neutral-300 dark:border-neutral-600">
            <Link
              href={SPONSOR_DASHBOARD_BASELINE_UPLOAD_WIZARD_HREF}
              data-testid="sponsor-baseline-upload-wizard-link"
            >
  ...
  return (
    <div
      ...
      role="alert"
      data-testid="sponsor-baseline-upload-warning-banner"
    >
      ...
      <DismissControl
        ...
        data-testid="sponsor-baseline-upload-warning-dismiss"
