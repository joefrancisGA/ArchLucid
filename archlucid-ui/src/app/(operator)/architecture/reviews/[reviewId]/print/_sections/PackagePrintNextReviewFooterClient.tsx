...
export type PackagePrintNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review print footer when available. */
export function PackagePrintNextReviewFooterClient(
  props: PackagePrintNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: `/architecture/reviews/${encodeURIComponent(target.runId)}/print`,
    };
  }, [props.runId, runs]);
  ...
  return <PackagePrintNextReviewFooter target={nextReview} />;
}
