...
type ItsmAtlassianOAuthCallbackHeaderActionsProps = {
  readonly phase: "loading" | "success" | "failure";
};

/** Header actions for `/integrations/itsm/oauth/callback` (IIO). */
export function ItsmAtlassianOAuthCallbackHeaderActions({
  phase,
}: ItsmAtlassianOAuthCallbackHeaderActionsProps): React.JSX.Element {
  const buyerPolishedShell = isBuyerPolishedOperatorShellEnv();
  const showOpenJiraHeaderAction = buyerPolishedShell && phase === "loading";

  return (
    <div className="flex flex-wrap items-center gap-2" data-testid="itsm-oauth-callback-header-actions">
      {showOpenJiraHeaderAction ? (
