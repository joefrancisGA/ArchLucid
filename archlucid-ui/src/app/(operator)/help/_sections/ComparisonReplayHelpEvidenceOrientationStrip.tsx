...
/** Workspace Sources + claim discipline for CO `/help/comparison-replay`. */
export function ComparisonReplayHelpEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="mb-4 space-y-3" data-testid="comparison-replay-help-orientation">
      <section
      ...
      >
        <h2 className={cn("m-0 text-al-text-primary", OPERATOR_TYPOGRAPHY.cardTitle)}>
          Compare and replay orientation only
        </h2>
        <p className={cn("m-0 mt-2", OPERATOR_TYPOGRAPHY.body)}>{COMPARISON_REPLAY_HELP_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
