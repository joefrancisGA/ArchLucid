...
import {
  ...
  ARCHITECTURE_DRAFTS_HELP_HEADER_CLAIM_DISCIPLINE_TEST_ID,
...
export function HelpArchitectureDraftsGuideView(props: HelpArchitectureDraftsGuideViewProps): React.ReactElement {
  ...
  const contentGridClass = resolveHelpPageContentGridClass(
    buyerPolishedShell ? 0 : ARCHITECTURE_DRAFTS_HELP_GUIDE_HEADINGS.length,
  );
  const readingBodyClass = cn("m-0 leading-relaxed", HELP_PAGE_LAYOUT.readingBody);
  const skipTargetId = buyerPolishedShell
    ? ARCHITECTURE_DRAFTS_HELP_SKIP_TARGET_ID
    : ARCHITECTURE_DRAFTS_HELP_PRIMARY_CONTENT_ID;
  ...
  const buyerPrimaryBody = (
    <>
      <div
        id={ARCHITECTURE_DRAFTS_HELP_SKIP_TARGET_ID}
        data-testid={ARCHITECTURE_DRAFTS_HELP_FIRST_VIEWPORT_TEST_ID}
        className={cn(
          "scroll-mt-24 space-y-4 border-b border-neutral-200 pb-6 dark:border-neutral-800",
          OPERATOR_LAYOUT.sectionStack,
        )}
      >
        <div className="space-y-4" data-testid="help-architecture-drafts-buyer-intro">
          <p className={readingBodyClass} data-testid="help-architecture-drafts-intro">
            {ARCHITECTURE_DRAFTS_HELP_PAGE_LEAD}
          </p>
          <p
            className={cn("m-0 max-w-3xl text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}
            data-testid="help-architecture-drafts-start-here-helper"
          >
            {ARCHITECTURE_DRAFTS_HELP_START_HERE_HELPER}
          </p>
        </div>
      ...
      <div data-testid={ARCHITECTURE_DRAFTS_HELP_ORIENTATION_BOTTOM_TEST_ID}>
        <HelpArchitectureDraftsClaimOrientationStrip />
      </div>
    </>
  );
  ...
  return (
    ...
    >
      <a href={`#${skipTargetId}`} className={HELP_PAGE_LAYOUT.technicalReferenceSkipLink}>
        {ARCHITECTURE_DRAFTS_HELP_SKIP_LINK_LABEL}
      </a>
      ...
      <HelpTopicGuidePageHeader
        ...
        headingLevel="h1"
        claimDiscipline={buyerPolishedShell ? ARCHITECTURE_DRAFTS_HELP_CLAIM_DISCIPLINE : undefined}
        claimDisciplineTestId={
          buyerPolishedShell ? ARCHITECTURE_DRAFTS_HELP_HEADER_CLAIM_DISCIPLINE_TEST_ID : undefined
        }
      ...
      {buyerPolishedShell ? (
        <div
          id={ARCHITECTURE_DRAFTS_HELP_PRIMARY_CONTENT_ID}
          data-testid={ARCHITECTURE_DRAFTS_HELP_PRIMARY_CONTENT_ID}
