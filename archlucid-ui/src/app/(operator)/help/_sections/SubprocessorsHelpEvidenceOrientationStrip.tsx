...
/** Workspace Sources + claim discipline for HEU `/help/subprocessors`. */
export function SubprocessorsHelpEvidenceOrientationStrip(): React.JSX.Element {
  return (
    <div className="mb-4 space-y-3" data-testid="subprocessors-help-orientation">
      <section
      ...
      >
        <h2 className={cn("m-0 text-al-text-primary", OPERATOR_TYPOGRAPHY.cardTitle)}>
          Register orientation only
        </h2>
        <p className={cn("m-0 mt-2", OPERATOR_TYPOGRAPHY.body)}>{SUBPROCESSORS_HELP_CLAIM_DISCIPLINE}</p>
      </aside>
    </div>
  );
}
