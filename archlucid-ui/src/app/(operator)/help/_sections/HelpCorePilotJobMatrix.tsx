...
/** Job-matrix IA dual for full first-review guide vs evidence-only fast path (TB-1683). */
export function HelpCorePilotJobMatrix(): React.JSX.Element {
  return (
    <section
      aria-labelledby="help-core-pilot-job-matrix-heading"
      data-testid={CORE_PILOT_HELP_JOB_MATRIX_TEST_ID}
    >
      <h2
        id="help-core-pilot-job-matrix-heading"
        className={cn("m-0 text-al-text-primary", OPERATOR_TYPOGRAPHY.sectionTitle)}
      >
        {CORE_PILOT_HELP_IA_DUAL_HEADING}
      </h2>
      <ul className={cn("m-0 mt-2 list-none space-y-2 p-0", OPERATOR_TYPOGRAPHY.body)}>
        {CORE_PILOT_HELP_JOB_MATRIX.map((row) => (
