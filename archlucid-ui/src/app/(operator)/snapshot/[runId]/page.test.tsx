...
describe("SnapshotPage (TB-1951 / TB-1953 / TB-1955)", () => {
  ...
  it("redirects showcase leave-behind links to the Claims Intake review workspace with readOnly=1", async () => {
    ...
    await expect(
      ...
      `redirect:/architecture/reviews/${SHOWCASE_STATIC_DEMO_RUN_ID}?readOnly=1`,

    );



    expect(redirect).toHaveBeenCalledWith(`/architecture/reviews/${SHOWCASE_STATIC_DEMO_RUN_ID}?readOnly=1`);
  ...
  it("redirects non-showcase runs to their review workspace with readOnly=1", async () => {
    ...
    ).rejects.toThrow(`redirect:/architecture/reviews/${runId}?readOnly=1`);
