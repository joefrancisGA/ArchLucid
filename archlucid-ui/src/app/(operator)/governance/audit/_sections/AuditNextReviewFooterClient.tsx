...
export type AuditNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review audit footer when available. */
export function AuditNextReviewFooterClient(
  props: AuditNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: auditTrailNextReviewHref(target.runId),
    };
  }, [props.runId, runs]);
  ...
  return <AuditNextReviewFooter target={nextReview} />;
}
