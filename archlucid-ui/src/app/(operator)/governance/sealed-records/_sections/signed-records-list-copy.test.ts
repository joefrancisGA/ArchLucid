...
describe("signed-records-list-copy", () => {
  ...
  it("formats pagination summary with more-available honesty (TB-1944)", () => {
    expect(formatSignedRecordsListPaginationSummary(1, 100, true)).toMatch(/Page 1 · Showing 100 finalized records · more available/);
    expect(formatSignedRecordsListPaginationSummary(2, 1, false)).toBe("Page 2 · Showing 1 finalized record");
  });
  ...
  it("formats page-scoped record counts without implying workspace totals", () => {
    expect(formatSignedRecordsListRecordCount(12, { page: 1, hasMore: true })).toBe(
      "12 finalized review records on this page · more available",
    );
