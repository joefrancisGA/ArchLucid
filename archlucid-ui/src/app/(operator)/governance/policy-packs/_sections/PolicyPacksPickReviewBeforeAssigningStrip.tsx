...
export type PolicyPacksPickReviewBeforeAssigningStripProps = {
  readonly selectedReviewId: string;
  readonly onSelectReview: (reviewId: string) => void;
};

/** Review picker shown before assigning policy packs without a scoped review. */
export function PolicyPacksPickReviewBeforeAssigningStrip(
  props: PolicyPacksPickReviewBeforeAssigningStripProps,
): React.JSX.Element {
  ...
  return (
    <section
      aria-labelledby="policy-packs-pick-review-before-assigning-heading"
      className="mb-4 rounded-lg border border-neutral-200 bg-al-surface-raised px-4 py-3 dark:border-neutral-800"
      data-testid="policy-packs-pick-review-before-assigning-strip"
    >
      <h2
        id="policy-packs-pick-review-before-assigning-heading"
        className={cn("m-0 font-medium text-al-text-primary", OPERATOR_TYPOGRAPHY.body)}
      >
        Pick a review before assigning
      </h2>
      <p className={cn("m-0 mt-1 text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}>
        Policy pack assignments should align with a finalized architecture package so governance scope stays traceable.
      </p>
