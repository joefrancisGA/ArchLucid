...
vi.mock("@/components/alerts/use-alert-rules-hub-queries", () => ({
  useAlertRulesListQuery: () => loadedHubQuery,
  useAlertRoutingSubscriptionsQuery: () => loadedHubQuery,
  useCompositeAlertRulesListQuery: () => loadedHubQuery,
}));
...
describe("AlertRulesHubClient", () => {
  ...
  it("defaults to alert rules tab with page title and never-configured provenance at zero rules", async () => {
  ...
  it("shows composite rules when ?tab=advanced-rules", () => {
    tabValue.current = "advanced-rules";
    render(<AlertRulesHubClient />);
    expect(screen.getByTestId("stub-composite")).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: new RegExp(COMPOSITE_RULES_TAB_LABEL) })).toHaveAttribute("aria-selected", "true");
  });
  ...
  it("falls back to alert rules for unknown tab ids", async () => {
    tabValue.current = "routing";
    render(<AlertRulesHubClient />);
    expect(screen.getByTestId("stub-rules")).toBeInTheDocument();
    await waitFor(() => {
      expect(screen.getByRole("tab", { name: new RegExp(`${ALERT_RULES_TAB_LABEL} \\(0\\)`) })).toHaveAttribute(
        "aria-selected",
        "true",
      );
    });
  ...
  it("omits redundant tab-lead prose under the hub tabs", async () => {
    render(<AlertRulesHubClient />);
  ...
  it("moves tab selection with ArrowRight keyboard navigation", async () => {
    ...
    const rulesTab = await screen.findByRole("tab", { name: new RegExp(`${ALERT_RULES_TAB_LABEL} \\(0\\)`) });
    rulesTab.focus();
  ...
  it("shows tab counts on all hub tabs and disables Test alerts at zero rules (P0-5)", async () => {
    render(<AlertRulesHubClient />);

    expect(await screen.findByRole("tab", { name: new RegExp(`${ALERT_RULES_TAB_LABEL} \\(0\\)`) })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /Notifications \(0\)/ })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: new RegExp(`${COMPOSITE_RULES_TAB_LABEL} \\(0\\)`) })).toBeInTheDocument();
  ...
  it("TB-1663: renders Carbon line-tab chrome on shared Tabs", async () => {
    render(<AlertRulesHubClient />);
