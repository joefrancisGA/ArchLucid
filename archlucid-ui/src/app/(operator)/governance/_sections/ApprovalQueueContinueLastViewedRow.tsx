"use client";

import { Button } from "@/components/ui/button";
import { OPERATOR_TYPOGRAPHY } from "@/lib/design-tokens";
import { governanceEnvironmentPairDisplay } from "@/app/(operator)/governance/_sections/governance-workflow-helpers";
import type { ApprovalQueueContinueLastTarget } from "@/lib/resolve-continue-last-approval-request";
import { cn } from "@/lib/utils";

export type ApprovalQueueContinueLastViewedRowProps = {
  readonly target: ApprovalQueueContinueLastTarget;
  readonly onOpen: (approvalRequestId: string) => void;
};

/** Pinned continue row for the most recently viewed approval request. */
export function ApprovalQueueContinueLastViewedRow(
  props: ApprovalQueueContinueLastViewedRowProps,
): React.JSX.Element {
  return (
    <section
      aria-labelledby="approval-queue-continue-last-viewed-heading"
      className="mb-4 rounded-lg border border-teal-200 bg-teal-50/60 px-4 py-3 dark:border-teal-900/50 dark:bg-teal-950/20"
      data-testid="approval-queue-continue-last-viewed-row"
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h2
            id="approval-queue-continue-last-viewed-heading"
            className={cn("m-0 font-medium text-al-text-primary", OPERATOR_TYPOGRAPHY.body)}
          >
            Continue last viewed request
          </h2>
          <p className={cn("m-0 mt-1 text-al-text-secondary", OPERATOR_TYPOGRAPHY.helper)}>
            <span className="font-medium text-al-text-primary">{props.target.manifestVersion}</span>{" "}
            {governanceEnvironmentPairDisplay(props.target.sourceEnvironment, props.target.targetEnvironment)}
          </p>
        </div>
        <Button
          type="button"
          variant="primary"
          size="sm"
          data-testid="approval-queue-continue-last-viewed-open"
          onClick={() => {
            props.onOpen(props.target.approvalRequestId);
          }}
        >
          Open request
        </Button>
      </div>
    </section>
  );
}
