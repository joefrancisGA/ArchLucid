...
describe("GovernanceFindingsContinueLastViewedRow", () => {
  it("renders continue row with scoped queue href when provided", () => {
    render(
      <GovernanceFindingsContinueLastViewedRow
        target={{
          findingId: "finding-1",
          title: "Private endpoint gap",
          href: "/architecture/reviews/run-1/findings/finding-1?runId=run-scope",
        }}
      />,
    );
    ...
    expect(screen.getByTestId("governance-findings-continue-last-viewed-open")).toHaveAttribute(
      "href",
      "/architecture/reviews/run-1/findings/finding-1?runId=run-scope",
    );
  });

  it("renders continue row with open link without queue scope", () => {
    render(
      <GovernanceFindingsContinueLastViewedRow
        target={{
          findingId: "finding-1",
