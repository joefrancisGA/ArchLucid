...
describe("OperatorHomePageView buyer-polished shell (HOM)", () => {
  ...
  it("renders skip link, intro lead, overview, workspace before follow-ups on first-run overview", () => {
    ...
    expect(intro).toHaveTextContent(OPERATOR_HOME_BUYER_ORIENTATION_PARAGRAPH);
    ...
    expect(screen.queryByTestId("operator-home-overview")).toBeNull();
  ...
  it("renders returning-home hierarchy with metrics before recent reviews and orientation follow-ups", () => {
    ...
    expect(firstViewport.compareDocumentPosition(intro) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(intro.compareDocumentPosition(workspace) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
