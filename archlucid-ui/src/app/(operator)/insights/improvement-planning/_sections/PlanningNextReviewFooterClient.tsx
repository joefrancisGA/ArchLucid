...
export type PlanningNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review improvement planning footer when available. */
export function PlanningNextReviewFooterClient(
  props: PlanningNextReviewFooterClientProps,
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: planningNextReviewHref(target.runId),
    };
  }, [props.runId, runs]);
  ...
  return <PlanningNextReviewFooter target={nextReview} />;
}
