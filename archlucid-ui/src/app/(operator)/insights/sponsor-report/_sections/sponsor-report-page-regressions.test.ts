...
import { SPONSOR_SUMMARY_TRAFFIC_PATH } from "@/lib/ui-route-traffic-sponsor-report";
 ...
 * Workbook drift: `ui-route-traffic-sponsor-report.test.ts` (TB-1961).
 */
describe("sponsor-report sponsor-report regressions (TB-1965)", () => {
  it("traffic workbook path matches canonical sponsor-report route (TB-1961)", () => {
    expect(SPONSOR_SUMMARY_TRAFFIC_PATH).toBe(SPONSOR_REPORT_PATH);
  });
  ...
  it("contextual help maps sponsor report under sponsor-report", () => {
    expect(pageHelpTopicForPathname(SPONSOR_REPORT_PATH)?.slug).toBe("sponsor-report");
  });
});
