...
export type AskNextReviewFooterClientProps = {
  readonly runId: string;
};

/** Loads reviews list context and renders the next-review ask footer when available. */
export function AskNextReviewFooterClient(props: AskNextReviewFooterClientProps): React.JSX.Element | null {
  ...
  const nextReview = useMemo(() => {
    ...
    return {
      runId: target.runId,
      reviewTitle,
      href: askNextReviewHref(target.runId),
    };
  }, [props.runId, runs]);
  ...
  return <AskNextReviewFooter target={nextReview} />;
}
