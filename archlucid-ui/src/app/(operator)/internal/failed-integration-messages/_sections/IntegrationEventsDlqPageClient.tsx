...
import { INTEGRATION_EVENTS_DLQ_CROSS_TENANT_OPS_SUFFIX } from "@/lib/internal/integration-events-dlq-career-honesty";
...
export function IntegrationEventsDlqPageClient() {
  ...
  return (
    <OperatorPageContainer variant="dashboard" className={OPERATOR_LAYOUT.sectionStack} data-testid="integration-events-dlq-page">
      ...
      <IntegrationEventsDlqCareerHonestyStrip />
      ...
      <div className={cn(DESIGN_TOKENS.callout.warn, "px-4 py-3")} role="status" data-testid="integration-events-dlq-cross-tenant-callout">
        ...
        <p className="m-0 mt-1">Failed messages span all tenants and event types — not your current workspace only. Fix the root cause before bulk retry. {INTEGRATION_EVENTS_DLQ_CROSS_TENANT_OPS_SUFFIX}</p>
      </div>
