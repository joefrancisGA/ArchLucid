...
describe("settings-roles-page-keys-tab-copy", () => {
  it("frames the keys tab as role assignment, not credential lifecycle (TB-1931)", () => {
    ...
    expect(SETTINGS_ROLES_KEYS_TAB_LIFECYCLE_HREF).toBe("/administration/api-keys");
  });

  it("names the keys empty primary CTA Open API keys (TB-1213)", () => {
    expect(SETTINGS_ROLES_KEYS_TAB_OPEN_CTA_LABEL).toBe("Open API keys");
  });
});
