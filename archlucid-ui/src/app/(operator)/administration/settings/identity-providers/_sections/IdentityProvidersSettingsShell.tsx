...
const NAV_ITEMS: ReadonlyArray<{ readonly id: IdentityProvidersNavId; readonly label: string; readonly href: string }> = [
  { id: "overview", label: IDENTITY_PROVIDERS_NAV_OVERVIEW, href: "/administration/settings/identity-providers" },
  { id: "saml", label: IDENTITY_PROVIDERS_NAV_SAML, href: "/administration/settings/identity-providers/saml" },
  { id: "oidc", label: IDENTITY_PROVIDERS_NAV_OIDC, href: "/administration/settings/identity-providers/oidc" },
  { id: "role-mapping", label: IDENTITY_PROVIDERS_NAV_ROLE_MAPPING, href: "/administration/settings/identity-providers/role-mapping" },
  { id: "diagnostics", label: IDENTITY_PROVIDERS_NAV_DIAGNOSTICS, href: "/administration/settings/identity-providers/diagnostics" },
];

function resolveActiveNavId(pathname: string): IdentityProvidersNavId {
  if (pathname.startsWith("/administration/settings/identity-providers/saml")) {
    return "saml";
  }

  if (pathname.startsWith("/administration/settings/identity-providers/oidc")) {
    return "oidc";
  }

  if (pathname.startsWith("/administration/settings/identity-providers/role-mapping")) {
    return "role-mapping";
  }

  if (pathname.startsWith("/administration/settings/identity-providers/diagnostics")) {
    return "diagnostics";
  }
