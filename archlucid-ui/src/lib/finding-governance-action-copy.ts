...
export const EXPIRATION_HELP =
  "Stored in UTC. The signed review record is not modified — expiration applies to the exception only.";
...
export function dispositionTransitionCopy(disposition: FindingDispositionKind): string {
  const base = "Appends a disposition event to the audit trail. The signed review record is not automatically changed.";
...
export function createWaiverTransitionCopy(): string {
  return "Creates a time-bounded risk exception with owner and evidence reference. Recorded in audit history. Revocable before expiration. Does not alter the signed review record.";
}
