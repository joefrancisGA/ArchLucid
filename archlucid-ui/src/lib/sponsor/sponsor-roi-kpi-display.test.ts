...
describe("presentSponsorKpiCount", () => {
  ...
  it("distinguishes zero from missing", () => {
    const result = presentSponsorKpiCount(0, { loading: false });
  ...
  it("can suppress repeated zero footnotes on sponsor surfaces", () => {
    const result = presentSponsorKpiCount(0, { loading: false, suppressZeroFootnote: true });

    expect(result.footnote).toBeNull();
  });

  it("formats finite counts", () => {
    const result = presentSponsorKpiCount(1284, { loading: false });
...
describe("presentCostEvidenceFreshness", () => {
  ...
  it("uses sponsor-friendly labels when requested", () => {
    const result = presentCostEvidenceFreshness({
      loading: false,
      status: "Missing",
      savingsPricingBasis: "Measured",
