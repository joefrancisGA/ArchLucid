...
describe("ui-route-traffic-legacy-architecture-graph (TB-1806)", () => {
  it("tracks the legacy Operate graph shim on OAX with honest redirect-only workbook notes", () => {
    const rows = extractMasterTableRows(readTemplateMarkdown());
    const row = findTrafficRowById(rows, LEGACY_ARCHITECTURE_GRAPH_TRAFFIC_ROW_ID);

    expect(row).toBeDefined();
    expect(row?.path).toBe(LEGACY_ARCHITECTURE_GRAPH_TRAFFIC_PATH);
    expect(row?.notes).toBe(LEGACY_ARCHITECTURE_GRAPH_TRAFFIC_NOTE);
    expect(row?.notes.toLowerCase()).toContain("redirect");
    expect(row?.notes).toContain("/graph");
  });
});
