...
export function stripCliUsageContributorLeakage(markdown: string): string {
  return stripCliUsageContributorSections(markdown)
    ...
    .replace(/dbo\.AuditEvents/gi, "audit trail")
    .replace(/\bReadAuthority\b/gi, "workspace reader permission")
    .replace(/\bExecuteAuthority\b/gi, "permission to execute reviews")
    .replace(/\bAdminAuthority\b/gi, "workspace administrator permission")
