...
describe("legacy-snapshot-redirect (TB-1952)", () => {
  ...
  it("resolves tenant runs to their review workspace, not sponsor executive summary", () => {
    expect(resolveSnapshotRedirectDestination("pilot-run-42")).toBe("/reviews/pilot-run-42");
  ...
  it("builds readOnly=1 redirect targets for showcase and tenant runs", () => {
    expect(buildSnapshotRedirectPath("pilot-run-42", {})).toBe("/reviews/pilot-run-42?readOnly=1");
    expect(buildSnapshotRedirectPath(SHOWCASE_STATIC_DEMO_RUN_ID, { v: "demo" })).toBe(
      `/reviews/${SHOWCASE_STATIC_DEMO_RUN_ID}?readOnly=1&v=demo`,
    );
  });
});
