...
export const SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_CLAIM_DISCIPLINE =
  "This page explains how SecureNow handles cloud inventory evidence and enforces tenant scope. It is operator orientation, and it is not a countersigned DPA.";
...
export const SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES_INTRO =
  "Use these follow-ups when isolation, audit lineage, or AI provider claims need a cite trail beyond this guide.";
...
/** Operator diligence cites for SecureNow — no self-href to `/help/data-handling`. */
export const SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES: readonly EvidenceDiligenceSourceLink[] = [
  {
    label: "Trust Center",
    href: "/trust",
    evidences: "Public assurance downloads and procurement posture summaries",
  ...
  },
  {
    ...
    evidences: "Sealed evidence records and lineage exports retained in your tenant",
    access: "Tenant workspace",
  },
  {
    label: "Audit trail",
...
export const SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_ORIENTATION_SOURCES: readonly EvidenceDiligenceSourceLink[] =
  SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES.filter(
    (source) => !DATA_HANDLING_TENANT_ISOLATION_HELP_EXCLUDED_ORIENTATION_SOURCE_HREFS.has(source.href),
  );

export function resolveDataHandlingTenantIsolationHelpClaimDiscipline(
  productLineId: ProductLineId = "architecture",
): string {
  return isSecureNowProductLine(productLineId)
    ? SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_CLAIM_DISCIPLINE
    : DATA_HANDLING_TENANT_ISOLATION_HELP_CLAIM_DISCIPLINE;
}

export function resolveDataHandlingTenantIsolationHelpSourcesIntro(
  productLineId: ProductLineId = "architecture",
): string {
  return isSecureNowProductLine(productLineId)
    ? SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES_INTRO
    : DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES_INTRO;
}

export function resolveDataHandlingTenantIsolationHelpSources(
  productLineId: ProductLineId = "architecture",
): readonly EvidenceDiligenceSourceLink[] {
  return isSecureNowProductLine(productLineId)
    ? SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES
    : DATA_HANDLING_TENANT_ISOLATION_HELP_SOURCES;
}

export function resolveDataHandlingTenantIsolationHelpOrientationSources(
  productLineId: ProductLineId = "architecture",
): readonly EvidenceDiligenceSourceLink[] {
  return isSecureNowProductLine(productLineId)
    ? SECURENOW_DATA_HANDLING_TENANT_ISOLATION_HELP_ORIENTATION_SOURCES
