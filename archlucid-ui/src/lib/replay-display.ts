...
const REPLAY_MODE_LABELS: Record<string, string> = {
  ReconstructOnly:
    "Reconstruct only — replay pipeline steps without rebuilding the signed review record or artifacts.",
  RebuildManifest:
    "Rebuild review package — replay and regenerate the signed review record from stored inputs.",
  RebuildArtifacts:
    "Rebuild artifacts — replay through review package regeneration and artifact synthesis where applicable.",
};
...
export const REPLAY_MODE_PLAIN_OPTIONS: readonly { readonly mode: string; readonly label: string }[] = [
  ...
  { mode: "RebuildManifest", label: "Rebuild signed review record — use when you suspect review record drift" },
