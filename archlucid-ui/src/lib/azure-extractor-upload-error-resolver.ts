...
function guidanceForSemanticCode(code: AzureExtractorUploadSemanticCode): string {
  switch (code) {
    case "AZURE_EXTRACTOR_UNSUPPORTED_SCHEMA_VERSION":
      return "Re-run Get-ArchLucidAzurePackage.ps1 from the current packager script download so manifest.json uses a supported schemaVersion, then upload the new ZIP.";
