...
export function buildReviewsHubSources(workingMode: boolean): readonly EvidenceSourceLink[] {
  const portfolioParent = resolveWorkingArchitecturePortfolioParentLink(workingMode);

  const sources: EvidenceSourceLink[] = [
  ...
  if (workingMode) {
    return [{ label: portfolioParent.label, href: portfolioParent.href }, ...sources];
  }

  return sources;
}

/** Guided default — prefer {@link buildReviewsHubSources}. */
export const REVIEWS_HUB_SOURCES: readonly EvidenceSourceLink[] = buildReviewsHubSources(false);
