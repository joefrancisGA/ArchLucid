...
export const SETTINGS_USERS_ROLES_TAB_TRAFFIC_NOTE =
 "Users and roles Roles tab (Tab surface) - inherits AUX hub Evidence chrome (SettingsRolesPageView PageContextualHelpButton + Category-1 registry on /administration/users; Sources follow-up chrome removed (TB-2092) above tabs). SettingsRolesPageView syncs ?tab=roles via router.replace; mounts roles matrix + assignable roles. Sibling SSU = users; SEU = keys; AUX = hub. Template SRX renamed to SER to match owner. Does not imply CPA SOC 2 or third-party pen-test publication. Score 48/100 (2026-08-08) - path-tab surface hard-caps higher Evidence (below AUX access hub). Owner pass: Evidence chrome shipped; cannot improve further toward 80 without turning this into a signed-record diligence Sources trail.";
