...
export const SETTINGS_USERS_ROLES_TAB_TRAFFIC_NOTE =
  "Users and roles Roles tab (Tab surface) - inherits AUX hub Evidence chrome (SettingsRolesPageView PageContextualHelpButton + Category-1 registry on /administration/users; SettingsUsersEvidenceOrientationStrip above tabs). SettingsRolesPageView syncs ?tab=roles via router.replace; mounts roles matrix + assignable roles. Sibling SSU = users; SEU = keys; AUX = hub. Template SRX renamed to SER to match owner. Score 48/100 (2026-08-08) - path-tab surface hard-caps higher Evidence (below AUX access hub).";
