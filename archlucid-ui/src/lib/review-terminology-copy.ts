 ...
 * and finalized review record over internal "golden manifest" jargon. API fields (`runId`, routes) are unchanged.
 */
...
const MANIFEST_PRIMARY_REPLACEMENTS: ReadonlyArray<readonly [RegExp, string]> = [
  // Package mislabeled as "decision record" (legacy sponsor synonym) → finalized review record.
...
/** Maps API or legacy labels that still say "golden manifest" into buyer-facing finalized review record copy. */
