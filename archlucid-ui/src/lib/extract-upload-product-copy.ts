...
export const EXTRACT_UPLOAD_QUICK_START_DESCRIPTION =
  "From your ArchLucid checkout: sign in to Azure when prompted, then upload ./archlucid-azure-package.zip here." as const;
...
export function extractUploadQuickStartDescription(productLineId: ProductLineId = "architecture"): string {
  return localizeProductCopy(productLineId, EXTRACT_UPLOAD_QUICK_START_DESCRIPTION);
}
