...
export const ARCHITECTURE_DRAFT_EDITING_HELP_OVERVIEW =
  `Two architects can open the same architecture draft. ${PRODUCT_LINE_DISPLAY_NAME.architecture} uses a soft work lease and version checks on save so you do not silently overwrite each other. This is not Google Docs-style co-editing, live cursors, or chat on the draft desk.`;
...
export const ARCHITECTURE_DRAFT_EDITING_HELP_LEASE_COPY =
  `When you open a drafting desk, ${PRODUCT_LINE_DISPLAY_NAME.architecture} tries to acquire a short-lived edit lease. If someone else already holds an unexpired lease, you see a banner naming the holder and when the lease expires. That banner is not live presence — it only reflects the last lease the server recorded. This work lease is not the run execute lease that owns a Record job. When the lease expires, saving is blocked until you acquire it again — the other architect did not vanish from a presence feed.`;
