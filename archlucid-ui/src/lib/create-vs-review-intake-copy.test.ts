...
describe("create-vs-review-intake-copy (TB-1459)", () => {
  it("discloses account sync for saved drafts without implying review filing is required", () => {
    ...
    expect(ARCHITECTURE_CREATION_NO_DRAFTS_GUIDANCE.toLowerCase()).toContain("account");
    expect(ARCHITECTURE_CREATION_NO_DRAFTS_GUIDANCE.toLowerCase()).not.toContain("other browsers");
  });
