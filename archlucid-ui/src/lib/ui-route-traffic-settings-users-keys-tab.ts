...
export const SETTINGS_USERS_KEYS_TAB_TRAFFIC_NOTE =
  "Users and roles API keys tab (Tab surface) - inherits AUX hub Evidence chrome (SettingsRolesPageView PageContextualHelpButton + Category-1 registry on /administration/users; SettingsUsersEvidenceOrientationStrip above tabs). SettingsRolesPageView syncs ?tab=keys via router.replace; AdminAuthority-gated keys panel. Sibling SSU = users; SER = roles; AUX = hub. Template SEK renamed to SEU to match owner. Score 48/100 (2026-08-07) - path-tab surface hard-caps higher Evidence (below AUX access hub).";
