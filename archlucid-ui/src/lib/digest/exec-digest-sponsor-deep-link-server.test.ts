...
describe("fetchExecDigestSponsorDeepLinkView", () => {
  ...
  it("normalizes legacy /auth/sign-in signInUrl from API to canonical /auth/signin", async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          target: "dashboard",
  ...
  it("normalizes legacy /auth/sign-in/ signInUrl with trailing slash to canonical /auth/signin", async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          target: "dashboard",
