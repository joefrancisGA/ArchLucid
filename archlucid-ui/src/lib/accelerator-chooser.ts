...
export const ACCELERATOR_COST_GOVERNANCE_GROUP: AcceleratorCostGovernanceGroup = {
  ...
  packLabel: "Cost policy pack",
  summary: "FinOps-oriented orphan and cost findings with ROI labels for finance stakeholders.",
  requiredInputs: "Cloud inventory ZIP or cost baseline (recommended after a prior finalized review)",
  technicalInputs: "second-run.json (optional extractor ZIP)",
  expectedOutputs: "Findings with ROI labels and export-ready output",
};

/** Top buyer-job → accelerator pack rows surfaced on operator home (TB-170). Greenfield first. */
export const ACCELERATOR_CHOOSER_ENTRIES: readonly AcceleratorChooserEntry[] = [
  ...
  },
  {
    ...
    requiredInputs: "Architecture description and policy context in the new-review wizard",
    technicalInputs: "second-run.json, policy-context.json",
    expectedOutputs: "Findings, architecture review, and export-ready output checklist",
    doNotUseWhen: "Buyer demands CPA attestation",
