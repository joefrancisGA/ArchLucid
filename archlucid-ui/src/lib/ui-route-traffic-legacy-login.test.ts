...
describe("ui-route-traffic-legacy-login (TB-1794)", () => {
  it("tracks the legacy login shim on LOG with honest redirect-only workbook notes", () => {
    const rows = extractMasterTableRows(readTemplateMarkdown());
    const row = findTrafficRowById(rows, LEGACY_LOGIN_TRAFFIC_ROW_ID);

    expect(row).toBeDefined();
    expect(row?.path).toBe(LEGACY_LOGIN_TRAFFIC_PATH);
    expect(row?.notes).toBe(LEGACY_LOGIN_TRAFFIC_NOTE);
    expect(row?.notes.toLowerCase()).toContain("redirect");
    expect(row?.notes).toContain("/auth/signin");
  });
});
