...
function localizePreflightTopics(
  topics: readonly CloudSecurityPreflightTopic[],
  productLineId: ProductLineId,
): readonly CloudSecurityPreflightTopic[] {
  return topics.map((topic) => ({
    ...topic,
    detail: localizeProductCopy(productLineId, topic.detail),
  }));
}

export function cloudSecurityPreflightTopics(
  provider: CloudProviderId,
  productLineId: ProductLineId = "architecture",
): readonly CloudSecurityPreflightTopic[] {
  const topics = withProviderTrustControls(PROVIDER_TOPICS_BASE[provider], provider);

  return localizePreflightTopics(topics, productLineId);
}