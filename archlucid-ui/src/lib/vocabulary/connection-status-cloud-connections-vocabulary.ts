...
/** Pairwise model for Connection status ↔ Cloud connections (fixed routes). */
export function buildConnectionStatusCloudConnectionsPairwiseRail(): PairwiseVocabularyRailModel<ConnectionStatusCloudConnectionsSurfaceId> {
  return {
    ...
    currentLink: CONNECTION_STATUS_CLOUD_CONNECTIONS_STATUS_LINK,
    peerLink: CONNECTION_STATUS_CLOUD_CONNECTIONS_CLOUD_LINK,
  };
}
...
export function buildConnectionStatusCloudConnectionsVocabulary(): ConnectionStatusCloudConnectionsVocabularyModel {
  const rail = buildConnectionStatusCloudConnectionsPairwiseRail();
