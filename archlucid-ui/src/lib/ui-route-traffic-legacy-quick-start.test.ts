...
describe("ui-route-traffic-legacy-quick-start (TB-1816)", () => {
  it("tracks the legacy quick-start shim on QUI with honest redirect-only workbook notes", () => {
    const rows = extractMasterTableRows(readTemplateMarkdown());
    const row = findTrafficRowById(rows, LEGACY_QUICK_START_TRAFFIC_ROW_ID);

    expect(row).toBeDefined();
    expect(row?.path).toBe(LEGACY_QUICK_START_TRAFFIC_PATH);
    expect(row?.notes).toBe(LEGACY_QUICK_START_TRAFFIC_NOTE);
    expect(row?.notes.toLowerCase()).toContain("redirect");
    expect(row?.notes).toContain("/get-started");
  });
});
