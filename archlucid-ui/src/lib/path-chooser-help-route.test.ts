...
describe("path-chooser-help-route (HPX)", () => {
  ...
  it("routes the canonical slug through HelpPathChooserGuideView specialty chrome (TB-1601)", () => {
    const pageSource = readFileSync(HELP_TOPIC_PAGE, "utf8");

    expect(pageSource).toContain('loaded.entry.slug === "choose-your-next-step"');
    expect(pageSource).toContain("HelpPathChooserGuideView");
    expect(pageSource).toContain("PATH_CHOOSER_HELP_ROUTE_METADATA");
  });



  it("sanitizes choose-your-next-step markdown via stripPathChooserContributorLeakage (TB-1712)", () => {
    ...
    expect(ruleSet?.id).toBe("choose-your-next-step");
  ...
  it("routes contributor leakage out of presented choose-your-next-step copy (TB-1712)", () => {
  ...
  it("keeps product handoffs on canonical /help/choose-your-next-step", () => {
  ...
  it("does not expose choose-your-next-step on the customer Help Center featured grid", () => {

    expect(HELP_CENTER_FEATURED_SLUGS).not.toContain("choose-your-next-step");
