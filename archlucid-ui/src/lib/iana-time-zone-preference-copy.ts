/** Personal preferences card — time zone selection. */
export const PREFERENCES_TIME_ZONE_HEADING = "Time zone";

export const PREFERENCES_TIME_ZONE_LEAD =
  "Choose how dates and times appear across ArchLucid. Your preference is saved to your account and syncs across browsers where you sign in.";

export const IANA_TIME_ZONE_PREFERENCE_ACCOUNT_SYNC_LOCAL_ONLY_MESSAGE =
  "Saved on this device only. Account sync failed — check connectivity and try again.";
