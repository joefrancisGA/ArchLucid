...
export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_ADR_0096_IN_APP_HREF = inAppHelpHref("proxy-timeout-real-execute");

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_ADR_0096_IN_APP_LABEL = "ADR 0096" as const;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_OVERVIEW_LEAD =
  `On Working seats, long-running analysis does not require babysitting one browser tab. ${DAYTIME_WAIT_WORKING_BACKGROUND_WAIT_HELPER} Track named stages from the shell header, open Activity on the child review when you need package context, and use cooperative Cancel when you intend to stop — not when you only navigate away.` as const;

/** @deprecated Merged into {@link DAYTIME_WAIT_HELP_BACKGROUND_WAIT_OVERVIEW_LEAD} — kept for drift guards. */
...
export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_SEAT_APPLICABILITY_WORKING =
  `Working seats use background wait: the shell in-flight strip (header in-progress control) is the resume path across pages. ${CAREER_REHEARSAL_HELP_RECORD_EXECUTE_LABEL} follows the async operations pattern in ${DAYTIME_WAIT_HELP_BACKGROUND_WAIT_ADR_0096_IN_APP_LABEL} — poll GET /v1/operations/{operationId}, not a fictional run-progress URL.` as const;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_SEAT_APPLICABILITY_GUIDED_TAG =
  "Guided / demo / trial only" as const;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_SEAT_APPLICABILITY_GUIDED =
  "Guided, demo, and trial seats may still teach stay-on-this-page wait chrome — that teaching copy does not apply to all-day Working production seats." as const;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_RESUME_PATH_INTRO =
  `Use the shell in-flight strip as your resume path: the header ${REVIEW_PIPELINE_OPEN_IN_FLIGHT_STRIP_LABEL} opens the in-progress popover with one row per tracked operation, named stage labels, and elapsed time. On a nested review, the in-pipeline banner offers the same ${REVIEW_PIPELINE_OPEN_IN_FLIGHT_STRIP_LABEL} control — it does not replace the shell strip.` as const;
...
export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_IN_FLIGHT_SHORTCUT_SCOPE =
  "Architecture desk (global shell header on any page)" as const;
...
export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_KEYBOARD_SHORTCUTS_BODY =
  architectureDeskInFlightShortcut === undefined
    ...
    : `On an architecture desk, ${architectureDeskInFlightShortcut.label} opens the first in-flight review job listed on that desk. Open the shortcuts dialog from any page for the full Review section map.` as const;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_ADR_0096_DOC_PATH = DAYTIME_WAIT_ADR_0096_RELATIVE_PATH;

export const DAYTIME_WAIT_HELP_BACKGROUND_WAIT_ADR_0096_CANONICAL_IN_APP_PATH =
  DAYTIME_WAIT_HELP_PROXY_TIMEOUT_PATH;
