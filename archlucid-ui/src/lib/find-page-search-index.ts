...
export function searchGuidedLockedFindPageEntries(
  query: string,
  ...
  if (normalizedQuery.length === 0 || lockedDestinations.length === 0) {
    return [];
  }

  const limit = options?.limit ?? 4;

  return lockedDestinations
    .map((destination) => ({
      id: `locked:${destination.href}`,
