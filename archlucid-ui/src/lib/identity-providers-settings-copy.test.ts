...
describe("identity-providers-settings-copy", () => {
  ...
  it("uses a distinct role mapping shell subtitle instead of configure workspace intro (TB-1917)", () => {
    expect(IDENTITY_PROVIDERS_ROLE_MAPPING_PAGE_SUBTITLE).not.toBe(IDENTITY_PROVIDERS_PAGE_SUBTITLE);
    expect(IDENTITY_PROVIDERS_ROLE_MAPPING_PAGE_SUBTITLE).not.toBe(IDENTITY_PROVIDERS_PAGE_INTRO);
    expect(IDENTITY_PROVIDERS_ROLE_MAPPING_PAGE_SUBTITLE).not.toBe(IDENTITY_PROVIDERS_ROLE_MAPPING_PAGE_INTRO);
    expect(IDENTITY_PROVIDERS_ROLE_MAPPING_PAGE_SUBTITLE).not.toBe(IDENTITY_PROVIDERS_ROLE_MAPPING_HELPER);
