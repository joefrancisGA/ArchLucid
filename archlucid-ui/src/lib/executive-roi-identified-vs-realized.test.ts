...
describe("resolveExecutiveRoiIdentifiedVsRealized", () => {
  it("maps open + needs-evidence to identified pending and remediated to realized", () => {
    const buckets = resolveExecutiveRoiIdentifiedVsRealized(sampleSummary());
  ...
  it("falls back to headline total when basis breakdown is absent", () => {
    const buckets = resolveExecutiveRoiIdentifiedVsRealized(
