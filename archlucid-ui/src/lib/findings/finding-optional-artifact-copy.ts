...
export function resolveFindingOptionalArtifactUnavailableCopy(
  ...
  if (presentation === "not-found") {
    if (kind === "explainability-trace") {
      return {
        ...
        tryNext: "Open the evidence graph or Finalized review record for persisted citations.",
        showRetry: false,
      };
    }
    ...
    if (kind === "audit-record") {
      return {
        ...
        tryNext: "Use the evidence graph and Finalized review record for the authoritative trail.",
        showRetry: false,
      };
    }
  ...
  if (presentation === "transient" || isApiTransientLoadFailure(failure)) {
    return {
      ...
      tryNext: "Retry in a moment, or continue from the evidence graph and Finalized review record.",
      showRetry: true,
    };
  }
  ...
  if (kind === "explainability-trace") {
    return {
      ...
      tryNext: "Use the evidence graph and Finalized review record for review-grade traceability.",
      showRetry: true,
    };
  }
