...
describe("finalize-quality-scorecard transparency trail (LK-09)", () => {
  it("blocks finalize when required capabilities lack topology evidence (TB-2346)", () => {
    const result = evaluateFinalizeQualityScorecard({
      ...
      missingRequiredCapabilityCount: 1,
      openDeferredCount: 0,
    ...
    expect(result.blockingReasons).toContain(
      "1 required capability lacks topology evidence on the context graph.",
    );
  });
