...
export function formatDraftBranchQuotaSummary(quota: DraftBranchQuotaResponse): string {
  ...
  return `Branches used: ${quota.existingBranchCount}/${quota.maxBranchesPerParent} · ${quota.remainingBranches} remaining · each submit runs one billable ${BILLING_ARCHITECTURE_PACKAGE_OVERAGE_UNIT_LABEL} (full pipeline; ~$${cost} estimated AI spend).`;
}
