/**
 * Printable architecture package view (TB-2205) — source-of-truth copy and helpers.
...
export const PACKAGE_PRINT_PAGE_TITLE = "Architecture package";

/** Screen-only instructions above the print button (hidden when printing). */
export const PACKAGE_PRINT_INSTRUCTIONS =
  "Use Print / Save as PDF for a clean package summary without operator chrome or export-format choices. This view is for printing — not a signed export artifact.";
...
export const PACKAGE_PRINT_BACK_LABEL = "Back to package";
...
export const PACKAGE_PRINT_LOADING_LABEL = "Loading package for print…";
export const PACKAGE_PRINT_ERROR_FALLBACK = "This architecture package could not be loaded for print.";
...
export function buildPackagePrintFindingsSummary(summary: RunSummary): string {
  ...
  if (parts.length === 0) {
    if (summary.hasGoldenManifest === true) {
      return "Findings counts are not on this summary — open the package findings tab for the full list.";
    }

    return "No findings summary is available yet. Finalize the review to publish findings with the architecture package.";
  }

  const base = parts.join(" · ");

  if (summary.hasGoldenManifest === true) {
    return `${base}. Included in the finalized architecture package.`;
  }
...
export function buildPackagePrintSponsorSynopsis(summary: RunSummary): string | null {
  ...
  const findingsPhrase =
    findings === null
      ? "findings are available on the package"
  ...
  return `Sponsor synopsis for "${title}": finalized architecture package with ${findingsPhrase}.${warningsPhrase}`;
}
