...
describe("marketing design tokens", () => {
  it("shares operator page shell but uses a wider marketing rail", () => {
    ...
    expect(MARKETING_TYPOGRAPHY.body).toContain("text-[17px]");
    expect(MARKETING_TYPOGRAPHY.body).not.toBe(OPERATOR_TYPOGRAPHY.body);
    expect(MARKETING_TYPOGRAPHY.meta).toContain("text-[15px]");
  ...
  it("shares operator primary-action tokens for marketing CTAs (TB-2292)", () => {
    ...
    expect(MARKETING_PRIMARY_CTA_CLASS).toContain("font-bold");
    expect(MARKETING_PRIMARY_CTA_CLASS).toContain("text-[11px]");
  });
