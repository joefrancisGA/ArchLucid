    ...
    return {
      ...
      detail:
        lastFailureReason.length > 0
          ...
          : "Server marked this review id as dead-lettered. Check API logs and AuthorityPipelineWork outbox for this review id.",
    };
  }
  ...
  if (legacyStatus === "Failed" && completedStages === 0) {
    return {
      ...
      detail:
        lastFailureReason.length > 0
          ? lastFailureReason
          : "Execution failed with no stage snapshots. Re-execute or inspect API logs for this review id.",
    };
  }
