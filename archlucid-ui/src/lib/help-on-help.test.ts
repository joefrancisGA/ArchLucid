...
describe("help-on-help", () => {
  it("detects help routes for help-on-help mode (TB-1046)", () => {
    expect(isHelpOnHelpPath("/help/core-pilot")).toBe(true);
    expect(isHelpOnHelpPath("/help/core-pilot#first-review-path")).toBe(true);
  ...
  it("lists in-page section anchors for core-pilot from the help search index", () => {
    const anchors = listHelpOnHelpSectionAnchors("/help/core-pilot");
    ...
    expect(anchors.every((anchor) => helpDocRecordTargetsPath(anchor, "/help/core-pilot"))).toBe(true);
  });
  ...
  it("prioritizes current-page search hits when on a help article", () => {
    ...
    const prioritized = prioritizeHelpSearchHitsForCurrentPage(hits, "/help/core-pilot");

    expect(prioritized.length).toBe(hits.length);

    const firstCurrentIndex = prioritized.findIndex((hit) =>
      helpDocRecordTargetsPath(hit, "/help/core-pilot"),
    );
    const firstOtherIndex = prioritized.findIndex(
      (hit) => !helpDocRecordTargetsPath(hit, "/help/core-pilot"),
    );
