...
export function buildStaticDemoManifestSummaryFromShowcase(urlRunId: string): ManifestSummary {
  ...
  return {
    ...
    decisionCount: m.decisionCount ?? 0,
    warningCount: m.warningCount ?? 0,
    unresolvedIssueCount: m.unresolvedIssueCount ?? 0,
    status: m.status,
    hasWarnings: (m.warningCount ?? 0) > 0,
    hasUnresolvedIssues: (m.unresolvedIssueCount ?? 0) > 0,
