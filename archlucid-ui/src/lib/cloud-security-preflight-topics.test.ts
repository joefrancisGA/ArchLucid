...
describe("cloudSecurityPreflightTopics", () => {
  ...
  it("names SecureNow in security-shell preflight copy", () => {
    const topics = cloudSecurityPreflightTopics("aws", "security");
    const noSecrets = topics.find((topic) => topic.id === "no-long-lived-secrets");

    expect(noSecrets?.detail).toContain("SecureNow stores connection metadata");
    expect(noSecrets?.detail).not.toMatch(/\bArchLucid\b/);
  });
});
