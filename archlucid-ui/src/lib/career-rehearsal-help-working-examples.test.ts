...
describe("career-rehearsal help Working examples (AS-082)", () => {
  it("registers a help topic that names both Career and Rehearsal doors", () => {
    const entry = getProductDocumentationEntry("career-vs-rehearsal");

    expect(entry?.slug).toBe("career-vs-rehearsal");
    expect(entry?.title).toContain("Career");
    expect(entry?.title).toContain("Rehearsal");
  ...
  it("describes both doors and states Simulator is not sponsor proof", () => {
    const titles = CAREER_REHEARSAL_HELP_DOOR_CARDS.map((card) => card.title);

    expect(titles).toContain("Career");
    expect(titles).toContain("Rehearsal");
