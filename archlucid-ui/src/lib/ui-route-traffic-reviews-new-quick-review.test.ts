...
import {
  ...
  REVIEWS_NEW_QUICK_REVIEW_TAB_TRAFFIC_SECTION,
} from "@/lib/ui-route-traffic-reviews-new-quick-review";

describe("ui-route-traffic-reviews-new-quick-review (REQ)", () => {
  it("tracks quick-review path tab with honest workbook notes", () => {
    const rows = extractMasterTableRows(readUiRouteTrafficEstimatesTemplateMarkdown());
    const row = rows.find((candidate) => candidate.id === REVIEWS_NEW_QUICK_REVIEW_TAB_TRAFFIC_ROW_ID);
