...
export const EXECUTIVE_SCORECARD_AVERAGE_MANUAL_REVIEW_HOURS = 3;

export type ExecutiveScorecardHoursSavedDisplay = {
  readonly valueText: string;
  readonly caption: string;
};

export function resolveExecutiveScorecardHoursSavedDisplay(input: {
  hoursRoi: number;
  reviewsCount: number;
  buyerPolished: boolean;
  precommitBlocksExact: boolean;
  averageManualReviewHours?: number;
}): ExecutiveScorecardHoursSavedDisplay {
  const averageManualReviewHours =
    input.averageManualReviewHours ?? EXECUTIVE_SCORECARD_AVERAGE_MANUAL_REVIEW_HOURS;
