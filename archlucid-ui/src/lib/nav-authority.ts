 ...
 *
 * @see `authority-seam-regression.test.ts` — `/me` claims → rank vs Enterprise nav vs mutation capability; config-wide Execute rows vs Read; Auditor Enterprise parity; rank **`0`** vs **`ReadAuthority`**; nav ordering after **`filterNavLinksByAuthority`**; **`/alerts`** **`essential`** tier; Enterprise nav **monotonicity** Read→Execute→Admin; Advanced default shell **`/ask`**-only; **`/governance`** behind extended+advanced at Execute rank; **`LAYER_PAGE_GUIDANCE`** Enterprise (**`enterpriseFootnote`**) vs Advanced (no footnote) for **`LayerHeader`** packaging parity.
 * @see `authority-execute-floor-regression.test.ts` — **one boolean for Execute-class:** synthetic **`ExecuteAuthority`** **`NavLinkItem`** visibility vs **`enterpriseMutationCapabilityFromRank`** across ranks; real **`alerts-governance`** monotonic filtered counts; Reader list keeps **`/alerts`**, omits **`/governance`**.
