/**
 * Deterministic executive ROI dashboard mock payloads for Playwright E2E.
 *
 * Deduplication scenario (mirrors `ExecutiveRoiSummaryServiceExtendedTests.BuildExportAsync_deduplicates_findings…`):
 * - The same stable finding id appears on two committed runs.
 * - Export and systemic-issue rollups count it once, not twice.
 */
export const EXECUTIVE_ROI_DEDUP_SCENARIO = {
    ...
    topSystemicIssues: [
      {
        ...
        severity: "Warning",
        count: EXECUTIVE_ROI_DEDUP_SCENARIO.deduplicatedSystemicIssueCount,
      },
      {
        category: "Security",
        severity: "Critical",
    ...
    historicalTrends: [
      {
        category: "CostOptimization",
        severity: "Warning",
        findingId: EXECUTIVE_ROI_DEDUP_SCENARIO.sharedFindingId,
        points: [
...
/** GET /v1/roi/executive-summary/export — deduplicated rows and environment slices. */
export function getExecutiveRoiExportMockJson(): unknown {
  return {
    rows: [
      {
        findingId: EXECUTIVE_ROI_DEDUP_SCENARIO.sharedFindingId,
        ...
        estimatedUsdSavings: EXECUTIVE_ROI_DEDUP_SCENARIO.deduplicatedProductionSavingsUsd,
      },
    ],
...
export function getExecutiveRoiHistoryMockJson(): unknown {
  return {
    points: [
      {
