...
test.describe("live-api-error-states", () => {
  ...
  test("run detail shows problem UI for non-existent run id", async ({ page }) => {
    ...
    // state (no page heading ΓÇö title is a `role="status"` `<strong>`, not a heading role) instead
  ...
  test("runs list page renders without error alerts (empty list is ok)", async ({ page }) => {
    ...
    // renamed to RUNS_LIST_PAGE_TITLES ("Review Packages") as part of the runs ΓåÆ reviews vocabulary
    // consolidation ΓÇö it no longer contains the literal substring "runs".
    await expect(
  ...
  test("governance dashboard loads without uncaught errors", async ({ page }) => {
    ...
    // overview" (buyer-polished) in ExecutiveWorkspaceHealthDashboard.tsx ΓÇö "Governance dashboard"
