...
export async function expectExecutiveRoiPortfolioPanels(page: Page): Promise<void> {
  ...
  await expect(page.getByTestId("exec-roi-identified-pending-usd")).toHaveTextContent("$120,000");
  await expect(page.getByTestId("exec-roi-realized-usd")).toHaveTextContent("$25,000");
