...
**Canonical buyer narrative:** [../docs/EXECUTIVE_SPONSOR_BRIEF.md](../docs/EXECUTIVE_SPONSOR_BRIEF.md) — outward sponsor story of record; keep UI-facing value lines consistent with it. Pilot measurement: [../docs/PILOT_ROI_MODEL.md](../docs/PILOT_ROI_MODEL.md).

**In-product guidance:** sidebar group **captions** summarize each layer; key Advanced Analysis / Enterprise routes render a **`LayerHeader`** strip (what question the page answers); **Home** lists Advanced/Enterprise **below** the Core Pilot checklist and marks them **not first-pilot proof**; after all checklist steps, **AfterCorePilotChecklistHint** nudges to analysis only when there is a real question; **run detail** shows an optional Advanced Analysis strip after commit (still not a success criterion). Enterprise pages add **short leads** (often **inspect vs change** and first-pilot deferral in `layer-guidance.ts` + page intros) so read-heavy context is not weighted the same as configuration forms. Full routing logic stays in **`docs/OPERATOR_DECISION_GUIDE.md`** (repo root).
...
**Enterprise mutation affordances:** `useEnterpriseMutationCapability()` (same **Execute+** threshold as `EnterpriseControlsExecutePageHint`) soft-disables governance dashboard/workflow and policy-pack **write** buttons for read-tier principals, with **`enterpriseMutationControlDisabledTitle`** tooltips — API enforcement unchanged (`@/lib/enterprise-mutation-capability`, `@/hooks/use-enterprise-mutation-capability`).

**Enterprise context copy:** `enterprise-controls-context-copy.ts` + `EnterpriseControlsNavGroupHint` / `EnterpriseControlsExecutePageHint` add short, rank-aware lines; **`LayerHeader`** adds optional **`enterpriseFootnote`** on governance dashboard, alerts, and audit (`layer-guidance.ts`). **Page-level rank cues:** `GovernanceResolutionRankCue`, `GovernanceDashboardReaderActionCue`, `AlertsInboxRankCue`, `AuditLogRankCue`, and `EnterpriseExecutePlusPageCue` in `EnterpriseControlsContextHints.tsx` add muted `role="note"` lines on governance resolution, governance dashboard (reader line + operator `Execute+` line), governance workflow, policy packs, **alerts inbox**, **audit log**, and alert operator tooling pages—without changing Core Pilot routes.
