...
**Seam contract (one sentence):** **`ArchLucid.Api`** **`[Authorize(Policy = …)]`** decides success; **`nav-config.ts`** + **`nav-shell-visibility.ts`** + **`useNavSurface()`** (**Visibility**) decide **sidebar/palette** inclusion after **tier then authority** filters (higher rank does not skip progressive disclosure); **`useOperateCapability()`** (**Capability**) decides **Operate POST/toggle** soft-enable — all three use the **same policy names** and the **same `AUTHORITY_RANK.ExecuteAuthority` floor** for “Execute-class” work.
...
- **`NEXT_PUBLIC_SELF_DEMO_URL`** — **`/welcome`** hero: **Try the self-demo** navigates in the **same tab** to this path or absolute URL (Workspace A product tour; default in code matches **`DemoWorkspaceStableIds.ProductTourArchitectureReviewRunId`**). Current-page UTM / click-id params are appended.
