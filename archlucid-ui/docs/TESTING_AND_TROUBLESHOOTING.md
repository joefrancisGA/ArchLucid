...
**Live accessibility (merge-blocking in `ui-e2e-live`):** **`e2e/live-api-accessibility.spec.ts`** runs **`@axe-core/playwright`** (**wcag22aa** tags via **`e2e/helpers/axe-helper.ts`**) across a route matrix that **always** includes golden-path operator URLs (home **`/`**, canonical **`/reviews`** list + detail, manifest detail) in the default CI subset, plus marketing and many authenticated shell routes. Critical and serious violations fail the job. Rare false positives may be suppressed in **`e2e/axe-rule-allowlist.ts`** with an **expiry date** and reason — **empty by default**. **`e2e/live-api-accessibility-focus.spec.ts`** covers skip-link focus, client-nav focus to **`#main-content`**, route announcer text, and a dark-mode axe smoke. **Automated axe scans are not a substitute for assistive-technology studies** (screen readers, voice control, etc.); they catch many regressions only in the DOM rules axe encodes. The live **`webServer`** passes through **`LIVE_API_URL`** (defaults to **`http://127.0.0.1:5128`**) and **`ARCHLUCID_PROXY_BEARER_TOKEN`** when set — see **`playwright.config.ts`**.
