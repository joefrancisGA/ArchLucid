...
**Source files:** `src/lib/nav-config.ts` (composed from `*-nav-group-builder.ts`), **`docs/../../archlucid-ui/docs/../../archlucid-ui/docs/../../archlucid-ui/docs/NAV_CONFIG_CONTRACT.md`** (tier / authority / drift contract), `src/lib/nav-authority.ts`, `src/lib/current-principal.ts`, `src/lib/nav-shell-visibility.ts`, `src/components/OperatorNavAuthorityProvider.tsx`, `src/lib/enterprise-controls-context-copy.ts`, `src/components/EnterpriseControlsContextHints.tsx`, `src/lib/layer-guidance.ts`. **Product doc:** [../../docs/PRODUCT_PACKAGING.md](../../docs/library/PRODUCT_PACKAGING.md).
...
| API returns empty collection | Dense operator hubs: `EnterpriseCompactEmptyState` per **TB-1552**; nested panels / rare first-run may use `OperatorEmptyState` / centered `EmptyState` when steps are the product |
...
- **Operator side rails (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator side rails* (**TB-1572**). Operator pages default to **single-column**. Persistent right columns are allowed only for named kinds (working-object sticky, master-detail, live preview/readiness when live, TOC/wizard). Teaching helpers, static scope columns, and about-asides are **not** free-form two-col layouts — demote or inline them (**TB-1573** Done: Recurrence helper collapsed disclosure; Advisory Schedules scope inline). Live preview/readiness rails stack when sparse and pin when live (**TB-1574** Done: Digests Schedule + Alert rules; `operator-live-preview-readiness-rail`). Hub inventory + about-aside demotion (**TB-1575** Done: `operator-side-rail-inventory.ts`; Teams/Slack/Azure Boards/ServiceNow single-column). Do not invent a second primary CTA in a rail. Remaining Vitest allowlist: **TB-1576**.
- **Operator primary CTA (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator primary CTA* (**TB-1539**). One page/tab job; at most one `Button variant="primary" size="sm"` in the first viewport (header + empty combined). Header order: Help → Primary → outline utilities. Exemplars: Digests browse (`DigestsHubClient`), Recurrence (`RecurrenceSchedulesClient`, **TB-1540**), Reviews hub (`ReviewsHubHeaderActions`, **TB-1541**), Advisory Schedules (**TB-1542**). Hub inventory: `operator-primary-cta-inventory.ts` (**TB-1543**); Vitest dual-primary guard **TB-1544**.
- **Operator empty states (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator empty states* (**TB-1552**). Name empty kind (collection / hub-zone / filtered / prerequisite / permission / error). Default collection + hub-zone → `EnterpriseCompactEmptyState` + presets (`enterprise-compact-empty-state-presets.ts`). Migration inventory: `operator-empty-state-migration-inventory.ts` (**TB-1554**). Ban form+rail+empty and checklist+preview+empty first-viewport stacks (**TB-1477**–**TB-1482**). CTAs align with **TB-1539**. `OperatorEmptyState` remains for nested panels and server-rendered empty collections.
- **Metadata chip taxonomy (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Metadata chip taxonomy* (**TB-2284**). Read-only status → `StatusTag` / `SeverityTag` / `BooleanStatusChip` (`--al-status-*`, neutral **TB-2277**). Interactive filters → `FilterChip`. Action counts → `Badge`. **`StatusPill` is deprecated** — no new imports; migrate under **TB-2286** / **TB-2287**; drift guard **TB-2289**. List status columns stay on **TB-1646** (`EnterpriseTable` + `StatusTag`).
- **Button variant/color matrix (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Button variant/color matrix* (**TB-2290**). Operator `Button` color via `variant` only — no inline `bg-teal-*` / semantic fill overrides; pairs **TB-1539** / **TB-2279** / **TB-2168**. Migrations **TB-2291**–**TB-2295**.
- **Operator populated lists (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator populated lists* (**TB-1646**). Name a list kind (inventory / master-detail / entity-summary / config-checklist / scale-table). Default inventory and master-detail to `EnterpriseTable` + `StatusTag`. Entity-summary cards only for low cardinality + nested detail (justify in PR notes). ≤2 visible row actions; buyer labels in primary columns; ban parallel raw HTML tables on operator hubs. Apply migrations: Advisory/Alert cards (**TB-1647**), Slack/Webhooks destinations (**TB-1648**), honesty + action budget (**TB-1649**). Do not invent a second table dialect beside `EnterpriseTable` / Audit scale-table.
- **Operator line tabs (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator line tabs — visual contract* (**TB-1661**). One boring Carbon line-tab look for `<Tabs>` on operator and buyer-polished shells: full-width bottom rule, plain 13px labels, 2px teal active underline. Ban `border-0` on `TabsList`, `rounded-full` / filled chip / segmented-tray / folder-tab chrome on `TabsTrigger`, and per-call-site override classes. Cross-route section switchers are nav (`Link` / `aria-current`), not `role="tab"`. Gold exemplars: `/architecture/digests`, Settings roles, `/architecture/reviews/new`, review detail workspace. Strip overrides and migrate pill call sites under **TB-1662**–**TB-1665** (semantics remain **TB-665** / **TB-671**).
- **Page-scoped help mount + interaction contract (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator page contextual help — mount + interaction contract* (**TB-1666**). Every sidebar destination and primary workflow page mounts `PageContextualHelpButton`, and it must resolve a non-null `pageHelpTopicForPathname` topic — a button that renders `null` is a defect. Help is **press-triggered** and opens a non-modal right drawer (`HelpDrawerContent` via `PageScopedContextualHelpPanel`). Field-level `HelpPopover` is not the page-header primitive. Never carry help in a native `title` attribute (hover-only; enforced by `no-restricted-syntax`, legacy baseline swept under **TB-2147**). Mount waves **TB-1667**–**TB-1670** Done.
- **Operator / marketing inline links (normative):** `docs/library/UI_DESIGN_SYSTEM.md` § *Operator / marketing inline links — affordance contract* (**TB-1671**). Because `globals.css` sets `a { color: inherit }`, every body/inline navigational `<Link>` or `<a>` must use `OPERATOR_LINK` (`nav` / `inline` / `optional` / `step*`) or `MARKETING_SURFACES.inlineLink` — resting underline or link-color token required; hover-only affordance banned. Ghost `Button asChild` wrappers around links are in scope (strip wrapper, apply link tokens; **TB-2170**–**TB-2173**). Filled/outline `Button asChild` CTAs, shell nav, and card/jump chrome stay excluded. Button-vs-link boundary: § *Visible-boundary `Button` contract* (**TB-2168**); a11y baseline `.cursor/rules/UI-Accessibility-Baseline.mdc`. Surface migrations: **TB-1672**–**TB-1675** ( **TB-1673** Done).
