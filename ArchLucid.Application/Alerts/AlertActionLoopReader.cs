...
public sealed class AlertActionLoopReader(IAlertRecordRepository alertRepository, IAlertDeliveryAttemptRepository deliveryAttemptRepository)
    : IAlertActionLoopReader
{
    ...
    public async Task<AlertActionLoopSnapshot?> GetAsync(Guid alertId, ScopeContext scope, CancellationToken cancellationToken)
    {
        ...
        List<AlertDeliveryAttemptSummary> rows = attempts.Select(static a => new AlertDeliveryAttemptSummary
        {
            ...
            Status = a.Status,
            AttemptedUtc = new DateTimeOffset(DateTime.SpecifyKind(a.AttemptedUtc, DateTimeKind.Utc), TimeSpan.Zero),
        ...
        return new AlertActionLoopSnapshot
        {
            ...
            RunId = alert.RunId,
            LastUpdatedUtc =
                alert.LastUpdatedUtc is null ? null : new DateTimeOffset(DateTime.SpecifyKind(alert.LastUpdatedUtc.Value, DateTimeKind.Utc), TimeSpan.Zero),
