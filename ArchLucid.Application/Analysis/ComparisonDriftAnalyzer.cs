            ...
            case JsonValueKind.Object:
                ...
                foreach (string prop in leftProps.Keys.Union(rightProps.Keys))
                {
                    ...
                    if (!leftProps.ContainsKey(prop))
                    {
                        items.Add(new DriftItem
                        {
                            Category = "Added", Path = $"{path}.{prop}", RegeneratedValue = rightProp.Value.ToString(), Description = "Property added."
                        });
                        continue;
                    }
                    ...
                    if (!rightProps.ContainsKey(prop))
                    {
                        items.Add(new DriftItem
                        {
                            Category = "Removed", Path = $"{path}.{prop}", StoredValue = leftProp.Value.ToString(), Description = "Property removed."
                        });
                        continue;
                    }
