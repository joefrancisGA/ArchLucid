...
public sealed class CapabilityToFlowEngine(
    ...
    ILogger<CapabilityToFlowEngine> logger) : ICapabilityToFlowEngine
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (CapabilityToFlowCandidate candidate in candidates)
        {
            Guid pathId = Guid.NewGuid();
            IReadOnlyList<SecurityEvidencePathHopRecord> hopRecords =
                CapabilityToFlowMaterializer.BuildHopRecords(scope.TenantId, pathId, snapshotId, candidate.Hops);
            ...
            SecurityEvidencePathRecord header = new()
            {
                ...
                PathKind = PathKind.CapabilityToFlow,
            ...
            {
                logger.LogWarning(ex, "Capability-to-flow path persist failed for SnapshotId={SnapshotId}.", snapshotId);
            ...
            PrivilegePathSeverityResult severity = CapabilityToFlowSeverityTable.Resolve(candidate);
            string title = CapabilityToFlowNarrative.BuildTitle(candidate);
            string description = CapabilityToFlowNarrative.BuildDescription(candidate);
            ...
            ingestItems.Add(new OperationalSecurityFindingIngestItem
            {
                ...
                ExternalResourceId = candidate.DataAssetNodeId,
                ResourceType = candidate.TerminalResourceType,
                SubscriptionOrAccountId = snapshot.Header.SubscriptionId,
                ControlId = SecureNowArchitectConstants.CapabilityToFlowControlId,
                ...
                {
                    ["pathKind"] = PathKind.CapabilityToFlow.ToString(),
                    ["hasUnrestrictedEgressHop"] = candidate.HasUnrestrictedEgressHop.ToString(),
