...
public sealed class TrialTenantBootstrapService(IDemoSeedService demoSeedService, ITenantRepository tenantRepository, IAuditService auditService, ITrialBootstrapEmailVerificationPolicy emailVerificationPolicy, ILogger<TrialTenantBootstrapService> logger) : ITrialTenantBootstrapService
{
    ...
    private static byte __ValidatePrimaryConstructorArguments(ArchLucid.Application.Bootstrap.IDemoSeedService demoSeedService, ArchLucid.Core.Tenancy.ITenantRepository tenantRepository, ArchLucid.Core.Audit.IAuditService auditService, ArchLucid.Application.Identity.ITrialBootstrapEmailVerificationPolicy emailVerificationPolicy, Microsoft.Extensions.Logging.ILogger<ArchLucid.Application.Tenancy.TrialTenantBootstrapService> logger)
    {
        ...
        ArgumentNullException.ThrowIfNull(emailVerificationPolicy);
    ...
    private readonly ITrialBootstrapEmailVerificationPolicy _emailVerificationPolicy = emailVerificationPolicy ?? throw new ArgumentNullException(nameof(emailVerificationPolicy));
    ...
    /// <inheritdoc/>
    public async Task TryBootstrapAfterSelfRegistrationAsync(TenantProvisioningResult result, string auditActorEmail, TrialSignupBaselineReviewCycleCapture? baselineReviewCycle, TrialSignupCompanyProfileCapture? companyProfile, CancellationToken cancellationToken)
    {
        ...
        if (!await _emailVerificationPolicy.CanProvisionTrialForRegisteredEmailAsync(auditActorEmail, cancellationToken))
        {
            if (_logger.IsEnabled(LogLevel.Information))
                _logger.LogInformation("Skipping trial bootstrap for tenant {TenantId}: email verification policy blocked provisioning for {Email}.", result.TenantId, auditActorEmail);
            ArchLucidInstrumentation.RecordTrialSignupFailure("email_verification", "policy_blocked");
            await _auditService.LogAsync(new AuditEvent { EventType = AuditEventTypes.TrialSignupFailed, ActorUserId = auditActorEmail.Trim(), ActorUserName = auditActorEmail.Trim(), TenantId = result.TenantId, WorkspaceId = result.DefaultWorkspaceId, ProjectId = result.DefaultProjectId, DataJson = JsonSerializer.Serialize(new { stage = "email_verification", reason = "policy_blocked" }) }, cancellationToken);
            return;
        }
        ...
        using (AmbientScopeContext.Push(scope))
            try
            {
                ...
                await _auditService.LogAsync(new AuditEvent { EventType = AuditEventTypes.TrialProvisioned, ActorUserId = actor, ActorUserName = actor, TenantId = result.TenantId, WorkspaceId = result.DefaultWorkspaceId, ProjectId = result.DefaultProjectId, DataJson = JsonSerializer.Serialize(new { trialExpiresUtc = expires, sampleRunId = demoIds.AuthorityRunBaselineId }) }, cancellationToken);
            ...
            catch (Exception ex)
            {
                if (_logger.IsEnabled(LogLevel.Error))
                    _logger.LogError(ex, "Trial bootstrap failed for tenant {TenantId}; tenant row exists without trial metadata.", result.TenantId);
                ArchLucidInstrumentation.RecordTrialSignupFailure("trial_bootstrap", ex.GetType().Name);
                await _auditService.LogAsync(new AuditEvent { EventType = AuditEventTypes.TrialSignupFailed, ActorUserId = auditActorEmail.Trim(), ActorUserName = auditActorEmail.Trim(), TenantId = result.TenantId, WorkspaceId = result.DefaultWorkspaceId, ProjectId = result.DefaultProjectId, DataJson = JsonSerializer.Serialize(new { stage = "trial_bootstrap", reason = ex.GetType().Name }) }, cancellationToken);
            }
    }
}