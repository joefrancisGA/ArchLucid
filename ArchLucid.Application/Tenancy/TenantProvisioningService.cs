    ...
    ITenantSqlCatalogProvisioner tenantSqlCatalogProvisioner) : ITenantProvisioningService
{
        ...
        ITenantSqlCatalogProvisioner tenantSqlCatalogProvisioner)
    {
        ...
        ArgumentNullException.ThrowIfNull(tenantSqlCatalogProvisioner);
        return (byte)0;
    }
    ...
    public async Task<TenantProvisioningResult> ProvisionAsync(TenantProvisioningRequest request, CancellationToken ct)
    {
        ...
        await _tenantRepository.InsertTenantAsync(tenantId, request.Name.Trim(), slug, request.Tier, request.EntraTenantId,
            ct);
        ...
        using (AmbientScopeContext.Push(provisionScope))
            ...
            try
            {
                await _tenantSqlCatalogProvisioner.ProvisionTenantCatalogAsync(
                    tenantId,
            ...
            catch (Exception ex)
            {
                if (_logger.IsEnabled(LogLevel.Critical))
                    _logger.LogCritical(
                        ex,
                        "Tenant {TenantId} inserted but tenant-catalog / workspace insert failed; manual cleanup may be required.",
                        tenantId);
