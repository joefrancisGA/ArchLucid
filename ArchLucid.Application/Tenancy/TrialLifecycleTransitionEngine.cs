...
public sealed class TrialLifecycleTransitionEngine(ITenantRepository tenantRepository, ITenantHardPurgeService tenantHardPurgeService, IAuditService auditService, IOptionsMonitor<TrialLifecycleSchedulerOptions> lifecycleOptions, TimeProvider timeProvider, ILogger<TrialLifecycleTransitionEngine> logger)
{
    ...
    private readonly ITenantHardPurgeService _tenantHardPurgeService = tenantHardPurgeService ?? throw new ArgumentNullException(nameof(tenantHardPurgeService));
    ...
    public async Task<bool> TryAdvanceTenantAsync(Guid tenantId, CancellationToken cancellationToken)
    {
        ...
        {
            TenantHardPurgeResult retry = await _tenantHardPurgeService.PurgeTenantAsync(tenantId, new TenantHardPurgeOptions { DryRun = false, MaxRowsPerStatement = options.HardPurgeMaxRowsPerStatement }, cancellationToken);
        ...
        {
            bool recorded = await _tenantRepository.TryRecordTrialLifecycleTransitionAsync(tenantId, advancement.FromStatus, advancement.ToStatus, advancement.Reason, cancellationToken);
            if (!recorded)
                return false;
            ...
            TenantHardPurgeResult purgeResult = await _tenantHardPurgeService.PurgeTenantAsync(tenantId, new TenantHardPurgeOptions { DryRun = false, MaxRowsPerStatement = options.HardPurgeMaxRowsPerStatement }, cancellationToken);
            _logger.LogInformation("Trial hard purge completed for tenant {TenantId}: rowsDeleted={Rows}.", tenantId, purgeResult.RowsDeleted);
            return true;
        }

        bool ok = await _tenantRepository.TryRecordTrialLifecycleTransitionAsync(tenantId, advancement.FromStatus, advancement.ToStatus, advancement.Reason, cancellationToken);
        if (!ok)
            return false;
        await EmitAuditAsync(tenant, advancement, cancellationToken);
        ArchLucidInstrumentation.RecordTrialExpiration($"{advancement.FromStatus}->{advancement.ToStatus}");
    ...
    private async Task EmitAuditAsync(TenantRecord tenant, TrialLifecycleAdvancement advancement, CancellationToken cancellationToken)
    {
        ...
        await _auditService.LogAsync(new AuditEvent { EventType = AuditEventTypes.TrialLifecycleTransition, ActorUserId = "system:trial-lifecycle-scheduler", ActorUserName = "system:trial-lifecycle-scheduler", TenantId = tenant.Id, WorkspaceId = workspaceId, ProjectId = projectId, DataJson = JsonSerializer.Serialize(new { fromStatus = advancement.FromStatus, toStatus = advancement.ToStatus, reason = advancement.Reason }) }, cancellationToken);
    }
}