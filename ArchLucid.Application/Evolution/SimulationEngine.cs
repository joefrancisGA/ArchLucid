...
public sealed class SimulationEngine(IArchitectureAnalysisService analysisService) : ISimulationEngine
{
    ...
    public async Task<SimulationResult> SimulateAsync(SimulationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        ArchitectureAnalysisReport baselineReport =
            await analysisService.BuildAsync(ToAnalysisRequest(request.BaselineArchitectureRunId, baselineProfile), cancellationToken);
    ...
    private static SimulationDiff BuildDiff(CandidateChangeSet candidate, string baselineRunId, bool singlePass, ArchitectureAnalysisReport baseline,
    ...
    private static string BuildDiffSummary(bool singlePass, ArchitectureAnalysisReport baseline, ArchitectureAnalysisReport simulated,
        ManifestDiffResult? manifestDiff)
    {
        if (singlePass)
            return string.Format(CultureInfo.InvariantCulture, "Single read-only pass: warnings={0}, summary length={1}.", baseline.Warnings.Count,
                baseline.Summary?.Length ?? 0);
        string manifestLine = manifestDiff is null
            ? "No manifest diff in simulated pass."
            : string.Format(CultureInfo.InvariantCulture, "Manifest diff: +{0}/-{1} services, +{2}/-{3} datastores.", manifestDiff.AddedServices.Count,
                manifestDiff.RemovedServices.Count, manifestDiff.AddedDatastores.Count, manifestDiff.RemovedDatastores.Count);
        return string.Format(CultureInfo.InvariantCulture, "Baseline warnings={0}, simulated warnings={1}. {2}", baseline.Warnings.Count,
            simulated.Warnings.Count, manifestLine);
    }
    ...
    private static EvaluationScore BuildScores(ArchitectureAnalysisReport baseline, ArchitectureAnalysisReport simulated, bool singlePass)
    {
        ...
        return new EvaluationScore { SimulationScore = simulationScore, DeterminismScore = null, RegressionRiskScore = regressionRisk };
    }
