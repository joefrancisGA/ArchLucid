    ...
    : ISimulationEvaluationService
{
    ...
    /// <inheritdoc />
    public async Task<SimulationEvaluationResult> EvaluateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        (ManifestDiffResult? diff, bool usedPrecomputedManifestDiff, bool usedComputedManifestDiff) =
            ResolveManifestDiff(baseline, simulated);

        DeterminismResolution determinismResolution =
            await ResolveDeterminismAsync(request, options, cancellationToken);
        ...
        double confidenceScore = ComputeConfidenceScore(
            baseline,
            simulated,
            diff,
            determinismResolution.Result,
            options);

        double simulationScore = ComputeSimulationScore(
            improvementDelta,
            determinismScore,
            regressionRisk,
            confidenceScore);
        ...
        string regressionPart = regressionRisk.HasValue
            ? string.Format(CultureInfo.InvariantCulture, "{0:F3}", regressionRisk.Value)
            : "n/a";

        string summary = string.Format(
            CultureInfo.InvariantCulture,
            "Rule={0}; ImprovementDelta={1:F3}; SimulationScore={2:F3}; Determinism={3}; RegressionRisk={4}; Confidence={5:F3}; Signals={6}",
            RuleVersion,
            improvementDelta,
            simulationScore,
            FormatDeterminism(determinismScore),
            regressionPart,
            confidenceScore,
    ...
    private async Task<DeterminismResolution> ResolveDeterminismAsync(
        SimulationEvaluationRequest request,
    ...
    private static List<string> BuildRegressionSignals(
        ManifestDiffResult? diff,
        DeterminismCheckResult? determinism)
    {
        List<string> signals = [];
        ...
        ManifestDiffResult? diff)
    {
        ...
        int adds =
            ...
            diff.AddedRequiredControls.Count;
    ...
    private static double ComputeSimulationScore(
        double improvementDelta,
        double? determinismScore,
        double? regressionRisk,
        double confidenceScore)
    {
        ...
        return Math.Clamp(
            normImprovement * 0.35 + det * 0.35 + (1.0 - reg) * 0.2 + confidenceScore * 0.1,
            0,
            1);
    }
