...
{
    public required ArchitectureAnalysisReport BaselineReport { get; init; }

    public ArchitectureAnalysisReport? SimulatedReport { get; init; }

    /// <summary>When set, takes precedence over <see cref="ArchitectureAnalysisReport.Determinism"/> on the baseline report.</summary>
    public DeterminismCheckResult? SuppliedDeterminism { get; init; }

    /// <summary>Optional run id for live determinism when options do not specify one.</summary>
    public string? BaselineArchitectureRunId { get; init; }

    public SimulationEvaluationOptions? Options { get; init; }
}
