...
{
    private readonly byte __primaryConstructorArgumentValidation = __ValidatePrimaryConstructorArguments(runDetailQueryService, architectureAnalysisService);
    ...
    public async Task<ArchitectureAnalysisReport> ExecuteAsync(ShadowExecutionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        ArchitectureRunDetail? loaded = await runDetailQueryService.GetRunDetailAsync(request.BaselineArchitectureRunId, cancellationToken);
        if (loaded is null)
