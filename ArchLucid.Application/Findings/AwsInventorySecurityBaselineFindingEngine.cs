...
/// <summary>Deterministic security-baseline findings from scoped AWS inventory ZIP (TB-2262).</summary>
public sealed class AwsInventorySecurityBaselineFindingEngine(
    ...
    IOptions<RoiCostEvidenceFreshnessOptions> freshnessOptions) : IEffectfulFindingEngine
{
    ...
    public string EngineType => "aws-inventory-security-baseline";
    ...
    public async Task<IReadOnlyList<Finding>> AnalyzeAsync(GraphSnapshot graphSnapshot, CancellationToken ct)
    {
        ...
        InventoryTopologyResourceNodeIndex topologyNodes =
            InventoryTopologyResourceNodeIndex.Build(graphSnapshot, InventoryTopologyCloudProvider.Aws);

        return gaps
            .Select(gap => InventorySecurityBaselineFindingMapper.ToFinding(
                gap,
                EngineType,
                "AwsInventorySecurityBaseline",
                "AWS",
                topologyNodes))
            .ToList();
    }
