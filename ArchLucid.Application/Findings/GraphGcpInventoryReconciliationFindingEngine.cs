...
/// <summary>Deterministic graph ↔ GCP inventory reconciliation findings (TB-2248).</summary>
public sealed class GraphGcpInventoryReconciliationFindingEngine(
    ...
    IOptions<RoiCostEvidenceFreshnessOptions> freshnessOptions) : IFindingEngine
{
    ...
    public string EngineType => "gcp-inventory-reconciliation";
    ...
    public async Task<IReadOnlyList<Finding>> AnalyzeAsync(GraphSnapshot graphSnapshot, CancellationToken ct)
    {
        ...
        InventoryReconciliationResult reconciliation =
            GraphGcpInventoryReconciliationAnalyzer.Analyze(resourcesJson, graphSnapshot);
        ...
        [
            new Finding
            {
                ...
                Title = "Topology graph and GCP inventory are out of sync",
                Rationale =
                ...
                [
                    ...
                    "Remove or re-label graph resources that are not present in the scoped GCP inventory."
                ],
                Trace = new ExplainabilityTrace
                {
                    GraphNodeIdsExamined = graphOnlyNodeIds.ToList(),
                    RulesApplied = ["graph-gcp-inventory-reconciliation"],
                    DecisionsTaken =
                    [
                        "Compared topology resource identifiers to scoped GCP inventory resources.json rows."
                    ],
                    AlternativePathsConsidered =
                    [
                        "Refresh the GCP inventory package before reconciling if collection is stale.",
        ...
        InventoryReconciliationResult reconciliation)
    {
        ...
        {
            string? resourceId = GraphGcpInventoryReconciliationAnalyzer.TryReadTopologyResourceId(node);

            if (string.IsNullOrWhiteSpace(resourceId))
                continue;

            string normalized = GraphGcpInventoryReconciliationAnalyzer.NormalizeGcpResourceId(resourceId);
