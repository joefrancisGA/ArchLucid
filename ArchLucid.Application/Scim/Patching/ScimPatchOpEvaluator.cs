...
{
    public static IReadOnlyDictionary<string, JsonElement> ApplyFlat(IReadOnlyDictionary<string, JsonElement> current, JsonElement patchDocument)
    {
    ...
    private static void ApplyOne(Dictionary<string, JsonElement> target, JsonElement op)
    {
        ...
        if (!string.Equals(opName, "add", StringComparison.OrdinalIgnoreCase) && !string.Equals(opName, "replace", StringComparison.OrdinalIgnoreCase))
