...
/// </summary>
public sealed class ScimTokenRotationReminderJob(IServiceScopeFactory scopeFactory, IOptions<ScimOptions> options, ILogger<ScimTokenRotationReminderJob> logger)
    : BackgroundService
{
    ...
    private async Task RunReminderScanAsync(CancellationToken ct)
    {
        ...
        foreach (ScimTokenRotationCandidate row in due)
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning("archlucid.scim.token.rotation_due tenantId={TenantId} tokenId={TokenId} createdUtc={CreatedUtc:o}", row.TenantId, row.Id,
                    row.CreatedUtc);
            string dataJson = JsonSerializer.Serialize(new { tenantId = row.TenantId, tokenId = row.Id, createdUtc = row.CreatedUtc });
            await notices.InsertAsync("scim_token_rotation_due",
                ...
                dataJson, ct).ConfigureAwait(false);
        }
    }
}
