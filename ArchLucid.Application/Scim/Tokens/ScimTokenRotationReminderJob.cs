...
{
    private readonly ILogger<ScimTokenRotationReminderJob> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    private async Task RunReminderScanAsync(CancellationToken ct)
    {
        ...
        IReadOnlyList<ScimTokenRotationCandidate> due = await tokens.ListActiveCreatedOnOrBeforeAsync(cutoffUtc, ct).ConfigureAwait(false);
        foreach (ScimTokenRotationCandidate row in due)
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning("archlucid.scim.token.rotation_due tenantId={TenantId} tokenId={TokenId} createdUtc={CreatedUtc:o}", row.TenantId, row.Id, row.CreatedUtc);
            string dataJson = JsonSerializer.Serialize(new { tenantId = row.TenantId, tokenId = row.Id, createdUtc = row.CreatedUtc });
            await notices.InsertAsync("scim_token_rotation_due", $"SCIM bearer token {row.Id:D} for tenant {row.TenantId:D} is older than the configured rotation reminder ({reminderDays} days). Rotate or revoke in admin.", dataJson, ct).ConfigureAwait(false);
        }
    }
}