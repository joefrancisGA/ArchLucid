...
public sealed class ScimUserService(IScimUserRepository users, ITenantRepository tenants, IGroupToRoleMapper roleMapper, IAuditService audit) : IScimUserService
{
    ...
    /// <inheritdoc/>
    public async Task<(IReadOnlyList<ScimUserRecord> items, int totalResults)> ListAsync(Guid tenantId, string? filter, int startIndex, int count, CancellationToken cancellationToken)
    {
    ...
    public async Task<ScimUserRecord> CreateAsync(Guid tenantId, JsonElement resource, CancellationToken cancellationToken)
    {
        ...
        ScimUserRecord created = await _users.InsertAsync(tenantId, externalId, userName, displayName, active, null, ScimResolvedRoleOrigin.Unknown, cancellationToken);
        ...
        await LogAsync(tenantId, AuditEventTypes.ScimUserProvisioned, $"{{\"userId\":\"{created.Id:D}\",\"externalId\":\"{JsonEncoded(externalId)}\"}}", cancellationToken);
        return created;
    }
    ...
    private ResolveRoleChoices DecideResolvedRole(ScimUserRecord existing, string? manualFromRequest, string? groupMapped)
    {
        if (groupMapped is null)
            return manualFromRequest is not null ? new ResolveRoleChoices(manualFromRequest, ScimResolvedRoleOrigin.Manual, false) : new ResolveRoleChoices(existing.ResolvedRole, existing.ResolvedRoleOrigin, false);
        bool fire = existing.ResolvedRoleOrigin == ScimResolvedRoleOrigin.Manual && !string.Equals(existing.ResolvedRole, groupMapped, StringComparison.OrdinalIgnoreCase);
