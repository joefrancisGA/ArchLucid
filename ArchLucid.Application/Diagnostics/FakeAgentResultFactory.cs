...
public static class FakeAgentResultFactory
{
    public static AgentResult CreateTopologyResult(string runId, string taskId, ArchitectureRequest request)
    {
        ...
        return new AgentResult
        {
            ...
            Claims = [$"Use App Service for the primary API for system '{request.SystemName}'.", "Use Azure AI Search for enterprise retrieval.", "Use SQL Server for metadata storage."],
            EvidenceRefs = ["request", "service-catalog:azure-core-services", "service-catalog:azure-ai-search", "service-catalog:azure-sql"],
            Confidence = 0.91,
            Findings = [new ArchitectureFinding
            {
            ...
            ProposedChanges = new ManifestDeltaProposal
            {
                ...
                SourceAgent = AgentType.Topology,
                AddedServices = [new ManifestService
                {
                    ServiceId = "svc-api",
                    ServiceName = "rag-api",
                ...
                AddedDatastores = [new ManifestDatastore
                {
                ...
                AddedRelationships = [new ManifestRelationship
                {
    ...
    public static AgentResult CreateCostResult(string runId, string taskId, ArchitectureRequest _)
    {
        ...
        return new AgentResult
        {
            ...
            Claims = ["App Service is likely a lower operational overhead option than AKS for the initial MVP.", "Azure AI Search cost should be monitored as index size and query volume increase.", "Managed platform choices reduce support burden at modest additional platform cost."],
    ...
    public static AgentResult CreateComplianceResult(string runId, string taskId, ArchitectureRequest request)
    {
        ...
        if (!request.RequiredCapabilities.Any(x => x.Contains("private", StringComparison.OrdinalIgnoreCase)))
            return new AgentResult
            {
                ...
                Claims = ["Managed identity is required for service-to-service authentication.", "Private endpoints are required for data-bearing services.", "Secrets should be externalized into Key Vault."],
    ...
    public static AgentResult CreateCriticResult(string runId, string taskId, ArchitectureRequest request)
    {
        ...
        return new AgentResult
        {
            ...
            Claims = [$"The proposed topology for '{request.SystemName}' uses well-understood Azure services with no obvious architectural anti-patterns.", "Cost estimate assumptions are reasonable for an MVP scope; revisit as load increases.", "Compliance controls are aligned with the stated policy baseline."],
    ...
    public static IReadOnlyList<AgentResult> CreateStarterResults(string runId, IReadOnlyCollection<AgentTask> tasks, ArchitectureRequest request)
    {
        ...
        ArgumentNullException.ThrowIfNull(request);
        AgentTask topologyTask = tasks.FirstOrDefault(t => t.AgentType == AgentType.Topology) ?? throw new InvalidOperationException("Topology task was not found.");
        ...
        AgentTask complianceTask = tasks.FirstOrDefault(t => t.AgentType == AgentType.Compliance) ?? throw new InvalidOperationException("Compliance task was not found.");
        ...
        return[CreateTopologyResult(runId, topologyTask.TaskId, request), CreateCostResult(runId, costTask.TaskId, request), CreateComplianceResult(runId, complianceTask.TaskId, request), CreateCriticResult(runId, criticTask.TaskId, request)];
    }
}