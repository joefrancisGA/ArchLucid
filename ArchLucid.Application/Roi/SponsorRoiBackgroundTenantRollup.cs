...
/// <summary>
///     Iterates active tenants for leader-elected Sponsor ROI background work with ambient scope + fail-closed guard.
/// </summary>
public static class SponsorRoiBackgroundTenantRollup
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (TenantRecord tenant in tenants)
        {
            ...
            if (!SponsorRoiBackgroundScopeGuard.TryValidate(tenantScope, out string reason))
            {
                SponsorRoiBackgroundScopeTelemetry.RecordViolation(reason);

                if (logger.IsEnabled(LogLevel.Warning))
                {
                    logger.LogWarning(
                        "Sponsor ROI background job skipped tenant {TenantId}: invalid scope ({Reason}).",
                        tenant.Id,
                        reason);
                }
