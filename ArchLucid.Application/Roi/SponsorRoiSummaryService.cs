...
public sealed class SponsorRoiSummaryService(
    ...
    ILogger<SponsorRoiSummaryService> logger) : ISponsorRoiSummaryService
{
    ...
    private readonly ILogger<SponsorRoiSummaryService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<SponsorRoiSummaryResponse> BuildAsync(CancellationToken cancellationToken = default)
    {
        ...
        SponsorRoiBasisBreakdown basisBreakdown = await BuildBasisBreakdownAsync(
            latestDetails,
            tenantId,
            projectId,
    ...
    private async Task<SponsorRoiBasisBreakdown> BuildBasisBreakdownAsync(
    ...
    public async Task<CrossTenantPortfolioSummaryResponse> GetCrossTenantPortfolioSummaryAsync(string userDirectoryKey, CancellationToken cancellationToken = default)
    {
        ...
        foreach (Guid tenantId in accessibleTenantIds)
        {
            ...
            SponsorRoiBasisBreakdown tenantBasis = await BuildBasisBreakdownAsync(
                latestDetails,
                tenantId,
                projectId: null,
                cancellationToken).ConfigureAwait(false);
