...
/// <summary>
///     Maps finding categories into sponsor dashboard business-impact pillars (TB-105).
/// </summary>
public static class SponsorBusinessImpactCategoryClassifier
{
    /// <summary>Counts deduplicated active findings into named sponsor-facing pillars.</summary>
    public static SponsorBusinessImpactCategoryCounts Build(IEnumerable<ArchitectureFinding> dedupedFindings)
    {
        ...
        return new SponsorBusinessImpactCategoryCounts
        {
    ...
    {
        if (MatchesAny(category, SponsorBusinessImpactPillarMatchers.Security))
            return "Security";

        if (MatchesAny(category, SponsorBusinessImpactPillarMatchers.Compliance))
            return "Compliance";

        if (MatchesAny(category, SponsorBusinessImpactPillarMatchers.Reliability))
            return "Reliability";

        if (MatchesAny(category, SponsorBusinessImpactPillarMatchers.Cost))
            return "Cost";

        if (MatchesAny(category, SponsorBusinessImpactPillarMatchers.Governance))
            return "Governance";
