...
public static class RoiEvidenceCompletenessMarkdownFormatter
{
    ...
    internal static (string Headline, string Body) Describe(ValueReportSnapshot snapshot) =>
        snapshot.ReviewCycleBaselineProvenance switch
        {
            ReviewCycleBaselineProvenance.TenantSuppliedAtSignup => DescribeTenantCaptured(
                snapshot,
                "Strong",
                "Tenant supplied baseline review-cycle hours at signup"),

            ReviewCycleBaselineProvenance.TenantSuppliedViaSettings => DescribeTenantCaptured(
                snapshot,
                "Strong",
                "Tenant maintained baseline inputs via baseline settings"),
    ...
    private static (string Headline, string Body) DescribeTenantCaptured(
        ...
        string headlinePrefix)
    {
        ...
        if (snapshot.TenantBaselineReviewCycleCapturedUtc is { } cap)
            ...
            parts.Add(
                $"**Captured UTC:** `{cap.ToString("O", CultureInfo.InvariantCulture)}`.");
