...
public sealed class TenantMeasuredRoiService(IWhyArchLucidSnapshotService snapshotService, ITenantCostEstimateService costEstimateService)
    : ITenantMeasuredRoiService
{
    ...
    public async Task<TenantMeasuredRoiSummary> GetAsync(Guid tenantId, CancellationToken cancellationToken = default)
    {
        ...
        string disclaimer = "Process counters are cumulative since this API replica started (not a billing invoice). " +
