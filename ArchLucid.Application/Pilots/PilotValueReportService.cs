    ...
    ILogger<PilotValueReportService> logger) : IPilotValueReportService
{
    ...
    private static readonly HashSet<string> PolicyPackAssignmentEventTypes =
    [
        AuditEventTypes.PolicyPackAssigned,
        AuditEventTypes.PolicyPackAssignmentCreated
    ];

    private static readonly HashSet<string> ComparisonDriftEventTypes =
    [
        AuditEventTypes.ComparisonSummaryPersisted,
        AuditEventTypes.AlertRuleCandidateComparisonExecuted,
        AuditEventTypes.GovernanceConflictDetected,
        AuditEventTypes.GovernancePreCommitSimulationEvaluated,
        AuditEventTypes.ReplayExecuted,
        ...
        CancellationToken cancellationToken)
    {
        ...
        GovernanceDashboardSummary dashboard =
            await _governanceDashboardService
                .GetDashboardAsync(scope.TenantId, 50, 50, 50, cancellationToken)
                .ConfigureAwait(false);
        ...
        int recommendations = auditRows.Count(e =>
            string.Equals(e.EventType, AuditEventTypes.RecommendationGenerated, StringComparison.Ordinal));
        ...
        if (runDetailsTruncated)
        {
            ...
            {
                _logger.LogWarning(
                    "Pilot value report: loading run details for {Loaded} of {Total} committed runs (cap {Cap}).",
                    runsForDetails.Count,
                    committedRuns.Count,
                    DefaultRunDetailCap);
            }
        }
        ...
        foreach (CommittedRunRef runRef in runsForDetails)
        {
            ArchitectureRunDetail? detail =
                await _runDetailQuery.GetRunDetailAsync(runRef.RunId, cancellationToken).ConfigureAwait(false);
        ...
        double? avgSeconds = completionSeconds.Count > 0
            ? completionSeconds.Average()
            : null;
