    ...
    ILogger<RecentPilotRunDeltasService> logger) : IRecentPilotRunDeltasService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        double? medianSeconds = ComputeMedian(
            rows
                .Where(r => r.TimeToCommittedManifestTotalSeconds is { } s && double.IsFinite(s) && s >= 0)
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            bool isDemo =
                deltas.IsDemoTenant
                || ContosoRetailDemoIdentifiers.IsDemoRunId(summary.RunId)
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogWarning(
                ex,
                "Recent pilot run deltas: skipping run {RunId} because the per-run delta projection failed.",
                summary.RunId);
