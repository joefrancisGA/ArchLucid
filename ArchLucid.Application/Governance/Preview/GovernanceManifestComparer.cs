...
public static class GovernanceManifestComparer
{
    ...
    private static string? NormalizeList(IReadOnlyList<string>? list)
    {
        if (list is null || list.Count == 0)
            return null;
        List<string> ordered = list.Where(s => !string.IsNullOrWhiteSpace(s)).Select(s => s.Trim()).OrderBy(s => s, StringComparer.OrdinalIgnoreCase).ToList();
        return ordered.Count == 0 ? null : string.Join(", ", ordered);
    }

    private static List<GovernanceDiffItem> CompareDictionaries(IReadOnlyDictionary<string, string?> current, IReadOnlyDictionary<string, string?> preview)
    {
