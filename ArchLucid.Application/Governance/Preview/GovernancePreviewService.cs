...
public sealed class GovernancePreviewService(IGovernanceEnvironmentActivationRepository activationRepository, IRunDetailQueryService runDetailQueryService, IUnifiedGoldenManifestReader unifiedGoldenManifestReader) : IGovernancePreviewService
{
    ...
    public async Task<GovernancePreviewResult> PreviewActivationAsync(GovernancePreviewRequest request, CancellationToken cancellationToken = default)
    {
        ...
        ArchitectureRunDetail runDetail = await runDetailQueryService.GetRunDetailAsync(request.RunId, cancellationToken) ?? throw new RunNotFoundException(request.RunId);
        ...
        GoldenManifest? candidateManifest = runDetail.Manifest is not null && string.Equals(runDetail.Run.CurrentManifestVersion, request.ManifestVersion, StringComparison.Ordinal) ? runDetail.Manifest : await unifiedGoldenManifestReader.GetByVersionAsync(request.ManifestVersion, cancellationToken);
        if (candidateManifest is null)
            throw new InvalidOperationException($"Golden manifest version '{request.ManifestVersion}' was not found for run '{request.RunId}'. " + "Ensure the manifest version belongs to this run and has been committed.");
        if (!string.Equals(candidateManifest.RunId, request.RunId, StringComparison.Ordinal))
            throw new InvalidOperationException($"Manifest version '{request.ManifestVersion}' belongs to run '{candidateManifest.RunId}', not '{request.RunId}'.");
        ...
        {
            notes.Add($"Compared current run '{active.RunId}' (manifest '{active.ManifestVersion}') to preview run '{request.RunId}' (manifest '{request.ManifestVersion}').");
    ...
    public async Task<GovernanceEnvironmentComparisonResult> CompareEnvironmentsAsync(GovernanceEnvironmentComparisonRequest request, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);
        ...
        GoldenManifest? sourceManifest = sourceActive is not null ? await unifiedGoldenManifestReader.GetByVersionAsync(sourceActive.ManifestVersion, cancellationToken) : null;
        GoldenManifest? targetManifest = targetActive is not null ? await unifiedGoldenManifestReader.GetByVersionAsync(targetActive.ManifestVersion, cancellationToken) : null;
    ...
    {
        return string.Equals(value, GovernanceEnvironment.Dev, StringComparison.OrdinalIgnoreCase) || string.Equals(value, GovernanceEnvironment.Test, StringComparison.OrdinalIgnoreCase) || string.Equals(value, GovernanceEnvironment.Prod, StringComparison.OrdinalIgnoreCase);
    }
}