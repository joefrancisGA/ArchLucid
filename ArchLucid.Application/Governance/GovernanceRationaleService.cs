...
public sealed class GovernanceRationaleService(IGovernanceLineageService lineageService) : IGovernanceRationaleService
{
    ...
    public async Task<GovernanceRationaleResult?> GetApprovalRequestRationaleAsync(string approvalRequestId, CancellationToken cancellationToken = default)
    {
        ...
        List<string> bullets =
        [
            $"Environment path: {approval.SourceEnvironment} → {approval.TargetEnvironment}", $"Manifest version: {approval.ManifestVersion}",
            $"Workflow status: {approval.Status}", $"Run id: {approval.RunId}"
        ];
        if (lineage.Run is { } run)
            bullets.Add($"Run status: {run.Status}; created {run.CreatedUtc:O} UTC");
        if (lineage.Manifest is { } manifest)
            bullets.Add($"Authority manifest: version {manifest.ManifestVersion ?? "—"}; " +
        ...
        string summary = $"Governance rationale for approval {approval.ApprovalRequestId}: " +
        ...
        return new GovernanceRationaleResult { SchemaVersion = 1, ApprovalRequestId = approval.ApprovalRequestId, Summary = summary, Bullets = bullets };
    }
}
