...
public sealed class FindingReviewTrailAppendService(IFindingReviewTrailRepository trailRepository, IAuditService auditService) : IFindingReviewTrailAppendService
{
    ...
    public async Task AppendAsync(FindingReviewEventRecord reviewEvent, CancellationToken cancellationToken = default)
    {
        ...
        await _auditService.LogAsync(new AuditEvent { EventType = eventType, ActorUserId = reviewEvent.ReviewerUserId, ActorUserName = reviewEvent.ReviewerUserId, TenantId = reviewEvent.TenantId, WorkspaceId = reviewEvent.WorkspaceId, ProjectId = reviewEvent.ProjectId, RunId = reviewEvent.RunId, DataJson = JsonSerializer.Serialize(new { reviewEvent.EventId, reviewEvent.FindingId, reviewEvent.Action, reviewEvent.Notes, reviewEvent.OccurredAtUtc }, AuditJsonSerializationOptions.Instance) }, cancellationToken);
    }
    ...
    private static string MapActionToAuditEventType(string action)
    {
        ...
        throw new ArgumentOutOfRangeException(nameof(action), action, string.Create(CultureInfo.InvariantCulture, $"Unsupported finding review Action '{normalized}'. Extend MapActionToAuditEventType when adding new verbs."));
    }
}