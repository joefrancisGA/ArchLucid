...
public sealed class TraceabilityBundleBuilder(
    ...
    IAuditRepository auditRepository) : ITraceabilityBundleBuilder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        Guid runGuid = Guid.TryParseExact(runId, "N", out Guid g1)
            ? g1
            : Guid.TryParse(runId, out Guid g2)
                ? g2
                : Guid.Empty;

        IReadOnlyList<AuditEvent> audits = runGuid == Guid.Empty
            ? []
            : await _auditRepository.GetFilteredAsync(
                scope.TenantId,
                scope.WorkspaceId,
                scope.ProjectId,
                new AuditEventFilter { RunId = runGuid, Take = 1000 },
                cancellationToken);
