...
public sealed class TraceabilityBundleBuilder(IRunDetailQueryService runDetailQueryService, IAuditRepository auditRepository) : ITraceabilityBundleBuilder
{
    ...
    public async Task<Byte[]?> BuildAsync(string runId, ScopeContext scope, long maxZipBytes, CancellationToken cancellationToken)
    {
        ...
        byte[] zipBytes = BuildZipInMemory(detail.Run.RunId, summary, audits, auditTruncated, detail.DecisionTraces);
    ...
    private static byte[] BuildZipInMemory(
        ...
        bool auditTruncated,
        IReadOnlyList<DecisionTraceDto> traces)
    {
        ...
        using (ZipArchive zip = new(ms, ZipArchiveMode.Create, true))
        {
            ...
            AddTextEntry(
                ...
                BuyerSafeAuditEvidenceSummaryBuilder.BuildJson(runId, audits, auditTruncated));
            AddTextEntry(
                zip,
