...
/// <summary>
///     Markdown projection of <see cref="SponsorDigestComposition"/> for board-pack PDF reuse (same fields as weekly
///     email model).
/// </summary>
public static class SponsorDigestCompositionMarkdownFormatter
{
    /// <summary>
    ///     Formats digest highlights without duplicating ROI math (value section comes from
    ///     <see cref="ValueReportBuilder"/>).
    /// </summary>
    public static string Format(SponsorDigestComposition composition)
    {
        ...
        if (composition.TopManifestRuns is { Count: > 0 } runs)
        {
            ...
            foreach (SponsorDigestHighlightedRun run in runs)
            {
