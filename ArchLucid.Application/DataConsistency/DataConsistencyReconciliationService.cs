    ...
    ILogger<DataConsistencyReconciliationService> logger) : IDataConsistencyReconciliationService
{
    ...
    private static void RecordInstrumentation(double elapsedMs, IReadOnlyList<DataConsistencyFinding> findings)
    {
        ...
        {
            KeyValuePair<string, object?>[] tags = [new("severity", f.Severity.ToString()), new("check_name", f.CheckName)];
        ...
        List<DataConsistencyFinding> findings, CancellationToken ct)
    {
        ...
        findings.Add(new DataConsistencyFinding(checkName, DataConsistencyFindingSeverity.Warning, $"{description} Count={count}.", ids));
        if (_logger.IsEnabled(LogLevel.Warning))
            _logger.LogWarning("Data consistency [{CheckName}]: {Description}. Count={Count}.", checkName, description, count);
    }

    private async Task AddStaleRunFindingsAsync(DbConnection connection, List<DataConsistencyFinding> findings, CancellationToken ct)
    {
        long count = await ExecuteCountAsync(connection, DataConsistencyReconciliationSql.StaleInFlightRuns, ct).ConfigureAwait(false);
        if (count <= 0)
            return;
    ...
    private async Task AddCacheVersusDatabaseFindingsAsync(DbConnection connection, List<DataConsistencyFinding> findings, CancellationToken ct)
    {
        ...
        foreach (RecentRunRow row in rows)
        {
            ...
            if (!string.Equals(NormalizeVersion(cached.CurrentManifestVersion), NormalizeVersion(row.CurrentManifestVersion), StringComparison.Ordinal))
    ...
    private static async Task<IReadOnlyList<string>> ReadStringColumnAsync(DbConnection connection, string sql, CancellationToken ct)
    {
        List<string> ids = [];
        await using DbCommand command = connection.CreateCommand();
        command.CommandText = sql;
