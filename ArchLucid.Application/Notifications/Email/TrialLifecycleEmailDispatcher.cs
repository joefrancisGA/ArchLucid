...
public sealed class TrialLifecycleEmailDispatcher(ITenantRepository tenantRepository, ITenantTrialEmailContactLookup contactLookup, IEmailTemplateRenderer templateRenderer, IEmailProvider emailProvider, ISentEmailLedger sentEmailLedger, IOptionsMonitor<EmailNotificationOptions> emailOptionsMonitor, ILogger<TrialLifecycleEmailDispatcher> logger) : ITrialLifecycleEmailDispatcher
{
    ...
    public async Task DispatchAsync(TrialLifecycleEmailIntegrationEnvelope envelope, CancellationToken cancellationToken)
    {
        ...
        if (envelope.SchemaVersion != 1)
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning("Ignoring trial lifecycle email envelope with unsupported schemaVersion {SchemaVersion}.", envelope.SchemaVersion);
            return;
        }
    ...
    private TrialDispatchPlan? TryBuildPlan(TrialLifecycleEmailIntegrationEnvelope envelope, TenantRecord tenant, string productName, string? baseUrl, DateTimeOffset utcNow)
    {
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.TrialProvisioned)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialWelcome, $"Welcome to {productName}", model);
        }
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.FirstRunCommitted)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialFirstRunComplete, "Your first architecture run completed", model);
        }
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.MidTrialDay7)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialMidTrialDay7, $"Day 7 check-in — your {productName} trial", model);
        }
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.ApproachingRunLimit)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialApproachingRunLimit, "Approaching your trial run limit", model);
        }
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.ExpiringSoon)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialExpiringSoon, "Your trial is ending soon", model);
        }
        ...
        if (envelope.Trigger is TrialLifecycleEmailTrigger.Expired)
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialExpired, $"Your {productName} trial has ended", model);
        }
            ...
            return null;
        {
            ...
            return new TrialDispatchPlan(idempotencyKey, EmailTemplateIds.TrialConverted, $"Welcome to {tier}", model);
        }
    }
