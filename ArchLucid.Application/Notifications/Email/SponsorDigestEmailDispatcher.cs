...
/// <inheritdoc cref = "ISponsorDigestEmailDispatcher"/>
public sealed class SponsorDigestEmailDispatcher(
    IEmailTemplateRenderer templateRenderer,
    IEmailProvider emailProvider,
    ISentEmailLedger sentEmailLedger,
    IOptionsMonitor<EmailNotificationOptions> emailOptionsMonitor,
    ILogger<SponsorDigestEmailDispatcher> logger) : ISponsorDigestEmailDispatcher
{
    public const string TemplateId = "SponsorDigest";
    ...
    private readonly ILogger<SponsorDigestEmailDispatcher> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    /// <inheritdoc/>
    public async Task<bool> TryDispatchAsync(Guid tenantId, string isoWeekIdempotencyKey, SponsorDigestComposition composition, IReadOnlyList<string> toMailboxes,
        string unsubscribeAbsoluteUrl, CancellationToken cancellationToken)
    {
        ...
        SponsorDigestEmailModel model = new()
        {
        ...
        string idempotencyKey = $"sponsor-digest:{tenantId:N}:{isoWeekIdempotencyKey}";
        ...
        foreach (string mailbox in toMailboxes)
        {
            ...
            EmailMessage message = new()
            {
                ...
                Tags = new EmailMessageTags { TenantId = tenantId, EventType = "sponsor-digest-weekly" }
            };
            try
            {
            ...
            catch (Exception ex)when (!cancellationToken.IsCancellationRequested)
            {
                if (_logger.IsEnabled(LogLevel.Error))
                    _logger.LogError(ex, "Sponsor digest email send failed for tenant {TenantId}.", tenantId);
                throw;
            }
        }
