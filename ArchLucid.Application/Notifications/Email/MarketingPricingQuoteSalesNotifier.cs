...
public sealed class MarketingPricingQuoteSalesNotifier(IEmailProvider emailProvider, IOptionsMonitor<EmailNotificationOptions> emailOptionsMonitor, ILogger<MarketingPricingQuoteSalesNotifier> logger) : IMarketingPricingQuoteSalesNotifier
{
    ...
    private readonly ILogger<MarketingPricingQuoteSalesNotifier> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    /// <inheritdoc/>
    public async Task NotifyAsync(MarketingPricingQuoteRequestInsertResult insert, string workEmail, string companyName, string tierInterest, string message, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(workEmail);
        ArgumentNullException.ThrowIfNull(companyName);
        ArgumentNullException.ThrowIfNull(tierInterest);
        ...
        if (string.IsNullOrWhiteSpace(inbox))
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning("Marketing pricing quote sales notification skipped: {ConfigKey} is empty.", nameof(EmailNotificationOptions.PricingQuoteSalesInbox));
            return;
        }
        ...
        if (string.Equals(_emailProvider.ProviderName, EmailProviderNames.Noop, StringComparison.OrdinalIgnoreCase))
        {
            if (_logger.IsEnabled(LogLevel.Information))
                _logger.LogInformation("Would notify pricing quote inbox {Inbox} for request id {RequestId} (Email:Provider is {Provider}).", inbox.Trim(), insert.Id, _emailProvider.ProviderName);
            return;
        }
        ...
        string html = "<p>A new anonymous pricing quote was submitted from <strong>archlucid.net/pricing</strong>.</p>" + $"<p><strong>Request id:</strong> {insert.Id:N}</p>" + $"<p><strong>Created (UTC):</strong> {insert.CreatedUtc:O}</p>" + $"<p><strong>Work email:</strong> {safeEmail}</p>" + $"<p><strong>Company:</strong> {safeCompany}</p>" + $"<p><strong>Tier interest:</strong> {safeTier}</p>" + $"<p><strong>Message:</strong></p><pre style=\"white-space:pre-wrap\">{safeMessage}</pre>";
        string text = $"ArchLucid pricing quote request\nRequest id: {insert.Id:N}\nCreated (UTC): {insert.CreatedUtc:O}\n" + $"Work email: {workEmail}\nCompany: {companyName}\nTier: {tierInterest}\nMessage:\n{message}\n";
