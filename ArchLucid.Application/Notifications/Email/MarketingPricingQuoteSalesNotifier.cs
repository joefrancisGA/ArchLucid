    ...
    ILogger<MarketingPricingQuoteSalesNotifier> logger) : IMarketingPricingQuoteSalesNotifier
{
    ...
    /// <inheritdoc/>
    public async Task NotifyAsync(MarketingPricingQuoteRequestInsertResult insert, string workEmail, string companyName, string tierInterest, string message,
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(inbox))
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning("Marketing pricing quote sales notification skipped: {ConfigKey} is empty.",
        ...
        if (string.Equals(_emailProvider.ProviderName, EmailProviderNames.Noop, StringComparison.OrdinalIgnoreCase))
        {
            if (_logger.IsEnabled(LogLevel.Information))
                _logger.LogInformation("Would notify pricing quote inbox {Inbox} for request id {RequestId} (Email:Provider is {Provider}).", inbox.Trim(),
                    insert.Id, _emailProvider.ProviderName);
            return;
        }
        ...
        string html = "<p>A new anonymous pricing quote was submitted from <strong>archlucid.net/pricing</strong>.</p>" +
                      $"<p><strong>Request id:</strong> {insert.Id:N}</p>" + $"<p><strong>Created (UTC):</strong> {insert.CreatedUtc:O}</p>" +
                      $"<p><strong>Work email:</strong> {safeEmail}</p>" + $"<p><strong>Company:</strong> {safeCompany}</p>" +
