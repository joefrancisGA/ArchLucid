...
public sealed class BaselineMutationAuditService(
    ...
    IScopeContextProvider scopeContextProvider) : IBaselineMutationAuditService
{
    ...
    public async Task RecordAsync(string eventType, string actor, string entityId, string? details = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogInformation("BaselineMutation {EventType} Actor={Actor} EntityId={EntityId} Details={Details}", LogSanitizer.Sanitize(eventType),
                LogSanitizer.Sanitize(actor), LogSanitizer.Sanitize(entityId),
                LogSanitizer.Sanitize(
                    safeDetails)); // codeql[cs/log-forging]: string placeholders sanitized via LogSanitizer; details length-capped in TruncateDetails.
        }
        ...
        try
        {
            await BaselineMutationAuditArchitectureDurableWriter.TryWriteArchitectureDurableEchoAsync(eventType, actor, entityId, safeDetails, auditService,
                scopeContextProvider, logger, cancellationToken);
        }
        ...
        catch (Exception ex)
        {
            ...
            {
                logger.LogWarning(ex, "Baseline durable echo failed for {EventType} EntityId={EntityId}", LogSanitizer.Sanitize(eventType),
    ...
    private static bool IsArchitectureBaselineMutationEvent(string eventType)
    {
        return string.Equals(eventType, AuditEventTypes.Baseline.Architecture.RunFailed, StringComparison.Ordinal) ||
               ...
               string.Equals(eventType, AuditEventTypes.Baseline.Architecture.RunCompleted, StringComparison.Ordinal) || string.Equals(eventType,
                   AuditEventTypes.Baseline.Architecture.RunQualityGateRejected, StringComparison.Ordinal);
    }
