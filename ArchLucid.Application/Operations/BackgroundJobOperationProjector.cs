...
{
  internal static OperationDetail Project(string operationId, BackgroundJobInfo job)
  {
    OperationState state = MapState(job.State);
    string stepLabel = ResolveStepLabel(job.State);
  ...
  private static OperationState MapState(BackgroundJobState state) =>
    state switch
    {
  ...
  private static string ResolveStepLabel(BackgroundJobState state) =>
    state switch
    {
