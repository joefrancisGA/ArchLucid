    ...
    public static FastPathContextPreviewResponse Build(string? rawUrl)
    {
        ...
        [
            new FastPathContextElementDto
            {
                ...
                Kind = "SoftwareSystem",
                ReasoningTrace =
                    "Heuristic: software system name taken from the last non-empty path segment of the repository URL."
            },
            new FastPathContextElementDto
            {
                ...
                Kind = "Container",
                ReasoningTrace =
                    "Heuristic placeholder for compute / application tier. Run full context ingest for repository-accurate components."
            },
            new FastPathContextElementDto
            {
                ...
                Kind = "Container",
                ReasoningTrace =
                    "Heuristic placeholder for persistence. Refine with connector inventory after deep ingest."
            }
        ];
        ...
        if (combined.Contains("UI", StringComparison.Ordinal) ||
            combined.Contains("WEB", StringComparison.Ordinal) ||
            combined.Contains("FRONT", StringComparison.Ordinal))
        {
            elements.Add(new FastPathContextElementDto
            {
                ...
                Kind = "Container",
                ReasoningTrace =
                    "Heuristic: UI/web/front token detected in URL or slug — likely browser or SPA surface."
            });
        }
