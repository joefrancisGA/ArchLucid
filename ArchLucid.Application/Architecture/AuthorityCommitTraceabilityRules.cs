...
public static class AuthorityCommitTraceabilityRules
{
    ...
    /// </summary>
    public static IReadOnlyList<string> GetLinkageGaps(Cm.GoldenManifest? manifest, IReadOnlyList<DecisionTrace> traces)
    {
        ...
        foreach (DecisionTrace t in traces)
        {
            ...
            {
                gaps.Add($"Decision trace is not a {nameof(RuleAuditTrace)} (Kind={t.Kind}); authority commits expect rule-audit traces only.");
                continue;
            }
