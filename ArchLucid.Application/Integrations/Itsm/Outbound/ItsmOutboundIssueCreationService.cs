    ...
    ServiceNowOutboundIncidentClient serviceNowClient)
{
        ...
        CancellationToken ct)
    {
        ...
        {
            ItsmOutboundIssueProvider.Jira => await TryJiraAsync(scope, inspect, tenantRow, severity, summary, description, ct)
    ...
    private async Task<ItsmOutboundIssueCreationResult> TryJiraAsync(
        ScopeContext scope,
        FindingInspectResponse inspect,
        TenantItsmOutboundSettings? tenantRow,
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(jiraOpts.CloudBaseUrl) ||
            string.IsNullOrWhiteSpace(jiraOpts.ServiceAccountEmail) ||
            string.IsNullOrWhiteSpace(jiraOpts.ApiToken))
        {
            AuditEvent ev = SkippedAudit(
        ...
        JiraOutboundIssueHttpResult http = await _jiraClient
            .CreateIssueAsync(
                issueUri,
                jiraOpts.ServiceAccountEmail.Trim(),
                jiraOpts.ApiToken,
                projectKey.Trim(),
                summary,
                adf,
    ...
    private async Task<ItsmOutboundIssueCreationResult> TryServiceNowAsync(
        ScopeContext scope,
        FindingInspectResponse inspect,
        TenantItsmOutboundSettings? tenantRow,
        FindingSeverity severity,
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(sn.InstanceBaseUrl) ||
            string.IsNullOrWhiteSpace(sn.Username) ||
            string.IsNullOrWhiteSpace(sn.Password))
        {
            AuditEvent ev = SkippedAudit(
            ...
            return new ItsmOutboundIssueCreationResult
            {
                Kind = ItsmOutboundCreateTerminalKind.Skipped,
                UserMessage =
                    "ServiceNow outbound connector is not configured (instance base URL, username, and password are required).",
                AuditEvents = [ev]
            };
        }
        ...
        {
            ArchitectureRequest? req =
                await _architectureRequests.GetByIdAsync(run.ArchitectureRequestId, ct).ConfigureAwait(false);
        ...
        ServiceNowCmdbCiResolveResult cmdb = await _serviceNowClient
            .TryResolveCmdbCiApplSysIdAsync(
                instanceUri,
                sn.Username,
                sn.Password,
        ...
        AuditEvent ok = new()
        {
            ...
            RunId = inspect.RunId,
            DataJson = JsonSerializer.Serialize(
                new { findingId = inspect.FindingId, sysId = http.SysId, number = http.Number })
        };
