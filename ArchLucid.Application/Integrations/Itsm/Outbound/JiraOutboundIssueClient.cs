...
public sealed class JiraOutboundIssueClient(HttpClient http, ILogger<JiraOutboundIssueClient> logger)
{
    ...
    private readonly ILogger<JiraOutboundIssueClient> _logger =
        logger ?? throw new ArgumentNullException(nameof(logger));

    public async Task<JiraOutboundIssueHttpResult> CreateIssueAsync(
        Uri issuePostUri,
        string serviceAccountEmail,
        string apiToken,
        string projectKey,
        string summary,
        JsonElement descriptionAdf,
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            ...
            return new JiraOutboundIssueHttpResult(
                false,
                null,
                null,
        ...
        if (response.IsSuccessStatusCode)
        {
            try
            {
                ...
                if (parsed is null || string.IsNullOrWhiteSpace(parsed.Key))
                    return new JiraOutboundIssueHttpResult(
                        ...
                        null,
                        response.StatusCode,
                        "Jira returned success but no issue key.");
            ...
            catch (JsonException)
            {
                return new JiraOutboundIssueHttpResult(
                    ...
                    null,
                    response.StatusCode,
                    "Jira returned success but the response could not be parsed.");
            }
        }
...
public sealed record JiraOutboundIssueHttpResult(
    bool Ok,
    string? IssueKey,
    string? RemoteId,
