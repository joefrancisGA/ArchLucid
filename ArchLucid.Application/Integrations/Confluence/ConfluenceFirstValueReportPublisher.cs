...
public sealed class ConfluenceFirstValueReportPublisher(IRunRepository runRepository, IFirstValueReportBuilder firstValueReportBuilder, IPublisherConnector publisherConnector, IOptionsMonitor<ConfluencePublishingOptions> options, ILogger<ConfluenceFirstValueReportPublisher> logger) : IConfluenceFirstValueReportPublisher
{
    ...
    public async Task<PublishOutcome> PublishFirstValueReportAsync(string runId, string apiBaseForLinks, CancellationToken cancellationToken)
    {
        ...
        {
            return new PublishOutcome(false, null, ConfluencePublishFailureReason.BadResponse, "Run id must be a 32-character hex GUID.");
        }
        ...
        {
            markdown = await _firstValueReportBuilder.BuildMarkdownAsync(normalizedRunId, baseUrl, cancellationToken).ConfigureAwait(false);
        }

        if (markdown is null)
        {
            return new PublishOutcome(false, null, ConfluencePublishFailureReason.NotFound, "First-value report could not be built for this run.");
        }
    ...
    private static bool TryParseRunKey(string runId, out Guid guid)
    {
        ...
        if (s.Length is 32 && Guid.TryParseExact(s, "N", out guid))
            return true;
