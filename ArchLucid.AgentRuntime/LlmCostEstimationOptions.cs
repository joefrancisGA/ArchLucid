...
public sealed class LlmCostEstimationOptions
{
    ...
    /// <remarks>Defaults to <see langword="true"/> so hosts without an explicit section still surface FinOps estimates; disable via configuration when required.</remarks>
    public bool Enabled { get; set; } = true;

    /// <summary>USD per 1M prompt (input) tokens.</summary>
    public decimal InputUsdPerMillionTokens { get; set; } = 0.5m;

    /// <summary>USD per 1M completion (output) tokens.</summary>
    public decimal OutputUsdPerMillionTokens { get; set; } = 1.5m;
}
