#!/usr/bin/env python3
"""Ship /al-bug hunts #6456-#6555 (batch 68, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "baf", "bbg", "bch", "bdi", "bej", "bfk", "bgl", "bhm", "bin", "bjo",
...
CONTEXT_HITS = {
    ...
    6482: "degraded",
    6507: "environment",
    6532: "availability_zones",
}

CONTEXT_META = {
    "replicated": {
        "ledger_prop": "replicated",
        "commit_prop": "replicated",
        "test_class": "TerraformShowJsonReplicatedTests",
        "test_method": "ParseAsync_replicated_maps_tf_replicated_property",
        "summary": "terraform-show-json replicated property gap",
    },
    "degraded": {
        "ledger_prop": "degraded",
        "commit_prop": "degraded",
        "test_class": "TerraformShowJsonDegradedTests",
        "test_method": "ParseAsync_degraded_maps_tf_degraded_property",
        "summary": "terraform-show-json degraded property gap",
    },
    "environment": {
        "ledger_prop": "environment",
        "commit_prop": "environment",
        "test_class": "TerraformShowJsonEnvironmentTests",
        "test_method": "ParseAsync_environment_maps_tf_environment_property",
        "summary": "terraform-show-json environment property gap",
    },
    "availability_zones": {
        "ledger_prop": "availability_zones",
        "commit_prop": "availability_zones",
        "test_class": "TerraformShowJsonAvailabilityZonesTests",
        "test_method": "ParseAsync_availability_zones_maps_tf_availability_zones_property",
        "summary": "terraform-show-json availability_zones property gap",
    },
}
...
def add_replicated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.locations"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "replicated", "replicated"), encoding="utf-8")


def add_degraded_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.replicated"] = replicated.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "degraded", "degraded"), encoding="utf-8")


def add_environment_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.environment"]' in text:
        return
    old = """            properties["tf.degraded"] = degraded.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text: raise RuntimeError("degraded anchor not found")
    RESOURCE_MAPPING.write_text(text.replace(old,new,1),encoding="utf-8")


def add_availability_zones_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.environment"] = environmentText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "availability_zones", "availabilityZones", "availability_zones"), encoding="utf-8")


def write_replicated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReplicatedTests.cs"
    if path.exists():
        return path
    path.write_text(
...
def write_degraded_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDegradedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_environment_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonEnvironmentTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonEnvironmentTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_environment_maps_tf_environment_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "environment.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","environment":"production","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.environment"].Should().Be("production");
    }
}
''',
        encoding="utf-8",
...
def write_availability_zones_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAvailabilityZonesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6457:
        add_replicated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_replicated_test().relative_to(REPO))]
    if hunt == 6482:
        add_degraded_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_degraded_test().relative_to(REPO))]
    if hunt == 6507:
        add_environment_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_environment_test().relative_to(REPO))]
    if hunt == 6532:
        add_availability_zones_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_availability_zones_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6456"))
    end_hunt = int(os.environ.get("END_HUNT", "6555"))
