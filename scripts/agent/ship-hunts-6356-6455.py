#!/usr/bin/env python3
"""Ship /al-bug hunts #6356-#6455 (batch 67, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "aaf", "abg", "ach", "adi", "aej", "afk", "agl", "ahm", "ain", "ajo",
    "akp", "alq", "amr", "ans", "aot", "apu", "aqv", "arw", "asx", "aty",
]
TOPOLOGY = {
    6356 + index * 5: (
        token,
...
CONTEXT_META = {
    "sharded": {
        "ledger_prop": "sharded",
        "commit_prop": "sharded",
        "test_class": "TerraformShowJsonShardedTests",
        "test_method": "ParseAsync_sharded_maps_tf_sharded_property",
        "summary": "terraform-show-json sharded property gap",
    },
    "drained": {
        "ledger_prop": "drained",
        "commit_prop": "drained",
        "test_class": "TerraformShowJsonDrainedTests",
        "test_method": "ParseAsync_drained_maps_tf_drained_property",
        "summary": "terraform-show-json drained property gap",
    },
    "scope": {
        "ledger_prop": "scope",
        "commit_prop": "scope",
        "test_class": "TerraformShowJsonScopeTests",
        "test_method": "ParseAsync_scope_maps_tf_scope_property",
        "summary": "terraform-show-json scope property gap",
    },
    "locations": {
        "ledger_prop": "locations",
        "commit_prop": "locations",
        "test_class": "TerraformShowJsonLocationsTests",
        "test_method": "ParseAsync_locations_maps_tf_locations_property",
        "summary": "terraform-show-json locations property gap",
    },
}
...
def add_sharded_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.regions"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "sharded", "sharded"), encoding="utf-8")


def add_drained_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.sharded"] = sharded.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "drained", "drained"), encoding="utf-8")


def add_scope_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.scope"]' in text:
        return
    old = """            properties["tf.drained"] = drained.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("drained anchor not found for scope")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_locations_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.scope"] = scopeText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "locations", "locations", "locations"), encoding="utf-8")


def write_sharded_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonShardedTests.cs"
    if path.exists():
        return path
    path.write_text(
...
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonShardedTests
{
    ...
    [Fact]
    public async Task ParseAsync_sharded_maps_tf_sharded_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "sharded.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","sharded":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.sharded"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_drained_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDrainedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonDrainedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_drained_maps_tf_drained_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "drained.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","drained":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.drained"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_scope_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonScopeTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_locations_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonLocationsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonLocationsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_locations_maps_tf_locations_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "locations.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","locations":["eastus"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.locations"].Should().Be("eastus");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6357:
        add_sharded_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_sharded_test().relative_to(REPO))]
    if hunt == 6382:
        add_drained_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_drained_test().relative_to(REPO))]
    if hunt == 6407:
        add_scope_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_scope_test().relative_to(REPO))]
    if hunt == 6432:
        add_locations_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_locations_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6356"))
    end_hunt = int(os.environ.get("END_HUNT", "6455"))
