#!/usr/bin/env python3
"""Ship /al-bug hunts #3856-#3955 (batch 42, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "azf", "bag", "bch", "bdi", "bej", "bfk", "bgl", "bhm", "bin", "bjo",
    "bkp", "blq", "bmr", "bns", "bot", "bpu", "bqv", "brw", "bsx", "bty",
]
TOPOLOGY = {
    3856 + index * 5: (
        token,
...
CONTEXT_META = {
    "refresh_only": {
        "ledger_prop": "refreshOnly",
        "commit_prop": "refreshOnly",
        "test_class": "TerraformShowJsonRefreshOnlyTests",
        "test_method": "ParseAsync_refresh_only_maps_tf_refresh_only_property",
        "summary": "terraform-show-json refreshOnly property gap",
    },
    "recreate": {
        "ledger_prop": "recreate",
        "commit_prop": "recreate",
        "test_class": "TerraformShowJsonRecreateTests",
        "test_method": "ParseAsync_recreate_maps_tf_recreate_property",
        "summary": "terraform-show-json recreate property gap",
    },
    "import_id": {
        "ledger_prop": "importId",
        "commit_prop": "importId",
        "test_class": "TerraformShowJsonImportIdTests",
        "test_method": "ParseAsync_import_id_maps_tf_import_id_property",
        "summary": "terraform-show-json importId property gap",
    },
    "retain": {
        "ledger_prop": "retain",
        "commit_prop": "retain",
        "test_class": "TerraformShowJsonRetainTests",
        "test_method": "ParseAsync_retain_maps_tf_retain_property",
        "summary": "terraform-show-json retain property gap",
    },
}
...
def add_refresh_only_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.write_only"] = writeOnly.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "refresh_only", "refreshOnly"), encoding="utf-8")


def add_recreate_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.refresh_only"] = refreshOnly.GetBoolean() ? "true" : "false";
        }"""
    if anchor not in text:
        anchor = """            properties["tf.refresh_only"] = RefreshOnly.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "recreate", "recreate"), encoding="utf-8")


def add_import_id_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.import_id"]' in text:
        return
    old = """            properties["tf.recreate"] = recreate.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("recreate anchor not found for import_id")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_retain_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.import_id"] = importIdText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "retain", "retain"), encoding="utf-8")
...
def write_refresh_only_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRefreshOnlyTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_recreate_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRecreateTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRecreateTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_recreate_maps_tf_recreate_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "recreate.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","recreate":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.recreate"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_import_id_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonImportIdTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonImportIdTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_import_id_maps_tf_import_id_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "import-id.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","importId":"id-123","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.import_id"].Should().Be("id-123");
    }
}
''',
        encoding="utf-8",
...
def write_retain_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRetainTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRetainTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_retain_maps_tf_retain_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "retain.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","retain":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.retain"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 3857:
        add_refresh_only_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_refresh_only_test().relative_to(REPO))]
    if hunt == 3882:
        add_recreate_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_recreate_test().relative_to(REPO))]
    if hunt == 3907:
        add_import_id_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_import_id_test().relative_to(REPO))]
    if hunt == 3932:
        add_retain_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_retain_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "3856"))
    end_hunt = int(os.environ.get("END_HUNT", "3955"))
