#!/usr/bin/env python3
"""Ship /al-bug hunts #4456-#4555 (batch 48, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "haf", "hbg", "hch", "hdi", "hej", "hfk", "hgl", "hhm", "hin", "hjo",
    "hkp", "hlq", "hmr", "hns", "hot", "hpu", "hqv", "hrw", "hsx", "hty",
]
TOPOLOGY = {
    4456 + index * 5: (
        token,
...
CONTEXT_META = {
    "nested": {
        "ledger_prop": "nested",
        "commit_prop": "nested",
        "test_class": "TerraformShowJsonNestedTests",
        "test_method": "ParseAsync_nested_maps_tf_nested_property",
        "summary": "terraform-show-json nested property gap",
    },
    "suspended": {
        "ledger_prop": "suspended",
        "commit_prop": "suspended",
        "test_class": "TerraformShowJsonSuspendedTests",
        "test_method": "ParseAsync_suspended_maps_tf_suspended_property",
        "summary": "terraform-show-json suspended property gap",
    },
    "target": {
        "ledger_prop": "target",
        "commit_prop": "target",
        "test_class": "TerraformShowJsonTargetTests",
        "test_method": "ParseAsync_target_maps_tf_target_property",
        "summary": "terraform-show-json target property gap",
    },
    "members": {
        "ledger_prop": "members",
        "commit_prop": "members",
        "test_class": "TerraformShowJsonMembersTests",
        "test_method": "ParseAsync_members_maps_tf_members_property",
        "summary": "terraform-show-json members property gap",
    },
}
...
def add_nested_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.module"] = moduleText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "nested", "nested"), encoding="utf-8")


def add_suspended_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.nested"] = nested.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "suspended", "suspended"), encoding="utf-8")


def add_target_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.target"]' in text:
        return
    old = """            properties["tf.suspended"] = suspended.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("suspended anchor not found for target")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_members_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.overrides"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "members", "members", "members"), encoding="utf-8")
...
def write_nested_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonNestedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonNestedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_nested_maps_tf_nested_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "nested.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","nested":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.nested"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_suspended_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSuspendedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSuspendedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_suspended_maps_tf_suspended_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "suspended.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","suspended":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.suspended"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_target_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonTargetTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonTargetTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_target_maps_tf_target_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "target.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","target":"azurerm_subnet.main","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.target"].Should().Be("azurerm_subnet.main");
    }
}
''',
        encoding="utf-8",
...
def write_members_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMembersTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMembersTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_members_maps_tf_members_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "members.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","members":["node-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.members"].Should().Be("node-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4457:
        add_nested_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_nested_test().relative_to(REPO))]
    if hunt == 4482:
        add_suspended_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_suspended_test().relative_to(REPO))]
    if hunt == 4507:
        add_target_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_target_test().relative_to(REPO))]
    if hunt == 4532:
        add_members_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_members_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4456"))
    end_hunt = int(os.environ.get("END_HUNT", "4555"))
