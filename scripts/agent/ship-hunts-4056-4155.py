#!/usr/bin/env python3
"""Ship /al-bug hunts #4056-#4155 (batch 44, 100 checkins).
...
CONTEXT_META = {
    "upgrade": {
        "ledger_prop": "upgrade",
        "commit_prop": "upgrade",
        "test_class": "TerraformShowJsonUpgradeTests",
        "test_method": "ParseAsync_upgrade_maps_tf_upgrade_property",
        "summary": "terraform-show-json upgrade property gap",
    },
    "ephemeral": {
        "ledger_prop": "ephemeral",
        "commit_prop": "ephemeral",
        "test_class": "TerraformShowJsonEphemeralTests",
        "test_method": "ParseAsync_ephemeral_maps_tf_ephemeral_property",
        "summary": "terraform-show-json ephemeral property gap",
    },
    "replace": {
        "ledger_prop": "replace",
        "commit_prop": "replace",
        "test_class": "TerraformShowJsonReplaceTests",
        "test_method": "ParseAsync_replace_maps_tf_replace_property",
        "summary": "terraform-show-json replace property gap",
    },
    "replace_on_changes": {
        "ledger_prop": "replaceOnChanges",
        "commit_prop": "replaceOnChanges",
        "test_class": "TerraformShowJsonReplaceOnChangesTests",
        "test_method": "ParseAsync_replace_on_changes_maps_tf_replace_on_changes_property",
        "summary": "terraform-show-json replaceOnChanges property gap",
    },
}
...
def add_upgrade_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.destroy"] = destroy.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "upgrade", "upgrade"), encoding="utf-8")


def add_ephemeral_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.upgrade"] = upgrade.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "ephemeral", "ephemeral"), encoding="utf-8")


def add_replace_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.ephemeral"] = ephemeral.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "replace", "replace"), encoding="utf-8")


def add_replace_on_changes_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.conflicts_with"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "replace_on_changes", "replaceOnChanges", "replace_on_changes"), encoding="utf-8")
...
def write_upgrade_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonUpgradeTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonUpgradeTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_upgrade_maps_tf_upgrade_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "upgrade.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","upgrade":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.upgrade"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_ephemeral_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonEphemeralTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonEphemeralTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_ephemeral_maps_tf_ephemeral_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "ephemeral.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","ephemeral":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.ephemeral"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_replace_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReplaceTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_replace_on_changes_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReplaceOnChangesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4057:
        add_upgrade_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_upgrade_test().relative_to(REPO))]
    if hunt == 4082:
        add_ephemeral_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_ephemeral_test().relative_to(REPO))]
    if hunt == 4107:
        add_replace_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_replace_test().relative_to(REPO))]
    if hunt == 4132:
        add_replace_on_changes_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_replace_on_changes_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4056"))
    end_hunt = int(os.environ.get("END_HUNT", "4155"))
