#!/usr/bin/env python3
"""Ship /al-bug hunts #4756-#4855 (batch 51, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "kaf", "kbg", "kch", "kdi", "kej", "kfk", "kgl", "khm", "kin", "kjo",
    "kkp", "klq", "kmr", "kns", "kot", "kpu", "kqv", "krw", "ksx", "kty",
]
TOPOLOGY = {
    4756 + index * 5: (
        token,
...
CONTEXT_META = {
    "archived": {
        "ledger_prop": "archived",
        "commit_prop": "archived",
        "test_class": "TerraformShowJsonArchivedTests",
        "test_method": "ParseAsync_archived_maps_tf_archived_property",
        "summary": "terraform-show-json archived property gap",
    },
    "verified": {
        "ledger_prop": "verified",
        "commit_prop": "verified",
        "test_class": "TerraformShowJsonVerifiedTests",
        "test_method": "ParseAsync_verified_maps_tf_verified_property",
        "summary": "terraform-show-json verified property gap",
    },
    "owner": {
        "ledger_prop": "owner",
        "commit_prop": "owner",
        "test_class": "TerraformShowJsonOwnerTests",
        "test_method": "ParseAsync_owner_maps_tf_owner_property",
        "summary": "terraform-show-json owner property gap",
    },
    "ancestors": {
        "ledger_prop": "ancestors",
        "commit_prop": "ancestors",
        "test_class": "TerraformShowJsonAncestorsTests",
        "test_method": "ParseAsync_ancestors_maps_tf_ancestors_property",
        "summary": "terraform-show-json ancestors property gap",
    },
}
...
def add_archived_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.siblings"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "archived", "archived"), encoding="utf-8")


def add_verified_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.archived"] = archived.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "verified", "verified"), encoding="utf-8")


def add_owner_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.owner"]' in text:
        return
    old = """            properties["tf.verified"] = verified.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("verified anchor not found for owner")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_ancestors_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.owner"] = ownerText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "ancestors", "ancestors", "ancestors"), encoding="utf-8")
...
def write_archived_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonArchivedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonArchivedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_archived_maps_tf_archived_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "archived.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","archived":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.archived"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_verified_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonVerifiedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonVerifiedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_verified_maps_tf_verified_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "verified.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","verified":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.verified"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_owner_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonOwnerTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonOwnerTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_owner_maps_tf_owner_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "owner.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","owner":"platform-team","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.owner"].Should().Be("platform-team");
    }
}
''',
        encoding="utf-8",
...
def write_ancestors_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAncestorsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4757:
        add_archived_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_archived_test().relative_to(REPO))]
    if hunt == 4782:
        add_verified_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_verified_test().relative_to(REPO))]
    if hunt == 4807:
        add_owner_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_owner_test().relative_to(REPO))]
    if hunt == 4832:
        add_ancestors_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_ancestors_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4756"))
    end_hunt = int(os.environ.get("END_HUNT", "4855"))
