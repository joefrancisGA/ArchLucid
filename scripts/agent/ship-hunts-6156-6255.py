#!/usr/bin/env python3
"""Ship /al-bug hunts #6156-#6255 (batch 65, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "yaf", "ybg", "ych", "ydi", "yej", "yfk", "ygl", "yhm", "yin", "yjo",
    "ykp", "ylq", "ymr", "yns", "yot", "ypu", "yqv", "yrw", "ysx", "yty",
]
TOPOLOGY = {
    6156 + index * 5: (
        token,
...
CONTEXT_META = {
    "throttled": {
        "ledger_prop": "throttled",
        "commit_prop": "throttled",
        "test_class": "TerraformShowJsonThrottledTests",
        "test_method": "ParseAsync_throttled_maps_tf_throttled_property",
        "summary": "terraform-show-json throttled property gap",
    },
    "stalled": {
        "ledger_prop": "stalled",
        "commit_prop": "stalled",
        "test_class": "TerraformShowJsonStalledTests",
        "test_method": "ParseAsync_stalled_maps_tf_stalled_property",
        "summary": "terraform-show-json stalled property gap",
    },
    "category": {
        "ledger_prop": "category",
        "commit_prop": "category",
        "test_class": "TerraformShowJsonCategoryTests",
        "test_method": "ParseAsync_category_maps_tf_category_property",
        "summary": "terraform-show-json category property gap",
    },
    "regions": {
        "ledger_prop": "regions",
        "commit_prop": "regions",
        "test_class": "TerraformShowJsonRegionsTests",
        "test_method": "ParseAsync_regions_maps_tf_regions_property",
        "summary": "terraform-show-json regions property gap",
    },
}
...
def add_throttled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.policies"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "throttled", "throttled"), encoding="utf-8")


def add_stalled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.throttled"] = throttled.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "stalled", "stalled"), encoding="utf-8")


def add_category_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.category"]' in text:
        return
    old = """            properties["tf.stalled"] = stalled.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("stalled anchor not found for category")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_regions_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.category"] = categoryText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "regions", "regions", "regions"), encoding="utf-8")
...
def write_throttled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonThrottledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonThrottledTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_throttled_maps_tf_throttled_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "throttled.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","throttled":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.throttled"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_stalled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonStalledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_category_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCategoryTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCategoryTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_category_maps_tf_category_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "category.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","category":"network","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.category"].Should().Be("network");
    }
}
''',
        encoding="utf-8",
...
def write_regions_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRegionsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRegionsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_regions_maps_tf_regions_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "regions.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","regions":["eastus"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.regions"].Should().Be("eastus");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6157:
        add_throttled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_throttled_test().relative_to(REPO))]
    if hunt == 6182:
        add_stalled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_stalled_test().relative_to(REPO))]
    if hunt == 6207:
        add_category_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_category_test().relative_to(REPO))]
    if hunt == 6232:
        add_regions_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_regions_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6156"))
    end_hunt = int(os.environ.get("END_HUNT", "6255"))
