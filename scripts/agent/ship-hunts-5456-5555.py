#!/usr/bin/env python3
"""Ship /al-bug hunts #5456-#5555 (batch 58, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "raf", "rbg", "rch", "rdi", "rej", "rfk", "rgl", "rhm", "rin", "rjo",
    "rkp", "rlq", "rmr", "rns", "rot", "rpu", "rqv", "rrw", "rsx", "rty",
]
TOPOLOGY = {
    5456 + index * 5: (
        token,
...
CONTEXT_META = {
    "accepted": {
        "ledger_prop": "accepted",
        "commit_prop": "accepted",
        "test_class": "TerraformShowJsonAcceptedTests",
        "test_method": "ParseAsync_accepted_maps_tf_accepted_property",
        "summary": "terraform-show-json accepted property gap",
    },
    "rejected": {
        "ledger_prop": "rejected",
        "commit_prop": "rejected",
        "test_class": "TerraformShowJsonRejectedTests",
        "test_method": "ParseAsync_rejected_maps_tf_rejected_property",
        "summary": "terraform-show-json rejected property gap",
    },
    "intent": {
        "ledger_prop": "intent",
        "commit_prop": "intent",
        "test_class": "TerraformShowJsonIntentTests",
        "test_method": "ParseAsync_intent_maps_tf_intent_property",
        "summary": "terraform-show-json intent property gap",
    },
    "endpoints": {
        "ledger_prop": "endpoints",
        "commit_prop": "endpoints",
        "test_class": "TerraformShowJsonEndpointsTests",
        "test_method": "ParseAsync_endpoints_maps_tf_endpoints_property",
        "summary": "terraform-show-json endpoints property gap",
    },
}
...
def add_accepted_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.providers"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "accepted", "accepted"), encoding="utf-8")


def add_rejected_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.accepted"] = accepted.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "rejected", "rejected"), encoding="utf-8")


def add_intent_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.intent"]' in text:
        return
    old = """            properties["tf.rejected"] = rejected.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("rejected anchor not found for intent")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_endpoints_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.intent"] = intentText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "endpoints", "endpoints", "endpoints"), encoding="utf-8")
...
def write_accepted_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAcceptedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonAcceptedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_accepted_maps_tf_accepted_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "accepted.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","accepted":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.accepted"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_rejected_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRejectedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_intent_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonIntentTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_endpoints_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonEndpointsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5457:
        add_accepted_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_accepted_test().relative_to(REPO))]
    if hunt == 5482:
        add_rejected_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_rejected_test().relative_to(REPO))]
    if hunt == 5507:
        add_intent_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_intent_test().relative_to(REPO))]
    if hunt == 5532:
        add_endpoints_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_endpoints_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5456"))
    end_hunt = int(os.environ.get("END_HUNT", "5555"))
