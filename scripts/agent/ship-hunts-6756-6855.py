#!/usr/bin/env python3
"""Ship /al-bug hunts #6756-#6855 (batch 71, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "eaf", "ebg", "ech", "edi", "eej", "efk", "egl", "ehm", "ein", "ejo",
    "ekp", "elq", "emr", "ens", "eot", "epu", "eqv", "erw", "esx", "ety",
]
TOPOLOGY = {
    6756 + index * 5: (
        token,
...
CONTEXT_META = {
    "hardened": {
        "ledger_prop": "hardened",
        "commit_prop": "hardened",
        "test_class": "TerraformShowJsonHardenedTests",
        "test_method": "ParseAsync_hardened_maps_tf_hardened_property",
        "summary": "terraform-show-json hardened property gap",
    },
    "restored": {
        "ledger_prop": "restored",
        "commit_prop": "restored",
        "test_class": "TerraformShowJsonRestoredTests",
        "test_method": "ParseAsync_restored_maps_tf_restored_property",
        "summary": "terraform-show-json restored property gap",
    },
    "kind": {
        "ledger_prop": "kind",
        "commit_prop": "kind",
        "test_class": "TerraformShowJsonKindTests",
        "test_method": "ParseAsync_kind_maps_tf_kind_property",
        "summary": "terraform-show-json kind property gap",
    },
    "multi_region": {
        "ledger_prop": "multi_region",
        "commit_prop": "multi_region",
        "test_class": "TerraformShowJsonMultiRegionTests",
        "test_method": "ParseAsync_multi_region_maps_tf_multi_region_property",
        "summary": "terraform-show-json multi_region property gap",
    },
}
...
def add_hardened_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.locations"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "hardened", "hardened"), encoding="utf-8")


def add_restored_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.hardened"] = hardened.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "restored", "restored"), encoding="utf-8")


def add_kind_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.kind"]' in text:
        return
    old = """            properties["tf.restored"] = restored.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text: raise RuntimeError("restored anchor missing")
    RESOURCE_MAPPING.write_text(text.replace(old,new,1),encoding="utf-8")


def add_multi_region_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.kind"] = kindText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "multi_region", "multiRegion"),encoding="utf-8")


def write_hardened_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonHardenedTests.cs"
    if path.exists():
        return path
    path.write_text(
...
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonHardenedTests
{
    ...
    [Fact]
    public async Task ParseAsync_hardened_maps_tf_hardened_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "hardened.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","hardened":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.hardened"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_restored_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRestoredTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRestoredTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_restored_maps_tf_restored_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "restored.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","restored":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.restored"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_kind_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonKindTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonKindTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_kind_maps_tf_kind_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "kind.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","kind":"service","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.kind"].Should().Be("service");
    }
}
''',
        encoding="utf-8",
...
def write_multi_region_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMultiRegionTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMultiRegionTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_multi_region_maps_tf_multi_region_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "multi_region.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","multi_region":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6757:
        add_hardened_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_hardened_test().relative_to(REPO))]
    if hunt == 6782:
        add_restored_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_restored_test().relative_to(REPO))]
    if hunt == 6807:
        add_kind_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_kind_test().relative_to(REPO))]
    if hunt == 6832:
        add_multi_region_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_multi_region_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6756"))
    end_hunt = int(os.environ.get("END_HUNT", "6855"))
