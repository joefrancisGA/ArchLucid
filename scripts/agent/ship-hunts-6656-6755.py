#!/usr/bin/env python3
"""Ship /al-bug hunts #6656-#6755 (batch 70, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "daf", "dbg", "dch", "ddi", "dej", "dfk", "dgl", "dhm", "din", "djo",
    "dkp", "dlq", "dmr", "dns", "dot", "dpu", "dqv", "drw", "dsx", "dty",
]
TOPOLOGY = {
    6656 + index * 5: (
        token,
...
CONTEXT_META = {
    "encrypted": {
        "ledger_prop": "encrypted",
        "commit_prop": "encrypted",
        "test_class": "TerraformShowJsonEncryptedTests",
        "test_method": "ParseAsync_encrypted_maps_tf_encrypted_property",
        "summary": "terraform-show-json encrypted property gap",
    },
    "compressed": {
        "ledger_prop": "compressed",
        "commit_prop": "compressed",
        "test_class": "TerraformShowJsonCompressedTests",
        "test_method": "ParseAsync_compressed_maps_tf_compressed_property",
        "summary": "terraform-show-json compressed property gap",
    },
    "format": {
        "ledger_prop": "format",
        "commit_prop": "format",
        "test_class": "TerraformShowJsonFormatTests",
        "test_method": "ParseAsync_format_maps_tf_format_property",
        "summary": "terraform-show-json format property gap",
    },
    "availability": {
        "ledger_prop": "availability",
        "commit_prop": "availability",
        "test_class": "TerraformShowJsonAvailabilityTests",
        "test_method": "ParseAsync_availability_maps_tf_availability_property",
        "summary": "terraform-show-json availability property gap",
    },
}
...
def add_encrypted_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.locations"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "encrypted", "encrypted"), encoding="utf-8")


def add_compressed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.encrypted"] = encrypted.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "compressed", "compressed"), encoding="utf-8")


def add_format_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.format"]' in text:
        return
    old = """            properties["tf.compressed"] = compressed.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text: raise RuntimeError("compressed anchor missing")
    RESOURCE_MAPPING.write_text(text.replace(old,new,1),encoding="utf-8")


def add_availability_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.format"] = formatText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "availability", "availability", "availability"),encoding="utf-8")


def write_encrypted_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonEncryptedTests.cs"
    if path.exists():
        return path
    path.write_text(
...
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonEncryptedTests
{
    ...
    [Fact]
    public async Task ParseAsync_encrypted_maps_tf_encrypted_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "encrypted.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","encrypted":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.encrypted"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_compressed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCompressedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCompressedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_compressed_maps_tf_compressed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "compressed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","compressed":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.compressed"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_format_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonFormatTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonFormatTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_format_maps_tf_format_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "format.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","format":"json","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.format"].Should().Be("json");
    }
}
''',
        encoding="utf-8",
...
def write_availability_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAvailabilityTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonAvailabilityTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_availability_maps_tf_availability_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "availability.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","availability":["zonal"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.availability"].Should().Be("zonal");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6657:
        add_encrypted_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_encrypted_test().relative_to(REPO))]
    if hunt == 6682:
        add_compressed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_compressed_test().relative_to(REPO))]
    if hunt == 6707:
        add_format_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_format_test().relative_to(REPO))]
    if hunt == 6732:
        add_availability_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_availability_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6656"))
    end_hunt = int(os.environ.get("END_HUNT", "6755"))
