#!/usr/bin/env python3
"""Ship /al-bug hunts #3956-#4055 (batch 43, 100 checkins).
...
CONTEXT_META = {
    "allow_missing": {
        "ledger_prop": "allowMissing",
        "commit_prop": "allowMissing",
        "test_class": "TerraformShowJsonAllowMissingTests",
        "test_method": "ParseAsync_allow_missing_maps_tf_allow_missing_property",
        "summary": "terraform-show-json allowMissing property gap",
    },
    "destroy": {
        "ledger_prop": "destroy",
        "commit_prop": "destroy",
        "test_class": "TerraformShowJsonDestroyTests",
        "test_method": "ParseAsync_destroy_maps_tf_destroy_property",
        "summary": "terraform-show-json destroy property gap",
    },
    "triggers": {
        "ledger_prop": "triggers",
        "commit_prop": "triggers",
        "test_class": "TerraformShowJsonTriggersTests",
        "test_method": "ParseAsync_triggers_maps_tf_triggers_property",
        "summary": "terraform-show-json triggers property gap",
    },
    "conflicts_with": {
        "ledger_prop": "conflictsWith",
        "commit_prop": "conflictsWith",
        "test_class": "TerraformShowJsonConflictsWithTests",
        "test_method": "ParseAsync_conflicts_with_maps_tf_conflicts_with_property",
        "summary": "terraform-show-json conflictsWith property gap",
    },
}
...
def add_allow_missing_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.retain"] = retain.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "allow_missing", "allowMissing"), encoding="utf-8")


def add_destroy_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.allow_missing"] = allowMissing.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "destroy", "destroy"), encoding="utf-8")


def add_triggers_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.moved"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "triggers", "triggers", "triggers"), encoding="utf-8")


def add_conflicts_with_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.triggers"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "conflicts_with", "conflictsWith", "conflicts_with"), encoding="utf-8")
...
def write_allow_missing_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAllowMissingTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_destroy_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDestroyTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_triggers_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonTriggersTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_conflicts_with_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonConflictsWithTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 3957:
        add_allow_missing_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_allow_missing_test().relative_to(REPO))]
    if hunt == 3982:
        add_destroy_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_destroy_test().relative_to(REPO))]
    if hunt == 4007:
        add_triggers_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_triggers_test().relative_to(REPO))]
    if hunt == 4032:
        add_conflicts_with_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_conflicts_with_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "3956"))
    end_hunt = int(os.environ.get("END_HUNT", "4055"))
