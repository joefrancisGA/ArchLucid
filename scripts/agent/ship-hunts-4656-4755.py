#!/usr/bin/env python3
"""Ship /al-bug hunts #4656-#4755 (batch 50, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "jaf", "jbg", "jch", "jdi", "jej", "jfk", "jgl", "jhm", "jin", "jjo",
    "jkp", "jlq", "jmr", "jns", "jot", "jpu", "jqv", "jrw", "jsx", "jty",
]
TOPOLOGY = {
    4656 + index * 5: (
        token,
...
CONTEXT_META = {
    "annotated": {
        "ledger_prop": "annotated",
        "commit_prop": "annotated",
        "test_class": "TerraformShowJsonAnnotatedTests",
        "test_method": "ParseAsync_annotated_maps_tf_annotated_property",
        "summary": "terraform-show-json annotated property gap",
    },
    "secured": {
        "ledger_prop": "secured",
        "commit_prop": "secured",
        "test_class": "TerraformShowJsonSecuredTests",
        "test_method": "ParseAsync_secured_maps_tf_secured_property",
        "summary": "terraform-show-json secured property gap",
    },
    "label": {
        "ledger_prop": "label",
        "commit_prop": "label",
        "test_class": "TerraformShowJsonLabelTests",
        "test_method": "ParseAsync_label_maps_tf_label_property",
        "summary": "terraform-show-json label property gap",
    },
    "siblings": {
        "ledger_prop": "siblings",
        "commit_prop": "siblings",
        "test_class": "TerraformShowJsonSiblingsTests",
        "test_method": "ParseAsync_siblings_maps_tf_siblings_property",
        "summary": "terraform-show-json siblings property gap",
    },
}
...
def add_annotated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.schema"] = schemaText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "annotated", "annotated"), encoding="utf-8")


def add_secured_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.annotated"] = annotated.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "secured", "secured"), encoding="utf-8")


def add_label_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.label"]' in text:
        return
    old = """            properties["tf.secured"] = secured.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("secured anchor not found for label")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_siblings_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.children"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "siblings", "siblings", "siblings"), encoding="utf-8")
...
def write_annotated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAnnotatedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonAnnotatedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_annotated_maps_tf_annotated_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "annotated.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","annotated":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.annotated"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_secured_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSecuredTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSecuredTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_secured_maps_tf_secured_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "secured.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","secured":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.secured"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_label_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonLabelTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonLabelTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_label_maps_tf_label_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "label.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","label":"prod","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.label"].Should().Be("prod");
    }
}
''',
        encoding="utf-8",
...
def write_siblings_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSiblingsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSiblingsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_siblings_maps_tf_siblings_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "siblings.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","siblings":["peer-b"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.siblings"].Should().Be("peer-b");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4657:
        add_annotated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_annotated_test().relative_to(REPO))]
    if hunt == 4682:
        add_secured_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_secured_test().relative_to(REPO))]
    if hunt == 4707:
        add_label_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_label_test().relative_to(REPO))]
    if hunt == 4732:
        add_siblings_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_siblings_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4656"))
    end_hunt = int(os.environ.get("END_HUNT", "4755"))
