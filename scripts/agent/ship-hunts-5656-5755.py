#!/usr/bin/env python3
"""Ship /al-bug hunts #5656-#5755 (batch 60, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "taf", "tbg", "tch", "tdi", "tej", "tfk", "tgl", "thm", "tin", "tjo",
    "tkp", "tlq", "tmr", "tns", "tot", "tpu", "tqv", "trw", "tsx", "tty",
]
TOPOLOGY = {
    5656 + index * 5: (
        token,
...
CONTEXT_META = {
    "completed": {
        "ledger_prop": "completed",
        "commit_prop": "completed",
        "test_class": "TerraformShowJsonCompletedTests",
        "test_method": "ParseAsync_completed_maps_tf_completed_property",
        "summary": "terraform-show-json completed property gap",
    },
    "deleted": {
        "ledger_prop": "deleted",
        "commit_prop": "deleted",
        "test_class": "TerraformShowJsonDeletedTests",
        "test_method": "ParseAsync_deleted_maps_tf_deleted_property",
        "summary": "terraform-show-json deleted property gap",
    },
    "title": {
        "ledger_prop": "title",
        "commit_prop": "title",
        "test_class": "TerraformShowJsonTitleTests",
        "test_method": "ParseAsync_title_maps_tf_title_property",
        "summary": "terraform-show-json title property gap",
    },
    "gateways": {
        "ledger_prop": "gateways",
        "commit_prop": "gateways",
        "test_class": "TerraformShowJsonGatewaysTests",
        "test_method": "ParseAsync_gateways_maps_tf_gateways_property",
        "summary": "terraform-show-json gateways property gap",
    },
}
...
def add_completed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.subnets"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "completed", "completed"), encoding="utf-8")


def add_deleted_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.completed"] = completed.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "deleted", "deleted"), encoding="utf-8")


def add_title_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.title"]' in text:
        return
    old = """            properties["tf.deleted"] = deleted.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("deleted anchor not found for title")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_gateways_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.title"] = titleText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "gateways", "gateways", "gateways"), encoding="utf-8")
...
def write_completed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCompletedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCompletedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_completed_maps_tf_completed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "completed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","completed":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.completed"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_deleted_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDeletedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonDeletedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_deleted_maps_tf_deleted_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "deleted.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","deleted":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.deleted"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_title_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonTitleTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonTitleTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_title_maps_tf_title_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "title.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","title":"primary gateway","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.title"].Should().Be("primary gateway");
    }
}
''',
        encoding="utf-8",
...
def write_gateways_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonGatewaysTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonGatewaysTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_gateways_maps_tf_gateways_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "gateways.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","gateways":["gw-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.gateways"].Should().Be("gw-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5657:
        add_completed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_completed_test().relative_to(REPO))]
    if hunt == 5682:
        add_deleted_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_deleted_test().relative_to(REPO))]
    if hunt == 5707:
        add_title_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_title_test().relative_to(REPO))]
    if hunt == 5732:
        add_gateways_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_gateways_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5656"))
    end_hunt = int(os.environ.get("END_HUNT", "5755"))
