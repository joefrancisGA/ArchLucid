#!/usr/bin/env python3
"""Ship /al-bug hunts #3781-#3855 (batch 41, 75 checkins).
...
_TOPOLOGY_TOKENS = [
    "akp", "alq", "amr", "ans", "aot", "apu", "aqv", "arw", "asx", "aty", "auz", "avb", "awc", "axd", "aye",
]
TOPOLOGY = {
    3781 + index * 5: (
        token,
...
CONTEXT_HITS = {3782: "removed", 3807: "moved", 3832: "write_only"}

CONTEXT_META = {
    "removed": {
        "ledger_prop": "removed",
        "commit_prop": "removed",
        "test_class": "TerraformShowJsonRemovedTests",
        "test_method": "ParseAsync_removed_maps_tf_removed_property",
        "summary": "terraform-show-json removed property gap",
    },
    "moved": {
        "ledger_prop": "moved",
        "commit_prop": "moved",
        "test_class": "TerraformShowJsonMovedTests",
        "test_method": "ParseAsync_moved_maps_tf_moved_property",
        "summary": "terraform-show-json moved property gap",
    },
    "write_only": {
        "ledger_prop": "writeOnly",
        "commit_prop": "writeOnly",
        "test_class": "TerraformShowJsonWriteOnlyTests",
        "test_method": "ParseAsync_write_only_maps_tf_write_only_property",
        "summary": "terraform-show-json writeOnly property gap",
    },
}
...
def add_removed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.postcondition"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "removed", "removed", "removed"), encoding="utf-8")


def add_moved_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.removed"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "moved", "moved", "moved"), encoding="utf-8")


def add_write_only_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.write_only"]' in text:
        return
    old = """        if (TryGetPropertyIgnoreCase(res, "refresh", out JsonElement refresh)
    ...
    if old not in text:
        raise RuntimeError("refresh anchor not found for write_only")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")
...
def write_removed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRemovedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRemovedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_removed_maps_tf_removed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "removed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","removed":["azurerm_resource_group.legacy"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.removed"].Should().Be("azurerm_resource_group.legacy");
    }
}
''',
        encoding="utf-8",
...
def write_moved_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMovedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMovedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_moved_maps_tf_moved_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "moved.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","moved":["azurerm_resource_group.old"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.moved"].Should().Be("azurerm_resource_group.old");
    }
}
''',
        encoding="utf-8",
...
def write_write_only_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonWriteOnlyTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonWriteOnlyTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_write_only_maps_tf_write_only_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "write-only.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","writeOnly":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.write_only"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 3782:
        add_removed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_removed_test().relative_to(REPO))]
    if hunt == 3807:
        add_moved_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_moved_test().relative_to(REPO))]
    if hunt == 3832:
        add_write_only_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_write_only_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "3781"))
    end_hunt = int(os.environ.get("END_HUNT", "3855"))
