#!/usr/bin/env python3
"""Ship /al-bug hunts #5256-#5355 (batch 56, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "paf", "pbg", "pch", "pdi", "pej", "pfk", "pgl", "phm", "pin", "pjo",
    "pkp", "plq", "pmr", "pns", "pot", "ppu", "pqv", "prw", "psx", "pty",
]
TOPOLOGY = {
    5256 + index * 5: (
        token,
...
CONTEXT_META = {
    "approved": {
        "ledger_prop": "approved",
        "commit_prop": "approved",
        "test_class": "TerraformShowJsonApprovedTests",
        "test_method": "ParseAsync_approved_maps_tf_approved_property",
        "summary": "terraform-show-json approved property gap",
    },
    "deprecated": {
        "ledger_prop": "deprecated",
        "commit_prop": "deprecated",
        "test_class": "TerraformShowJsonDeprecatedTests",
        "test_method": "ParseAsync_deprecated_maps_tf_deprecated_property",
        "summary": "terraform-show-json deprecated property gap",
    },
    "purpose": {
        "ledger_prop": "purpose",
        "commit_prop": "purpose",
        "test_class": "TerraformShowJsonPurposeTests",
        "test_method": "ParseAsync_purpose_maps_tf_purpose_property",
        "summary": "terraform-show-json purpose property gap",
    },
    "bindings": {
        "ledger_prop": "bindings",
        "commit_prop": "bindings",
        "test_class": "TerraformShowJsonBindingsTests",
        "test_method": "ParseAsync_bindings_maps_tf_bindings_property",
        "summary": "terraform-show-json bindings property gap",
    },
}
...
def add_approved_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.dependents"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "approved", "approved"), encoding="utf-8")


def add_deprecated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.approved"] = approved.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "deprecated", "deprecated"), encoding="utf-8")


def add_purpose_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.purpose"]' in text:
        return
    old = """            properties["tf.deprecated"] = deprecated.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("deprecated anchor not found for purpose")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_bindings_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.purpose"] = purposeText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "bindings", "bindings", "bindings"), encoding="utf-8")
...
def write_approved_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonApprovedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonApprovedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_approved_maps_tf_approved_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "approved.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","approved":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.approved"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_deprecated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDeprecatedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_purpose_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonPurposeTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_bindings_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonBindingsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonBindingsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_bindings_maps_tf_bindings_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "bindings.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","bindings":["bind-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.bindings"].Should().Be("bind-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5257:
        add_approved_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_approved_test().relative_to(REPO))]
    if hunt == 5282:
        add_deprecated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_deprecated_test().relative_to(REPO))]
    if hunt == 5307:
        add_purpose_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_purpose_test().relative_to(REPO))]
    if hunt == 5332:
        add_bindings_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_bindings_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5256"))
    end_hunt = int(os.environ.get("END_HUNT", "5355"))
