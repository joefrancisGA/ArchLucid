#!/usr/bin/env python3
"""Ship /al-bug hunts #4956-#5055 (batch 53, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "maf", "mbg", "mch", "mdi", "mej", "mfk", "mgl", "mhm", "min", "mjo",
    "mkp", "mlq", "mmr", "mns", "mot", "mpu", "mqv", "mrw", "msx", "mty",
]
TOPOLOGY = {
    4956 + index * 5: (
        token,
...
CONTEXT_META = {
    "active": {
        "ledger_prop": "active",
        "commit_prop": "active",
        "test_class": "TerraformShowJsonActiveTests",
        "test_method": "ParseAsync_active_maps_tf_active_property",
        "summary": "terraform-show-json active property gap",
    },
    "managed": {
        "ledger_prop": "managed",
        "commit_prop": "managed",
        "test_class": "TerraformShowJsonManagedTests",
        "test_method": "ParseAsync_managed_maps_tf_managed_property",
        "summary": "terraform-show-json managed property gap",
    },
    "role": {
        "ledger_prop": "role",
        "commit_prop": "role",
        "test_class": "TerraformShowJsonRoleTests",
        "test_method": "ParseAsync_role_maps_tf_role_property",
        "summary": "terraform-show-json role property gap",
    },
    "peers": {
        "ledger_prop": "peers",
        "commit_prop": "peers",
        "test_class": "TerraformShowJsonPeersTests",
        "test_method": "ParseAsync_peers_maps_tf_peers_property",
        "summary": "terraform-show-json peers property gap",
    },
}
...
def add_active_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.descendants"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "active", "active"), encoding="utf-8")


def add_managed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.active"] = active.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "managed", "managed"), encoding="utf-8")


def add_role_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.role"]' in text:
        return
    old = """            properties["tf.managed"] = managed.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("managed anchor not found for role")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_peers_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.role"] = roleText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "peers", "peers", "peers"), encoding="utf-8")
...
def write_active_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonActiveTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonActiveTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_active_maps_tf_active_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "active.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","active":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.active"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_managed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonManagedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonManagedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_managed_maps_tf_managed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "managed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","managed":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.managed"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_role_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRoleTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRoleTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_role_maps_tf_role_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "role.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","role":"contributor","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.role"].Should().Be("contributor");
    }
}
''',
        encoding="utf-8",
...
def write_peers_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonPeersTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonPeersTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_peers_maps_tf_peers_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "peers.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","peers":["peer-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.peers"].Should().Be("peer-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4957:
        add_active_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_active_test().relative_to(REPO))]
    if hunt == 4982:
        add_managed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_managed_test().relative_to(REPO))]
    if hunt == 5007:
        add_role_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_role_test().relative_to(REPO))]
    if hunt == 5032:
        add_peers_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_peers_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4956"))
    end_hunt = int(os.environ.get("END_HUNT", "5055"))
