#!/usr/bin/env python3
"""Ship /al-bug hunts #4556-#4655 (batch 49, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "iaf", "ibg", "ich", "idi", "iej", "ifk", "igl", "ihm", "iin", "ijo",
    "ikp", "ilq", "imr", "ins", "iot", "ipu", "iqv", "irw", "isx", "ity",
]
TOPOLOGY = {
    4556 + index * 5: (
        token,
...
CONTEXT_META = {
    "checkpoint": {
        "ledger_prop": "checkpoint",
        "commit_prop": "checkpoint",
        "test_class": "TerraformShowJsonCheckpointTests",
        "test_method": "ParseAsync_checkpoint_maps_tf_checkpoint_property",
        "summary": "terraform-show-json checkpoint property gap",
    },
    "revoked": {
        "ledger_prop": "revoked",
        "commit_prop": "revoked",
        "test_class": "TerraformShowJsonRevokedTests",
        "test_method": "ParseAsync_revoked_maps_tf_revoked_property",
        "summary": "terraform-show-json revoked property gap",
    },
    "schema": {
        "ledger_prop": "schema",
        "commit_prop": "schema",
        "test_class": "TerraformShowJsonSchemaTests",
        "test_method": "ParseAsync_schema_maps_tf_schema_property",
        "summary": "terraform-show-json schema property gap",
    },
    "children": {
        "ledger_prop": "children",
        "commit_prop": "children",
        "test_class": "TerraformShowJsonChildrenTests",
        "test_method": "ParseAsync_children_maps_tf_children_property",
        "summary": "terraform-show-json children property gap",
    },
}
...
def add_checkpoint_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.target"] = targetText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "checkpoint", "checkpoint"), encoding="utf-8")


def add_revoked_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.checkpoint"] = checkpoint.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "revoked", "revoked"), encoding="utf-8")


def add_schema_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.schema"]' in text:
        return
    old = """            properties["tf.revoked"] = revoked.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("revoked anchor not found for schema")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_children_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.members"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "children", "children", "children"), encoding="utf-8")
...
def write_checkpoint_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCheckpointTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCheckpointTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_checkpoint_maps_tf_checkpoint_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "checkpoint.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","checkpoint":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.checkpoint"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_revoked_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRevokedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonRevokedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_revoked_maps_tf_revoked_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "revoked.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","revoked":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.revoked"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_schema_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSchemaTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSchemaTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_schema_maps_tf_schema_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "schema.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","schema":"v2","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.schema"].Should().Be("v2");
    }
}
''',
        encoding="utf-8",
...
def write_children_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonChildrenTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonChildrenTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_children_maps_tf_children_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "children.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","children":["child-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.children"].Should().Be("child-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4557:
        add_checkpoint_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_checkpoint_test().relative_to(REPO))]
    if hunt == 4582:
        add_revoked_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_revoked_test().relative_to(REPO))]
    if hunt == 4607:
        add_schema_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_schema_test().relative_to(REPO))]
    if hunt == 4632:
        add_children_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_children_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4556"))
    end_hunt = int(os.environ.get("END_HUNT", "4655"))
