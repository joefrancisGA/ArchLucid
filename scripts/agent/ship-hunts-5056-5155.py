#!/usr/bin/env python3
"""Ship /al-bug hunts #5056-#5155 (batch 54, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "naf", "nbg", "nch", "ndi", "nej", "nfk", "ngl", "nhm", "nin", "njo",
    "nkp", "nlq", "nmr", "nns", "not", "npu", "nqv", "nrw", "nsx", "nty",
]
TOPOLOGY = {
    5056 + index * 5: (
        token,
...
CONTEXT_META = {
    "enabled": {
        "ledger_prop": "enabled",
        "commit_prop": "enabled",
        "test_class": "TerraformShowJsonEnabledTests",
        "test_method": "ParseAsync_enabled_maps_tf_enabled_property",
        "summary": "terraform-show-json enabled property gap",
    },
    "exposed": {
        "ledger_prop": "exposed",
        "commit_prop": "exposed",
        "test_class": "TerraformShowJsonExposedTests",
        "test_method": "ParseAsync_exposed_maps_tf_exposed_property",
        "summary": "terraform-show-json exposed property gap",
    },
    "comment": {
        "ledger_prop": "comment",
        "commit_prop": "comment",
        "test_class": "TerraformShowJsonCommentTests",
        "test_method": "ParseAsync_comment_maps_tf_comment_property",
        "summary": "terraform-show-json comment property gap",
    },
    "references": {
        "ledger_prop": "references",
        "commit_prop": "references",
        "test_class": "TerraformShowJsonReferencesTests",
        "test_method": "ParseAsync_references_maps_tf_references_property",
        "summary": "terraform-show-json references property gap",
    },
}
...
def add_enabled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.peers"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "enabled", "enabled"), encoding="utf-8")


def add_exposed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.enabled"] = enabled.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "exposed", "exposed"), encoding="utf-8")


def add_comment_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.comment"]' in text:
        return
    old = """            properties["tf.exposed"] = exposed.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("exposed anchor not found for comment")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_references_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.comment"] = commentText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "references", "references", "references"), encoding="utf-8")
...
def write_enabled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonEnabledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonEnabledTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_enabled_maps_tf_enabled_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "enabled.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","enabled":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.enabled"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_exposed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonExposedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonExposedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_exposed_maps_tf_exposed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "exposed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","exposed":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.exposed"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_comment_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCommentTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCommentTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_comment_maps_tf_comment_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "comment.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","comment":"shared services","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.comment"].Should().Be("shared services");
    }
}
''',
        encoding="utf-8",
...
def write_references_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReferencesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonReferencesTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_references_maps_tf_references_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "references.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","references":["ref-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.references"].Should().Be("ref-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5057:
        add_enabled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_enabled_test().relative_to(REPO))]
    if hunt == 5082:
        add_exposed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_exposed_test().relative_to(REPO))]
    if hunt == 5107:
        add_comment_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_comment_test().relative_to(REPO))]
    if hunt == 5132:
        add_references_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_references_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5056"))
    end_hunt = int(os.environ.get("END_HUNT", "5155"))
