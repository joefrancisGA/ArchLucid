#!/usr/bin/env python3
"""Ship /al-bug hunts #5856-#5955 (batch 62, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "vaf", "vbg", "vch", "vdi", "vej", "vfk", "vgl", "vhm", "vin", "vjo",
    "vkp", "vlq", "vmr", "vns", "vot", "vpu", "vqv", "vrw", "vsx", "vty",
]
TOPOLOGY = {
    5856 + index * 5: (
        token,
...
CONTEXT_META = {
    "committed": {
        "ledger_prop": "committed",
        "commit_prop": "committed",
        "test_class": "TerraformShowJsonCommittedTests",
        "test_method": "ParseAsync_committed_maps_tf_committed_property",
        "summary": "terraform-show-json committed property gap",
    },
    "failed": {
        "ledger_prop": "failed",
        "commit_prop": "failed",
        "test_class": "TerraformShowJsonFailedTests",
        "test_method": "ParseAsync_failed_maps_tf_failed_property",
        "summary": "terraform-show-json failed property gap",
    },
    "message": {
        "ledger_prop": "message",
        "commit_prop": "message",
        "test_class": "TerraformShowJsonMessageTests",
        "test_method": "ParseAsync_message_maps_tf_message_property",
        "summary": "terraform-show-json message property gap",
    },
    "listeners": {
        "ledger_prop": "listeners",
        "commit_prop": "listeners",
        "test_class": "TerraformShowJsonListenersTests",
        "test_method": "ParseAsync_listeners_maps_tf_listeners_property",
        "summary": "terraform-show-json listeners property gap",
    },
}
...
def add_committed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.routes"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "committed", "committed"), encoding="utf-8")


def add_failed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.committed"] = committed.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "failed", "failed"), encoding="utf-8")


def add_message_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.message"]' in text:
        return
    old = """            properties["tf.failed"] = failed.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("failed anchor not found for message")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_listeners_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.message"] = messageText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "listeners", "listeners", "listeners"), encoding="utf-8")
...
def write_committed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCommittedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCommittedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_committed_maps_tf_committed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "committed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","committed":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.committed"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_failed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonFailedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonFailedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_failed_maps_tf_failed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "failed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","failed":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.failed"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_message_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMessageTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMessageTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_message_maps_tf_message_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "message.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","message":"apply succeeded","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.message"].Should().Be("apply succeeded");
    }
}
''',
        encoding="utf-8",
...
def write_listeners_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonListenersTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonListenersTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_listeners_maps_tf_listeners_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "listeners.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","listeners":["listener-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.listeners"].Should().Be("listener-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5857:
        add_committed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_committed_test().relative_to(REPO))]
    if hunt == 5882:
        add_failed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_failed_test().relative_to(REPO))]
    if hunt == 5907:
        add_message_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_message_test().relative_to(REPO))]
    if hunt == 5932:
        add_listeners_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_listeners_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5856"))
    end_hunt = int(os.environ.get("END_HUNT", "5955"))
