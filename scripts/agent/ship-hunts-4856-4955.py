#!/usr/bin/env python3
"""Ship /al-bug hunts #4856-#4955 (batch 52, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "laf", "lbg", "lch", "ldi", "lej", "lfk", "lgl", "lhm", "lin", "ljo",
    "lkp", "llq", "lmr", "lns", "lot", "lpu", "lqv", "lrw", "lsx", "lty",
]
TOPOLOGY = {
    4856 + index * 5: (
        token,
...
CONTEXT_META = {
    "pending": {
        "ledger_prop": "pending",
        "commit_prop": "pending",
        "test_class": "TerraformShowJsonPendingTests",
        "test_method": "ParseAsync_pending_maps_tf_pending_property",
        "summary": "terraform-show-json pending property gap",
    },
    "visible": {
        "ledger_prop": "visible",
        "commit_prop": "visible",
        "test_class": "TerraformShowJsonVisibleTests",
        "test_method": "ParseAsync_visible_maps_tf_visible_property",
        "summary": "terraform-show-json visible property gap",
    },
    "description": {
        "ledger_prop": "description",
        "commit_prop": "description",
        "test_class": "TerraformShowJsonDescriptionTests",
        "test_method": "ParseAsync_description_maps_tf_description_property",
        "summary": "terraform-show-json description property gap",
    },
    "descendants": {
        "ledger_prop": "descendants",
        "commit_prop": "descendants",
        "test_class": "TerraformShowJsonDescendantsTests",
        "test_method": "ParseAsync_descendants_maps_tf_descendants_property",
        "summary": "terraform-show-json descendants property gap",
    },
}
...
def add_pending_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.ancestors"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "pending", "pending"), encoding="utf-8")


def add_visible_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.pending"] = pending.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "visible", "visible"), encoding="utf-8")


def add_description_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.description"]' in text:
        return
    old = """            properties["tf.visible"] = visible.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("visible anchor not found for description")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_descendants_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.description"] = descriptionText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "descendants", "descendants", "descendants"), encoding="utf-8")
...
def write_pending_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonPendingTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonPendingTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_pending_maps_tf_pending_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "pending.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","pending":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.pending"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_visible_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonVisibleTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonVisibleTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_visible_maps_tf_visible_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "visible.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","visible":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.visible"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_description_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDescriptionTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonDescriptionTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_description_maps_tf_description_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "description.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","description":"core workload","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.description"].Should().Be("core workload");
    }
}
''',
        encoding="utf-8",
...
def write_descendants_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDescendantsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4857:
        add_pending_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_pending_test().relative_to(REPO))]
    if hunt == 4882:
        add_visible_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_visible_test().relative_to(REPO))]
    if hunt == 4907:
        add_description_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_description_test().relative_to(REPO))]
    if hunt == 4932:
        add_descendants_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_descendants_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4856"))
    end_hunt = int(os.environ.get("END_HUNT", "4955"))
