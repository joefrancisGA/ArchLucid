#!/usr/bin/env python3
"""Ship /al-bug hunts #6256-#6355 (batch 66, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "zaf", "zbg", "zch", "zdi", "zej", "zfk", "zgl", "zhm", "zin", "zjo",
    "zkp", "zlq", "zmr", "zns", "zot", "zpu", "zqv", "zrw", "zsx", "zty",
]
TOPOLOGY = {
    6256 + index * 5: (
        token,
...
CONTEXT_META = {
    "mirrored": {
        "ledger_prop": "mirrored",
        "commit_prop": "mirrored",
        "test_class": "TerraformShowJsonMirroredTests",
        "test_method": "ParseAsync_mirrored_maps_tf_mirrored_property",
        "summary": "terraform-show-json mirrored property gap",
    },
    "blocked": {
        "ledger_prop": "blocked",
        "commit_prop": "blocked",
        "test_class": "TerraformShowJsonBlockedTests",
        "test_method": "ParseAsync_blocked_maps_tf_blocked_property",
        "summary": "terraform-show-json blocked property gap",
    },
    "priority": {
        "ledger_prop": "priority",
        "commit_prop": "priority",
        "test_class": "TerraformShowJsonPriorityTests",
        "test_method": "ParseAsync_priority_maps_tf_priority_property",
        "summary": "terraform-show-json priority property gap",
    },
    "zones": {
        "ledger_prop": "zones",
        "commit_prop": "zones",
        "test_class": "TerraformShowJsonZonesTests",
        "test_method": "ParseAsync_zones_maps_tf_zones_property",
        "summary": "terraform-show-json zones property gap",
    },
}
...
def add_mirrored_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.regions"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "mirrored", "mirrored"), encoding="utf-8")


def add_blocked_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.mirrored"] = mirrored.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "blocked", "blocked"), encoding="utf-8")


def add_priority_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.priority"]' in text:
        return
    old = """            properties["tf.blocked"] = blocked.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("blocked anchor not found for priority")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_zones_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.priority"] = priorityText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "zones", "zones", "zones"), encoding="utf-8")
...
def write_mirrored_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMirroredTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMirroredTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_mirrored_maps_tf_mirrored_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "mirrored.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","mirrored":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.mirrored"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_blocked_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonBlockedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonBlockedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_blocked_maps_tf_blocked_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "blocked.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","blocked":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.blocked"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_priority_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonPriorityTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonPriorityTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_priority_maps_tf_priority_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "priority.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","priority":"high","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.priority"].Should().Be("high");
    }
}
''',
        encoding="utf-8",
...
def write_zones_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonZonesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonZonesTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_zones_maps_tf_zones_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "zones.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","zones":["zone-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.zones"].Should().Be("zone-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6257:
        add_mirrored_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_mirrored_test().relative_to(REPO))]
    if hunt == 6282:
        add_blocked_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_blocked_test().relative_to(REPO))]
    if hunt == 6307:
        add_priority_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_priority_test().relative_to(REPO))]
    if hunt == 6332:
        add_zones_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_zones_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6256"))
    end_hunt = int(os.environ.get("END_HUNT", "6355"))
