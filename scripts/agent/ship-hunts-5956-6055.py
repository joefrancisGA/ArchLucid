#!/usr/bin/env python3
"""Ship /al-bug hunts #5956-#6055 (batch 63, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "waf", "wbg", "wch", "wdi", "wej", "wfk", "wgl", "whm", "win", "wjo",
    "wkp", "wlq", "wmr", "wns", "wot", "wpu", "wqv", "wrw", "wsx", "wty",
]
TOPOLOGY = {
    5956 + index * 5: (
        token,
...
CONTEXT_META = {
    "isolated": {
        "ledger_prop": "isolated",
        "commit_prop": "isolated",
        "test_class": "TerraformShowJsonIsolatedTests",
        "test_method": "ParseAsync_isolated_maps_tf_isolated_property",
        "summary": "terraform-show-json isolated property gap",
    },
    "flagged": {
        "ledger_prop": "flagged",
        "commit_prop": "flagged",
        "test_class": "TerraformShowJsonFlaggedTests",
        "test_method": "ParseAsync_flagged_maps_tf_flagged_property",
        "summary": "terraform-show-json flagged property gap",
    },
    "status": {
        "ledger_prop": "status",
        "commit_prop": "status",
        "test_class": "TerraformShowJsonStatusTests",
        "test_method": "ParseAsync_status_maps_tf_status_property",
        "summary": "terraform-show-json status property gap",
    },
    "rules": {
        "ledger_prop": "rules",
        "commit_prop": "rules",
        "test_class": "TerraformShowJsonRulesTests",
        "test_method": "ParseAsync_rules_maps_tf_rules_property",
        "summary": "terraform-show-json rules property gap",
    },
}
...
def add_isolated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.listeners"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "isolated", "isolated"), encoding="utf-8")


def add_flagged_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.isolated"] = isolated.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "flagged", "flagged"), encoding="utf-8")


def add_status_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.status"]' in text:
        return
    old = """            properties["tf.flagged"] = flagged.GetBoolean() ? "true" : "false";
        }"""
    new = old + """

        if ((TryGetPropertyIgnoreCase(res, "status", out JsonElement status)
                || TryGetPropertyIgnoreCase(res, "status", out status))
            && status.ValueKind == JsonValueKind.String)
        {
            string? statusText = status.GetString();

            if (!string.IsNullOrWhiteSpace(statusText))
                properties["tf.status"] = statusText.Trim();
        }"""
    if old not in text:
        raise RuntimeError("flagged anchor not found for status")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_rules_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.status"] = statusText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "rules", "rules", "rules"), encoding="utf-8")
...
def write_isolated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonIsolatedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_flagged_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonFlaggedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonFlaggedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_flagged_maps_tf_flagged_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "flagged.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","flagged":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.flagged"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_status_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonStatusTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonStatusTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_status_maps_tf_status_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "status.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","status":"healthy","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.status"].Should().Be("healthy");
    }
}
''',
        encoding="utf-8",
...
def write_rules_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRulesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5957:
        add_isolated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_isolated_test().relative_to(REPO))]
    if hunt == 5982:
        add_flagged_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_flagged_test().relative_to(REPO))]
    if hunt == 6007:
        add_status_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_status_test().relative_to(REPO))]
    if hunt == 6032:
        add_rules_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_rules_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5956"))
    end_hunt = int(os.environ.get("END_HUNT", "6055"))
