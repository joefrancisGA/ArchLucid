#!/usr/bin/env python3
"""Ship /al-bug hunts #4356-#4455 (batch 47, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "gaf", "gbg", "gch", "gdi", "gej", "gfk", "ggl", "ghm", "gin", "gjo",
    "gkp", "glq", "gmr", "gns", "got", "gpu", "gqv", "grw", "gsx", "gty",
]
TOPOLOGY = {
    4356 + index * 5: (
        token,
...
CONTEXT_META = {
    "declared": {
        "ledger_prop": "declared",
        "commit_prop": "declared",
        "test_class": "TerraformShowJsonDeclaredTests",
        "test_method": "ParseAsync_declared_maps_tf_declared_property",
        "summary": "terraform-show-json declared property gap",
    },
    "orphan": {
        "ledger_prop": "orphan",
        "commit_prop": "orphan",
        "test_class": "TerraformShowJsonOrphanTests",
        "test_method": "ParseAsync_orphan_maps_tf_orphan_property",
        "summary": "terraform-show-json orphan property gap",
    },
    "module": {
        "ledger_prop": "module",
        "commit_prop": "module",
        "test_class": "TerraformShowJsonModuleTests",
        "test_method": "ParseAsync_module_maps_tf_module_property",
        "summary": "terraform-show-json module property gap",
    },
    "overrides": {
        "ledger_prop": "overrides",
        "commit_prop": "overrides",
        "test_class": "TerraformShowJsonOverridesTests",
        "test_method": "ParseAsync_overrides_maps_tf_overrides_property",
        "summary": "terraform-show-json overrides property gap",
    },
}
...
def add_declared_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.version"] = versionText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "declared", "declared"), encoding="utf-8")


def add_orphan_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.declared"] = declared.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "orphan", "orphan"), encoding="utf-8")


def add_module_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.module"]' in text:
        return
    old = """            properties["tf.orphan"] = orphan.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("orphan anchor not found for module")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_overrides_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.parents"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "overrides", "overrides", "overrides"), encoding="utf-8")
...
def write_declared_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDeclaredTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonDeclaredTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_declared_maps_tf_declared_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "declared.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","declared":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.declared"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_orphan_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonOrphanTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonOrphanTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_orphan_maps_tf_orphan_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "orphan.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","orphan":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.orphan"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_module_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonModuleTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonModuleTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_module_maps_tf_module_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "module.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","module":"child","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.module"].Should().Be("child");
    }
}
''',
        encoding="utf-8",
...
def write_overrides_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonOverridesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonOverridesTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_overrides_maps_tf_overrides_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "overrides.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","overrides":["size"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.overrides"].Should().Be("size");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4357:
        add_declared_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_declared_test().relative_to(REPO))]
    if hunt == 4382:
        add_orphan_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_orphan_test().relative_to(REPO))]
    if hunt == 4407:
        add_module_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_module_test().relative_to(REPO))]
    if hunt == 4432:
        add_overrides_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_overrides_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4356"))
    end_hunt = int(os.environ.get("END_HUNT", "4455"))
