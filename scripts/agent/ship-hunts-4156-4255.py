#!/usr/bin/env python3
"""Ship /al-bug hunts #4156-#4255 (batch 45, 100 checkins).
...
CONTEXT_META = {
    "disabled": {
        "ledger_prop": "disabled",
        "commit_prop": "disabled",
        "test_class": "TerraformShowJsonDisabledTests",
        "test_method": "ParseAsync_disabled_maps_tf_disabled_property",
        "summary": "terraform-show-json disabled property gap",
    },
    "locked": {
        "ledger_prop": "locked",
        "commit_prop": "locked",
        "test_class": "TerraformShowJsonLockedTests",
        "test_method": "ParseAsync_locked_maps_tf_locked_property",
        "summary": "terraform-show-json locked property gap",
    },
    "source": {
        "ledger_prop": "source",
        "commit_prop": "source",
        "test_class": "TerraformShowJsonSourceTests",
        "test_method": "ParseAsync_source_maps_tf_source_property",
        "summary": "terraform-show-json source property gap",
    },
    "aliases": {
        "ledger_prop": "aliases",
        "commit_prop": "aliases",
        "test_class": "TerraformShowJsonAliasesTests",
        "test_method": "ParseAsync_aliases_maps_tf_aliases_property",
        "summary": "terraform-show-json aliases property gap",
    },
}
...
def add_disabled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.replace"] = replace.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "disabled", "disabled"), encoding="utf-8")


def add_locked_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.disabled"] = disabled.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "locked", "locked"), encoding="utf-8")


def add_source_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.source"]' in text:
        return
    old = """            properties["tf.locked"] = locked.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("locked anchor not found for source")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_aliases_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.replace_on_changes"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "aliases", "aliases", "aliases"), encoding="utf-8")
...
def write_disabled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDisabledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonDisabledTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_disabled_maps_tf_disabled_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "disabled.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","disabled":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.disabled"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_locked_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonLockedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonLockedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_locked_maps_tf_locked_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "locked.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","locked":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.locked"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_source_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSourceTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_aliases_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonAliasesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonAliasesTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_aliases_maps_tf_aliases_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "aliases.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","aliases":["peer"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.aliases"].Should().Be("peer");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 4157:
        add_disabled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_disabled_test().relative_to(REPO))]
    if hunt == 4182:
        add_locked_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_locked_test().relative_to(REPO))]
    if hunt == 4207:
        add_source_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_source_test().relative_to(REPO))]
    if hunt == 4232:
        add_aliases_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_aliases_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "4156"))
    end_hunt = int(os.environ.get("END_HUNT", "4255"))
