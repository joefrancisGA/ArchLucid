#!/usr/bin/env python3
"""Ship /al-bug hunts #5156-#5255 (batch 55, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "oaf", "obg", "och", "odi", "oej", "ofk", "ogl", "ohm", "oin", "ojo",
    "okp", "olq", "omr", "ons", "oot", "opu", "oqv", "orw", "osx", "oty",
]
TOPOLOGY = {
    5156 + index * 5: (
        token,
...
CONTEXT_META = {
    "tracked": {
        "ledger_prop": "tracked",
        "commit_prop": "tracked",
        "test_class": "TerraformShowJsonTrackedTests",
        "test_method": "ParseAsync_tracked_maps_tf_tracked_property",
        "summary": "terraform-show-json tracked property gap",
    },
    "published": {
        "ledger_prop": "published",
        "commit_prop": "published",
        "test_class": "TerraformShowJsonPublishedTests",
        "test_method": "ParseAsync_published_maps_tf_published_property",
        "summary": "terraform-show-json published property gap",
    },
    "caption": {
        "ledger_prop": "caption",
        "commit_prop": "caption",
        "test_class": "TerraformShowJsonCaptionTests",
        "test_method": "ParseAsync_caption_maps_tf_caption_property",
        "summary": "terraform-show-json caption property gap",
    },
    "dependents": {
        "ledger_prop": "dependents",
        "commit_prop": "dependents",
        "test_class": "TerraformShowJsonDependentsTests",
        "test_method": "ParseAsync_dependents_maps_tf_dependents_property",
        "summary": "terraform-show-json dependents property gap",
    },
}
...
def add_tracked_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.references"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "tracked", "tracked"), encoding="utf-8")


def add_published_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.tracked"] = tracked.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "published", "published"), encoding="utf-8")


def add_caption_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.caption"]' in text:
        return
    old = """            properties["tf.published"] = published.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("published anchor not found for caption")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_dependents_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.caption"] = captionText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "dependents", "dependents", "dependents"), encoding="utf-8")
...
def write_tracked_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonTrackedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_published_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonPublishedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonPublishedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_published_maps_tf_published_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "published.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","published":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.published"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_caption_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCaptionTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_dependents_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonDependentsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5157:
        add_tracked_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_tracked_test().relative_to(REPO))]
    if hunt == 5182:
        add_published_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_published_test().relative_to(REPO))]
    if hunt == 5207:
        add_caption_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_caption_test().relative_to(REPO))]
    if hunt == 5232:
        add_dependents_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_dependents_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5156"))
    end_hunt = int(os.environ.get("END_HUNT", "5255"))
