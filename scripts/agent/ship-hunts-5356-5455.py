#!/usr/bin/env python3
"""Ship /al-bug hunts #5356-#5455 (batch 57, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "qaf", "qbg", "qch", "qdi", "qej", "qfk", "qgl", "qhm", "qin", "qjo",
    "qkp", "qlq", "qmr", "qns", "qot", "qpu", "qqv", "qrw", "qsx", "qty",
]
TOPOLOGY = {
    5356 + index * 5: (
        token,
...
CONTEXT_META = {
    "finalized": {
        "ledger_prop": "finalized",
        "commit_prop": "finalized",
        "test_class": "TerraformShowJsonFinalizedTests",
        "test_method": "ParseAsync_finalized_maps_tf_finalized_property",
        "summary": "terraform-show-json finalized property gap",
    },
    "reviewed": {
        "ledger_prop": "reviewed",
        "commit_prop": "reviewed",
        "test_class": "TerraformShowJsonReviewedTests",
        "test_method": "ParseAsync_reviewed_maps_tf_reviewed_property",
        "summary": "terraform-show-json reviewed property gap",
    },
    "summary": {
        "ledger_prop": "summary",
        "commit_prop": "summary",
        "test_class": "TerraformShowJsonSummaryTests",
        "test_method": "ParseAsync_summary_maps_tf_summary_property",
        "summary": "terraform-show-json summary property gap",
    },
    "providers": {
        "ledger_prop": "providers",
        "commit_prop": "providers",
        "test_class": "TerraformShowJsonProvidersTests",
        "test_method": "ParseAsync_providers_maps_tf_providers_property",
        "summary": "terraform-show-json providers property gap",
    },
}
...
def add_finalized_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.bindings"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "finalized", "finalized"), encoding="utf-8")


def add_reviewed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.finalized"] = finalized.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "reviewed", "reviewed"), encoding="utf-8")


def add_summary_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.summary"]' in text:
        return
    old = """            properties["tf.reviewed"] = reviewed.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("reviewed anchor not found for summary")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_providers_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.summary"] = summaryText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "providers", "providers", "providers"), encoding="utf-8")
...
def write_finalized_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonFinalizedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonFinalizedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_finalized_maps_tf_finalized_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "finalized.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","finalized":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.finalized"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_reviewed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReviewedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonReviewedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_reviewed_maps_tf_reviewed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "reviewed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","reviewed":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.reviewed"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_summary_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSummaryTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSummaryTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_summary_maps_tf_summary_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "summary.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","summary":"core network","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.summary"].Should().Be("core network");
    }
}
''',
        encoding="utf-8",
...
def write_providers_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonProvidersTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5357:
        add_finalized_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_finalized_test().relative_to(REPO))]
    if hunt == 5382:
        add_reviewed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_reviewed_test().relative_to(REPO))]
    if hunt == 5407:
        add_summary_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_summary_test().relative_to(REPO))]
    if hunt == 5432:
        add_providers_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_providers_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5356"))
    end_hunt = int(os.environ.get("END_HUNT", "5455"))
