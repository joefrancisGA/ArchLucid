#!/usr/bin/env python3
"""Ship /al-bug hunts #5756-#5855 (batch 61, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "uaf", "ubg", "uch", "udi", "uej", "ufk", "ugl", "uhm", "uin", "ujo",
    "ukp", "ulq", "umr", "uns", "uot", "upu", "uqv", "urw", "usx", "uty",
]
TOPOLOGY = {
    5756 + index * 5: (
        token,
...
CONTEXT_META = {
    "scheduled": {
        "ledger_prop": "scheduled",
        "commit_prop": "scheduled",
        "test_class": "TerraformShowJsonScheduledTests",
        "test_method": "ParseAsync_scheduled_maps_tf_scheduled_property",
        "summary": "terraform-show-json scheduled property gap",
    },
    "expired": {
        "ledger_prop": "expired",
        "commit_prop": "expired",
        "test_class": "TerraformShowJsonExpiredTests",
        "test_method": "ParseAsync_expired_maps_tf_expired_property",
        "summary": "terraform-show-json expired property gap",
    },
    "reason": {
        "ledger_prop": "reason",
        "commit_prop": "reason",
        "test_class": "TerraformShowJsonReasonTests",
        "test_method": "ParseAsync_reason_maps_tf_reason_property",
        "summary": "terraform-show-json reason property gap",
    },
    "routes": {
        "ledger_prop": "routes",
        "commit_prop": "routes",
        "test_class": "TerraformShowJsonRoutesTests",
        "test_method": "ParseAsync_routes_maps_tf_routes_property",
        "summary": "terraform-show-json routes property gap",
    },
}
...
def add_scheduled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.gateways"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "scheduled", "scheduled"), encoding="utf-8")


def add_expired_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.scheduled"] = scheduled.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "expired", "expired"), encoding="utf-8")


def add_reason_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.reason"]' in text:
        return
    old = """            properties["tf.expired"] = expired.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("expired anchor not found for reason")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_routes_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.reason"] = reasonText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "routes", "routes", "routes"), encoding="utf-8")
...
def write_scheduled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonScheduledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_expired_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonExpiredTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def write_reason_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonReasonTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonReasonTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_reason_maps_tf_reason_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "reason.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","reason":"maintenance window","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.reason"].Should().Be("maintenance window");
    }
}
''',
        encoding="utf-8",
...
def write_routes_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonRoutesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5757:
        add_scheduled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_scheduled_test().relative_to(REPO))]
    if hunt == 5782:
        add_expired_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_expired_test().relative_to(REPO))]
    if hunt == 5807:
        add_reason_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_reason_test().relative_to(REPO))]
    if hunt == 5832:
        add_routes_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_routes_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5756"))
    end_hunt = int(os.environ.get("END_HUNT", "5855"))
