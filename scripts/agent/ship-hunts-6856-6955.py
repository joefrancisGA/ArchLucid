#!/usr/bin/env python3
"""Ship /al-bug hunts #6856-#6955 (batch 72, 100 checkins).
...
CONTEXT_META = {
    "indexed": {
        "ledger_prop": "indexed",
        "commit_prop": "indexed",
        "test_class": "TerraformShowJsonIndexedTests",
        "test_method": "ParseAsync_indexed_maps_tf_indexed_property",
        "summary": "terraform-show-json indexed property gap",
    },
    "migrated": {
        "ledger_prop": "migrated",
        "commit_prop": "migrated",
        "test_class": "TerraformShowJsonMigratedTests",
        "test_method": "ParseAsync_migrated_maps_tf_migrated_property",
        "summary": "terraform-show-json migrated property gap",
    },
    "protocol": {
        "ledger_prop": "protocol",
        "commit_prop": "protocol",
        "test_class": "TerraformShowJsonProtocolTests",
        "test_method": "ParseAsync_protocol_maps_tf_protocol_property",
        "summary": "terraform-show-json protocol property gap",
    },
    "failover_regions": {
        "ledger_prop": "failover_regions",
        "commit_prop": "failover_regions",
        "test_class": "TerraformShowJsonFailoverRegionsTests",
        "test_method": "ParseAsync_failover_regions_maps_tf_failover_regions_property",
        "summary": "terraform-show-json failover_regions property gap",
    },
}
...
def add_indexed_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.locations"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "indexed", "indexed"), encoding="utf-8")


def add_migrated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.indexed"] = indexed.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "migrated", "migrated"), encoding="utf-8")


def add_protocol_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.protocol"]' in text:
        return
    old = """            properties["tf.migrated"] = migrated.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text: raise RuntimeError("migrated anchor missing")
    RESOURCE_MAPPING.write_text(text.replace(old,new,1),encoding="utf-8")


def add_failover_regions_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.protocol"] = protocolText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "failover_regions", "failoverRegions", "failover_regions"),encoding="utf-8")


def write_indexed_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonIndexedTests.cs"
    if path.exists():
        return path
    path.write_text(
...
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonIndexedTests
{
    ...
    [Fact]
    public async Task ParseAsync_indexed_maps_tf_indexed_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "indexed.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","indexed":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.indexed"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_migrated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonMigratedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonMigratedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_migrated_maps_tf_migrated_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "migrated.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","migrated":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.migrated"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_protocol_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonProtocolTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonProtocolTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_protocol_maps_tf_protocol_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "protocol.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","protocol":"https","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.protocol"].Should().Be("https");
    }
}
''',
        encoding="utf-8",
...
def write_failover_regions_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonFailoverRegionsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonFailoverRegionsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_failover_regions_maps_tf_failover_regions_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "failover_regions.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","failover_regions":["westus"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.failover_regions"].Should().Be("westus");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 6857:
        add_indexed_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_indexed_test().relative_to(REPO))]
    if hunt == 6882:
        add_migrated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_migrated_test().relative_to(REPO))]
    if hunt == 6907:
        add_protocol_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_protocol_test().relative_to(REPO))]
    if hunt == 6932:
        add_failover_regions_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_failover_regions_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "6856"))
    end_hunt = int(os.environ.get("END_HUNT", "6955"))
