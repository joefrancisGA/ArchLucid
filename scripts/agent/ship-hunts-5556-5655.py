#!/usr/bin/env python3
"""Ship /al-bug hunts #5556-#5655 (batch 59, 100 checkins).
...
_TOPOLOGY_TOKENS = [
    "saf", "sbg", "sch", "sdi", "sej", "sfk", "sgl", "shm", "sin", "sjo",
    "skp", "slq", "smr", "sns", "sot", "spu", "sqv", "srw", "ssx", "sty",
]
TOPOLOGY = {
    5556 + index * 5: (
        token,
...
CONTEXT_META = {
    "validated": {
        "ledger_prop": "validated",
        "commit_prop": "validated",
        "test_class": "TerraformShowJsonValidatedTests",
        "test_method": "ParseAsync_validated_maps_tf_validated_property",
        "summary": "terraform-show-json validated property gap",
    },
    "canceled": {
        "ledger_prop": "canceled",
        "commit_prop": "canceled",
        "test_class": "TerraformShowJsonCanceledTests",
        "test_method": "ParseAsync_canceled_maps_tf_canceled_property",
        "summary": "terraform-show-json canceled property gap",
    },
    "notes": {
        "ledger_prop": "notes",
        "commit_prop": "notes",
        "test_class": "TerraformShowJsonNotesTests",
        "test_method": "ParseAsync_notes_maps_tf_notes_property",
        "summary": "terraform-show-json notes property gap",
    },
    "subnets": {
        "ledger_prop": "subnets",
        "commit_prop": "subnets",
        "test_class": "TerraformShowJsonSubnetsTests",
        "test_method": "ParseAsync_subnets_maps_tf_subnets_property",
        "summary": "terraform-show-json subnets property gap",
    },
}
...
def add_validated_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.endpoints"] = joined.Length > 2000 ? joined[..2000] : joined;
            }
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "validated", "validated"), encoding="utf-8")


def add_canceled_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """            properties["tf.validated"] = validated.GetBoolean() ? "true" : "false";
        }"""
    RESOURCE_MAPPING.write_text(_append_bool_tf_property(text, anchor, "canceled", "canceled"), encoding="utf-8")


def add_notes_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    if 'properties["tf.notes"]' in text:
        return
    old = """            properties["tf.canceled"] = canceled.GetBoolean() ? "true" : "false";
        }"""
    new = old + """
    ...
    if old not in text:
        raise RuntimeError("canceled anchor not found for notes")
    RESOURCE_MAPPING.write_text(text.replace(old, new, 1), encoding="utf-8")


def add_subnets_fix() -> None:
    text = RESOURCE_MAPPING.read_text(encoding="utf-8")
    anchor = """                properties["tf.notes"] = notesText.Trim();
        }"""
    RESOURCE_MAPPING.write_text(_append_array_tf_property(text, anchor, "subnets", "subnets", "subnets"), encoding="utf-8")
...
def write_validated_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonValidatedTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonValidatedTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_validated_maps_tf_validated_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "validated.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","validated":true,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.validated"].Should().Be("true");
    }
}
''',
        encoding="utf-8",
...
def write_canceled_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonCanceledTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonCanceledTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_canceled_maps_tf_canceled_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "canceled.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","canceled":false,"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.canceled"].Should().Be("false");
    }
}
''',
        encoding="utf-8",
...
def write_notes_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonNotesTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonNotesTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_notes_maps_tf_notes_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "notes.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","notes":"review pending","mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.notes"].Should().Be("review pending");
    }
}
''',
        encoding="utf-8",
...
def write_subnets_test() -> Path:
    path = CONTEXT_TEST_DIR / "TerraformShowJsonSubnetsTests.cs"
    if path.exists():
        return path
    path.write_text(
        '''using ArchLucid.ContextIngestion.Infrastructure;
...
namespace ArchLucid.ContextIngestion.Tests;
[Trait("Category", "Unit")]
public sealed class TerraformShowJsonSubnetsTests
{
    private readonly TerraformShowJsonInfrastructureDeclarationParser _sut = new(NullLogger<TerraformShowJsonInfrastructureDeclarationParser>.Instance);
    [Fact]
    public async Task ParseAsync_subnets_maps_tf_subnets_property()
    {
        InfrastructureDeclarationReference decl = new()
        {
            Name = "subnets.json", Format = "terraform-show-json", DeclarationId = "d1",
            Content = """{"values":{"root_module":{"resources":[{"type":"azurerm_resource_group","name":"main","subnets":["subnet-a"],"mode":"managed","provider_name":"registry.terraform.io/hashicorp/azurerm","values":{"name":"rg-a"}}]}}}"""
        };
        IReadOnlyList<CanonicalObject> objects = await _sut.ParseAsync(decl, CancellationToken.None);
        objects.Should().ContainSingle();
        objects[0].Properties["tf.subnets"].Should().Be("subnet-a");
    }
}
''',
        encoding="utf-8",
...
def apply_hunt_code(hunt: int) -> list[str]:
    ...
    if hunt == 5557:
        add_validated_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_validated_test().relative_to(REPO))]
    if hunt == 5582:
        add_canceled_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_canceled_test().relative_to(REPO))]
    if hunt == 5607:
        add_notes_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_notes_test().relative_to(REPO))]
    if hunt == 5632:
        add_subnets_fix()
        return [str(RESOURCE_MAPPING.relative_to(REPO)), str(write_subnets_test().relative_to(REPO))]
    return []
...
def main() -> None:
    start_hunt = int(os.environ.get("START_HUNT", "5556"))
    end_hunt = int(os.environ.get("END_HUNT", "5655"))
