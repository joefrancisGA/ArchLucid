...
# Run: Invoke-Pester -Strict 'scripts/tests/Get-ArchLucidAwsPackage.Tests.ps1'
Set-StrictMode -Version Latest

Describe 'Get-ArchLucidAwsPackage.ps1' {

    BeforeAll {
        [string]$script:repoRoot = Split-Path -Parent $PSScriptRoot
        [string]$script:extractorScript = Join-Path $script:repoRoot 'Get-ArchLucidAwsPackage.ps1'
    }

    It 'writes a schema-version-1 ZIP with manifest.json and resources.json in DryRun mode' {
        [string]$zipPath = Join-Path $TestDrive 'aws-inventory.zip'
        ...
        try {
            ...
            try {
                ...
                [string]$manifestJson.cloudProvider | Should -Be 'Aws'
            }
            finally {
                $reader.Dispose()
