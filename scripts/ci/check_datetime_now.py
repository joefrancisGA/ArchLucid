#!/usr/bin/env python3
"""Diff-scoped guard: ban DateTime.Now / DateTime.Today in changed product C# (prefer UtcNow)."""
...
DATETIME_LOCAL = re.compile(r"\bDateTime\.(?:Now|Today)\b")
...
def scan_source(source: str) -> list[int]:
    ...
    for index, line in enumerate(lines):
        ...
        if DATETIME_LOCAL.search(candidate):
...
def scan_file(path: Path) -> list[str]:
    ...
    return [
        f"{path.as_posix()}:{line_no}: use DateTime.UtcNow or an injectable clock instead of DateTime.Now/Today"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if diff_range is None:
        if ci_range_hint and should_skip_push_range(ci_range_hint):
            print("Skipping DateTime.Now guard: GitHub push with no meaningful before SHA.")
            return 0

        print(
            "Skipping DateTime.Now guard: set --diff-range or ARCHLUCID_GIT_DIFF_RANGE.",
        )
        return 0
    ...
    if not violations:
        print("DateTime.Now guard OK for changed product .cs files.")
        return 0

    print("DateTime.Now guard failed:", file=sys.stderr)
