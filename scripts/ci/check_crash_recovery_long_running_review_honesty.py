#!/usr/bin/env python3
"""TB-1524 / M-277: Anti-Worker-resumes-agent-execute / auto-orphan-fail / exactly-once-LLM honesty CI."""
...
ALLOWLIST_MARKER = "crash-recovery-long-running-review-honesty: allow"

CONTRACT_REL = Path("docs/library/CRASH_RECOVERY_LONG_RUNNING_REVIEW_CLAIM_MAP.md")
...
AUTHORITY_PIPELINE_REL = Path("ArchLucid.Host.Core/Hosted/AuthorityPipelineWorkHostedService.cs")
...
CLAIM_PATTERNS: tuple[ClaimPattern, ...] = (
    ClaimPattern(
        re.compile(
            r"\bworker\b[^.\n]{0,60}\b(?:resumes?|auto[-\s]?resumes?)\b[^.\n]{0,60}\b"
            r"(?:agent\s+)?execute\b",
            re.IGNORECASE,
        ),
        "Worker resumes authority outbox — not sync agent execute on API (TB-1523).",
    ),
    ClaimPattern(
        re.compile(
            r"\borphaned\b[^.\n]{0,40}\bagent\s*tasks?\b[^.\n]{0,60}\b(?:are\s+)?"
            r"(?:auto[-\s]?)?(?:detected|failed|reconciled)\b",
            re.IGNORECASE,
        ),
        "Orphaned AgentTasks are not auto-reconciled — TB-943 open (TB-1523).",
    ),
    ClaimPattern(
        re.compile(
            r"\b(?:process|crash|kill)\b[^.\n]{0,60}\b(?:always|automatically)\b[^.\n]{0,40}\b"
            r"(?:surfaces?|shows?|marks?)\b[^.\n]{0,40}\b(?:failed|partial)",
            re.IGNORECASE,
        ),
        "Honest Failed/Partial only when execute completion handlers run (TB-937).",
    ),
    ClaimPattern(
        re.compile(
            r"\bexactly[-\s]once\b[^.\n]{0,40}\b(?:llm|agent|execute)\b",
            re.IGNORECASE,
        ),
        "Process skip only after persist — provider at-least-once (TB-1270 / M-170).",
    ),
    ClaimPattern(
        re.compile(
            r"\bno\s+rebill\b[^.\n]{0,40}\b(?:on\s+)?(?:kill|crash|interrupt)",
            re.IGNORECASE,
        ),
        "Crash before AgentResult persist can rebill on retry (TB-1270).",
    ),
)
...
def contract_violations(root: Path) -> list[str]:
    ...
    if not path.is_file():
        return [f"{CONTRACT_REL.as_posix()}: missing crash-recovery claim map (TB-1523)."]
    text = path.read_text(encoding="utf-8", errors="replace")
    violations: list[str] = []
    for marker in _missing_markers(text, REQUIRED_CONTRACT_MARKERS):
        violations.append(
            f"{CONTRACT_REL.as_posix()}: missing required honesty anchor {marker!r} (TB-1524)."
        )
    return violations
...
def code_anchor_violations(root: Path) -> list[str]:
    ...
    for rel, needles in (
        ...
        (AUTHORITY_PIPELINE_REL, ("AuthorityPipelineWorkHostedService",)),
    ):
        path = root / rel
        if not path.is_file():
            violations.append(f"{rel.as_posix()}: missing crash-recovery code anchor (TB-1524).")
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        for needle in needles:
            if needle not in text:
                violations.append(f"{rel.as_posix()}: expected {needle!r} anchor (TB-1524).")
    return violations
...
def scan_doc_claims(root: Path, rel: Path) -> list[str]:
    path = root / rel
    if not path.is_file():
        return [f"{rel.as_posix()}: missing crash-recovery honesty scan target."]
    text = path.read_text(encoding="utf-8", errors="replace")
    violations: list[str] = []
...
def crash_recovery_long_running_review_honesty_violations(root: Path) -> list[str]:
    violations: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    violations = crash_recovery_long_running_review_honesty_violations(REPO_ROOT)
    if violations:
        label = "warnings" if args.advisory else "errors"
        print(f"Crash-recovery long-running review honesty guard: FAIL ({label})", file=sys.stderr)
        for violation in violations:
            print(f"  - {violation}", file=sys.stderr)
        return 0 if args.advisory else 1
    print("Crash-recovery long-running review honesty guard: PASS")
    return 0
