...
class TestAdoptionBatch5AE(unittest.TestCase):
    ...
    def test_tb_217_startup_work_skips_when_anonymous_viewer_disabled(self) -> None:

        path = REPO_ROOT / "ArchLucid.Api" / "Hosting" / "DemoSeedStartupWork.cs"

        text = path.read_text(encoding="utf-8")

        self.assertIn("AnonymousViewer disabled", text)

        self.assertIn("Demo seed applied on startup", text)

        self.assertIn("IDemoSeedService", text)

        self.assertIn("Demo seed failed on startup", text)
    ...
    def test_tb_217_unit_tests_cover_startup_seed(self) -> None:
        ...
        self.assertIn("when_anonymous_viewer_disabled_skips_seed", text)

        self.assertIn("when_anonymous_viewer_enabled_calls_seed", text)
