...
NULL_EQUALITY = re.compile(r"(?<![\w\.])==\s*null\b|(?<![\w\.])!=\s*null\b")
...
def scan_source(source: str) -> list[int]:
    ...
    for index, line in enumerate(lines):
        ...
        if NULL_EQUALITY.search(candidate):
...
def scan_file(path: Path) -> list[str]:
    ...
    return [
        f"{path.as_posix()}:{line_no}: use `is null` / `is not null` instead of == null / != null"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if diff_range is None:
        if ci_range_hint and should_skip_push_range(ci_range_hint):
            print("Skipping C# is-null guard: GitHub push with no meaningful before SHA.")
            return 0

        print(
            "Skipping C# is-null guard: set --diff-range or ARCHLUCID_GIT_DIFF_RANGE.",
        )
        return 0
    ...
    if not violations:
        print("C# is-null guard OK for changed .cs files.")
        return 0

    print("C# is-null guard failed:", file=sys.stderr)
