#!/usr/bin/env python3
"""TB-1002 / M-152: Anti-per-tenant-Search-index / filter-optional honesty CI."""
...
ALLOWLIST_MARKER = "retrieval-tenancy-hit-guarantee-honesty: allow"

CONTRACT_REL = Path("docs/library/RETRIEVAL_TENANCY_HIT_GUARANTEE_CONTRACT.md")
PA_ONE_PAGER_REL = Path("docs/go-to-market/RETRIEVAL_TENANCY_HIT_GUARANTEE_PA_ONE_PAGER.md")
...
CLAIM_PATTERNS: tuple[ClaimPattern, ...] = (
    ClaimPattern(
        re.compile(
            r"\b(?:dedicated|separate|per[-\s]tenant)\b[^.\n]{0,80}\b(?:azure\s+ai\s+search|search\s+index|search\s+service)\b",
            re.IGNORECASE,
        ),
        "Do not claim a dedicated Azure AI Search index/service per tenant (TB-1001 / M-153).",
    ),
    ClaimPattern(
        re.compile(
            r"\b(?:each|every)\s+tenant\b[^.\n]{0,80}\b(?:has|gets|receives)\b[^.\n]{0,60}\b(?:its\s+own|a\s+dedicated)\b[^.\n]{0,40}\bsearch\b",
            re.IGNORECASE,
        ),
        "Shared index + mandatory OData filter — not per-tenant Search isolation (TB-1001).",
    ),
    ClaimPattern(
        re.compile(
            r"\b(?:tenant|odata)\s*\$?filter\b[^.\n]{0,60}\b(?:is\s+)?(?:optional|best[-\s]effort|nice[-\s]to[-\s]have)\b",
            re.IGNORECASE,
        ),
        "Production Ask/Search requires mandatory scope `$filter` (TB-1001 / TB-071).",
    ),
    ClaimPattern(
        re.compile(
            r"\b(?:without|even\s+without)\b[^.\n]{0,40}\b(?:a\s+)?(?:tenant|scope)\s*\$?filter\b[^.\n]{0,80}\b(?:impossible|cannot|can't)\b[^.\n]{0,40}\b(?:another|other)\s+tenant",
            re.IGNORECASE,
        ),
        "Hit guarantee depends on required scope filter — not filterless impossibility (TB-1001).",
    ),
    ClaimPattern(
        re.compile(
            r"\b(?:cryptograph(?:ic|y)|crypto)[-\s]proof\b[^.\n]{0,80}\b(?:search|retrieval|ask|rag)\b",
            re.IGNORECASE,
        ),
        "Search hits are not cryptographic tenancy proof (TB-1001).",
    ),
    ClaimPattern(
        re.compile(
            r"\bgraph[-\s]rag\b[^.\n]{0,80}\b(?:fetch|retrieve|load)\b[^.\n]{0,60}\b(?:any|arbitrary)\b[^.\n]{0,40}\b(?:index|document)",
            re.IGNORECASE,
        ),
        "Graph-RAG expand uses scoped snapshot loads — not unscoped index fetch (TB-1001).",
    ),
)
...
def contract_violations(root: Path) -> list[str]:
    ...
    if not path.is_file():
        return [f"{CONTRACT_REL.as_posix()}: missing retrieval tenancy contract (TB-1001)."]
    ...
    for marker in _missing_markers(text, REQUIRED_CONTRACT_MARKERS):
        violations.append(
            f"{CONTRACT_REL.as_posix()}: missing required honesty anchor {marker!r} (TB-1002)."
        )
...
def search_client_anchor_violations(root: Path) -> list[str]:
    ...
    if not path.is_file():
        return [f"{SEARCH_CLIENT_REL.as_posix()}: missing AzureSearchSdkClient anchor (TB-1002)."]

    text = path.read_text(encoding="utf-8", errors="replace")

    if "BuildRequiredScopeFilter" not in text:
        return [
            f"{SEARCH_CLIENT_REL.as_posix()}: expected BuildRequiredScopeFilter call anchor (TB-1002)."
        ]
...
def scan_doc_claims(root: Path, rel: Path) -> list[str]:
    ...
    if not path.is_file():
        return [f"{rel.as_posix()}: missing retrieval tenancy honesty scan target."]
...
def retrieval_tenancy_hit_guarantee_honesty_violations(root: Path) -> list[str]:
    violations: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    violations = retrieval_tenancy_hit_guarantee_honesty_violations(REPO_ROOT)

    if violations:
        label = "warnings" if args.advisory else "errors"
        print(f"Retrieval tenancy hit guarantee honesty guard: FAIL ({label})", file=sys.stderr)
    ...
    print("Retrieval tenancy hit guarantee honesty guard: PASS")
    return 0
