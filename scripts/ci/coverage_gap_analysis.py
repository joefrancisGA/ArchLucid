...
def main() -> int:
    ...
    out_lines: list[str] = [
        ...
        "",
        "To verify **CI parity**, run **`assert_merged_line_coverage_min.py`** on merged **`Cobertura.xml`** with **`76`**, **`--min-branch-pct 61`**, **`--min-package-line-pct 81`** (same as CI; include the workflow **`--skip-package-line-gate`** list). For the **strict-profile / V1.1** dry run, use **`95`** instead of **`76`** and **`assert_coverage_floor_ratchet.py`**.",
        "",
