...
function Add-ProfileEvidenceRows {
    ...
    if ([string]$Metrics.liveEvidenceProfile -eq 'full-pipeline') {
        ...
        if ($claims -gt 0) {
            Add-EvidenceRow $List "$ProfileLabel structural smoke" "Passed" "topology-only: claims=$claims (merge not required)"
        }
        else {
            Add-EvidenceRow $List "$ProfileLabel structural smoke" "Failed" "topology-only: claims=$claims"
        }
    }
...
if ($credsPresent) {
    ...
    dotnet test .\ArchLucid.AgentRuntime.Tests\ArchLucid.AgentRuntime.Tests.csproj `
        --filter "FullyQualifiedName~Live_topology_agent_only_produces_valid_agent_result"
    ...
    dotnet test .\ArchLucid.AgentRuntime.Tests\ArchLucid.AgentRuntime.Tests.csproj `
        --filter "FullyQualifiedName~Live_pipeline_topology_compliance_cost_merge_produces_non_empty_manifest"
