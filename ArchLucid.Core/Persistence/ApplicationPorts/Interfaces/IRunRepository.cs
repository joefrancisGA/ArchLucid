...
public interface IRunRepository
{
    ...
    /// <summary>
    ///     Returns the run with the given <paramref name="runId" /> within <paramref name="scope" />, including archived
    ///     rows, or <see langword="null" /> when not found or outside the caller's scope.
    /// </summary>
    Task<RunRecord?> GetByIdIncludingArchivedAsync(ScopeContext scope, Guid runId, CancellationToken ct);
    ...
    /// <summary>
    ///     Most recent non-archived committed run for <paramref name="projectId" /> strictly before
    ///     <paramref name="currentRunId" /> in <see cref="RunRecord.CreatedUtc" /> / run-id order (matches dashboard list
    ///     ordering).
    /// </summary>
    Task<Guid?> GetPriorCommittedRunIdBeforeCurrentAsync(
        ScopeContext scope,
        string projectId,
        Guid currentRunId,
