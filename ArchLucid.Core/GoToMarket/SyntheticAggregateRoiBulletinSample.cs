...
/// <remarks>
/// <para>
/// The quarterly aggregate bulletin is normally sourced from <see cref="IRoiBulletinAggregateReader"/> (tenant-supplied
/// baseline hours). <see cref="ArchLucid.Application.Pilots.PilotRunDeltaComputer"/> instead computes <em>per-run</em>
/// deltas (findings histogram, audit counts, LLM calls) for sponsor reports. This synthetic payload uses <strong>fixed
/// illustrative</strong> baseline-hour aggregates so procurement can preview bulletin shape before SQL-backed N≥5
/// samples exist; numbers are chosen to sit in the same order of magnitude as typical self-reported review-cycle hours
/// documented alongside the Contoso retail demo narrative in <c>docs/PILOT_ROI_MODEL.md</c> — not read from production.
/// </para>
/// </remarks>
public static class SyntheticAggregateRoiBulletinSample
{
    ...
    public static RoiBulletinAggregateReadResult ForQuarter(string quarterLabel) =>
        new(
            ...
            TenantCount: SyntheticTenantCount,
            MeanBaselineHours: MeanBaselineHours,
            MedianBaselineHours: MedianBaselineHours,
            P90BaselineHours: P90BaselineHours,
