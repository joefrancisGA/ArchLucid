...
public sealed class TrialLocalIdentityOptions
{
    ...
    /// <summary>RSA private key PEM path used to mint short-lived API JWTs after password validation (must pair with ArchLucidAuth public PEM validation).</summary>
    public string JwtPrivateKeyPemPath { get; set; } = string.Empty;

    /// <summary>Issuer and audience copied into minted JWTs; must match ArchLucidAuth JwtLocalIssuer / JwtLocalAudience when using local PEM validation.</summary>
