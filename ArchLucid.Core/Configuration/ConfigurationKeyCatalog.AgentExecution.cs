    ...
    private static void AddAgentExecution(List<ConfigurationKeyEntry> entries)
    {
        ...
        "PilotStrict: minimum AgentResult→evidence grounding ratio when an evidence package exists; required when Mode is PilotStrict (options validation).", ConfigKeyRequirementKind.None),
            ...
            E("ArchLucid", "ArchLucid:Agents:LlmJudge:DeploymentName", M("appsettings", "env"), "empty", "—",
                ...
                ConfigKeyRequirementKind.None),
            E("ArchLucid", "ArchLucid:Findings:SemanticSupportBand:EnableLlmJudge", M("appsettings", "env"), "false",
                "—",
                "When true, findings merge/emit may call the semantic-support LLM judge. Default false (AS-074 / TB-1228) so emit stays the quote-overlap heuristic.",
                ConfigKeyRequirementKind.None),
            E("ArchLucid", "ArchLucid:Findings:SemanticSupportBand:EnableLlmJudgeOnFinalize", M("appsettings", "env"),
                "true", "—",
                "When true, Working Career Real finalize/readiness runs the Premium semantic-support LLM judge on Unchecked decision-grade rows (ADR 0099). Simulator/Fallback ignore this flag. Warn-not-block stays.",
                ConfigKeyRequirementKind.None),
