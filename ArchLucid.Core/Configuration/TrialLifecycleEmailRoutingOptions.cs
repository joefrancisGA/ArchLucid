...
public sealed class TrialLifecycleEmailRoutingOptions
{
    ...
    /// <summary>Flat configuration key for <see cref="Owner"/> (bind with <see cref="SectionName"/> or read via <c>configuration[key]</c>).</summary>
    public const string OwnerConfigurationKey = $"{SectionName}:Owner";

    /// <summary><see cref="OwnerModes.Hosted"/> keeps <c>TrialLifecycleEmailScanHostedService</c> / jobs; <see cref="OwnerModes.LogicApp"/> skips scan enqueue (external recurrence).</summary>
    public string Owner { get; set; } = OwnerModes.Hosted;
