...
public static class IdentityClaimRoleMappingValidator
{
    ...
    public static IReadOnlyList<SamlTestConfigComponentResult> Evaluate(IdentityClaimRoleMappingDocument mapping)
    {
        ...
        try
        {
            _ = new Regex(
                regexPattern,
                RegexOptions.CultureInvariant | RegexOptions.IgnoreCase); // codeql[cs/regex-injection]: admin-configured SAML regex compiled in validation-only test path with ArgumentException guard.
