...
public sealed class RunRationale
{
    ...
    /// <summary><c>authority</c> when an authority findings snapshot exists; <c>coordinator</c> when only the architecture run aggregate applies.</summary>
    public string PipelineType { get; set; } = string.Empty;

    public string Summary { get; set; } = string.Empty;

    public List<FindingRationale> Findings { get; set; } = [];

    public List<DecisionTraceEntry> DecisionTraceEntries { get; set; } = [];
