...
/// <summary>
///     Parses normalized <c>messaging-associations.json</c> companion rows (AX-DE-13).
/// </summary>
public static class AzureInventoryMessagingAssociationParser
{
    private const int MaxIdentifierLength = 512;

    private const int MaxNameLength = 260;

    public static bool TryParse(JsonElement element, out AzureInventoryMessagingAssociationRow? row, out string? errorMessage)
    {
        row = null;
        errorMessage = null;
        ...
        {
            errorMessage = "Messaging association row must be a JSON object.";
        ...
        string? associationType = TryReadBoundedString(element, "associationType", MaxNameLength);
        ...
        row = new AzureInventoryMessagingAssociationRow
        {
            ...
            AssociationType = string.IsNullOrWhiteSpace(associationType)
                ? AzureInventoryMessagingAssociationTypes.MessagingChild
                : associationType.Trim(),
            CaptureStorageAccountId = TryReadBoundedString(element, "captureStorageAccountId", MaxIdentifierLength),
