...
{
    /// <summary>
    ///     When <c>true</c>, invokes the determinism check pipeline using
    ///     <see cref="BaselineArchitectureRunIdForDeterminism" />.
    ///     Replays may create architecture run rows (even when manifest commits are disabled); use only in controlled
    ///     environments.
    /// </summary>
    public bool InvokeLiveDeterminismCheck
    {
        get;
        init;
    }
    ...
    public string? BaselineArchitectureRunIdForDeterminism
    {
        get;
        init;
    }
