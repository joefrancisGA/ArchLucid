...
public sealed class FindingConfidenceCalculatorTests
{
    ...
    public void Calculate_when_schema_and_reference_pass_and_ratio_zero_scores_75_and_High()
    {
        ...
        r.Status.Should().Be(FindingConfidenceStatus.Computed);
    ...
    [Fact]
    public void Calculate_when_neither_flag_and_ratio_null_is_Unknown_not_zero()
    {
        FindingConfidenceCalculationResult r = _sut.Calculate(false, false, null);

        r.Status.Should().Be(FindingConfidenceStatus.Unknown);
        r.Score.Should().BeNull();
        r.Level.Should().BeNull();
    }
