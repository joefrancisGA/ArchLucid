...
[Trait("Category", "Unit")]
public sealed class SponsorRoiSavingsGaugeTelemetryTests
{
    [Fact]
    public void BuildMeasurements_platform_only_when_per_tenant_disabled()
    {
        ...
        MeasurementHasScopeTag(measurements[0], SponsorRoiSavingsGaugeTelemetry.PlatformScope).Should().BeTrue();
    }
    ...
    public void BuildMeasurements_includes_per_tenant_rows_when_enabled()
    {
        ...
        measurements.Should().Contain(m => MeasurementHasScopeTag(m, SponsorRoiSavingsGaugeTelemetry.PlatformScope));
        measurements.Should().Contain(m =>
            m.Value == 9000d
            && MeasurementHasScopeTag(m, SponsorRoiSavingsGaugeTelemetry.TenantScope)
    ...
    {
        SponsorRoiSavingsGaugeTelemetry.ToPrometheusDouble(-10m).Should().Be(0d);
    }
}
