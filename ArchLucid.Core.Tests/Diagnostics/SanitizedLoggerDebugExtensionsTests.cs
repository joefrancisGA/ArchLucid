...
public sealed class SanitizedLoggerDebugExtensionsTests
{
    ...
    [Fact]
    public void LogDebugCuratedEvidenceProposalSkipped_strips_control_chars_in_user_strings()
    {
        Mock<ILogger> mock = new();
        mock.Setup(l => l.IsEnabled(It.IsAny<LogLevel>())).Returns(true);
        ...
        mock.Object.LogDebugCuratedEvidenceProposalSkipped(new Exception("oops"), "run\nid", "agent\rtype");

        rendered.Should().NotBeNull();
        string text = rendered!;

        text.Should().Contain("run_id");
        text.Should().Contain("agent_type");
        text.Should().NotContain("\n");
        text.Should().NotContain("\r");
    }

    [Fact]
    public void LogDebugCuratedEvidenceProposalSkipped_throws_when_logger_null()
    {
        ILogger logger = null!;

        Action act = () => logger.LogDebugCuratedEvidenceProposalSkipped(new Exception(), "a", "b");
    ...
    [Fact]
    public void LogDebugAwsPricingProbeFailed_strips_control_chars_in_region_and_instance_type()
    {
        Mock<ILogger> mock = new();
        mock.Setup(l => l.IsEnabled(It.IsAny<LogLevel>())).Returns(true);
        ...
        mock.Object.LogDebugAwsPricingProbeFailed(
            new Exception("probe failed"),
            "us-east\n1",
            "m5.large\ttype");
        ...
        text.Should().Contain("AWS Price List EC2 probe failed for us-east_1/m5.large_type.");
        text.Should().NotContain("\n");
        text.Should().NotContain("\t");
    }
}
