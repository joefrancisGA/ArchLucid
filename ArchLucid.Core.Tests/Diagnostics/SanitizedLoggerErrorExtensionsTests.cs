...
public sealed class SanitizedLoggerErrorExtensionsTests
{
    ...
    [Fact]
    public void LogErrorMappedProblemDetailsException_strips_control_chars_and_passes_exception()
    {
        Mock<ILogger> mock = new();
        mock.Setup(l => l.IsEnabled(It.IsAny<LogLevel>())).Returns(true);
        ...
        InvalidOperationException ex = new("connection refused");

        mock.Object.LogErrorMappedProblemDetailsException(
            ex,
            "GET\n",
            "/v1/roi/board-pack\t",
            503,
            "https://archlucid.example.org/errors#database-unavailable",
            4060,
            "cid\t123");
        ...
        text.Should().Contain("GET_");
        text.Should().Contain("/v1/roi/board-pack_");
        text.Should().Contain("503");
        text.Should().Contain("4060");
        text.Should().NotContain("\n");
        text.Should().NotContain("\t");
        capturedEx.Should().BeSameAs(ex);
    }
