...
{
    /// <summary>
    ///     Deserializes the snapshot for <paramref name="runId" /> in <paramref name="scope" /> and returns the full mapped
    ///     graph.
    /// </summary>
    ...
    /// <param name="decisionKey">
    ///     Provenance decision node id (GUID string) or the node's
    ///     <see cref="ProvenanceNode.ReferenceId" />.
    /// </param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Filtered view model, or <see langword="null" /> when the run or decision cannot be resolved.</returns>
    Task<GraphViewModel?> GetDecisionSubgraphAsync(ScopeContext scope, Guid runId, string decisionKey,
    ...
    Task<GraphViewModel?> GetNodeNeighborhoodAsync(ScopeContext scope, Guid runId, Guid nodeId, int depth,
        CancellationToken ct);
}
