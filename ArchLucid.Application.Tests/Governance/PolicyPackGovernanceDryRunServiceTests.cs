...
public sealed class PolicyPackGovernanceDryRunServiceTests
{
    ...
    [Fact]
    public async Task EvaluateAsync_skips_severity_gate_when_metadata_minimum_severity_is_undefined_numeric()
    {
        Guid runGuid = Guid.NewGuid();
        Guid snapshotId = Guid.NewGuid();
        InMemoryRunRepository runs = new();
        await runs.SaveAsync(
            new RunRecord
            {
                ...
                ScopeProjectId = TestScope.ProjectId,
                ProjectId = "default",
                ArchitectureRequestId = "req-dry-undefined-severity",
                LegacyRunStatus = "ReadyForCommit",
                FindingsSnapshotId = snapshotId,
                CreatedUtc = TimeProvider.System.UtcNowDateTime(),
            },
        ...
        PolicyPackGovernanceDryRunResult? result = await fixture.Sut.EvaluateAsync(
            """{"metadata":{"governance.blockCommitMinimumSeverity":"99"},"complianceRuleIds":[],"complianceRuleKeys":[],"alertRuleIds":[],"compositeAlertRuleIds":[],"advisoryDefaults":{}}""",
            runGuid.ToString("N"),
            null,
            null,
            null,
        ...
        result!.GateResult.Blocked.Should().BeFalse(
            "undefined numeric minimum severity must not activate a severity gate that silently passes on Critical findings");
        result.PassedChecks.Should().Contain(check =>
            check.Contains("pre_commit_severity_gate: skipped", StringComparison.Ordinal));
    }
