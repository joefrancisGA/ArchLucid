...
public sealed class DefaultRequestContentSafetyPrecheckTests
{
    ...
    [Fact]
    public async Task EvaluateAsync_allows_long_benign_architecture_description_without_timeout()
    {
        string repeatedTier =
            "Each tier uses private endpoints, managed identity, and Key Vault references. ";
        string description =
            "Design a multi-region Azure landing zone with hub-spoke networking, Azure Firewall, "
            + "Application Gateway WAF, and SQL failover groups. "
            + string.Concat(Enumerable.Repeat(repeatedTier, 40))
            + "Document residual risks for cross-region replication and backup RPO.";

        ArchitectureRequest request = new()
        {
            RequestId = "req-safe-long-1",
            Description = description,
            SystemName = "PlatformCore",
            Environment = "prod",
            CloudProvider = CloudProvider.Azure,
        };
