...
public sealed class TenantLlmCostReportingServiceTests
{
    ...
    [SkippableFact]
    public async Task BuildDashboardAsync_daily_buckets_stay_within_current_utc_month()
    {
        Guid tenantId = Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa");
        Guid workspaceId = Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb");
        Guid projectId = Guid.Parse("cccccccc-cccc-cccc-cccc-cccccccccccc");
        ...
        TenantLlmCostReportingService sut = new(
            new FixedUtcTimeProvider(new DateTime(2026, 8, 3, 12, 0, 0, DateTimeKind.Utc)),
            scopeProvider.Object,
            budgetRepository,
            tenants.Object,
            budgetOptions.Object,
        ...
        result.Daily.Select(bucket => bucket.BucketUtc.UtcDateTime).Should().AllSatisfy(day =>
            day.Should().BeOnOrAfter(new DateTime(2026, 8, 1)));
        result.Daily.Sum(bucket => bucket.EstimatedCostUsd).Should().Be(30m);
    }

    [SkippableFact]
    public async Task BuildDashboardAsync_labels_breakdown_as_tenant_wide_estimate()
    {
        Guid tenantId = Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa");
        Guid workspaceId = Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb");
        Guid projectId = Guid.Parse("cccccccc-cccc-cccc-cccc-cccccccccccc");
