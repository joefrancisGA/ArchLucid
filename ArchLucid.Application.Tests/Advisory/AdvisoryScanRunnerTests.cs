...
public sealed class AdvisoryScanRunnerTests
{
    ...
    public async Task RunScheduleAsync_WhenNoRuns_FailsExecutionAndAdvancesSchedule()
    {
        ...
        calculator
            ...
            .Returns(TimeProvider.System.UtcNowDateTime().AddDays(1));
    ...
    public async Task RunScheduleAsync_WhenLatestRunHasGoldenManifest_PersistsDigestAndDelivers()
    {
        ...
        authority
            .Setup(x => x.ListRunsByProjectAsync(It.IsAny<ScopeContext>(), It.IsAny<string>(), 2, It.IsAny<CancellationToken>()))
            .ReturnsAsync([new RunSummaryDto { RunId = runId, CreatedUtc = TimeProvider.System.UtcNowDateTime(), ProjectId = "default" }]);
        authority
