...
[Trait("Category", "Unit")]
public sealed class AgentTopologyProposalMergeGateAutomationScheduleTests
{
    [Fact]
    public void FilterValidatedProposals_keeps_relationship_when_automation_schedule_node_has_compute_category_but_synthetic_datastore_id_used()
    {
        GraphSnapshot graph = Graph(
            ComputeNode(nodeId: "svc-1", label: "api", sourceId: "azurerm_linux_virtual_machine.main"),
            ComputeNode(nodeId: "as-1", label: "nightly", sourceId: "azurerm_automation_schedule.nightly"));

        AgentResult topology = TopologyResult(RelationshipProposal(Relationship(targetId: "ds-nightly")));
