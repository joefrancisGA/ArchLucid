...
[Trait("Category", "Unit")]
public sealed class AgentTopologyProposalMergeGateAutomationConnectionTests
{
    [Fact]
    public void FilterValidatedProposals_keeps_relationship_when_automation_connection_node_has_compute_category_but_synthetic_datastore_id_used()
    {
        GraphSnapshot graph = Graph(
            ComputeNode(nodeId: "svc-1", label: "api", sourceId: "azurerm_linux_virtual_machine.main"),
            ComputeNode(nodeId: "ac-1", label: "aws", sourceId: "azurerm_automation_connection.aws"));

        AgentResult topology = TopologyResult(RelationshipProposal(Relationship(targetId: "ds-aws")));
