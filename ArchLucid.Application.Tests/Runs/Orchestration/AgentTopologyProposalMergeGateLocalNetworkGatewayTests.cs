...
[Trait("Category", "Unit")]
public sealed class AgentTopologyProposalMergeGateLocalNetworkGatewayTests
{
    [Fact]
    public void FilterValidatedProposals_keeps_relationship_when_local_network_gateway_node_has_compute_category_but_synthetic_datastore_id_used()
    {
        GraphSnapshot graph = Graph(
            ComputeNode(nodeId: "svc-1", label: "api", sourceId: "azurerm_linux_virtual_machine.main"),
            ComputeNode(nodeId: "lng-1", label: "onprem", sourceId: "azurerm_local_network_gateway.onprem"));

        AgentResult topology = TopologyResult(RelationshipProposal(Relationship(targetId: "ds-onprem")));
