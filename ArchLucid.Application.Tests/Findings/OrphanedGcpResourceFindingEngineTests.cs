...
public sealed class OrphanedGcpResourceFindingEngineTests
{
    ...
    public async Task AnalyzeAsync_returns_orphan_findings_from_resources_json()
    {
        ...
        (OrphanedGcpResourceFindingEngine sut, FindingAnalysisContext context) = CreateSut(CreatePackage(resourcesJson));
    ...
    private static (OrphanedGcpResourceFindingEngine Engine, FindingAnalysisContext Context) CreateSut(
        CloudInventoryExtractorPackageDownloadRecord package)
    {
        ...
        OrphanedGcpResourceFindingEngine engine = new(
