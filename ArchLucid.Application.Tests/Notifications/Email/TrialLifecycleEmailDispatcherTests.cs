...
public sealed class TrialLifecycleEmailDispatcherTests
{
    ...
    [Fact]
    public async Task DispatchAsync_sends_welcome_when_trial_status_is_lowercase_active()
    {
        ...
        provider.Verify(
            p => p.SendAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()),
            Times.Once,
            "lowercase active trial status must not suppress trial lifecycle mail after Core tenancy parity fixes");
    }
    ...
    [Fact]
    public async Task DispatchAsync_sends_converted_email_when_trial_status_is_lowercase_converted()
    {
        ...
        provider.Verify(
            p => p.SendAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()),
            Times.Once,
            "lowercase converted trial status must not suppress conversion mail");
    }
}
