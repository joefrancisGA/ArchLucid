...
[Trait("Suite", "Core")]
[Trait("Category", "Unit")]
public sealed class SponsorRoiBackgroundScopeGuardTests
{
    [Fact]
    public void TryValidate_accepts_explicit_tenant_workspace_project()
    {
        ...
        SponsorRoiBackgroundScopeGuard.TryValidate(scope, out string reason).Should().BeTrue();
    ...
    public void TryValidate_rejects_dev_default_scope_triple()
    {
        ...
        SponsorRoiBackgroundScopeGuard.TryValidate(scope, out string reason).Should().BeFalse();
...
[Trait("Suite", "Core")]
[Trait("Category", "Unit")]
public sealed class SponsorRoiBackgroundTenantRollupIsolationTests
{
    ...
    public async Task ForEachActiveTenantAsync_isolates_tenant_rollups_and_rejects_default_scope()
    {
        ...
        findingsSnapshots
            ...
            .ReturnsAsync((ScopeContext _, Guid snapshotId, CancellationToken _) =>
            {
                ...
                return SponsorRoiSummaryServiceTestSupport.CreateOpenFindingSnapshot(savings);
            });
        ...
        int processed = await SponsorRoiBackgroundTenantRollup.ForEachActiveTenantAsync(
            tenantRepository.Object,
            async (tenantScope, ct) =>
            {
                ...
                if (!SponsorRoiBackgroundScopeGuard.TryValidate(ambient, out _))
                {
                    invalidScopeCallbacks++;
                    return;
                }

                SponsorRoiSummaryResponse summary = await roiService.BuildAsync(ct);
    ...
    ///     pushed by <see cref="SponsorRoiBackgroundTenantRollup.ForEachActiveTenantAsync"/>, so the
    ///     resolver runs under the same per-tenant scope as the rest of the ROI service under test.
    /// </summary>
    private static SponsorRoiTenantPricingContextResolver CreateAmbientPricingContextResolver()
    {
