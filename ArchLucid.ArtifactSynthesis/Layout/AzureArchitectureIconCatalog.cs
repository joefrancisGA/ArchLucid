...
public sealed class AzureArchitectureIconCatalog
{
    ...
    private readonly IReadOnlyList<AzureArchitectureIconCatalogEntry> _entries;
    ...
    public static AzureArchitectureIconCatalog Load(Assembly? assembly = null)
    {
        ...
        AzureArchitectureIconManifest? manifest =
            JsonSerializer.Deserialize<AzureArchitectureIconManifest>(
                manifestReader.ReadToEnd(),
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

        if (manifest is null || manifest.Icons is null)
        {
        ...
        foreach (AzureArchitectureIconManifestEntry item in manifest.Icons)
        {
            ...
            string resourceName = IconResourcePrefix + item.File;
            using Stream iconStream = sourceAssembly.GetManifestResourceStream(resourceName)
                ?? throw new InvalidOperationException(
                    $"Embedded Azure icon was not found: {item.File} ({resourceName})");
            using MemoryStream buffer = new();
            iconStream.CopyTo(buffer);
            string dataUri = $"data:image/png;base64,{Convert.ToBase64String(buffer.ToArray())}";
