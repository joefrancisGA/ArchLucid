...
/// plus an audit-friendly trace of <em>which</em> pack won for each rule ID / key / metadata entry and <em>why</em>.
/// </para>
/// <para>
...
public interface IEffectiveGovernanceResolver
{
    ...
    /// <returns>
    /// A populated <see cref="EffectiveGovernanceResolutionResult"/>; <see cref="EffectiveGovernanceResolutionResult.EffectiveContent"/>
    /// may be empty if no packs apply or all content JSON is invalid. <see cref="EffectiveGovernanceResolutionResult.Notes"/> always include summary counts.
    /// </returns>
    /// <remarks>
    /// Delegates to <see cref="IPolicyPackAssignmentRepository.ListByScopeAsync"/> (repository applies hierarchical scope matching),
    /// then applies <see cref="EffectiveGovernanceResolver"/> filtering and merge rules. Malformed <c>ContentJson</c> or missing
