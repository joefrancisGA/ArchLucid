...
/// <summary>
///     Tenant-scoped CRON-style definition for recurring advisory scans: which authority project slug to read and when the
///     next run is due.
/// </summary>
/// <remarks>
///     Persisted in <c>dbo.AdvisoryScanSchedules</c> (SQL) or in-memory for tests. API requests set scope ids from the
///     caller; <see cref="NextRunUtc" /> is computed at create/update via <see cref="IScanScheduleCalculator" />.
///     Enabled schedules with non-null <see cref="NextRunUtc" /> ≤ “now” are returned by schedule repositories’ due
///     queries for the hosted poller.
/// </remarks>
public class AdvisoryScanSchedule
{
    ...
    /// <summary>
    ///     Cadence expression interpreted by <see cref="IScanScheduleCalculator" /> (v1 supports a small preset set plus daily
    ///     07:00 UTC).
    /// </summary>
    ...
    /// <summary>
    ///     UTC time when the next poll should consider this schedule due; <see langword="null" /> excludes it from due
    ///     lists.
    /// </summary>
