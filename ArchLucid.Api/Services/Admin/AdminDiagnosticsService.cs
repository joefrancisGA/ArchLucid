...
public sealed class AdminDiagnosticsService(
    ...
    DataConsistencyRemediationExecutor remediationExecutor)
    : IAdminDiagnosticsService
{
    ...
    private readonly DataConsistencyRemediationExecutor _remediationExecutor =
        remediationExecutor ?? throw new ArgumentNullException(nameof(remediationExecutor));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        DataConsistencyRemediationOutcome outcome = await _remediationExecutor.ExecuteAsync(
            DataConsistencyRemediationDefinitions.OrphanComparisonRecords,
            dryRun,
            maxRows,
            cancellationToken);

        return new OrphanComparisonRemediationResult(outcome.DryRun, outcome.RowCount, outcome.DeletedIds);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new OrphanGoldenManifestRemediationResult(outcome.DryRun, outcome.RowCount, outcome.DeletedIds);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        DataConsistencyRemediationOutcome outcome = await _remediationExecutor.ExecuteAsync(
            DataConsistencyRemediationDefinitions.OrphanFindingsSnapshots,
            dryRun,
            maxRows,
            cancellationToken);

        return new OrphanFindingsSnapshotRemediationResult(outcome.DryRun, outcome.RowCount, outcome.DeletedIds);
    }
