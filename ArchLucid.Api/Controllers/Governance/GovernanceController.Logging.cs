        ...
        await _auditService.LogAsync(
            new AuditEvent
            {
                ...
                DataJson = JsonSerializer.Serialize(new
                {
                    ...
                    manifestVersion = (request.ManifestVersion ?? string.Empty).Trim(),
                    sourceEnvironment = (request.SourceEnvironment ?? string.Empty).Trim(),
                    targetEnvironment = (request.TargetEnvironment ?? string.Empty).Trim()
                })
            },
            cancellationToken);
    }
