    ...
    ITenantRepository tenantRepository) : ControllerBase
{
    ...
    public async Task<IActionResult> GetSponsorDigestPreferences(CancellationToken cancellationToken)
    {
        ...
        return Ok(row ?? SponsorDigestPreferencesResponse.Unconfigured(scope.TenantId));
    }

    /// <summary>Upserts digest preferences (Execute+).</summary>
    // idempotency-posture: operator-documented-safe-retry
    [HttpPost("sponsor-digest-preferences")]
    [Authorize(Policy = ArchLucidPolicies.ExecuteAuthority)]
    [ProducesResponseType(typeof(SponsorDigestPreferencesResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> PostSponsorDigestPreferences(
        [FromBody] SponsorDigestPreferencesUpsertRequest? body,
