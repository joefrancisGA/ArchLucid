...
/// <summary>
/// Test double that records <see cref="IBillingLedger.ChangePlanAsync"/> / <see cref="IBillingLedger.ChangeQuantityAsync"/>
/// invocations before delegating. The SQL ledger maps those methods to Dapper calls against <c>dbo.sp_Billing_ChangePlan</c>
/// and <c>dbo.sp_Billing_ChangeQuantity</c>; this recorder is the lightweight stand-in for a Dapper command interceptor when
/// wiring full SQL hosts in <c>WebApplicationFactory</c> tests.
/// </summary>
public sealed class BillingLedgerDapperDispatchRecorder(IBillingLedger inner, ConcurrentBag<string> recordedLogicalProcedures)
    : IBillingLedger
{
    ...
    public Task<bool> TenantHasActiveSubscriptionAsync(Guid tenantId, CancellationToken cancellationToken) =>
        _inner.TenantHasActiveSubscriptionAsync(tenantId, cancellationToken);
    ...
    public Task UpsertPendingCheckoutAsync(
        ...
        _inner.UpsertPendingCheckoutAsync(
            tenantId,
            workspaceId,
            projectId,
            provider,
    ...
    public Task<bool> TryInsertWebhookEventAsync(
        ...
        string payloadJson,
        CancellationToken cancellationToken) =>
        _inner.TryInsertWebhookEventAsync(dedupeKey, provider, eventType, payloadJson, cancellationToken);

    public Task MarkWebhookProcessedAsync(string dedupeKey, string resultStatus, CancellationToken cancellationToken) =>
        _inner.MarkWebhookProcessedAsync(dedupeKey, resultStatus, cancellationToken);

    public Task<string?> GetWebhookEventResultStatusAsync(string dedupeKey, CancellationToken cancellationToken) =>
        _inner.GetWebhookEventResultStatusAsync(dedupeKey, cancellationToken);
    ...
    public Task ActivateSubscriptionAsync(
        ...
        CancellationToken cancellationToken) =>
        _inner.ActivateSubscriptionAsync(
            tenantId,
            workspaceId,
            projectId,
            provider,
    ...
    public Task SuspendSubscriptionAsync(Guid tenantId, CancellationToken cancellationToken) =>
        _inner.SuspendSubscriptionAsync(tenantId, cancellationToken);

    public Task ReinstateSubscriptionAsync(Guid tenantId, CancellationToken cancellationToken) =>
        _inner.ReinstateSubscriptionAsync(tenantId, cancellationToken);

    public Task CancelSubscriptionAsync(Guid tenantId, CancellationToken cancellationToken) =>
        _inner.CancelSubscriptionAsync(tenantId, cancellationToken);
